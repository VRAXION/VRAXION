# STABLE_LOOP_PHASE_LOCK_078_CHAT_COMPOSITION_REPAIR Contract

Status: contract for bounded runner-local chat composition repair.
...
The runner warm-starts conceptually from the 076 chat PoC checkpoint but keeps
that upstream checkpoint read-only. It trains a new experimental checkpoint
under `target/` only.
...
Main prediction must not use direct response-table lookup:
...
Token objective proof must record:
...
and refreshed during major phases. This keeps the run non-black-box.
...
fresh_context_carry_accuracy >= 0.70
slot_binding_accuracy >= 0.70
two_turn_dialogue_accuracy >= 0.70
boundary_refusal_accuracy >= 0.70
novel_response_rate >= 0.60
template_copy_rate <= 0.30
response_table_copy_rate <= 0.30
exact_train_response_copy_rate <= 0.20
...
```text
delta_vs_response_table_only_control > 0.20 on novel_response_rate
delta_vs_no_context_slot_control > 0.20 on context_carry_accuracy
delta_vs_no_dropout_control > 0.05 on template_copy_rate reduction
```
...
Context slot report must include:
