...
```text
These cards describe the current scoped visible calculation-trace pocket family.
They are not open-domain reasoning skills and not Core / TrueGolden claims.
```
...
commits valid traces
rejects invalid traces through the governed route
```
...
type         = seed / native trace support
```
...
status       = StableSupport
type         = notation grounding / ingress adapter
```
...
```text
Small but real notation-grounding pocket.
```
...
type         = scope guard / safety support
```
