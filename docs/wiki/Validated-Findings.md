...
| Freq+Phase: fix random init | Validated finding | Wave gating sweep: frozen random=`16.2%` > learnable=`12.9%` > none=`6.7%`. Wave gating is load-bearing (+142% vs none) but random init diversity suffices. Learnable values barely moved from init after 2000 steps (freq mean 1.23 stable, phase std 1.88->1.81). Learning wastes 2 schedule slots that theta needs more. 0 learnable bytes. | `sweep_fp_all.py` |
