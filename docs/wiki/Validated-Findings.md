...
| Tentacle I/O vs Holographic Projection | Validated finding | A/B/C sweep (H=256, IO=64, 3000 step budget with plateau detection): **Tentacle I/O** (first V neurons = input, last V = output, mask edges learn routing) peaked at `4.7%` vs holographic projection (fixed random V×H broadcast) at `1.2%` — a **3.9x improvement**. Mode C (fully random 5% prefill + BFS connectivity guarantee) beat Mode B (structured I/O seeding) and reached peak in half the steps. Key: the network learns better when I/O routing is part of the evolvable mask, not frozen in random projections. | Not promoted into `graph.py` defaults; recipe at `instnct/recipes/ab_projection_vs_tentacles.py` |
