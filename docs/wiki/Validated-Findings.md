...
| Navigable infinity principle | Validated finding | Across ALL layer optimizations, discrete/integer beats continuous/float: theta int4>`15.6%` vs float`13.5%`, rho fix>`14.5%` vs float`14.1%`, wave binary>`21.4%` vs uint8`16.2%`. Mutation-selection cannot navigate continuous space — discrete provides structural "handrails" for evolutionary search. Infinity=chaos; navigable infinity=structured chaos. | All sweep recipes |
