...
| Freq+Phase: binary wave (2-bit LUT) | Validated finding | Binary wave(2×2)=`21.4%` > uint8(256×256)=`16.2%` > learnable=`12.9%` > none=`6.7%`. 4 neuron types = 4 orthogonal temporal channels in 8 ticks (Nyquist optimal). Phase is primary (16.6% alone), freq adds +4.8%. Phase {0.32π,0.83π} beats {0,π} by +64% (H=64 test). Enables sin()-free LUT: 2-bit `wave_type` + 32-value ROM. | `ab_wave_int4_vs_int8.py`, `ab_wave_binary_split.py`, `ab_phase_optimal.py` |
| Learnable channel: 23.8% new peak | Validated finding | `channel` uint8 [0-7] per neuron = tick specialization. Cos-shaped 8×8 LUT replaces sin(tick*freq+phase)*rho. Learnable(`23.8%`) > sin binary(`21.4%`) > cos fixed(`21.6%`) > uint8(`16.2%`) > none(`6.7%`). 12→1 byte/neuron. Baked into `graph.py`. | `ab_wave_learnable_channel.py` |
| New schedule: enhance+reverse+mirror | Validated finding | Schedule `[add,enhance,reverse,mirror,flip,theta,channel,remove]` = `20.8%` (+7.3% vs old schedule 13.5%). reverse=184 accepts (top structural op), mirror=111 (negative feedback loops), enhance=127 (hub building). Reciprocal edges 4.9%→11.0%, inhibitory 8.6%→22.3%. | `ab_new_schedule.py` |
| Breed + crystallize: 24.4% new peak | Validated finding | Consensus breed from 2 trained checkpoints + crystallize pruning = `24.4%`. Beats fix schedule (`23.8%`). 2006 consensus edges (62% agreement). Crystallize removed 189/300 edges -- fewer = cleaner = better. | `breed_from_checkpoints.py` |
| Big ReLU controller: 23.6% | Validated finding | 33K param controller learns phase transitions: flip->remove->reverse(75%). Nearly matches fix schedule. Prunable to 64 hidden (9K params). | `run_big_relu_ctrl.py` |
| C edge port: 29.4x speedup | Validated finding | Pure integer C: 95K tokens/sec (vs Python 3.2K). 24KB footprint. Arduino-ready. ReLU controller int8 ported. | `instnct/edge/` |
| Navigable infinity principle | Validated finding | Across ALL layer optimizations, discrete/integer beats continuous/float: theta int4>`15.6%` vs float`13.5%`, rho fix>`14.5%` vs float`14.1%`, channel int3>`23.8%` vs float`16.2%`. Mutation-selection cannot navigate continuous space — discrete provides structural "handrails" for evolutionary search. Infinity=chaos; navigable infinity=structured chaos. | All sweep recipes |
