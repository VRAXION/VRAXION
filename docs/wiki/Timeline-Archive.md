...
| Current frontier | **Float gradient + connectome + native output = the pipeline.** Train: random float init → MSE gradient (50-100 steps) → 100%. Deploy: i8 quantize → integer-only inference. Passive connectome (relay) for inter-cluster communication. Scaling validated to 10 digits (100 pairs). Overparameterization risk identified. Generalization gap shrinks with scale. Next: benchmark i8 inference speed, test on harder tasks (language/bigram), integrate with INSTNCT library. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks), [INSTNCT Architecture](INSTNCT-Architecture) |
