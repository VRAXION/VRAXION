...
| **2026-04-10 — CIRCUIT REUSE: frozen ADD speeds up compatible tasks 3.8x** | Pre-trained ADD(a,b) frozen → train new workers on compound tasks. max(a+b,c): **3.8x speedup** (pre-trained finds 100% in 50 steps vs 190 fresh). a+b+c: 2.1x speedup. 2a+b: 1.5x speedup. BUT (a+b)>3 (binary output, incompatible format): **pre-trained is WORSE** (slower convergence). Circuit reuse only helps when output format is compatible. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — OVERNIGHT SCALING: 10 digit perfect, 20 digit gradient limit** | EXP 1 (Scaling): 5d=20/20 solved, 10d=15-20/20, 15d=10-15/20 (overparameterization HURTS: 10w=15/20 vs 20w=7/20), 20d=partial. EXP 2 (Generalization): improves with scale — 5d: 76% test, 20d: 96% test, gap shrinks from 24pp→3pp. EXP 3 (Multi-task ADD+MAX): 8 workers = 95% combined. EXP 4 (All ops 10d): ADD/MAX/MIN 100%, \|a-b\| 85%. EXP 5 (Composition depth): 2-input=100%, 3-input=high, 4-5 input partial (killed before completion, ~9.5h runtime). | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — C19 vs ReLU: C19 wins on robustness (250/250 vs 189/250)** | C19 periodic parabolic activation (rho+C learnable per neuron) vs ReLU on 5 tasks × 50 seeds. C19: 250/250 total solve rate. ReLU: 189/250 (fails on MAX 29/50, MIN 21/50, MUL 39/50). Learnable rho converges to ~5.1 (ADD/MUL) and ~4.1 (\|a-b\|), not fixed 4.0. C converges task-dependent: ADD≈5.0, MAX≈3.0, MIN≈2.5, \|a-b\|≈1.5. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — TWO-POOL CONNECTOME: C19 structural advantage PROVEN** | Two isolated neighborhoods (A sees input_a, B sees input_b), communicate ONLY through connectome. ReLU MIN: **10/50**, C19 MIN: **50/50** — 40-point gap! C19 advantage GROWS with bottleneck (single-pool gap=61, two-pool gap=80). ReLU degrades with more ticks (overprocessing), C19 stays 50/50. The periodic wave genuinely helps cross-cluster info transfer. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — BINARY+C19 EXHAUSTIVE: 3/5 tasks solved with ±1 weights** | Greedy constructive: binary ±1 weights + C sweep (exhaustive, guaranteed). ADD: **100% with 2 workers**, MAX: 100% (7w), MIN: 100% (5w). \|a-b\| and MUL stuck at 92%. ReLU binary: 0/5 solved (best=88% MUL). C19's periodic wave makes binary search space VIABLE — binary+ReLU was 0/8192 solutions. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — FRACTION EXTRACTION: float → integer via common denominator** | Float train → find denominator d (1-30) → weights = round(float×d) / d → test. **ALL 5 tasks convert to integer: ADD 20/20, MAX 20/20, MIN 20/20, \|a-b\| 20/20, MUL 18/20.** Bits per weight: ADD=4-5, MAX=4-5, MIN=5, \|a-b\|=5-6, MUL=6. No search needed — float training finds the ratios, denominator gives the integer grid. C baked as LUT. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — ITERATIVE QUANTIZATION: float→int neuron-by-neuron with retrain** | Quantize one neuron → retrain remaining float neurons to compensate → repeat. Iterative: 14-18/20 survive vs one-shot: 8-14/20. Retrain compensates quantization error in ~22 steps. Greedy freeze-per-layer tested (N=1,2,3,4): all-at-once float still best (10/10), freeze quantization is the bottleneck not the greedy strategy. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| **2026-04-10 — ANALYTIC BACKPROP: 1688x speedup, LANGUAGE prediction works** | Replaced numerical gradient (800K forward/step) with chain-rule backprop (2 pass/step). 5000 steps in 12s vs estimated 5+ hours. Softmax + cross-entropy loss. **C19 beats bigram on real FineWeb text**: test=25.0% top5=62.7% vs bigram 24.2% vs ReLU 13.9%. ReLU beragad "always space" at step 500; C19 learns actual patterns ("and "→'a', "is a"→'n'). First proof C19 works on chaotic/language data, not just arithmetic. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks) |
| Current frontier | **C19 + backprop + fraction quantization = the full pipeline.** Train: analytic backprop (C19 rho+C learnable, softmax+CE) → works on arithmetic AND language. Deploy: fraction extraction (4-6 bit integer weights) + C19 baked as LUT → pure integer chip. Validated on FineWeb real text (25% next-char accuracy, beats bigram). Next: scale up (more neurons, longer context), deeper architectures, multi-neighborhood for language. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks), [INSTNCT Architecture](INSTNCT-Architecture) |
