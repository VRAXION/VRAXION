...
| Current frontier | **Recurrent ReLU + Connection Point architecture = scaling path.** 1 neuron, 5 bits = universal ADD accumulator (native output, no readout). CP architecture: constant 3^12 search space at any network size, exhaustive per neuron. Pipeline composition for non-accumulative ops (MUL). Next: bigram language task with CP architecture. | [Rust Implementation Surface](v5-Rust-Port-Benchmarks), [INSTNCT Architecture](INSTNCT-Architecture) |
