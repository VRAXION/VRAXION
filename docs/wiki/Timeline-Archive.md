...
Preliminary investigation into an L2 layer above the L1 Huffman-packed champion: can two L1 merger outputs be further compressed with a second mirror-tied stage? Phase-0 geometry probe found 1.5% byte-exact window at a 384-dim bottleneck — well below the Phase 0 PCA baseline of 2.6% at D=128, suggesting the current training setup under-fits even the linear baseline. Findings are still preliminary; no cluster designation yet.
