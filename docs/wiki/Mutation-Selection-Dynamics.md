...
**Cross-H pilot (D1b sandbox H=128, 2026-04-25) + D1 v2.2 final (H=384)**: a complementary 18-cell parallel sweep at H=128 (mutual_inhibition, 20k steps, 3 seeds; output `output/d1_h128_quick_20260425_194650/`) was run in parallel with GPT's D1 v2.2 H=384 sweep (45/45 final, 7.8M candidate rows, output `output/phase_d1_zero_drive_20260425/`, report `docs/research/PHASE_D1_VERDICT.md`). **The two are not directly comparable** (different horizon, different n), so the H=128 side is *D1b — a pilot, not a cross-H replication*. The combined data generate three directional hypotheses worth testing under matched conditions:

- (a) **K=1 strict/ties ranking is H-dependent**: H=128 strict wins (4.33 vs 3.70); H=384 ties wins (4.02 vs 1.92).
- (b) **K=3 strict/ties ranking flips across H**: H=128 ties wins by +1.37pp; H=384 strict wins by +0.46pp.
- (c) **K=9 strict/ties ranking also flips across H** (new from D1 v2.2): H=128 ties wins by +0.50pp; H=384 strict wins by +0.20pp. This corrects the prior B.1-era "ties is universally helpful at saturated K" reading — at the matched 40k horizon, K=9 strict beats K=9 ties at H=384.

Per-H winner: H=128 (K=3, ties, 4.90%); H=384 (K=9, strict, 5.50%).

Whether `A(K, τ, s)` extends to a 4-parameter object `A(K, τ, s; H)` is a hypothesis generated by D1b + D1 v2.2 jointly, not yet established at strict Bonferroni significance. See [Constructed Computation § H-dependence of the (K, s) interaction](Constructed-Computation) for the cross-H pilot table and the explicit caveats.
