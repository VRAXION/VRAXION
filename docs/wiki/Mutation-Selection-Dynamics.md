...
**Cross-H pilot (D1b sandbox H=128, 2026-04-25)**: a complementary 18-cell parallel sweep at H=128 (mutual_inhibition, 20k steps, 3 seeds; output `output/d1_h128_quick_20260425_194650/`) was run in parallel with GPT's D1 v2.2 H=384 sweep. **The two are not directly comparable** (different horizon, different n), so this is *D1b — a pilot, not a cross-H replication*. The pilot generates two directional hypotheses worth testing under matched conditions: (a) the K=1 strict/ties ranking is H-dependent (H=128: strict wins; H=384: ties wins), and (b) the K=3 strict/ties ranking flips across H (H=128: ties wins by +1.37pp; H=384: strict wins by +0.46pp). Whether `A(K, τ, s)` extends to a 4-parameter object `A(K, τ, s; H)` is a hypothesis generated by D1b, not yet established. See [Constructed Computation § H-dependence of the (K, s) interaction (D1b sandbox H=128 pilot)](Constructed-Computation) for the cross-H pilot table and the explicit caveats.
