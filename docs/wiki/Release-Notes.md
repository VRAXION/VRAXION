...
- **Theta bit-width Pareto frontier** — binary[1,15]=10.1%/32B, ternary[1,7,15]=12.1%/64B, 5-val[1,2,4,7,13]=13.5%/96B, int4[1-15]=15.6%/128B. Float32 DOMINATED (13.5%/1024B = same as 5-val at 10x cost). Int8[1-255]=7.1% (lazy mode — 96% neurons never fire, mean=110). **Range > levels**: [1,15] 2 values > [1-7] 7 values. Int4 cap at MAX_CHARGE=15 is a feature — forces participation. Peak analysis: theta=1(65n) + theta=6(21n) + theta=10(16n). Relay/gate binary ratio 73/27 near phi^2. Testing peak-optimized [1,6,10] and [1,6,10,15].
