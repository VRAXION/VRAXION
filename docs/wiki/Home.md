...
- **Current frontier:** The **byte-level pipeline** is the active project line as of 2026-04-18. Both L0 and L1 layers are 100% lossless and have shipped deploy artifacts. The Brain-side lexical layer direction is the **Word/Subword Tokenizer V1** (Cluster 15, 2026-04-19): 52.22% compression on FineWeb-EDU 100 MB, exact lossless, subword+byte-fallback architecture under active iteration. An **L2 merger investigation** (byte-roundtrip geometry probe, commit `ed30073`) is ongoing — preliminary 1.5% byte-exact window found at 384-bottleneck (below the 2.6% PCA baseline at D=128; training setup under-fits the linear baseline), findings still accumulating. The earlier character-level abstract-core track (Beukers gate, 83.6% masked char prediction) is a **validated but archived prior track** — see "Earlier exploration" below.
