...
- **Current frontier:** the bias-free grower has frozen the B0 engine on a six-task regression matrix (mean val `88.417`, max test `100.0`, mean neurons `5.667`, max depth `6` per the golden fixture in `instnct-core/tests/fixtures/grower_regression_golden.json`), and the active promotion gate is **byte/opcode v1**: `1 byte data + 4 opcode -> 1 byte` over a frozen 8-bit-head latent + exact LUT translator. The B1 golden fixture records the direct bitbank negative control at `75.0%` and the exact translator at `100.0%` over the full `1024`-sample domain (`distinct_keys=1024`, `conflicting_keys=0`, `key_bits=61`, `total_neurons=61`). The **abstract-core pipeline** is now a named four-layer architecture with L0 FROZEN as canonical:
  - **L0 Byte Interpreter** (CANONICAL): binary {-1,+1}, 4 neurons, C19(c=5, rho=0), 36 bits total, pure POPCOUNT hardware. Exhaustive search guaranteed optimal (0.05s). 100% round-trip on 27 symbols. Winner of bitwidth sweep: 1-bit binary beats ternary (3 neurons, 43 bits) and 2-bit (2 neurons, 42 bits) on total model size and hardware simplicity. Backprop STE validated as fast alternative (0.7s vs 194s for 2-bit exhaustive).
  - **L1 Input Merger** (linear 112->96, exact 100% at 86% compression, no sigmoid).
  - **L2 Feature Extractor** (Conv1D(k=3,f=64)+MLP, 96.6% train, +33pp over previous, beats one-hot 93.8% — but overfits: test 48.5% with 474K params).
  - **L3 Brain** (not yet built — target for INSTNCT evolution on frozen L0-L2 features).
  
  Key insight: binary weights are sufficient for encoding (topology matters more than weight precision). For prediction: binary + smart topology (sparse, evolved) may work — INSTNCT's sparse edge-list format unifies with ternary-to-binary deployment. Int8 quantization is lossless across all layers (+/-0.3%). ReLU beats C19 in deep networks (70% vs 42%); C19 still wins in shallow. MLP backprop is ~18x more parameter-efficient than evolution. Next steps: BERT-style masked char pretraining for L2, INSTNCT evolution as L3 brain, larger corpus to fix overfitting. See [`docs/wiki/pipeline-architecture.svg`](pipeline-architecture.svg) for the visual pipeline diagram. Earlier spiking-network and Connection Point lines are preserved as research history, not current mainline.
