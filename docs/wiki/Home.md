...
- **Current frontier:** the bias-free grower has frozen the B0 engine on a six-task regression matrix (mean val `88.417`, max test `100.0`, mean neurons `5.667`, max depth `6` per the golden fixture in `instnct-core/tests/fixtures/grower_regression_golden.json`), and the active promotion gate is **byte/opcode v1**: `1 byte data + 4 opcode -> 1 byte` over a frozen 8-bit-head latent + exact LUT translator. The B1 golden fixture records the direct bitbank negative control at `75.0%` and the exact translator at `100.0%` over the full `1024`-sample domain (`distinct_keys=1024`, `conflicting_keys=0`, `key_bits=61`, `total_neurons=61`). The **abstract-core pipeline** is now a named four-layer architecture validated through L0-L2: **L0 Byte Interpreter** (ternary {-1,0,+1}, 3 neurons, 22 connections, 100% — new record, binary needed 4); **L1 Input Merger** (linear 112->96, exact 100% at 86% compression, no sigmoid); **L2 Feature Extractor** (Conv1D(k=3,f=64)+MLP, 96.6% train, +33pp over previous, beats one-hot 93.8% — but overfits: test 48.5% with 474K params); **L3 Brain** (not yet built — target for INSTNCT evolution on frozen L0-L2 features). Int8 quantization is lossless across all layers (+/-0.3%). Binary weights are perfect for encoding but fail for prediction. ReLU beats C19 in deep networks (70% vs 42%); C19 still wins in shallow. MLP backprop is ~18x more parameter-efficient than evolution. Next steps: BERT-style masked char pretraining for L2, INSTNCT evolution as L3 brain, larger corpus to fix overfitting. Earlier spiking-network and Connection Point lines are preserved as research history, not current mainline.
