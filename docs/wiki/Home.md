...
- **Current frontier:** the bias-free grower has frozen the B0 engine on a six-task regression matrix (mean val `88.417`, max test `100.0`, mean neurons `5.667`, max depth `6` per the golden fixture in `instnct-core/tests/fixtures/grower_regression_golden.json`), and the active promotion gate is **byte/opcode v1**: `1 byte data + 4 opcode -> 1 byte` over a frozen 8-bit-head latent + exact LUT translator. The B1 golden fixture records the direct bitbank negative control at `75.0%` and the exact translator at `100.0%` over the full `1024`-sample domain (`distinct_keys=1024`, `conflicting_keys=0`, `key_bits=61`, `total_neurons=61`). The **abstract-core preprocessor** is validated end-to-end: an all-binary {-1,+1} exhaustive-search grower with C19 activation achieves 100% lossless byte round-trip encoding in just 77 bits (10 bytes), exhaustive-search guaranteed optimal — the entire preprocessor fits in a single `u128` integer. The language model float baseline reaches 34.5% next-char accuracy (vs INSTNCT 24.6%); int8 or QAT quantization is the next step for binary deployment. Earlier spiking-network and Connection Point lines are preserved as research history, not current mainline.
