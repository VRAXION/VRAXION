...
- **Current frontier:** the bias-free grower has frozen the B0 engine on a six-task regression matrix (mean val `88.417`, max test `100.0`, mean neurons `5.667`, max depth `6` per the golden fixture in `instnct-core/tests/fixtures/grower_regression_golden.json`), and the active promotion gate is **byte/opcode v1**: `1 byte data + 4 opcode -> 1 byte` over a frozen 8-bit-head latent + exact LUT translator. The B1 golden fixture records the direct bitbank negative control at `75.0%` and the exact translator at `100.0%` over the full `1024`-sample domain (`distinct_keys=1024`, `conflicting_keys=0`, `key_bits=61`, `total_neurons=61`). The **abstract-core pipeline** is now a named four-layer architecture with L0 FROZEN as canonical:
  - **L0 Byte Interpreter** (LOCKED): flat 8->4 neurons, binary {-1,+1} weights, 36 bits total. Pure integer pipeline for deployment: POPCOUNT -> int32 sum -> int8 output. NO C19 activation needed for deployment (C19 was used during search only). NO floating point. NO multiply hardware (binary weight = pass or negate). Exhaustive search guaranteed optimal (0.05s). 100% round-trip on 27 symbols. Multi-layer encoders tested and confirmed WORSE than flat (bottleneck effect). Winner of bitwidth sweep: 1-bit binary beats ternary (3 neurons, 43 bits) and 2-bit (2 neurons, 42 bits) on total model size and hardware simplicity. Backprop STE validated (matches exhaustive, 278x faster for 2-bit, but unnecessary for byte encoder where exhaustive is faster).
  - **L1 Input Merger** (NEXT TARGET: linear 112->96, exact 100% at 86% compression, no sigmoid. Design via backprop STE or INSTNCT evolution).
  - **L2 Feature Extractor** (Conv1D(k=3,f=64)+MLP, 96.6% train, +33pp over previous, beats one-hot 93.8% — but overfits: test 48.5% with 474K params. Needs work).
  - **L3 Brain** (not yet built — target for INSTNCT sparse spiking evolution on frozen L0-L2 features).
  
  Key insights: binary weights are sufficient for encoding — topology carries intelligence, not weight precision (INSTNCT philosophy validated at L0). Binary weight = pass or negate, no multiply unit needed. Sum accumulates in int32, scales back to int8 — standard HW support everywhere. Ternary training -> binary deployment via add_list + sub_list format (zero-weight edges pruned). For byte encoder: exhaustive beats backprop (0.05s vs 9s) because search space is tiny; for L1+ backprop STE is essential. INSTNCT's sparse edge-list format unifies naturally with binary deployment. Int8 quantization is lossless across all layers (+/-0.3%). ReLU beats C19 in deep networks (70% vs 42%); C19 still wins in shallow. MLP backprop is ~18x more parameter-efficient than evolution. Next steps: L1 Input Merger design, BERT-style masked char pretraining for L2, INSTNCT evolution as L3 brain, larger corpus to fix overfitting. See [`docs/wiki/pipeline-architecture.svg`](pipeline-architecture.svg) for the visual pipeline diagram. Earlier spiking-network and Connection Point lines are preserved as research history, not current mainline.
