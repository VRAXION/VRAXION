"""
Per-Neuron Tick Budget — Electron Shell Model
===============================================
Each neuron has a budget (1-15): how many ticks it's active.
Higher budget = deeper processing but weaker signal (decay cost).
Max 16 ticks, per-neuron mask: active = (budget > tick).
Bigram 2seq, charge ReLU, thresh=0.00005, run to plateau.
"""
import sys, os, time, random
import numpy as np
from multiprocessing import Pool

sys.path.insert(0, os.path.join(os.path.dirname(__file__), "model"))
from graph import SelfWiringGraph

_bp = None; _all_data = None; _seq_len = 200; _n_train = 2
_input_projection = None; _output_projection = None; _bigram = None
_max_ticks = 16
_budget = None  # per-neuron int array

def init_w(b, d, sl, nt, wi, wo, bg, mt, bud):
    global _bp, _all_data, _seq_len, _n_train, _input_projection, _output_projection, _bigram, _max_ticks, _budget
    _bp, _all_data, _seq_len, _n_train = b, d, sl, nt
    _input_projection, _output_projection, _bigram = wi, wo, bg
    _max_ticks, _budget = mt, bud

def make_bp(io_dim, seed=12345):
    rng = np.random.RandomState(seed)
    p = rng.randn(256, io_dim).astype(np.float32)
    p /= np.linalg.norm(p, axis=1, keepdims=True)
    return p

def _eval_bigram(mask, H, theta, decay, budget, seqs):
    rs, cs = np.where(mask != 0)
    sp_vals = mask[rs, cs]
    pat_norm = _bp / (np.linalg.norm(_bp, axis=1, keepdims=True) + 1e-8)
    ret = 1.0 - decay
    mt = int(budget.max()) if len(budget) else _max_ticks
    total = 0.0
    for text_bytes in seqs:
        state = np.zeros(H, dtype=np.float32)
        charge = np.zeros(H, dtype=np.float32)
        seq_score = 0.0; n = 0
        for i in range(len(text_bytes)-1):
            act = state.copy()
            for t in range(mt):
                if t == 0:
                    act = act + _bp[text_bytes[i]] @ _input_projection
                raw = np.zeros(H, dtype=np.float32)
                if len(rs):
                    np.add.at(raw, cs, act[rs] * sp_vals)
                # Budget mask: only neurons with budget > t receive signal
                active = (budget > t).astype(np.float32)
                raw *= active
                charge += raw; charge *= ret
                act = np.maximum(charge - theta, 0.0)
                charge = np.maximum(charge, 0.0)
            state = act.copy()
            out = charge @ _output_projection
            out_n = out / (np.linalg.norm(out) + 1e-8)
            sims = out_n @ pat_norm.T
            e = np.exp(sims - sims.max())
            pred = e / e.sum()
            target_dist = _bigram[text_bytes[i]]
            cos = np.dot(pred, target_dist) / (np.linalg.norm(pred) * np.linalg.norm(target_dist) + 1e-8)
            seq_score += cos
            n += 1
        total += seq_score / n if n else 0
    return total / len(seqs)

def worker_eval(args):
    mask_flat, theta, decay, budget, H, seed, proposal_type = args
    rng = random.Random(seed)
    np_rng = np.random.RandomState(seed)
    mask = mask_flat.reshape(H, H)
    new_mask = mask; new_theta = theta; new_decay = decay; new_budget = budget

    if proposal_type == 'add':
        r = rng.randint(0, H-1); c = rng.randint(0, H-1)
        if r == c or mask[r, c] != 0:
            return {'delta': -1e9, 'type': 'add'}
        val = 0.6 if rng.random() < 0.5 else -0.6
        new_mask = mask.copy(); new_mask[r, c] = val
    elif proposal_type == 'flip':
        alive = list(zip(*np.where(mask != 0)))
        if not alive:
            return {'delta': -1e9, 'type': 'flip'}
        r, c = alive[rng.randint(0, len(alive)-1)]
        new_mask = mask.copy(); new_mask[r, c] = -mask[r, c]
    elif proposal_type == 'theta':
        idx = rng.randint(0, H-1)
        new_theta = theta.copy()
        new_theta[idx] = max(0.0, min(1.0, theta[idx] + rng.uniform(-0.05, 0.05)))
    elif proposal_type == 'budget':
        idx = rng.randint(0, H-1)
        new_budget = budget.copy()
        if rng.random() < 0.5:
            new_budget[idx] = min(15, budget[idx] + 1)
        else:
            new_budget[idx] = max(1, budget[idx] - 1)

    seqs = []
    data_len = len(_all_data)
    for _ in range(_n_train):
        off = np_rng.randint(0, data_len - _seq_len)
        seqs.append(_all_data[off:off+_seq_len])

    old_score = _eval_bigram(mask, H, theta, decay, budget, seqs)
    new_score = _eval_bigram(new_mask, H, new_theta, new_decay, new_budget, seqs)

    return {'delta': new_score - old_score, 'type': proposal_type,
            'new_mask_flat': new_mask.flatten() if new_score > old_score else None,
            'new_theta': new_theta if proposal_type == 'theta' else None,
            'new_budget': new_budget if proposal_type == 'budget' else None}

def eval_accuracy_classic(mask, H, input_projection, output_projection, theta, decay, budget, text_bytes, bp):
    pat_norm = bp / (np.linalg.norm(bp, axis=1, keepdims=True) + 1e-8)
    rs, cs = np.where(mask != 0); sp_vals = mask[rs, cs]
    ret = 1.0 - decay
    mt = int(budget.max()) if len(budget) else 16
    state = np.zeros(H, dtype=np.float32); charge = np.zeros(H, dtype=np.float32)
    correct = 0; total = 0
    for i in range(len(text_bytes)-1):
        act = state.copy()
        for t in range(mt):
            if t == 0: act = act + bp[text_bytes[i]] @ input_projection
            raw = np.zeros(H, dtype=np.float32)
            if len(rs): np.add.at(raw, cs, act[rs] * sp_vals)
            active = (budget > t).astype(np.float32)
            raw *= active
            charge += raw; charge *= ret
            act = np.maximum(charge - theta, 0.0)
            charge = np.maximum(charge, 0.0)
        state = act.copy()
        out = charge @ output_projection
        out_n = out / (np.linalg.norm(out) + 1e-8)
        sims = out_n @ pat_norm.T
        if np.argmax(sims) == text_bytes[i+1]: correct += 1
        total += 1
    return correct/total if total else 0


def run_config(name, init_budget, learnable_budget,
               bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection,
               max_steps=1500, n_workers=18, threshold=0.00005):
    mask = np.zeros((H, H), dtype=np.float32)
    theta = np.full(H, 0.03, dtype=np.float32)
    decay = np.full(H, 0.15, dtype=np.float32)

    if isinstance(init_budget, int):
        budget = np.full(H, init_budget, dtype=np.int32)
    else:
        budget = init_budget.copy()

    print(f"\n{'='*70}")
    print(f"  {name} (learnable={learnable_budget})")
    print(f"  Budget: mean={budget.mean():.1f}, min={budget.min()}, max={budget.max()}")
    print(f"{'='*70}")
    sys.stdout.flush()

    if learnable_budget:
        schedule = ['add', 'add', 'add', 'flip', 'theta', 'budget']
    else:
        schedule = ['add', 'add', 'add', 'flip', 'theta', 'add']

    accepts = {'add': 0, 'flip': 0, 'theta': 0, 'budget': 0}
    acc_history = []
    t0 = time.time()

    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, 200, 2, input_projection, output_projection, bigram, 16, budget))
    try:
        for step in range(1, max_steps+1):
            ptype = schedule[(step-1) % len(schedule)]
            if ptype in ('flip', 'theta', 'budget') and np.count_nonzero(mask) == 0:
                ptype = 'add'

            mask_flat = mask.flatten()
            args = [(mask_flat, theta.copy(), decay.copy(), budget.copy(),
                     H, 22000+step*50+w, ptype) for w in range(n_workers)]
            results = pool.map(worker_eval, args)

            best_r = max(results, key=lambda x: x['delta'])
            if best_r['delta'] > threshold:
                if best_r['type'] in ('add', 'flip') and best_r['new_mask_flat'] is not None:
                    mask = best_r['new_mask_flat'].reshape(H, H)
                    accepts[best_r['type']] += 1
                elif best_r['type'] == 'theta' and best_r['new_theta'] is not None:
                    theta = best_r['new_theta']
                    accepts['theta'] += 1
                elif best_r['type'] == 'budget' and best_r['new_budget'] is not None:
                    budget = best_r['new_budget']
                    accepts['budget'] += 1

            if step % 100 == 0:
                elapsed = time.time() - t0
                edges = int(np.count_nonzero(mask))
                ea = np.mean([eval_accuracy_classic(mask, H, input_projection, output_projection, theta, decay, budget, s, bp)
                              for s in eval_seqs])
                acc_history.append((step, ea))
                quality = ea / max(edges, 1) * 100
                bm = budget.mean(); bs = budget.std()
                # Budget histogram: how many at each level
                hist = np.bincount(budget, minlength=16)[1:16]
                active_ticks = bm  # average ticks used

                print(f"  [{step:4d}] acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
                      f"A={accepts['add']}|F={accepts['flip']}|T={accepts['theta']}|B={accepts['budget']} "
                      f"budget={bm:.1f}+/-{bs:.1f} [{budget.min()}-{budget.max()}] {elapsed:.0f}s")
                sys.stdout.flush()

                if len(acc_history) >= 4:
                    last4 = [a for _, a in acc_history[-4:]]
                    if max(last4) - min(last4) < 0.01:
                        print(f"  PLATEAU @ step {step}")
                        break

    finally:
        pool.terminate(); pool.join()

    edges = int(np.count_nonzero(mask))
    ea = np.mean([eval_accuracy_classic(mask, H, input_projection, output_projection, theta, decay, budget, s, bp)
                  for s in eval_seqs])
    elapsed = time.time() - t0
    quality = ea / max(edges, 1) * 100

    print(f"\n  FINAL: acc={ea*100:.2f}% edges={edges} quality={quality:.3f}%/edge")
    print(f"  Accepts: A={accepts['add']}|F={accepts['flip']}|T={accepts['theta']}|B={accepts['budget']}")
    print(f"  Budget: mean={budget.mean():.2f} std={budget.std():.2f} "
          f"min={budget.min()} max={budget.max()}")
    # Print budget distribution
    hist = np.bincount(budget, minlength=16)[1:16]
    nonzero = [(i+1, int(h)) for i, h in enumerate(hist) if h > 0]
    print(f"  Distribution: {nonzero}")
    print(f"  Time: {elapsed:.0f}s")
    sys.stdout.flush()
    return {'name': name, 'acc': ea, 'edges': edges, 'quality': quality,
            'budget_mean': float(budget.mean()), 'budget_std': float(budget.std()),
            'time': elapsed}


if __name__ == "__main__":
    IO = 256; NV = 4; H = IO * NV
    SelfWiringGraph.NV_RATIO = NV
    bp = make_bp(IO)

    DATA = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "..", "Diamond Code", "data", "traindat", "fineweb_edu.traindat")
    with open(DATA, 'rb') as f:
        ALL_DATA = np.frombuffer(f.read(), dtype=np.uint8)
    print(f"Loaded {len(ALL_DATA)/1e6:.1f} MB text")

    bigram = np.load(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                   "data", "bigram_table.npy"))

    eval_rng = np.random.RandomState(9999)
    eval_seqs = [ALL_DATA[off:off+200] for off in
                 [eval_rng.randint(0, len(ALL_DATA)-200) for _ in range(10)]]

    random.seed(42); np.random.seed(42)
    ref = SelfWiringGraph(IO)
    input_projection = ref.input_projection / ref.INJ_SCALE * 1.0
    output_projection = ref.output_projection / ref.INJ_SCALE * 1.0

    results = []

    # A: FIX 8 (current baseline)
    results.append(run_config("FIX budget=8", 8, False,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # B: FIX 4 (cheap)
    results.append(run_config("FIX budget=4", 4, False,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # C: FIX 15 (max, expensive)
    results.append(run_config("FIX budget=15", 15, False,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # D: LEARN from 8
    results.append(run_config("LEARN init=8", 8, True,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # E: LEARN from 4
    results.append(run_config("LEARN init=4", 4, True,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # F: LEARN from random 1-15
    rng_init = np.random.RandomState(123)
    random_budget = rng_init.randint(1, 16, size=H).astype(np.int32)
    results.append(run_config("LEARN init=random", random_budget, True,
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    print(f"\n{'='*75}")
    print(f"  SUMMARY -- TICK BUDGET (bigram 2seq, charge ReLU)")
    print(f"{'='*75}")
    print(f"  {'Name':<20} {'Acc%':>6} {'Edges':>6} {'Q(%/e)':>8} {'Bud':>5} {'Std':>5} {'Time':>6}")
    print(f"  {'-'*20} {'-'*6} {'-'*6} {'-'*8} {'-'*5} {'-'*5} {'-'*6}")
    for r in results:
        print(f"  {r['name']:<20} {r['acc']*100:6.2f} {r['edges']:6d} {r['quality']:8.3f} "
              f"{r['budget_mean']:5.1f} {r['budget_std']:5.1f} {r['time']:5.0f}s")
    sys.stdout.flush()
