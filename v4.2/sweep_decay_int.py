...
def init_w(b, d, sl, nt, wi, wo, bg, uid):
    global _bp, _all_data, _seq_len, _n_train, _W_in, _W_out, _bigram, _use_int_decay
...
def eval_accuracy_classic(mask, H, W_in, W_out, decay_param, use_int, text_bytes, bp):
...
def run_config(name, use_int, init_decay, bp, ALL_DATA, bigram, eval_seqs, H, W_in, W_out,
    ...
    try:
        for step in range(1, max_steps+1):
            ...
            if step % 100 == 0:
                ...
                ea = np.mean([eval_accuracy_classic(mask, H, W_in, W_out, decay_param, use_int, s, bp)
                              for s in eval_seqs])
