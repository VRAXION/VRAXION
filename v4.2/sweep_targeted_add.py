"""
Targeted Add — add edges near existing ones vs fully random
=============================================================
Instead of random (r,c), pick r or c from an existing edge's
neighborhood. Should help in larger search spaces.
A: Random add (current), B: Targeted add (neighbor of existing)
1024 neurons, 1000 steps.
"""
...
def init_w(b, d, sl, nt, wof, bg, it, am):
    global _bp, _all_data, _seq_len, _n_train, _output_projection_f, _bigram, _inj_table, _add_mode
...
def worker_eval(args):
    ...
    if ptype == 'add':
        ...
        if _add_mode == 'targeted' and len(alive_rs) > 0:
            # 50% chance: pick source from existing edge's source or target
            if rng.random() < 0.5:
                idx = rng.randint(0, len(alive_rs)-1)
                r = alive_rs[idx]  # reuse existing source
                c = rng.randint(0, H-1)
            else:
                idx = rng.randint(0, len(alive_cs)-1)
                r = rng.randint(0, H-1)
                c = alive_cs[idx]  # reuse existing target
        else:
...
def run_config(name, add_mode, bp, ALL_DATA, bigram, eval_seqs, H, output_projection_f, inj_table,
    ...
    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, 200, 2, output_projection_f, bigram, inj_table, add_mode))
    try:
        for step in range(1, max_steps+1):
            ...
            args = [(msign.flatten(), mmag.flatten(), H,
                     44000+step*50+w, ptype) for w in range(n_workers)]
            ...
            if step % 200 == 0:
                ...
                ea = np.mean([eval_accuracy(msign, mmag, H, output_projection_f, s, bp, inj_table)
                              for s in eval_seqs])
                # Count unique neurons used
                rs, cs = np.where(mmag > 0)
                unique_neurons = len(set(rs.tolist() + cs.tolist()))
                print(f"  [{step:4d}] acc={ea*100:.2f}% edges={edges} neurons={unique_neurons}/{H} "
    ...
    rs, cs = np.where(mmag > 0)
    unique_neurons = len(set(rs.tolist() + cs.tolist()))
    print(f"  FINAL: acc={ea*100:.2f}% edges={edges} neurons={unique_neurons}/{H} {elapsed:.0f}s")
    sys.stdout.flush()
    return {'name': name, 'acc': ea, 'edges': edges, 'neurons': unique_neurons, 'time': elapsed}
...
if __name__ == "__main__":
    ...
    results.append(run_config("RANDOM add", 'random',
                              bp, ALL_DATA, bigram, eval_seqs, H, output_projection_f, inj_table))
    results.append(run_config("TARGETED add", 'targeted',
                              bp, ALL_DATA, bigram, eval_seqs, H, output_projection_f, inj_table))

    print(f"\n{'='*60}")
    print(f"  RANDOM vs TARGETED ADD (1000 steps, 1024 neurons)")
    print(f"{'='*60}")
    for r in results:
        print(f"  {r['name']:<15} {r['acc']*100:6.2f}% {r['edges']} edges "
              f"{r['neurons']} neurons {r['time']:.0f}s")
    sys.stdout.flush()
