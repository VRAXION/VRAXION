...
def init_w(b, d, sl, nt, wi, wo, bg, it, uic):
    global _bp, _all_data, _seq_len, _n_train, _W_in, _W_out, _bigram, _inj_table, _use_int_charge
    ...
    for text_bytes in seqs:
        ...
        for i in range(len(text_bytes)-1):
            if _use_int_charge:
                ...
                state = act.copy()
                # Output: convert back to float for W_out matmul
...
def eval_acc(msign, mmag, H, W_in, W_out, text_bytes, bp, inj_table, use_int):
...
def run_config(name, use_int, bp, ALL_DATA, bigram, eval_seqs, H, W_in, W_out, inj_table,
    ...
    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, 200, 2, W_in, W_out, bigram, inj_table, use_int))
    try:
        for step in range(1, max_steps+1):
            ...
            if step % 100 == 0:
                ...
                ea = np.mean([eval_acc(msign, mmag, H, W_in, W_out, s, bp, inj_table, use_int)
                              for s in eval_seqs])
