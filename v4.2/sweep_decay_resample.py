"""
Decay Resample vs Perturbation — full random vs small step
============================================================
A: perturbation +-0.03 (current)
B: full resample [0.01, 0.50] (any value)
C: full resample [0.01, 0.50] + batch 5 neurons at once
D: full resample + wider range [0.001, 0.99]
...
_decay_mode = 'perturb'  # 'perturb', 'resample', 'resample_batch', 'resample_wide'

def init_w(b, d, sl, nt, wi, wo, bg, dm):
    global _bp, _all_data, _seq_len, _n_train, _input_projection, _output_projection, _bigram, _decay_mode
...
def worker_eval(args):
    ...
    if proposal_type == 'add':
        ...
        new_theta[idx] = rng.uniform(0.0, 1.0)  # full resample for theta too
    elif proposal_type == 'decay':
        new_decay = decay.copy()
        if _decay_mode == 'perturb':
            idx = rng.randint(0, H-1)
            new_decay[idx] = max(0.01, min(0.5, decay[idx] + rng.uniform(-0.03, 0.03)))
        elif _decay_mode == 'resample':
            idx = rng.randint(0, H-1)
            new_decay[idx] = rng.uniform(0.01, 0.50)
        elif _decay_mode == 'resample_batch':
            for _ in range(5):
                idx = rng.randint(0, H-1)
                new_decay[idx] = rng.uniform(0.01, 0.50)
        elif _decay_mode == 'resample_wide':
            idx = rng.randint(0, H-1)
            new_decay[idx] = rng.uniform(0.001, 0.99)
...
def run_config(name, decay_mode, bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection,
    ...
    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, 200, 2, input_projection, output_projection, bigram, decay_mode))
    try:
        for step in range(1, max_steps+1):
            ...
            args = [(mask_flat, theta.copy(), decay.copy(), H,
                     25000+step*50+w, ptype) for w in range(n_workers)]
    ...
    print(f"  FINAL: acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
          f"D_acc={accepts['decay']} decay={decay.mean():.4f}+/-{decay.std():.4f} "
          f"[{decay.min():.3f}-{decay.max():.3f}] {elapsed:.0f}s")
    sys.stdout.flush()
    return {'name': name, 'acc': ea, 'edges': edges, 'quality': quality,
            'decay_mean': float(decay.mean()), 'decay_std': float(decay.std()),
            'decay_min': float(decay.min()), 'decay_max': float(decay.max()),
...
if __name__ == "__main__":
    ...
    results.append(run_config("PERTURB +-0.03", 'perturb',
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    results.append(run_config("RESAMPLE [0.01,0.5]", 'resample',
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    results.append(run_config("RESAMPLE BATCH 5", 'resample_batch',
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    results.append(run_config("RESAMPLE WIDE", 'resample_wide',
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    print(f"\n{'='*75}")
    print(f"  SUMMARY -- DECAY RESAMPLE vs PERTURB (18w, 8t, bigram)")
    print(f"{'='*75}")
    print(f"  {'Name':<22} {'Acc%':>6} {'Edges':>6} {'D_acc':>6} {'Decay':>14} {'Range':>14}")
    print(f"  {'-'*22} {'-'*6} {'-'*6} {'-'*6} {'-'*14} {'-'*14}")
    for r in results:
        print(f"  {r['name']:<22} {r['acc']*100:6.2f} {r['edges']:6d} {r['decay_accepts']:6d} "
