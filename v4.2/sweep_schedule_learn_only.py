"""
Learnable Schedule — just the 2 missing configs from the crashed sweep.
"""
...
def run_config(name, init_counts, bp, ALL_DATA, bigram, eval_seqs, H, W_in, W_out,
    ...
    print(f"\n--- {name} ---")
    print(f"  Init: {counts} -> len={len(schedule)}")
    sys.stdout.flush()
    ...
    try:
        for step in range(1, n_steps+1):
            ...
            args = [(mask_flat, theta.copy(), decay.copy(), H,
                     15000+step*50+w, ptype) for w in range(n_workers)]
            ...
            # Random walk schedule every 20 steps
            if step % 20 == 0:
...
if __name__ == "__main__":
    ...
    # F: Learnable (start 3a/1f/1t)
    results.append(run_config("LEARN 3a/1f/1t", {'add': 3, 'flip': 1, 'theta': 1},
                              bp, ALL_DATA, bigram, eval_seqs, H, W_in, W_out))

    # G: Learnable with decay (start 3a/1f/1t/1d)
    results.append(run_config("LEARN 3a/1f/1t/1d", {'add': 3, 'flip': 1, 'theta': 1, 'decay': 1},
                              bp, ALL_DATA, bigram, eval_seqs, H, W_in, W_out))

    print(f"\n{'='*75}")
    print(f"  LEARNABLE SCHEDULE RESULTS (300 steps, 8 ticks, bigram 2seq, ReLU)")
    print(f"{'='*75}")
    print(f"  {'Name':<22} {'Acc%':>6} {'Edges':>6} {'A':>4} {'F':>4} {'T':>4} {'D':>4} {'Final sched':<20}")
    print(f"  {'-'*22} {'-'*6} {'-'*6} {'-'*4} {'-'*4} {'-'*4} {'-'*4} {'-'*20}")
    for r in results:
        a = r['accepts']
        fc = r['final_counts']
