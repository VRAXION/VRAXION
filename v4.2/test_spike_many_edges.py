"""
INSTNCT Spike + Many Edges Test
=================================
Spike thr=3 + threshold=0 (accept ALL positive mutations)
→ sok edge, spike véd overflow ellen
→ kérdés: több edge = jobb accuracy?

vs baseline: threshold=0.00005, no spike (korábbi legjobb: 21.8%)
"""
...
if __name__ == "__main__":
    ...
    print(f"SPIKE + MANY EDGES TEST")
    print(f"spike_thr=3, threshold=0 (accept ALL positive)")
    print(f"vs previous best: threshold=0.00005, no spike = 21.8%")
    print(f"H={H}, {N_WORKERS}w, {BUDGET} steps, w=2 superposition")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    try:
        for step in range(1, BUDGET+1):
            ...
            # threshold=0: accept ALL positive
            ...
            if step % 200 == 0:
                ...
                overall, _ = eval_facts(msign, mmag, H, wof, bp, inj_table, SPIKE_THR)
    ...
    elapsed = time.time() - t0
    edges = int((mmag > 0).sum())
    overall, fact_res = eval_facts(msign, mmag, H, wof, bp, inj_table, SPIKE_THR)

    print(f"\n{'='*60}")
    print(f"  SPIKE + MANY EDGES RESULT ({BUDGET} steps)")
    print(f"{'='*60}")
    print(f"  Answer: {overall*100:.2f}% (prev best: 21.8%)")
    print(f"  Edges: {edges}")
    print(f"  Time: {elapsed:.0f}s")
    print(f"\n  Top facts:")
    scored = sorted(fact_res.items(), key=lambda x: -x[1]['acc'])
    for key, v in scored[:10]:
        print(f"    {key:15s} -> {v['val']:20s} {v['acc']*100:5.1f}%  {v['preds'][:50]}")
    print(f"{'='*60}")
    sys.stdout.flush()
