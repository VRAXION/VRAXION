"""
Decay Step Size Sweep — how big should decay mutations be?
===========================================================
4 configs, 18 workers, 800 steps, bigram 2seq, 8 ticks, charge ReLU.
"""
...
def init_w(b, d, sl, nt, wi, wo, bg, ds):
    global _bp, _all_data, _seq_len, _n_train, _input_projection, _output_projection, _bigram, _decay_step
...
def run_config(name, decay_step, bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection,
    ...
    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, 200, 2, input_projection, output_projection, bigram, decay_step))
    try:
        for step in range(1, max_steps+1):
            ...
            args = [(mask_flat, theta.copy(), decay.copy(), H,
                     24000+step*50+w, ptype) for w in range(n_workers)]
            ...
            if step % 100 == 0:
                ...
                print(f"  [{step:4d}] acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
                      ...
                      f"decay={decay.mean():.4f}+/-{decay.std():.4f} {elapsed:.0f}s")
                sys.stdout.flush()
    ...
    print(f"  FINAL: acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
          f"decay={decay.mean():.4f}+/-{decay.std():.4f} D_acc={accepts['decay']} {elapsed:.0f}s")
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    for ds in [0.03, 0.1, 0.2, 0.3]:
        results.append(run_config(f"DECAY +/-{ds}", ds,
                                  bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    print(f"\n{'='*70}")
    print(f"  SUMMARY -- DECAY STEP SIZE (18w, 8t, bigram 2seq, ReLU)")
    print(f"{'='*70}")
    print(f"  {'Name':<18} {'Acc%':>6} {'Edges':>6} {'Q(%/e)':>8} {'Decay':>12} {'D_acc':>6} {'Time':>6}")
    print(f"  {'-'*18} {'-'*6} {'-'*6} {'-'*8} {'-'*12} {'-'*6} {'-'*6}")
    for r in results:
        print(f"  {r['name']:<18} {r['acc']*100:6.2f} {r['edges']:6d} {r['quality']:8.3f} "
              f"{r['decay_mean']:.4f}+/-{r['decay_std']:.4f} {r['decay_accepts']:6d} {r['time']:5.0f}s")
    sys.stdout.flush()
