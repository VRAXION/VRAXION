...
def run_config(name, scale, theta_init, drive_mag, bp, ALL_DATA, eval_seqs, H, W_in_raw, W_out_raw,
               n_steps=100, n_workers=18, threshold=0.0005):
    W_in = W_in_raw * scale
    W_out = W_out_raw * scale
    ...
    pool = Pool(n_workers, initializer=init_w, initargs=(bp, ALL_DATA, 200, 10, W_in, W_out))
    try:
        for step in range(1, n_steps+1):
            ...
            if step % 25 == 0:
                ...
                ea = np.mean([eval_accuracy(mask, H, W_in, W_out, theta, decay, s, bp)
                              for s in eval_seqs])
...
if __name__ == "__main__":
    ...
    results.append(run_config("A: scale=3 theta=0.1", 3.0, 0.1, 0.6,
                              bp, ALL_DATA, eval_seqs, H, W_in_raw, W_out_raw))
