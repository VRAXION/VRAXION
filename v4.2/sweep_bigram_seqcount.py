"""
Bigram Eval Sample Efficiency — fewer seqs needed?
====================================================
Classic 10 seq vs Bigram cosine 2 seq vs Bigram cosine 1 seq.
If bigram gives reliable signal from fewer samples → big speedup.
...
def run_config(name, mode, n_train_seqs, bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection,
    ...
    print(f"\n--- {name} (mode={mode}, n_seq={n_train_seqs}) ---")
    sys.stdout.flush()
    ...
    try:
        for step in range(1, n_steps+1):
            ...
            args = [(mask_flat, theta.copy(), decay.copy(), H,
                     7000+step*50+w, ptype) for w in range(n_workers)]
...
if __name__ == "__main__":
    ...
    print(f"  acc% always measured with classic 10-seq eval")
    print(f"{'='*70}")
    print(f"  {'Name':<18} {'Acc%':>6} {'Edges':>6} {'Accepts':>8} {'Step/s':>7} {'Time':>6}")
    print(f"  {'-'*18} {'-'*6} {'-'*6} {'-'*8} {'-'*7} {'-'*6}")
    for r in results:
        tot = sum(r['accepts'].values())
        print(f"  {r['name']:<18} {r['acc']*100:6.2f} {r['edges']:6d} {tot:8d} {r['sps']:7.2f} {r['time']:5.0f}s")
    sys.stdout.flush()
