...
- **released tag**: `v5.0.0-beta.9` is the current shipped Rust public beta (Phase D10u state-anchored wiring search — `top_01` release-candidate research checkpoint from `seed_2042 edge_threshold_coadapted`, passed full 16k × 30 fresh seeds adversarial gate, verdict `D10U_TOP01_16K_SHARDED_PASS` — a release-candidate research checkpoint, **not** a public mainline replacement; CPU/Rust cross-check is the recommended next gate before final ship); `v5.0.0-beta.8` is the prior Phase D9.2 multi-objective confirmation + `seed2042_improved_generalist_v1` validated H=384 research checkpoint — a validated finding, **not** a grower-mainline replacement; `v5.0.0-beta.7` is the prior Phase D9 direct-genome landscape audit + `seed2042_improved_v1` smooth+accuracy specialist checkpoint; `v5.0.0-beta.6` is the prior Phase D6/D7/D8 research-line checkpoint + wiki/HTML doc-drift correction patch; `v5.0.0-beta.5` is the prior Phase D3/D3.1/D4 SAF lock + 56-example archive cleanup release; `v5.0.0-beta.4` is the prior Phase D2 cross-H verdict release; `v5.0.0-beta.3` is the prior consolidated mainline + Phase A→B→D research line release; `v5.0.0-beta.2` is the prior grower-based beta and `v5.0.0-beta.1` the original language-evolution beta as historical references
