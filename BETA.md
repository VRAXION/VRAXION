...
- **released tag**: `v5.0.0-beta.5` is the current shipped Rust public beta (Phase D3 K-lock + D3.1 fine K + SAF formula lock + D4 softness verdict + 56-example archive cleanup); `v5.0.0-beta.4` is the prior Phase D2 cross-H verdict release; `v5.0.0-beta.3` is the prior consolidated mainline + Phase A→B→D research line release; `v5.0.0-beta.2` is the prior grower-based beta and `v5.0.0-beta.1` the original language-evolution beta as historical references
