//! Concrete train -> inference anti-collapse probe.
//!
//! 045 verifies that the route-grammar training path produces input-conditioned
//! outputs on explicit examples instead of static/majority/copy shortcuts.
...
const MINIMUM_EXPECTED_ENTROPY: f64 = 1.50;
...
impl ArmKind {
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::FinalTraining044Reference => "FINAL_TRAINING_044_REFERENCE",
            Self::RouteGrammarTrainAndInfer => "ROUTE_GRAMMAR_TRAIN_AND_INFER",
            Self::RouteGrammarTrainAndInferRollbackGated => {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_045_CONCRETE_DATA_INFERENCE_ANTI_COLLAPSE",
            "seeds": cfg.seeds,
    ...
    write_json(
        ...
        &json!({
            "contract": "STABLE_LOOP_PHASE_LOCK_045_CONCRETE_DATA_INFERENCE_ANTI_COLLAPSE",
            "positive_gate": "heldout>=0.90, ood>=0.80, family_min>=0.75, anti-collapse controls fail"
        }),
    )?;
...
fn input_conditioned_answer(example: &ConcreteExample) -> Option<String> {
    match example.task_family.as_str() {
        "route_answer" => parse_route_answer(&example.input).map(|p| format!("phase_{}", p)),
...
fn derive_verdicts(rows: &[MetricsRow]) -> Vec<String> {
    ...
    let main = get("ROUTE_GRAMMAR_TRAIN_AND_INFER");
    let rollback = get("ROUTE_GRAMMAR_TRAIN_AND_INFER_ROLLBACK_GATED");
    ...
    if main_pass && rollback_pass && controls_fail && shuffled_fails {
        verdicts.insert("CONCRETE_DATA_INFERENCE_POSITIVE".to_string());
        verdicts.insert("INPUT_CONDITIONED_OUTPUTS_LEARNED".to_string());
        ...
        verdicts.insert("FINAL_TRAINING_NEEDS_MORE_BEHAVIORAL_WORK".to_string());
        if rows
            .iter()
...
fn write_report(out: &Path, rows: &[MetricsRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    text.push_str("# STABLE_LOOP_PHASE_LOCK_045_CONCRETE_DATA_INFERENCE_ANTI_COLLAPSE\n\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_045_concrete_data_inference_anti_collapse/dev",
    );
