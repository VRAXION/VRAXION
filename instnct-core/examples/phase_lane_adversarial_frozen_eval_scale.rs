//! Adversarial frozen eval scale probe.
//!
//! 049 scales the 048 frozen heldout/OOD eval into a larger adversarial corpus
//! with exact, near-duplicate, and semantic overlap audits.
...
const FROZEN_EVAL_JSONL: &str = include_str!(
...
impl ArmKind {
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::BaseTrainNoRouteGrammar => "NO_ROUTE_GRAMMAR_ADVERSARIAL_FROZEN_BASELINE",
            Self::FinalTraining044Reference => "FROZEN_EVAL_048_REFERENCE",
            Self::RouteGrammarTrainAndInfer => "ADVERSARIAL_FROZEN_ROUTE_GRAMMAR_TRAIN_AND_INFER",
            Self::RouteGrammarTrainAndInferRollbackGated => {
...
struct MetricsRow {
    ...
    train_eval_near_duplicate_count: usize,
    train_eval_semantic_overlap_count: usize,
    max_train_eval_token_jaccard: f64,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_049_ADVERSARIAL_FROZEN_EVAL_SCALE",
            "seeds": cfg.seeds,
            ...
            "frozen_eval_source": "docs/research/STABLE_LOOP_PHASE_LOCK_049_ADVERSARIAL_FROZEN_EVAL_CORPUS.jsonl",
    ...
    write_json(
        ...
        &json!({
            "contract": "STABLE_LOOP_PHASE_LOCK_049_ADVERSARIAL_FROZEN_EVAL_SCALE",
            "positive_gate": "adversarial frozen heldout>=0.90, ood>=0.85, family_min>=0.80, hard_distractor>=0.85, long_ood>=0.85, no exact/near/semantic leakage, entropy>=2.25, controls fail"
        }),
    )?;
    ...
    for (idx, arm) in arms.iter().enumerate() {
        ...
        append_jsonl(
            &cfg.out.join("adversarial_frozen_metrics.jsonl"),
            &result.row,
        )?;
        if arm.is_control() {
        ...
        append_jsonl(
            ...
            &json!({
                ...
                "train_eval_near_duplicate_count": result.row.train_eval_near_duplicate_count,
                "train_eval_semantic_overlap_count": result.row.train_eval_semantic_overlap_count
            }),
        )?;
...
fn evaluate_arm(
    ...
    let hard_distractor_accuracy = group_prefix_accuracy(&inference, "hard_distractor_");
    let long_ood_accuracy = group_prefix_accuracy(&inference, "long_ood_");
    ...
    let positive_gate = matches!(
        ...
        && leakage.train_eval_near_duplicate_count == 0
        && leakage.train_eval_semantic_overlap_count == 0
        && !collapse_detected
    ...
    EvalResult {
        row: MetricsRow {
            ...
            train_eval_near_duplicate_count: leakage.train_eval_near_duplicate_count,
            train_eval_semantic_overlap_count: leakage.train_eval_semantic_overlap_count,
            max_train_eval_token_jaccard: leakage.max_train_eval_token_jaccard,
...
fn leakage_audit(
    ...
    let train_inputs = train
        ...
        .collect::<BTreeSet<_>>();
    let train_fingerprints = train
        .iter()
        .map(|ex| semantic_fingerprint(ex))
        .collect::<BTreeSet<_>>();
    let train_token_sets = train
        .iter()
        .map(|ex| input_token_set(&ex.input))
        .collect::<Vec<_>>();
    let mut eval_ids = BTreeSet::new();
    let mut id_overlap = 0usize;
    let mut input_overlap = 0usize;
    let mut near_duplicate = 0usize;
    let mut semantic_overlap = 0usize;
    ...
    for ex in heldout.iter().chain(ood.iter()) {
        ...
        let eval_fingerprint = semantic_fingerprint(ex);
        if train_fingerprints.contains(&eval_fingerprint) {
            semantic_overlap += 1;
        }
        let eval_tokens = input_token_set(&ex.input);
        let row_max = train_token_sets
            .iter()
            .map(|tokens| token_jaccard(tokens, &eval_tokens))
            .fold(0.0, f64::max);
    ...
    LeakageAudit {
        ...
        train_eval_near_duplicate_count: near_duplicate,
        train_eval_semantic_overlap_count: semantic_overlap,
        max_train_eval_token_jaccard: max_jaccard,
        frozen_eval_hash: stable_hash(FROZEN_EVAL_JSONL),
    }
}

fn semantic_fingerprint(example: &ConcreteExample) -> String {
    format!(
        "{}|{}|{}",
        example.task_family,
        example.expected_output,
        stable_hash(&normalize_for_semantic_overlap(&example.input))
    )
}

fn normalize_for_semantic_overlap(input: &str) -> String {
    input
        .split_whitespace()
        .map(|token| {
            token
                .trim_matches(|c: char| !c.is_ascii_alphanumeric() && c != '_' && c != '>')
                .trim_start_matches("ADV_")
                .trim_start_matches("FROZEN_")
                .trim_start_matches("OOD_")
                .to_ascii_lowercase()
        })
        .collect::<Vec<_>>()
        .join(" ")
...
fn input_token_set(input: &str) -> BTreeSet<String> {
    input
...
fn derive_verdicts(rows: &[MetricsRow]) -> Vec<String> {
    ...
    let main = get("ADVERSARIAL_FROZEN_ROUTE_GRAMMAR_TRAIN_AND_INFER");
    let rollback = get("ADVERSARIAL_FROZEN_ROUTE_GRAMMAR_ROLLBACK_GATED");
    ...
    let no_leak = main
        .map(|r| {
            r.train_eval_id_overlap_count == 0
                && r.train_eval_input_overlap_count == 0
                && r.train_eval_near_duplicate_count == 0
                && r.train_eval_semantic_overlap_count == 0
        })
        .unwrap_or(false);
    if main_pass && rollback_pass && controls_fail && shuffled_fails && no_leak {
        verdicts.insert("ADVERSARIAL_FROZEN_EVAL_SCALE_POSITIVE".to_string());
        verdicts.insert("ADVERSARIAL_FROZEN_INPUT_CONDITIONING_PASSES".to_string());
        verdicts.insert("ADVERSARIAL_FROZEN_NO_TRAIN_LEAKAGE".to_string());
        verdicts.insert("NEAR_DUPLICATE_LEAKAGE_AUDIT_PASSES".to_string());
        verdicts.insert("SEMANTIC_OVERLAP_AUDIT_PASSES".to_string());
        verdicts.insert("TEMPLATE_HOLDOUT_PASSES".to_string());
        verdicts.insert("FAMILY_HOLDOUT_PASSES".to_string());
        verdicts.insert("HARD_DISTRACTOR_PASSES".to_string());
        verdicts.insert("LONG_OOD_PASSES".to_string());
        ...
        verdicts.insert("ADVERSARIAL_FROZEN_EVAL_FAILS".to_string());
        if !no_leak {
...
fn write_report(out: &Path, rows: &[MetricsRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    text.push_str("# STABLE_LOOP_PHASE_LOCK_049_ADVERSARIAL_FROZEN_EVAL_SCALE\n\n");
    ...
    text.push_str("```\n\n## Arm Metrics\n\n");
    text.push_str("| arm | heldout | ood | family_min | hard | long_ood | leaks | near | semantic | top | entropy | collapse |\n");
    text.push_str(
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n",
    );
    for row in rows {
        text.push_str(&format!(
            ...
            row.hard_distractor_accuracy,
            row.long_ood_accuracy,
            row.train_eval_id_overlap_count + row.train_eval_input_overlap_count,
            row.train_eval_near_duplicate_count,
            row.train_eval_semantic_overlap_count,
            row.top_output_rate,
            row.output_entropy,
    ...
    text.push_str(
        "\nProduction defaults remain disabled. This is an adversarial frozen-eval behavioral gate, not a production claim.\n",
    );
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_049_adversarial_frozen_eval_scale/dev",
    );
