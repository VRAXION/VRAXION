//! Runner-local route-grammar API/generalization probe.
//!
//! 033 tests whether the 032 route-grammar loop can be expressed as a reusable
//! subsystem and hold across generalized route-task families.
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Loop032Baseline => "LOOP_032_BASELINE",
            Self::GeneralizedSinglePath => "GENERALIZED_SINGLE_PATH",
            Self::VariableWidthPath => "VARIABLE_WIDTH_PATH",
            Self::LongRouteStress => "LONG_ROUTE_STRESS",
            Self::MultiTargetRouteSet => "MULTI_TARGET_ROUTE_SET",
            Self::BranchingRouteTree => "BRANCHING_ROUTE_TREE",
            Self::VariableGateRuleFamily => "VARIABLE_GATE_RULE_FAMILY",
            Self::ProductionLikeRouteGrammarApi => "PRODUCTION_LIKE_ROUTE_GRAMMAR_API",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_033_ROUTE_GRAMMAR_API_GENERALIZATION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_033_ROUTE_GRAMMAR_API_GENERALIZATION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("generalization_metrics.jsonl"), &row)?;
...
fn route_grammar_subsystem_path(case: &Case, family: TaskFamily) -> Vec<usize> {
    match family {
        TaskFamily::SinglePath => autonomous_loop_path(case, LabelPolicy::Mixed, 1),
        TaskFamily::MultiTarget => {
            let path = autonomous_loop_path(case, LabelPolicy::PruneResidual, 1);
            if graph_path_quality(case, &path).source_to_target_reachability >= 1.0 {
                path
            } else {
                autonomous_loop_path(case, LabelPolicy::Mixed, 2)
            }
        }
        TaskFamily::BranchingTree => {
            let path = autonomous_loop_path(case, LabelPolicy::GraphInvariant, 1);
            if graph_path_quality(case, &path).scaffold_branch_count <= 0.05 {
                path
            } else {
                autonomous_loop_path(case, LabelPolicy::Mixed, 2)
        ...
        TaskFamily::VariableGate => autonomous_loop_path(case, LabelPolicy::Frontier, 1),
    }
}
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if generalized_positive {
        verdicts.insert("ROUTE_GRAMMAR_API_GENERALIZATION_POSITIVE".to_string());
    }
    if get("GENERALIZED_SINGLE_PATH")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("GENERALIZED_SINGLE_PATH_WORKS".to_string());
    }
    if get("VARIABLE_WIDTH_PATH")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("VARIABLE_WIDTH_PATH_WORKS".to_string());
    }
    if get("LONG_ROUTE_STRESS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("LONG_ROUTE_STRESS_WORKS".to_string());
    }
    if get("MULTI_TARGET_ROUTE_SET")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MULTI_TARGET_ROUTE_SET_WORKS".to_string());
    }
    if get("BRANCHING_ROUTE_TREE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCHING_ROUTE_TREE_WORKS".to_string());
    }
    if get("VARIABLE_GATE_RULE_FAMILY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("VARIABLE_GATE_RULE_FAMILY_WORKS".to_string());
    }
    if get("PRODUCTION_LIKE_ROUTE_GRAMMAR_API")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("PRODUCTION_LIKE_ROUTE_GRAMMAR_API_WORKS".to_string());
    }
    ...
    if partial_signal && !generalized_positive {
        verdicts.insert("GENERALIZATION_PARTIAL_SIGNAL".to_string());
    }
    ...
    if !generalized_positive {
        verdicts.insert("ROUTE_GRAMMAR_API_GENERALIZATION_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_033_ROUTE_GRAMMAR_API_GENERALIZATION Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("033 is a route-grammar API/generalization diagnostic. Passing arms show that the 032 loop can be expressed as a reusable route-grammar subsystem and applied across generalized toy route-task families. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_033_route_grammar_api_generalization/dev",
    );
