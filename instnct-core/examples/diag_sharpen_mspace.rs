//! Sharpened Beukers — CANONICAL m-space version.
//!
//! Per GPT's correction: threshold lives in the same space as the Beukers
//! output |m|, not in raw |ab|. This keeps the comparison clean (everything
//! in saturated [0,1) space) and avoids mixing scale-unstable raw signals
//! with the Beukers-normalized output.
//!
//! The ρ-semantics from the raw side are preserved via the map
//!   |m| = |ab| / (1 + |ab|)
//! so raw thresholds {1/ρ, 1, ρ} map to m-space thresholds:
//!   (1/ρ)/(1 + 1/ρ)  =  1/(1+ρ)    ≈ 0.430
//!   1/(1+1)          =  0.500
//!   ρ/(1+ρ)                          ≈ 0.570
//!
//! Mechanism:
//!   m = ab / (1 + |ab|)                      ← standard Beukers signal
//!   g = σ(β · (|m| - θ))                     ← decision gate on |m|
//!   co = m · g                               ← sharpened output
//!
//! β = 8 fixed. 4 variants × 3 seeds = 12 parallel runs on FineWeb 30MB.
//! All iso-param (2-proj, nf=128, ~32,411 params).
//!
//! Run from repo root:
//!   cargo run --release --example diag_sharpen_mspace -- <corpus_path>
...
/// Gate — threshold on |m|, not |ab|.
/// kind=0: B0 Beukers baseline
/// kind=1: sharpen θ = 1/(1+ρ) ≈ 0.430 (maps from raw 1/ρ)
/// kind=2: sharpen θ = 0.500          (maps from raw 1)
/// kind=3: sharpen θ = ρ/(1+ρ) ≈ 0.570 (maps from raw ρ)
#[inline(always)]
fn gate(kind: usize, pv: &[f32]) -> f32 {
    ...
    match kind {
        0 => m,
        _ => {
            let theta: f32 = match kind {
                1 => 1.0 / (1.0 + RHO),
                2 => 0.5,
                3 => RHO / (1.0 + RHO),
                _ => 0.5,
            };
            let g = 1.0 / (1.0 + (-BETA * (m.abs() - theta)).exp());
            m * g
        }
    }
}
...
fn run(
    ...
    println!("[{}-s{}] start  variant={} nf={} proj={} params={} theta_m={:.4}",
    ...
    for ep in 0..max_ep {
        ...
        for _ in 0..samples {
            ...
            // Track g-gate opening (computed on |m|).
            if is_sharpen {
                for f in 0..nf {
                    let pv = &projs_by_f[f];
                    let p = pv[0] * pv[1];
                    let absm = (p / (1.0 + p.abs())).abs();
                    let g = 1.0 / (1.0 + (-BETA * (absm - theta_val)).exp());
                    g_sum += g;
                    g_count += 1;
                }
            }
...
fn main() {
    ...
    println!("=== SHARPENED BEUKERS (m-space, canonical) — corpus = {} ===", corpus_path);
    ...
    println!("    S-lo        sharpen θ = 1/(1+ρ) ≈ 0.430  ↔ raw 1/ρ");
    println!("    S-mid       sharpen θ = 0.500             ↔ raw 1");
    println!("    S-hi        sharpen θ = ρ/(1+ρ) ≈ 0.570   ↔ raw ρ");
    println!();
    println!("  Gate: co = m · σ(β·(|m| - θ))   where m = ab/(1+|ab|), β = 8");
    println!();
    ...
    rayon::scope(|s| {
        ...
        for &seed in &seeds {
            s.spawn(move |_| {
                let r = run("S-lo", corpus_ref, split, "sharpen θ_m=0.430", 1, seed);
                results_ref.lock().unwrap().push(r);
            });
        }
        for &seed in &seeds {
            s.spawn(move |_| {
                let r = run("S-mid", corpus_ref, split, "sharpen θ_m=0.500", 2, seed);
                results_ref.lock().unwrap().push(r);
            });
        }
        for &seed in &seeds {
            s.spawn(move |_| {
                let r = run("S-hi", corpus_ref, split, "sharpen θ_m=0.570", 3, seed);
                results_ref.lock().unwrap().push(r);
            });
        }
    });
    ...
    for var in &["Beukers baseline", "sharpen θ_m=0.430", "sharpen θ_m=0.500", "sharpen θ_m=0.570"] {
    ...
    for var in &["sharpen θ_m=0.430", "sharpen θ_m=0.500", "sharpen θ_m=0.570"] {
        let gs: Vec<f64> = all.iter().filter(|r| r.variant == *var)
            .map(|r| r.final_g_mean as f64).collect();
        if !gs.is_empty() {
            println!("    {:<22} g_mean = {:.3}  ({})", var, mean(&gs),
