//! Runner-local route-grammar supervision probe.
//!
//! 027 tests whether explicit staged route-grammar supervision can construct
//! the order objective that 026 could not recover from delivery-only or local
//! self-supervised critics.
...
enum ArmKind {
    HandOrderRegularizerReference,
    SuccessorValidityOnly,
    BranchCycleOnly,
    RouteContinuityOnly,
    FamilyMinOnly,
    LearnedSuccessorGrammar,
    LearnedBranchCycleGrammar,
    LearnedContinuityGrammar,
    ComposedRouteGrammarCritic,
    ComposedGrammarPlusDelivery,
    SelfSupervisedDeliveryGrammar,
    RandomGrammarControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::HandOrderRegularizerReference => "HAND_ORDER_REGULARIZER_REFERENCE",
            Self::SuccessorValidityOnly => "SUCCESSOR_VALIDITY_ONLY",
            Self::BranchCycleOnly => "BRANCH_CYCLE_ONLY",
            Self::RouteContinuityOnly => "ROUTE_CONTINUITY_ONLY",
            Self::FamilyMinOnly => "FAMILY_MIN_ONLY",
            Self::LearnedSuccessorGrammar => "LEARNED_SUCCESSOR_GRAMMAR",
            Self::LearnedBranchCycleGrammar => "LEARNED_BRANCH_CYCLE_GRAMMAR",
            Self::LearnedContinuityGrammar => "LEARNED_CONTINUITY_GRAMMAR",
            Self::ComposedRouteGrammarCritic => "COMPOSED_ROUTE_GRAMMAR_CRITIC",
            Self::ComposedGrammarPlusDelivery => "COMPOSED_GRAMMAR_PLUS_DELIVERY",
            Self::SelfSupervisedDeliveryGrammar => "SELF_SUPERVISED_DELIVERY_GRAMMAR",
            Self::RandomGrammarControl => "RANDOM_GRAMMAR_CONTROL",
            Self::RandomPhaseRuleControl => "RANDOM_PHASE_RULE_CONTROL",
        }
    }

    fn diagnostic_only(self) -> bool {
        matches!(self, Self::HandOrderRegularizerReference)
    }

    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomGrammarControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_027_ROUTE_GRAMMAR_SUPERVISION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_027_ROUTE_GRAMMAR_SUPERVISION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::SelfSupervisedDeliveryGrammar,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::SelfSupervisedDeliveryGrammar {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::SelfSupervisedDeliveryGrammar {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::SelfSupervisedDeliveryGrammar,
        ArmKind::HandOrderRegularizerReference,
        ArmKind::SuccessorValidityOnly,
        ArmKind::BranchCycleOnly,
        ArmKind::RouteContinuityOnly,
        ArmKind::FamilyMinOnly,
        ArmKind::LearnedSuccessorGrammar,
        ArmKind::LearnedBranchCycleGrammar,
        ArmKind::LearnedContinuityGrammar,
        ArmKind::ComposedRouteGrammarCritic,
        ArmKind::ComposedGrammarPlusDelivery,
        ArmKind::RandomGrammarControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::HandOrderRegularizerReference => GraphMode::PerfectSuccessor,
        ArmKind::SuccessorValidityOnly => GraphMode::SuccessorConsistency,
        ArmKind::BranchCycleOnly => GraphMode::LocalBranchCycleCritic,
        ArmKind::RouteContinuityOnly => GraphMode::RouteContinuity,
        ArmKind::FamilyMinOnly => GraphMode::FamilyMinAdversarial,
        ArmKind::LearnedSuccessorGrammar => GraphMode::SupervisedSuccessorGrammar,
        ArmKind::LearnedBranchCycleGrammar => GraphMode::SupervisedBranchCycleGrammar,
        ArmKind::LearnedContinuityGrammar => GraphMode::SupervisedContinuityGrammar,
        ArmKind::ComposedRouteGrammarCritic => GraphMode::ComposedSupervisedGrammar,
        ArmKind::ComposedGrammarPlusDelivery => GraphMode::ComposedGrammarDelivery,
        ArmKind::SelfSupervisedDeliveryGrammar => GraphMode::DeliveryOnly,
        ArmKind::RandomGrammarControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomGrammarControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_GRAMMAR_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
        ) && r.phase_final_accuracy >= 0.75
    });
    if get("HAND_ORDER_REGULARIZER_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("HAND_ORDER_REGULARIZER_REFERENCE_REPRODUCED".to_string());
    }
    let supervision_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
            && !r.arm.starts_with("RANDOM_")
            && r.arm != "HAND_ORDER_REGULARIZER_REFERENCE"
    });
    if supervision_positive {
        verdicts.insert("ROUTE_GRAMMAR_SUPERVISION_POSITIVE".to_string());
    }
    if get("LEARNED_SUCCESSOR_GRAMMAR")
        .map(|r| r.retained_successor_accuracy >= 0.50)
        .unwrap_or(false)
    {
        verdicts.insert("SUCCESSOR_VALIDITY_LEARNABLE".to_string());
    }
    if get("LEARNED_BRANCH_CYCLE_GRAMMAR")
        .map(|r| r.branch_count <= 0.05 && r.cycle_count <= 0.10)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCH_CYCLE_GRAMMAR_LEARNABLE".to_string());
    }
    if get("LEARNED_CONTINUITY_GRAMMAR")
        .map(|r| r.source_to_target_reachability >= 0.90)
        .unwrap_or(false)
    {
        verdicts.insert("CONTINUITY_GRAMMAR_LEARNABLE".to_string());
    }
    if !get("SELF_SUPERVISED_DELIVERY_GRAMMAR")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SELF_SUPERVISED_GRAMMAR_INSUFFICIENT".to_string());
    }
    if get("COMPOSED_ROUTE_GRAMMAR_CRITIC")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("COMPOSED_GRAMMAR_CRITIC_WORKS".to_string());
    }
    if get("COMPOSED_GRAMMAR_PLUS_DELIVERY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DELIVERY_PLUS_GRAMMAR_WORKS".to_string());
    }
    if get("FAMILY_MIN_ONLY")
        .map(|r| r.family_min_accuracy < 0.85)
        .unwrap_or(false)
    {
        verdicts.insert("FAMILY_MIN_GRAMMAR_REQUIRED".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("GRAMMAR_CONTROL_CONTAMINATION".to_string());
    } else {
        verdicts.insert("RANDOM_GRAMMAR_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if supervision_positive {
        verdicts.insert("ORDER_OBJECTIVE_NO_LONGER_HAND_SPECIFIED".to_string());
    } else {
        verdicts.insert("ORDER_OBJECTIVE_STILL_HAND_SPECIFIED".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_027_ROUTE_GRAMMAR_SUPERVISION Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Retained succ | Order | Grammar P | Grammar R | FPR | FNR | Missing succ | Branch | Cycle | Continuity | Reach | Gate collapse | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("027 is a route-grammar supervision diagnostic. Passing composed grammar arms show that explicit staged route-grammar labels can reproduce the 025 order objective; they do not prove self-supervised or production route construction. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_locality_audit(cfg: &Config) -> std::io::Result<()> {
    let audit = json!({
        ...
        "route_grammar_supervision_arms": [
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out =
        PathBuf::from("target/pilot_wave/stable_loop_phase_lock_027_route_grammar_supervision/dev");
