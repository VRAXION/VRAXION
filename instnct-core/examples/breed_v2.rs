...
use instnct_core::{load_corpus,
    build_network, DirectedEdge, eval_accuracy, evolution_step, InitConfig, Int8Projection,
    Network, PropagationConfig, SdrTable, StepOutcome,
};
...
fn main() {
    ...
    for i in 0..nc {
        offspring.threshold_mut()[i] = net_a.threshold()[i];
        offspring.channel_mut()[i] = net_a.channel()[i];
