//! ANCHOR-MINI-005 sparse process-internalization probe.
//!
//! This is an audit-sized sparse mutation-selection carrier for the
//! MINI-003 shortcut-flip task. MINI-004 showed that routed carriers work when
//! handed oracle match bits. MINI-005 masks those oracle bits for learned
//! process carriers and requires the process route to infer them from raw
//! goal/effect category indicators.
...
enum Carrier {
    SparseDirect,
    SparseOracleRouted,
    SparseLearnedProcess,
    SparseLearnedHybrid,
    SparseShuffledProcess,
}
...
impl Carrier {
    fn parse(raw: &str) -> Self {
        match raw {
            ...
            "SPARSE_ORACLE_ROUTED" => Self::SparseOracleRouted,
            "SPARSE_LEARNED_PROCESS" => Self::SparseLearnedProcess,
            "SPARSE_LEARNED_HYBRID" => Self::SparseLearnedHybrid,
            "SPARSE_SHUFFLED_PROCESS" => Self::SparseShuffledProcess,
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SparseOracleRouted => "SPARSE_ORACLE_ROUTED",
            Self::SparseLearnedProcess => "SPARSE_LEARNED_PROCESS",
            Self::SparseLearnedHybrid => "SPARSE_LEARNED_HYBRID",
            Self::SparseShuffledProcess => "SPARSE_SHUFFLED_PROCESS",
        }
    }

    fn uses_direct(self) -> bool {
        matches!(self, Self::SparseDirect | Self::SparseLearnedHybrid)
    }

    fn uses_process(self) -> bool {
        !matches!(self, Self::SparseDirect)
    }
    ...
    fn is_oracle(self) -> bool {
        matches!(self, Self::SparseOracleRouted)
    }

    fn uses_shuffled_process(self) -> bool {
        matches!(self, Self::SparseShuffledProcess)
    }
}
...
fn parse_args() -> Config {
    ...
    let mut carrier = Carrier::SparseLearnedProcess;
    ...
    match branch {
        ...
        Branch::Process => {
            if !carrier.uses_process() {
                ...
                vec![FeatureKind::OracleMatch, FeatureKind::Bias]
            } else {
                category_features()
            }
        }
...
fn target_bit(carrier: Carrier, ex: &Example, candidate: usize) -> u8 {
...
fn process_bit_accuracy(genome: &Genome, carrier: Carrier, rows: &[Example]) -> f64 {
    ...
    for ex in rows {
        ...
        for i in 0..CANDIDATE_COUNT {
            total += 1;
            correct += ((scores[i] > 0.0) as u8 == target_bit(carrier, ex, i)) as usize;
        }
    }
    correct as f64 / total as f64
}

fn true_process_bit_accuracy(genome: &Genome, carrier: Carrier, rows: &[Example]) -> f64 {
    if !carrier.uses_process() {
        return 0.0;
    }
    let mut total = 0usize;
...
fn process_exact_row_accuracy(genome: &Genome, carrier: Carrier, rows: &[Example]) -> f64 {
    ...
    rows.iter()
        .filter(|ex| {
            let scores = branch_scores(genome, Branch::Process, ex);
            (0..CANDIDATE_COUNT).all(|i| (scores[i] > 0.0) as u8 == target_bit(carrier, ex, i))
        })
        .count() as f64
        / rows.len() as f64
}

fn true_process_exact_row_accuracy(genome: &Genome, carrier: Carrier, rows: &[Example]) -> f64 {
    if !carrier.uses_process() {
        return 0.0;
    }
    rows.iter()
...
fn evolve(dataset: &Dataset, cfg: &Config) -> (Genome, f64, usize) {
    let mut rng = Rng::new(cfg.seed ^ 0xA17C_005u64);
...
fn main() {
    ...
    let payload = json!({
        "status": if invalid_stress { "ANCHOR_MINI_005_JOB_INVALID_STRESS" } else { "ANCHOR_MINI_005_JOB_COMPLETE" },
        "config": {
        ...
        "metrics": {
            ...
            "true_process_bit_accuracy": true_process_bit_accuracy(&genome, cfg.carrier, &dataset.eval_examples),
            "true_process_exact_row_accuracy": true_process_exact_row_accuracy(&genome, cfg.carrier, &dataset.eval_examples),
        ...
        "feature_visibility": {
            "oracle_match_visible": cfg.carrier.is_oracle(),
            "learned_process_uses_match_bits_as_input": false,
            ...
            "shuffled_process_target": if cfg.carrier.uses_shuffled_process() {
