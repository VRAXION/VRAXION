//! Sweep 4: Long run on ListNet H=512 (best from sweep 1) — 300s/seed, 5 seeds
//!
//! Run: cargo run --example overnight_longrun --release
...
fn main() {
    ...
    println!("=== SWEEP 4: Long Run — ListNet H={}, edge_cap={} ===", H, EDGE_CAP);
    println!("{}s/seed | {} seeds | Corpus: {} bytes", WALL_CLOCK_SECS, SEEDS.len(), corpus.len());
    println!("{:>6} {:>8} {:>8} {:>8} {:>8} {:>8}", "seed", "edges", "steps", "step/s", "acc%", "µs/tok");
    println!("{:-<6} {:-<8} {:-<8} {:-<8} {:-<8} {:-<8}", "", "", "", "", "", "");
    ...
    for &seed in &SEEDS {
        ...
        while Instant::now() < deadline {
            ...
            if !mutated { let _ = eval_cos(&mut net, &corpus, 20, &mut eval_rng, &sdr, &bigram); steps += 1; continue; }
        ...
        let step_s = steps as f64 / WALL_CLOCK_SECS as f64;
        let tok_per_step = 20.0 * 2.0; // 2 evals × 20 tokens
        let us_per_tok = 1_000_000.0 / (step_s * tok_per_step * TICKS as f64);
        all_acc.push(acc);

        println!("{:>6} {:>8} {:>8} {:>8.0} {:>7.1}% {:>8.1}",
            seed, net.edge_count(), steps, step_s, acc * 100.0, us_per_tok);
    }

    let best = all_acc.iter().cloned().fold(0.0f64, f64::max);
    let mean = all_acc.iter().sum::<f64>() / all_acc.len() as f64;
    let min = all_acc.iter().cloned().fold(f64::INFINITY, f64::min);
    println!("\nSummary: best={:.1}%, mean={:.1}%, min={:.1}%, spread={:.1}pp",
        best * 100.0, mean * 100.0, min * 100.0, (best - min) * 100.0);
}
