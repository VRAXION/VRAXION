//! Runner-local public directed route construction probe.
//!
//! 020 asks whether public wall/source/target information can construct a
//! clean forward-only route now that 019 established receive-commit delivery.
...
enum ArmKind {
    ...
    PublicBfsShortestRouteReceiveCommitLedger,
    PublicDistanceFieldDagReceiveCommitLedger,
    PublicDistanceFieldSingleSuccessorReceiveCommitLedger,
    PublicFrontierParentRouteReceiveCommitLedger,
    PublicWallFollowRightReceiveCommitLedger,
    PublicWallFollowLeftReceiveCommitLedger,
    PublicAcyclicNoReciprocalReceiveCommitLedger,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::BidirectionalGridBaseline019 => "BIDIRECTIONAL_GRID_BASELINE_019",
            ...
            Self::PublicBfsShortestRouteReceiveCommitLedger => "PUBLIC_BFS_SHORTEST_ROUTE_RECEIVE_COMMIT_LEDGER",
            Self::PublicDistanceFieldDagReceiveCommitLedger => "PUBLIC_DISTANCE_FIELD_DAG_RECEIVE_COMMIT_LEDGER",
            Self::PublicDistanceFieldSingleSuccessorReceiveCommitLedger => {
                "PUBLIC_DISTANCE_FIELD_SINGLE_SUCCESSOR_RECEIVE_COMMIT_LEDGER"
            }
            Self::PublicFrontierParentRouteReceiveCommitLedger => "PUBLIC_FRONTIER_PARENT_ROUTE_RECEIVE_COMMIT_LEDGER",
            Self::PublicWallFollowRightReceiveCommitLedger => "PUBLIC_WALL_FOLLOW_RIGHT_RECEIVE_COMMIT_LEDGER",
            Self::PublicWallFollowLeftReceiveCommitLedger => "PUBLIC_WALL_FOLLOW_LEFT_RECEIVE_COMMIT_LEDGER",
            Self::PublicAcyclicNoReciprocalReceiveCommitLedger => "PUBLIC_ACYCLIC_NO_RECIPROCAL_RECEIVE_COMMIT_LEDGER",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(
            self,
            Self::TruePathReceiveCommitLedgerDiagnostic | Self::TruePathPlusReverseReceiveCommitLedger
        )
    }
...
enum GraphMode {
    ...
    PublicGradient,
    PublicMonotone,
    PublicBfsShortestRoute,
    PublicDistanceFieldDag,
    PublicDistanceFieldSingleSuccessor,
    PublicFrontierParentRoute,
    PublicWallFollowRight,
    PublicWallFollowLeft,
    PublicAcyclicNoReciprocal,
    RandomSameCount,
    DirectionShuffle,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_020_PUBLIC_DIRECTED_ROUTE_CONSTRUCTION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!("../../docs/research/STABLE_LOOP_PHASE_LOCK_020_PUBLIC_DIRECTED_ROUTE_CONSTRUCTION_CONTRACT.md"),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::BidirectionalGridBaseline019,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::BidirectionalGridBaseline019 {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::BidirectionalGridBaseline019 {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::PublicBfsShortestRouteReceiveCommitLedger,
        ArmKind::PublicDistanceFieldDagReceiveCommitLedger,
        ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger,
        ArmKind::PublicFrontierParentRouteReceiveCommitLedger,
        ArmKind::PublicWallFollowRightReceiveCommitLedger,
        ArmKind::PublicWallFollowLeftReceiveCommitLedger,
        ArmKind::PublicAcyclicNoReciprocalReceiveCommitLedger,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::BidirectionalGridBaseline019 => GraphMode::Bidirectional,
        ...
        ArmKind::PublicBfsShortestRouteReceiveCommitLedger => GraphMode::PublicBfsShortestRoute,
        ArmKind::PublicDistanceFieldDagReceiveCommitLedger => GraphMode::PublicDistanceFieldDag,
        ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger => {
            GraphMode::PublicDistanceFieldSingleSuccessor
        }
        ArmKind::PublicFrontierParentRouteReceiveCommitLedger => GraphMode::PublicFrontierParentRoute,
        ArmKind::PublicWallFollowRightReceiveCommitLedger => GraphMode::PublicWallFollowRight,
        ArmKind::PublicWallFollowLeftReceiveCommitLedger => GraphMode::PublicWallFollowLeft,
        ArmKind::PublicAcyclicNoReciprocalReceiveCommitLedger => GraphMode::PublicAcyclicNoReciprocal,
...
fn build_graph(mode: GraphMode, case: &Case, seed: u64) -> DirectedGraph {
    ...
    match mode {
        ...
        GraphMode::PublicBfsShortestRoute | GraphMode::PublicFrontierParentRoute => {
            for pair in public_bfs_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::PublicDistanceFieldDag | GraphMode::PublicAcyclicNoReciprocal => {
            ...
            for id in 0..cells {
                ...
                for (ny, nx) in neighbors(width, y, x) {
                    let to = cell_id(width, ny, nx);
                    if !case.public.wall[to] && dist[to].is_some_and(|to_d| to_d + 1 == from_d) {
                        add_edge(&mut edges, &mut seen, id, to);
                    }
                }
            }
        ...
        GraphMode::PublicWallFollowRight => {
            for pair in public_wall_follow_path(case, true).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::PublicWallFollowLeft => {
            for pair in public_wall_follow_path(case, false).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::RandomSameCount => {
            ...
            let mut rng = StdRng::seed_from_u64(seed ^ 0xA51D_020);
        ...
        GraphMode::DirectionShuffle => {
            let mut rng = StdRng::seed_from_u64(seed ^ 0xD1EC_020);
            for pair in case.private.true_path.windows(2) {
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if let (Some(s), Some(r)) = (true_path, reverse) {
    ...
    if !public_positive.is_empty() {
        verdicts.insert("PUBLIC_DIRECTED_ROUTE_CONSTRUCTION_POSITIVE".to_string());
    } else {
        verdicts.insert("PUBLIC_ROUTE_CONSTRUCTION_FAILS".to_string());
        verdicts.insert("ROUTING_POLICY_STILL_BLOCKED".to_string());
    }
    if get("PUBLIC_BFS_SHORTEST_ROUTE_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
        || get("PUBLIC_FRONTIER_PARENT_ROUTE_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
    {
        verdicts.insert("PUBLIC_BFS_ROUTE_WORKS".to_string());
    }
    if get("PUBLIC_DISTANCE_FIELD_DAG_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
        || get("PUBLIC_DISTANCE_FIELD_SINGLE_SUCCESSOR_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
        || get("PUBLIC_ACYCLIC_NO_RECIPROCAL_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
    {
        verdicts.insert("PUBLIC_DISTANCE_FIELD_ROUTE_WORKS".to_string());
    }
    if get("PUBLIC_DISTANCE_FIELD_DAG_RECEIVE_COMMIT_LEDGER")
        .map(|r| r.positive && r.edge_count > true_path.map(|t| t.edge_count * 1.5).unwrap_or(f64::INFINITY))
        .unwrap_or(false)
    {
        verdicts.insert("PUBLIC_DISTANCE_FIELD_DAG_FANOUT_CONTAMINATION".to_string());
    }
    if !get("PUBLIC_WALL_FOLLOW_RIGHT_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
        && !get("PUBLIC_WALL_FOLLOW_LEFT_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
    {
        verdicts.insert("PUBLIC_WALL_FOLLOW_FAILS".to_string());
    }
    if !get("PUBLIC_GRADIENT_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("PUBLIC_GRADIENT_STILL_FAILS".to_string());
    }
    if !get("PUBLIC_MONOTONE_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("PUBLIC_MONOTONE_STILL_FAILS".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("ROUTE_PRIOR_OVERPOWERS_CONTROL".to_string());
    } else {
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_020_PUBLIC_DIRECTED_ROUTE_CONSTRUCTION Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("020 is a public route-construction diagnostic only. True-path route arms are diagnostic upper bounds. It does not claim production architecture, FlyWire validation, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from("target/pilot_wave/stable_loop_phase_lock_020_public_directed_route_construction/dev");
...
fn random_phase_table(seed: u64) -> [[usize; K]; K] {
    let mut rng = StdRng::seed_from_u64(seed ^ 0xA11CE020);
