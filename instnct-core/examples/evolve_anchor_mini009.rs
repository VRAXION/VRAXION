//! ANCHOR-MINI-009 byte-serialized PLAN bridge probe.
//!
//! This audit-sized sparse mutation-selection carrier tests whether the
//! process-first route from MINI-008 still works when the task situation is
//! read from a fixed ASCII byte record instead of direct symbolic features.
...
fn digit_after(bytes: &[u8], marker: &[u8]) -> Option<usize> {
    bytes
        .windows(marker.len() + 1)
        .find(|window| &window[..marker.len()] == marker)
        .and_then(|window| {
            let value = window[marker.len()];
...
fn byte_goal_category(ex: &Example) -> usize {
    digit_after(task_bytes(ex), b"G=").expect("missing G= category")
}

fn byte_effect_category(ex: &Example, candidate: usize) -> usize {
    let marker = [SLOT_LETTERS[candidate], b'=', b'E'];
    digit_after(task_bytes(ex), &marker).expect("missing candidate effect category")
}

fn byte_surface_bucket(ex: &Example, candidate: usize) -> usize {
    let marker = [SLOT_LETTERS[candidate], b'=', b'E'];
    let bytes = task_bytes(ex);
    for idx in 0..bytes.len().saturating_sub(6) {
        if bytes[idx] == marker[0]
            && bytes[idx + 1] == marker[1]
            && bytes[idx + 2] == marker[2]
            && bytes[idx + 4] == b':'
            && bytes[idx + 5] == b'S'
        {
            let value = bytes[idx + 6];
            if value.is_ascii_digit() {
                return (value - b'0') as usize;
            }
        }
    }
    panic!("missing candidate surface bucket");
}

fn byte_surface_shortcut_label(ex: &Example) -> usize {
    let mut best = 0usize;
    let mut best_bucket = byte_surface_bucket(ex, 0);
    for candidate in 1..CANDIDATE_COUNT {
        let bucket = byte_surface_bucket(ex, candidate);
...
fn byte_input_integrity(ex: &Example) -> bool {
    if task_bytes_forbidden_leak(ex) {
        return false;
    }
    if byte_goal_category(ex) != ex.goal_category {
        return false;
    }
    if byte_surface_shortcut_label(ex) != ex.surface_shortcut_label {
        return false;
    }
    for candidate in 0..CANDIDATE_COUNT {
        if byte_effect_category(ex, candidate) != ex.effect_categories[candidate] {
            return false;
        }
    }
    true
...
fn target_policy_bit(carrier: Carrier, ex: &Example, candidate: usize) -> u8 {
    if carrier.uses_shuffled_teacher() {
        (byte_effect_category(ex, candidate) == shifted_category(byte_goal_category(ex))) as u8
    } else if carrier.uses_shortcut_teacher() {
        (candidate == byte_surface_shortcut_label(ex)) as u8
    } else {
...
fn feature_value(feature: FeatureKind, carrier: Carrier, ex: &Example, candidate: usize) -> f64 {
    match feature {
        FeatureKind::ByteSurface => byte_surface_bucket(ex, candidate) as f64 / 9.0,
        FeatureKind::ByteGoalCat(category) => (byte_goal_category(ex) == category) as u8 as f64,
        FeatureKind::ByteEffectCat(category) => {
            (byte_effect_category(ex, candidate) == category) as u8 as f64
        }
        FeatureKind::ByteGoalEffectConjunction(category) => {
            (byte_goal_category(ex) == category && byte_effect_category(ex, candidate) == category)
                as u8 as f64
        }
        FeatureKind::ByteShiftedConjunction(category) => {
            let shifted = shifted_category(category);
            (byte_goal_category(ex) == category && byte_effect_category(ex, candidate) == shifted)
                as u8 as f64
        }
...
fn evolve(dataset: &Dataset, cfg: &Config) -> (Genome, f64, usize) {
    let mut rng = Rng::new(cfg.seed ^ 0xA17C_009u64);
...
fn main() {
    ...
    let invalid_stress = train_align < 0.85 || eval_flip < 0.85 || !byte_integrity;
    let (genome, train_fitness, accepted_steps) = evolve(&dataset, &cfg);
    let payload = json!({
        "status": if invalid_stress {
            "ANCHOR_MINI_009_JOB_INVALID_STRESS"
        } else {
            "ANCHOR_MINI_009_JOB_COMPLETE"
        },
        "config": {
