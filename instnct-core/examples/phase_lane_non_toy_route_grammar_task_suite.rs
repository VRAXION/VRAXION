//! Non-toy route-grammar task-suite probe.
//!
//! 035 tests whether the experimental `instnct-core` route-grammar API holds
//! under harder route-task families and bounded API error handling.
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Loop032Baseline => "API_BASELINE_034",
            Self::GeneralizedSinglePath => "NOISY_GRAPH_ROUTE_TASK",
            Self::VariableWidthPath => "MULTI_REQUEST_BATCH",
            Self::LongRouteStress => "DYNAMIC_EDGE_DELETION_REPAIR",
            Self::MultiTargetRouteSet => "DISTRACTOR_SAME_SOURCE_TARGET",
            Self::BranchingRouteTree => "COMPOSITIONAL_ROUTE_LABELS",
            Self::VariableGateRuleFamily => "VARIABLE_PHASE_GATE_POLICY",
            Self::ProductionLikeRouteGrammarApi => "REACHABLE_SEED_BUG_REGRESSION",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_035_NON_TOY_ROUTE_GRAMMAR_TASK_SUITE",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_035_NON_TOY_ROUTE_GRAMMAR_TASK_SUITE_CONTRACT.md"
        ),
    )?;

    write_api_error_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("non_toy_task_metrics.jsonl"), &row)?;
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if generalized_positive {
        verdicts.insert("NON_TOY_ROUTE_GRAMMAR_SUITE_POSITIVE".to_string());
    }
    if get("NOISY_GRAPH_ROUTE_TASK")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("NOISY_GRAPH_ROUTE_TASK_WORKS".to_string());
    }
    if get("MULTI_REQUEST_BATCH")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MULTI_REQUEST_BATCH_WORKS".to_string());
    }
    if get("DYNAMIC_EDGE_DELETION_REPAIR")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DYNAMIC_EDGE_DELETION_REPAIR_WORKS".to_string());
    }
    if get("DISTRACTOR_SAME_SOURCE_TARGET")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DISTRACTOR_SAME_SOURCE_TARGET_WORKS".to_string());
    }
    if get("COMPOSITIONAL_ROUTE_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("COMPOSITIONAL_ROUTE_LABELS_WORKS".to_string());
    }
    if get("VARIABLE_PHASE_GATE_POLICY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("VARIABLE_PHASE_GATE_POLICY_WORKS".to_string());
    }
    if get("REACHABLE_SEED_BUG_REGRESSION")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
    ...
    if partial_signal && !generalized_positive {
        verdicts.insert("NON_TOY_PARTIAL_SIGNAL".to_string());
    }
    ...
    if !generalized_positive {
        verdicts.insert("NON_TOY_ROUTE_GRAMMAR_SUITE_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_035_NON_TOY_ROUTE_GRAMMAR_TASK_SUITE Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("035 is a non-toy route-grammar task-suite diagnostic over the instnct-core experimental route-grammar API. Passing arms show that the API-backed route grammar survives noisy graphs, multiple route requests, dynamic repair, distractors, compositional labels, variable phase/gate policies, and the reachable-seed regression check. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_api_error_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    append_jsonl(
        &cfg.out.join("api_error_metrics.jsonl"),
        &json!({
            "arm": "BOUNDED_API_ERROR_HANDLING",
            "source_out_of_bounds_rejected": source_ok,
            "edge_out_of_bounds_rejected": edge_ok,
            "bounded_api_error_handling_pass": source_ok && edge_ok,
        }),
    )
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_035_non_toy_route_grammar_task_suite/dev",
    );
