//! Wave EP Kill Experiment #1: Does C19 converge in Equilibrium Propagation?
//!
...
//!
//! Run: cargo run --example wave_ep_kill1 --release
...
struct EpNet {
    w1: Vec<f32>,  // h_dim × in_dim
    w2: Vec<f32>,  // out_dim × h_dim
    b1: Vec<f32>,  // h_dim
    b2: Vec<f32>,  // out_dim
    in_dim: usize,
    h_dim: usize,
...
struct EpNet2 {
    w1: Vec<f32>,  // h × in
    w2: Vec<f32>,  // h × h (symmetric: also h2→h1)
    w3: Vec<f32>,  // out × h
    b1: Vec<f32>,
    b2: Vec<f32>,
    b3: Vec<f32>,
...
fn main() {
    let mut log = Log::new("instnct-core/wave_ep_log.txt");
    let mut tsv = std::fs::File::create("instnct-core/wave_ep_results.tsv").unwrap();
    let mut curves_file = std::fs::File::create("instnct-core/wave_ep_curves.tsv").unwrap();
    ...
    log.w("╔══════════════════════════════════════════════════╗");
    log.w("║  WAVE EP KILL TEST: C19 + Equilibrium Propagation ║");
    log.w("╚══════════════════════════════════════════════════╝");
