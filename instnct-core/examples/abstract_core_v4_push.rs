//! Abstract Core v4 — Push accuracy ceiling: longer training + best architectures
//!
//! v3 showed no plateau at 100 epochs. Push to 200+ epochs on best configs.
//! Also try: dropout regularization, learning rate warmup, batch size effects.
//!
//! Run: cargo run --example abstract_core_v4_push --release
...
impl Mlp {
    ...
    fn forward_train(&self, input: &[f32], rng: &mut Rng, dropout: f32) -> Vec<Vec<f32>> {
        ...
        for (li, (w, b)) in self.layers.iter().enumerate() {
            ...
            for i in 0..w.len() {
                ...
                if li < n_layers - 1 {
                    a[i] = a[i].max(0.0); // ReLU
                    // Dropout (training only, hidden layers only)
                    if dropout > 0.0 && rng.f32() < dropout { a[i] = 0.0; } else if dropout > 0.0 { a[i] /= 1.0 - dropout; }
                }
            }
            acts.push(a);
        }
    ...
    fn forward(&self, input: &[f32]) -> Vec<f32> {
        ...
        for (li, (w, b)) in self.layers.iter().enumerate() {
            ...
            for i in 0..w.len() {
                next[i] = b[i];
                for j in 0..act.len() { next[i] += w[i][j] * act[j]; }
                if li < n_layers - 1 { next[i] = next[i].max(0.0); }
            }
            act = next;
        }
        act
    ...
    fn train_step(&mut self, input: &[f32], target: usize, lr: f32, dropout: f32, rng: &mut Rng) -> f32 {
        let acts = self.forward_train(input, rng, dropout);
        ...
        for li in (0..n_layers).rev() {
            ...
            for i in 0..w.len() {
                let d = if li < n_layers-1 { if acts[li+1][i] > 0.0 { delta[i] } else { 0.0 } } else { delta[i] };
...
fn eval_int8(pp: &Preproc, m: &Mlp, corpus: &[u8], n: usize, seed: u64) -> f64 {
    let scales: Vec<(f32,f32)> = m.layers.iter().map(|(w,b)| {
        ...
        let sb=b.iter().map(|x|x.abs()).fold(0.0f32,f32::max).max(1e-7)/127.0; (sw,sb)
    }).collect();
    let mut rng=Rng::new(seed); let mut ok=0; let mut tot=0;
    for _ in 0..n { if corpus.len()<CTX+1{break;} let off=rng.range(0,corpus.len()-CTX-1);
        ...
        for (li,(w,b)) in m.layers.iter().enumerate() {
            ...
            for i in 0..w.len() { next[i]=(b[i]/sb).round().max(-127.0).min(127.0)*sb;
                ...
                if li<nl-1 { next[i]=next[i].max(0.0); } }
            act=next; }
...
fn main() {
    ...
    println!("=== ABSTRACT CORE v4 — Push Accuracy Ceiling ===\n");

    // Config A: 2L 256→128 with extended training (200 epochs)
    // Config B: 3L 512→256→128 with extended training (200 epochs)
    // Config C: 2L 256→128 with dropout 0.1
    // Config D: 2L 256→128 with dropout 0.2

    let configs: Vec<(&str, Vec<usize>, usize, usize, f32, f32)> = vec![
        // (name, dims, epochs, samples/epoch, lr, dropout)
        ("2L-256-128 200ep",      vec![MI,256,128,NC], 200, 15000, 0.005, 0.0),
        ("3L-512-256-128 200ep",  vec![MI,512,256,128,NC], 200, 15000, 0.003, 0.0),
        ("2L-256-128 drop=0.1",   vec![MI,256,128,NC], 150, 15000, 0.005, 0.1),
        ("2L-256-128 drop=0.2",   vec![MI,256,128,NC], 150, 15000, 0.005, 0.2),
        ("2L-512-256 200ep",      vec![MI,512,256,NC], 200, 15000, 0.003, 0.0),
    ];

    for (name, dims, epochs, samples, lr_init, dropout) in &configs {
        println!("━━━ {} ({} params) ━━━", name, { let mut r=Rng::new(0); Mlp::new(dims,&mut r).total_params() });
        let mut rng = Rng::new(42);
        let mut m = Mlp::new(dims, &mut rng);
        let mut best_acc = 0.0f64;

        for epoch in 0..*epochs {
            let lr = lr_init * (1.0 - epoch as f32 / *epochs as f32 * 0.85);
            ...
            for _ in 0..*samples {
                ...
                m.train_step(&s, corpus[off+CTX] as usize, lr, *dropout, &mut rng_t);
            }
            if (epoch+1) % 20 == 0 {
                let fa = eval_acc(&pp, &m, &corpus, 5000, 999+epoch as u64);
                let ia = eval_int8(&pp, &m, &corpus, 5000, 999+epoch as u64);
                if fa > best_acc { best_acc = fa; }
                println!("  epoch {:>3}: float={:.1}% int8={:.1}% best={:.1}%", epoch+1, fa, ia, best_acc);
            }
        }
        let fa = eval_acc(&pp, &m, &corpus, 10000, 777);
        let ia = eval_int8(&pp, &m, &corpus, 10000, 777);
        if fa > best_acc { best_acc = fa; }
        println!("  FINAL (10K): float={:.1}% int8={:.1}% best={:.1}%", fa, ia, best_acc);
        println!("  Time: {:.1}s\n", t0.elapsed().as_secs_f64());
    }

    println!("Baselines: frequency=20.3%, INSTNCT=24.6%");
