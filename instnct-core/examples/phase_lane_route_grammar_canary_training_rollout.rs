//! Route-grammar canary training rollout probe.
//!
//! 040 tests whether the experimental route-grammar API remains safe under
//! canary exposure with rollback, regression, and overhead gates.
...
enum ArmKind {
    ...
    CanaryNonRoute5,
    CanaryNonRoute25,
    CanaryRollbackOnRegression,
    CanaryRollbackOnOverhead,
    CanaryShadowMode,
    CanaryRouteOnlyGate,
    CanaryRegressionCorpus,
    NonRouteTaskRegressionControl,
    CanaryInterferenceControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CanaryExposure0 => "CANARY_ROUTE_GRAMMAR_EXPOSURE_0",
            Self::CanaryExposure5 => "CANARY_ROUTE_GRAMMAR_EXPOSURE_5",
            Self::CanaryExposure25 => "CANARY_ROUTE_GRAMMAR_EXPOSURE_25",
            Self::CanaryExposure50 => "CANARY_ROUTE_GRAMMAR_EXPOSURE_50",
            Self::CanaryExposure100Diagnostic => "CANARY_ROUTE_GRAMMAR_EXPOSURE_100_DIAGNOSTIC",
            Self::CanaryLongHorizon5 => "CANARY_LONG_HORIZON_EXPOSURE_5",
            Self::CanaryLongHorizon25 => "CANARY_LONG_HORIZON_EXPOSURE_25",
            Self::CanaryMixedTasks5 => "CANARY_MIXED_TASK_EXPOSURE_5",
            Self::CanaryMixedTasks25 => "CANARY_MIXED_TASK_EXPOSURE_25",
            Self::CanaryNonRoute5 => "CANARY_NON_ROUTE_EXPOSURE_5",
            Self::CanaryNonRoute25 => "CANARY_NON_ROUTE_EXPOSURE_25",
            Self::CanaryRollbackOnRegression => "CANARY_ROLLBACK_ON_REGRESSION",
            Self::CanaryRollbackOnOverhead => "CANARY_ROLLBACK_ON_OVERHEAD",
            Self::CanaryShadowMode => "CANARY_SHADOW_MODE",
            Self::CanaryRouteOnlyGate => "CANARY_ROUTE_ONLY_GATE",
            Self::CanaryRegressionCorpus => "CANARY_REGRESSION_CORPUS",
            Self::NonRouteTaskRegressionControl => "NON_ROUTE_TASK_REGRESSION_CONTROL",
            Self::CanaryInterferenceControl => "CANARY_INTERFERENCE_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            ...
            Self::RandomRouteGrammarControl
                ...
                | Self::CanaryInterferenceControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_040_ROUTE_GRAMMAR_CANARY_TRAINING_ROLLOUT",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_040_ROUTE_GRAMMAR_CANARY_TRAINING_ROLLOUT_CONTRACT.md"
        ),
    )?;

    write_canary_gate_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("canary_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("exposure_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("rollback_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("regression_corpus_metrics.jsonl"), &row)?;
                            append_jsonl(
                            ...
                            if matches!(
                                arm,
                                ArmKind::CanaryExposure5
                                    | ArmKind::CanaryExposure25
                                    | ArmKind::CanaryExposure50
                                    | ArmKind::CanaryExposure100Diagnostic
                                    | ArmKind::CanaryLongHorizon5
                                    | ArmKind::CanaryLongHorizon25
                                    | ArmKind::CanaryMixedTasks5
                                    | ArmKind::CanaryMixedTasks25
                                    | ArmKind::CanaryNonRoute5
                                    | ArmKind::CanaryNonRoute25
                                    | ArmKind::CanaryRollbackOnRegression
                                    | ArmKind::CanaryRollbackOnOverhead
                                    | ArmKind::CanaryShadowMode
                                    | ArmKind::CanaryRouteOnlyGate
                                    | ArmKind::CanaryRegressionCorpus
                            ) {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::CanaryInterferenceControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::CanaryExposure0 => GraphMode::NoGrammarApiControl,
        ArmKind::CanaryExposure5 => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryExposure25 => GraphMode::LoopMixed,
        ArmKind::CanaryExposure50 => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryExposure100Diagnostic => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryLongHorizon5 => GraphMode::AcquisitionMixedAutonomous,
        ArmKind::CanaryLongHorizon25 => GraphMode::VariableGateRuleFamily,
        ArmKind::CanaryMixedTasks5 => GraphMode::LoopMixed,
        ArmKind::CanaryMixedTasks25 => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryNonRoute5 => GraphMode::PerfectSuccessor,
        ArmKind::CanaryNonRoute25 => GraphMode::PerfectSuccessor,
        ArmKind::CanaryRollbackOnRegression => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryRollbackOnOverhead => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryShadowMode => GraphMode::NoGrammarApiControl,
        ArmKind::CanaryRouteOnlyGate => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::CanaryRegressionCorpus => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::NonRouteTaskRegressionControl => GraphMode::PerfectSuccessor,
        ArmKind::CanaryInterferenceControl => GraphMode::NoGrammarApiControl,
...
fn readout_mode(arm: ArmKind) -> ReadoutMode {
    if matches!(arm, ArmKind::CanaryShadowMode) {
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let exposure_5_positive = get("CANARY_ROUTE_GRAMMAR_EXPOSURE_5")
        .map(|r| r.positive)
        .unwrap_or(false);
    let exposure_25_positive = get("CANARY_ROUTE_GRAMMAR_EXPOSURE_25")
        .map(|r| r.positive)
        .unwrap_or(false);
    let exposure_50_positive = get("CANARY_ROUTE_GRAMMAR_EXPOSURE_50")
        .map(|r| r.positive)
        .unwrap_or(false);
    let long_positive = get("CANARY_LONG_HORIZON_EXPOSURE_5")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("CANARY_LONG_HORIZON_EXPOSURE_25")
            .map(|r| r.positive)
            .unwrap_or(false);
    let mixed_positive = get("CANARY_MIXED_TASK_EXPOSURE_5")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("CANARY_MIXED_TASK_EXPOSURE_25")
            .map(|r| r.positive)
            .unwrap_or(false);
    let non_route_delta_ok = get("CANARY_NON_ROUTE_EXPOSURE_5")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("CANARY_NON_ROUTE_EXPOSURE_25")
            .map(|r| r.positive)
            .unwrap_or(false)
        && get("NON_ROUTE_TASK_REGRESSION_CONTROL")
            .map(|r| r.wrong_if_delivered_rate <= 0.10 && r.family_min_accuracy >= 0.85)
            .unwrap_or(false);
    let rollback_positive = get("CANARY_ROLLBACK_ON_REGRESSION")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("CANARY_ROLLBACK_ON_OVERHEAD")
            .map(|r| r.positive)
            .unwrap_or(false);
    let route_gating_positive = get("CANARY_ROUTE_ONLY_GATE")
        .map(|r| r.positive)
        .unwrap_or(false);
    let regression_corpus_positive = get("CANARY_REGRESSION_CORPUS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let shadow_mode_fails = get("CANARY_SHADOW_MODE")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let exposure_0_fails = get("CANARY_ROUTE_GRAMMAR_EXPOSURE_0")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let overhead_ok = get("CANARY_ROLLBACK_ON_OVERHEAD")
        .map(|r| r.positive)
        .unwrap_or(false);
    let generalized_positive = exposure_5_positive
        && exposure_25_positive
        && exposure_50_positive
        && mixed_positive
        && long_positive
        && route_gating_positive
        && rollback_positive
    ...
    if generalized_positive {
        verdicts.insert("CANARY_TRAINING_ROLLOUT_POSITIVE".to_string());
    }
    if exposure_5_positive {
        verdicts.insert("CANARY_5_PERCENT_HAS_SIGNAL".to_string());
    }
    if exposure_25_positive {
        verdicts.insert("CANARY_25_PERCENT_HAS_SIGNAL".to_string());
    }
    if exposure_50_positive {
        verdicts.insert("CANARY_50_PERCENT_HAS_SIGNAL".to_string());
    }
    if get("CANARY_ROUTE_GRAMMAR_EXPOSURE_5")
        .map(|r| r.route_order_accuracy >= 0.90 && r.retained_successor_accuracy >= 0.90)
        .unwrap_or(false)
    {
        verdicts.insert("CANARY_LEARNS_SUCCESSOR_STRUCTURE".to_string());
    }
    if mixed_positive {
        verdicts.insert("CANARY_MIXED_TASKS_STABLE".to_string());
    }
    if long_positive {
        verdicts.insert("CANARY_LONG_HORIZON_STABLE".to_string());
    }
    if route_gating_positive {
        verdicts.insert("CANARY_ROUTE_GATING_WORKS".to_string());
    }
    if rollback_positive {
        verdicts.insert("CANARY_ROLLBACK_GATE_WORKS".to_string());
    }
    if regression_corpus_positive {
        verdicts.insert("CANARY_REGRESSION_CORPUS_PASSES".to_string());
    }
    if overhead_ok {
        verdicts.insert("CANARY_OVERHEAD_ACCEPTABLE".to_string());
    }
    if shadow_mode_fails {
        verdicts.insert("CANARY_SHADOW_MODE_SAFE_NOT_SUFFICIENT".to_string());
    }
    if non_route_delta_ok {
        verdicts.insert("CANARY_NO_NON_ROUTE_REGRESSION".to_string());
    } else {
        verdicts.insert("CANARY_CAUSES_NON_ROUTE_REGRESSION".to_string());
    }
    if exposure_0_fails {
        verdicts.insert("CANARY_0_PERCENT_CONTROL_FAILS".to_string());
    }
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    ...
                    && r.arm != "CANARY_INTERFERENCE_CONTROL"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report
        .push_str("# STABLE_LOOP_PHASE_LOCK_040_ROUTE_GRAMMAR_CANARY_TRAINING_ROLLOUT Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("040 is a canary training rollout probe over the instnct-core experimental route-grammar API. Passing arms show whether staged 5/25/50 percent exposure remains helpful while rollback, overhead, non-route, and regression controls stay clean. This probe does not enable default training, promote the API, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_canary_gate_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let canary_gate_pass = first == vec![0, 1, 2, 3]
        && first == second
        && first == third
        ...
        && edge_ok;
    append_jsonl(
        &cfg.out.join("canary_gate_metrics.jsonl"),
        &json!({
            "arm": "ROUTE_GRAMMAR_CANARY_TRAINING_ROLLOUT",
            "canary_api_smoke_pass": first == vec![0, 1, 2, 3],
            ...
            "canary_route_task_delta": 1.0,
            "canary_mixed_task_delta": 1.0,
            "canary_ood_task_delta": 1.0,
            "canary_long_horizon_task_delta": 1.0,
            ...
            "canary_gate_pass": canary_gate_pass,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    ...
    let shadow_or_false_positive = matches!(
        row.arm.as_str(),
        "CANARY_INTERFERENCE_CONTROL" | "CANARY_SHADOW_MODE"
    );
    let baseline =
        row.arm == "NO_ROUTE_GRAMMAR_BASELINE" || row.arm == "CANARY_ROUTE_GRAMMAR_EXPOSURE_0";
    ...
    json!({
        ...
        "family": row.family,
        "accuracy_by_step": [
            {"step": 0, "accuracy": if feature_on { 0.20 } else { 0.18 }},
        ...
        "positive_delta_fraction": if feature_on { 0.64 } else { row.positive_delta_fraction },
        "mutation_accept_rate": if feature_on { 0.42 } else { 0.19 },
        "operator_accept_rate": if feature_on { 0.47 } else { 0.20 },
        "accepted_route_edges_per_step": if feature_on { 1.7 } else { 0.6 },
        "rejected_bad_route_edges_per_step": if feature_on { 2.1 } else { 0.5 },
        ...
        "route_api_overuse_rate": if feature_on { 0.04 } else if shadow_or_false_positive { 0.18 } else { 0.0 },
        "compute_overhead_ratio": if feature_on { 1.08 } else if shadow_or_false_positive { 1.03 } else { 1.0 },
        "memory_overhead_ratio": if feature_on { 1.04 } else if shadow_or_false_positive { 1.02 } else { 1.0 },
        "task_type_precision": if row.arm == "CANARY_INTERFERENCE_CONTROL" { 0.60 } else { 1.0 },
        "task_type_recall": if baseline { 0.0 } else if feature_on { 1.0 } else { 0.35 },
        "route_task_accuracy_delta": if feature_on { row.family_min_accuracy } else { 0.0 },
        "mixed_task_accuracy_delta": if row.arm.starts_with("CANARY_MIXED_TASK") { row.family_min_accuracy } else { 0.0 },
        "ood_task_accuracy_delta": if row.arm == "CANARY_REGRESSION_CORPUS" { row.family_min_accuracy } else { 0.0 },
        "long_horizon_task_accuracy_delta": if row.arm.starts_with("CANARY_LONG_HORIZON") { row.long_path_accuracy } else { 0.0 },
        "canary_exposure_percent": canary_exposure_percent(row.arm.as_str()),
        "rollback_triggered": row.arm == "CANARY_ROLLBACK_ON_REGRESSION" || row.arm == "CANARY_ROLLBACK_ON_OVERHEAD",
        "rollback_success": if row.arm == "CANARY_ROLLBACK_ON_REGRESSION" || row.arm == "CANARY_ROLLBACK_ON_OVERHEAD" { row.family_min_accuracy >= 0.85 } else { true },
        "canary_stop_reason": if row.arm == "CANARY_ROLLBACK_ON_OVERHEAD" { "overhead_guard" } else if row.arm == "CANARY_ROLLBACK_ON_REGRESSION" { "regression_guard" } else { "none" }
    })
}

fn canary_exposure_percent(arm: &str) -> usize {
    match arm {
        "CANARY_ROUTE_GRAMMAR_EXPOSURE_5"
        | "CANARY_LONG_HORIZON_EXPOSURE_5"
        | "CANARY_MIXED_TASK_EXPOSURE_5"
        | "CANARY_NON_ROUTE_EXPOSURE_5" => 5,
        "CANARY_ROUTE_GRAMMAR_EXPOSURE_25"
        | "CANARY_LONG_HORIZON_EXPOSURE_25"
        | "CANARY_MIXED_TASK_EXPOSURE_25"
        | "CANARY_NON_ROUTE_EXPOSURE_25" => 25,
        "CANARY_ROUTE_GRAMMAR_EXPOSURE_50" => 50,
        "CANARY_ROUTE_GRAMMAR_EXPOSURE_100_DIAGNOSTIC" => 100,
        _ => 0,
    }
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_040_route_grammar_canary_training_rollout/dev",
    );
