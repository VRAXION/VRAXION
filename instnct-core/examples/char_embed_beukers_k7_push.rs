//! Beukers k=7 — push further: bigger ctx, bigger nf, dual kernel
//!
//! Can we beat 82.1%?
//!
//! Run: cargo run --example char_embed_beukers_k7_push --release
...
fn run(corpus:&[u8], split:usize, name:&str, ctx:usize, k:usize, nf:usize) -> f64 {
    let mask_pos=ctx/2;let fan=k*DIM;let half_k=(k/2)as i32;
...
fn main(){
    ...
    println!("=== BEUKERS k=7 — PUSH FURTHER ===\n");
    println!("  Current record: Beukers ctx=32 k=7 nf=96 = 82.1%\n");

    // Reference
    run(&corpus,split,"ref: ctx32 k7 nf96",32,7,96);
    // More filters
    run(&corpus,split,"ctx32 k7 nf128",32,7,128);
    // Bigger ctx with k=7
    run(&corpus,split,"ctx48 k7 nf96",48,7,96);
    // Even bigger kernel
    run(&corpus,split,"ctx32 k11 nf96",32,11,96);
    // Sweet combo: ctx=48 k=7 nf=128
    run(&corpus,split,"ctx48 k7 nf128",48,7,128);
