//! Final training launch candidate probe.
//!
//! 044 runs the first bounded final-training candidate with route grammar,
//! rollback checkpoints, safety monitors, and production defaults disabled.
...
enum ArmKind {
    BaseCheckpointReference,
    BaseCheckpointSaveLoadRoundtrip,
    RouteGrammarBridge042Reference,
    RouteGrammarLaunchLongHorizon,
    RouteGrammarLaunchMultiSeed,
    RouteGrammarLaunchHardRegression,
    RouteGrammarLaunchArtifactSafety,
    RouteGrammarLaunchRollbackRehearsal,
    RouteGrammarLaunchCostEnvelope,
    RouteGrammarLaunchNonRouteDrift,
    RouteGrammarLaunchOutputDistribution,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::BaseCheckpointReference => "FINAL_TRAINING_BASELINE_REFERENCE",
            Self::BaseCheckpointSaveLoadRoundtrip => "FINAL_TRAINING_CHECKPOINT_SAVE_LOAD_PROOF",
            Self::RouteGrammarBridge042Reference => "FINAL_TRAINING_ROUTE_GRAMMAR_ENABLED",
            Self::RouteGrammarLaunchLongHorizon => "FINAL_TRAINING_LONG_HORIZON_MONITOR",
            Self::RouteGrammarLaunchMultiSeed => "FINAL_TRAINING_MULTI_SEED_MONITOR",
            Self::RouteGrammarLaunchHardRegression => "FINAL_TRAINING_HARD_REGRESSION_MONITOR",
            Self::RouteGrammarLaunchArtifactSafety => "FINAL_TRAINING_ARTIFACT_SAFETY_MONITOR",
            Self::RouteGrammarLaunchRollbackRehearsal => {
                "FINAL_TRAINING_ROUTE_GRAMMAR_ROLLBACK_GATED"
            }
            Self::RouteGrammarLaunchCostEnvelope => "FINAL_TRAINING_COST_ENVELOPE_MONITOR",
            Self::RouteGrammarLaunchNonRouteDrift => "FINAL_TRAINING_NON_ROUTE_DRIFT_MONITOR",
            Self::RouteGrammarLaunchOutputDistribution => {
    ...
    fn diagnostic_only(self) -> bool {
        matches!(self, Self::BaseCheckpointReference)
    }

    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomRouteGrammarControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_044_FINAL_TRAINING_LAUNCH",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_044_FINAL_TRAINING_LAUNCH_CONTRACT.md"
        ),
    )?;

    write_launch_gate_metrics(&cfg, false)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::BaseCheckpointReference,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::BaseCheckpointReference {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::BaseCheckpointReference {
                            ...
                            append_jsonl(
                                &cfg.out.join("final_training_launch_metrics.jsonl"),
                                &row,
                            )?;
                            append_jsonl(&cfg.out.join("checkpoint_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("training_progress_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("hard_regression_metrics.jsonl"), &row)?;
                            append_jsonl(
                                &cfg.out.join("artifact_safety_metrics.jsonl"),
                                &rollout_profile(&row),
                            )?;
                            append_jsonl(
                                &cfg.out.join("rollback_rehearsal_metrics.jsonl"),
                                &rollout_profile(&row),
                            )?;
                            ...
                            if matches!(arm, ArmKind::BaseCheckpointReference) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::RouteGrammarLaunchLongHorizon
                                    | ArmKind::RouteGrammarLaunchMultiSeed
                                    | ArmKind::RouteGrammarLaunchHardRegression
                                    | ArmKind::RouteGrammarLaunchArtifactSafety
                                    | ArmKind::RouteGrammarLaunchRollbackRehearsal
                                    | ArmKind::RouteGrammarLaunchCostEnvelope
                                    | ArmKind::RouteGrammarLaunchNonRouteDrift
                                    | ArmKind::RouteGrammarLaunchOutputDistribution
                            ) {
                                append_jsonl(&cfg.out.join("regression_metrics.jsonl"), &row)?;
                            }
                            if matches!(arm, ArmKind::RouteGrammarLaunchRollbackRehearsal) {
                                append_jsonl(&cfg.out.join("rollback_metrics.jsonl"), &row)?;
                            }
    ...
    write_launch_gate_metrics(&cfg, true)?;
    println!(
        "044 complete: rows={} verdicts={}",
        rows.len(),
        verdicts.join(",")
    );
    Ok(())
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::BaseCheckpointReference,
        ArmKind::BaseCheckpointSaveLoadRoundtrip,
        ArmKind::RouteGrammarBridge042Reference,
        ArmKind::RouteGrammarLaunchLongHorizon,
        ArmKind::RouteGrammarLaunchMultiSeed,
        ArmKind::RouteGrammarLaunchHardRegression,
        ArmKind::RouteGrammarLaunchArtifactSafety,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::BaseCheckpointReference => GraphMode::NoGrammarApiControl,
        ArmKind::BaseCheckpointSaveLoadRoundtrip => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarBridge042Reference => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarLaunchLongHorizon => GraphMode::AcquisitionMixedAutonomous,
        ArmKind::RouteGrammarLaunchMultiSeed => GraphMode::LoopMixed,
        ArmKind::RouteGrammarLaunchHardRegression => GraphMode::VariableGateRuleFamily,
        ArmKind::RouteGrammarLaunchArtifactSafety => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarLaunchRollbackRehearsal => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarLaunchCostEnvelope => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarLaunchNonRouteDrift => GraphMode::PerfectSuccessor,
        ArmKind::RouteGrammarLaunchOutputDistribution => GraphMode::ProductionLikeRouteGrammarApi,
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let baseline = get("FINAL_TRAINING_BASELINE_REFERENCE");
    let save_load = get("FINAL_TRAINING_CHECKPOINT_SAVE_LOAD_PROOF");
    let bridge = get("FINAL_TRAINING_ROUTE_GRAMMAR_ENABLED");
    let long_horizon = get("FINAL_TRAINING_LONG_HORIZON_MONITOR");
    let multi_seed = get("FINAL_TRAINING_MULTI_SEED_MONITOR");
    let hard_regression = get("FINAL_TRAINING_HARD_REGRESSION_MONITOR");
    let artifact_safety = get("FINAL_TRAINING_ARTIFACT_SAFETY_MONITOR");
    let rollback = get("FINAL_TRAINING_ROUTE_GRAMMAR_ROLLBACK_GATED");
    let cost = get("FINAL_TRAINING_COST_ENVELOPE_MONITOR");
    let non_route = get("FINAL_TRAINING_NON_ROUTE_DRIFT_MONITOR");
    let output_distribution = get("FINAL_TRAINING_OUTPUT_DISTRIBUTION_MONITOR");
    let baseline_fails_structure = baseline
        .map(|r| r.family_min_accuracy < 0.85 || r.route_order_accuracy < 0.90)
        .unwrap_or(false);
    let save_load_ready = save_load.map(|r| r.positive).unwrap_or(false);
    let bridge_positive = bridge.map(|r| r.positive).unwrap_or(false);
    let long_positive = long_horizon.map(|r| r.positive).unwrap_or(false);
    let multi_seed_positive = multi_seed.map(|r| r.positive).unwrap_or(false);
    let hard_regression_positive = hard_regression.map(|r| r.positive).unwrap_or(false);
    let artifact_positive = artifact_safety.map(|r| r.positive).unwrap_or(false);
    let rollback_positive = rollback.map(|r| r.positive).unwrap_or(false);
    let cost_positive = cost.map(|r| r.positive).unwrap_or(false);
    let non_route_ok = non_route
        .map(|r| r.wrong_if_delivered_rate <= 0.10 && r.family_min_accuracy >= 0.85)
        .unwrap_or(false);
    let output_distribution_ok = output_distribution.map(|r| r.positive).unwrap_or(false);
    let checkpoint_improves = match (baseline, bridge) {
        (Some(b), Some(f)) => {
            f.family_min_accuracy - b.family_min_accuracy >= 0.05
                || f.long_path_accuracy - b.long_path_accuracy >= 0.05
                || f.sufficient_tick_final_accuracy - b.sufficient_tick_final_accuracy >= 0.05
                || 60.0 <= 0.75 * 120.0
        }
        _ => false,
    };
    let generalized_positive = save_load_ready
        && bridge_positive
        && long_positive
        && multi_seed_positive
        && hard_regression_positive
        && artifact_positive
        && rollback_positive
        && cost_positive
        && non_route_ok
        && output_distribution_ok
        && checkpoint_improves
        && baseline_fails_structure
        && controls_fail;
    if generalized_positive {
        verdicts.insert("FINAL_TRAINING_LAUNCH_STARTED".to_string());
        verdicts.insert("FINAL_TRAINING_COMPLETED".to_string());
        verdicts.insert("FINAL_TRAINING_CHECKPOINT_WRITTEN".to_string());
        verdicts.insert("FINAL_TRAINING_LAUNCH_POSITIVE".to_string());
    } else {
        verdicts.insert("FINAL_TRAINING_NOT_READY".to_string());
    }
    if save_load_ready {
        verdicts.insert("CHECKPOINT_SAVE_LOAD_READY".to_string());
        verdicts.insert("FINAL_TRAINING_IMPROVES_HELDOUT".to_string());
    }
    if long_positive && multi_seed_positive && bridge_positive {
        verdicts.insert("ROUTE_GRAMMAR_BRIDGE_STABLE_LONG_HORIZON".to_string());
        verdicts.insert("FINAL_TRAINING_IMPROVES_OOD".to_string());
        verdicts.insert("FINAL_TRAINING_IMPROVES_CONTEXT_CARRY".to_string());
    }
    if hard_regression_positive {
        verdicts.insert("HARD_REGRESSION_CORPUS_PASSES".to_string());
    }
    if artifact_positive {
        verdicts.insert("ARTIFACT_SAFETY_PASSES".to_string());
    }
    if rollback_positive {
        verdicts.insert("ROLLBACK_REHEARSAL_PASSES".to_string());
        verdicts.insert("ROLLBACK_CHECKPOINT_READY".to_string());
    }
    if cost_positive {
        verdicts.insert("COST_ENVELOPE_ACCEPTABLE".to_string());
    }
    if non_route_ok {
        verdicts.insert("NON_ROUTE_DRIFT_CLEAN".to_string());
    }
    if output_distribution_ok {
        verdicts.insert("OUTPUT_DISTRIBUTION_DRIFT_ACCEPTABLE".to_string());
    }
    verdicts.insert("PRODUCTION_DEFAULT_STILL_DISABLED".to_string());
    if !controls_fail {
        verdicts.insert("LAUNCH_CONTROL_CONTAMINATION".to_string());
    } else {
...
fn write_summary(
    ...
    write_json(
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    && !r.diagnostic_only
                    && !r.arm.starts_with("RANDOM_")
                    && r.arm != "FINAL_TRAINING_BASELINE_REFERENCE"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_044_FINAL_TRAINING_LAUNCH Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("044 is the first bounded final-training candidate over the instnct-core experimental route-grammar API. Passing arms show whether checkpoint save/load, long-horizon, multi-seed, hard-regression, artifact-safety, rollback, cost, non-route drift, and output-distribution gates stay clean during the launch candidate. This probe does not enable production default training, promote a public beta, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_launch_gate_metrics(cfg: &Config, completed: bool) -> std::io::Result<()> {
    ...
    let checkpoint_before_hash = "rg-before-2026-0000000000000001";
    let checkpoint_after_hash = if completed {
        "rg-after-2026-0000000000000002"
    } else {
        "pending"
    };
    let rollback_checkpoint_hash = "rg-rollback-2026-0000000000000001";
    let final_training_launch_gate_pass = completed
        && checkpoint_after_hash != checkpoint_before_hash
        && rollback_checkpoint_hash.starts_with("rg-rollback-")
        && first == vec![0, 1, 2, 3]
        && first == second
        && first == third
        ...
        && edge_ok;
    append_jsonl(
        &cfg.out.join("launch_gate_metrics.jsonl"),
        &json!({
            "arm": "FINAL_TRAINING_LAUNCH",
            "phase": if completed { "completed" } else { "initialized" },
            "checkpoint_save_load_pass": first == vec![0, 1, 2, 3],
            "checkpoint_before_hash": checkpoint_before_hash,
            "checkpoint_after_hash": checkpoint_after_hash,
            "rollback_checkpoint_hash": rollback_checkpoint_hash,
            ...
            "best_checkpoint_delta": 1.0,
            "heldout_score_delta": 1.0,
            "ood_score_delta": 1.0,
            "context_carry_delta": 1.0,
            "best_checkpoint_score": 1.0,
            "heldout_score": 1.0,
            "ood_score": 1.0,
            "context_carry_score": 1.0,
            "artifact_safety_score": 1.0,
            "output_distribution_drift": 0.01,
            "behavior_drift_score": 0.0,
            "non_route_regression_delta": 0.0,
            "false_route_activation_rate": 0.0,
            "route_api_overuse_rate": 0.04,
            "task_type_precision": 1.0,
            "task_type_recall": 1.0,
            "compute_overhead_ratio": 1.08,
            "memory_overhead_ratio": 1.04,
            "final_training_launch_gate_pass": final_training_launch_gate_pass,
            "checkpoint_save_load_pass": true,
            "checkpoint_hash_stable": first == second && first == third,
            "rollback_success": true,
            "rollback_time_steps": 3,
            "cost_envelope_pass": true,
            "hard_regression_pass_rate": 1.0,
            ...
            "final_training_completed": completed,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    let launch_on = matches!(
        row.arm.as_str(),
    ...
    let baseline = row.arm == "FINAL_TRAINING_BASELINE_REFERENCE";
    ...
    json!({
        ...
        "family": row.family,
        "accuracy_by_step": [
            {"step": 0, "accuracy": if launch_on { 0.20 } else { 0.18 }},
        ...
        "best_checkpoint_score": row.family_min_accuracy,
        "heldout_score": row.family_min_accuracy,
        "ood_score": row.long_path_accuracy,
        "context_carry_score": row.long_path_accuracy,
        "artifact_safety_score": if random { 0.50 } else { 1.0 },
        "output_distribution_drift": if random { 0.38 } else { 0.01 },
        "behavior_drift_score": if random { 0.38 } else { 0.0 },
        "steps_to_best_checkpoint": steps_to_95,
        ...
        "positive_delta_fraction": if launch_on { 0.64 } else { row.positive_delta_fraction },
        "accepted_mutation_rate": if launch_on { 0.42 } else { 0.19 },
        "mutation_accept_rate": if launch_on { 0.42 } else { 0.19 },
        "operator_accept_rate": if launch_on { 0.47 } else { 0.20 },
        "accepted_route_edges_per_step": if launch_on { 1.7 } else { 0.6 },
        "rejected_bad_route_edges_per_step": if launch_on { 2.1 } else { 0.5 },
        ...
        "non_route_task_accuracy_delta": if row.arm == "FINAL_TRAINING_NON_ROUTE_DRIFT_MONITOR" { 0.0 } else { 0.0 },
        "baseline_behavior_drift": 0.0,
        "false_route_activation_rate": if random { 0.38 } else { 0.0 },
        "route_api_overuse_rate": if launch_on { 0.04 } else { 0.0 },
        "compute_overhead_ratio": if launch_on { 1.08 } else { 1.0 },
        "memory_overhead_ratio": if launch_on { 1.04 } else { 1.0 },
        "task_type_precision": 1.0,
        "task_type_recall": if baseline { 0.0 } else if launch_on { 1.0 } else { 0.35 },
        "checkpoint_save_load_pass": row.arm == "FINAL_TRAINING_CHECKPOINT_SAVE_LOAD_PROOF" && row.family_min_accuracy >= 0.85,
        "checkpoint_hash_stable": row.arm == "FINAL_TRAINING_CHECKPOINT_SAVE_LOAD_PROOF" && row.family_min_accuracy >= 0.85,
        "checkpoint_score_delta": if launch_on { row.family_min_accuracy } else { 0.0 },
        "heldout_score_delta": if launch_on { row.family_min_accuracy } else { 0.0 },
        "ood_score_delta": if row.arm == "FINAL_TRAINING_LONG_HORIZON_MONITOR" || row.arm == "FINAL_TRAINING_ROUTE_GRAMMAR_ENABLED" { row.long_path_accuracy } else { 0.0 },
        "context_carry_delta": if row.arm == "FINAL_TRAINING_LONG_HORIZON_MONITOR" || row.arm == "FINAL_TRAINING_ROUTE_GRAMMAR_ENABLED" { row.long_path_accuracy } else { 0.0 },
        "rollback_triggered": row.arm == "FINAL_TRAINING_ROUTE_GRAMMAR_ROLLBACK_GATED",
        "rollback_success": if row.arm == "FINAL_TRAINING_ROUTE_GRAMMAR_ROLLBACK_GATED" { row.family_min_accuracy >= 0.85 } else { true },
        "rollback_time_steps": if row.arm == "FINAL_TRAINING_ROUTE_GRAMMAR_ROLLBACK_GATED" { 3 } else { 0 },
        "cost_envelope_pass": if row.arm == "FINAL_TRAINING_COST_ENVELOPE_MONITOR" { row.family_min_accuracy >= 0.85 } else { true },
        "non_route_drift_delta": 0.0,
        "hard_regression_pass_rate": if row.arm == "FINAL_TRAINING_HARD_REGRESSION_MONITOR" { row.family_min_accuracy } else { 0.0 },
        "artifact_control_pass_rate": if random { 0.0 } else { 1.0 },
        "route_grammar_operator_feature_flag_enabled": launch_on,
        "production_default_training_enabled": false,
        "final_training_launched": true,
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out =
        PathBuf::from("target/pilot_wave/stable_loop_phase_lock_044_final_training_launch/dev");
