//! Runner-local dense route grow/prune policy probe.
//!
//! 024 turns the 023 dense-candidate-field -> crystallize/prune branch into
//! controlled policy arms with edge budgets and prune schedules.
...
enum ArmKind {
    TruePathUpperBound,
    HandDenseThenPruneReference,
    DenseFieldEdgeBudget1x,
    DenseFieldEdgeBudget2x,
    DenseFieldEdgeBudget4x,
    DenseFieldEdgeBudget8x,
    CostPenalizedDenseGrow,
    EarlyPruneSchedule,
    LatePruneSchedule,
    GrowThenPruneOnce,
    GrowPruneAlternating,
    SourceTargetAnchorPlusDenseGrow,
    RandomDenseControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::HandDenseThenPruneReference => "HAND_DENSE_THEN_PRUNE_REFERENCE",
            Self::DenseFieldEdgeBudget1x => "DENSE_FIELD_EDGE_BUDGET_1X",
            Self::DenseFieldEdgeBudget2x => "DENSE_FIELD_EDGE_BUDGET_2X",
            Self::DenseFieldEdgeBudget4x => "DENSE_FIELD_EDGE_BUDGET_4X",
            Self::DenseFieldEdgeBudget8x => "DENSE_FIELD_EDGE_BUDGET_8X",
            Self::CostPenalizedDenseGrow => "COST_PENALIZED_DENSE_GROW",
            Self::EarlyPruneSchedule => "EARLY_PRUNE_SCHEDULE",
            Self::LatePruneSchedule => "LATE_PRUNE_SCHEDULE",
            Self::GrowThenPruneOnce => "GROW_THEN_PRUNE_ONCE",
            Self::GrowPruneAlternating => "GROW_PRUNE_ALTERNATING",
            Self::SourceTargetAnchorPlusDenseGrow => "SOURCE_TARGET_ANCHOR_PLUS_DENSE_GROW",
            Self::RandomDenseControl => "RANDOM_DENSE_CONTROL",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(
            self,
            Self::TruePathUpperBound | Self::HandDenseThenPruneReference
        )
    }
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomDenseControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_024_DENSE_ROUTE_GROW_PRUNE_POLICY",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_024_DENSE_ROUTE_GROW_PRUNE_POLICY_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::DenseFieldEdgeBudget1x,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::DenseFieldEdgeBudget1x {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::DenseFieldEdgeBudget1x {
                            ...
                            append_jsonl(&cfg.out.join("grow_prune_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("budget_metrics.jsonl"), &row)?;
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::HandDenseThenPruneReference,
        ArmKind::DenseFieldEdgeBudget2x,
        ArmKind::DenseFieldEdgeBudget4x,
        ArmKind::DenseFieldEdgeBudget8x,
        ArmKind::CostPenalizedDenseGrow,
        ArmKind::EarlyPruneSchedule,
        ArmKind::LatePruneSchedule,
        ArmKind::GrowThenPruneOnce,
        ArmKind::GrowPruneAlternating,
        ArmKind::SourceTargetAnchorPlusDenseGrow,
        ArmKind::RandomDenseControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::HandDenseThenPruneReference => GraphMode::HandDenseThenPrune,
        ArmKind::DenseFieldEdgeBudget1x => GraphMode::DenseBudget(1),
        ArmKind::DenseFieldEdgeBudget2x => GraphMode::DenseBudget(2),
        ArmKind::DenseFieldEdgeBudget4x => GraphMode::DenseBudget(4),
        ArmKind::DenseFieldEdgeBudget8x => GraphMode::DenseBudget(8),
        ArmKind::CostPenalizedDenseGrow => GraphMode::CostPenalizedDenseGrow,
        ArmKind::EarlyPruneSchedule => GraphMode::EarlyPrune,
        ArmKind::LatePruneSchedule => GraphMode::LatePrune,
        ArmKind::GrowThenPruneOnce => GraphMode::GrowThenPruneOnce,
        ArmKind::GrowPruneAlternating => GraphMode::GrowPruneAlternating,
        ArmKind::SourceTargetAnchorPlusDenseGrow => GraphMode::SourceTargetAnchorDenseGrow,
        ArmKind::RandomDenseControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomDenseControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn build_graph(mode: GraphMode, case: &Case, seed: u64) -> DirectedGraph {
    ...
    match mode {
        ...
        GraphMode::DenseBudget(multiplier) => {
            let candidates = dense_budget_candidate_edges(case, multiplier);
            for pair in phase_aware_path_on_edges(case, &candidates).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::CostPenalizedDenseGrow => {
            let candidates = dense_budget_candidate_edges(case, 2);
            for pair in cost_penalized_phase_path(case, &candidates).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::EarlyPrune => {
            let candidates = dense_budget_candidate_edges(case, 1);
            for pair in phase_aware_path_on_edges(case, &candidates).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::LatePrune | GraphMode::GrowThenPruneOnce => {
            let candidates = dense_budget_candidate_edges(case, 4);
            for pair in phase_aware_path_on_edges(case, &candidates).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::GrowPruneAlternating => {
            let mut path = Vec::new();
            for budget in [1usize, 2, 4] {
                let candidates = dense_budget_candidate_edges(case, budget);
                path = phase_aware_path_on_edges(case, &candidates);
                if path.last().copied()
                    == Some(cell_id(width, case.public.target.0, case.public.target.1))
                {
                    break;
                }
            }
        ...
        GraphMode::SourceTargetAnchorDenseGrow => {
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        GraphMode::HandDenseThenPrune => dense_breadcrumb_edges(case).len(),
        GraphMode::DenseBudget(multiplier) => dense_budget_candidate_edges(case, multiplier).len(),
        GraphMode::CostPenalizedDenseGrow => dense_budget_candidate_edges(case, 2).len(),
        GraphMode::EarlyPrune => dense_budget_candidate_edges(case, 1).len(),
        GraphMode::LatePrune | GraphMode::GrowThenPruneOnce => {
            dense_budget_candidate_edges(case, 4).len()
        }
        GraphMode::GrowPruneAlternating => dense_budget_candidate_edges(case, 4).len(),
        GraphMode::SourceTargetAnchorDenseGrow => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_DENSE_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    let policy_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
            && !r.arm.starts_with("RANDOM_")
            && r.arm != "HAND_DENSE_THEN_PRUNE_REFERENCE"
    });
    if policy_positive {
        verdicts.insert("DENSE_GROW_PRUNE_POLICY_POSITIVE".to_string());
    }
    if get("DENSE_FIELD_EDGE_BUDGET_2X")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("EDGE_BUDGET_2X_SUFFICIENT".to_string());
    }
    if !get("DENSE_FIELD_EDGE_BUDGET_2X")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("DENSE_FIELD_EDGE_BUDGET_4X")
            .map(|r| r.positive)
            .unwrap_or(false)
    {
        verdicts.insert("EDGE_BUDGET_4X_REQUIRED".to_string());
    }
    if get("COST_PENALIZED_DENSE_GROW")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("COST_PENALIZED_GROW_WORKS".to_string());
    }
    if !get("EARLY_PRUNE_SCHEDULE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("EARLY_PRUNE_FAILS".to_string());
    }
    if get("LATE_PRUNE_SCHEDULE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("LATE_PRUNE_WORKS".to_string());
    }
    if get("GROW_PRUNE_ALTERNATING")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ALTERNATING_GROW_PRUNE_WORKS".to_string());
    }
    if get("SOURCE_TARGET_ANCHOR_PLUS_DENSE_GROW")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SOURCE_TARGET_ANCHOR_IMPROVES_GROW".to_string());
    }
    if get("HAND_DENSE_THEN_PRUNE_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("HAND_DENSE_REFERENCE_REPRODUCED".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("DENSE_POLICY_CONTROL_CONTAMINATION".to_string());
    } else {
        verdicts.insert("RANDOM_DENSE_CONTROL_FAILS".to_string());
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_024_DENSE_ROUTE_GROW_PRUNE_POLICY Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Initial edges | Final edges | Prune | Retained succ | Order | Branch | Cycle | Cost score | Gate collapse | Counterfactual | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
        report.push_str(&format!(
            "| {} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.1} | {:.1} | {:.3} | {:.3} | {:.3} | {:.1} | {:.1} | {:.3} | {:.3} | {:.3} | {} | {} |\n",
            row.arm,
            ...
            row.initial_edge_count,
            row.final_edge_count,
            row.prune_fraction,
            row.retained_successor_accuracy,
            row.route_order_accuracy,
            row.branch_count,
            row.cycle_count,
            row.edge_cost_adjusted_score,
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("024 is a dense-route grow/prune policy diagnostic only. Passing policy arms show that a dense candidate route field can be grown under a specific budget/prune schedule and crystallized into an ordered successor route in this toy substrate. It does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_024_dense_route_grow_prune_policy/dev",
    );
