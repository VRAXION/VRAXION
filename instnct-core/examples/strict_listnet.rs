//! Strict ListNet: expensive eval (200 tok) + strict accept (only better, no ties)
//!
//! Run: cargo run --example strict_listnet --release
...
const EVAL_TOKENS: usize = 200; // expensive
...
fn main() {
    ...
    println!("Strict ListNet — 200 tok eval, strict accept (only BETTER), edge_cap={}",  EDGE_CAP);
    println!("{}s/seed | {} seeds", WALL_SECS, SEEDS.len());
    println!("{:>6} {:>8} {:>8} {:>8} {:>8} {:>8}", "H", "edges", "steps", "step/s", "best%", "mean%");
    println!("{:-<6} {:-<8} {:-<8} {:-<8} {:-<8} {:-<8}", "", "", "", "", "", "");
    ...
    for &h in &h_configs {
        ...
        for &seed in &SEEDS {
            ...
            while Instant::now() < deadline {
                // Paired eval with EXPENSIVE 200 tokens
                let sr = eval_rng.clone();
                let before = eval_cos(&mut net, &corpus, EVAL_TOKENS, &mut eval_rng, &sdr_t, &bigram);
                eval_rng = sr;

                let genome = net.save();
                let mutated = net.mutate(&mut rng);
                if !mutated {
                    let _ = eval_cos(&mut net, &corpus, EVAL_TOKENS, &mut eval_rng, &sdr_t, &bigram);
                    steps += 1; continue;
                }

                let after = eval_cos(&mut net, &corpus, EVAL_TOKENS, &mut eval_rng, &sdr_t, &bigram);
        ...
        println!("{:>6} {:>8} {:>8} {:>8.0} {:>7.1}% {:>7.1}%",
            h, me, ms, ms as f64 / WALL_SECS as f64, best * 100.0, mean * 100.0);
    }

    println!("\nComparison (cheap 20tok, accept ties):");
    println!("  H=512:  2085 step/s, 23.8% best, 21.1% mean");
    println!("  H=1024: 1091 step/s, 20.8% best, 20.2% mean");
    println!("  H=2048:  574 step/s, 21.4% best, 20.2% mean");
}
