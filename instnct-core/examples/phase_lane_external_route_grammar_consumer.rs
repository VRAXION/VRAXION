//! External route-grammar consumer probe.
//!
//! 036 tests whether external, non-runner consumers can call the experimental
//! `instnct-core` route-grammar API deterministically and without hidden state.
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Loop032Baseline => "EXTERNAL_CONSUMER_SMOKE",
            Self::GeneralizedSinglePath => "MULTI_CALL_API_STATE_ISOLATION",
            Self::VariableWidthPath => "INVALID_INPUT_FUZZ",
            Self::LongRouteStress => "DETERMINISM_REPLAY",
            Self::MultiTargetRouteSet => "SERDE_ROUNDTRIP_TASKS",
            Self::BranchingRouteTree => "NO_GLOBAL_STATE_LEAK",
            Self::VariableGateRuleFamily => "CONCURRENT_CALLS_SAFE",
            Self::ProductionLikeRouteGrammarApi => "REGRESSION_REACHABLE_SEED_BUG",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_036_EXTERNAL_ROUTE_GRAMMAR_CONSUMER",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_036_EXTERNAL_ROUTE_GRAMMAR_CONSUMER_CONTRACT.md"
        ),
    )?;

    write_external_consumer_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("external_consumer_metrics.jsonl"), &row)?;
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if generalized_positive {
        verdicts.insert("EXTERNAL_ROUTE_GRAMMAR_CONSUMER_POSITIVE".to_string());
    }
    if get("MULTI_CALL_API_STATE_ISOLATION")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MULTI_CALL_API_STATE_ISOLATION_WORKS".to_string());
    }
    if get("INVALID_INPUT_FUZZ")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("INVALID_INPUT_FUZZ_WORKS".to_string());
    }
    if get("DETERMINISM_REPLAY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DETERMINISM_REPLAY_WORKS".to_string());
    }
    if get("SERDE_ROUNDTRIP_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SERDE_ROUNDTRIP_TASKS_WORK".to_string());
    }
    if get("NO_GLOBAL_STATE_LEAK")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("NO_GLOBAL_STATE_LEAK_WORKS".to_string());
    }
    if get("CONCURRENT_CALLS_SAFE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("CONCURRENT_CALLS_SAFE".to_string());
    }
    if get("REGRESSION_REACHABLE_SEED_BUG")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("REACHABLE_SEED_BUG_REGRESSION_WORKS".to_string());
    }
    ...
    if partial_signal && !generalized_positive {
        verdicts.insert("EXTERNAL_CONSUMER_PARTIAL_SIGNAL".to_string());
    }
    ...
    if !generalized_positive {
        verdicts.insert("EXTERNAL_ROUTE_GRAMMAR_CONSUMER_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_036_EXTERNAL_ROUTE_GRAMMAR_CONSUMER Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("036 is an external-consumer diagnostic over the instnct-core experimental route-grammar API. Passing arms show that the API can be used deterministically by non-runner consumers with isolated calls, invalid-input rejection, serde-style task roundtrip, no global state leak, concurrent calls, and the reachable-seed regression check. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_external_consumer_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    append_jsonl(
        &cfg.out.join("external_consumer_contract_metrics.jsonl"),
        &json!({
            "arm": "EXTERNAL_CONSUMER_API_CONTRACT",
            "external_consumer_smoke_pass": first == vec![0, 1, 2, 3],
            "multi_call_state_isolation_pass": first == second && first == third && other_path == vec![1, 2, 3, 4],
            "determinism_replay_pass": first == second,
            "serde_roundtrip_pass": first == roundtrip_path,
            "no_global_state_leak_pass": first == third,
            "concurrent_calls_safe_pass": concurrent_ok,
            "reachable_seed_bug_regression_pass": first == vec![0, 1, 2, 3],
            "source_out_of_bounds_rejected": source_ok,
            "edge_out_of_bounds_rejected": edge_ok,
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_036_external_route_grammar_consumer/dev",
    );
