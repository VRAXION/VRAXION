//! Runner-local ordered breadcrumb constructability probe.
//!
//! 022 asks whether ordered successor route-token fields can be repaired,
//! completed, or grown by simple search/mutation-style operators.
...
enum ArmKind {
    HandBuiltSuccessorUpperBound,
    DamageRepair1Successor,
    DamageRepair2Successor,
    DamageRepair4Successor,
    DamageRepair8Successor,
    PartialSeed25Completion,
    PartialSeed50Completion,
    PartialSeed75Completion,
    RandomGrowthBaseline,
    CanonicalMutationOnly,
    AddSuccessorBreadcrumbOperator,
    AddSuccessorNoReciprocalPenalty,
    AddSuccessorDeliveryReward,
    DenseRouteFieldPrune,
    RandomSuccessorControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::HandBuiltSuccessorUpperBound => "HAND_BUILT_SUCCESSOR_UPPER_BOUND",
            Self::DamageRepair1Successor => "DAMAGE_REPAIR_SUCCESSOR_1",
            Self::DamageRepair2Successor => "DAMAGE_REPAIR_SUCCESSOR_2",
            Self::DamageRepair4Successor => "DAMAGE_REPAIR_SUCCESSOR_4",
            Self::DamageRepair8Successor => "DAMAGE_REPAIR_SUCCESSOR_8",
            Self::PartialSeed25Completion => "PARTIAL_SEED_COMPLETION_25",
            Self::PartialSeed50Completion => "PARTIAL_SEED_COMPLETION_50",
            Self::PartialSeed75Completion => "PARTIAL_SEED_COMPLETION_75",
            Self::RandomGrowthBaseline => "RANDOM_GROWTH_BASELINE",
            Self::CanonicalMutationOnly => "CANONICAL_MUTATION_ONLY",
            Self::AddSuccessorBreadcrumbOperator => "ADD_SUCCESSOR_BREADCRUMB_OPERATOR",
            Self::AddSuccessorNoReciprocalPenalty => "ADD_SUCCESSOR_BREADCRUMB_NO_RECIPROCAL_PENALTY",
            Self::AddSuccessorDeliveryReward => "ADD_SUCCESSOR_BREADCRUMB_DELIVERY_REWARD",
            Self::DenseRouteFieldPrune => "DENSE_ROUTE_FIELD_PRUNE",
            Self::RandomSuccessorControl => "RANDOM_SUCCESSOR_CONTROL",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(
            self,
            Self::HandBuiltSuccessorUpperBound
        )
    }
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomSuccessorControl | Self::RandomPhaseRuleControl
        )
    }
}
...
enum GraphMode {
    PerfectSuccessor,
    DamagedRepair(usize),
    PartialCompletion(usize),
    RandomGrowth,
    CanonicalDistance,
    AddSuccessorOperator,
    DeliveryRewardSearch,
    DensePrune,
    RandomSuccessor,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_022_ORDERED_BREADCRUMB_CONSTRUCTABILITY",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!("../../docs/research/STABLE_LOOP_PHASE_LOCK_022_ORDERED_BREADCRUMB_CONSTRUCTABILITY_CONTRACT.md"),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::CanonicalMutationOnly {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::CanonicalMutationOnly {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::CanonicalMutationOnly,
        ArmKind::HandBuiltSuccessorUpperBound,
        ArmKind::DamageRepair1Successor,
        ArmKind::DamageRepair2Successor,
        ArmKind::DamageRepair4Successor,
        ArmKind::DamageRepair8Successor,
        ArmKind::PartialSeed25Completion,
        ArmKind::PartialSeed50Completion,
        ArmKind::PartialSeed75Completion,
        ArmKind::RandomGrowthBaseline,
        ArmKind::AddSuccessorBreadcrumbOperator,
        ArmKind::AddSuccessorNoReciprocalPenalty,
        ArmKind::AddSuccessorDeliveryReward,
        ArmKind::DenseRouteFieldPrune,
        ArmKind::RandomSuccessorControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::HandBuiltSuccessorUpperBound => GraphMode::PerfectSuccessor,
        ArmKind::DamageRepair1Successor => GraphMode::DamagedRepair(1),
        ArmKind::DamageRepair2Successor => GraphMode::DamagedRepair(2),
        ArmKind::DamageRepair4Successor => GraphMode::DamagedRepair(4),
        ArmKind::DamageRepair8Successor => GraphMode::DamagedRepair(8),
        ArmKind::PartialSeed25Completion => GraphMode::PartialCompletion(25),
        ArmKind::PartialSeed50Completion => GraphMode::PartialCompletion(50),
        ArmKind::PartialSeed75Completion => GraphMode::PartialCompletion(75),
        ArmKind::RandomGrowthBaseline => GraphMode::RandomGrowth,
        ArmKind::CanonicalMutationOnly => GraphMode::CanonicalDistance,
        ArmKind::AddSuccessorBreadcrumbOperator => GraphMode::AddSuccessorOperator,
        ArmKind::AddSuccessorNoReciprocalPenalty => GraphMode::AddSuccessorOperator,
        ArmKind::AddSuccessorDeliveryReward => GraphMode::DeliveryRewardSearch,
        ArmKind::DenseRouteFieldPrune => GraphMode::DensePrune,
        ArmKind::RandomSuccessorControl => GraphMode::RandomSuccessor,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomSuccessorControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn build_graph(mode: GraphMode, case: &Case, seed: u64) -> DirectedGraph {
    ...
    match mode {
        ...
        GraphMode::DamagedRepair(breaks) => {
            let path = ordered_successor_path(case);
            let damaged = remove_successor_links(&path, breaks);
            let repaired = repair_from_ordered_field(case, &damaged);
            for pair in repaired.windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::PartialCompletion(percent) => {
            let completed = complete_from_partial_seed(case, percent);
            for pair in completed.windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::RandomGrowth => {
            for pair in random_breadcrumb_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::AddSuccessorOperator => {
            for pair in public_breadcrumb_bfs_path(case, false, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::DeliveryRewardSearch => {
            for pair in phase_aware_delivery_reward_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::DensePrune => {
            let dense = dense_breadcrumb_edges(case);
            for (from, to) in prune_dense_to_ordered_path(case, &dense) {
                add_edge(&mut edges, &mut seen, from, to);
            }
        }
        GraphMode::RandomSuccessor => {
            for pair in random_breadcrumb_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_SUCCESSOR_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
        ) && r.phase_final_accuracy >= 0.75
    });
    if get("HAND_BUILT_SUCCESSOR_UPPER_BOUND").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("HAND_BUILT_SUCCESSOR_UPPER_BOUND_REPRODUCED".to_string());
    }
    if ["DAMAGE_REPAIR_SUCCESSOR_1", "DAMAGE_REPAIR_SUCCESSOR_2", "DAMAGE_REPAIR_SUCCESSOR_4"]
        .iter()
        .any(|name| get(name).map(|r| r.positive).unwrap_or(false))
    {
        verdicts.insert("SUCCESSOR_FIELD_REPAIRABLE".to_string());
    }
    if ["PARTIAL_SEED_COMPLETION_25", "PARTIAL_SEED_COMPLETION_50", "PARTIAL_SEED_COMPLETION_75"]
        .iter()
        .any(|name| get(name).map(|r| r.positive).unwrap_or(false))
    {
        verdicts.insert("SUCCESSOR_FIELD_COMPLETABLE_FROM_PARTIAL".to_string());
    }
    if get("RANDOM_GROWTH_BASELINE").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("SUCCESSOR_FIELD_GROWS_FROM_RANDOM".to_string());
    }
    if get("ADD_SUCCESSOR_BREADCRUMB_OPERATOR").map(|r| r.positive).unwrap_or(false)
        || get("ADD_SUCCESSOR_BREADCRUMB_DELIVERY_REWARD").map(|r| r.positive).unwrap_or(false)
    {
        verdicts.insert("SUCCESSOR_OPERATOR_REQUIRED".to_string());
        verdicts.insert("ROUTE_IDENTITY_CONSTRUCTABILITY_POSITIVE".to_string());
    }
    if !get("CANONICAL_MUTATION_ONLY").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("CANONICAL_MUTATION_INSUFFICIENT".to_string());
    }
    if get("DENSE_ROUTE_FIELD_PRUNE").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("DENSE_ROUTE_FIELD_PRUNABLE".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("ROUTE_CONSTRUCTABILITY_OVERPOWERS_CONTROL".to_string());
    } else {
        verdicts.insert("RANDOM_CONTROL_FAILS".to_string());
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_022_ORDERED_BREADCRUMB_CONSTRUCTABILITY Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("022 is an ordered-successor constructability diagnostic only. Delivery-reward search arms may use supervised evaluation signal and do not prove production routing. It does not claim production architecture, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_locality_audit(cfg: &Config) -> std::io::Result<()> {
    let audit = json!({
        ...
        "direct_output_leak_rate": 0.0,
        "diagnostic_upper_bound_arms": [
            "HAND_BUILT_SUCCESSOR_UPPER_BOUND"
        ],
        "constructability_arms": [
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from("target/pilot_wave/stable_loop_phase_lock_022_ordered_breadcrumb_constructability/dev");
