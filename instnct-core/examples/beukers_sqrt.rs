//! Square root variants in Beukers gate
//!
//! √ = geometric mean, softer than raw multiplication
//!
//! Run: cargo run --example beukers_sqrt --release
...
fn run(corpus:&[u8],split:usize,name:&str,gate:&dyn Fn(f32,f32,f32)->f32,n_proj:usize)->f64{
    ...
    for ep in 0..max_ep{
        ...
        for _ in 0..samples{
            ...
            let mut pv=vec![vec![0.0f32;nf];n_proj];
            for p in 0..n_proj{for f in 0..nf{pv[p][f]=bs[p][f];
                for ki in 0..k{let pos=mask_pos as i32+ki as i32-hk;
                    if pos>=0&&(pos as usize)<ctx{for d in 0..DIM{pv[p][f]+=ws[p][f][ki*DIM+d]*emb[pos as usize][d];}}}}}
            let mut co=vec![0.0f32;nf];
            for f in 0..nf{let c=if n_proj>2{pv[2][f]}else{0.0};
                co[f]=gate(pv[0][f],pv[1][f],c).max(-10.0).min(10.0);}
            ...
            for f in 0..nf{let eps_g=0.01;
                for p in 0..n_proj{let old=pv[p][f];
                    pv[p][f]=old+eps_g;let c=if n_proj>2{pv[2][f]}else{0.0};
                    let cp=gate(pv[0][f],pv[1][f],c).max(-10.0).min(10.0);
                    pv[p][f]=old-eps_g;let c=if n_proj>2{pv[2][f]}else{0.0};
                    let cm=gate(pv[0][f],pv[1][f],c).max(-10.0).min(10.0);
                    pv[p][f]=old;
                    let g=dc[f]*(cp-cm)/(2.0*eps_g);
                    for ki in 0..k{let pos=mask_pos as i32+ki as i32-hk;
                        if pos>=0&&(pos as usize)<ctx{let pi=pos as usize;
                            for d in 0..DIM{if pi!=mask_pos{embed[chunk[pi]as usize][d]-=lr*g*ws[p][f][ki*DIM+d]*0.15/n_proj as f32;}
                                ws[p][f][ki*DIM+d]-=lr*g*emb[pi][d];}}}
                    bs[p][f]-=lr*g;}}
        }
        if ep%50==0{
            let eval=|start:usize,end:usize|->f64{let mut rng3=Rng::new(999);let mut ok=0usize;let mut tot=0usize;
                for _ in 0..1000{if end<start+ctx+1{break;}let off=rng3.range(start,end.saturating_sub(ctx+1));
                    ...
                    let emb:Vec<[f32;DIM]>=(0..ctx).map(|i|if i==mask_pos{[0.0;DIM]}else{embed[chunk[i]as usize]}).collect();
                    let mut co=vec![0.0f32;nf];
                    for f in 0..nf{let mut pp=vec![0.0f32;n_proj];
                        for p in 0..n_proj{pp[p]=bs[p][f];for ki in 0..k{let pos=mask_pos as i32+ki as i32-hk;
                            if pos>=0&&(pos as usize)<ctx{for d in 0..DIM{pp[p]+=ws[p][f][ki*DIM+d]*emb[pos as usize][d];}}}}
                        let c=if n_proj>2{pp[2]}else{0.0};
                        co[f]=gate(pp[0],pp[1],c).max(-10.0).min(10.0);}
...
fn main(){
    ...
    println!("=== SQRT VARIANTS ===\n");

    // Baselines
    run(&corpus,split,"Beukers ab/(1+|ab|)",
        &|a,b,_|{let p=a*b;p/(1.0+p.abs())},2);

    // Pure sqrt variants
    run(&corpus,split,"Geometric mean: sign(ab)√|ab|",
        &|a,b,_|{let ab=a*b;ab.signum()*(ab.abs()+0.01).sqrt()},2);

    run(&corpus,split,"√ normalized: sign(ab)√|ab|/(1+√|ab|)",
        &|a,b,_|{let ab=a*b;let s=ab.signum()*(ab.abs()+0.01).sqrt();s/(1.0+s.abs())},2);

    // Sqrt + division (3-proj)
    run(&corpus,split,"√arith: sign(ab)√|ab|/(|c|+ε)",
        &|a,b,c|{let ab=a*b;ab.signum()*(ab.abs()+0.01).sqrt()/(c.abs()+0.5)},3);

    // Ultimate: ab×exp / √|c| (multiply + exp + sqrt-divide)
    run(&corpus,split,"ultimate: ab×exp(-|ab|)/√(|c|+ε)",
        &|a,b,c|{let ab=a*b;ab*(-ab.abs()*0.1).exp()/(c.abs()+0.5).sqrt()},3);

    // Full arith baseline for comparison
    run(&corpus,split,"full arith: ab/(|c|+ε)",
        &|a,b,c|{let ab=a*b;ab/(c.abs()+0.5)},3);

    // Learnable power: sign(ab)|ab|^α/(|c|+ε)
    run(&corpus,split,"power arith: sign(ab)|ab|^0.7/(|c|+ε)",
        &|a,b,c|{let ab=a*b;ab.signum()*(ab.abs()+0.01).powf(0.7)/(c.abs()+0.5)},3);
