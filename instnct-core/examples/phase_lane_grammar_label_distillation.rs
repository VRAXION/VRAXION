//! Runner-local grammar-label distillation probe.
//!
//! 028 tests whether the route-grammar labels that made 027 work can be
//! generated or distilled from weaker teacher sources.
...
enum ArmKind {
    ...
    SelfSupervisedDeliveryGrammar027Baseline,
    DensePruneTeacherTraceDistillation,
    ShortRouteTeacherDistillation,
    CounterfactualCorruptionLabels,
    SyntheticBranchCycleLabels,
    SyntheticSuccessorValidityLabels,
    SyntheticContinuityLabels,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SelfSupervisedDeliveryGrammar027Baseline => {
                "SELF_SUPERVISED_DELIVERY_GRAMMAR_027_BASELINE"
            }
            Self::DensePruneTeacherTraceDistillation => "DENSE_PRUNE_TEACHER_TRACE_DISTILLATION",
            Self::ShortRouteTeacherDistillation => "SHORT_ROUTE_TEACHER_DISTILLATION",
            Self::CounterfactualCorruptionLabels => "COUNTERFACTUAL_CORRUPTION_LABELS",
            Self::SyntheticBranchCycleLabels => "SYNTHETIC_BRANCH_CYCLE_LABELS",
            Self::SyntheticSuccessorValidityLabels => "SYNTHETIC_SUCCESSOR_VALIDITY_LABELS",
            Self::SyntheticContinuityLabels => "SYNTHETIC_CONTINUITY_LABELS",
            Self::ShortToLongCurriculum => "SHORT_TO_LONG_CURRICULUM",
            Self::TeacherStudentSelfTraining => "TEACHER_STUDENT_SELF_TRAINING",
            Self::MixedWeakLabelDistillation => "MIXED_WEAK_LABEL_DISTILLATION",
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomLabelControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_028_GRAMMAR_LABEL_DISTILLATION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_028_GRAMMAR_LABEL_DISTILLATION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::SelfSupervisedDeliveryGrammar027Baseline,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row =
                                if arm == ArmKind::SelfSupervisedDeliveryGrammar027Baseline {
                                    baseline.clone()
                                } else {
                                    eval_bucket(
                                        arm,
                            ...
                            if matches!(
                                arm,
                                ArmKind::DensePruneTeacherTraceDistillation
                                    | ArmKind::ShortRouteTeacherDistillation
                            ) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::CounterfactualCorruptionLabels
                                    | ArmKind::SyntheticBranchCycleLabels
                                    | ArmKind::SyntheticSuccessorValidityLabels
                                    | ArmKind::SyntheticContinuityLabels
                            ) {
                                append_jsonl(
                            ...
                            if arm == ArmKind::ShortToLongCurriculum {
                                append_jsonl(&cfg.out.join("curriculum_metrics.jsonl"), &row)?;
                            }
                            if arm == ArmKind::TeacherStudentSelfTraining {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::SyntheticSuccessorValidityLabels,
        ArmKind::SyntheticContinuityLabels,
        ArmKind::ShortToLongCurriculum,
        ArmKind::TeacherStudentSelfTraining,
        ArmKind::MixedWeakLabelDistillation,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::SelfSupervisedDeliveryGrammar027Baseline => GraphMode::DeliveryOnly,
        ArmKind::DensePruneTeacherTraceDistillation => GraphMode::DensePruneTeacherTrace,
        ArmKind::ShortRouteTeacherDistillation => GraphMode::ShortRouteTeacher,
        ArmKind::CounterfactualCorruptionLabels => GraphMode::CounterfactualCorruption,
        ArmKind::SyntheticBranchCycleLabels => GraphMode::SyntheticBranchCycle,
        ArmKind::SyntheticSuccessorValidityLabels => GraphMode::SyntheticSuccessorValidity,
        ArmKind::SyntheticContinuityLabels => GraphMode::SyntheticContinuity,
        ArmKind::ShortToLongCurriculum => GraphMode::ShortToLongCurriculum,
        ArmKind::TeacherStudentSelfTraining => GraphMode::TeacherStudentSelfTraining,
        ArmKind::MixedWeakLabelDistillation => GraphMode::MixedWeakLabelDistillation,
        ArmKind::RandomLabelControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomLabelControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::ShortToLongCurriculum
        | GraphMode::TeacherStudentSelfTraining
        | GraphMode::MixedWeakLabelDistillation => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_LABEL_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    if distillation_positive {
        verdicts.insert("GRAMMAR_LABEL_DISTILLATION_POSITIVE".to_string());
    }
    if get("DENSE_PRUNE_TEACHER_TRACE_DISTILLATION")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DENSE_PRUNE_TEACHER_TRACES_WORK".to_string());
    }
    if get("COUNTERFACTUAL_CORRUPTION_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SYNTHETIC_CORRUPTIONS_WORK".to_string());
    }
    if get("SHORT_TO_LONG_CURRICULUM")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SHORT_ROUTE_CURRICULUM_TRANSFERS".to_string());
    }
    if get("TEACHER_STUDENT_SELF_TRAINING")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("TEACHER_STUDENT_SELF_TRAINING_WORKS".to_string());
    }
    if get("MIXED_WEAK_LABEL_DISTILLATION")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MIXED_WEAK_LABELS_WORK".to_string());
    }
    if !get("SELF_SUPERVISED_DELIVERY_GRAMMAR_027_BASELINE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SELF_SUPERVISED_DELIVERY_STILL_INSUFFICIENT".to_string());
    }
    ...
    if distillation_positive {
        verdicts.insert("ORDER_OBJECTIVE_DISTILLED_FROM_WEAK_LABELS".to_string());
    } else {
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_028_GRAMMAR_LABEL_DISTILLATION Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Retained succ | Order | Label P | Label R | FPR | FNR | Missing succ | Branch | Cycle | Continuity | Reach | Gate collapse | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("028 is a grammar-label distillation diagnostic. Passing weak-label arms show that the 027 route grammar can be sourced from weaker teachers; failing arms mean an external grammar teacher is still required. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_locality_audit(cfg: &Config) -> std::io::Result<()> {
    let audit = json!({
        ...
        "grammar_label_distillation_arms": [
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_028_grammar_label_distillation/dev",
    );
