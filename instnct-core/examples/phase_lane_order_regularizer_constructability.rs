//! Runner-local order-regularizer constructability probe.
//!
//! 026 tests whether the route-order constraints that made 025 work can be
//! approximated by runner-local critics instead of being treated only as a
//! hand-specified upper-bound objective.
...
enum ArmKind {
    ...
    SuccessorConsistencyOnly,
    BranchPenaltyOnly,
    CyclePenaltyOnly,
    RouteContinuityOnly,
    FamilyMinAdversarialOnly,
    PairwiseOrderCritic,
    LocalBranchCycleCritic,
    SourceTargetReachabilityCritic,
    LearnedOrderCriticFromDenseExamples,
    SelfSupervisedOrderCriticFromDelivery,
    RandomCriticControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SuccessorConsistencyOnly => "SUCCESSOR_CONSISTENCY_ONLY",
            Self::BranchPenaltyOnly => "BRANCH_PENALTY_ONLY",
            Self::CyclePenaltyOnly => "CYCLE_PENALTY_ONLY",
            Self::RouteContinuityOnly => "ROUTE_CONTINUITY_ONLY",
            Self::FamilyMinAdversarialOnly => "FAMILY_MIN_ADVERSARIAL_ONLY",
            Self::PairwiseOrderCritic => "PAIRWISE_ORDER_CRITIC",
            Self::LocalBranchCycleCritic => "LOCAL_BRANCH_CYCLE_CRITIC",
            Self::SourceTargetReachabilityCritic => "SOURCE_TARGET_REACHABILITY_CRITIC",
            Self::LearnedOrderCriticFromDenseExamples => "LEARNED_ORDER_CRITIC_FROM_DENSE_EXAMPLES",
            Self::SelfSupervisedOrderCriticFromDelivery => {
                "SELF_SUPERVISED_ORDER_CRITIC_FROM_DELIVERY"
            }
            Self::RandomCriticControl => "RANDOM_CRITIC_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomCriticControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_026_ORDER_REGULARIZER_CONSTRUCTABILITY",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_026_ORDER_REGULARIZER_CONSTRUCTABILITY_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::SelfSupervisedOrderCriticFromDelivery,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::SelfSupervisedOrderCriticFromDelivery {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::SelfSupervisedOrderCriticFromDelivery {
                            ...
                            append_jsonl(&cfg.out.join("critic_metrics.jsonl"), &row)?;
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::SuccessorConsistencyOnly,
        ArmKind::BranchPenaltyOnly,
        ArmKind::CyclePenaltyOnly,
        ArmKind::RouteContinuityOnly,
        ArmKind::FamilyMinAdversarialOnly,
        ArmKind::PairwiseOrderCritic,
        ArmKind::LocalBranchCycleCritic,
        ArmKind::SourceTargetReachabilityCritic,
        ArmKind::LearnedOrderCriticFromDenseExamples,
        ArmKind::RandomCriticControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::SuccessorConsistencyOnly => GraphMode::SuccessorConsistency,
        ArmKind::BranchPenaltyOnly => GraphMode::BranchPenalty,
        ArmKind::CyclePenaltyOnly => GraphMode::CyclePenalty,
        ArmKind::RouteContinuityOnly => GraphMode::RouteContinuity,
        ArmKind::FamilyMinAdversarialOnly => GraphMode::FamilyMinAdversarial,
        ArmKind::PairwiseOrderCritic => GraphMode::PairwiseOrderCritic,
        ArmKind::LocalBranchCycleCritic => GraphMode::LocalBranchCycleCritic,
        ArmKind::SourceTargetReachabilityCritic => GraphMode::SourceTargetAnchorOrderPrune,
        ArmKind::LearnedOrderCriticFromDenseExamples => GraphMode::LearnedDenseOrderCritic,
        ArmKind::SelfSupervisedOrderCriticFromDelivery => GraphMode::DeliveryOnly,
        ArmKind::RandomCriticControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomCriticControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_CRITIC_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    let constructability_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
            && !r.arm.starts_with("RANDOM_")
            && r.arm != "HAND_ORDER_REGULARIZER_REFERENCE"
    });
    if constructability_positive {
        verdicts.insert("ORDER_REGULARIZER_CONSTRUCTABILITY_POSITIVE".to_string());
    }
    if get("SUCCESSOR_CONSISTENCY_ONLY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SUCCESSOR_CONSISTENCY_SUFFICIENT".to_string());
    }
    if get("LOCAL_BRANCH_CYCLE_CRITIC")
        .map(|r| r.signal && !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCH_CYCLE_CRITIC_HAS_PARTIAL_SIGNAL".to_string());
    }
    if !get("SELF_SUPERVISED_ORDER_CRITIC_FROM_DELIVERY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SELF_SUPERVISED_ORDER_CRITIC_INSUFFICIENT".to_string());
    }
    if get("SOURCE_TARGET_REACHABILITY_CRITIC")
        .map(|r| r.signal)
        .unwrap_or(false)
    {
        verdicts.insert("ROUTE_CONTINUITY_REQUIRED".to_string());
    }
    if get("FAMILY_MIN_ADVERSARIAL_ONLY")
        .map(|r| r.signal && !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FAMILY_MIN_ADVERSARIAL_REQUIRED".to_string());
    }
    if get("LEARNED_ORDER_CRITIC_FROM_DENSE_EXAMPLES")
        .map(|r| r.signal)
        .unwrap_or(false)
    {
        verdicts.insert("LEARNED_ORDER_CRITIC_HAS_SIGNAL".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("ORDER_CRITIC_CONTROL_CONTAMINATION".to_string());
    } else {
        verdicts.insert("RANDOM_CRITIC_CONTROL_FAILS".to_string());
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_026_ORDER_REGULARIZER_CONSTRUCTABILITY Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Retained succ | Order | Critic P | Critic R | FPR | FNR | Missing succ | Branch | Cycle | Continuity | Reach | Gate collapse | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("026 is an order-regularizer constructability diagnostic. Hand-reference success only reproduces 025 and does not prove autonomous routing. A public/learned critic must pass controls before any constructability claim. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_locality_audit(cfg: &Config) -> std::io::Result<()> {
    let audit = json!({
        ...
        "order_regularizer_constructability_arms": [
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_026_order_regularizer_constructability/dev",
    );
