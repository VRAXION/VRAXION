//! Sweep 3: ListNet vs INSTNCT head-to-head at H=256 and H=2048
//!
//! Run: cargo run --example overnight_h2h --release
...
// ---- ListNet (copy from overnight_listnet_sweep) ----
...
impl ListNet {
    ...
    fn propagate(&mut self, input: &[i8], tick: usize) {
        ...
        for i in 0..h {
            ...
            if self.charge[i] * 10 >= te { self.activation[i] = self.polarity[i]; self.charge[i] = 0; } else { self.activation[i] = 0; }
        }
    }
    fn readout(&self, nc: usize) -> Vec<f64> {
        let mut out = vec![0.0f64; nc]; let zl = self.h - self.output_start; if zl == 0 || nc == 0 { return out; }
    ...
    fn remove_edge(&mut self, rng: &mut impl Rng) -> bool {
        ...
        for ri in 0..self.topology.len() { let e = self.topology[ri].len() - 1; if c + e > pick { let ti = 1 + (pick - c); self.topology[ri].remove(ti); if self.topology[ri].len() <= 1 { self.topology.remove(ri); } return true; } c += e; } false
    }
    fn rewire(&mut self, rng: &mut impl Rng) -> bool {
        let total = self.edge_count(); if total == 0 { return false; } let pick = rng.gen_range(0..total); let mut c = 0;
        for ri in 0..self.topology.len() { let e = self.topology[ri].len() - 1; if c + e > pick { let ti = 1 + (pick - c); let source = self.topology[ri][0]; let nt = rng.gen_range(0..self.h) as u16; if nt == source { return false; } self.topology[ri].remove(ti); if self.topology[ri].len() <= 1 { self.topology.remove(ri); } return self.add_edge(source, nt); } c += e; } false
    }
...
fn ln_eval_cos(net: &mut ListNet, corpus: &[u8], tokens: usize, rng: &mut StdRng, sdr: &[Vec<i8>], bigram: &[Vec<f64>]) -> f64 {
...
fn ln_eval_acc(net: &mut ListNet, corpus: &[u8], len: usize, rng: &mut StdRng, sdr: &[Vec<i8>]) -> f64 {
...
fn main() {
    ...
    let h_max = 2048;
    let sdr_ln: Vec<Vec<i8>> = (0..VOCAB).map(|sym| {
    ...
    let h_configs = [256usize, 2048];

    println!("=== SWEEP 3: ListNet vs INSTNCT Head-to-Head ===");
    println!("{}s/seed | {} seeds | edge_cap={} | Corpus: {} bytes\n", WALL_CLOCK_SECS, SEEDS.len(), EDGE_CAP, corpus.len());

    for &h in &h_configs {
        println!("--- H={} ---", h);
        println!("{:>10} {:>8} {:>8} {:>8} {:>8} {:>8}", "method", "edges", "steps", "step/s", "best%", "mean%");
        ...
        // --- ListNet ---
        {
            ...
            for &seed in &SEEDS {
                let sdr_t: Vec<Vec<i8>> = sdr_ln.iter().map(|p| p[..h].to_vec()).collect();
                ...
                while Instant::now() < deadline {
                    let sr = eval_rng.clone(); let before = ln_eval_cos(&mut net, &corpus, 20, &mut eval_rng, &sdr_t, &bigram); eval_rng = sr;
                    let genome = net.save(); let mutated = net.mutate(&mut rng, EDGE_CAP);
                    if !mutated { let _ = ln_eval_cos(&mut net, &corpus, 20, &mut eval_rng, &sdr_t, &bigram); steps += 1; continue; }
                    let after = ln_eval_cos(&mut net, &corpus, 20, &mut eval_rng, &sdr_t, &bigram);
                    if after < before { net.restore(genome); } steps += 1;
                }
                let acc = ln_eval_acc(&mut net, &corpus, 500, &mut eval_rng, &sdr_t);
            ...
            println!("{:>10} {:>8} {:>8} {:>8.0} {:>7.1}% {:>7.1}%", "ListNet", me, ms, ms as f64 / WALL_CLOCK_SECS as f64, best * 100.0, mean * 100.0);
        }
        ...
        // --- INSTNCT (1+9 jackpot) ---
        {
            ...
            for &seed in &SEEDS {
                ...
                let deadline = Instant::now() + Duration::from_secs(WALL_CLOCK_SECS);
                while Instant::now() < deadline {
                    evolution_step_jackpot(&mut net, &mut proj, &mut rng, &mut eval_rng,
                        |net, proj, eval_rng| { eval_smooth(net, proj, &corpus, 20, eval_rng, &sdr, &prop_cfg, output_start, neuron_count, &bigram) },
                        &evo_config, 9);
                    steps += 1;
                }
                let acc = eval_accuracy(&mut net, &proj, &corpus, 500, &mut eval_rng, &sdr, &prop_cfg, output_start, neuron_count);
                all_acc.push(acc); all_steps.push(steps); all_edges.push(net.edge_count());
            }
            let best = all_acc.iter().cloned().fold(0.0f64, f64::max);
            let mean = all_acc.iter().sum::<f64>() / all_acc.len() as f64;
            let ms = all_steps.iter().sum::<usize>() / all_steps.len();
            let me = all_edges.iter().sum::<usize>() / all_edges.len();
            println!("{:>10} {:>8} {:>8} {:>8.0} {:>7.1}% {:>7.1}%", "INSTNCT", me, ms, ms as f64 / WALL_CLOCK_SECS as f64, best * 100.0, mean * 100.0);
        }
        println!();
    }
}
