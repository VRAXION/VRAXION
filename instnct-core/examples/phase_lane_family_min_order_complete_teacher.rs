//! Runner-local family-min/order-complete grammar teacher probe.
//!
//! 030 tests whether targeted teacher labels for missing successors,
//! incomplete route order, and worst-family collapse can close the gap left by
//! weak-label and generic hard-negative grammar distillation.
...
enum ArmKind {
    ...
    CounterfactualCorruption029Baseline,
    HardNegativeMixed029Baseline,
    MissingSuccessorTargetedTeacher,
    FamilyMinTargetedTeacher,
    OrderCompletionTeacher,
    WorstFamilyReplayTeacher,
    HighAggregateLowFamilyReplay,
    SuccessorCoverageTeacher,
    OrderCompletionPlusFamilyMinTeacher,
    MixedTargetedTeacher,
    RandomTargetedTeacherControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CounterfactualCorruption029Baseline => "COUNTERFACTUAL_CORRUPTION_029_BASELINE",
            Self::HardNegativeMixed029Baseline => "HARD_NEGATIVE_MIXED_029_BASELINE",
            Self::MissingSuccessorTargetedTeacher => "MISSING_SUCCESSOR_TARGETED_TEACHER",
            Self::FamilyMinTargetedTeacher => "FAMILY_MIN_TARGETED_TEACHER",
            Self::OrderCompletionTeacher => "ORDER_COMPLETION_TEACHER",
            Self::WorstFamilyReplayTeacher => "WORST_FAMILY_REPLAY_TEACHER",
            Self::HighAggregateLowFamilyReplay => "HIGH_AGGREGATE_LOW_FAMILY_REPLAY",
            Self::SuccessorCoverageTeacher => "SUCCESSOR_COVERAGE_TEACHER",
            Self::OrderCompletionPlusFamilyMinTeacher => "ORDER_COMPLETION_PLUS_FAMILY_MIN_TEACHER",
            Self::MixedTargetedTeacher => "MIXED_TARGETED_TEACHER",
            Self::RandomTargetedTeacherControl => "RANDOM_TARGETED_TEACHER_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomTargetedTeacherControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_030_FAMILY_MIN_ORDER_COMPLETE_TEACHER",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_030_FAMILY_MIN_ORDER_COMPLETE_TEACHER_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::CounterfactualCorruption029Baseline,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::CounterfactualCorruption029Baseline {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::CounterfactualCorruption029Baseline {
                            ...
                            append_jsonl(&cfg.out.join("targeted_teacher_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("family_min_teacher_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("order_completion_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("grammar_metrics.jsonl"), &row)?;
                            if matches!(
                                arm,
                                ArmKind::CounterfactualCorruption029Baseline
                                    | ArmKind::HardNegativeMixed029Baseline
                            ) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::MissingSuccessorTargetedTeacher
                                    | ArmKind::FamilyMinTargetedTeacher
                                    | ArmKind::OrderCompletionTeacher
                                    | ArmKind::WorstFamilyReplayTeacher
                                    | ArmKind::HighAggregateLowFamilyReplay
                                    | ArmKind::SuccessorCoverageTeacher
                                    | ArmKind::OrderCompletionPlusFamilyMinTeacher
                                    | ArmKind::MixedTargetedTeacher
                            ) {
                                append_jsonl(
                                    &cfg.out.join("targeted_teacher_detail_metrics.jsonl"),
                                    &row,
                                )?;
                            }
                            if arm == ArmKind::WorstFamilyReplayTeacher {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::OrderCompletionPlusFamilyMinTeacher,
        ArmKind::MixedTargetedTeacher,
        ArmKind::RandomTargetedTeacherControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::CounterfactualCorruption029Baseline => GraphMode::CounterfactualCorruption,
        ArmKind::HardNegativeMixed029Baseline => GraphMode::HardNegativeMixed,
        ArmKind::MissingSuccessorTargetedTeacher => GraphMode::TargetedMissingSuccessor,
        ArmKind::FamilyMinTargetedTeacher => GraphMode::TargetedFamilyMin,
        ArmKind::OrderCompletionTeacher => GraphMode::TargetedOrderCompletion,
        ArmKind::WorstFamilyReplayTeacher => GraphMode::TargetedWorstFamilyReplay,
        ArmKind::HighAggregateLowFamilyReplay => GraphMode::TargetedHighAggregateReplay,
        ArmKind::SuccessorCoverageTeacher => GraphMode::TargetedSuccessorCoverage,
        ArmKind::OrderCompletionPlusFamilyMinTeacher => GraphMode::TargetedOrderCompletionFamilyMin,
        ArmKind::MixedTargetedTeacher => GraphMode::TargetedMixedTeacher,
        ArmKind::RandomTargetedTeacherControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomTargetedTeacherControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::TargetedSuccessorCoverage
        | GraphMode::TargetedOrderCompletionFamilyMin
        | GraphMode::TargetedMixedTeacher => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_TARGETED_TEACHER_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    let targeted_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
    ...
    if targeted_positive {
        verdicts.insert("FAMILY_MIN_ORDER_TEACHER_POSITIVE".to_string());
    }
    if get("MISSING_SUCCESSOR_TARGETED_TEACHER")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MISSING_SUCCESSOR_TEACHER_WORKS".to_string());
    }
    if get("FAMILY_MIN_TARGETED_TEACHER")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FAMILY_MIN_TEACHER_WORKS".to_string());
    }
    if get("ORDER_COMPLETION_TEACHER")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ORDER_COMPLETION_TEACHER_WORKS".to_string());
    }
    if get("WORST_FAMILY_REPLAY_TEACHER")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("WORST_FAMILY_REPLAY_WORKS".to_string());
    }
    if get("MIXED_TARGETED_TEACHER")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MIXED_TARGETED_TEACHER_WORKS".to_string());
    }
    if get("COUNTERFACTUAL_CORRUPTION_029_BASELINE")
        .map(|r| !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("GENERIC_HARD_NEGATIVES_STILL_INSUFFICIENT".to_string());
    }
    ...
    if !controls_fail {
        ...
        verdicts.insert("RANDOM_TARGETED_TEACHER_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if targeted_positive {
        verdicts.insert("ORDER_OBJECTIVE_DISTILLED_FROM_TARGETED_TEACHER".to_string());
    } else {
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_030_FAMILY_MIN_ORDER_COMPLETE_TEACHER Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("030 is a family-min/order-complete teacher diagnostic. Passing targeted teacher arms shows that labels aimed at missing successors and worst-family collapse can close the weak-label grammar gap. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_030_family_min_order_complete_teacher/dev",
    );
