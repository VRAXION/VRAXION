//! Beukers gate — context size sweep
//!
//! Does bigger context help? ctx=16,32,48,64 with Beukers nf=96
//! Also: conv kernel size sweep k=3,5,7
//!
//! Run: cargo run --example char_embed_beukers_ctx --release
...
fn run(corpus:&[u8], split:usize, ctx:usize, k:usize, nf:usize) -> f64 {
    let mask_pos=ctx/2;let fan=k*DIM;
    ...
    for ep in 0..max_ep{
        ...
        for _ in 0..samples{
            ...
            for f in 0..nf{let mut v1=b1[f];let mut v2=b2[f];
                for ki in 0..k{let pos=mask_pos as i32+ki as i32-half_k;
...
fn main(){
    ...
    println!("=== BEUKERS CONTEXT + KERNEL SWEEP ===\n");
    println!("  Beukers gate, nf=96 (unless noted)\n");

    // Context size sweep (k=5 fixed)
    run(&corpus,split,16,5,96);
    run(&corpus,split,32,5,96);
    run(&corpus,split,48,5,96);
    run(&corpus,split,64,5,96);

    // Kernel size sweep (ctx=32 fixed)
    run(&corpus,split,32,3,96);
    run(&corpus,split,32,7,96);
    run(&corpus,split,32,9,96);

    println!("\n  Reference: Beukers ctx=32 k=5 nf=96 = 80.1%");
