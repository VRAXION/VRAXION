//! Iso-parameter multi-seed gauntlet: the definitive A/B for α vs Beukers.
//!
//! Question: is α's win (+1.4pp on FineWeb 30MB) a real architectural
//! improvement, or just the +44% parameter-count advantage of its 3rd
//! projection?
//!
//! Answer: test α at nf=88 (which gives ~32,235 params) against B0 at nf=128
//! (32,411 params). Now both have ~essentially the same parameter count.
//! If α iso-param still wins → honest architecture gain. If it loses or
//! ties → the "α win" was capacity, not architecture.
//!
//! δ runs alongside at iso-param-to-B0 as a bonus (it's already 2-proj
//! nf=128, so naturally iso-param with B0).
//!
//! Each variant runs with 3 seeds (42, 1337, 9999) so we can compute
//! mean ± std and know whether differences are within noise.
//!
//! Run from repo root:
//!   cargo run --release --example diag_isoparam_multiseed -- <corpus_path>
//! Default corpus: Alice fixture.
...
fn gate_raw(kind: usize, pv: &[f32]) -> f32 {
    match kind {
        ...
        // α Modulated Beukers (3-proj)
        1 => {
            let p = pv[0] * pv[1];
            let b = p / (1.0 + p.abs());
            b / (1.0 + pv[2].abs())
        }
        // δ Group-normed Beukers (2-proj) — raw is just Beukers; group-norm applied in the outer loop
        2 => { let p = pv[0] * pv[1]; p / (1.0 + p.abs()) }
        _ => pv[0],
    }
}
...
fn run(
    ...
    for ep in 0..max_ep {
        ...
        for _ in 0..samples {
            ...
            let group_denom = if is_groupnorm {
                1.0 + raw.iter().map(|x| x.abs()).sum::<f32>() / nf as f32
            } else { 1.0 };
        ...
        if (ep + 1) % log_every == 0 || ep + 1 == max_ep {
            ...
            println!("[{}-s{}]  ep={} tr={:.1} te={:.1} co_max={:.3}", tag, seed, ep + 1, tr, te, co_max_abs);
        }
    }
...
fn main() {
    ...
    println!("=== ISO-PARAM MULTI-SEED GAUNTLET — corpus = {} ===", corpus_path);
    let corpus = load_corpus(&corpus_path);
    let split = corpus.len() * 80 / 100;
    println!("  corpus len = {} chars, split = {}/{}", corpus.len(), split, corpus.len() - split);
    println!("  300 epochs, 2000 samples/ep, 3 seeds per variant");
    println!();
    println!("  Variants:");
    println!("    B0    = Beukers baseline              nf=128  2-proj  ~32,411 params");
    println!("    α-iso = Modulated Beukers (iso-FLOP)  nf=88   3-proj  ~32,235 params");
    println!("    δ     = Group-norm Beukers            nf=128  2-proj  ~32,411 params");
    println!();
    ...
    rayon::scope(|s| {
        ...
        for &seed in &seeds {
            s.spawn(move |_| {
                let r = run("α-iso", corpus_ref, split, "Modulated Beukers iso", 1, 3, 88, seed);
                results_ref.lock().unwrap().push(r);
            });
        }
        for &seed in &seeds {
            s.spawn(move |_| {
                let r = run("δ", corpus_ref, split, "Group-norm Beukers", 2, 2, 128, seed);
                results_ref.lock().unwrap().push(r);
            });
        }
    });
    ...
    println!("=== RAW RESULTS ===");
    for r in &all {
        println!("  {:<24} seed={:>4}  best_test={:.2}  final_te={:.2}  final_tr={:.2}  co_max={:.3}",
            r.variant, r.seed, r.best_test, r.final_test, r.final_train, r.final_comax);
    }

    println!();
    println!("=== SUMMARY (mean ± std over 3 seeds) ===");
    for var in &["Beukers baseline", "Modulated Beukers iso", "Group-norm Beukers"] {
        let bests: Vec<f64> = all.iter().filter(|r| r.variant == *var).map(|r| r.best_test).collect();
        let comaxes: Vec<f64> = all.iter().filter(|r| r.variant == *var).map(|r| r.final_comax as f64).collect();
        if !bests.is_empty() {
            let p = all.iter().find(|r| r.variant == *var).map(|r| r.params).unwrap_or(0);
            println!("  {:<24} (params={:>6})  best_test = {:.2} ± {:.2}  (seeds: {:?})  co_max_mean={:.3}",
                var, p, mean(&bests), std(&bests),
                all.iter().filter(|r| r.variant == *var).map(|r| format!("{:.2}", r.best_test)).collect::<Vec<_>>(),
                mean(&comaxes));
        }
    }
