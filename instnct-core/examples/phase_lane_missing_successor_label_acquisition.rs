//! Runner-local missing-successor label acquisition probe.
//!
//! 031 tests whether the targeted missing-successor / order-completion labels
//! that worked in 030 can be generated from graph diagnostics instead of being
//! externally supplied.
...
enum ArmKind {
    HandTargetedTeacherReference,
    MissingSuccessorTargetedTeacher030Baseline,
    ReachabilityGapLabels,
    DeadEndBacktraceLabels,
    DeliveryFailureAttributionLabels,
    FrontierExpansionTraceLabels,
    PruneResidualMissingLinkLabels,
    GraphInvariantSuccessorLabels,
    GraphInvariantContinuityLabels,
    MixedAutonomousLabels,
    RandomLabelControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::HandTargetedTeacherReference => "HAND_TARGETED_TEACHER_REFERENCE",
            Self::MissingSuccessorTargetedTeacher030Baseline => {
                "MISSING_SUCCESSOR_TARGETED_TEACHER_030_BASELINE"
            }
            Self::ReachabilityGapLabels => "REACHABILITY_GAP_LABELS",
            Self::DeadEndBacktraceLabels => "DEAD_END_BACKTRACE_LABELS",
            Self::DeliveryFailureAttributionLabels => "DELIVERY_FAILURE_ATTRIBUTION_LABELS",
            Self::FrontierExpansionTraceLabels => "FRONTIER_EXPANSION_TRACE_LABELS",
            Self::PruneResidualMissingLinkLabels => "PRUNE_RESIDUAL_MISSING_LINK_LABELS",
            Self::GraphInvariantSuccessorLabels => "GRAPH_INVARIANT_SUCCESSOR_LABELS",
            Self::GraphInvariantContinuityLabels => "GRAPH_INVARIANT_CONTINUITY_LABELS",
            Self::MixedAutonomousLabels => "MIXED_AUTONOMOUS_LABELS",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(self, Self::HandTargetedTeacherReference)
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_031_MISSING_SUCCESSOR_LABEL_ACQUISITION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_031_MISSING_SUCCESSOR_LABEL_ACQUISITION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::MissingSuccessorTargetedTeacher030Baseline,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row =
                                if arm == ArmKind::MissingSuccessorTargetedTeacher030Baseline {
                                    baseline.clone()
                                } else {
                                    eval_bucket(
                                        arm,
                            ...
                            if matches!(
                                arm,
                                ArmKind::HandTargetedTeacherReference
                                    | ArmKind::MissingSuccessorTargetedTeacher030Baseline
                            ) {
                            ...
                            if matches!(
                                arm,
                                ArmKind::ReachabilityGapLabels
                                    | ArmKind::DeadEndBacktraceLabels
                                    | ArmKind::DeliveryFailureAttributionLabels
                                    | ArmKind::FrontierExpansionTraceLabels
                                    | ArmKind::PruneResidualMissingLinkLabels
                                    | ArmKind::GraphInvariantSuccessorLabels
                                    | ArmKind::GraphInvariantContinuityLabels
                                    | ArmKind::MixedAutonomousLabels
                            ) {
                                append_jsonl(
                                    &cfg.out.join("label_acquisition_metrics.jsonl"),
                                    &row,
                                )?;
                            }
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::HandTargetedTeacherReference => GraphMode::TargetedOrderCompletionFamilyMin,
        ArmKind::MissingSuccessorTargetedTeacher030Baseline => GraphMode::TargetedMissingSuccessor,
        ArmKind::ReachabilityGapLabels => GraphMode::AcquisitionReachabilityGap,
        ArmKind::DeadEndBacktraceLabels => GraphMode::AcquisitionDeadEndBacktrace,
        ArmKind::DeliveryFailureAttributionLabels => GraphMode::AcquisitionDeliveryAttribution,
        ArmKind::FrontierExpansionTraceLabels => GraphMode::AcquisitionFrontierExpansion,
        ArmKind::PruneResidualMissingLinkLabels => GraphMode::AcquisitionPruneResidual,
        ArmKind::GraphInvariantSuccessorLabels => GraphMode::AcquisitionGraphInvariantSuccessor,
        ArmKind::GraphInvariantContinuityLabels => GraphMode::AcquisitionGraphInvariantContinuity,
        ArmKind::MixedAutonomousLabels => GraphMode::AcquisitionMixedAutonomous,
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::AcquisitionGraphInvariantSuccessor
        | GraphMode::AcquisitionGraphInvariantContinuity
        | GraphMode::AcquisitionMixedAutonomous => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if get("HAND_TARGETED_TEACHER_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("HAND_TARGETED_TEACHER_REFERENCE_REPRODUCED".to_string());
    }
    let acquisition_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
            && !r.arm.starts_with("RANDOM_")
            && r.arm != "MISSING_SUCCESSOR_TARGETED_TEACHER_030_BASELINE"
    });
    if acquisition_positive {
        verdicts.insert("MISSING_SUCCESSOR_LABEL_ACQUISITION_POSITIVE".to_string());
    }
    if get("REACHABILITY_GAP_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("REACHABILITY_GAP_LABELS_WORK".to_string());
    }
    if get("DEAD_END_BACKTRACE_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DEAD_END_BACKTRACE_LABELS_WORK".to_string());
    }
    if get("DELIVERY_FAILURE_ATTRIBUTION_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DELIVERY_FAILURE_ATTRIBUTION_WORKS".to_string());
    }
    if get("FRONTIER_EXPANSION_TRACE_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FRONTIER_TRACE_LABELS_WORK".to_string());
    }
    if get("PRUNE_RESIDUAL_MISSING_LINK_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("PRUNE_RESIDUAL_LABELS_WORK".to_string());
    }
    if get("GRAPH_INVARIANT_SUCCESSOR_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("GRAPH_INVARIANT_LABELS_WORK".to_string());
    }
    if get("GRAPH_INVARIANT_CONTINUITY_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("GRAPH_INVARIANT_LABELS_WORK".to_string());
    }
    if get("MIXED_AUTONOMOUS_LABELS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("MIXED_AUTONOMOUS_LABELS_WORK".to_string());
    }
    ...
    if partial_signal && !acquisition_positive {
        verdicts.insert("AUTONOMOUS_LABELS_PARTIAL_SIGNAL".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_031_MISSING_SUCCESSOR_LABEL_ACQUISITION Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("031 is a missing-successor label acquisition diagnostic. Passing autonomous label arms shows that graph diagnostics can generate the order-completion labels that 030 supplied externally. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_031_missing_successor_label_acquisition/dev",
    );
