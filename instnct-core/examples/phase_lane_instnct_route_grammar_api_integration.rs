//! Instnct-core route-grammar API integration probe.
//!
//! 034 tests whether the 033 runner-local route-grammar subsystem can run
//! through a real `instnct-core` experimental API and still hold across
//! generalized route-task families.
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Loop032Baseline => "RUNNER_LOCAL_033_BASELINE",
            ...
            Self::ProductionLikeRouteGrammarApi => "INSTNCT_EXPERIMENTAL_ROUTE_GRAMMAR_API",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_034_INSTNCT_ROUTE_GRAMMAR_API_INTEGRATION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_034_INSTNCT_ROUTE_GRAMMAR_API_INTEGRATION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("api_integration_metrics.jsonl"), &row)?;
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if generalized_positive {
        verdicts.insert("INSTNCT_ROUTE_GRAMMAR_API_INTEGRATION_POSITIVE".to_string());
    }
    ...
    if get("INSTNCT_EXPERIMENTAL_ROUTE_GRAMMAR_API")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("INSTNCT_EXPERIMENTAL_ROUTE_GRAMMAR_API_WORKS".to_string());
    }
    ...
    if !generalized_positive {
        verdicts.insert("INSTNCT_ROUTE_GRAMMAR_API_INTEGRATION_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report
        .push_str("# STABLE_LOOP_PHASE_LOCK_034_INSTNCT_ROUTE_GRAMMAR_API_INTEGRATION Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("034 is an instnct-core experimental API integration diagnostic. Passing arms show that the 033 runner-local loop can run through an instnct-core owned experimental route-grammar module and still apply across generalized toy route-task families. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_034_instnct_route_grammar_api_integration/dev",
    );
