//! Runner-local route-order-aware prune probe.
//!
//! 025 improves the prune objective used on dense candidate route fields so
//! pruning preserves ordered-successor route structure, not just delivery.
...
enum ArmKind {
    ...
    DenseGrow4x024Baseline,
    DeliveryOnlyPrune,
    DeliveryPlusSuccessorConsistency,
    DeliveryPlusBranchPenalty,
    DeliveryPlusCyclePenalty,
    DeliveryPlusRouteContinuity,
    DeliveryPlusFamilyMinAdversarial,
    DeliveryPlusAllOrderRegularizers,
    SourceTargetAnchorPlusOrderPrune,
    RandomDenseControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::DenseGrow4x024Baseline => "DENSE_GROW_4X_024_BASELINE",
            Self::DeliveryOnlyPrune => "DELIVERY_ONLY_PRUNE",
            Self::DeliveryPlusSuccessorConsistency => "DELIVERY_PLUS_SUCCESSOR_CONSISTENCY",
            Self::DeliveryPlusBranchPenalty => "DELIVERY_PLUS_BRANCH_PENALTY",
            Self::DeliveryPlusCyclePenalty => "DELIVERY_PLUS_CYCLE_PENALTY",
            Self::DeliveryPlusRouteContinuity => "DELIVERY_PLUS_ROUTE_CONTINUITY",
            Self::DeliveryPlusFamilyMinAdversarial => "DELIVERY_PLUS_FAMILY_MIN_ADVERSARIAL",
            Self::DeliveryPlusAllOrderRegularizers => "DELIVERY_PLUS_ALL_ORDER_REGULARIZERS",
            Self::SourceTargetAnchorPlusOrderPrune => "SOURCE_TARGET_ANCHOR_PLUS_ORDER_PRUNE",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_025_ROUTE_ORDER_AWARE_PRUNE",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_025_ROUTE_ORDER_AWARE_PRUNE_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::DenseGrow4x024Baseline,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::DenseGrow4x024Baseline {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::DenseGrow4x024Baseline {
                            ...
                            append_jsonl(&cfg.out.join("order_prune_metrics.jsonl"), &row)?;
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::DeliveryOnlyPrune,
        ArmKind::DeliveryPlusSuccessorConsistency,
        ArmKind::DeliveryPlusBranchPenalty,
        ArmKind::DeliveryPlusCyclePenalty,
        ArmKind::DeliveryPlusRouteContinuity,
        ArmKind::DeliveryPlusFamilyMinAdversarial,
        ArmKind::DeliveryPlusAllOrderRegularizers,
        ArmKind::SourceTargetAnchorPlusOrderPrune,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::DenseGrow4x024Baseline => GraphMode::DeliveryOnly,
        ArmKind::DeliveryOnlyPrune => GraphMode::DeliveryOnly,
        ArmKind::DeliveryPlusSuccessorConsistency => GraphMode::SuccessorConsistency,
        ArmKind::DeliveryPlusBranchPenalty => GraphMode::BranchPenalty,
        ArmKind::DeliveryPlusCyclePenalty => GraphMode::CyclePenalty,
        ArmKind::DeliveryPlusRouteContinuity => GraphMode::RouteContinuity,
        ArmKind::DeliveryPlusFamilyMinAdversarial => GraphMode::FamilyMinAdversarial,
        ArmKind::DeliveryPlusAllOrderRegularizers => GraphMode::AllOrderRegularizers,
        ArmKind::SourceTargetAnchorPlusOrderPrune => GraphMode::SourceTargetAnchorOrderPrune,
...
fn build_graph(mode: GraphMode, case: &Case, seed: u64) -> DirectedGraph {
    ...
    match mode {
        ...
        GraphMode::AllOrderRegularizers => {
            for pair in ordered_successor_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::SourceTargetAnchorOrderPrune => {
            let path = if case.public.source.0 <= case.public.target.0
                || case.public.source.1 <= case.public.target.1
            {
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::CyclePenalty
        | GraphMode::FamilyMinAdversarial
        | GraphMode::AllOrderRegularizers => dense_budget_candidate_edges(case, 4).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    if policy_positive {
        verdicts.insert("ROUTE_ORDER_AWARE_PRUNE_POSITIVE".to_string());
    }
    if !get("DELIVERY_ONLY_PRUNE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DELIVERY_ONLY_PRUNE_INSUFFICIENT".to_string());
    }
    if get("DELIVERY_PLUS_SUCCESSOR_CONSISTENCY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SUCCESSOR_CONSISTENCY_REQUIRED".to_string());
    }
    if get("DELIVERY_PLUS_BRANCH_PENALTY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCH_PENALTY_REQUIRED".to_string());
    }
    if get("DELIVERY_PLUS_CYCLE_PENALTY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("CYCLE_PENALTY_REQUIRED".to_string());
    }
    if get("DELIVERY_PLUS_ROUTE_CONTINUITY")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ROUTE_CONTINUITY_REQUIRED".to_string());
    }
    if get("DELIVERY_PLUS_FAMILY_MIN_ADVERSARIAL")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FAMILY_MIN_ADVERSARIAL_REQUIRED".to_string());
    }
    if get("DELIVERY_PLUS_ALL_ORDER_REGULARIZERS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ALL_ORDER_REGULARIZERS_WORK".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("ORDER_PRUNE_CONTROL_CONTAMINATION".to_string());
    } else {
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_025_ROUTE_ORDER_AWARE_PRUNE Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Retained succ | Order | Missing succ | Branch | Cycle | Continuity | Reach | Prune | Gate collapse | Counterfactual | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
        report.push_str(&format!(
            "| {} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.1} | {:.1} | {:.1} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {} | {} |\n",
            row.arm,
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("025 is a route-order-aware prune diagnostic only. Passing arms show that adding order regularizers to dense-field pruning can preserve an ordered successor route in this toy substrate. It does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out =
        PathBuf::from("target/pilot_wave/stable_loop_phase_lock_025_route_order_aware_prune/dev");
