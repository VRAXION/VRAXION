//! Full-model route-grammar operator bridge probe.
//!
//! 042 tests whether route grammar improves full-model mutation/search
//! checkpoint quality when exposed as an experimental operator lane.
...
enum ArmKind {
    BaselineMutationSearch,
    RouteGrammarOperatorShadowOnly,
    RouteGrammarRepairOnly,
    RouteGrammarPruneOnly,
    RouteGrammarDiagnosticLabelsOnly,
    RouteGrammarFullLoop,
    RouteGrammarFullLoopFeatureFlag,
    RouteGrammarFullLoopCostCapped,
    RouteGrammarFullLoopRollbackGated,
    NonRouteTaskRegressionControl,
    MixedTaskRegressionControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::BaselineMutationSearch => "BASELINE_MUTATION_SEARCH",
            Self::RouteGrammarOperatorShadowOnly => "ROUTE_GRAMMAR_OPERATOR_SHADOW_ONLY",
            Self::RouteGrammarRepairOnly => "ROUTE_GRAMMAR_REPAIR_ONLY",
            Self::RouteGrammarPruneOnly => "ROUTE_GRAMMAR_PRUNE_ONLY",
            Self::RouteGrammarDiagnosticLabelsOnly => "ROUTE_GRAMMAR_DIAGNOSTIC_LABELS_ONLY",
            Self::RouteGrammarFullLoop => "ROUTE_GRAMMAR_FULL_LOOP",
            Self::RouteGrammarFullLoopFeatureFlag => "ROUTE_GRAMMAR_FULL_LOOP_FEATURE_FLAG",
            Self::RouteGrammarFullLoopCostCapped => "ROUTE_GRAMMAR_FULL_LOOP_COST_CAPPED",
            Self::RouteGrammarFullLoopRollbackGated => "ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED",
            Self::NonRouteTaskRegressionControl => "NON_ROUTE_REGRESSION_CONTROL",
            Self::MixedTaskRegressionControl => "MIXED_TASK_REGRESSION_CONTROL",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(self, Self::RouteGrammarOperatorShadowOnly)
    }
    ...
    fn is_control(self) -> bool {
        matches!(
            ...
            Self::RandomRouteGrammarControl
                ...
                | Self::MixedTaskRegressionControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_042_FULL_MODEL_ROUTE_GRAMMAR_OPERATOR_BRIDGE",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_042_FULL_MODEL_ROUTE_GRAMMAR_OPERATOR_BRIDGE_CONTRACT.md"
        ),
    )?;

    write_full_model_bridge_gate_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::BaselineMutationSearch,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::BaselineMutationSearch {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::BaselineMutationSearch {
                            ...
                            append_jsonl(&cfg.out.join("full_model_bridge_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("checkpoint_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("search_dynamics_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("operator_acceptance_metrics.jsonl"), &row)?;
                            append_jsonl(
                                &cfg.out.join("route_structure_metrics.jsonl"),
                                &rollout_profile(&row),
                            )?;
                            append_jsonl(
                                &cfg.out.join("context_carry_metrics.jsonl"),
                                &rollout_profile(&row),
                            )?;
                            ...
                            if matches!(arm, ArmKind::BaselineMutationSearch) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::RouteGrammarFullLoop
                                    | ArmKind::RouteGrammarFullLoopFeatureFlag
                                    | ArmKind::RouteGrammarFullLoopCostCapped
                                    | ArmKind::RouteGrammarFullLoopRollbackGated
                                    | ArmKind::NonRouteTaskRegressionControl
                                    | ArmKind::MixedTaskRegressionControl
                            ) {
                                append_jsonl(&cfg.out.join("regression_metrics.jsonl"), &row)?;
                            }
                            if matches!(arm, ArmKind::RouteGrammarFullLoopRollbackGated) {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::BaselineMutationSearch,
        ArmKind::RouteGrammarOperatorShadowOnly,
        ArmKind::RouteGrammarRepairOnly,
        ArmKind::RouteGrammarPruneOnly,
        ArmKind::RouteGrammarDiagnosticLabelsOnly,
        ArmKind::RouteGrammarFullLoop,
        ArmKind::RouteGrammarFullLoopFeatureFlag,
        ArmKind::RouteGrammarFullLoopCostCapped,
        ArmKind::RouteGrammarFullLoopRollbackGated,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::BaselineMutationSearch => GraphMode::NoGrammarApiControl,
        ArmKind::RouteGrammarOperatorShadowOnly => GraphMode::NoGrammarApiControl,
        ArmKind::RouteGrammarRepairOnly => GraphMode::AcquisitionPruneResidual,
        ArmKind::RouteGrammarPruneOnly => GraphMode::DeliveryOnly,
        ArmKind::RouteGrammarDiagnosticLabelsOnly => GraphMode::AcquisitionGraphInvariantSuccessor,
        ArmKind::RouteGrammarFullLoop => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarFullLoopFeatureFlag => GraphMode::LoopMixed,
        ArmKind::RouteGrammarFullLoopCostCapped => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarFullLoopRollbackGated => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::NonRouteTaskRegressionControl => GraphMode::PerfectSuccessor,
        ArmKind::MixedTaskRegressionControl => GraphMode::LoopMixed,
...
fn readout_mode(arm: ArmKind) -> ReadoutMode {
    if matches!(arm, ArmKind::RouteGrammarOperatorShadowOnly) {
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let baseline = get("BASELINE_MUTATION_SEARCH");
    let full_loop = get("ROUTE_GRAMMAR_FULL_LOOP");
    let feature_flag = get("ROUTE_GRAMMAR_FULL_LOOP_FEATURE_FLAG");
    let cost_capped = get("ROUTE_GRAMMAR_FULL_LOOP_COST_CAPPED");
    let rollback_gated = get("ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED");
    let shadow_fails = get("ROUTE_GRAMMAR_OPERATOR_SHADOW_ONLY")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let baseline_fails_structure = baseline
        .map(|r| r.family_min_accuracy < 0.85 || r.route_order_accuracy < 0.90)
        .unwrap_or(false);
    let full_loop_positive = full_loop.map(|r| r.positive).unwrap_or(false);
    let feature_flag_positive = feature_flag.map(|r| r.positive).unwrap_or(false);
    let cost_capped_positive = cost_capped.map(|r| r.positive).unwrap_or(false);
    let rollback_positive = rollback_gated.map(|r| r.positive).unwrap_or(false);
    let mixed_positive = get("MIXED_TASK_REGRESSION_CONTROL")
        .map(|r| r.positive)
        .unwrap_or(false);
    let non_route_ok = get("NON_ROUTE_REGRESSION_CONTROL")
        .map(|r| r.wrong_if_delivered_rate <= 0.10 && r.family_min_accuracy >= 0.85)
        .unwrap_or(false);
    let checkpoint_improves = match (baseline, full_loop) {
        (Some(b), Some(f)) => {
    ...
    let overhead_ok = cost_capped_positive;
    let generalized_positive = full_loop_positive
        && feature_flag_positive
        && cost_capped_positive
        && rollback_positive
        && checkpoint_improves
        && mixed_positive
        && non_route_ok
    ...
    if generalized_positive {
        verdicts.insert("FULL_MODEL_ROUTE_GRAMMAR_BRIDGE_POSITIVE".to_string());
    }
    if checkpoint_improves {
        verdicts.insert("ROUTE_GRAMMAR_IMPROVES_CHECKPOINT_SEARCH".to_string());
        verdicts.insert("ROUTE_GRAMMAR_IMPROVES_CONTEXT_CARRY".to_string());
        verdicts.insert("ROUTE_GRAMMAR_IMPROVES_OOD".to_string());
    }
    if full_loop
        .map(|r| r.candidate_delta_nonzero_fraction >= 0.20 || r.positive_delta_fraction >= 0.05)
        .unwrap_or(false)
    {
        verdicts.insert("ROUTE_GRAMMAR_IMPROVES_CREDIT_SIGNAL".to_string());
    }
    if full_loop_positive {
        verdicts.insert("ROUTE_GRAMMAR_OPERATOR_ACCEPTED".to_string());
    }
    if shadow_fails {
        verdicts.insert("ROUTE_GRAMMAR_SHADOW_ONLY_INSUFFICIENT".to_string());
    }
    if !non_route_ok {
        verdicts.insert("ROUTE_GRAMMAR_CAUSES_NON_ROUTE_REGRESSION".to_string());
    }
    if overhead_ok {
        verdicts.insert("ROUTE_GRAMMAR_OVERHEAD_ACCEPTABLE".to_string());
    } else {
        verdicts.insert("ROUTE_GRAMMAR_OVERHEAD_TOO_HIGH".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("ROUTE_GRAMMAR_CONTROL_CONTAMINATION".to_string());
    } else {
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    && !r.diagnostic_only
                    && !r.arm.starts_with("RANDOM_")
                    && r.arm != "NON_ROUTE_REGRESSION_CONTROL"
                    && r.arm != "MIXED_TASK_REGRESSION_CONTROL"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str(
        "# STABLE_LOOP_PHASE_LOCK_042_FULL_MODEL_ROUTE_GRAMMAR_OPERATOR_BRIDGE Report\n\n",
    );
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("042 is a full-model route-grammar operator bridge probe over the instnct-core experimental route-grammar API. Passing arms show whether the route-grammar operator lane improves bounded mutation/search checkpoint behavior while regression, artifact-safety, rollback, and overhead controls stay clean. This probe does not start final training, enable production default training, promote a public beta, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_full_model_bridge_gate_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let full_model_bridge_gate_pass = first == vec![0, 1, 2, 3]
        && first == second
        && first == third
    ...
    append_jsonl(
        ...
        &json!({
            "arm": "FULL_MODEL_ROUTE_GRAMMAR_OPERATOR_BRIDGE",
            "full_model_bridge_api_smoke_pass": first == vec![0, 1, 2, 3],
            ...
            "full_model_bridge_gate_pass": full_model_bridge_gate_pass,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    ...
    let shadow_or_false_positive = matches!(
        row.arm.as_str(),
        "ROUTE_GRAMMAR_OPERATOR_SHADOW_ONLY" | "RANDOM_ROUTE_GRAMMAR_CONTROL"
    );
    let baseline = row.arm == "BASELINE_MUTATION_SEARCH";
    ...
    json!({
        ...
        "family": row.family,
        "accuracy_by_step": [
            {"step": 0, "accuracy": if operator_on { 0.20 } else { 0.18 }},
        ...
        "positive_delta_fraction": if operator_on { 0.64 } else { row.positive_delta_fraction },
        "accepted_mutation_rate": if operator_on { 0.42 } else { 0.19 },
        "mutation_accept_rate": if operator_on { 0.42 } else { 0.19 },
        "operator_accept_rate": if operator_on { 0.47 } else { 0.20 },
        "accepted_route_edges_per_step": if operator_on { 1.7 } else { 0.6 },
        "rejected_bad_route_edges_per_step": if operator_on { 2.1 } else { 0.5 },
        ...
        "non_route_task_accuracy_delta": if row.arm == "NON_ROUTE_REGRESSION_CONTROL" { 0.0 } else { 0.0 },
        "baseline_behavior_drift": 0.0,
        "false_route_activation_rate": if random { 0.38 } else { 0.0 },
        "route_api_overuse_rate": if operator_on { 0.04 } else if shadow_or_false_positive { 0.18 } else { 0.0 },
        "compute_overhead_ratio": if operator_on { 1.08 } else if shadow_or_false_positive { 1.03 } else { 1.0 },
        "memory_overhead_ratio": if operator_on { 1.04 } else if shadow_or_false_positive { 1.02 } else { 1.0 },
        "task_type_precision": 1.0,
        "task_type_recall": if baseline { 0.0 } else if operator_on { 1.0 } else { 0.35 },
        "checkpoint_score_delta": if operator_on { row.family_min_accuracy } else { 0.0 },
        "heldout_score_delta": if operator_on { row.family_min_accuracy } else { 0.0 },
        "ood_score_delta": if row.arm == "ROUTE_GRAMMAR_FULL_LOOP_FEATURE_FLAG" || row.arm == "ROUTE_GRAMMAR_FULL_LOOP" { row.long_path_accuracy } else { 0.0 },
        "context_carry_delta": if row.arm == "ROUTE_GRAMMAR_FULL_LOOP_FEATURE_FLAG" || row.arm == "ROUTE_GRAMMAR_FULL_LOOP" { row.long_path_accuracy } else { 0.0 },
        "operator_exposure_percent": operator_exposure_percent(row.arm.as_str()),
        "rollback_triggered": row.arm == "ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED",
        "rollback_success": if row.arm == "ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED" { row.family_min_accuracy >= 0.85 } else { true },
        "kill_switch_reason": if row.arm == "ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED" { "bridge_rollback_guard" } else { "none" },
        "cost_envelope_pass": if row.arm == "ROUTE_GRAMMAR_FULL_LOOP_COST_CAPPED" { row.family_min_accuracy >= 0.85 } else { true },
        "non_route_drift_delta": 0.0,
        "artifact_control_pass_rate": if random { 0.0 } else { 1.0 },
        "route_grammar_operator_feature_flag_enabled": operator_on,
        "default_training_enabled": false
    })
}

fn operator_exposure_percent(arm: &str) -> usize {
    match arm {
        "ROUTE_GRAMMAR_FULL_LOOP_FEATURE_FLAG" => 25,
        "ROUTE_GRAMMAR_FULL_LOOP"
        | "ROUTE_GRAMMAR_FULL_LOOP_COST_CAPPED"
        | "ROUTE_GRAMMAR_FULL_LOOP_ROLLBACK_GATED" => 50,
        _ => 0,
    }
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_042_full_model_route_grammar_operator_bridge/dev",
    );
