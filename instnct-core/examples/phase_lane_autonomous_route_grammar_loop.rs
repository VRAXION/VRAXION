//! Runner-local autonomous route-grammar construction loop probe.
//!
//! 032 integrates dense candidate growth, graph-diagnostic label acquisition,
//! route-grammar repair, order-aware prune, and delivery evaluation into one
//! autonomous loop.
...
enum ArmKind {
    ...
    OnePassGrowDiagnosePrune,
    IterativeGrowDiagnosePrune2,
    IterativeGrowDiagnosePrune4,
    IterativeGrowDiagnosePrune8,
    FrontierLabelLoop,
    PruneResidualLabelLoop,
    GraphInvariantLabelLoop,
    MixedLabelLoop,
    NoLabelLoopControl,
    RandomLabelLoopControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::OnePassGrowDiagnosePrune => "ONE_PASS_GROW_DIAGNOSE_PRUNE",
            Self::IterativeGrowDiagnosePrune2 => "ITERATIVE_GROW_DIAGNOSE_PRUNE_2",
            Self::IterativeGrowDiagnosePrune4 => "ITERATIVE_GROW_DIAGNOSE_PRUNE_4",
            Self::IterativeGrowDiagnosePrune8 => "ITERATIVE_GROW_DIAGNOSE_PRUNE_8",
            Self::FrontierLabelLoop => "FRONTIER_LABEL_LOOP",
            Self::PruneResidualLabelLoop => "PRUNE_RESIDUAL_LABEL_LOOP",
            Self::GraphInvariantLabelLoop => "GRAPH_INVARIANT_LABEL_LOOP",
            Self::MixedLabelLoop => "MIXED_LABEL_LOOP",
            Self::NoLabelLoopControl => "NO_LABEL_LOOP_CONTROL",
            Self::RandomLabelLoopControl => "RANDOM_LABEL_LOOP_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomLabelLoopControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_032_AUTONOMOUS_ROUTE_GRAMMAR_CONSTRUCTION_LOOP",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_032_AUTONOMOUS_ROUTE_GRAMMAR_CONSTRUCTION_LOOP_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::OnePassGrowDiagnosePrune,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::OnePassGrowDiagnosePrune {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::OnePassGrowDiagnosePrune {
                            ...
                            append_jsonl(&cfg.out.join("repair_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("prune_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("grammar_metrics.jsonl"), &row)?;
                            if matches!(
                                arm,
                                ArmKind::HandPipelineReference | ArmKind::OnePassGrowDiagnosePrune
                            ) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::FrontierLabelLoop
                                    | ArmKind::PruneResidualLabelLoop
                                    | ArmKind::GraphInvariantLabelLoop
                                    | ArmKind::MixedLabelLoop
                            ) {
                                append_jsonl(&cfg.out.join("label_loop_metrics.jsonl"), &row)?;
                            }
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::OnePassGrowDiagnosePrune,
        ArmKind::IterativeGrowDiagnosePrune2,
        ArmKind::IterativeGrowDiagnosePrune4,
        ArmKind::IterativeGrowDiagnosePrune8,
        ArmKind::FrontierLabelLoop,
        ArmKind::PruneResidualLabelLoop,
        ArmKind::GraphInvariantLabelLoop,
        ArmKind::MixedLabelLoop,
        ArmKind::NoLabelLoopControl,
        ArmKind::RandomLabelLoopControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::OnePassGrowDiagnosePrune => GraphMode::LoopOnePass,
        ArmKind::IterativeGrowDiagnosePrune2 => GraphMode::LoopIterative2,
        ArmKind::IterativeGrowDiagnosePrune4 => GraphMode::LoopIterative4,
        ArmKind::IterativeGrowDiagnosePrune8 => GraphMode::LoopIterative8,
        ArmKind::FrontierLabelLoop => GraphMode::LoopFrontier,
        ArmKind::PruneResidualLabelLoop => GraphMode::LoopPruneResidual,
        ArmKind::GraphInvariantLabelLoop => GraphMode::LoopGraphInvariant,
        ArmKind::MixedLabelLoop => GraphMode::LoopMixed,
        ArmKind::NoLabelLoopControl => GraphMode::LoopNoLabelControl,
        ArmKind::RandomLabelLoopControl => GraphMode::LoopRandomLabelControl,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomLabelLoopControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::LoopMixed
        | GraphMode::LoopNoLabelControl
        | GraphMode::LoopRandomLabelControl => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_LABEL_LOOP_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    let loop_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
    ...
    if loop_positive {
        verdicts.insert("AUTONOMOUS_ROUTE_GRAMMAR_LOOP_POSITIVE".to_string());
    }
    if get("ONE_PASS_GROW_DIAGNOSE_PRUNE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ONE_PASS_LOOP_WORKS".to_string());
    }
    if get("ITERATIVE_GROW_DIAGNOSE_PRUNE_2")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ITERATIVE_LOOP_REQUIRED".to_string());
    }
    if get("ITERATIVE_GROW_DIAGNOSE_PRUNE_4")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("ITERATIVE_LOOP_REQUIRED".to_string());
    }
    if get("FRONTIER_LABEL_LOOP")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FRONTIER_LABEL_LOOP_WORKS".to_string());
    }
    if get("PRUNE_RESIDUAL_LABEL_LOOP")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("PRUNE_RESIDUAL_LABEL_LOOP_WORKS".to_string());
    }
    if get("GRAPH_INVARIANT_LABEL_LOOP")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("GRAPH_INVARIANT_LABEL_LOOP_WORKS".to_string());
    }
    if get("MIXED_LABEL_LOOP").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("MIXED_LABEL_LOOP_WORKS".to_string());
    }
    if get("NO_LABEL_LOOP_CONTROL")
        .map(|r| !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("NO_LABEL_LOOP_FAILS".to_string());
    }
    ...
    if partial_signal && !loop_positive {
        verdicts.insert("AUTONOMOUS_LOOP_PARTIAL_SIGNAL".to_string());
    }
    ...
    if !controls_fail {
        ...
        verdicts.insert("RANDOM_LABEL_LOOP_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if !loop_positive {
        verdicts.insert("AUTONOMOUS_ROUTE_GRAMMAR_LOOP_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str(
        "# STABLE_LOOP_PHASE_LOCK_032_AUTONOMOUS_ROUTE_GRAMMAR_CONSTRUCTION_LOOP Report\n\n",
    );
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("032 is an autonomous route-grammar construction loop diagnostic. Passing loop arms show that dense growth, graph-diagnostic labels, order-aware prune, repair, and delivery evaluation work together as one staged loop. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_032_autonomous_route_grammar_construction_loop/dev",
    );
