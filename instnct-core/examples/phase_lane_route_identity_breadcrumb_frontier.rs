//! Runner-local route identity / breadcrumb frontier probe.
//!
//! 021 asks whether an explicit public breadcrumb/route-token field can supply
//! the missing route identity that simple public geometry did not provide.
...
enum ArmKind {
    PublicDistanceFieldSingleSuccessorReceiveCommitLedger,
    TruePathReceiveCommitLedgerDiagnostic,
    PublicBreadcrumbMaskBfsReceiveCommitLedger,
    PublicBreadcrumbOrderedSuccessorReceiveCommitLedger,
    PublicBreadcrumbFrontierParentReceiveCommitLedger,
    PublicBreadcrumbMaskPlusPublicNoBackflowReceiveCommitLedger,
    PublicBreadcrumbMaskWithSpursReceiveCommitLedger,
    RandomBreadcrumbSameCountReceiveCommitLedger,
    BreadcrumbOrderShuffleReceiveCommitLedger,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::PublicDistanceFieldSingleSuccessorReceiveCommitLedger => {
                "PUBLIC_DISTANCE_FIELD_SINGLE_SUCCESSOR_BASELINE"
            }
            Self::TruePathReceiveCommitLedgerDiagnostic => "TRUE_PATH_RECEIVE_COMMIT_LEDGER_DIAGNOSTIC",
            Self::PublicBreadcrumbMaskBfsReceiveCommitLedger => "PUBLIC_BREADCRUMB_MASK_BFS_RECEIVE_COMMIT_LEDGER",
            Self::PublicBreadcrumbOrderedSuccessorReceiveCommitLedger => {
                "PUBLIC_BREADCRUMB_ORDERED_SUCCESSOR_RECEIVE_COMMIT_LEDGER"
            }
            Self::PublicBreadcrumbFrontierParentReceiveCommitLedger => {
                "PUBLIC_BREADCRUMB_FRONTIER_PARENT_RECEIVE_COMMIT_LEDGER"
            }
            Self::PublicBreadcrumbMaskPlusPublicNoBackflowReceiveCommitLedger => {
                "PUBLIC_BREADCRUMB_MASK_PLUS_PUBLIC_NO_BACKFLOW_RECEIVE_COMMIT_LEDGER"
            }
            Self::PublicBreadcrumbMaskWithSpursReceiveCommitLedger => {
                "PUBLIC_BREADCRUMB_MASK_WITH_SPURS_RECEIVE_COMMIT_LEDGER"
            }
            Self::RandomBreadcrumbSameCountReceiveCommitLedger => "RANDOM_BREADCRUMB_SAME_COUNT_RECEIVE_COMMIT_LEDGER",
            Self::BreadcrumbOrderShuffleReceiveCommitLedger => "BREADCRUMB_ORDER_SHUFFLE_RECEIVE_COMMIT_LEDGER",
    ...
    fn diagnostic_only(self) -> bool {
        matches!(
            self,
            Self::TruePathReceiveCommitLedgerDiagnostic
        )
    }
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomBreadcrumbSameCountReceiveCommitLedger
                | Self::BreadcrumbOrderShuffleReceiveCommitLedger
...
enum GraphMode {
    TruePath,
    PublicDistanceFieldSingleSuccessor,
    PublicBreadcrumbMaskBfs,
    PublicBreadcrumbOrderedSuccessor,
    PublicBreadcrumbFrontierParent,
    PublicBreadcrumbMaskPlusNoBackflow,
    PublicBreadcrumbMaskWithSpurs,
    RandomBreadcrumbSameCount,
    BreadcrumbOrderShuffle,
    RandomSameCount,
    DirectionShuffle,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_021_ROUTE_IDENTITY_BREADCRUMB_FRONTIER",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!("../../docs/research/STABLE_LOOP_PHASE_LOCK_021_ROUTE_IDENTITY_BREADCRUMB_FRONTIER_CONTRACT.md"),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger {
                            ...
                            append_jsonl(&cfg.out.join("route_metrics.jsonl"), &row)?;
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger,
        ArmKind::TruePathReceiveCommitLedgerDiagnostic,
        ArmKind::PublicBreadcrumbMaskBfsReceiveCommitLedger,
        ArmKind::PublicBreadcrumbOrderedSuccessorReceiveCommitLedger,
        ArmKind::PublicBreadcrumbFrontierParentReceiveCommitLedger,
        ArmKind::PublicBreadcrumbMaskPlusPublicNoBackflowReceiveCommitLedger,
        ArmKind::PublicBreadcrumbMaskWithSpursReceiveCommitLedger,
        ArmKind::RandomBreadcrumbSameCountReceiveCommitLedger,
        ArmKind::BreadcrumbOrderShuffleReceiveCommitLedger,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::PublicDistanceFieldSingleSuccessorReceiveCommitLedger => {
            GraphMode::PublicDistanceFieldSingleSuccessor
        }
        ArmKind::PublicBreadcrumbMaskBfsReceiveCommitLedger => GraphMode::PublicBreadcrumbMaskBfs,
        ArmKind::PublicBreadcrumbOrderedSuccessorReceiveCommitLedger => GraphMode::PublicBreadcrumbOrderedSuccessor,
        ArmKind::PublicBreadcrumbFrontierParentReceiveCommitLedger => GraphMode::PublicBreadcrumbFrontierParent,
        ArmKind::PublicBreadcrumbMaskPlusPublicNoBackflowReceiveCommitLedger => {
            GraphMode::PublicBreadcrumbMaskPlusNoBackflow
        }
        ArmKind::PublicBreadcrumbMaskWithSpursReceiveCommitLedger => GraphMode::PublicBreadcrumbMaskWithSpurs,
        ArmKind::RandomBreadcrumbSameCountReceiveCommitLedger => GraphMode::RandomBreadcrumbSameCount,
        ArmKind::BreadcrumbOrderShuffleReceiveCommitLedger => GraphMode::BreadcrumbOrderShuffle,
...
fn readout_mode(_arm: ArmKind) -> ReadoutMode {
    ...
    match mode {
        ...
        GraphMode::PublicDistanceFieldSingleSuccessor => {
            ...
            for id in 0..cells {
                ...
                let best = neighbors(width, y, x)
                    .into_iter()
                    .map(|(ny, nx)| cell_id(width, ny, nx))
                    .filter(|&to| !case.public.wall[to])
                    .filter(|&to| dist[to].is_some_and(|to_d| to_d < from_d))
                    .min_by_key(|&to| (dist[to].unwrap_or(usize::MAX), to));
                if let Some(to) = best {
                    add_edge(&mut edges, &mut seen, id, to);
                }
            }
        }
        GraphMode::PublicBreadcrumbMaskBfs | GraphMode::PublicBreadcrumbFrontierParent => {
        ...
        GraphMode::PublicBreadcrumbMaskWithSpurs => {
            for pair in public_breadcrumb_bfs_path(case, true, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::PublicBreadcrumbOrderedSuccessor => {
            let mut ids = (0..cells)
                .filter(|&id| case.public.route_order[id] != usize::MAX)
                .collect::<Vec<_>>();
            ids.sort_by_key(|&id| case.public.route_order[id]);
            for pair in ids.windows(2) {
                if case.public.route_order[pair[1]] == case.public.route_order[pair[0]] + 1 {
                    add_edge(&mut edges, &mut seen, pair[0], pair[1]);
                }
            }
        }
        GraphMode::RandomBreadcrumbSameCount => {
            for pair in random_breadcrumb_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::BreadcrumbOrderShuffle => {
            for pair in shuffled_breadcrumb_order_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::RandomSameCount => {
            ...
            let mut rng = StdRng::seed_from_u64(seed ^ 0xA51D_021);
        ...
        GraphMode::DirectionShuffle => {
            let mut rng = StdRng::seed_from_u64(seed ^ 0xD1EC_021);
            for pair in case.private.true_path.windows(2) {
...
fn random_breadcrumb_path(case: &Case, seed: u64) -> Vec<usize> {
    let width = case.public.width;
    let source = cell_id(width, case.public.source.0, case.public.source.1);
    let mut rng = StdRng::seed_from_u64(seed ^ 0xB4EAD021);
...
fn shuffled_breadcrumb_order_path(case: &Case, seed: u64) -> Vec<usize> {
    let width = case.public.width;
    let mut rng = StdRng::seed_from_u64(seed ^ 0x5EED_021);
...
fn aggregate_ranking(rows: &[MetricsRow]) -> Vec<RankingRow> {
    ...
    for (arm, group) in by_arm {
        ...
        let counterfactual = mean(sufficient_rows.iter().map(|r| r.same_target_counterfactual_accuracy));
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let true_path = get("TRUE_PATH_RECEIVE_COMMIT_LEDGER_DIAGNOSTIC");
    ...
    let public_positive = ranking.iter().filter(|r| {
        !r.diagnostic_only
            && !matches!(
                r.arm.as_str(),
                "PUBLIC_DISTANCE_FIELD_SINGLE_SUCCESSOR_BASELINE"
    ...
    if true_path.map(|r| r.positive).unwrap_or(false) && controls_fail {
        verdicts.insert("TRUE_PATH_UPPER_BOUND_CONFIRMED".to_string());
    }

    if !public_positive.is_empty() {
        verdicts.insert("ROUTE_IDENTITY_BREADCRUMB_POSITIVE".to_string());
    } else {
        verdicts.insert("ROUTE_IDENTITY_BREADCRUMB_FAILS".to_string());
    }
    if get("PUBLIC_BREADCRUMB_MASK_BFS_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("BREADCRUMB_MASK_BFS_WORKS".to_string());
    }
    if get("PUBLIC_BREADCRUMB_ORDERED_SUCCESSOR_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("BREADCRUMB_ORDERED_SUCCESSOR_WORKS".to_string());
    }
    if get("PUBLIC_BREADCRUMB_FRONTIER_PARENT_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("BREADCRUMB_FRONTIER_WORKS".to_string());
    }
    if get("PUBLIC_BREADCRUMB_MASK_BFS_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
        && !get("PUBLIC_BREADCRUMB_MASK_WITH_SPURS_RECEIVE_COMMIT_LEDGER").map(|r| r.positive).unwrap_or(false)
    {
        verdicts.insert("BREADCRUMB_SPURS_BREAK_ROUTE_IDENTITY".to_string());
    }
    if !get("PUBLIC_DISTANCE_FIELD_SINGLE_SUCCESSOR_BASELINE").map(|r| r.positive).unwrap_or(false) {
        verdicts.insert("PUBLIC_DISTANCE_FIELD_BASELINE_STILL_FAILS".to_string());
        verdicts.insert("ROUTE_IDENTITY_REQUIRED".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("BREADCRUMB_OVERPOWERS_CONTROL".to_string());
    } else {
        verdicts.insert("BREADCRUMB_RANDOM_CONTROL_FAILS".to_string());
        verdicts.insert("BREADCRUMB_ORDER_SHUFFLE_FAILS".to_string());
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_021_ROUTE_IDENTITY_BREADCRUMB_FRONTIER Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Gate collapse | Counterfactual | Ledger power | Duplicate | Stale | Reciprocal | Backflow | Edges | Delta acc | Delta wrong | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("021 is a route-identity/breadcrumb diagnostic only. Public breadcrumb arms assume an explicit route-token field is already present; they do not prove breadcrumb learning or production routing. It does not claim production architecture, FlyWire validation, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from("target/pilot_wave/stable_loop_phase_lock_021_route_identity_breadcrumb_frontier/dev");
...
fn random_phase_table(seed: u64) -> [[usize; K]; K] {
    let mut rng = StdRng::seed_from_u64(seed ^ 0xA11CE021);
