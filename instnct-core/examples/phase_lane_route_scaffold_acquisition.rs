//! Runner-local route scaffold acquisition probe.
//!
//! 023 asks where the first useful ordered-successor scaffold can come from,
//! now that 022 showed scaffolded successor fields are repairable,
//! completable, and prunable.
...
enum ArmKind {
    TruePathUpperBound,
    NoisyBfsScaffold,
    DistanceFieldScaffold,
    FrontierParentScaffold,
    SourceTargetAnchorScaffold,
    BidirectionalCandidateFieldThenPrune,
    RandomSparseScaffoldPlusDeliveryReward,
    HubDegreePriorScaffold,
    ShortPathCurriculumToLong,
    DenseThenCrystallizePruneRoute,
    RandomFromScratchBaseline,
    RandomScaffoldControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::TruePathUpperBound => "TRUE_PATH_UPPER_BOUND",
            Self::NoisyBfsScaffold => "NOISY_BFS_SCAFFOLD",
            Self::DistanceFieldScaffold => "DISTANCE_FIELD_SCAFFOLD",
            Self::FrontierParentScaffold => "FRONTIER_PARENT_SCAFFOLD",
            Self::SourceTargetAnchorScaffold => "SOURCE_TARGET_ANCHOR_SCAFFOLD",
            Self::BidirectionalCandidateFieldThenPrune => {
                "BIDIRECTIONAL_CANDIDATE_FIELD_THEN_PRUNE"
            }
            Self::RandomSparseScaffoldPlusDeliveryReward => {
                "RANDOM_SPARSE_SCAFFOLD_PLUS_DELIVERY_REWARD"
            }
            Self::HubDegreePriorScaffold => "HUB_DEGREE_PRIOR_SCAFFOLD",
            Self::ShortPathCurriculumToLong => "SHORT_PATH_CURRICULUM_TO_LONG",
            Self::DenseThenCrystallizePruneRoute => "DENSE_THEN_CRYSTALLIZE_PRUNE_ROUTE",
            Self::RandomFromScratchBaseline => "RANDOM_FROM_SCRATCH_BASELINE",
            Self::RandomScaffoldControl => "RANDOM_SCAFFOLD_CONTROL",
            Self::RandomPhaseRuleControl => "RANDOM_PHASE_RULE_CONTROL",
        }
    }

    fn diagnostic_only(self) -> bool {
        matches!(self, Self::TruePathUpperBound)
    }

    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomScaffoldControl | Self::RandomPhaseRuleControl
        )
    }
}
...
enum GraphMode {
    ...
    FrontierParent,
    SourceTargetAnchor,
    BidirectionalCandidateThenPrune,
    RandomSparseDeliveryReward,
    HubDegreePrior,
    ShortPathCurriculum,
    DenseThenPrune,
    RandomFromScratch,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_023_ROUTE_SCAFFOLD_ACQUISITION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_023_ROUTE_SCAFFOLD_ACQUISITION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::DistanceFieldScaffold {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::DistanceFieldScaffold {
                            ...
                            append_jsonl(&cfg.out.join("scaffold_metrics.jsonl"), &row)?;
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::DistanceFieldScaffold,
        ArmKind::TruePathUpperBound,
        ArmKind::NoisyBfsScaffold,
        ArmKind::FrontierParentScaffold,
        ArmKind::SourceTargetAnchorScaffold,
        ArmKind::BidirectionalCandidateFieldThenPrune,
        ArmKind::RandomSparseScaffoldPlusDeliveryReward,
        ArmKind::HubDegreePriorScaffold,
        ArmKind::ShortPathCurriculumToLong,
        ArmKind::DenseThenCrystallizePruneRoute,
        ArmKind::RandomFromScratchBaseline,
        ArmKind::RandomScaffoldControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::TruePathUpperBound => GraphMode::PerfectSuccessor,
        ArmKind::NoisyBfsScaffold => GraphMode::NoisyBfs,
        ArmKind::DistanceFieldScaffold => GraphMode::CanonicalDistance,
        ArmKind::FrontierParentScaffold => GraphMode::FrontierParent,
        ArmKind::SourceTargetAnchorScaffold => GraphMode::SourceTargetAnchor,
        ArmKind::BidirectionalCandidateFieldThenPrune => GraphMode::BidirectionalCandidateThenPrune,
        ArmKind::RandomSparseScaffoldPlusDeliveryReward => GraphMode::RandomSparseDeliveryReward,
        ArmKind::HubDegreePriorScaffold => GraphMode::HubDegreePrior,
        ArmKind::ShortPathCurriculumToLong => GraphMode::ShortPathCurriculum,
        ArmKind::DenseThenCrystallizePruneRoute => GraphMode::DenseThenPrune,
        ArmKind::RandomFromScratchBaseline => GraphMode::RandomFromScratch,
        ArmKind::RandomScaffoldControl => GraphMode::RandomScaffold,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomScaffoldControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn build_graph(mode: GraphMode, case: &Case, seed: u64) -> DirectedGraph {
    ...
    match mode {
        ...
        GraphMode::NoisyBfs => {
            let path = noisy_bfs_scaffold_path(case, seed);
        ...
        GraphMode::FrontierParent => {
            for pair in public_bfs_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::SourceTargetAnchor => {
            for pair in source_target_anchor_scaffold_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::BidirectionalCandidateThenPrune => {
            let dense = bidirectional_candidate_edges(case);
            for (from, to) in prune_dense_to_ordered_path(case, &dense) {
                add_edge(&mut edges, &mut seen, from, to);
            }
        }
        GraphMode::RandomSparseDeliveryReward => {
            for pair in random_sparse_delivery_reward_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::HubDegreePrior => {
            build_hub_rich_edges(case, &mut edges, &mut seen, false, seed);
        }
        GraphMode::ShortPathCurriculum => {
            for pair in short_path_curriculum_path(case).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::DenseThenPrune => {
            let dense = dense_breadcrumb_edges(case);
            for (from, to) in prune_dense_to_ordered_path(case, &dense) {
                add_edge(&mut edges, &mut seen, from, to);
            }
        }
        GraphMode::RandomFromScratch => {
            for pair in random_breadcrumb_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
        GraphMode::RandomScaffold => {
            for pair in random_breadcrumb_path(case, seed).windows(2) {
                add_edge(&mut edges, &mut seen, pair[0], pair[1]);
            }
        }
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_SCAFFOLD_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
        ) && r.phase_final_accuracy >= 0.75
    });
    if get("TRUE_PATH_UPPER_BOUND")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("TRUE_PATH_UPPER_BOUND_CONFIRMED".to_string());
    }
    let public_positive = ranking
        .iter()
        .any(|r| r.positive && !r.diagnostic_only && !r.arm.starts_with("RANDOM_"));
    if public_positive {
        verdicts.insert("SCAFFOLD_ACQUISITION_POSITIVE".to_string());
    }
    if get("NOISY_BFS_SCAFFOLD")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BFS_SCAFFOLD_USEFUL".to_string());
    }
    if get("DISTANCE_FIELD_SCAFFOLD")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DISTANCE_SCAFFOLD_USEFUL".to_string());
    }
    if get("FRONTIER_PARENT_SCAFFOLD")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FRONTIER_PARENT_SCAFFOLD_USEFUL".to_string());
    }
    if get("SOURCE_TARGET_ANCHOR_SCAFFOLD")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SOURCE_TARGET_ANCHOR_SCAFFOLD_USEFUL".to_string());
    }
    if get("BIDIRECTIONAL_CANDIDATE_FIELD_THEN_PRUNE")
        .map(|r| r.positive)
        .unwrap_or(false)
        || get("DENSE_THEN_CRYSTALLIZE_PRUNE_ROUTE")
            .map(|r| r.positive)
            .unwrap_or(false)
    {
        verdicts.insert("DENSE_THEN_PRUNE_ROUTE_WORKS".to_string());
    }
    if get("SHORT_PATH_CURRICULUM_TO_LONG")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("CURRICULUM_REQUIRED".to_string());
    }
    if get("RANDOM_SPARSE_SCAFFOLD_PLUS_DELIVERY_REWARD")
        .map(|r| r.signal && !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DELIVERY_REWARD_HAS_BUT_INSUFFICIENT_SIGNAL".to_string());
    }
    if !get("RANDOM_FROM_SCRATCH_BASELINE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("RANDOM_FROM_SCRATCH_STILL_FAILS".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("SCAFFOLD_CONTROL_CONTAMINATION".to_string());
    } else {
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_023_ROUTE_SCAFFOLD_ACQUISITION Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Link acc | Order acc | Coverage | Noise | Branch | Cycle | Gate collapse | Counterfactual | Delta acc | Delta wrong | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
        report.push_str(&format!(
            "| {} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.3} | {:.1} | {:.1} | {:.3} | {:.3} | {:+.3} | {:+.3} | {} | {} |\n",
            row.arm,
            ...
            row.initial_successor_link_accuracy,
            row.initial_route_order_accuracy,
            row.scaffold_coverage,
            row.scaffold_noise_rate,
            row.scaffold_branch_count,
            row.scaffold_cycle_count,
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("023 is a route-scaffold acquisition diagnostic only. Passing scaffold arms show that a weak/public or supplied scaffold can feed the 022 repair/completion/prune machinery; they do not prove production routing or mutation from scratch. It does not claim production architecture, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_023_route_scaffold_acquisition/dev",
    );
