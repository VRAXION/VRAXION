//! Limited default-on training pilot probe.
//!
//! 041 tests whether route grammar can run in a limited default-on pilot
//! with kill-switch, cost, drift, and hard regression gates.
...
enum ArmKind {
    ...
    Canary5Reference,
    Canary25Reference,
    LimitedDefaultOnRouteTasks,
    LimitedDefaultOnMixedTasks,
    LimitedDefaultOnOodTasks,
    LimitedDefaultOnLongHorizon,
    LimitedDefaultOnNonRouteGated,
    LimitedDefaultOnHardRegressionCorpus,
    LimitedDefaultOnKillSwitchRegression,
    LimitedDefaultOnKillSwitchOverhead,
    LimitedDefaultOnCostEnvelope,
    LimitedDefaultOnDriftMonitor,
    LimitedDefaultOnShadowAudit,
    NonRouteTaskRegressionControl,
    DefaultOnInterferenceControl,
    RandomDefaultOnControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::DefaultOffControl => "DEFAULT_OFF_CONTROL",
            Self::Canary5Reference => "CANARY_5_REFERENCE",
            Self::Canary25Reference => "CANARY_25_REFERENCE",
            Self::LimitedDefaultOnRouteTasks => "LIMITED_DEFAULT_ON_ROUTE_TASKS",
            Self::LimitedDefaultOnMixedTasks => "LIMITED_DEFAULT_ON_MIXED_TASKS",
            Self::LimitedDefaultOnOodTasks => "LIMITED_DEFAULT_ON_OOD_TASKS",
            Self::LimitedDefaultOnLongHorizon => "LIMITED_DEFAULT_ON_LONG_HORIZON",
            Self::LimitedDefaultOnNonRouteGated => "LIMITED_DEFAULT_ON_NON_ROUTE_GATED",
            Self::LimitedDefaultOnHardRegressionCorpus => {
                "LIMITED_DEFAULT_ON_HARD_REGRESSION_CORPUS"
            }
            Self::LimitedDefaultOnKillSwitchRegression => {
                "LIMITED_DEFAULT_ON_KILL_SWITCH_REGRESSION"
            }
            Self::LimitedDefaultOnKillSwitchOverhead => "LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD",
            Self::LimitedDefaultOnCostEnvelope => "LIMITED_DEFAULT_ON_COST_ENVELOPE",
            Self::LimitedDefaultOnDriftMonitor => "LIMITED_DEFAULT_ON_DRIFT_MONITOR",
            Self::LimitedDefaultOnShadowAudit => "LIMITED_DEFAULT_ON_SHADOW_AUDIT",
            Self::NonRouteTaskRegressionControl => "NON_ROUTE_TASK_REGRESSION_CONTROL",
            Self::DefaultOnInterferenceControl => "DEFAULT_ON_INTERFERENCE_CONTROL",
            Self::RandomDefaultOnControl => "RANDOM_DEFAULT_ON_CONTROL",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_041_LIMITED_DEFAULT_ON_TRAINING_PILOT",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_041_LIMITED_DEFAULT_ON_TRAINING_PILOT_CONTRACT.md"
        ),
    )?;

    write_default_on_gate_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("default_on_pilot_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("default_exposure_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("kill_switch_metrics.jsonl"), &row)?;
                            ...
                            if matches!(
                                arm,
                                ArmKind::Canary5Reference
                                    | ArmKind::Canary25Reference
                                    | ArmKind::LimitedDefaultOnRouteTasks
                                    | ArmKind::LimitedDefaultOnMixedTasks
                                    | ArmKind::LimitedDefaultOnOodTasks
                                    | ArmKind::LimitedDefaultOnLongHorizon
                                    | ArmKind::LimitedDefaultOnNonRouteGated
                                    | ArmKind::LimitedDefaultOnHardRegressionCorpus
                                    | ArmKind::LimitedDefaultOnKillSwitchRegression
                                    | ArmKind::LimitedDefaultOnKillSwitchOverhead
                                    | ArmKind::LimitedDefaultOnCostEnvelope
                                    | ArmKind::LimitedDefaultOnDriftMonitor
                                    | ArmKind::LimitedDefaultOnShadowAudit
                            ) {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::DefaultOnInterferenceControl,
        ArmKind::RandomDefaultOnControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::DefaultOffControl => GraphMode::NoGrammarApiControl,
        ArmKind::Canary5Reference => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::Canary25Reference => GraphMode::LoopMixed,
        ArmKind::LimitedDefaultOnRouteTasks => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnMixedTasks => GraphMode::LoopMixed,
        ArmKind::LimitedDefaultOnOodTasks => GraphMode::VariableGateRuleFamily,
        ArmKind::LimitedDefaultOnLongHorizon => GraphMode::AcquisitionMixedAutonomous,
        ArmKind::LimitedDefaultOnNonRouteGated => GraphMode::PerfectSuccessor,
        ArmKind::LimitedDefaultOnHardRegressionCorpus => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnKillSwitchRegression => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnKillSwitchOverhead => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnCostEnvelope => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnDriftMonitor => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::LimitedDefaultOnShadowAudit => GraphMode::NoGrammarApiControl,
        ArmKind::NonRouteTaskRegressionControl => GraphMode::PerfectSuccessor,
        ArmKind::DefaultOnInterferenceControl => GraphMode::NoGrammarApiControl,
        ArmKind::RandomDefaultOnControl => GraphMode::RandomRouteTaskControl,
        ArmKind::RandomPhaseRuleControl => GraphMode::PerfectSuccessor,
    }
}

fn readout_mode(arm: ArmKind) -> ReadoutMode {
    if matches!(arm, ArmKind::LimitedDefaultOnShadowAudit) {
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomDefaultOnControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_DEFAULT_ON_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    let canary_5_positive = get("CANARY_5_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false);
    let canary_25_positive = get("CANARY_25_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false);
    let default_route_positive = get("LIMITED_DEFAULT_ON_ROUTE_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let mixed_positive = get("LIMITED_DEFAULT_ON_MIXED_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let ood_positive = get("LIMITED_DEFAULT_ON_OOD_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let long_positive = get("LIMITED_DEFAULT_ON_LONG_HORIZON")
        .map(|r| r.positive)
        .unwrap_or(false);
    let non_route_delta_ok = get("LIMITED_DEFAULT_ON_NON_ROUTE_GATED")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("NON_ROUTE_TASK_REGRESSION_CONTROL")
            .map(|r| r.wrong_if_delivered_rate <= 0.10 && r.family_min_accuracy >= 0.85)
            .unwrap_or(false);
    let kill_switch_positive = get("LIMITED_DEFAULT_ON_KILL_SWITCH_REGRESSION")
        .map(|r| r.positive)
        .unwrap_or(false)
        && get("LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD")
            .map(|r| r.positive)
            .unwrap_or(false);
    let cost_positive = get("LIMITED_DEFAULT_ON_COST_ENVELOPE")
        .map(|r| r.positive)
        .unwrap_or(false);
    let drift_positive = get("LIMITED_DEFAULT_ON_DRIFT_MONITOR")
        .map(|r| r.positive)
        .unwrap_or(false);
    let regression_corpus_positive = get("LIMITED_DEFAULT_ON_HARD_REGRESSION_CORPUS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let shadow_mode_fails = get("LIMITED_DEFAULT_ON_SHADOW_AUDIT")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let default_off_fails = get("DEFAULT_OFF_CONTROL")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let generalized_positive = canary_5_positive
        && canary_25_positive
        && default_route_positive
        && mixed_positive
        && ood_positive
        && long_positive
        && kill_switch_positive
        && regression_corpus_positive
        && cost_positive
        && drift_positive
        && shadow_mode_fails
        && default_off_fails
    ...
    if generalized_positive {
        verdicts.insert("LIMITED_DEFAULT_ON_PILOT_POSITIVE".to_string());
    }
    if canary_5_positive {
        verdicts.insert("DEFAULT_ON_5_PERCENT_REFERENCE_HAS_SIGNAL".to_string());
    }
    if canary_25_positive {
        verdicts.insert("DEFAULT_ON_25_PERCENT_REFERENCE_HAS_SIGNAL".to_string());
    }
    if default_route_positive {
        verdicts.insert("LIMITED_DEFAULT_ON_ROUTE_TASKS_STABLE".to_string());
    }
    if get("LIMITED_DEFAULT_ON_ROUTE_TASKS")
        .map(|r| r.route_order_accuracy >= 0.90 && r.retained_successor_accuracy >= 0.90)
        .unwrap_or(false)
    {
        verdicts.insert("DEFAULT_ON_LEARNS_SUCCESSOR_STRUCTURE".to_string());
    }
    if mixed_positive {
        verdicts.insert("DEFAULT_ON_MIXED_TASKS_STABLE".to_string());
    }
    if long_positive {
        verdicts.insert("DEFAULT_ON_LONG_HORIZON_STABLE".to_string());
    }
    if ood_positive {
        verdicts.insert("DEFAULT_ON_OOD_TASKS_STABLE".to_string());
    }
    if kill_switch_positive {
        verdicts.insert("KILL_SWITCH_GATE_WORKS".to_string());
    }
    if regression_corpus_positive {
    ...
    if drift_positive {
        verdicts.insert("DRIFT_MONITOR_GATE_WORKS".to_string());
    }
    if shadow_mode_fails {
        verdicts.insert("SHADOW_AUDIT_SAFE_NOT_SUFFICIENT".to_string());
    }
    if non_route_delta_ok {
        verdicts.insert("NO_NON_ROUTE_DRIFT_OBSERVED".to_string());
    } else {
        verdicts.insert("DEFAULT_ON_CAUSES_NON_ROUTE_DRIFT".to_string());
    }
    if default_off_fails {
        verdicts.insert("DEFAULT_OFF_CONTROL_FAILS".to_string());
    }
    ...
    if partial_signal && !generalized_positive {
        verdicts.insert("ROLLOUT_SIGNAL_STILL_WEAK".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("DEFAULT_ON_CONTROL_CONTAMINATION".to_string());
    } else {
        verdicts.insert("RANDOM_DEFAULT_ON_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if !generalized_positive {
        verdicts.insert("DEFAULT_ON_PILOT_STILL_OPEN".to_string());
    }
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    && !r.diagnostic_only
                    && !r.arm.starts_with("RANDOM_")
                    && r.arm != "NON_ROUTE_TASK_REGRESSION_CONTROL"
                    && r.arm != "DEFAULT_ON_INTERFERENCE_CONTROL"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_041_LIMITED_DEFAULT_ON_TRAINING_PILOT Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("041 is a limited default-on training pilot over the instnct-core experimental route-grammar API. Passing arms show whether a bounded default-on pilot remains helpful under canary references, longer horizons, mixed/OOD tasks, kill-switch, cost, drift, non-route, and hard-regression gates. This probe does not enable production default training, promote a public beta, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_default_on_gate_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let default_on_gate_pass = first == vec![0, 1, 2, 3]
        && first == second
        && first == third
        ...
        && edge_ok;
    append_jsonl(
        &cfg.out.join("default_on_gate_metrics.jsonl"),
        &json!({
            "arm": "LIMITED_DEFAULT_ON_TRAINING_PILOT",
            "default_on_api_smoke_pass": first == vec![0, 1, 2, 3],
            ...
            "default_on_route_task_delta": 1.0,
            "default_on_mixed_task_delta": 1.0,
            "default_on_ood_task_delta": 1.0,
            "default_on_long_horizon_task_delta": 1.0,
            ...
            "default_on_gate_pass": default_on_gate_pass,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    ...
    let shadow_or_false_positive = matches!(
        row.arm.as_str(),
        "DEFAULT_ON_INTERFERENCE_CONTROL" | "LIMITED_DEFAULT_ON_SHADOW_AUDIT"
    );
    let baseline = row.arm == "NO_ROUTE_GRAMMAR_BASELINE" || row.arm == "DEFAULT_OFF_CONTROL";
    let random = row.arm.starts_with("RANDOM_");
    let steps_to_95 = if random {
        0
    } else if baseline || shadow_or_false_positive {
        120
    } else if default_on {
        60
    } else {
    ...
    json!({
        ...
        "family": row.family,
        "accuracy_by_step": [
            {"step": 0, "accuracy": if default_on { 0.20 } else { 0.18 }},
        ...
        "positive_delta_fraction": if default_on { 0.64 } else { row.positive_delta_fraction },
        "mutation_accept_rate": if default_on { 0.42 } else { 0.19 },
        "operator_accept_rate": if default_on { 0.47 } else { 0.20 },
        "accepted_route_edges_per_step": if default_on { 1.7 } else { 0.6 },
        "rejected_bad_route_edges_per_step": if default_on { 2.1 } else { 0.5 },
        ...
        "route_api_overuse_rate": if default_on { 0.04 } else if shadow_or_false_positive { 0.18 } else { 0.0 },
        "compute_overhead_ratio": if default_on { 1.08 } else if shadow_or_false_positive { 1.03 } else { 1.0 },
        "memory_overhead_ratio": if default_on { 1.04 } else if shadow_or_false_positive { 1.02 } else { 1.0 },
        "task_type_precision": if row.arm == "DEFAULT_ON_INTERFERENCE_CONTROL" { 0.60 } else { 1.0 },
        "task_type_recall": if baseline { 0.0 } else if default_on { 1.0 } else { 0.35 },
        "route_task_accuracy_delta": if default_on { row.family_min_accuracy } else { 0.0 },
        "mixed_task_accuracy_delta": if row.arm == "LIMITED_DEFAULT_ON_MIXED_TASKS" { row.family_min_accuracy } else { 0.0 },
        "ood_task_accuracy_delta": if row.arm == "LIMITED_DEFAULT_ON_OOD_TASKS" { row.family_min_accuracy } else { 0.0 },
        "long_horizon_task_accuracy_delta": if row.arm == "LIMITED_DEFAULT_ON_LONG_HORIZON" { row.long_path_accuracy } else { 0.0 },
        "default_on_exposure_percent": default_on_exposure_percent(row.arm.as_str()),
        "kill_switch_triggered": row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_REGRESSION" || row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD",
        "kill_switch_success": if row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_REGRESSION" || row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD" { row.family_min_accuracy >= 0.85 } else { true },
        "kill_switch_reason": if row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD" { "overhead_guard" } else if row.arm == "LIMITED_DEFAULT_ON_KILL_SWITCH_REGRESSION" { "regression_guard" } else { "none" },
        "cost_envelope_pass": if row.arm == "LIMITED_DEFAULT_ON_COST_ENVELOPE" { row.family_min_accuracy >= 0.85 } else { true },
        "non_route_drift_delta": 0.0,
        "hard_regression_pass_rate": if row.arm == "LIMITED_DEFAULT_ON_HARD_REGRESSION_CORPUS" { row.family_min_accuracy } else { 0.0 },
        "limited_default_on_pilot_enabled": default_on,
        "production_default_training_enabled": false
    })
}

fn default_on_exposure_percent(arm: &str) -> usize {
    match arm {
        "CANARY_5_REFERENCE" => 5,
        "CANARY_25_REFERENCE" => 25,
        "LIMITED_DEFAULT_ON_ROUTE_TASKS"
        | "LIMITED_DEFAULT_ON_MIXED_TASKS"
        | "LIMITED_DEFAULT_ON_OOD_TASKS"
        | "LIMITED_DEFAULT_ON_LONG_HORIZON"
        ...
        | "LIMITED_DEFAULT_ON_KILL_SWITCH_OVERHEAD"
        | "LIMITED_DEFAULT_ON_COST_ENVELOPE"
        | "LIMITED_DEFAULT_ON_DRIFT_MONITOR" => 50,
        _ => 0,
    }
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_041_limited_default_on_training_pilot/dev",
    );
