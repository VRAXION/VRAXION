//! Mixer large context test — ctx=16,32,64,128 with flat backprop autoencoder
//! Same proven architecture, just bigger context. Tests 57%/71%/86% compression.
//!
//! Results (100 epochs, 5000 samples/ep):
//!   ctx=16:  57%→91.3%  71%→99.0% ★★★  86%→99.8% ★★★
//!   ctx=32:  57%→86.4%  71%→98.0% ★★   86%→99.8% ★★★
//!   ctx=64:  57%→64.7%  71%→80.2%      86%→91.6% ★
//!   ctx=128: 57%→37.8%  71%→49.3%      86%→61.3%
//!
//! Key finding: ctx=16/32 are sweet spot (99.8% at 86%). Larger ctx undertrained
//! at 100 epochs — accuracy drops sharply past ctx=32.
//!
//! Run: cargo run --example mixer_large_ctx --release
...
impl MixerAE {
    fn new(idim: usize, bneck: usize, rng: &mut Rng) -> Self {
        ...
        MixerAE { w: (0..bneck).map(|_| (0..idim).map(|_| rng.normal()*s).collect()).collect(),
    ...
    fn eval(&self, pp: &Preproc, corpus: &[u8], ctx: usize, n: usize, seed: u64) -> f64 {
        ...
        for _ in 0..n { if corpus.len()<ctx{break;} let off=rng.range(0,corpus.len()-ctx);
            ...
            let mut o=vec![0.0f32;self.idim];
            for j in 0..self.idim { o[j]=self.dec_b[j]; for k in 0..self.bneck { o[k]+=self.w[k][j]*h[k]; } }
            // Oops, wrong index - fix decode
            let mut o2=vec![0.0f32;self.idim];
            for j in 0..self.idim { o2[j]=self.dec_b[j]; for k in 0..self.bneck { o2[j]+=self.w[k][j]*h[k]; } }
            for i in 0..ctx { let rs=&o2[i*7..(i+1)*7];
...
fn main() {
    ...
    println!("=== LARGE CONTEXT MIXER — flat backprop, 86% ratio target ===\n");
    println!("  {:>4} {:>6} {:>6} {:>10} {:>10} {:>10} {:>8}",
        "ctx", "input", "bneck", "ratio", "params", "char%", "time");
    println!("  {}", "─".repeat(60));

    // Test 86% ratio at each context size
    // Also test 57% and 71% for comparison
    for &ctx in &[16, 32, 64, 128, 256] {
        let idim = ctx * 7;

        for &ratio_pct in &[57, 71, 86] {
            let bneck = (idim as f32 * ratio_pct as f32 / 100.0).round() as usize;
            if bneck < 4 || bneck >= idim { continue; }
            ...
            let epochs = 100;
            let samples = 5000.min(corpus.len() / ctx);
            for ep in 0..epochs {
                let lr = 0.01 * (1.0 - ep as f32 / epochs as f32 * 0.7);
            ...
            let acc = ae.eval(&pp, &corpus, ctx, 2000, 999);
            let params = bneck * idim + bneck + idim;
            let m = if acc > 99.0 { " ★★★" } else if acc > 95.0 { " ★★" } else if acc > 90.0 { " ★" } else { "" };

            println!("  {:>4} {:>6} {:>6} {:>9}% {:>10} {:>9.1}% {:>7.1}s{}",
                ctx, idim, bneck, ratio_pct, params, acc, tc.elapsed().as_secs_f64(), m);
        }
        println!();
    }
