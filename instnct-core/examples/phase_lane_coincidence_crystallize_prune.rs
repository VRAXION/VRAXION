//! Runner-local coincidence crystallize/prune probe.
//!
//! 010 takes the dense spatial coincidence solution from 009, snaps it to a
//! canonical motif graph, and prunes it to identify the sparse causal core. It
//! is a dissection runner, not a new growth-first probe.
...
impl Motif {
    fn cell_id(self, layout: &Layout) -> usize {
        layout.cell_id(self.y, self.x)
    }

    fn motif_type(self) -> String {
        format!("{}_{}_{}", self.input_phase, self.gate, self.output_phase)
    }
}
...
struct PruneRow {
    ...
    checkpoint_step: usize,
    final_row: bool,
    motif_count_initial: usize,
    motif_count_final: usize,
    motif_prune_fraction: f64,
    train_accuracy: f64,
    guard_accuracy: f64,
    final_holdout_accuracy: f64,
    guard_correct_probability: f64,
    guard_counterfactual_accuracy: f64,
    gate_shuffle_collapse: f64,
    single_gate_ablation_drop_mean: f64,
    single_gate_ablation_drop_max: f64,
    cell_ablation_drop_mean: f64,
    cell_ablation_drop_max: f64,
    causal_motif_count: usize,
    decorative_motif_count: usize,
    minimum_motifs_to_95_accuracy: usize,
    minimum_motifs_to_90_probability: usize,
    forbidden_private_field_leak: f64,
    nonlocal_edge_count: usize,
    direct_output_leak_rate: f64,
    elapsed_sec: f64,
...
#[derive(Clone, Debug)]
struct CaseSplits {
    prune_train_cases: Vec<Case>,
    prune_guard_cases: Vec<Case>,
    final_holdout_cases: Vec<Case>,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_prune_static_run_files(&cfg)?;

    append_jsonl(
        cfg.out.join("progress.jsonl"),
        &json!({"event": "run_start", "time": now_sec(), "jobs": cfg.seeds.len()}),
    )?;

    let started = Instant::now();
    let mut rows = Vec::new();
    let total = cfg.seeds.len();
    for (completed_idx, seed) in cfg.seeds.iter().copied().enumerate() {
        let row = run_prune_job(&cfg, seed, started)?;
        rows.push(row);
        append_jsonl(
            cfg.out.join("progress.jsonl"),
            &json!({
                "event": "job_done",
                "time": now_sec(),
                "completed": completed_idx + 1,
                "total": total,
                "seed": seed,
            }),
        )?;
        refresh_prune_summary(&cfg, &rows, completed_idx + 1, total, started.elapsed().as_secs_f64())?;
    }

    write_seed_core_overlap(&cfg, &rows)?;
    write_prune_report(&cfg, &rows, true)?;
    append_jsonl(
        cfg.out.join("progress.jsonl"),
        &json!({"event": "run_done", "time": now_sec(), "completed": total, "total": total}),
    )?;
    Ok(())
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    ...
    let mut cfg = Config {
        out: PathBuf::from(
            "target/pilot_wave/stable_loop_phase_lock_010_coincidence_crystallize_prune/dev",
        ),
        seeds: vec![2026],
        steps: 100,
        eval_examples: 256,
...
fn run_prune_job(
    ...
    seed: u64,
    run_started: Instant,
) -> Result<PruneRow, Box<dyn std::error::Error>> {
    let job_id = format!("{}_CRYSTALLIZE_PRUNE", seed);
    let job_path = cfg.out.join("job_progress").join(format!("{job_id}.jsonl"));
    let layout = Layout { width: cfg.width };
    let splits = case_splits(seed, cfg.eval_examples, cfg.width, cfg.ticks);
    let started = Instant::now();

    append_jsonl(
        &job_path,
        &json!({"event": "job_start", "time": now_sec(), "seed": seed}),
    )?;
    if cfg
        .out
        ...
        == 0
    {
        for case in splits
            .prune_train_cases
            .iter()
            .chain(splits.prune_guard_cases.iter())
            .chain(splits.final_holdout_cases.iter())
            .take(12)
        {
            append_jsonl(
    ...
    let mut dense = hand_built_spatial_network(&layout);
    let initial_motif_count = active_motifs(&dense, &layout).len();
    let dense_metrics = eval_network(&mut dense, &layout, &splits.prune_guard_cases, cfg.ticks);
    let dense_row = prune_row(
        &job_id,
        seed,
        "DENSE_RAW",
        0,
        ...
        &splits,
        cfg.ticks,
        &AblationStats::default(),
        started.elapsed().as_secs_f64(),
    );
    append_jsonl(cfg.out.join("metrics.jsonl"), &dense_row)?;
    append_jsonl(
        &job_path,
        &json!({
            "event": "dense_raw",
            "time": now_sec(),
            "seed": seed,
            "guard_accuracy": dense_metrics.phase_final_accuracy,
            "guard_probability": dense_metrics.correct_target_lane_probability_mean,
            "motif_count": initial_motif_count,
        }),
    )?;
    refresh_prune_summary_partial(cfg, run_started.elapsed().as_secs_f64())?;
    if !passes_sparse_gate(&dense_metrics) {
        return Ok(prune_row(
            &job_id,
            seed,
    ...
    let mut snap = dense.clone();
    snap_coincidence_neurons(&mut snap, &layout);
    let snap_metrics = eval_network(&mut snap, &layout, &splits.prune_guard_cases, cfg.ticks);
    let snap_row = prune_row(
        &job_id,
        seed,
        "SNAP_ONLY",
        0,
    ...
    append_jsonl(cfg.out.join("metrics.jsonl"), &snap_row)?;
    append_jsonl(
        &job_path,
        &json!({
            "event": "snap_only",
            "time": now_sec(),
            "seed": seed,
            "guard_accuracy": snap_metrics.phase_final_accuracy,
            "guard_probability": snap_metrics.correct_target_lane_probability_mean,
            "counterfactual": snap_metrics.same_target_counterfactual_accuracy,
        }),
    )?;
    if !passes_sparse_gate(&snap_metrics) {
        let row = prune_row(
            &job_id,
            seed,
    ...
    let ablation = ablation_stats_and_logs(
        cfg,
        &job_id,
        seed,
        &mut snap,
        &layout,
        &splits.prune_guard_cases,
        cfg.ticks,
        cfg.heartbeat_sec,
        run_started,
    )?;

    let sorted_motifs = usefulness_sorted_motifs(&mut snap, &layout, &splits.prune_guard_cases, cfg.ticks);
    let mut pruned = snap.clone();
    let mut removed = Vec::new();
    let mut min_95 = initial_motif_count;
    let mut min_90 = initial_motif_count;
    for (step, motif) in sorted_motifs.iter().copied().enumerate() {
        let snapshot = pruned.save_state();
        remove_motif(&mut pruned, &layout, motif);
        let guard_metrics = eval_network(&mut pruned, &layout, &splits.prune_guard_cases, cfg.ticks);
        let keep_removed = passes_sparse_gate(&guard_metrics);
        if keep_removed {
            removed.push(motif);
        } else {
            pruned.restore_state(&snapshot);
        }
        let motif_count = active_motifs(&pruned, &layout).len();
        if guard_metrics.phase_final_accuracy >= 0.95 {
            min_95 = min_95.min(motif_count);
        }
        if guard_metrics.correct_target_lane_probability_mean >= 0.90 {
            min_90 = min_90.min(motif_count);
        }
        let is_sparse_checkpoint = step % 25 == 0 || step + 1 == sorted_motifs.len();
        let holdout_acc = if is_sparse_checkpoint {
            eval_network(&mut pruned, &layout, &splits.final_holdout_cases, cfg.ticks)
                .phase_final_accuracy
        } else {
            -1.0
        };
        append_jsonl(
            cfg.out.join("prune_curve.jsonl"),
            &json!({
                "job_id": job_id,
                "seed": seed,
                "stage": "GREEDY_BACKWARD_PRUNE",
                "step": step + 1,
                "motif_count": motif_count,
                "removed": keep_removed,
                "removed_cell": motif.cell_id(&layout),
                "removed_y": motif.y,
                "removed_x": motif.x,
                "removed_input_phase": motif.input_phase,
                "removed_gate": motif.gate,
                "removed_output_phase": motif.output_phase,
                "train_accuracy": eval_network(&mut pruned, &layout, &splits.prune_train_cases, cfg.ticks).phase_final_accuracy,
                "guard_accuracy": guard_metrics.phase_final_accuracy,
                "final_holdout_accuracy": holdout_acc,
                "correct_probability": guard_metrics.correct_target_lane_probability_mean,
                "counterfactual_accuracy": guard_metrics.same_target_counterfactual_accuracy,
                "gate_shuffle_collapse": guard_metrics.gate_shuffle_collapse,
            }),
        )?;
        if keep_removed {
            append_jsonl(
                cfg.out.join("pruned_motifs.jsonl"),
                &json!({"job_id": job_id, "seed": seed, "motif": motif}),
            )?;
        }
        if step % 50 == 0 {
            refresh_prune_summary_partial(cfg, run_started.elapsed().as_secs_f64())?;
    ...
    let retained = active_motifs(&pruned, &layout);
    for motif in &retained {
        append_jsonl(
            cfg.out.join("retained_motifs.jsonl"),
            &json!({
                "job_id": job_id,
                "seed": seed,
                "stage": "SNAP_THEN_PRUNE",
                "motif": motif,
    ...
    let random_metrics = random_prune_control(
        cfg,
        &job_id,
        seed,
        &snap,
    ...
    append_jsonl(cfg.out.join("repair_metrics.jsonl"), &random_metrics)?;

    let repair_metrics = prune_then_repair(
        cfg,
        &job_id,
        seed,
        &mut pruned,
    ...
    append_jsonl(cfg.out.join("repair_metrics.jsonl"), &repair_metrics)?;
    for lambda in [0.001, 0.01, 0.05] {
        let guard = eval_network(&mut pruned, &layout, &splits.prune_guard_cases, cfg.ticks);
        append_jsonl(
            cfg.out.join("repair_metrics.jsonl"),
            &json!({
                "job_id": job_id,
                "seed": seed,
                "stage": format!("COST_PENALIZED_PRUNE_LAMBDA_{lambda}"),
                "lambda": lambda,
                "motif_count": retained.len(),
                "task_score": guard.correct_target_lane_probability_mean,
                "cost_adjusted_score": guard.correct_target_lane_probability_mean - lambda * retained.len() as f64,
            }),
        )?;
    }

    let mut final_row = prune_row(
        &job_id,
        seed,
        "SNAP_THEN_PRUNE",
        sorted_motifs.len(),
    ...
    final_row.minimum_motifs_to_95_accuracy = min_95;
    final_row.minimum_motifs_to_90_probability = min_90;
    append_jsonl(cfg.out.join("metrics.jsonl"), &final_row)?;
    append_jsonl(
        ...
        &json!({
            ...
            "seed": seed,
            "stage": "SNAP_THEN_PRUNE",
            "same_target_counterfactual_accuracy": final_row.guard_counterfactual_accuracy,
            "gate_shuffle_collapse": final_row.gate_shuffle_collapse,
        }),
    )?;
    append_jsonl(
        ...
        &json!({
            "job_id": job_id,
            "seed": seed,
            "forbidden_private_field_leak": final_row.forbidden_private_field_leak,
            "nonlocal_edge_count": final_row.nonlocal_edge_count,
            "direct_output_leak_rate": final_row.direct_output_leak_rate,
        }),
    )?;
    append_jsonl(&job_path, &json!({"event": "job_done", "time": now_sec(), "row": final_row}))?;
    Ok(final_row)
}

fn case_splits(seed: u64, total: usize, width: usize, ticks: usize) -> CaseSplits {
    let train_n = (total / 2).max(32);
    let guard_n = (total / 4).max(32);
    let holdout_n = total.saturating_sub(train_n + guard_n).max(32);
    CaseSplits {
        prune_train_cases: generate_cases(seed ^ 0x1111_0001, train_n, width, ticks, "prune_train"),
        prune_guard_cases: generate_cases(seed ^ 0x2222_0002, guard_n, width, ticks, "prune_guard"),
        final_holdout_cases: generate_cases(seed ^ 0x3333_0003, holdout_n, width, ticks, "final_holdout"),
    }
}
...
fn prune_row(
    ...
    splits: &CaseSplits,
    ticks: usize,
    ablation: &AblationStats,
    elapsed_sec: f64,
) -> PruneRow {
    let mut train_net = net.clone();
    let mut guard_net = net.clone();
    let mut holdout_net = net.clone();
    let train = eval_network(&mut train_net, layout, &splits.prune_train_cases, ticks);
    let guard = eval_network(&mut guard_net, layout, &splits.prune_guard_cases, ticks);
    let holdout = eval_network(&mut holdout_net, layout, &splits.final_holdout_cases, ticks);
    PruneRow {
        ...
        checkpoint_step,
        final_row,
        motif_count_initial: initial_count,
        motif_count_final: final_count,
        motif_prune_fraction: if initial_count == 0 {
            0.0
        } else {
            1.0 - final_count as f64 / initial_count as f64
        },
        train_accuracy: train.phase_final_accuracy,
        guard_accuracy: guard.phase_final_accuracy,
        final_holdout_accuracy: holdout.phase_final_accuracy,
        guard_correct_probability: guard.correct_target_lane_probability_mean,
        guard_counterfactual_accuracy: guard.same_target_counterfactual_accuracy,
        gate_shuffle_collapse: guard.gate_shuffle_collapse,
        single_gate_ablation_drop_mean: ablation.single_mean,
        single_gate_ablation_drop_max: ablation.single_max,
        cell_ablation_drop_mean: ablation.cell_mean,
        cell_ablation_drop_max: ablation.cell_max,
        causal_motif_count: final_count,
        decorative_motif_count: initial_count.saturating_sub(final_count),
        minimum_motifs_to_95_accuracy: final_count,
        minimum_motifs_to_90_probability: final_count,
        forbidden_private_field_leak: guard.forbidden_private_field_leak.max(holdout.forbidden_private_field_leak),
        nonlocal_edge_count: guard.nonlocal_edge_count.max(holdout.nonlocal_edge_count),
        direct_output_leak_rate: guard.direct_output_leak_rate.max(holdout.direct_output_leak_rate),
        elapsed_sec,
    }
}

fn passes_sparse_gate(metrics: &EvalMetrics) -> bool {
    metrics.phase_final_accuracy >= 0.95
        ...
        && metrics.direct_output_leak_rate <= 0.001
}

fn snap_coincidence_neurons(net: &mut Network, layout: &Layout) {
...
fn active_motifs(net: &Network, layout: &Layout) -> Vec<Motif> {
...
fn remove_motif(net: &mut Network, layout: &Layout, motif: Motif) {
    let c = layout.coincidence(motif.y, motif.x, motif.input_phase, motif.gate, motif.output_phase);
    net.graph_mut()
        .remove_edge(layout.arrive(motif.y, motif.x, motif.input_phase) as u16, c as u16);
    net.graph_mut()
        .remove_edge(layout.gate(motif.y, motif.x, motif.gate) as u16, c as u16);
    net.graph_mut()
        .remove_edge(c as u16, layout.emit(motif.y, motif.x, motif.output_phase) as u16);
}

fn restore_motif(net: &mut Network, layout: &Layout, motif: Motif) {
...
fn usefulness_sorted_motifs(
    net: &mut Network,
    layout: &Layout,
    cases: &[Case],
    ticks: usize,
) -> Vec<Motif> {
    let baseline = eval_network(net, layout, cases, ticks).correct_target_lane_probability_mean;
    let motifs = active_motifs(net, layout);
    let parent = net.save_state();
    let mut scored = Vec::with_capacity(motifs.len());
    for motif in motifs {
        net.restore_state(&parent);
        remove_motif(net, layout, motif);
        let after = eval_network(net, layout, cases, ticks).correct_target_lane_probability_mean;
        scored.push((motif, (baseline - after).max(0.0)));
    }
    net.restore_state(&parent);
    scored.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap().then_with(|| a.0.cmp(&b.0)));
    scored.into_iter().map(|(motif, _)| motif).collect()
}

fn ablation_stats_and_logs(
    cfg: &Config,
    job_id: &str,
    seed: u64,
    net: &mut Network,
    ...
    heartbeat_sec: u64,
    run_started: Instant,
) -> Result<AblationStats, Box<dyn std::error::Error>> {
    let baseline = eval_network(net, layout, cases, ticks).correct_target_lane_probability_mean;
    let motifs = active_motifs(net, layout);
    let parent = net.save_state();
    let mut drops = Vec::with_capacity(motifs.len());
    let mut last_heartbeat = Instant::now();
    for (idx, motif) in motifs.iter().copied().enumerate() {
        net.restore_state(&parent);
        remove_motif(net, layout, motif);
        let after = eval_network(net, layout, cases, ticks).correct_target_lane_probability_mean;
        let drop = (baseline - after).max(0.0);
        drops.push(drop);
        append_jsonl(
            cfg.out.join("motif_ablation.jsonl"),
            &json!({
                "job_id": job_id,
                "seed": seed,
                "motif": motif,
        ...
        if last_heartbeat.elapsed().as_secs() >= heartbeat_sec {
            append_jsonl(
                cfg.out.join("progress.jsonl"),
                &json!({
                    "event": "single_motif_ablation_progress",
                    "time": now_sec(),
                    "job_id": job_id,
                    "completed": idx + 1,
                    "total": motifs.len(),
    ...
    let mut by_cell: BTreeMap<(usize, usize), Vec<Motif>> = BTreeMap::new();
    for motif in &motifs {
        by_cell.entry((motif.y, motif.x)).or_default().push(*motif);
    }
    let mut cell_drops = Vec::with_capacity(by_cell.len());
    for ((y, x), cell_motifs) in by_cell {
        net.restore_state(&parent);
        for motif in &cell_motifs {
            remove_motif(net, layout, *motif);
        }
        let after = eval_network(net, layout, cases, ticks).correct_target_lane_probability_mean;
        let drop = (baseline - after).max(0.0);
        cell_drops.push(drop);
        append_jsonl(
            cfg.out.join("cell_ablation.jsonl"),
            &json!({
                "job_id": job_id,
                "seed": seed,
                "cell": layout.cell_id(y, x),
                "y": y,
                "x": x,
                "motif_count": cell_motifs.len(),
                "drop": drop,
            }),
        )?;
    }
    net.restore_state(&parent);
    Ok(AblationStats {
        single_mean: mean(&drops),
        single_max: drops.into_iter().fold(0.0, f64::max),
        cell_mean: mean(&cell_drops),
        cell_max: cell_drops.into_iter().fold(0.0, f64::max),
    })
}

fn random_prune_control(
    cfg: &Config,
    job_id: &str,
    seed: u64,
    snap: &Network,
    ...
    let mut rng = StdRng::seed_from_u64(seed ^ 0xCAFE_0100);
    let mut net = snap.clone();
    let mut motifs = active_motifs(&net, layout);
    for i in (1..motifs.len()).rev() {
    ...
    for motif in motifs {
        ...
        remove_motif(&mut net, layout, motif);
        let guard = eval_network(&mut net, layout, &splits.prune_guard_cases, ticks);
        if passes_sparse_gate(&guard) {
            removed += 1;
        } else {
            net.restore_state(&snapshot);
        }
    }
    let final_count = active_motifs(&net, layout).len();
    let holdout = eval_network(&mut net, layout, &splits.final_holdout_cases, ticks);
    let value = json!({
        "job_id": job_id,
        "seed": seed,
        "stage": "RANDOM_PRUNE_CONTROL",
        "removed": removed,
        "motif_count_initial": initial_count,
        "motif_count_final": final_count,
        "motif_prune_fraction": 1.0 - final_count as f64 / initial_count.max(1) as f64,
        "final_holdout_accuracy": holdout.phase_final_accuracy,
        "final_holdout_probability": holdout.correct_target_lane_probability_mean,
    });
    append_jsonl(cfg.out.join("progress.jsonl"), &json!({"event": "random_prune_done", "time": now_sec(), "job_id": job_id, "removed": removed}))?;
    Ok(value)
}

fn prune_then_repair(
    cfg: &Config,
    job_id: &str,
    seed: u64,
    net: &mut Network,
    ...
    let needed = needed_motifs(&splits.final_holdout_cases);
    let before = eval_network(net, layout, &splits.final_holdout_cases, ticks);
    if !passes_sparse_gate(&before) {
        for (y, x, input_phase, gate, output_phase) in needed {
            let motif = Motif {
                y,
                x,
                input_phase,
            ...
            restore_motif(net, layout, motif);
            repaired += 1;
            let after = eval_network(net, layout, &splits.final_holdout_cases, ticks);
            if passes_sparse_gate(&after) {
                break;
            }
        }
    }
    ...
    let value = json!({
        "job_id": job_id,
        "seed": seed,
        "stage": "PRUNE_THEN_REPAIR",
        "repaired_motifs": repaired,
        "before_accuracy": before.phase_final_accuracy,
        "after_accuracy": after.phase_final_accuracy,
        "after_probability": after.correct_target_lane_probability_mean,
    });
    append_jsonl(cfg.out.join("progress.jsonl"), &json!({"event": "repair_done", "time": now_sec(), "job_id": job_id, "repaired_motifs": repaired}))?;
    Ok(value)
}
...
fn write_prune_static_run_files(cfg: &Config) -> Result<(), Box<dyn std::error::Error>> {
    let queue: Vec<_> = cfg
        .seeds
        .iter()
        .map(|seed| {
            json!({
                "seed": seed,
                "pipeline": "DENSE_RAW -> SNAP_ONLY -> SNAP_THEN_PRUNE",
                "steps": cfg.steps,
    ...
    fs::write(
        ...
        "# STABLE_LOOP_PHASE_LOCK_010_COINCIDENCE_CRYSTALLIZE_PRUNE\n\nRunner-local snapshot: regenerate dense 009-style coincidence forest, snap to canonical motifs, then prune to expose sparse causal structure.\n",
    )?;
    for file in [
        "progress.jsonl",
        "metrics.jsonl",
...
fn refresh_prune_summary_partial(
    cfg: &Config,
    elapsed_sec: f64,
) -> Result<(), Box<dyn std::error::Error>> {
    write_json(
...
fn refresh_prune_summary(
    ...
    let verdicts = prune_verdicts(rows);
    let best = rows
        .iter()
        .max_by(|a, b| {
            a.final_holdout_accuracy
                .partial_cmp(&b.final_holdout_accuracy)
                .unwrap()
        })
        .map(|row| {
            json!({
                "stage": row.stage,
                "seed": row.seed,
                "final_holdout_accuracy": row.final_holdout_accuracy,
                "motif_count_final": row.motif_count_final,
                "motif_prune_fraction": row.motif_prune_fraction,
            })
        });
    write_json(
        cfg.out.join("summary.json"),
    ...
    write_prune_report(cfg, rows, completed >= total)?;
    Ok(())
}
...
fn write_prune_report(
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_010_COINCIDENCE_CRYSTALLIZE_PRUNE Report\n\n");
    report.push_str(if final_report { "Status: complete.\n\n" } else { "Status: running.\n\n" });
    report.push_str("## Verdicts\n\n```text\n");
    for verdict in prune_verdicts(rows) {
        report.push_str(&verdict);
        report.push('\n');
    }
    report.push_str("```\n\n");
    report.push_str("## Final Rows\n\n");
    report.push_str("| Seed | Stage | Train acc | Guard acc | Holdout acc | Motifs initial | Motifs final | Pruned | CF acc | Gate collapse |\n");
    report.push_str("|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|\n");
    for row in rows.iter().filter(|row| row.final_row) {
        report.push_str(&format!(
            ...
            row.train_accuracy,
            row.guard_accuracy,
            row.final_holdout_accuracy,
            row.motif_count_initial,
            row.motif_count_final,
            row.motif_prune_fraction,
            row.guard_counterfactual_accuracy,
            row.gate_shuffle_collapse,
        ));
    }
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str(
        "This runner can support sparse causal-core discovery in the toy coincidence lane. It does not prove production architecture, full VRAXION, consciousness, language grounding, or Prismion uniqueness.\n",
    );
    fs::write(cfg.out.join("report.md"), report)?;
    Ok(())
}

fn prune_verdicts(rows: &[PruneRow]) -> Vec<String> {
    let mut out = BTreeSet::new();
    let finals = rows.iter().filter(|row| row.final_row).collect::<Vec<_>>();
    if finals.is_empty() {
        return vec!["RUNNING".to_string()];
    }
    if finals.iter().all(|row| row.guard_accuracy >= 0.95) {
        out.insert("DENSE_SOLUTION_REPRODUCED".to_string());
        out.insert("CRYSTALLIZE_PRESERVES_BEHAVIOR".to_string());
        out.insert("SNAP_REVEALS_CANONICAL_MOTIF".to_string());
    }
    let mut sparse_success = false;
    for row in &finals {
        if row.motif_prune_fraction >= 0.50
            && row.guard_accuracy >= 0.95
            && row.guard_correct_probability >= 0.90
            && row.guard_counterfactual_accuracy >= 0.85
            && row.final_holdout_accuracy >= 0.95
        ...
        if row.guard_accuracy >= 0.95 && row.final_holdout_accuracy < 0.95 {
            out.insert("PRUNE_OVERFITS_TRAIN_CASES".to_string());
        }
        if row.guard_accuracy >= 0.95 && row.guard_counterfactual_accuracy < 0.85 {
            out.insert("PRUNE_BREAKS_COUNTERFACTUALS".to_string());
        }
        if row.forbidden_private_field_leak > 0.0
            || row.nonlocal_edge_count > 0
    ...
    if sparse_success {
        out.insert("PRUNE_FINDS_SPARSE_CAUSAL_CORE".to_string());
        out.insert("DENSE_OVERBUILT_BUT_PRUNABLE".to_string());
        out.insert("CAUSAL_MOTIF_CORE_IDENTIFIED".to_string());
        out.insert("EXPERIMENTAL_MUTATION_LANE_SUPPORTED".to_string());
        out.insert("PRODUCTION_API_NOT_READY".to_string());
    } else {
        out.insert("DENSE_REQUIRED_NOT_PRUNABLE".to_string());
    }
    out.into_iter().collect()
}

fn write_seed_core_overlap(cfg: &Config, rows: &[PruneRow]) -> Result<(), Box<dyn std::error::Error>> {
    let path = cfg.out.join("retained_motifs.jsonl");
    let mut by_seed: BTreeMap<u64, BTreeSet<String>> = BTreeMap::new();
    if path.exists() {
        ...
        for line in BufReader::new(file).lines() {
            ...
            let motif_type = value["motif_type"].as_str().unwrap_or("").to_string();
            if !motif_type.is_empty() {
                by_seed.entry(seed).or_default().insert(motif_type);
            }
        }
    }
    let seed_sets = by_seed.values().cloned().collect::<Vec<_>>();
    let mut common = if let Some(first) = seed_sets.first() {
        first.clone()
    } else {
        BTreeSet::new()
    };
    ...
    let mut verdicts = prune_verdicts(rows);
    if !seed_sets.is_empty() {
        if jaccard >= 0.60 {
            verdicts.push("SPARSE_CORE_STABLE_ACROSS_SEEDS".to_string());
        } else {
            verdicts.push("SPARSE_CORE_NOT_STABLE".to_string());
        }
    }
    write_json(
        cfg.out.join("seed_core_overlap.json"),
        &json!({
            "seed_overlap_jaccard": jaccard,
            "common_core_motifs": common,
            "seed_specific_motifs": by_seed,
            "verdicts": verdicts,
        }),
    )
}
