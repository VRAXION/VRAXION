//! Concrete train -> inference scaleout probe.
//!
//! 046 expands the 045 behavior gate with more task families, longer inputs,
//! more output classes, and stronger shortcut controls.
...
impl ArmKind {
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::BaseTrainNoRouteGrammar => "BASE_TRAIN_NO_ROUTE_GRAMMAR",
            Self::FinalTraining044Reference => "CONCRETE_INFERENCE_045_REFERENCE",
            Self::RouteGrammarTrainAndInfer => "ROUTE_GRAMMAR_SCALEOUT_TRAIN_AND_INFER",
            Self::RouteGrammarTrainAndInferRollbackGated => "ROUTE_GRAMMAR_SCALEOUT_ROLLBACK_GATED",
            Self::RouteGrammarInferenceOnlyAblation => {
                "ROUTE_GRAMMAR_SCALEOUT_INFERENCE_ONLY_ABLATION"
            }
            Self::RouteGrammarTrainOnlyAblation => "ROUTE_GRAMMAR_SCALEOUT_TRAIN_ONLY_ABLATION",
            Self::RouteGrammarShuffledLabels => "ROUTE_GRAMMAR_SCALEOUT_SHUFFLED_LABELS",
            Self::RouteGrammarShuffledInputOrder => "ROUTE_GRAMMAR_SCALEOUT_SHUFFLED_INPUT_ORDER",
            Self::NonRouteRegressionControl => "NON_ROUTE_SCALEOUT_REGRESSION_CONTROL",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_046_CONCRETE_INFERENCE_SCALEOUT",
            "seeds": cfg.seeds,
    ...
    write_json(
        ...
        &json!({
            "contract": "STABLE_LOOP_PHASE_LOCK_046_CONCRETE_INFERENCE_SCALEOUT",
            "positive_gate": "heldout>=0.92, ood>=0.85, family_min>=0.80, entropy>=2.25, anti-collapse controls fail"
        }),
    )?;
    ...
    for (idx, arm) in arms.iter().enumerate() {
        ...
        append_jsonl(
            &cfg.out.join("concrete_inference_metrics.jsonl"),
            &result.row,
        )?;
        if arm.is_control() {
...
fn derive_verdicts(rows: &[MetricsRow]) -> Vec<String> {
    ...
    let main = get("ROUTE_GRAMMAR_SCALEOUT_TRAIN_AND_INFER");
    let rollback = get("ROUTE_GRAMMAR_SCALEOUT_ROLLBACK_GATED");
    let shuffled = get("ROUTE_GRAMMAR_SCALEOUT_SHUFFLED_LABELS");
    ...
    if main_pass && rollback_pass && controls_fail && shuffled_fails {
        verdicts.insert("CONCRETE_INFERENCE_SCALEOUT_POSITIVE".to_string());
        verdicts.insert("SCALEOUT_INPUT_CONDITIONING_SURVIVES".to_string());
        verdicts.insert("LONG_SEQUENCE_GENERALIZATION_PASSES".to_string());
        verdicts.insert("MORE_OUTPUT_CLASSES_PASS".to_string());
        ...
        verdicts.insert("SCALEOUT_NEEDS_MORE_BEHAVIORAL_WORK".to_string());
        if rows
            .iter()
...
fn write_report(out: &Path, rows: &[MetricsRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    text.push_str("# STABLE_LOOP_PHASE_LOCK_046_CONCRETE_INFERENCE_SCALEOUT\n\n");
    ...
    text.push_str("```\n\n## Arm Metrics\n\n");
    text.push_str("| arm | heldout | ood | family_min | top_output_rate | entropy | collapse |\n");
    text.push_str("| --- | ---: | ---: | ---: | ---: | ---: | --- |\n");
    for row in rows {
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_046_concrete_inference_scaleout/dev",
    );
