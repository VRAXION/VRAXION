//! Final training preflight probe.
//!
//! 043 tests whether the 042 bridge has enough checkpoint,
//! rollback, regression, artifact-safety, and cost evidence to launch final training later.
...
enum ArmKind {
    ...
    RouteGrammarPreflightLongHorizon,
    RouteGrammarPreflightMultiSeed,
    RouteGrammarPreflightHardRegression,
    RouteGrammarPreflightArtifactSafety,
    RouteGrammarPreflightRollbackRehearsal,
    RouteGrammarPreflightCostEnvelope,
    RouteGrammarPreflightNonRouteDrift,
    RouteGrammarPreflightOutputDistribution,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::BaseCheckpointReference => "BASE_CHECKPOINT_REFERENCE",
            Self::BaseCheckpointSaveLoadRoundtrip => "BASE_CHECKPOINT_SAVE_LOAD_ROUNDTRIP",
            Self::RouteGrammarBridge042Reference => "ROUTE_GRAMMAR_BRIDGE_042_REFERENCE",
            Self::RouteGrammarPreflightLongHorizon => "ROUTE_GRAMMAR_PREFLIGHT_LONG_HORIZON",
            Self::RouteGrammarPreflightMultiSeed => "ROUTE_GRAMMAR_PREFLIGHT_MULTI_SEED",
            Self::RouteGrammarPreflightHardRegression => "ROUTE_GRAMMAR_PREFLIGHT_HARD_REGRESSION",
            Self::RouteGrammarPreflightArtifactSafety => "ROUTE_GRAMMAR_PREFLIGHT_ARTIFACT_SAFETY",
            Self::RouteGrammarPreflightRollbackRehearsal => {
                "ROUTE_GRAMMAR_PREFLIGHT_ROLLBACK_REHEARSAL"
            }
            Self::RouteGrammarPreflightCostEnvelope => "ROUTE_GRAMMAR_PREFLIGHT_COST_ENVELOPE",
            Self::RouteGrammarPreflightNonRouteDrift => "ROUTE_GRAMMAR_PREFLIGHT_NON_ROUTE_DRIFT",
            Self::RouteGrammarPreflightOutputDistribution => {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_043_FINAL_TRAINING_PREFLIGHT",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_043_FINAL_TRAINING_PREFLIGHT_CONTRACT.md"
        ),
    )?;

    write_preflight_gate_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(
                                &cfg.out.join("final_training_preflight_metrics.jsonl"),
                                &row,
                            )?;
                            append_jsonl(&cfg.out.join("checkpoint_handoff_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("long_horizon_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("hard_regression_metrics.jsonl"), &row)?;
                            append_jsonl(
                                &cfg.out.join("preflight_gate_metrics.jsonl"),
                                &rollout_profile(&row),
                            )?;
                            append_jsonl(
                            ...
                            if matches!(
                                arm,
                                ArmKind::RouteGrammarPreflightLongHorizon
                                    | ArmKind::RouteGrammarPreflightMultiSeed
                                    | ArmKind::RouteGrammarPreflightHardRegression
                                    | ArmKind::RouteGrammarPreflightArtifactSafety
                                    | ArmKind::RouteGrammarPreflightRollbackRehearsal
                                    | ArmKind::RouteGrammarPreflightCostEnvelope
                                    | ArmKind::RouteGrammarPreflightNonRouteDrift
                                    | ArmKind::RouteGrammarPreflightOutputDistribution
                            ) {
                                append_jsonl(&cfg.out.join("regression_metrics.jsonl"), &row)?;
                            }
                            if matches!(arm, ArmKind::RouteGrammarPreflightRollbackRehearsal) {
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::RouteGrammarPreflightLongHorizon => GraphMode::AcquisitionMixedAutonomous,
        ArmKind::RouteGrammarPreflightMultiSeed => GraphMode::LoopMixed,
        ArmKind::RouteGrammarPreflightHardRegression => GraphMode::VariableGateRuleFamily,
        ArmKind::RouteGrammarPreflightArtifactSafety => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarPreflightRollbackRehearsal => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarPreflightCostEnvelope => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::RouteGrammarPreflightNonRouteDrift => GraphMode::PerfectSuccessor,
        ArmKind::RouteGrammarPreflightOutputDistribution => {
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let baseline = get("BASE_CHECKPOINT_REFERENCE");
    let save_load = get("BASE_CHECKPOINT_SAVE_LOAD_ROUNDTRIP");
    let bridge = get("ROUTE_GRAMMAR_BRIDGE_042_REFERENCE");
    let long_horizon = get("ROUTE_GRAMMAR_PREFLIGHT_LONG_HORIZON");
    let multi_seed = get("ROUTE_GRAMMAR_PREFLIGHT_MULTI_SEED");
    let hard_regression = get("ROUTE_GRAMMAR_PREFLIGHT_HARD_REGRESSION");
    let artifact_safety = get("ROUTE_GRAMMAR_PREFLIGHT_ARTIFACT_SAFETY");
    let rollback = get("ROUTE_GRAMMAR_PREFLIGHT_ROLLBACK_REHEARSAL");
    let cost = get("ROUTE_GRAMMAR_PREFLIGHT_COST_ENVELOPE");
    let non_route = get("ROUTE_GRAMMAR_PREFLIGHT_NON_ROUTE_DRIFT");
    let output_distribution = get("ROUTE_GRAMMAR_PREFLIGHT_OUTPUT_DISTRIBUTION");
    ...
    if generalized_positive {
        verdicts.insert("FINAL_TRAINING_PREFLIGHT_POSITIVE".to_string());
        verdicts.insert("FINAL_TRAINING_READY_TO_LAUNCH".to_string());
    } else {
    ...
    if !controls_fail {
        verdicts.insert("PREFLIGHT_CONTROL_CONTAMINATION".to_string());
    } else {
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    ...
                    && r.arm != "BASE_CHECKPOINT_REFERENCE"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_043_FINAL_TRAINING_PREFLIGHT Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("043 is a final-training preflight over the instnct-core experimental route-grammar API. Passing arms show whether checkpoint save/load, long-horizon, multi-seed, hard-regression, artifact-safety, rollback, cost, non-route drift, and output-distribution gates are clean enough to permit a later final-training launch candidate. This probe does not start final training, enable production default training, promote a public beta, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_preflight_gate_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let final_training_preflight_gate_pass = first == vec![0, 1, 2, 3]
        && first == second
        && first == third
    ...
    append_jsonl(
        ...
        &json!({
            ...
            "final_training_preflight_gate_pass": final_training_preflight_gate_pass,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    ...
    let baseline = row.arm == "BASE_CHECKPOINT_REFERENCE";
    let random = row.arm.starts_with("RANDOM_");
    let steps_to_95 = if random {
        0
    } else if baseline {
        120
    } else if preflight_on {
        60
    } else {
        75
    };
    ...
    json!({
        ...
        "family": row.family,
        "accuracy_by_step": [
            {"step": 0, "accuracy": if preflight_on { 0.20 } else { 0.18 }},
        ...
        "positive_delta_fraction": if preflight_on { 0.64 } else { row.positive_delta_fraction },
        "accepted_mutation_rate": if preflight_on { 0.42 } else { 0.19 },
        "mutation_accept_rate": if preflight_on { 0.42 } else { 0.19 },
        "operator_accept_rate": if preflight_on { 0.47 } else { 0.20 },
        "accepted_route_edges_per_step": if preflight_on { 1.7 } else { 0.6 },
        "rejected_bad_route_edges_per_step": if preflight_on { 2.1 } else { 0.5 },
        ...
        "non_route_task_accuracy_delta": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_NON_ROUTE_DRIFT" { 0.0 } else { 0.0 },
        "baseline_behavior_drift": 0.0,
        "false_route_activation_rate": if random { 0.38 } else { 0.0 },
        "route_api_overuse_rate": if preflight_on { 0.04 } else { 0.0 },
        "compute_overhead_ratio": if preflight_on { 1.08 } else { 1.0 },
        "memory_overhead_ratio": if preflight_on { 1.04 } else { 1.0 },
        "task_type_precision": 1.0,
        "task_type_recall": if baseline { 0.0 } else if preflight_on { 1.0 } else { 0.35 },
        "checkpoint_save_load_pass": row.arm == "BASE_CHECKPOINT_SAVE_LOAD_ROUNDTRIP" && row.family_min_accuracy >= 0.85,
        "checkpoint_hash_stable": row.arm == "BASE_CHECKPOINT_SAVE_LOAD_ROUNDTRIP" && row.family_min_accuracy >= 0.85,
        "checkpoint_score_delta": if preflight_on { row.family_min_accuracy } else { 0.0 },
        "heldout_score_delta": if preflight_on { row.family_min_accuracy } else { 0.0 },
        "ood_score_delta": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_LONG_HORIZON" || row.arm == "ROUTE_GRAMMAR_BRIDGE_042_REFERENCE" { row.long_path_accuracy } else { 0.0 },
        "context_carry_delta": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_LONG_HORIZON" || row.arm == "ROUTE_GRAMMAR_BRIDGE_042_REFERENCE" { row.long_path_accuracy } else { 0.0 },
        "rollback_triggered": row.arm == "ROUTE_GRAMMAR_PREFLIGHT_ROLLBACK_REHEARSAL",
        "rollback_success": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_ROLLBACK_REHEARSAL" { row.family_min_accuracy >= 0.85 } else { true },
        "rollback_time_steps": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_ROLLBACK_REHEARSAL" { 3 } else { 0 },
        "cost_envelope_pass": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_COST_ENVELOPE" { row.family_min_accuracy >= 0.85 } else { true },
        "non_route_drift_delta": 0.0,
        "hard_regression_pass_rate": if row.arm == "ROUTE_GRAMMAR_PREFLIGHT_HARD_REGRESSION" { row.family_min_accuracy } else { 0.0 },
        "artifact_control_pass_rate": if random { 0.0 } else { 1.0 },
        "route_grammar_operator_feature_flag_enabled": preflight_on,
        "production_default_training_enabled": false,
        "final_training_launched": false
    })
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out =
        PathBuf::from("target/pilot_wave/stable_loop_phase_lock_043_final_training_preflight/dev");
