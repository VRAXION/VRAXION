//! Weighted edges: each edge has an integer weight (1-8).
//! Spike effect = activation * weight (not just ±1).
//!
//! Run: cargo run --example weighted_addition --release
...
#[derive(Clone)]
struct WeightedNet {
    edges: Vec<(u16, u16, i8)>,  // (source, target, weight) — weight in -8..8
    threshold: Vec<u8>,
    channel: Vec<u8>,
    polarity: Vec<i8>,
    charge: Vec<i16>,
...
impl WeightedNet {
    fn new(h: usize, rng: &mut impl Rng) -> Self {
        ...
        WeightedNet { edges: Vec::new(), threshold, channel, polarity, charge: vec![0; h], activation: vec![0; h], h }
    }
    ...
    fn propagate(&mut self, input: &[i8], tick: usize) {
        ...
        for &(src, tgt, weight) in &self.edges {
            ...
            if act != 0 {
                // Spike effect scaled by weight
                incoming[t] = incoming[t].saturating_add(act as i16 * weight as i16);
            }
        }
    ...
    fn mutate_weight(&mut self, rng: &mut impl Rng) -> bool {
        if self.edges.is_empty() { return false; }
        let idx = rng.gen_range(0..self.edges.len());
        self.edges[idx].2 = rng.gen_range(-8..=8i8);
        true
    }
    ...
    fn mutate(&mut self, rng: &mut impl Rng) -> bool {
        match rng.gen_range(0..100u32) {
            0..15 => { if self.edges.len() >= EDGE_CAP { return false; }
                self.add_edge(rng.gen_range(0..self.h) as u16, rng.gen_range(0..self.h) as u16, rng.gen_range(1..=4i8)) }
            15..25 => self.remove_edge(rng),
            25..45 => self.rewire(rng),
            45..65 => self.mutate_weight(rng),  // 20% weight mutation!
...
fn eval(net: &mut WeightedNet, proj: &Proj, examples: &[(usize,usize,usize)], sdr_a: &[Vec<i8>], sdr_b: &[Vec<i8>]) -> f64 {
    let mut correct = 0;
...
fn main() {
    ...
    println!("=== WEIGHTED EDGE ADDITION ===");
    println!("Edge weight: i8 (-8..8). Spike effect = activation * weight.");
    println!("H={}, {}s/seed, separated I/O, train sum≠4, test sum=4\n", H, WALL_SECS);

    println!("--- Weighted edges ---");
    for &seed in &[42u64, 1042, 2042, 3042, 4042] {
        let mut rng = StdRng::seed_from_u64(seed);
        let mut net = WeightedNet::new(H, &mut rng);
        let mut proj = Proj::new(H, SUMS, &mut rng);

        // Init bridges with random weights
        for _ in 0..50 {
            let s = rng.gen_range(0..INPUT_END) as u16; let t = rng.gen_range(OUTPUT_START..H) as u16;
            net.add_edge(s, t, rng.gen_range(1..=4i8));
        }
        ...
        net.edges.clear();
        let no_edge = eval(&mut net, &proj, &all, &sdr_a, &sdr_b);
        net.edges = edges_bak;

        // Weight stats
        let weights: Vec<i8> = net.edges.iter().map(|e| e.2).collect();
        let mean_w = weights.iter().map(|&w| w as f64).sum::<f64>() / weights.len().max(1) as f64;
        let neg = weights.iter().filter(|&&w| w < 0).count();

        println!("  seed {}: train={:.0}% test={:.0}% noedge={:.0}% | edges={} mean_w={:.1} neg={} ",
            seed, train_acc*100.0, test_acc*100.0, no_edge*100.0, net.edges.len(), mean_w, neg);
    }

    // Comparison: unweighted (weight=1 always)
    println!("\n--- Unweighted (weight=1, same topology mutations) ---");
    for &seed in &[42u64, 1042, 2042] {
        let mut rng = StdRng::seed_from_u64(seed);
        let mut net = WeightedNet::new(H, &mut rng);
        let mut proj = Proj::new(H, SUMS, &mut rng);
        for _ in 0..50 { let s = rng.gen_range(0..INPUT_END) as u16; let t = rng.gen_range(OUTPUT_START..H) as u16; net.add_edge(s, t, 1); }
        ...
        while Instant::now() < deadline {
            ...
            if roll < 25 { let (pi,pc,old) = proj.mutate(&mut rng); let after = eval(&mut net, &proj, &train, &sdr_a, &sdr_b); if after <= before { proj.undo(pi,pc,old); } }
                ...
                let genome = net.save();
                // Only topology + params, no weight mutation
                let mutated = match rng.gen_range(0..100u32) {
                    0..20 => { if net.edges.len() >= EDGE_CAP { false } else { net.add_edge(rng.gen_range(0..H) as u16, rng.gen_range(0..H) as u16, 1) } }
                    20..35 => net.remove_edge(&mut rng), 35..60 => net.rewire(&mut rng),
                    60..80 => { let n = rng.gen_range(0..H); net.threshold[n] = rng.gen_range(0..=15); true }
                    80..92 => { let n = rng.gen_range(0..H); net.channel[n] = rng.gen_range(1..=8); true }
                    _ => { net.polarity[rng.gen_range(0..H)] *= -1; true }
                };
                if mutated { let after = eval(&mut net, &proj, &train, &sdr_a, &sdr_b); if after <= before { net.restore(genome); } }
            }
        }
        let train_acc = eval(&mut net, &proj, &train, &sdr_a, &sdr_b);
        let test_acc = eval(&mut net, &proj, &test, &sdr_a, &sdr_b);
        println!("  seed {}: train={:.0}% test={:.0}% edges={}", seed, train_acc*100.0, test_acc*100.0, net.edges.len());
    }
}
