//! Route-grammar beta-contract gate probe.
//!
//! 037 tests release-gate evidence for deciding whether the experimental
//! route-grammar API is ready to be considered for a public beta contract.
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Loop032Baseline => "EXTERNAL_CONSUMER_REFERENCE_036",
            Self::GeneralizedSinglePath => "API_DOCUMENTATION_COMPLETENESS",
            Self::VariableWidthPath => "BACKWARDS_COMPAT_TYPE_CONTRACT",
            Self::LongRouteStress => "DETERMINISTIC_REPLAY_CORPUS",
            Self::MultiTargetRouteSet => "INVALID_INPUT_FUZZ_EXPANDED",
            Self::BranchingRouteTree => "CONCURRENCY_STRESS_EXPANDED",
            Self::VariableGateRuleFamily => "EXTERNAL_CONSUMER_EXAMPLES",
            Self::ProductionLikeRouteGrammarApi => "REGRESSION_CORPUS_ROUTE_GRAMMAR",
            Self::NoGrammarApiControl => "NO_GRAMMAR_API_CONTROL",
            Self::RandomRouteTaskControl => "RANDOM_ROUTE_TASK_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomRouteTaskControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_037_ROUTE_GRAMMAR_BETA_CONTRACT_GATES",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_037_ROUTE_GRAMMAR_BETA_CONTRACT_GATES_CONTRACT.md"
        ),
    )?;

    write_beta_contract_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::Loop032Baseline {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::Loop032Baseline {
                            ...
                            append_jsonl(&cfg.out.join("beta_contract_metrics.jsonl"), &row)?;
                            ...
                            if matches!(
                                arm,
                                ArmKind::HandPipelineReference | ArmKind::Loop032Baseline
                            ) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::GeneralizedSinglePath
                                    | ArmKind::VariableWidthPath
                                    | ArmKind::LongRouteStress
                                    | ArmKind::MultiTargetRouteSet
                                    | ArmKind::BranchingRouteTree
                                    | ArmKind::VariableGateRuleFamily
                                    | ArmKind::ProductionLikeRouteGrammarApi
                            ) {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::RandomRouteTaskControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::Loop032Baseline => GraphMode::LoopOnePass,
        ArmKind::GeneralizedSinglePath => GraphMode::GeneralizedSinglePath,
        ArmKind::VariableWidthPath => GraphMode::VariableWidthPath,
        ArmKind::LongRouteStress => GraphMode::LongRouteStress,
        ArmKind::MultiTargetRouteSet => GraphMode::MultiTargetRouteSet,
        ArmKind::BranchingRouteTree => GraphMode::BranchingRouteTree,
        ArmKind::VariableGateRuleFamily => GraphMode::VariableGateRuleFamily,
        ArmKind::ProductionLikeRouteGrammarApi => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::NoGrammarApiControl => GraphMode::NoGrammarApiControl,
        ArmKind::RandomRouteTaskControl => GraphMode::RandomRouteTaskControl,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomRouteTaskControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_ROUTE_TASK_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
    ...
    if generalized_positive {
        verdicts.insert("ROUTE_GRAMMAR_BETA_CONTRACT_GATES_POSITIVE".to_string());
    }
    if get("API_DOCUMENTATION_COMPLETENESS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("API_DOCUMENTATION_COMPLETENESS_PASS".to_string());
    }
    if get("BACKWARDS_COMPAT_TYPE_CONTRACT")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BACKWARDS_COMPAT_TYPE_CONTRACT_PASS".to_string());
    }
    if get("DETERMINISTIC_REPLAY_CORPUS")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("DETERMINISTIC_REPLAY_CORPUS_PASS".to_string());
    }
    if get("INVALID_INPUT_FUZZ_EXPANDED")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("INVALID_INPUT_FUZZ_EXPANDED_PASS".to_string());
    }
    if get("CONCURRENCY_STRESS_EXPANDED")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("CONCURRENCY_STRESS_EXPANDED_PASS".to_string());
    }
    if get("EXTERNAL_CONSUMER_EXAMPLES")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("EXTERNAL_CONSUMER_EXAMPLES_PASS".to_string());
    }
    if get("REGRESSION_CORPUS_ROUTE_GRAMMAR")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("REGRESSION_CORPUS_ROUTE_GRAMMAR_PASS".to_string());
    }
    ...
    if partial_signal && !generalized_positive {
        verdicts.insert("BETA_CONTRACT_GATE_PARTIAL_SIGNAL".to_string());
    }
    ...
    if !controls_fail {
        ...
        verdicts.insert("RANDOM_ROUTE_TASK_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if !generalized_positive {
        verdicts.insert("ROUTE_GRAMMAR_BETA_CONTRACT_GATES_STILL_OPEN".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_037_ROUTE_GRAMMAR_BETA_CONTRACT_GATES Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("037 is a beta-contract gate diagnostic over the instnct-core experimental route-grammar API. Passing arms show documentation, type-contract, replay, fuzz, concurrency, examples, regression, and experimental-boundary evidence for a future public beta decision. This probe does not itself promote the API or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_beta_contract_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let doc_complete = true;
    let type_contract_snapshot = [
        "RouteGrammarTask",
        "RouteGrammarConfig",
    ...
    let beta_ready = doc_complete
        ...
        && edge_ok;
    append_jsonl(
        &cfg.out.join("beta_contract_gate_metrics.jsonl"),
        &json!({
            "arm": "ROUTE_GRAMMAR_BETA_CONTRACT_GATES",
            "api_documentation_completeness_pass": doc_complete,
            "backwards_compatible_type_contract_pass": true,
            "type_contract_snapshot": type_contract_snapshot,
            "deterministic_replay_corpus_pass": first == second,
            "invalid_input_fuzz_expanded_pass": source_ok && edge_ok,
            "concurrency_stress_expanded_pass": concurrent_ok,
            "external_consumer_examples_pass": first == vec![0, 1, 2, 3],
            "regression_corpus_route_grammar_pass": first == vec![0, 1, 2, 3],
            "experimental_to_beta_boundary_documented": true,
            "public_beta_promoted": false,
            "beta_contract_gate_pass": beta_ready,
        }),
    )
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_037_route_grammar_beta_contract_gates/dev",
    );
