//! Runner-local hard-negative grammar distillation probe.
//!
//! 029 tests whether the partial weak-label signal from 028 can be completed
//! by hard negatives that look good under aggregate delivery but violate route
//! grammar.
...
enum ArmKind {
    HandGrammarSupervisionReference,
    CounterfactualCorruption028Baseline,
    MixedWeakLabel028Baseline,
    ShortcutDeliversWrongOrderNegatives,
    BranchReachesTargetNegatives,
    CycleReachesTargetNegatives,
    MissingSuccessorAltPathNegatives,
    DuplicateDeliveryNegatives,
    StaleDeliveryNegatives,
    FamilyMinAdversarialNegatives,
    HighAggregateLowFamilyMinNegatives,
    HardNegativeMixedDistillation,
    HardNegativeCurriculumShortToLong,
    HardNegativeTeacherStudent,
    RandomHardNegativeControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            Self::HandGrammarSupervisionReference => "HAND_GRAMMAR_SUPERVISION_REFERENCE",
            Self::CounterfactualCorruption028Baseline => "COUNTERFACTUAL_CORRUPTION_028_BASELINE",
            Self::MixedWeakLabel028Baseline => "MIXED_WEAK_LABEL_028_BASELINE",
            Self::ShortcutDeliversWrongOrderNegatives => "SHORTCUT_DELIVERS_WRONG_ORDER_NEGATIVES",
            Self::BranchReachesTargetNegatives => "BRANCH_REACHES_TARGET_NEGATIVES",
            Self::CycleReachesTargetNegatives => "CYCLE_REACHES_TARGET_NEGATIVES",
            Self::MissingSuccessorAltPathNegatives => "MISSING_SUCCESSOR_ALT_PATH_NEGATIVES",
            Self::DuplicateDeliveryNegatives => "DUPLICATE_DELIVERY_NEGATIVES",
            Self::StaleDeliveryNegatives => "STALE_DELIVERY_NEGATIVES",
            Self::FamilyMinAdversarialNegatives => "FAMILY_MIN_ADVERSARIAL_NEGATIVES",
            Self::HighAggregateLowFamilyMinNegatives => "HIGH_AGGREGATE_LOW_FAMILY_MIN_NEGATIVES",
            Self::HardNegativeMixedDistillation => "HARD_NEGATIVE_MIXED_DISTILLATION",
            Self::HardNegativeCurriculumShortToLong => "HARD_NEGATIVE_CURRICULUM_SHORT_TO_LONG",
            Self::HardNegativeTeacherStudent => "HARD_NEGATIVE_TEACHER_STUDENT",
            Self::RandomHardNegativeControl => "RANDOM_HARD_NEGATIVE_CONTROL",
            Self::RandomPhaseRuleControl => "RANDOM_PHASE_RULE_CONTROL",
        }
    }

    fn diagnostic_only(self) -> bool {
        matches!(self, Self::HandGrammarSupervisionReference)
    }

    fn is_control(self) -> bool {
        matches!(
            self,
            Self::RandomHardNegativeControl | Self::RandomPhaseRuleControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_029_HARD_NEGATIVE_GRAMMAR_DISTILLATION",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_029_HARD_NEGATIVE_GRAMMAR_DISTILLATION_CONTRACT.md"
        ),
    )?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        let baseline = eval_bucket(
                            ArmKind::CounterfactualCorruption028Baseline,
                            seed,
                            width,
                            path_length,
                            ticks,
                        ...
                        for arm in arms() {
                            ...
                            let mut row = if arm == ArmKind::CounterfactualCorruption028Baseline {
                                baseline.clone()
                            } else {
                                eval_bucket(
                                    arm,
                            ...
                            if arm == ArmKind::CounterfactualCorruption028Baseline {
                            ...
                            append_jsonl(&cfg.out.join("hard_negative_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("distillation_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("grammar_metrics.jsonl"), &row)?;
                            if matches!(
                                arm,
                                ArmKind::CounterfactualCorruption028Baseline
                                    | ArmKind::MixedWeakLabel028Baseline
                            ) {
                                append_jsonl(&cfg.out.join("teacher_trace_metrics.jsonl"), &row)?;
                            }
                            if matches!(
                                arm,
                                ArmKind::ShortcutDeliversWrongOrderNegatives
                                    | ArmKind::BranchReachesTargetNegatives
                                    | ArmKind::CycleReachesTargetNegatives
                                    | ArmKind::MissingSuccessorAltPathNegatives
                                    | ArmKind::DuplicateDeliveryNegatives
                                    | ArmKind::StaleDeliveryNegatives
                                    | ArmKind::FamilyMinAdversarialNegatives
                                    | ArmKind::HighAggregateLowFamilyMinNegatives
                                    | ArmKind::HardNegativeMixedDistillation
                            ) {
                                append_jsonl(
                                    &cfg.out.join("synthetic_corruption_metrics.jsonl"),
                                    &row,
                                )?;
                            }
                            if arm == ArmKind::HardNegativeCurriculumShortToLong {
                                append_jsonl(&cfg.out.join("curriculum_metrics.jsonl"), &row)?;
                            }
                            if arm == ArmKind::HardNegativeTeacherStudent {
                                append_jsonl(&cfg.out.join("self_training_metrics.jsonl"), &row)?;
                            }
...
fn arms() -> Vec<ArmKind> {
    vec![
        ArmKind::HandGrammarSupervisionReference,
        ArmKind::CounterfactualCorruption028Baseline,
        ArmKind::MixedWeakLabel028Baseline,
        ArmKind::ShortcutDeliversWrongOrderNegatives,
        ArmKind::BranchReachesTargetNegatives,
        ...
        ArmKind::HighAggregateLowFamilyMinNegatives,
        ArmKind::HardNegativeMixedDistillation,
        ArmKind::HardNegativeCurriculumShortToLong,
        ArmKind::HardNegativeTeacherStudent,
        ArmKind::RandomHardNegativeControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ArmKind::HandGrammarSupervisionReference => GraphMode::PerfectSuccessor,
        ArmKind::CounterfactualCorruption028Baseline => GraphMode::CounterfactualCorruption,
        ArmKind::MixedWeakLabel028Baseline => GraphMode::MixedWeakLabelDistillation,
        ArmKind::ShortcutDeliversWrongOrderNegatives => GraphMode::HardNegativeShortcut,
        ArmKind::BranchReachesTargetNegatives => GraphMode::HardNegativeBranch,
        ArmKind::CycleReachesTargetNegatives => GraphMode::HardNegativeCycle,
        ArmKind::MissingSuccessorAltPathNegatives => GraphMode::HardNegativeMissingSuccessor,
        ArmKind::DuplicateDeliveryNegatives => GraphMode::HardNegativeDuplicateDelivery,
        ArmKind::StaleDeliveryNegatives => GraphMode::HardNegativeStaleDelivery,
        ArmKind::FamilyMinAdversarialNegatives => GraphMode::HardNegativeFamilyMin,
        ArmKind::HighAggregateLowFamilyMinNegatives => GraphMode::HardNegativeHighAggregateTrap,
        ArmKind::HardNegativeMixedDistillation => GraphMode::HardNegativeMixed,
        ArmKind::HardNegativeCurriculumShortToLong => GraphMode::HardNegativeCurriculum,
        ArmKind::HardNegativeTeacherStudent => GraphMode::HardNegativeTeacherStudent,
        ArmKind::RandomHardNegativeControl => GraphMode::RandomDense,
...
fn eval_bucket(
    ...
    MetricsRow {
        ...
        random_same_count_accuracy: if arm == ArmKind::RandomHardNegativeControl {
            accuracy
        } else {
            control_accuracy.unwrap_or(0.0)
        },
...
fn initial_edge_count(mode: GraphMode, case: &Case, seed: u64) -> usize {
    match mode {
        ...
        | GraphMode::HardNegativeMixed
        | GraphMode::HardNegativeCurriculum
        | GraphMode::HardNegativeTeacherStudent => source_target_anchor_dense_edges(case).len(),
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let controls_fail = !ranking.iter().any(|r| {
        matches!(
            r.arm.as_str(),
            "RANDOM_HARD_NEGATIVE_CONTROL" | "RANDOM_PHASE_RULE_CONTROL"
        ) && r.phase_final_accuracy >= 0.75
    });
    if get("HAND_GRAMMAR_SUPERVISION_REFERENCE")
        .map(|r| r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("HAND_GRAMMAR_SUPERVISION_REFERENCE_REPRODUCED".to_string());
    }
    let distillation_positive = ranking.iter().any(|r| {
        r.positive
            && !r.diagnostic_only
            && !r.arm.starts_with("RANDOM_")
            && r.arm != "HAND_GRAMMAR_SUPERVISION_REFERENCE"
    });
    if distillation_positive {
        verdicts.insert("HARD_NEGATIVE_GRAMMAR_DISTILLATION_POSITIVE".to_string());
    }
    if get("SHORTCUT_DELIVERS_WRONG_ORDER_NEGATIVES")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("SHORTCUT_NEGATIVES_REQUIRED".to_string());
    }
    if get("BRANCH_REACHES_TARGET_NEGATIVES")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCH_CYCLE_NEGATIVES_REQUIRED".to_string());
    }
    if get("CYCLE_REACHES_TARGET_NEGATIVES")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("BRANCH_CYCLE_NEGATIVES_REQUIRED".to_string());
    }
    if get("FAMILY_MIN_ADVERSARIAL_NEGATIVES")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FAMILY_MIN_NEGATIVES_REQUIRED".to_string());
    }
    if get("HIGH_AGGREGATE_LOW_FAMILY_MIN_NEGATIVES")
        .map(|r| r.signal || r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("HIGH_AGGREGATE_TRAP_NEGATIVES_REQUIRED".to_string());
    }
    if get("COUNTERFACTUAL_CORRUPTION_028_BASELINE")
        .map(|r| r.signal)
        .unwrap_or(false)
    {
        verdicts.insert("COUNTERFACTUAL_CORRUPTION_SIGNAL_CONFIRMED".to_string());
    }
    if !distillation_positive {
        verdicts.insert("WEAK_LABELS_STILL_INSUFFICIENT".to_string());
    }
    ...
    if !controls_fail {
        ...
        verdicts.insert("RANDOM_HARD_NEGATIVE_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if distillation_positive {
        verdicts.insert("ORDER_OBJECTIVE_DISTILLED_FROM_HARD_NEGATIVES".to_string());
    } else {
        verdicts.insert("EXTERNAL_GRAMMAR_TEACHER_STILL_REQUIRED".to_string());
    }
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_029_HARD_NEGATIVE_GRAMMAR_DISTILLATION Report\n\n");
    ...
    report.push_str("```\n\n## Mechanism Ranking\n\n");
    report.push_str("| Arm | Acc | SuffFinal | Long | Family min | Wrong-if-delivered | Retained succ | Order | HardNeg P | HardNeg R | FPR | FNR | Missing succ | Branch | Cycle | Continuity | Reach | Gate collapse | Signal | Positive |\n");
    report.push_str("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|\n");
    for row in ranking {
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("029 is a hard-negative grammar distillation diagnostic. Passing hard-negative arms would show that high-aggregate but grammar-invalid traps can supply the missing route-grammar labels. This probe does not claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_locality_audit(cfg: &Config) -> std::io::Result<()> {
    let audit = json!({
        ...
        "hard_negative_distillation_arms": [
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_029_hard_negative_grammar_distillation/dev",
    );
