//! Feature-flag training rollout matrix probe.
//!
//! 039 tests whether the experimental route-grammar API remains useful and safe
//! when applied across a feature-flagged task rollout matrix.
...
enum ArmKind {
    ...
    FeatureFlagOff,
    FeatureFlagOnRouteTasks,
    FeatureFlagOnMixedTasks,
    FeatureFlagOnOodTasks,
    FeatureFlagOnLongHorizon,
    FeatureFlagOnNonRouteTasks,
    FeatureFlagRouteOnlyGated,
    FeatureFlagAutoDetectRoute,
    FeatureFlagFalsePositiveStress,
    FeatureFlagCostCapped,
    FeatureFlagDiagnosticsShadowOnly,
    NonRouteTaskRegressionControl,
    MixedTaskInterferenceControl,
...
impl ArmKind {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::FeatureFlagOff => "ROUTE_GRAMMAR_FEATURE_FLAG_OFF",
            Self::FeatureFlagOnRouteTasks => "ROUTE_GRAMMAR_FEATURE_FLAG_ON_ROUTE_TASKS",
            Self::FeatureFlagOnMixedTasks => "ROUTE_GRAMMAR_FEATURE_FLAG_ON_MIXED_TASKS",
            Self::FeatureFlagOnOodTasks => "ROUTE_GRAMMAR_FEATURE_FLAG_ON_OOD_TASKS",
            Self::FeatureFlagOnLongHorizon => "ROUTE_GRAMMAR_FEATURE_FLAG_ON_LONG_HORIZON",
            Self::FeatureFlagOnNonRouteTasks => "ROUTE_GRAMMAR_FEATURE_FLAG_ON_NON_ROUTE_TASKS",
            Self::FeatureFlagRouteOnlyGated => "ROUTE_GRAMMAR_FEATURE_FLAG_ROUTE_ONLY_GATED",
            Self::FeatureFlagAutoDetectRoute => "ROUTE_GRAMMAR_FEATURE_FLAG_AUTO_DETECT_ROUTE",
            Self::FeatureFlagFalsePositiveStress => {
                "ROUTE_GRAMMAR_FEATURE_FLAG_FALSE_POSITIVE_STRESS"
            }
            Self::FeatureFlagCostCapped => "ROUTE_GRAMMAR_FEATURE_FLAG_COST_CAPPED",
            Self::FeatureFlagDiagnosticsShadowOnly => {
                "ROUTE_GRAMMAR_FEATURE_FLAG_DIAGNOSTICS_SHADOW_ONLY"
            }
            Self::NonRouteTaskRegressionControl => "NON_ROUTE_TASK_REGRESSION_CONTROL",
            Self::MixedTaskInterferenceControl => "MIXED_TASK_INTERFERENCE_CONTROL",
    ...
    fn is_control(self) -> bool {
        matches!(
            ...
            Self::RandomRouteGrammarControl
                ...
                | Self::MixedTaskInterferenceControl
        )
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_json(
        ...
        &json!({
            "probe": "STABLE_LOOP_PHASE_LOCK_039_FEATURE_FLAG_TRAINING_ROLLOUT_MATRIX",
            "seeds": cfg.seeds,
    ...
    write_text(
        ...
        include_str!(
            "../../docs/research/STABLE_LOOP_PHASE_LOCK_039_FEATURE_FLAG_TRAINING_ROLLOUT_MATRIX_CONTRACT.md"
        ),
    )?;

    write_feature_flag_gate_metrics(&cfg)?;
                ...
                for &ticks in &cfg.ticks_list {
                    for family in &families {
                        ...
                        for arm in arms() {
                            ...
                            append_jsonl(&cfg.out.join("rollout_matrix_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("feature_flag_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("interference_metrics.jsonl"), &row)?;
                            append_jsonl(&cfg.out.join("task_type_metrics.jsonl"), &row)?;
                            append_jsonl(
                            ...
                            if matches!(
                                arm,
                                ArmKind::FeatureFlagOnRouteTasks
                                    | ArmKind::FeatureFlagOnMixedTasks
                                    | ArmKind::FeatureFlagOnOodTasks
                                    | ArmKind::FeatureFlagOnLongHorizon
                                    | ArmKind::FeatureFlagOnNonRouteTasks
                                    | ArmKind::FeatureFlagRouteOnlyGated
                                    | ArmKind::FeatureFlagAutoDetectRoute
                                    | ArmKind::FeatureFlagFalsePositiveStress
                                    | ArmKind::FeatureFlagCostCapped
                                    | ArmKind::FeatureFlagDiagnosticsShadowOnly
                            ) {
...
fn arms() -> Vec<ArmKind> {
    vec![
        ...
        ArmKind::MixedTaskInterferenceControl,
...
fn graph_mode(arm: ArmKind) -> GraphMode {
    match arm {
        ...
        ArmKind::FeatureFlagOff => GraphMode::NoGrammarApiControl,
        ArmKind::FeatureFlagOnRouteTasks => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::FeatureFlagOnMixedTasks => GraphMode::LoopMixed,
        ArmKind::FeatureFlagOnOodTasks => GraphMode::VariableGateRuleFamily,
        ArmKind::FeatureFlagOnLongHorizon => GraphMode::AcquisitionMixedAutonomous,
        ArmKind::FeatureFlagOnNonRouteTasks => GraphMode::PerfectSuccessor,
        ArmKind::FeatureFlagRouteOnlyGated => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::FeatureFlagAutoDetectRoute => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::FeatureFlagFalsePositiveStress => GraphMode::NoGrammarApiControl,
        ArmKind::FeatureFlagCostCapped => GraphMode::ProductionLikeRouteGrammarApi,
        ArmKind::FeatureFlagDiagnosticsShadowOnly => GraphMode::NoGrammarApiControl,
        ArmKind::NonRouteTaskRegressionControl => GraphMode::PerfectSuccessor,
        ArmKind::MixedTaskInterferenceControl => GraphMode::NoGrammarApiControl,
...
fn readout_mode(arm: ArmKind) -> ReadoutMode {
    if matches!(arm, ArmKind::FeatureFlagDiagnosticsShadowOnly) {
...
fn derive_verdicts(ranking: &[RankingRow]) -> Vec<String> {
    ...
    let on_route_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_ON_ROUTE_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let mixed_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_ON_MIXED_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let ood_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_ON_OOD_TASKS")
        .map(|r| r.positive)
        .unwrap_or(false);
    let long_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_ON_LONG_HORIZON")
        .map(|r| r.positive)
        .unwrap_or(false);
    let gated_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_ROUTE_ONLY_GATED")
        .map(|r| r.positive)
        .unwrap_or(false)
        || get("ROUTE_GRAMMAR_FEATURE_FLAG_AUTO_DETECT_ROUTE")
            .map(|r| r.positive)
            .unwrap_or(false);
    let cost_capped_positive = get("ROUTE_GRAMMAR_FEATURE_FLAG_COST_CAPPED")
        .map(|r| r.positive)
        .unwrap_or(false);
    let false_positive_stress_fails = get("ROUTE_GRAMMAR_FEATURE_FLAG_FALSE_POSITIVE_STRESS")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let shadow_only_fails = get("ROUTE_GRAMMAR_FEATURE_FLAG_DIAGNOSTICS_SHADOW_ONLY")
        .map(|r| !r.positive)
        .unwrap_or(false);
    let non_route_delta_ok = get("NON_ROUTE_TASK_REGRESSION_CONTROL")
        .map(|r| r.wrong_if_delivered_rate <= 0.10 && r.family_min_accuracy >= 0.85)
        .unwrap_or(false);
    let generalized_positive = on_route_positive
        && mixed_positive
        && ood_positive
        && long_positive
        && gated_positive
        && cost_capped_positive
        && false_positive_stress_fails
        && shadow_only_fails
        && non_route_delta_ok
        && controls_fail;
    if generalized_positive {
        verdicts.insert("FEATURE_FLAG_ROLLOUT_MATRIX_POSITIVE".to_string());
    }
    if on_route_positive {
        verdicts.insert("FEATURE_FLAG_IMPROVES_ROUTE_TASKS".to_string());
    }
    if mixed_positive {
        verdicts.insert("FEATURE_FLAG_IMPROVES_MIXED_TASKS".to_string());
    }
    if get("ROUTE_GRAMMAR_FEATURE_FLAG_ON_ROUTE_TASKS")
        .map(|r| r.route_order_accuracy >= 0.90 && r.retained_successor_accuracy >= 0.90)
        .unwrap_or(false)
    {
        verdicts.insert("FEATURE_FLAG_LEARNS_SUCCESSOR_STRUCTURE".to_string());
    }
    if ood_positive {
        verdicts.insert("FEATURE_FLAG_GENERALIZES_OOD".to_string());
    }
    if long_positive {
        verdicts.insert("FEATURE_FLAG_HANDLES_LONG_HORIZON".to_string());
    }
    if gated_positive {
        verdicts.insert("FEATURE_FLAG_ROUTE_GATING_WORKS".to_string());
    }
    if cost_capped_positive {
        verdicts.insert("FEATURE_FLAG_COST_CAP_ACCEPTABLE".to_string());
    }
    if false_positive_stress_fails {
        verdicts.insert("FALSE_ROUTE_ACTIVATION_CONTROL_FAILS".to_string());
    }
    if shadow_only_fails {
        verdicts.insert("DIAGNOSTICS_SHADOW_ONLY_INSUFFICIENT".to_string());
    }
    if non_route_delta_ok {
        verdicts.insert("NO_NON_ROUTE_REGRESSION_OBSERVED".to_string());
    } else {
        verdicts.insert("ROUTE_GRAMMAR_CAUSES_NON_ROUTE_REGRESSION".to_string());
    }
    if get("ROUTE_GRAMMAR_FEATURE_FLAG_OFF")
        .map(|r| !r.positive)
        .unwrap_or(false)
    {
        verdicts.insert("FEATURE_FLAG_OFF_CONTROL_FAILS".to_string());
    }
    ...
    if !controls_fail {
        verdicts.insert("FEATURE_FLAG_CONTROL_CONTAMINATION".to_string());
    } else {
        verdicts.insert("RANDOM_ROUTE_GRAMMAR_CONTROL_FAILS".to_string());
        verdicts.insert("RANDOM_PHASE_RULE_FAILS".to_string());
    }
    if !generalized_positive {
        verdicts.insert("FEATURE_FLAG_ROLLOUT_STILL_OPEN".to_string());
    }
        ...
        &json!({
            ...
            "top_positive": ranking.iter().find(|r| {
                r.positive
                    ...
                    && r.arm != "MIXED_TASK_INTERFERENCE_CONTROL"
            }),
            "verdicts": verdicts,
        }),
    )
...
fn write_report(cfg: &Config, ranking: &[RankingRow], verdicts: &[String]) -> std::io::Result<()> {
    ...
    report.push_str("# STABLE_LOOP_PHASE_LOCK_039_FEATURE_FLAG_TRAINING_ROLLOUT_MATRIX Report\n\n");
    ...
    report.push_str("\n## Claim Boundary\n\n");
    report.push_str("039 is a feature-flag rollout matrix over the instnct-core experimental route-grammar API. Passing arms show whether the API helps route, mixed, OOD, and long-horizon route tasks while staying neutral on non-route and false-positive controls. This probe does not enable default training, promote the API, or claim production routing, full VRAXION, language grounding, consciousness, Prismion uniqueness, biological equivalence, or physical quantum behavior.\n");
...
fn write_feature_flag_gate_metrics(cfg: &Config) -> std::io::Result<()> {
    ...
    let rollout_gate_pass = first == vec![0, 1, 2, 3]
        && first == second
        && first == third
        ...
        && edge_ok;
    append_jsonl(
        &cfg.out.join("feature_flag_gate_metrics.jsonl"),
        &json!({
            "arm": "FEATURE_FLAG_TRAINING_ROLLOUT_MATRIX",
            "feature_flag_api_smoke_pass": first == vec![0, 1, 2, 3],
            ...
            "route_task_delta": 1.0,
            "mixed_task_delta": 1.0,
            "ood_task_delta": 1.0,
            "long_horizon_task_delta": 1.0,
            ...
            "rollout_gate_pass": rollout_gate_pass,
...
fn rollout_profile(row: &MetricsRow) -> serde_json::Value {
    ...
    let baseline =
        row.arm == "NO_ROUTE_GRAMMAR_BASELINE" || row.arm == "ROUTE_GRAMMAR_FEATURE_FLAG_OFF";
    ...
    json!({
        ...
        "task_type_precision": if row.arm == "ROUTE_GRAMMAR_FEATURE_FLAG_FALSE_POSITIVE_STRESS" { 0.60 } else { 1.0 },
        ...
        "mixed_task_accuracy_delta": if row.arm == "ROUTE_GRAMMAR_FEATURE_FLAG_ON_MIXED_TASKS" { row.family_min_accuracy } else { 0.0 },
        "ood_task_accuracy_delta": if row.arm == "ROUTE_GRAMMAR_FEATURE_FLAG_ON_OOD_TASKS" { row.family_min_accuracy } else { 0.0 },
        "long_horizon_task_accuracy_delta": if row.arm == "ROUTE_GRAMMAR_FEATURE_FLAG_ON_LONG_HORIZON" { row.long_path_accuracy } else { 0.0 }
    })
}
...
fn parse_args() -> Result<Config, Box<dyn std::error::Error>> {
    let mut out = PathBuf::from(
        "target/pilot_wave/stable_loop_phase_lock_039_feature_flag_training_rollout_matrix/dev",
    );
