...
REM  LCX: single L0, hash-bucketed, 61803 slots (phi*100K), key_dim=62, top_k=6
REM  Bottleneck: 618→62→C19→62→C19→618
REM  Starts Beta: tt=1, batch=10, LCX=ON
REM
REM  Params: ~4.35M   LCX: ~164MB   VRAM: ~308MB total   Step: ~0.74s
REM
REM  EFFORT TIERS (via Grafana or controls.json):
REM  ------------------------------------------------------------------
REM  Alpha(Reflex):     tt=0  batch=10  lcx=OFF
REM  Beta(Recall):      tt=1  batch=10  lcx=ON   <-- START
