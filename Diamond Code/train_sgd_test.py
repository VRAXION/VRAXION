"""
Test Diamond Code with vanilla SGD (NO momentum) vs Adam.

Hypothesis 4: Adam's momentum causes oscillations in TRUE Möbius.
Test: SGD with momentum=0.0 should eliminate oscillations if hypothesis is correct.

Expected outcomes:
- SGD reaches 75% WITHOUT oscillations → Adam momentum was the problem
- SGD still oscillates → Oscillations are intrinsic to TRUE Möbius
- SGD stuck at 50% → Gradient signal itself is broken (catastrophic)
"""
...
def main():
    # TASK CONFIG - Same as Adam baseline
    num_keys = 2
    num_pairs = 2
    seq_len = 32
    ...
    print("=" * 70)
    print("DIAMOND CODE - SGD TEST (NO MOMENTUM)")
    print("=" * 70)
    print(f"Task: {num_keys} keys, {num_pairs} pairs, seq_len={seq_len}")
    print(f"Model: {num_positions}x{embedding_dim}D")
    print(f"Optimizer: SGD (lr=0.01, momentum=0.0)")
    print()

    # Create model with Möbius helix
    ...
    # SGD with NO momentum (vs Adam lr=0.001)
    # SGD typically needs 10x higher LR than Adam
    ...
    # Log file
    log_path = Path(__file__).parent / "logs" / "diamond" / "sgd_test.log"
    ...
    log("=" * 70)
    log(f"SGD Test: {num_keys} keys, {num_pairs} pairs, len={seq_len}")
    log(f"Model: {num_positions}x{embedding_dim}D = {total_params} params")
    log(f"Optimizer: SGD(lr=0.01, momentum=0.0)")
    log("=" * 70)
    ...
    try:
        ...
        while True:
            ...
            # Eval every 10 steps (capture oscillations if they exist)
            if step % 10 == 0:
                model.eval()
                with torch.no_grad():
