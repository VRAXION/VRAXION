"""
Diamond Code Training with Dashboard-Compatible Logging.

This script trains RingMemoryModel on the COPY task and outputs metrics
in a format compatible with diamond_dashboard.py for real-time visualization.

Usage:
    python train_with_logging.py --steps 1000 --log logs/diamond/train.log
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train Diamond Code with logging")
    parser.add_argument('--steps', type=int, default=1000, help='Number of training steps')
    parser.add_argument('--log', type=str, default='logs/diamond/train.log', help='Log file path')
    ...
    x_data = torch.randint(0, 10, (args.batch_size, args.seq_len, 1)).float()
    y_labels = x_data[:, -1, 0].long()

    log("=" * 70)
    log("DIAMOND CODE - Training with Logging")
    log("=" * 70)
    log(f"Task: COPY (predict last token)")
    log(f"Data: {x_data.shape}, Labels: {y_labels.shape}, Classes: 10")
    ...
    for step in range(args.steps):
        ...
        with torch.no_grad():
            ...
            # Jump gate activation rate (approximation from model internals)
            # We'll track this by monitoring pointer position changes
            if step > 0 and 'pointer_trajectory' in debug_info:
                # Calculate how often pointer jumped vs walked
                ptr_traj = debug_info['pointer_trajectory']
                if len(ptr_traj) > 1:
                    # Rough estimate: jumps are large position changes
                    jumps = 0
                    total_moves = 0
                    for i in range(1, len(ptr_traj)):
                        ...
                        # Jumps are changes > 1.5 (not just sequential walk)
            ...
            # Pointer position (mean across batch)
            ...
            # Other gradient norm (sum of all other params)
            other_grad_norm = 0.0
