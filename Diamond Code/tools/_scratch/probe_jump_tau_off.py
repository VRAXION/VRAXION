#!/usr/bin/env python3
"""
Probe: JUMP_SIGMOID_TAU — ON (0.5) vs OFF (1.0)
=================================================
tau=0.5 = current production (sharpened sigmoid)
tau=1.0 = "off" — natural sigmoid, no temperature scaling

Quick A/B: 500 steps, 2 seeds, echo256 BLOCK=4, CPU.
"""
...
CONFIGS = [
    ('tau_0.5_ON',   0.5),   # current production
    ('tau_1.0_OFF',  1.0),   # tau "disabled" — natural sigmoid
]
...
LIVE_LOG     = os.path.join(LOG_DIR, 'probe_jump_tau_off_live.log')
os.makedirs(LOG_DIR, exist_ok=True)

with open(LIVE_LOG, 'w') as f:
    f.write(f'# probe_jump_tau_off — {time.strftime("%Y-%m-%d %H:%M:%S")}\n')
...
if __name__ == '__main__':
    print('=' * 70)
    print('PROBE: JUMP_SIGMOID_TAU — ON (0.5) vs OFF (1.0)')
    print('=' * 70)
    ...
    swarm_model.JUMP_SIGMOID_TAU = 0.5

    # Results
    print('\n' + '=' * 70)
    print('RESULTS: TAU ON (0.5) vs OFF (1.0)')
    print('=' * 70)
    print(f'  {"Config":<15} {"TAU":>5} {"sig(2/TAU)":>10} {"Mean Tail":>10} '
          f'{"Gap":>8} {"Seeds":>20}')
    print(f'  {"-"*15} {"-"*5} {"-"*10} {"-"*10} {"-"*8} {"-"*20}')

    for label, r in results.items():
        sig_val = 1.0 / (1.0 + math.exp(-2.0 / r['tau']))
        seeds_str = ', '.join(f'{t:.4f}' for t in r['tails'])
        print(f'  {label:<15} {r["tau"]:>5.1f} {sig_val:>10.3f} '
              f'{r["mean_tail"]:>10.4f} {r["seed_gap"]:>8.4f} {seeds_str:>20}')

    on_result = results.get('tau_0.5_ON', {}).get('mean_tail', 0)
    off_result = results.get('tau_1.0_OFF', {}).get('mean_tail', 0)
    diff = off_result - on_result
    print(f'\n  Delta (OFF - ON): {diff:+.4f}  ({diff*100:+.2f}%)')
    if abs(diff) < 0.005:
        print('  VERDICT: Negligible difference — tau scaling is cosmetic here.')
    elif diff < 0:
        print(f'  VERDICT: tau=0.5 wins by {abs(diff)*100:.2f}% — keep it.')
    else:
        print(f'  VERDICT: tau=1.0 (OFF) wins by {diff*100:.2f}% — tau scaling hurts!')
