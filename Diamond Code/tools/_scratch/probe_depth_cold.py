"""
probe_depth_cold.py — Level 2 of CPU probe ladder (#100)
Sweeps depth={2, 4, 8, 12} on copy_echo256, pure brain (tt=0, no LCX).
...
LR           = 1e-3
STEP_TIMEOUT = 30
LOG_PATH     = Path('logs/probe/probe_depth_cold_live.log')

DEPTH_VALUES = [2, 4, 8, 12]
...
def make_batch(batch_size, seq_len, seed_offset=0):
    """copy_echo256: target bit = same as input bit (trivial copy task).
    Pads 8 real bits + 8 zeros per byte to match num_bits=16 (GEM path)."""
    ...
    for _ in range(batch_size):
        ...
        for b in byte_vals.tolist():
            ...
            x_bits.extend(bits + [0]*8)   # pad to 16
            y_bits.extend(bits)
...
# ── Single run ───────────────────────────────────────────────────────────────
def run_depth(depth, seed):
    label = f'[depth={depth} s={seed}]'

    torch.manual_seed(seed)
    random.seed(seed)

    cfg = {**BASE_CFG, 'depth': depth}
    model = sm.SwarmByteRingModel(**cfg).to(DEVICE)
    ...
    for step in range(STEPS):
        ...
        optimizer.zero_grad()
        out = model(x)          # [B, T, num_bits] logits

        # out has shape [B, T, num_bits] — we only supervise first 8 bits
        logits = out[:, :, :8]
        ...
        s_per_step = elapsed
        log(f'{label} step {step} | loss {loss.item():.6f} | acc={acc:.4f} RD:{s_per_step:.4f} traction={acc:.4f} shard=0/0')
...
# ── Main sweep ───────────────────────────────────────────────────────────────
def main():
    log('=== probe_depth_cold.py — Level 2 CPU probe ladder ===')
    log(f'depth values: {DEPTH_VALUES}  seeds: {SEEDS}')
    log(f'D=128, tt=0, no LCX, copy_echo256, {STEPS} steps')
    log('')
    ...
    for r in all_results:
        by_depth[r['depth']].append(r['mean_tail'])

    log('\n\n=== RANKED RESULTS ===')
    log(f'{"depth":<10} {"mean_tail":>10} {"seed_gap":>10}')
    log('-' * 35)
    ranked = sorted(by_depth.items(), key=lambda x: sum(x[1])/len(x[1]), reverse=True)
    for depth, tails in ranked:
        mean = sum(tails)/len(tails)
        gap  = max(tails) - min(tails)
        log(f'{depth:<10} {mean:>10.4f} {gap:>10.4f}')

    winner = ranked[0][0]
    log(f'\nWINNER: depth={winner}')

    # ── Write summary ──────────────────────────────────────────────────────
    summary_path = Path('logs/probe/probe_depth_results.txt')
    with open(summary_path, 'w') as f:
        f.write('=== probe_depth_cold.py results ===\n')
        for r in all_results:
            f.write(f"depth={r['depth']} seed={r['seed']} mean_tail={r['mean_tail']:.4f} "
