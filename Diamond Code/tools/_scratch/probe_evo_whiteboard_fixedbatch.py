#!/usr/bin/env python3
"""
Probe: Evolutionary Whiteboard — FIXED SINGLE BATCH variant
=============================================================
Same as probe_evo_whiteboard_cpu.py but trains on ONE fixed batch
every step. Tests whether cross-batch contamination is why the
whiteboard doesn't help.

If the whiteboard helps with a fixed batch but not with random batches,
the problem is cross-batch noise (different data overwrites the slots).
"""
...
def run_config(cfg, fixed_x, fixed_y):
    name = cfg['name']
    use_lcx = cfg['use_lcx']
    evo = cfg['evo']

    print(f'\n{"="*60}')
    print(f'  CONFIG: {name}  [FIXED SINGLE BATCH]')
    ...
    for step in range(STEPS):
        ...
        # ── 2. Normal training step — SAME BATCH EVERY STEP ──
        x, y = fixed_x, fixed_y
        opt.zero_grad()
        out = model(x)
        ...
        if evo and use_lcx:
            ...
            # Use the SAME fixed batch for eval too (consistent signal)
            with torch.no_grad():
                restore_lcx(model, snap_new)
                out_new = model(fixed_x)
                if isinstance(out_new, tuple):
                    out_new = out_new[0]
                loss_new = F.binary_cross_entropy_with_logits(out_new, fixed_y).item()

                restore_lcx(model, snap_old)
                out_old = model(fixed_x)
                if isinstance(out_old, tuple):
                    out_old = out_old[0]
                loss_old = F.binary_cross_entropy_with_logits(out_old, fixed_y).item()
...
if __name__ == '__main__':
    print('=' * 60)
    print('PROBE: EVO WHITEBOARD — FIXED SINGLE BATCH')
    print('=' * 60)
    ...
    print(f'  MODE: same batch every step (no cross-batch noise)')
    print()

    # Generate ONE fixed batch for the entire run
    torch.manual_seed(99)  # different seed from model init
    fixed_x, fixed_y = make_echo_batch(BATCH, SEQ_LEN, BLOCK_SIZE, NUM_BITS, DEVICE)
    print(f'  Fixed batch generated: x={list(fixed_x.shape)}, y={list(fixed_y.shape)}')

    with open(LIVE_LOG, 'w') as f:
        f.write(f'# probe_evo_whiteboard_fixedbatch -- {time.strftime("%Y-%m-%d %H:%M:%S")}\n')

    results = []
    for cfg in CONFIGS:
        r = run_config(cfg, fixed_x, fixed_y)
        results.append(r)

    # ── Summary ──────────────────────────────────────────────────
    print(f'\n{"="*60}')
    print(f'SUMMARY: EVO WHITEBOARD — FIXED SINGLE BATCH')
    print(f'{"="*60}')
    for r in results:
        err = r.get('error', '')
    ...
    if brain and wb and evo and all('error' not in r for r in [brain, wb, evo]):
        ...
        # Compare against random-batch baseline
        print(f'\n  RANDOM-BATCH BASELINE (previous run):')
        print(f'    brain_only=53.43%  brain+wb=53.80%  evo_wb=53.76%')
        print(f'  FIXED-BATCH (this run):')
        print(f'    brain_only={brain["tail"]*100:.2f}%  brain+wb={wb["tail"]*100:.2f}%  evo_wb={evo["tail"]*100:.2f}%')

        if diff_wb > 0.03:
            print(f'\n  VERDICT: FIXED_BATCH_HELPS — cross-batch noise WAS the problem (+{diff_wb*100:.1f}%)')
        elif diff_wb > 0.01:
            print(f'\n  VERDICT: FIXED_BATCH_MODERATE — some improvement (+{diff_wb*100:.1f}%)')
        elif diff_wb > -0.01:
            print(f'\n  VERDICT: FIXED_BATCH_NO_EFFECT — cross-batch noise is NOT the problem')
        else:
            print(f'\n  VERDICT: FIXED_BATCH_HURTS — whiteboard hurts even with fixed batch')
