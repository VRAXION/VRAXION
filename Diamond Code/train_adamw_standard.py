"""
Test AdamW optimizer on STANDARD RING (mobius=False).

Control test: Does standard ring also oscillate with AdamW?
If standard ring is smooth but Möbius oscillates → topology causes it.
If both oscillate → optimizer causes it.
"""
...
def main():
    # TASK CONFIG - Same as Möbius test
    num_keys = 2
    num_pairs = 2
    seq_len = 32
    ...
    print("=" * 70)
    print("DIAMOND CODE - AdamW on STANDARD RING")
    print("=" * 70)
    ...
    print(f"Topology: STANDARD RING (mobius=False)")
    print()
    ...
    model = RingMemoryModel(
        ...
        mobius=False,  # STANDARD RING (control)
    )
    ...
    # AdamW with standard hyperparams
    ...
    # Log file
    log_path = Path(__file__).parent / "logs" / "diamond" / "adamw_standard.log"
    ...
    log("=" * 70)
    log(f"AdamW Test (Standard): {num_keys} keys, {num_pairs} pairs, len={seq_len}")
    ...
    try:
        # Run for 1500 steps for fair comparison
        while step < 1500:
