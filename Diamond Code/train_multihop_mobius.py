"""
Multi-hop Reasoning Test - Möbius Topology (1-hop baseline)
...
def main():
    ...
    # MODEL CONFIG (same capacity as hard task tests)
    num_positions = 64
    embedding_dim = 64

    print("=" * 70)
    print("1-HOP BASELINE TEST - MÖBIUS TOPOLOGY")
    print("=" * 70)
    ...
    print(f"Topology: MÖBIUS (holonomy 2× memory)")
    print()
    ...
    # Fixed eval set (500 samples, seed for reproducibility)
    ...
    # Log file
    log_path = Path(__file__).parent / "logs" / "diamond" / "multihop_mobius.log"
    ...
    log("=" * 70)
    log(f"1-hop baseline (Möbius): {num_chains} chains, {vocab_size} entities, len={seq_len}")
    log(f"Model: {num_positions}×{embedding_dim}D = {total_params} params")
    log(f"Optimizer: AdamW(lr=0.001, weight_decay=0.01)")
    log(f"Topology: MÖBIUS | Purpose: isolate classification bottleneck")
    log("=" * 70)
    ...
    try:
        # Run for 3000 steps (enough for convergence on medium tasks)
        while step < 3000:
            step_start = time.time()

            # Fresh training data (streaming, different seed each step)
    ...
    print()
    print("=" * 70)
    print("FINAL RESULTS - MÖBIUS (1-HOP BASELINE)")
    print("=" * 70)
