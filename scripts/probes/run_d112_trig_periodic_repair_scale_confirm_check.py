#!/usr/bin/env python3
"""Validate D112 trig-periodic repair scale confirmation artifacts."""
...
def check(out: Path) -> None:
    ...
    manifest = read_json(out / "d111t_upstream_manifest.json")
    scale = aggregate["scale"]
    m = aggregate["metrics"]
    gates = aggregate["positive_gates"]
    require(decision["decision"] == "d112_trig_periodic_repair_scale_confirmed", "unexpected decision")
    require(decision["next"] == "D113_SYMBOLIC_SEQUENCE_BRIDGE_PLAN_WITH_TRIG_GUARDRAILS", "unexpected next")
    require(decision["d113_ready"] is True and m["d113_ready"] is True, "D113 readiness missing")
    require(all(gates.values()), f"failed positive gates: {[k for k, v in gates.items() if not v]}")
    require(manifest["validation_status"] == "valid" and manifest["replayed_decision"] == "d111t_trig_periodic_targeted_repair_prototype_confirmed", "D111T replay invalid")
    require(manifest["replayed_next"] == "D112_TRIG_PERIODIC_REPAIR_SCALE_CONFIRM" and manifest["replayed_d112_ready"] is True, "D111T next/readiness invalid")
    require(manifest["replayed_trig_failure_reduction"] == 0.378 and manifest["replayed_trig_remains_repair_only"] is True and manifest["replayed_failed_jobs"] == [], "D111T replay metrics invalid")
    require(m["d111t_replay_validation_passed"] is True and m["d111t_replay_decision"] == "d111t_trig_periodic_targeted_repair_prototype_confirmed", "D111T metric replay invalid")
    require(scale["requested_total_rows"] == scale["actual_total_rows"] and scale["scale_reduced"] is False, "scale reduced")
    require(scale["requested_total_rows"] == 310320 and scale["stress_mode_count"] == 30 and scale["all_required_stress_modes_executed"] is True, "scale/stress mismatch")
    ...
    require(m["trig_repair_scale_training_executed"] is True and m["training_updates_executed"] is True and m["total_repair_steps_executed"] > 0 and 1 <= m["epochs_executed"] <= 4, "training execution mismatch")
    ...
    require(m["trig_loop_utility_delta"] > 0 and m["trig_mask_stability_delta"] > 0 and m["calibration_margin_delta"] > 0 and m["calibration_overcorrection_detected"] is False, "loop/mask/calibration repair gate failed")
    ...
    require(m["lane_a_interference"] <= 0.01 and m["lane_b_interference"] <= 0.01 and m["lane_d_interference"] <= 0.012 and m["heldout_rejected_interference"] <= 0.012, "preservation interference failed")
    ...
    require(m["lane_b_status_preserved"] is True and m["lane_d_expansion_preserved"] is True and m["heldout_rejected_noninterference_passed"] is True, "Lane B/D/heldout preservation failed")
    ...
    for key in ["failure_label_shortcut_detected", "phase_bin_shortcut_detected", "frequency_bin_shortcut_detected", "harmonic_overlap_shortcut_detected", "before_after_label_shortcut_detected", "scale_run_id_shortcut_detected"]:
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("target/pilot_wave/d112_trig_periodic_repair_scale_confirm"))
