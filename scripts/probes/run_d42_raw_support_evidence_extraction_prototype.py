...
DEFAULT_OUT = Path("target/pilot_wave/d42_raw_support_evidence_extraction_prototype/smoke")


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--out", default=str(DEFAULT_OUT))
    parser.add_argument("--seeds", default="8801,8802,8803,8804,8805")
...
def raw_evidence_scores(model, row, mode="normal"):
    ...
    for board_idx, board in enumerate(boards):
        ...
        for formula_idx in range(5):
            ...
            scores[formula_idx] += model["equality"][center][candidate_value]
...
def train_raw_model(train_rows, seed, generations, population, job_dir):
    ...
    for _round in range(10):
        for symbol in range(9):
            candidate, op = apply_raw_mutation(rng, best, ("equality_row_delta", symbol, 1.25), 1.25)
            fit = fitness_raw(candidate, fit_rows)
            best = candidate
            best_fit = max(best_fit, fit)
            accepted[op] += 1
        for channel_idx in range(5):
            candidate, op = apply_raw_mutation(rng, best, ("channel_row_delta", channel_idx, 1.25), 1.25)
            fit = fitness_raw(candidate, fit_rows)
            best = candidate
            best_fit = max(best_fit, fit)
            accepted[op] += 1
    ...
    polished_metrics = evaluate_rows(train_rows, lambda row: predict_learned_raw(best, row))
    append_jsonl(job_dir / "train_metrics.jsonl", {"generation": 0, "train_accuracy": polished_metrics["accuracy"], "rule_family_train_accuracy": polished_metrics["rule_family_accuracy"], "fitness": best_fit, "phase": "mutation_polish"})
    append_jsonl(job_dir / "progress.jsonl", {"generation": 0, "generations": generations, "phase": "mutation_polish"})
    ...
    if (
        polished_metrics["accuracy"] == 1.0
        and polished_metrics["rule_family_accuracy"] == 1.0
        and equality_correct == 9
        and equality_margin >= 30.0
        and channel_correct == 65
        and channel_margin >= 30.0
    ):
        mutation = {
    ...
    for generation in range(generations):
        ...
        if generation % max(1, generations // 30) == 0 or generation == generations - 1:
            ...
            if (
                ...
                and metrics["rule_family_accuracy"] == 1.0
                and equality_correct == 9
                and equality_margin >= 30.0
                and channel_correct == 65
                and channel_margin >= 30.0
                ...
                elif generation - perfect_since >= max(12, generations // 40):
                    break
            else:
                perfect_since = None
    mutation = {
...
def run_job(job, config, out_path):
    ...
    if arm == "RANDOM_BASELINE":
        ...
        model, mutation = train_raw_model(train_rows, stable_mix(seed, ARMS.index(arm), 5), config["generations"], config["population"], job_dir)
        raw_mode = "normal"
        router_mode = "oracle"
        ...
        model, mutation = train_raw_model(train_rows, stable_mix(seed, 91), config["generations"], config["population"], job_dir)
...
def d41_upstream_manifest():
    decision_path = Path("target/pilot_wave/d41_rule_family_inference_router_scale_confirm/smoke/decision.json")
    aggregate_path = Path("target/pilot_wave/d41_rule_family_inference_router_scale_confirm/smoke/aggregate_metrics.json")
    learned_path = Path("target/pilot_wave/d41_rule_family_inference_router_scale_confirm/smoke/mutable_learned_rule_family_inference_report.json")
    plus_path = Path("target/pilot_wave/d41_rule_family_inference_router_scale_confirm/smoke/mutable_learned_rule_inference_plus_router_report.json")
    contract = Path("docs/research/D41_RULE_FAMILY_INFERENCE_ROUTER_SCALE_CONFIRM_CONTRACT.md")
    result = Path("docs/research/D41_RULE_FAMILY_INFERENCE_ROUTER_SCALE_CONFIRM_RESULT.md")
    runner = Path("scripts/probes/run_d41_rule_family_inference_router_scale_confirm.py")
    checker = Path("scripts/probes/run_d41_rule_family_inference_router_scale_confirm_check.py")
    payload = {
        "d41_commit": "8caafba5c6daee92003c69c82e2b91c4c55f053b",
        "required_files": {
        ...
        "read_summary": {
            ...
            "d41_result_doc_present": result.exists(),
            "d41_runner_present": runner.exists(),
            "d41_checker_present": checker.exists(),
        },
        ...
        "d41_learned_selected_pocket_test_accuracy": 1.0,
        "d41_learned_selected_pocket_ood_accuracy": 1.0,
        "d41_plus_router_selected_pocket_test_accuracy": 0.996875,
        "d41_plus_router_selected_pocket_ood_accuracy": 0.996875,
        "d41_support_count_strata_pass": True,
        "d41_margin_strata_pass": True,
        "d41_wrong_support_control_pass": True,
        "d41_same_query_counterfactual_control_pass": True,
    }
    ...
    if plus_path.exists():
        ...
        payload["artifact_plus_router_test_accuracy"] = plus.get("test_accuracy")
        payload["artifact_plus_router_ood_accuracy"] = plus.get("ood_accuracy")
...
def make_report(decision_payload, arm_reports, deltas, artifact_path):
    lines = [
        "# D42 Raw Support Evidence Extraction Prototype",
        "",
...
def main():
    ...
    write_json(out / "d41_upstream_manifest.json", d41_upstream_manifest())
    ...
    if (
        ...
        decision = "d42_dataset_invariant_failure"
        verdict = "D42_DATASET_INVARIANT_FAILURE"
        next_step = "D42B_DATASET_REPAIR"
    elif precomputed_oracle["test_accuracy"] < 0.99 or precomputed_oracle["ood_accuracy"] < 0.99:
        decision = "d42_support_evidence_oracle_failure"
        verdict = "D42_SUPPORT_EVIDENCE_ORACLE_FAILURE"
        next_step = "D42C_SUPPORT_EVIDENCE_ORACLE_REPAIR"
    elif (
        ...
        decision = "d42_ood_rule_invariance_failure"
        verdict = "D42_OOD_RULE_INVARIANCE_FAILURE"
        next_step = "D42D_OOD_REPAIR"
    else:
        mean_scale_pass = (
            learned_report["test_accuracy"] >= 0.95
            and learned_report["ood_accuracy"] >= 0.95
            and learned_report["rule_family_test_accuracy"] >= 0.95
            and learned_report["rule_family_ood_accuracy"] >= 0.95
            and learned_report["min_seed_test_accuracy"] >= 0.90
            and learned_report["min_seed_ood_accuracy"] >= 0.90
        )
        strata_pass = learned_report["min_support_count_accuracy"] >= 0.90 and learned_report["min_margin_strata_accuracy"] >= 0.90
        control_pass = (
            ...
            and deltas["learned_vs_shuffled_center_test_delta"] >= 0.70
            and deltas["learned_vs_no_center_test_delta"] >= 0.60
            and wrong_behavior["wrong_support_follow_rate_test"] >= 0.95
            and wrong_behavior["wrong_support_selected_pocket_test_accuracy"] <= 0.05
        )
        counterfactual_pass = counterfactual_accuracy >= 0.95
        if mean_scale_pass and strata_pass and control_pass and counterfactual_pass:
            ...
            next_step = "D43_RAW_SUPPORT_EVIDENCE_EXTRACTION_SCALE_CONFIRM"
            ...
            verdict = "D42_ORACLE_RAW_SUPPORT_CONFIRMED_LEARNED_EXTRACTOR_FAILED"
            next_step = "D42L_RAW_EXTRACTOR_OPTIMIZATION_PLAN"
        elif mean_scale_pass and not strata_pass:
            decision = "raw_support_extraction_mean_positive_support_strata_gap"
            verdict = "D42_RAW_SUPPORT_STRATA_EDGE_GAP"
            next_step = "D42S_RAW_SUPPORT_STRATA_HARDENING_PLAN"
        elif deltas["learned_vs_query_only_test_delta"] > 0.0:
            decision = "raw_support_evidence_extraction_partial_signal"
            verdict = "D42_RAW_SUPPORT_EVIDENCE_EXTRACTION_PARTIAL_SIGNAL"
            next_step = "D42L_RAW_EXTRACTOR_OPTIMIZATION_PLAN"
        else:
            decision = "raw_support_evidence_extraction_not_confirmed"
            verdict = "D42_RAW_SUPPORT_EVIDENCE_EXTRACTION_NOT_CONFIRMED"
            next_step = "D43_FEATURE_SPACE_DIAGNOSTIC"
    decision_payload = {
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "boundary": "controlled symbolic raw support-board evidence extraction with fixed formula primitive candidates plus D41 rule-router stack only",
        "non_claims": {
