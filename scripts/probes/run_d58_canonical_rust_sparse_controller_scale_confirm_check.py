#!/usr/bin/env python3
"""Checker for D58 canonical Rust sparse controller scale confirm."""
...
ALLOWED_DECISIONS = {
    "canonical_rust_sparse_controller_scale_confirmed",
    "canonical_rust_sparse_controller_scale_lite_confirmed",
    "rust_sparse_path_not_cleanly_exercised",
    "canonical_rust_sparse_controller_scale_not_confirmed",
}
...
def inspect_jsonl(path):
    ...
    with Path(path).open(encoding="utf-8") as handle:
        for idx, line in enumerate(handle):
            ...
            if row["arm"] == "CANONICAL_RUST_SPARSE_CONTROLLER":
    ...
    require(canonical_fallback_rows == 0, f"{path} canonical arm used Python fallback")


def source_guardrails(repo_root):
    source = (repo_root / "scripts/probes/run_d58_canonical_rust_sparse_controller_scale_confirm.py").read_text(encoding="utf-8")
    d57_source = (repo_root / "scripts/probes/run_d57_canonical_rust_sparse_path_bridge.py").read_text(encoding="utf-8")
    ...
    for needle, message in [
        ...
        require(needle not in compact, f"{message} in D58 runner")
    require("Network::propagate_sparse" in source, "D58 runner does not declare propagate_sparse bridge")
    require("Network::propagate_sparse" in d57_source, "D57 bridge helper does not generate Rust propagate_sparse harness")
...
def main():
    ...
    upstream = load(out / "d57_upstream_manifest.json")
    ...
    require(upstream["expected_decision"] == "canonical_rust_sparse_path_bridge_positive", "bad D57 expected decision")
    require(upstream["expected_next"] == "D58_CANONICAL_RUST_SPARSE_CONTROLLER_SCALE_CONFIRM", "bad D57 expected next")
    require(upstream.get("d57_canonical_controller_loaded") is True, "D57 canonical controller not loaded")
    ...
    require("D58 only scale-confirms a canonical Rust sparse ECF action controller" in decision["boundary"], "boundary missing in decision")
    require("D58 only scale-confirms a canonical Rust sparse ECF action controller" in report, "boundary missing in report")
    require(summary["rust_network_path_invoked"] is True, "summary Rust flag missing")
    require(summary["python_fallback_rows"] == 0, "summary fallback rows nonzero")
    ...
    if decision["decision"] in {"canonical_rust_sparse_controller_scale_confirmed", "canonical_rust_sparse_controller_scale_lite_confirmed"}:
        ...
        if decision["decision"] == "canonical_rust_sparse_controller_scale_confirmed":
            require(summary["scale_mode"] == "full", "full scale decision without full scale mode")
    print(
        json.dumps(
            {
                "status": "ok",
                ...
                "rust_network_path_invoked": rust_usage["canonical_rust_network_path_invoked"],
                "python_vs_rust_action_parity": parity["parity_rate"],
                "canonical_exact": row["exact_joint_accuracy"],
                "scale_mode": summary["scale_mode"],
            },
            indent=2,
            sort_keys=True,
        )
