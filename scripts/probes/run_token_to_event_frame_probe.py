...
DEFAULT_OUT = ROOT / "target" / "pilot_wave" / "token_to_event_frame_001"
CONTRACT = ROOT / "docs" / "research" / "TOKEN_TO_EVENT_FRAME_001_CONTRACT.md"
DOC_REPORT = ROOT / "docs" / "research" / "TOKEN_TO_EVENT_FRAME_001_RESULT.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TOKEN_TO_EVENT_FRAME_001 raw clause to event-frame probe.")
...
def parse_clause_to_frame(clause: str) -> EventFrame:
    ...
    m = re.fullmatch(r"the ([a-z0-9_]+) (?:sits|shines|sleeps)", lower)
    if m:
    ...
    m = re.fullmatch(r"the (?:(first|second|previous|other) )?([a-z0-9_]+) (?:gets|is) stolen", lower)
    if m:
...
def verdict(agg: dict[str, dict[str, object]]) -> list[str]:
    ...
    if seg_frame >= 0.90 and seg_ledger >= 0.90 and seg_ledger >= shuffled + 0.15:
        labels.append("EVENT_FRAME_POSITIVE")
    ...
    if (bag_story >= 0.85 or static_story >= 0.85) and seg_ledger >= 0.85:
...
def write_outputs(out_dir: Path, results: list[JobResult], args: argparse.Namespace, status: str, jobs: int) -> tuple[dict[str, dict[str, object]], list[str]]:
    ...
    lines = [
        "# TOKEN_TO_EVENT_FRAME_001 Report",
        "",
        f"- Status: `{status}`",
        ...
        "## Arm Summary",
        "",
        "| Arm | Frame Exact | Ledger | Event | Entity | Ref | Qty | Illegal | Same-token | Order | Coref |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for arm in sorted(agg):
        lines.append(
            "| `{}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` |".format(
                ...
                metric_mean(agg, arm, "illegal_frame_rate"),
                metric_mean(agg, arm, "same_token_set_accuracy"),
                metric_mean(agg, arm, "event_order_contrast_accuracy"),
                metric_mean(agg, arm, "coreference_accuracy"),
            )
        )
