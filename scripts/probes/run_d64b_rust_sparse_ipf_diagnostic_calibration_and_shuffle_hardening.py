#!/usr/bin/env python3
"""D64B Rust sparse IPF diagnostic calibration and shuffle hardening.

D64 confirmed controller-level usefulness for a Rust sparse IPF diagnostic
layer, but some internal diagnostic bits were weak and a shuffled score-vector
control stayed close. D64B keeps the formula solver symbolic and tests
calibrated Rust sparse diagnostics plus stronger shuffle controls.
"""
...
TASK = "D64B_RUST_SPARSE_IPF_DIAGNOSTIC_CALIBRATION_AND_SHUFFLE_HARDENING"
BOUNDARY = (
    "D64B only hardens and calibrates a Rust sparse IPF diagnostic layer for "
    "controlled symbolic joint formula discovery. It does not prove full "
...
def make_d64_upstream_manifest(repo_root):
    root = repo_root / "target/pilot_wave/d64_rust_sparse_ipf_diagnostic_layer_prototype/smoke"
    manifest = {
        "upstream": "D64_RUST_SPARSE_IPF_DIAGNOSTIC_LAYER_PROTOTYPE",
        "expected_decision": "rust_sparse_ipf_diagnostic_layer_confirmed",
        "expected_next": "D65_RUST_SPARSE_IPF_SCORE_AGGREGATION_PROTOTYPE",
        "artifact_root": str(root),
        "decision_present": (root / "decision.json").exists(),
        "summary_present": (root / "summary.json").exists(),
    }
...
def random_estimates(pack):
    rng = stable_rng(pack["seed"], f"D64B_RANDOM_DIAGNOSTIC:{pack['row_id']}:{pack.get('track')}")
...
def select_policy_for_arm(pack, arm, estimates_by_variant, learned_gate):
    ...
    if arm == "D64_FULL_IPF_LAYER_REPLAY":
        features = estimates_to_gate_features(pack, base, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "d64_full_ipf_replay", base, False
    if arm == "CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER":
        features = estimates_to_gate_features(pack, calibrated, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "calibrated_full_ipf_diagnostic_layer", calibrated, False
    if arm == "ENTROPY_CALIBRATED_LAYER":
        estimates = with_one_calibrated(base, estimates_by_variant["ENTROPY"], "entropy_high")
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "entropy_only_calibrated", estimates, False
    if arm == "EXTERNAL_NEED_CALIBRATED_LAYER":
        estimates = with_one_calibrated(base, estimates_by_variant["EXTERNAL"], "external_test_need")
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "external_need_only_calibrated", estimates, False
    if arm == "UNRESOLVABLE_CALIBRATED_LAYER":
        estimates = with_one_calibrated(base, estimates_by_variant["UNRESOLVABLE"], "internal_unresolvable")
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "unresolvable_only_calibrated", estimates, False
    if arm == "SUPPORT_EFFORT_CALIBRATED_LAYER":
        estimates = with_one_calibrated(base, estimates_by_variant["SUPPORT"], "support_effort_pressure")
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "support_effort_only_calibrated", estimates, False
    if arm == "STRONG_DIAGNOSTICS_ONLY":
        features = estimates_to_gate_features(pack, calibrated, STRONG_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "strong_diagnostics_only_weak_bits_excluded", calibrated, False
    if arm == "WEAK_DIAGNOSTICS_ONLY":
        features = estimates_to_gate_features(pack, calibrated, WEAK_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "weak_diagnostics_only", calibrated, False
    if arm == "CANDIDATE_SHUFFLE_CONTROL":
        estimates = candidate_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "candidate_wise_shuffle_control", estimates, False
    if arm == "SUPPORT_SHUFFLE_CONTROL":
        estimates = support_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "support_wise_shuffle_control", estimates, False
    if arm == "TOPK_PRESERVING_SHUFFLE_CONTROL":
        estimates = topk_preserving_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "topk_preserving_shuffle_control", estimates, False
    if arm == "ENTROPY_PRESERVING_SHUFFLE_CONTROL":
        estimates = entropy_preserving_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "entropy_preserving_shuffle_control", estimates, False
    if arm == "ADVERSARIAL_SHUFFLE_CONTROL":
        estimates = adversarial_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "adversarial_shuffle_control", estimates, False
...
def write_partial_eval(out, split, track, outputs, completed, started):
    ...
    write_json(
        out / f"partial_{split}_{track}_metrics_snapshot.json",
        {
            ...
            "d64_replay_recent": partial["by_arm_core"].get("D64_FULL_IPF_LAYER_REPLAY", {}),
            "calibrated_recent": partial["by_arm_core"].get("CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER", {}),
            "strong_only_recent": partial["by_arm_core"].get("STRONG_DIAGNOSTICS_ONLY", {}),
            "diagnostic_recent": partial["diagnostic_accuracy"].get("CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER", {}),
        },
    )
...
def track_summary(metrics_by_track, arm, baseline_summary):
    ...
    out["saturated_regression_vs_D64"] = out[SATURATED_TRACK]["exact"] - baseline_summary[SATURATED_TRACK]["exact"]
    out["mixed_regression_vs_D64"] = out[MIXED_TRACK]["exact"] - baseline_summary[MIXED_TRACK]["exact"]
    return out
...
def default_d64_summary():
    ...
    for track in [SATURATED_TRACK, OOD_TRACK, EXTERNAL_TRACK, INDISTINGUISHABLE_TRACK]:
        base[track]["external"] = 0.995
    for track in [HARD_TRACK, HIDDEN_BUDGET_TRACK]:
        base[track]["external"] = 0.01225
    base["min_seed_exact"] = 0.9925
    return base


def load_d64_best_summary(repo_root):
    summary = load_json(repo_root / "target/pilot_wave/d64_rust_sparse_ipf_diagnostic_layer_prototype/smoke/summary.json") or {}
    return summary.get("best_summary") or default_d64_summary()


def positive_gate(summary, d64_summary):
    return (
        summary[SATURATED_TRACK]["exact"] >= d64_summary[SATURATED_TRACK]["exact"] - 0.003
        and summary[HARD_TRACK]["exact"] >= d64_summary[HARD_TRACK]["exact"] - 0.003
        and summary[MIXED_TRACK]["exact"] >= d64_summary[MIXED_TRACK]["exact"] - 0.003
        and summary[OOD_TRACK]["exact"] >= d64_summary[OOD_TRACK]["exact"] - 0.003
        and summary[GATE_CONFUSION_TRACK]["exact"] >= d64_summary[GATE_CONFUSION_TRACK]["exact"] - 0.003
        and summary[MIXED_TRACK]["corr"] >= d64_summary[MIXED_TRACK]["corr"] - 0.005
        and summary[MIXED_TRACK]["adv"] >= d64_summary[MIXED_TRACK]["adv"] - 0.005
...
def make_decision(metrics_by_track, d64_summary, failed_jobs):
    summaries = {arm: track_summary(metrics_by_track, arm, d64_summary) for arm in ARMS}
    if failed_jobs:
        best = max(FAIR_ARMS, key=lambda arm: summaries[arm][MIXED_TRACK]["exact"])
        return {
            "decision": "d64b_diagnostic_calibration_not_confirmed",
            "verdict": "D64B_DIAGNOSTIC_CALIBRATION_NOT_CONFIRMED",
            "next": "D64B_REPAIR",
            "best_arm": best,
            "boundary": BOUNDARY,
        }, summaries
    candidate_arms = ["CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER", "STRONG_DIAGNOSTICS_ONLY"]
    best = max(candidate_arms, key=lambda arm: summaries[arm][MIXED_TRACK]["exact"])
    best_pass = positive_gate(summaries[best], d64_summary)
    ...
    max_destructive_control = max(summaries[arm][MIXED_TRACK]["exact"] for arm in destructive_controls)
    destructive_gap_ok = (best_mixed - max_destructive_control) >= 0.03
    weak_improved = weak_accuracy_mean(metrics_by_track, "CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER") >= (
        weak_accuracy_mean(metrics_by_track, "D64_FULL_IPF_LAYER_REPLAY") + 0.05
    )
    strong_pass = positive_gate(summaries["STRONG_DIAGNOSTICS_ONLY"], d64_summary)
    if best_pass and destructive_gap_ok and (weak_improved or strong_pass):
        if best == "STRONG_DIAGNOSTICS_ONLY" and not weak_improved:
            return {
                "decision": "diagnostic_layer_positive_with_weak_bits_excluded",
                "verdict": "D64B_DIAGNOSTIC_LAYER_POSITIVE_WITH_WEAK_BITS_EXCLUDED",
        ...
        return {
            ...
            "verdict": "D64B_RUST_SPARSE_IPF_DIAGNOSTIC_CALIBRATION_CONFIRMED",
            "next": "D65_RUST_SPARSE_IPF_SCORE_AGGREGATION_PROTOTYPE",
            "best_arm": best,
            "boundary": BOUNDARY,
        }, summaries
    if best_pass and not destructive_gap_ok:
        return {
...
def make_reports(out, aggregate, decision, arm_summaries):
    ...
    weak_report = {
        name: {
            "d64_replay_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["D64_FULL_IPF_LAYER_REPLAY"][name]["accuracy"],
            "calibrated_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER"][name]["accuracy"],
            "excluded_in_strong_only": name in WEAK_DIAGNOSTICS,
        }
    ...
    strong_report = {
        name: {
            "d64_replay_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["D64_FULL_IPF_LAYER_REPLAY"][name]["accuracy"],
            "calibrated_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER"][name]["accuracy"],
            "preserved_in_strong_only": name in STRONG_DIAGNOSTICS,
        }
    ...
    reports = {
        "d64_upstream_manifest.json": aggregate["d64_upstream_manifest"],
        ...
        "shuffle_control_audit_report.json": {
            arm: {
                ...
                "control_kind": "topk_or_entropy_preserving_may_remain_close" if arm in {"TOPK_PRESERVING_SHUFFLE_CONTROL", "ENTROPY_PRESERVING_SHUFFLE_CONTROL"} else "destructive",
            }
            for arm in shuffle_arms
        },
        "diagnostic_calibration_report.json": {
            "mixed": {
                "d64_replay": metrics[MIXED_TRACK]["diagnostic_accuracy"]["D64_FULL_IPF_LAYER_REPLAY"],
                "calibrated": metrics[MIXED_TRACK]["diagnostic_accuracy"]["CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER"],
                "entropy_calibrated": metrics[MIXED_TRACK]["diagnostic_accuracy"]["ENTROPY_CALIBRATED_LAYER"],
                "external_need_calibrated": metrics[MIXED_TRACK]["diagnostic_accuracy"]["EXTERNAL_NEED_CALIBRATED_LAYER"],
                "unresolvable_calibrated": metrics[MIXED_TRACK]["diagnostic_accuracy"]["UNRESOLVABLE_CALIBRATED_LAYER"],
                "support_effort_calibrated": metrics[MIXED_TRACK]["diagnostic_accuracy"]["SUPPORT_EFFORT_CALIBRATED_LAYER"],
            }
        },
        ...
        "track_uniformity_audit_report.json": {
            ...
            "note": "Uniform values can mean the controller selected the same action pattern across tracks; D64B reports this explicitly instead of hiding it.",
        },
        "diagnostic_estimator_accuracy_report.json": {
            track: metrics[track]["diagnostic_accuracy"].get("CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER", {})
            for track in TRACKS
        },
        ...
        "noisy_perturbation_report.json": {
            "track": NOISY_TRACK,
            "calibrated_diagnostic_accuracy": metrics[NOISY_TRACK]["diagnostic_accuracy"].get("CALIBRATED_FULL_IPF_DIAGNOSTIC_LAYER", {}),
...
def main():
    ...
    d64_best = load_d64_best_summary(repo_root)
    ...
    decision, arm_summaries = make_decision(test_metrics_by_track, d64_best, failed_jobs)
    fallback_rows = 0
    rust_rows = 0
