#!/usr/bin/env python3
"""D64S score-vector structure repair.

D64B showed that calibrated Rust sparse diagnostics keep high controller
performance, but candidate/top-k preserving shuffles stayed too close. D64S
tests which score-vector structures actually matter before D65 can migrate
score aggregation.
"""
...
TASK = "D64S_SCORE_VECTOR_STRUCTURE_REPAIR"
BOUNDARY = (
    "D64S only tests score-vector structure dependency for a Rust sparse IPF "
    "diagnostic layer in controlled symbolic joint formula discovery. It does "
    "not prove full "
    "VRAXION brain, raw visual "
...
CALIBRATED_DIAGNOSTIC_MAP = copy.deepcopy(DIAGNOSTIC_MAP)
CALIBRATED_DIAGNOSTIC_MAP["entropy_high"]["threshold"] = 0.88
CALIBRATED_DIAGNOSTIC_MAP["external_test_need"]["threshold"] = 0.84
CALIBRATED_DIAGNOSTIC_MAP["internal_unresolvable"]["threshold"] = 0.62
CALIBRATED_DIAGNOSTIC_MAP["support_effort_pressure"]["threshold"] = 0.70
...
VARIANT_MAPS = {
    "D64": DIAGNOSTIC_MAP,
    "CAL": CALIBRATED_DIAGNOSTIC_MAP,
    "ENTROPY": {**DIAGNOSTIC_MAP, "entropy_high": CALIBRATED_DIAGNOSTIC_MAP["entropy_high"]},
    "EXTERNAL": {**DIAGNOSTIC_MAP, "external_test_need": CALIBRATED_DIAGNOSTIC_MAP["external_test_need"]},
    "UNRESOLVABLE": {**DIAGNOSTIC_MAP, "internal_unresolvable": CALIBRATED_DIAGNOSTIC_MAP["internal_unresolvable"]},
    "SUPPORT": {**DIAGNOSTIC_MAP, "support_effort_pressure": CALIBRATED_DIAGNOSTIC_MAP["support_effort_pressure"]},
}

POLICY_MODULES = d62.POLICY_MODULE_ARMS

CANDIDATE_IDENTITY_DIAGNOSTICS = ["margin_low"]
SCORE_SHAPE_DIAGNOSTICS = ["entropy_high", "margin_low", "support_effort_pressure"]
SUPPORT_COHERENCE_DIAGNOSTICS = ["collision_pressure", "support_independence_low", "adversarial_pressure"]
COUNTERFACTUAL_DELTA_DIAGNOSTICS = ["counterfactual_pressure", "margin_low"]
CLUSTER_STRUCTURE_DIAGNOSTICS = ["collision_pressure", "support_independence_low", "adversarial_pressure"]
FULL_STRUCTURE_DIAGNOSTICS = DIAGNOSTICS
...
RUST_DIAGNOSTIC_ARMS = list(FAIR_ARMS)
...
def make_d64b_upstream_manifest(repo_root):
    root = repo_root / "target/pilot_wave/d64b_rust_sparse_ipf_diagnostic_calibration_and_shuffle_hardening/smoke"
    manifest = {
        "upstream": "D64B_RUST_SPARSE_IPF_DIAGNOSTIC_CALIBRATION_AND_SHUFFLE_HARDENING",
        "expected_decision": "score_vector_shuffle_gap_insufficient",
        "expected_next": TASK,
        "artifact_root": str(root),
        "decision_present": (root / "decision.json").exists(),
        "summary_present": (root / "summary.json").exists(),
        "scale_lite_rows_per_seed": 200,
    }
    for name in [
        "decision.json",
        "summary.json",
...
def make_estimator_controller(name, diagnostic_name, diagnostic_map):
...
def estimator_controllers():
    controllers = {}
    for variant, diagnostic_map in VARIANT_MAPS.items():
        for name in DIAGNOSTICS:
            controllers[f"EST_{variant}_{name}"] = make_estimator_controller(f"EST_{variant}_{name}", name, diagnostic_map)
    return controllers
...
def estimator_values_from_actions(action_records, idx, variant):
    values = {}
    for diagnostic in DIAGNOSTICS:
        record = action_records[f"EST_{variant}_{diagnostic}"][idx]
        values[diagnostic] = 1.0 if record["action"] != "DECIDE" else 0.0
    return values


def candidate_shuffled_estimates(values):
    return {
...
def support_shuffled_estimates(values):
    out = dict(values)
    out["collision_pressure"] = values["support_independence_low"]
    out["support_independence_low"] = values["adversarial_pressure"]
    out["support_effort_pressure"] = values["collision_pressure"]
    out["counterfactual_pressure"] = values["support_effort_pressure"]
    out["adversarial_pressure"] = values["counterfactual_pressure"]
    return out


def topk_preserving_shuffled_estimates(values):
    out = candidate_shuffled_estimates(values)
    out["margin_low"] = values["margin_low"]
    out["counterfactual_pressure"] = values["counterfactual_pressure"]
    return out


def entropy_preserving_shuffled_estimates(values):
    out = candidate_shuffled_estimates(values)
    out["entropy_high"] = values["entropy_high"]
    return out


def adversarial_shuffled_estimates(values):
    return {
        "entropy_high": values["entropy_high"],
        "margin_low": 0.0,
        "collision_pressure": 0.0,
        "support_independence_low": 0.0,
        "support_effort_pressure": 0.0,
        "counterfactual_pressure": 0.0,
        "adversarial_pressure": 0.0,
        "internal_unresolvable": 0.0,
        "external_test_need": 1.0 - values["external_test_need"],
    }


def margin_preserving_shuffled_estimates(values):
    out = candidate_shuffled_estimates(values)
    out["margin_low"] = values["margin_low"]
    return out


def support_coherence_break_estimates(values):
    out = dict(values)
    for name in SUPPORT_COHERENCE_DIAGNOSTICS:
        out[name] = 1.0 - float(values[name])
    return out


def counterfactual_delta_shuffled_estimates(values):
    out = dict(values)
    out["counterfactual_pressure"] = 1.0 - float(values["counterfactual_pressure"])
    out["margin_low"] = values["entropy_high"]
    out["entropy_high"] = values["margin_low"]
    return out


def cluster_structure_shuffled_estimates(values):
    out = dict(values)
    out["collision_pressure"] = values["adversarial_pressure"]
    out["support_independence_low"] = values["collision_pressure"]
    out["adversarial_pressure"] = values["support_independence_low"]
    return out


def random_estimates(pack):
    rng = stable_rng(pack["seed"], f"D64S_RANDOM_DIAGNOSTIC:{pack['row_id']}:{pack.get('track')}")
...
def estimates_to_gate_features(pack, estimates, replace_names, neutralize_missing=False):
    features = d62.gate_features(pack)
...
def with_one_calibrated(base, calibrated, name):
    out = dict(base)
    out[name] = calibrated[name]
    return out


def select_policy_for_arm(pack, arm, estimates_by_variant, learned_gate):
    base = estimates_by_variant["D64"]
    calibrated = estimates_by_variant["CAL"]
    if arm == "D64B_CALIBRATED_REPLAY":
        features = estimates_to_gate_features(pack, calibrated, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "d64b_calibrated_replay", calibrated, False
    if arm == "CANDIDATE_IDENTITY_LAYER":
        features = estimates_to_gate_features(pack, calibrated, CANDIDATE_IDENTITY_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "candidate_identity_only", calibrated, False
    if arm == "SCORE_SHAPE_ONLY_LAYER":
        features = estimates_to_gate_features(pack, calibrated, SCORE_SHAPE_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "score_shape_only", calibrated, False
    if arm == "SUPPORT_COHERENCE_LAYER":
        features = estimates_to_gate_features(pack, calibrated, SUPPORT_COHERENCE_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "support_coherence_only", calibrated, False
    if arm == "COUNTERFACTUAL_DELTA_LAYER":
        features = estimates_to_gate_features(pack, calibrated, COUNTERFACTUAL_DELTA_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "counterfactual_delta_only", calibrated, False
    if arm == "CLUSTER_STRUCTURE_LAYER":
        features = estimates_to_gate_features(pack, calibrated, CLUSTER_STRUCTURE_DIAGNOSTICS, neutralize_missing=True)
        return policy_from_features(features, learned_gate), features, "cluster_structure_only", calibrated, False
    if arm == "FULL_STRUCTURE_AWARE_LAYER":
        features = estimates_to_gate_features(pack, calibrated, FULL_STRUCTURE_DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "full_structure_aware", calibrated, False
    if arm == "CANDIDATE_ID_SHUFFLE":
        estimates = candidate_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "candidate_id_shuffle", estimates, False
    if arm == "SUPPORT_ORDER_SHUFFLE":
        estimates = support_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "support_order_shuffle", estimates, False
    if arm == "TOPK_VALUE_SHUFFLE":
        estimates = topk_preserving_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "topk_value_shuffle", estimates, False
    if arm == "MARGIN_PRESERVING_SHUFFLE":
        estimates = margin_preserving_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "margin_preserving_shuffle", estimates, False
    if arm == "ENTROPY_PRESERVING_SHUFFLE":
        estimates = entropy_preserving_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "entropy_preserving_shuffle", estimates, False
    if arm == "SUPPORT_COHERENCE_BREAK":
        estimates = support_coherence_break_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "support_coherence_break", estimates, False
    if arm == "COUNTERFACTUAL_DELTA_SHUFFLE":
        estimates = counterfactual_delta_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "counterfactual_delta_shuffle", estimates, False
    if arm == "CLUSTER_STRUCTURE_SHUFFLE":
        estimates = cluster_structure_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "cluster_structure_shuffle", estimates, False
    if arm == "FULL_SCORE_NOISE_CONTROL":
        estimates = adversarial_shuffled_estimates(calibrated)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "full_score_noise_control", estimates, False
    if arm == "RANDOM_DIAGNOSTIC_CONTROL":
        estimates = random_estimates(pack)
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "stable_random_diagnostic_control", estimates, False
    if arm == "DIAGNOSTIC_ABLATION_CONTROL":
        estimates = ablated_estimates()
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "zeroed_diagnostic_control", estimates, False
    if arm == "TRUTH_LEAK_SENTINEL_REFERENCE_ONLY":
        track = pack.get("track")
        if track == INDISTINGUISHABLE_TRACK:
            return "ABSTAIN_POLICY", d62.gate_features(pack), "reference_only_track_label_sentinel", base, True
        if track == EXTERNAL_TRACK:
            return "EXTERNAL_TEST_POLICY", d62.gate_features(pack), "reference_only_track_label_sentinel", base, True
        if track in {HARD_TRACK, OOD_TRACK, HIDDEN_BUDGET_TRACK}:
            return "HARD_BUDGET_POLICY", d62.gate_features(pack), "reference_only_track_label_sentinel", base, True
        if track == GATE_CONFUSION_TRACK:
            return "ADVERSARIAL_REPAIR_POLICY", d62.gate_features(pack), "reference_only_track_label_sentinel", base, True
        return "SATURATED_POLICY", d62.gate_features(pack), "reference_only_track_label_sentinel", base, True
...
def evaluate_pack_all_arms(pack, idx, policy_actions, estimator_actions, learned_gate):
    ...
    estimates_by_variant = {
        variant: estimator_values_from_actions(estimator_actions, idx, variant)
        for variant in VARIANT_MAPS
    }
    for arm in ARMS:
        policy, features, basis, estimates, used_forbidden = select_policy_for_arm(pack, arm, estimates_by_variant, learned_gate)
...
def write_partial_eval(out, split, track, outputs, completed, started):
    ...
    write_json(
        out / f"partial_{split}_{track}_metrics_snapshot.json",
        {
            ...
            "d64b_replay_recent": partial["by_arm_core"].get("D64B_CALIBRATED_REPLAY", {}),
            "full_structure_recent": partial["by_arm_core"].get("FULL_STRUCTURE_AWARE_LAYER", {}),
            "support_coherence_recent": partial["by_arm_core"].get("SUPPORT_COHERENCE_LAYER", {}),
            "diagnostic_recent": partial["diagnostic_accuracy"].get("FULL_STRUCTURE_AWARE_LAYER", {}),
        },
    )
...
def track_summary(metrics_by_track, arm, baseline_summary):
    out = {}
    ...
    out["saturated_regression_vs_D64B"] = out[SATURATED_TRACK]["exact"] - baseline_summary[SATURATED_TRACK]["exact"]
    out["mixed_regression_vs_D64B"] = out[MIXED_TRACK]["exact"] - baseline_summary[MIXED_TRACK]["exact"]
    return out
...
def default_d64b_summary():
    ...
    for track in [SATURATED_TRACK, OOD_TRACK, EXTERNAL_TRACK, INDISTINGUISHABLE_TRACK]:
        base[track]["external"] = 0.997
    for track in [HARD_TRACK, HIDDEN_BUDGET_TRACK]:
        base[track]["external"] = 0.01225
    base["min_seed_exact"] = 0.991
    return base


def load_d64b_best_summary(repo_root):
    summary = load_json(repo_root / "target/pilot_wave/d64b_rust_sparse_ipf_diagnostic_calibration_and_shuffle_hardening/smoke/summary.json") or {}
    return summary.get("best_summary") or default_d64b_summary()


def positive_gate(summary, d64b_summary):
    return (
        summary[SATURATED_TRACK]["exact"] >= d64b_summary[SATURATED_TRACK]["exact"] - 0.003
        and summary[HARD_TRACK]["exact"] >= d64b_summary[HARD_TRACK]["exact"] - 0.003
        and summary[MIXED_TRACK]["exact"] >= d64b_summary[MIXED_TRACK]["exact"] - 0.003
        and summary[OOD_TRACK]["exact"] >= d64b_summary[OOD_TRACK]["exact"] - 0.003
        and summary[GATE_CONFUSION_TRACK]["exact"] >= d64b_summary[GATE_CONFUSION_TRACK]["exact"] - 0.003
        and summary[MIXED_TRACK]["corr"] >= d64b_summary[MIXED_TRACK]["corr"] - 0.005
        and summary[MIXED_TRACK]["adv"] >= d64b_summary[MIXED_TRACK]["adv"] - 0.005
...
def weak_accuracy_mean(metrics_by_track, arm):
    acc = metrics_by_track[MIXED_TRACK]["diagnostic_accuracy"][arm]
    return sum(float(acc[name]["accuracy"]) for name in WEAK_DIAGNOSTICS) / len(WEAK_DIAGNOSTICS)


def make_decision(metrics_by_track, d64b_summary, failed_jobs):
    summaries = {arm: track_summary(metrics_by_track, arm, d64b_summary) for arm in ARMS}
    if failed_jobs:
    ...
    candidate_arms = ["FULL_STRUCTURE_AWARE_LAYER"]
    best = max(candidate_arms, key=lambda arm: summaries[arm][MIXED_TRACK]["exact"])
    best_pass = positive_gate(summaries[best], d64b_summary)
    structure_controls = [
        "CANDIDATE_ID_SHUFFLE",
        "TOPK_VALUE_SHUFFLE",
        "MARGIN_PRESERVING_SHUFFLE",
    ...
    best_mixed = summaries[best][MIXED_TRACK]["exact"]
    control_gaps = {arm: best_mixed - summaries[arm][MIXED_TRACK]["exact"] for arm in structure_controls}
    max_meaningful_gap = max(control_gaps.values())
    destructive_gap_ok = max_meaningful_gap >= 0.03
    random_ablation_worse = all(best_mixed > summaries[arm][MIXED_TRACK]["exact"] for arm in ["RANDOM_DIAGNOSTIC_CONTROL", "DIAGNOSTIC_ABLATION_CONTROL"])
    candidate_gap = control_gaps["CANDIDATE_ID_SHUFFLE"]
    shape_or_support_gap = max(
        control_gaps[arm]
        for arm in [
            "SUPPORT_ORDER_SHUFFLE",
            "SUPPORT_COHERENCE_BREAK",
    ...
    if best_pass and destructive_gap_ok and random_ablation_worse:
        if candidate_gap < 0.03 and shape_or_support_gap >= 0.03:
            return {
                "decision": "score_shape_dependency_confirmed_candidate_identity_not_required",
                "verdict": "D64S_SCORE_SHAPE_DEPENDENCY_CONFIRMED_CANDIDATE_IDENTITY_NOT_REQUIRED",
                "next": "D65_RUST_SPARSE_IPF_SCORE_AGGREGATION_PROTOTYPE",
                "best_arm": best,
                "boundary": BOUNDARY,
                "structure_gaps": control_gaps,
            }, summaries
        return {
            "decision": "score_vector_structure_repair_confirmed",
            "verdict": "D64S_SCORE_VECTOR_STRUCTURE_REPAIR_CONFIRMED",
            "next": "D65_RUST_SPARSE_IPF_SCORE_AGGREGATION_PROTOTYPE",
            "best_arm": best,
            "boundary": BOUNDARY,
            "structure_gaps": control_gaps,
    ...
    return {
        ...
        "best_arm": best,
        "boundary": BOUNDARY,
        "structure_gaps": control_gaps if "control_gaps" in locals() else {},
    }, summaries
...
def make_reports(out, aggregate, decision, arm_summaries):
    ...
    best_mixed = arm_summaries[best][MIXED_TRACK]["exact"]
    shuffle_arms = [
        "CANDIDATE_ID_SHUFFLE",
        "TOPK_VALUE_SHUFFLE",
        "MARGIN_PRESERVING_SHUFFLE",
    ...
    weak_report = {
        name: {
            "d64b_replay_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["D64B_CALIBRATED_REPLAY"][name]["accuracy"],
            "full_structure_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["FULL_STRUCTURE_AWARE_LAYER"][name]["accuracy"],
            "score_shape_layer_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["SCORE_SHAPE_ONLY_LAYER"][name]["accuracy"],
        }
        for name in WEAK_DIAGNOSTICS
    }
    strong_report = {
        name: {
            "d64b_replay_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["D64B_CALIBRATED_REPLAY"][name]["accuracy"],
            "full_structure_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["FULL_STRUCTURE_AWARE_LAYER"][name]["accuracy"],
            "support_coherence_layer_accuracy": metrics[MIXED_TRACK]["diagnostic_accuracy"]["SUPPORT_COHERENCE_LAYER"][name]["accuracy"],
        }
        for name in STRONG_DIAGNOSTICS
    }
    proxy_violations = {
    ...
    reports = {
        "d64b_upstream_manifest.json": aggregate["d64b_upstream_manifest"],
        ...
        "ipf_diagnostic_definition_report.json": {
            ...
            "d64_diagnostics": DIAGNOSTIC_MAP,
            "calibrated_diagnostics": CALIBRATED_DIAGNOSTIC_MAP,
            "strong_diagnostics": STRONG_DIAGNOSTICS,
            "weak_diagnostics": WEAK_DIAGNOSTICS,
            "structure_groups": {
                "candidate_identity": CANDIDATE_IDENTITY_DIAGNOSTICS,
                "score_shape": SCORE_SHAPE_DIAGNOSTICS,
                "support_coherence": SUPPORT_COHERENCE_DIAGNOSTICS,
                "counterfactual_delta": COUNTERFACTUAL_DELTA_DIAGNOSTICS,
                "cluster_structure": CLUSTER_STRUCTURE_DIAGNOSTICS,
                "full_structure": FULL_STRUCTURE_DIAGNOSTICS,
            },
        ...
        "shuffle_control_audit_report.json": {
            arm: {
                ...
                "gap_vs_best": best_mixed - arm_summaries[arm][MIXED_TRACK]["exact"],
                "control_kind": "candidate_or_shape_preserving" if arm in {"CANDIDATE_ID_SHUFFLE", "TOPK_VALUE_SHUFFLE", "MARGIN_PRESERVING_SHUFFLE", "ENTROPY_PRESERVING_SHUFFLE"} else "structure_break",
            }
        ...
        "shuffle_control_matrix_report.json": {
            arm: {
                track: {
                    "exact": arm_summaries[arm][track]["exact"],
                    "gap_vs_full_structure": arm_summaries["FULL_STRUCTURE_AWARE_LAYER"][track]["exact"] - arm_summaries[arm][track]["exact"],
                }
                for track in TRACKS
            }
            for arm in shuffle_arms
        ...
        "score_structure_dependency_report.json": {
            "mixed": {
                arm: arm_summaries[arm][MIXED_TRACK]
        ...
        "structure_gap_report.json": {
            arm: {
                "mixed_exact": arm_summaries[arm][MIXED_TRACK]["exact"],
                "gap_vs_full_structure": best_mixed - arm_summaries[arm][MIXED_TRACK]["exact"],
            }
            for arm in CONTROL_ARMS
        },
        "candidate_vs_shape_report.json": {
            "candidate_identity_layer": arm_summaries["CANDIDATE_IDENTITY_LAYER"],
            "score_shape_only_layer": arm_summaries["SCORE_SHAPE_ONLY_LAYER"],
            "candidate_id_shuffle_gap": best_mixed - arm_summaries["CANDIDATE_ID_SHUFFLE"][MIXED_TRACK]["exact"],
            "topk_value_shuffle_gap": best_mixed - arm_summaries["TOPK_VALUE_SHUFFLE"][MIXED_TRACK]["exact"],
            "margin_preserving_shuffle_gap": best_mixed - arm_summaries["MARGIN_PRESERVING_SHUFFLE"][MIXED_TRACK]["exact"],
            "entropy_preserving_shuffle_gap": best_mixed - arm_summaries["ENTROPY_PRESERVING_SHUFFLE"][MIXED_TRACK]["exact"],
        },
        "support_coherence_report.json": {
            "support_coherence_layer": arm_summaries["SUPPORT_COHERENCE_LAYER"],
            "support_order_shuffle_gap": best_mixed - arm_summaries["SUPPORT_ORDER_SHUFFLE"][MIXED_TRACK]["exact"],
            "support_coherence_break_gap": best_mixed - arm_summaries["SUPPORT_COHERENCE_BREAK"][MIXED_TRACK]["exact"],
        },
        "counterfactual_delta_report.json": {
            "counterfactual_delta_layer": arm_summaries["COUNTERFACTUAL_DELTA_LAYER"],
            "counterfactual_delta_shuffle_gap": best_mixed - arm_summaries["COUNTERFACTUAL_DELTA_SHUFFLE"][MIXED_TRACK]["exact"],
        },
        "cluster_structure_report.json": {
            "cluster_structure_layer": arm_summaries["CLUSTER_STRUCTURE_LAYER"],
            "cluster_structure_shuffle_gap": best_mixed - arm_summaries["CLUSTER_STRUCTURE_SHUFFLE"][MIXED_TRACK]["exact"],
        },
        "weak_diagnostic_repair_report.json": weak_report,
        "strong_diagnostic_preservation_report.json": strong_report,
        "track_uniformity_audit_report.json": {
            "best_arm": best,
            "per_track_exact": {track: arm_summaries[best][track]["exact"] for track in TRACKS},
            "exact_range": max(arm_summaries[best][track]["exact"] for track in TRACKS) - min(arm_summaries[best][track]["exact"] for track in TRACKS),
            "note": "Uniform values can mean the controller selected the same action pattern across tracks; D64S reports this explicitly instead of hiding it.",
        },
        "diagnostic_estimator_accuracy_report.json": {
            track: metrics[track]["diagnostic_accuracy"].get("FULL_STRUCTURE_AWARE_LAYER", {})
            for track in TRACKS
        },
        ...
        "noisy_perturbation_report.json": {
            "track": NOISY_TRACK,
            "full_structure_diagnostic_accuracy": metrics[NOISY_TRACK]["diagnostic_accuracy"].get("FULL_STRUCTURE_AWARE_LAYER", {}),
...
def main():
    ...
    parser.add_argument("--seeds", default="12101,12102,12103,12104,12105")
    ...
    d64b_best = load_d64b_best_summary(repo_root)
    ...
    write_json(
        out / "dataset_manifest.json",
        {
            ...
            "strong_diagnostics": STRONG_DIAGNOSTICS,
            "weak_diagnostics": WEAK_DIAGNOSTICS,
            "calibrated_diagnostic_thresholds": {
                name: spec["threshold"] for name, spec in CALIBRATED_DIAGNOSTIC_MAP.items()
            },
            "structure_groups": {
                "candidate_identity": CANDIDATE_IDENTITY_DIAGNOSTICS,
                "score_shape": SCORE_SHAPE_DIAGNOSTICS,
    ...
    decision, arm_summaries = make_decision(test_metrics_by_track, d64b_best, failed_jobs)
    fallback_rows = 0
    rust_rows = 0
