#!/usr/bin/env python3
"""Checker for D70 support-cost oracle gap reduction."""
...
ALLOWED_DECISIONS = {
    ...
    "oracle_gap_reduction_regression",
    "oracle_gap_reduction_safety_failure",
}
...
def main() -> None:
    ...
    upstream = load_json(out / "d69_upstream_manifest.json")
    taxonomy = load_json(out / "oracle_gap_taxonomy_report.json")
    safe = load_json(out / "safe_deescalation_report.json")
    joint = load_json(out / "joint_counter_recall_report.json")
    top1 = load_json(out / "top1_top2_vs_joint_report.json")
    ...
    observed = upstream.get("d69_artifacts", {})
    if observed.get("decision") != "support_cost_optimization_scale_confirmed":
        fail(f"D69 upstream decision mismatch: {observed.get('decision')}")
    if observed.get("next") != "D70_SUPPORT_COST_ORACLE_GAP_REDUCTION":
        fail(f"D69 upstream next mismatch: {observed.get('next')}")
    ...
    if decision.get("decision") == "support_cost_oracle_gap_reduction_confirmed":
        gates = {
            ...
            "low_cost_regression_rows": best["low_cost_regression_rows"] == 0,
            "support_saved_vs_D69": best["support_saved_vs_D69"] >= 0.20,
            "distance_to_concrete_oracle_support": best["distance_to_concrete_oracle_support"] <= 0.50,
            "min_seed_exact": best["min_seed_exact"] >= 0.997,
        }
        failed = [key for key, passed in gates.items() if not passed]
        if failed:
            fail(f"D70 positive gates failed: {failed}")

    if taxonomy.get("gap_reduced_by", 0.0) <= 0:
        fail("oracle gap was not reduced")
    if not safe.get("safe_deescalation_confirmed"):
        fail("safe deescalation was not confirmed")
    if not joint.get("passed"):
        fail("joint recall gate failed")
    if not top1.get("does_not_repeat_d68_failure"):
        fail("D68 cheap-top1 failure repeated")
    if not external.get("passed"):
        fail("external recall gate failed")
    if repair.get("d68_loss_repair_preservation_rate") != 1.0:
        fail("D68 loss repair preservation failed")
    if not safety.get("passed"):
        fail("safety margin watch gate failed")
