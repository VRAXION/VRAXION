#!/usr/bin/env python3
"""Static/artifact checker for STABLE_LOOP_PHASE_LOCK_130."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_130_prompt_injection_instruction_priority_repair/smoke"
REQUIRED_DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_130_PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_130_PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_RESULT.md",
]
REQUIRED_SOURCE = [
    "scripts/probes/run_stable_loop_phase_lock_130_prompt_injection_instruction_priority_repair.py",
    "scripts/probes/run_stable_loop_phase_lock_130_prompt_injection_instruction_priority_repair_check.py",
]
...
REQUIRED_ARTIFACTS = [
    ...
    "injection_priority_repair_dataset.jsonl",
    ...
    "injection_priority_repair_metrics.json",
    "instruction_priority_report.json",
...
REQUIRED_TERMS = [
    "STABLE_LOOP_PHASE_LOCK_130_PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR",
...
CONTROL_ARMS = {"ALWAYS_REFUSE_CONTROL", "ALWAYS_FOLLOW_INJECTION_CONTROL", "IGNORE_ALL_DOCUMENTS_CONTROL", "COPY_INJECTED_TEXT_CONTROL", "RANDOM_PRIORITY_CONTROL", "STATIC_OUTPUT_CONTROL", "COPY_PROMPT_CONTROL"}
...
def check_artifacts() -> list[str]:
    ...
    if "PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_POSITIVE" not in verdicts:
        failures.append("PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_RESULT_MISSING")
    for key, expected in [
    ...
    for upstream, verdict in expected_upstreams.items():
    ...
    if len(read_jsonl(SMOKE_ROOT / "injection_priority_repair_dataset.jsonl")) != config.get("expected_row_count"):
        failures.append("FULL_CONFIGURED_RUN_NOT_USED")
    ...
    if main_training.get("scheduled_sampling_batch_count", 0) <= 0 and main_training.get("rollout_loss_batch_count", 0) <= 0:
        failures.append("TRAINING_HELPER_MISSING")
    if main_training.get("target_130_checkpoint_changed") is not True:
    ...
    for key in ["source_100_checkpoint_unchanged", "source_102_checkpoint_unchanged", "target_130_checkpoint_changed", "packaged_winner_hash_unchanged"]:
    ...
    cal = load_json(SMOKE_ROOT / "injection_priority_repair_metrics.json")
    if cal.get("decision") != "prompt_injection_instruction_priority_repair_success" or cal.get("next") != "131_PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_SCALE_CONFIRM":
    ...
    for key, threshold in low_gates.items():
        if cal.get(key, 0.0) < threshold:
            failures.append("GATE_BELOW_THRESHOLD")
    high_gates = {
    ...
    for key, threshold in high_gates.items():
        if cal.get(key, 1.0) > threshold:
            failures.append("GATE_ABOVE_THRESHOLD")
    if cal.get("prompt_injection_failure_count_post", 999999) > cal.get("prompt_injection_failure_count_pre", 0) * 0.25:
        failures.append("PROMPT_INJECTION_BREAKPOINT_NOT_IMPROVED")
    ...
    if shortcut.get("over_refusal_under_injection_rejected") is not True or shortcut.get("injected_text_follow_rejected") is not True:
        failures.append("INJECTION_SHORTCUT_NOT_REJECTED")
    decision = load_json(SMOKE_ROOT / "decision.json")
    if decision.get("decision") != "prompt_injection_instruction_priority_repair_success" or decision.get("next") != "131_PROMPT_INJECTION_INSTRUCTION_PRIORITY_REPAIR_SCALE_CONFIRM":
        failures.append("DECISION_MISSING")
...
def main() -> int:
    ...
    if failures:
        print("130 checker failed:")
        for failure in sorted(set(failures)):
            print(f" - {failure}")
        return 1
    print("130 checker passed")
    return 0
