#!/usr/bin/env python3
"""Checker for 140H minimal-marker real-task bridge probe."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_140h_instnct_pocket_gated_minimal_marker_real_task_bridge_probe/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_140h_instnct_pocket_gated_minimal_marker_real_task_bridge_probe.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_140h_instnct_pocket_gated_minimal_marker_real_task_bridge_probe_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_140H_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_PROBE_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_140H_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_PROBE_RESULT.md",
]
...
REQUIRED_ARTIFACTS = [
    ...
    "minimal_marker_bridge_metrics.json",
...
def check_static_files() -> list[str]:
    ...
    for rel in [RUNNER, CHECKER, *DOCS]:
        ...
        for term in ["140H", "minimal-marker", "real-task", "bridge", "pocket"]:
            if term not in text:
...
def check_artifacts() -> list[str]:
    ...
    upstream = load_json(SMOKE_ROOT / "upstream_140g_manifest.json")
    ...
    eval_manifest = load_json(SMOKE_ROOT / "minimal_marker_eval_manifest.json")
    prompt_manifest = load_json(SMOKE_ROOT / "real_task_bridge_prompt_manifest.json")
    ...
    metrics = load_json(SMOKE_ROOT / "minimal_marker_bridge_metrics.json")
    ...
    if upstream.get("decision") != "minimal_marker_real_task_bridge_plan_complete":
        failures.append("BAD_140G_DECISION")
    if upstream.get("next") != "140H_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_PROBE":
        failures.append("BAD_140G_NEXT")
    if upstream.get("target_milestone") != "140H_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_PROBE":
        failures.append("BAD_140G_TARGET")
    ...
    if eval_manifest.get("row_count", 0) < 512:
        failures.append("MINIMAL_MARKER_EVAL_TOO_SMALL")
    if eval_manifest.get("family_count", 0) < 4:
        failures.append("TOO_FEW_FAMILIES")
    if eval_manifest.get("scaffold_variant_count", 0) < 12:
    ...
    if implicit_gate.get("literal_gate_pocket_open_allowed_in_main") is not False:
        failures.append("IMPLICIT_GATE_POLICY_BAD")
    if audit.get("direct_pocket_value_marker_rate", 1.0) != 0.0:
        failures.append("DIRECT_POCKET_VALUE_MARKER_PRESENT")
    if audit.get("explicit_pocket_token_row_rate", 1.0) > 0.20:
        failures.append("EXPLICIT_POCKET_TOKEN_RATE_TOO_HIGH")
    if audit.get("explicit_gate_pocket_open_row_rate", 1.0) > 0.30:
        failures.append("EXPLICIT_GATE_RATE_TOO_HIGH")
    ...
    if len(selected_rows) != 1:
        ...
        if selected.get("candidate") != "open_minimal_marker_all_payloads":
            failures.append("BAD_SELECTED_CANDIDATE")
        if selected.get("answer_value_accuracy", 0.0) < 0.70:
            failures.append("SELECTED_ACCURACY_TOO_LOW")
        if selected.get("pocket_writeback_rate", 0.0) < 0.80:
            failures.append("SELECTED_WRITEBACK_TOO_LOW")
    if selection.get("gradient_used") is not False:
        failures.append("GRADIENT_USED")
    if selection.get("selected_candidate") != "open_minimal_marker_all_payloads":
    ...
    for control in ["VISIBLE_VALUE_BYPASS_CONTROL", "NOISY_DISTRACTOR_CONTROL", "CLOSED_POCKET_ABLATION_CONTROL", "STATIC_OUTPUT_CONTROL", "COPY_PROMPT_CONTROL", "TRAIN_NAMESPACE_REPLAY_CONTROL", "PREFIX_ONLY_CONTROL", "MINIMAL_GATE_REMOVED_CONTROL"]:
    ...
    if main.get("answer_value_accuracy", 0.0) < 0.70:
        failures.append("MAIN_ACCURACY_TOO_LOW")
    if main.get("exact_answer_accuracy", 0.0) < 0.70:
        failures.append("MAIN_EXACT_ACCURACY_TOO_LOW")
    if main.get("pocket_writeback_rate", 0.0) < 0.80:
        failures.append("MAIN_WRITEBACK_TOO_LOW")
    if main.get("phase_transport_success_rate", 0.0) < 0.80:
        failures.append("MAIN_PHASE_TRANSPORT_TOO_LOW")
    if main.get("contrast_group_accuracy", 0.0) < 0.70:
    ...
    if ablation.get("pocket_writeback_rate", 1.0) > 0.05:
        failures.append("ABLATION_WRITEBACK_TOO_HIGH")
    if comparison.get("pocket_ablation_delta_answer_value_accuracy", 0.0) < 0.45:
    ...
    for event in ["startup", "upstream verification", "helper and ast verification", "minimal marker eval row build", "candidate evaluated", "mutation selection", "canary", "final eval generation", "scoring", "controls", "determinism replay", "aggregate analysis", "decision", "final verdict"]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 140H minimal-marker real-task bridge probe")
    ...
    payload = {"schema_version": "phase_140h_checker_result_v1", "status": "pass" if not failures else "fail", "failures": failures}
