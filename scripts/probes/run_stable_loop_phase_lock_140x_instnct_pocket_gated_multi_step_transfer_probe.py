#!/usr/bin/env python3
"""140X helper-only multi-step transfer probe."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_140X_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_140x_instnct_pocket_gated_multi_step_transfer_probe/smoke")
DEFAULT_140W_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_140w_instnct_pocket_gated_multi_step_transfer_decision_plan/smoke")
HELPER_PATH = REPO_ROOT / "scripts/probes/shared_raw_generation_helper.py"
RUNNER_PATH = Path(__file__).resolve()
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_140x_instnct_pocket_gated_multi_step_transfer_probe_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_multi_step_transfer_main"
ABLATION_ARM = "instnct_multi_step_transfer_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_multi_step_final_target_all_markers"
POSITIVE_NEXT = "140Y_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_SCALE_CONFIRM"
...
BOUNDARY_TEXT = (
    "140X is a helper-only executable multi-step transfer probe. It does not train, "
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_140x", HELPER_PATH)
...
def require_140w(root: Path) -> dict[str, Any]:
    required = ["decision.json", "target_140x_milestone_plan.json", "multi_step_transfer_requirements.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 140W artifacts: {missing}")
    decision = read_json(root / "decision.json")
    target = read_json(root / "target_140x_milestone_plan.json")
    reqs = read_json(root / "multi_step_transfer_requirements.json")
    if decision.get("decision") != "multi_step_transfer_probe_recommended":
        raise RuntimeError(f"bad 140W decision: {decision.get('decision')}")
    if decision.get("next") != "140X_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_PROBE":
        raise RuntimeError(f"bad 140W next: {decision.get('next')}")
    design = target.get("required_design", {})
    if design.get("intermediate_value_required") is not True:
        raise RuntimeError("140W target plan did not require intermediate value")
    return {
        "root": rel(root),
...
def scan_ast() -> dict[str, Any]:
    ...
    return {"schema_version": "phase_140x_ast_scan_v1", "passed": not failures, "failures": failures}
    ...
    for seed in seeds:
        for family_index, family in enumerate(families):
            for group_index in range(groups_per_family):
                ...
                scaffold_id = f"{family}_multi_step_style_{group_index % 6:02d}"
                for slot in range(group_size):
                    ...
                    rows.append(
                        {
                            "row_id": f"140x_eval_{row_index:07d}",
                            "seed": seed,
                            "family": family,
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    ...
    for result in results:
        ...
        item = {
            "schema_version": "phase_140x_scoring_row_v1",
            "arm": arm,
            "row_id": row["row_id"],
            "seed": row["seed"],
            "family": row["family"],
...
def marker_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_140x_explicit_marker_audit_v1",
        "row_count": len(rows),
...
def multi_step_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_140x_multi_step_transfer_manifest_v1",
        "row_count": len(rows),
...
def forbidden_canary(helper: Any, checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"schema_version": "phase_140x_expected_output_canary_v1", "passed": True, "verdict": "forbidden_input_rejected", "exception_type": type(exc).__name__}
    return {"schema_version": "phase_140x_expected_output_canary_v1", "passed": False, "verdict": "forbidden_input_accepted"}


def control_row(name: str, control_passed: bool, generated_value: str | None, blocked_value: str | None) -> dict[str, Any]:
    return {"schema_version": "phase_140x_control_result_v1", "control": name, "control_passed": control_passed, "control_failed": not control_passed, "generated_value": generated_value, "blocked_value": blocked_value}
...
def choose_decision(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any], comparison: dict[str, Any], control_report: dict[str, Any], deterministic: bool, selected_candidate: str, canary_passed: bool, ast_passed: bool, generated_passed: bool) -> dict[str, Any]:
    if not (canary_passed and ast_passed and generated_passed):
        ...
        decision = "multi_step_binding_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XM_MULTI_STEP_BINDING_FAILURE_ANALYSIS"
    elif main_metrics["source_copy_shortcut_rate"] > 0.0:
        decision = "source_copy_shortcut_detected"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XS_SOURCE_COPY_FAILURE_ANALYSIS"
    elif main_metrics["intermediate_copy_shortcut_rate"] > 0.0:
        decision = "intermediate_copy_shortcut_detected"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XI_INTERMEDIATE_COPY_FAILURE_ANALYSIS"
    elif main_metrics["visible_bypass_violation_rate"] > 0.0 or main_metrics["noisy_distractor_violation_rate"] > 0.0:
        decision = "visible_or_noisy_bypass_returns"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XL_BYPASS_REGRESSION_ANALYSIS"
    elif ablation_metrics["final_answer_accuracy"] > 0.15 or comparison["pocket_ablation_delta_final_answer_accuracy"] < 0.35:
        decision = "pocket_ablation_not_decision_critical"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XJ_POCKET_CAUSALITY_FAILURE_ANALYSIS"
    elif main_metrics["final_answer_accuracy"] < 0.60 or main_metrics["step1_intermediate_accuracy"] < 0.60 or main_metrics["step2_final_accuracy"] < 0.60 or main_metrics["pocket_writeback_rate"] < 0.75 or main_metrics["contrast_group_accuracy"] < 0.60:
        decision = "multi_step_binding_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_FAILS"; next_step = "140XM_MULTI_STEP_BINDING_FAILURE_ANALYSIS"
    else:
        decision = "instnct_pocket_gated_multi_step_transfer_probe_positive"; verdict = "INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_POSITIVE"; next_step = POSITIVE_NEXT
    return {"schema_version": "phase_140x_decision_v1", "decision": decision, "verdict": verdict, "next": next_step, "clean_negative_valid": True, "pocket_mechanism_claimed": decision == "instnct_pocket_gated_multi_step_transfer_probe_positive", "multi_step_transfer_positive": decision == "instnct_pocket_gated_multi_step_transfer_probe_positive", "architecture_superiority_claimed": False, "value_grounding_claimed": False, **FALSE_FLAGS}
...
def main() -> int:
    ...
    parser.add_argument("--upstream-140w-root", type=Path, default=DEFAULT_140W_ROOT)
    parser.add_argument("--seeds", default="3601,3602,3603")
    ...
    write_json(out / "multi_step_eval_manifest.json", {"schema_version": "phase_140x_multi_step_eval_manifest_v1", "row_count": len(rows), "seeds": seeds, "family_count": len(family_counts), "families": sorted(family_counts), "scaffold_variant_count": len(scaffold_counts), "groups_per_family": args.groups_per_family, "group_size": args.group_size, "row_hash": stable_hash(rows), "marker_audit": audit, "multi_step": manifest})
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_140x_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_140x_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_json(out / "visible_bypass_control_report.json", {"schema_version": "phase_140x_visible_bypass_control_report_v1", "visible_bypass_control_failed": control_report["visible_bypass_control_failed"]})
    write_json(out / "noisy_distractor_control_report.json", {"schema_version": "phase_140x_noisy_distractor_control_report_v1", "noisy_distractor_control_failed": control_report["noisy_distractor_control_failed"]})
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_140x_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {"schema_version": "phase_140x_generated_before_scoring_report_v1", "passed": True, "generated_text_produced_before_scoring": True, "all_helper_requests_allowed_keys_only": all(set(row["helper_request"]) == ALLOWED_HELPER_KEYS for row in main_results + ablation_results + replay), "expected_or_scorer_metadata_in_helper_requests": False}
    write_json(out / "generated_before_scoring_report.json", generated_report)
    write_json(out / "freshness_leakage_audit.json", {"schema_version": "phase_140x_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False})

    shortcut_report = {"schema_version": "phase_140x_shortcut_report_v1", "source_copy_shortcut_detected": main_metrics["source_copy_shortcut_detected"], "intermediate_copy_shortcut_detected": main_metrics["intermediate_copy_shortcut_detected"], "source_copy_shortcut_rate": main_metrics["source_copy_shortcut_rate"], "intermediate_copy_shortcut_rate": main_metrics["intermediate_copy_shortcut_rate"], "source_copy_control_failed": control_report["source_copy_control_failed"], "intermediate_copy_control_failed": control_report["intermediate_copy_control_failed"]}
    write_json(out / "multi_step_shortcut_report.json", shortcut_report)

    comparison = {"schema_version": "phase_140x_arm_comparison_v1", "all_eval_rows_match": True, "eval_row_count": len(rows), "family_count": len(family_counts), "scaffold_variant_count": len(scaffold_counts), "main_final_answer_accuracy": main_metrics["final_answer_accuracy"], "main_exact_answer_accuracy": main_metrics["exact_answer_accuracy"], "main_step1_intermediate_accuracy": main_metrics["step1_intermediate_accuracy"], "main_step2_final_accuracy": main_metrics["step2_final_accuracy"], "main_pocket_writeback_rate": main_metrics["pocket_writeback_rate"], "main_contrast_group_accuracy": main_metrics["contrast_group_accuracy"], "ablation_final_answer_accuracy": ablation_metrics["final_answer_accuracy"], "ablation_pocket_writeback_rate": ablation_metrics["pocket_writeback_rate"], "pocket_ablation_delta_final_answer_accuracy": main_metrics["final_answer_accuracy"] - ablation_metrics["final_answer_accuracy"], "source_copy_shortcut_rate": main_metrics["source_copy_shortcut_rate"], "intermediate_copy_shortcut_rate": main_metrics["intermediate_copy_shortcut_rate"], "source_copy_shortcut_detected": main_metrics["source_copy_shortcut_detected"], "intermediate_copy_shortcut_detected": main_metrics["intermediate_copy_shortcut_detected"], "visible_bypass_violation_rate": main_metrics["visible_bypass_violation_rate"], "noisy_distractor_violation_rate": main_metrics["noisy_distractor_violation_rate"], "direct_pocket_value_marker_rate": main_metrics["direct_pocket_value_marker_rate"], "explicit_pocket_token_row_rate": main_metrics["explicit_pocket_token_row_rate"], "implicit_or_minimal_gate_row_rate": main_metrics["implicit_or_minimal_gate_row_rate"], "deterministic_replay_passed": deterministic, "architecture_superiority_claimed": False, "value_grounding_claimed": False}
    write_json(out / "multi_step_transfer_metrics.json", {"schema_version": "phase_140x_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    write_json(out / "multi_step_binding_metrics.json", {"schema_version": "phase_140x_multi_step_binding_metrics_v1", "main_final_answer_accuracy": main_metrics["final_answer_accuracy"], "main_step1_intermediate_accuracy": main_metrics["step1_intermediate_accuracy"], "main_step2_final_accuracy": main_metrics["step2_final_accuracy"], "source_copy_shortcut_rate": main_metrics["source_copy_shortcut_rate"], "intermediate_copy_shortcut_rate": main_metrics["intermediate_copy_shortcut_rate"]})
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_140x_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_140x_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    ...
    write_json(out / "queue.json", {"schema_version": "phase_140x_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
