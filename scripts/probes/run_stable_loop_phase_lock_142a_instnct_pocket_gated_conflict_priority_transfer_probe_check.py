#!/usr/bin/env python3
"""Checker for 142A conflict/priority transfer probe."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_142a_instnct_pocket_gated_conflict_priority_transfer_probe/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_142a_instnct_pocket_gated_conflict_priority_transfer_probe.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_142a_instnct_pocket_gated_conflict_priority_transfer_probe_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_142A_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_PROBE_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_142A_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_PROBE_RESULT.md",
]
...
REQUIRED_ARTIFACTS = [
    "queue.json", "progress.jsonl", "upstream_141z_manifest.json", "upstream_141f_manifest.json",
    ...
    "per_seed_metrics.json", "per_family_metrics.json", "priority_inversion_report.json",
...
def require_upstreams(root: Path, failures: list[str]) -> None:
    z = load_json(root / "upstream_141z_manifest.json")
    f = load_json(root / "upstream_141f_manifest.json")
    if z.get("decision") != "conflict_priority_transfer_probe_recommended":
        failures.append(f"BAD_141Z_DECISION:{z.get('decision')}")
    if z.get("next") != "142A_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_PROBE":
        failures.append(f"BAD_141Z_NEXT:{z.get('next')}")
    if f.get("decision") != "instnct_pocket_gated_multi_field_transfer_scale_confirmed":
        failures.append(f"BAD_141F_DECISION:{f.get('decision')}")
    if f.get("next") != "141Z_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_NEXT_DECISION_PLAN":
        failures.append(f"BAD_141F_NEXT:{f.get('next')}")
    audit = f.get("helper_request_audit", {})
    if audit.get("all_requests_allowed_keys_only") is not True or audit.get("forbidden_keys_present_count") != 0:
        failures.append(f"BAD_141F_HELPER_AUDIT:{audit}")
    if f.get("per_seed_gate_passed") is not True or f.get("per_family_gate_passed") is not True:
        failures.append("BAD_141F_HARDENED_GATE_REPORTS")
...
def require_manifests(root: Path, failures: list[str]) -> None:
    ...
    if manifest.get("row_count", 0) < 800:
    ...
    if pairs.get("same_template_opposite_winner_pairs", 0) < 100:
...
def require_metrics(root: Path, failures: list[str]) -> None:
    ...
    floors = {
        ...
        "priority_rule_accuracy": 0.70,
        "conflict_resolution_accuracy": 0.70,
        "priority_inversion_accuracy": 0.65,
        "same_template_opposite_winner_accuracy": 0.65,
    ...
    for key in ["priority_rule_accuracy", "conflict_resolution_accuracy", "wrong_priority_field_rate", "priority_default_shortcut_rate", "priority_inversion_accuracy", "same_template_opposite_winner_accuracy"]:
    ...
    if inversion.get("passed") is not True or inversion.get("priority_inversion_accuracy", 0.0) < 0.65:
...
def require_decision(root: Path, failures: list[str]) -> None:
    ...
    if decision.get("verdict") != "INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_POSITIVE":
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 142A conflict/priority transfer probe artifacts.")
    ...
    if failures:
        print("142A CHECK FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("142A CHECK PASS")
    return 0
