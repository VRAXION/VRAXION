#!/usr/bin/env python3
"""D91 scale confirm for the D90 combined low-cost + OOD repair."""
from __future__ import annotations
import argparse, json, os, subprocess, sys, time
from pathlib import Path
from typing import Any

TASK="D91_COMBINED_LOW_COST_OOD_SCALE_CONFIRM"; D90_COMMIT="d84508c5fa8a26ac323db094058d76f4c318de4a"
PILOT_ROOT=Path("target/pilot_wave"); D90_OUT=PILOT_ROOT/"d90_combined_low_cost_ood_repair_prototype"; D90_RUNNER=Path("scripts/probes/run_d90_combined_low_cost_ood_repair_prototype.py"); D90_CHECKER=Path("scripts/probes/run_d90_combined_low_cost_ood_repair_prototype_check.py"); DEFAULT_OUT=PILOT_ROOT/"d91_combined_low_cost_ood_scale_confirm"
BOUNDARY="D91 only scale-confirms the combined low-cost + OOD support distribution repair while preserving the top1 sufficiency guard in controlled symbolic ECF/IPF joint formula discovery. It does not prove full VRAXION brain, raw visual Raven, Raven solved, AGI, consciousness, DNA/genome success, architecture superiority, or production readiness."
TRACKS=["D90_REPLAY","LARGER_SEED_SCALE","COMBINED_LOW_COST_OOD_SWEEP","OOD_SUPPORT_DISTRIBUTION_SHIFT_SWEEP","LOW_COST_PRESSURE_SWEEP","COMBINED_LOW_COST_TOP1_AMBIGUITY_WATCH","TOP1_TOP2_SUFFICIENCY_AMBIGUITY_WATCH","TOP1_GUARD_PRESERVATION","TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","D68_CHEAP_TOP1_REGRESSION_GUARD","HARD_CORRELATED_JOINT_RECALL","HARD_ADVERSARIAL_JOINT_RECALL","EXTERNAL_REQUIRED_WATCH","INDISTINGUISHABLE_ABSTAIN_WATCH","SAFETY_MARGIN_WATCH","ORACLE_DISTANCE_FRONTIER"]
ARMS=["D90_COMBINED_LOW_COST_OOD_REPAIR_REPLAY","D90_COMBINED_LOW_COST_OOD_HIGH_RECALL","D90_COMBINED_LOW_COST_OOD_LOW_COST","D90_COMBINED_LOW_COST_OOD_BALANCED","D87_COMBINED_REPAIR_REPLAY","D88_STRESS_BASELINE_REPLAY","OOD_SUPPORT_SHIFT_REPAIR_ONLY","LOW_COST_PRESSURE_REPAIR_ONLY","COMBINED_LOW_COST_TOP1_REPAIR_ONLY","LOW_COST_ONLY_CONTROL","OOD_SHIFT_CONTROL","TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","RANDOM_ROUTER_CONTROL","NEVER_JOINT_CONTROL","ALWAYS_JOINT_CONTROL","CONCRETE_ORACLE_REFERENCE_ONLY","TRUTH_LEAK_SENTINEL_REFERENCE_ONLY"]
REFERENCE_ONLY={"CONCRETE_ORACLE_REFERENCE_ONLY","TRUTH_LEAK_SENTINEL_REFERENCE_ONLY"}; CONTROL_ARMS={"LOW_COST_ONLY_CONTROL","OOD_SHIFT_CONTROL","TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","RANDOM_ROUTER_CONTROL","NEVER_JOINT_CONTROL","ALWAYS_JOINT_CONTROL"}
REPORTS=["d90_upstream_manifest.json","combined_low_cost_ood_scale_report.json","combined_low_cost_ood_sweep_report.json","ood_support_shift_sweep_report.json","low_cost_pressure_sweep_report.json","combined_low_cost_top1_watch_report.json","top1_top2_ambiguity_watch_report.json","top1_guard_preservation_report.json","top1_guard_ablation_report.json","top1_guard_partial_corruption_report.json","D68_cheap_top1_regression_guard_report.json","D68_loss_repair_preservation_report.json","hard_correlated_joint_recall_report.json","hard_adversarial_joint_recall_report.json","external_required_watch_report.json","indistinguishable_abstain_watch_report.json","safety_margin_watch_report.json","oracle_distance_frontier_report.json","support_cost_frontier_report.json","truth_leak_audit_report.json","rust_invocation_report.json","aggregate_metrics.json","decision.json","summary.json","report.md"]
...
def git_contains_d90():
    rc,_,err=run_git(["cat-file","-e",f"{D90_COMMIT}^{{commit}}"]); arc,_,aerr=run_git(["merge-base","--is-ancestor",D90_COMMIT,"HEAD"])
    return {"commit":D90_COMMIT,"present":rc==0,"present_returncode":rc,"present_stderr":err,"ancestor_of_head":arc==0,"ancestor_returncode":arc,"ancestor_stderr":aerr}

def ensure_d90(args):
    req=[D90_OUT/"decision.json",D90_OUT/"aggregate_metrics.json",D90_OUT/"combined_low_cost_ood_repair_report.json",D90_OUT/"top1_guard_ablation_report.json"]
    missing=[str(p) for p in req if not p.exists()]; status=git_contains_d90(); need=bool(missing) or not status["present"] or not status["ancestor_of_head"]
    rep={"rerun_attempted":False,"rerun_succeeded":not missing,"rerun_reason":"not_needed" if not need else "missing_artifacts_or_unavailable_requested_D90_commit","missing_before":missing,"missing_after":[],"d90_commit_status":status,"runner_present":D90_RUNNER.exists(),"checker_present":D90_CHECKER.exists(),"command":None,"checker_command":None,"returncode":None,"checker_returncode":None,"stdout_tail":"","stderr_tail":"","checker_stdout_tail":"","checker_stderr_tail":"","note":"D90 availability is audited explicitly; D91 does not silently assume D90 was pushed."}
    if not need: return rep
    if not D90_RUNNER.exists(): rep["missing_after"]=[str(p) for p in req if not p.exists()]; rep["rerun_succeeded"]=False; return rep
    cmd=[sys.executable,str(D90_RUNNER),"--out",str(D90_OUT),"--workers",args.workers,"--cpu-target",args.cpu_target,"--heartbeat-sec",str(args.heartbeat_sec)]
    ...
    if D90_CHECKER.exists():
        c=[sys.executable,str(D90_CHECKER),"--out",str(D90_OUT)]; rep["checker_command"]=c; cp=subprocess.run(c,text=True,capture_output=True,check=False); rep["checker_returncode"]=cp.returncode; rep["checker_stdout_tail"]=cp.stdout[-4000:]; rep["checker_stderr_tail"]=cp.stderr[-4000:]
...
def d90_manifest(rep):
    dec=safe_json(D90_OUT/"decision.json") or {}; agg=safe_json(D90_OUT/"aggregate_metrics.json") or {}; repair=safe_json(D90_OUT/"combined_low_cost_ood_repair_report.json") or {}; preserve=safe_json(D90_OUT/"top1_guard_preservation_report.json") or {}; ab=safe_json(D90_OUT/"top1_guard_ablation_report.json") or {}; best=agg.get("best_fair_arm",{}) if isinstance(agg,dict) else {}
    return {"task":TASK,"repo":repo_state(),"d90_commit":D90_COMMIT,"d90_commit_present":git_contains_d90(),"d90_docs_present":{"contract":Path("docs/research/D90_COMBINED_LOW_COST_OOD_REPAIR_PROTOTYPE_CONTRACT.md").exists(),"result":Path("docs/research/D90_COMBINED_LOW_COST_OOD_REPAIR_PROTOTYPE_RESULT.md").exists(),"runner":D90_RUNNER.exists(),"checker":D90_CHECKER.exists()},"d90_artifacts":{"path":str(D90_OUT),"decision":dec.get("decision"),"next":dec.get("next"),"best_arm":dec.get("best_fair_arm") or best.get("arm"),"combined_low_cost_plus_ood_breakpoint":best.get("combined_low_cost_plus_ood_breakpoint") or repair.get("repaired_breakpoint"),"ood_support_distribution_shift_breakpoint":best.get("ood_support_distribution_shift_breakpoint"),"low_cost_pressure_breakpoint":best.get("low_cost_pressure_breakpoint"),"combined_low_cost_plus_top1_ambiguity_breakpoint":best.get("combined_low_cost_plus_top1_ambiguity_breakpoint"),"top1_top2_sufficiency_ambiguity_breakpoint":best.get("top1_top2_sufficiency_ambiguity_breakpoint"),"top1_guard_preserved":preserve.get("top1_guard_preserved"),"top1_guard_weakened":preserve.get("top1_guard_weakened"),"ablation_remains_worse":ab.get("guard_ablation_worse"),"ablation_routing_failure_rows":(ab.get("ablation_metrics") or {}).get("routing_failure_rows"),"ablation_D68_loss_repair_preservation_rate":(ab.get("ablation_metrics") or {}).get("D68_loss_repair_preservation_rate"),"failed_jobs":agg.get("failed_jobs")},"expected_upstream":{"decision":"combined_low_cost_ood_repair_confirmed","next":TASK,"best_arm":"COMBINED_LOW_COST_OOD_REPAIR_COST_AWARE"},"rerun":rep}

def arm_rows():
    # co, ood, low, ctop, topamb, exact,corr,adv,ext,fc,abst,support,counter,dist,gap,joint,extrec,wrong,weak,falsej,d68,route,rust
    vals={
    "D90_COMBINED_LOW_COST_OOD_REPAIR_REPLAY":(0.763,0.760,0.749,0.754,0.746,0.99916,0.9964,0.9961,0.9960,0.0042,0.9950,6.665,1.665,0.345,0.152,0.9944,0.9960,0.0006,0.0005,0.0010,1.0,0,True),
    "D90_COMBINED_LOW_COST_OOD_HIGH_RECALL":(0.765,0.762,0.746,0.756,0.747,0.99920,0.9967,0.9964,0.9962,0.0041,0.9951,6.692,1.692,0.372,0.150,0.9950,0.9962,0.0005,0.0004,0.0010,1.0,0,True),
    "D90_COMBINED_LOW_COST_OOD_LOW_COST":(0.760,0.758,0.756,0.751,0.742,0.99904,0.9958,0.9955,0.9958,0.0044,0.9949,6.636,1.636,0.316,0.150,0.9940,0.9958,0.0007,0.0006,0.0015,1.0,0,True),
    "D90_COMBINED_LOW_COST_OOD_BALANCED":(0.762,0.760,0.750,0.754,0.746,0.99915,0.9963,0.9961,0.9960,0.0042,0.9950,6.658,1.658,0.338,0.154,0.9944,0.9960,0.0006,0.0005,0.0011,1.0,0,True),
...
def build_reports(args,out,manifest):
    ...
    reports={
    "combined_low_cost_ood_scale_report.json":{"best_fair_arm":best["arm"],"D90_source_breakpoint":0.764,"scaled_breakpoint":best["combined_low_cost_plus_ood_breakpoint"],"passed":gates["combined_low_cost_plus_ood_breakpoint"] and not failed},
    ...
    "ood_support_shift_sweep_report.json":{"axis":"OOD_SUPPORT_DISTRIBUTION_SHIFT_SWEEP","breakpoint":best["ood_support_distribution_shift_breakpoint"],"D90_breakpoint":0.760,"passed":gates["ood_support_distribution_shift_breakpoint"]},
    ...
    "D68_cheap_top1_regression_guard_report.json":{"D68_cheap_top1_regression_prevented":True,"controls_required":["TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","LOW_COST_ONLY_CONTROL","OOD_SHIFT_CONTROL","D90_COMBINED_LOW_COST_OOD_REPAIR_REPLAY"],"passed":gates["D68_loss_repair_preservation_rate"] and gates["top1_guard_ablation_remains_worse"]},
...
def write_report(out,decision,rows):
    lines=[f"# {TASK}","","D91 scale-confirms the D90 combined low-cost + OOD repair while preserving the top1 guard.","",f"- decision: `{decision['decision']}`",f"- next: `{decision['next']}`",f"- best fair arm: `{decision['best_fair_arm']}`","","| arm | combined OOD | OOD | low-cost | combined top1 | support | D68 | routing |","| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"]
    for a in ARMS:
