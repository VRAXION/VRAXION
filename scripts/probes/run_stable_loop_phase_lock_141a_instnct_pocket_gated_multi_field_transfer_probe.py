#!/usr/bin/env python3
"""141A helper-only multi-field transfer probe."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_141a_instnct_pocket_gated_multi_field_transfer_probe/smoke")
DEFAULT_140Z_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_140z_instnct_pocket_gated_multi_field_transfer_decision_plan/smoke")
...
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_141a_instnct_pocket_gated_multi_field_transfer_probe_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_multi_field_transfer_main"
ABLATION_ARM = "instnct_multi_field_transfer_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_multi_field_final_all_markers"
POSITIVE_NEXT = "141F_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_SCALE_CONFIRM"
...
BOUNDARY_TEXT = (
    "141A is a helper-only multi-field transfer probe. It does not train, mutate "
    "source checkpoints, modify shared_raw_generation_helper.py, modify helper/"
    "backend/runtime/release/product surfaces, change public request keys, start "
    "services, deploy, or claim GPT-like or broad assistant readiness."
)
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_141a", HELPER_PATH)
...
def require_140z(root: Path) -> dict[str, Any]:
    required = ["decision.json", "target_141a_milestone_plan.json", "multi_field_transfer_requirements.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 140Z artifacts: {missing}")
    decision = read_json(root / "decision.json")
    target = read_json(root / "target_141a_milestone_plan.json")
    reqs = read_json(root / "multi_field_transfer_requirements.json")
    if decision.get("decision") != "multi_field_transfer_probe_recommended":
        raise RuntimeError(f"bad 140Z decision: {decision.get('decision')}")
    if decision.get("next") != "141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE":
        raise RuntimeError(f"bad 140Z next: {decision.get('next')}")
    if target.get("selected_candidate") != SELECTED_CANDIDATE:
        raise RuntimeError("140Z target plan selected wrong candidate")
    if reqs.get("must_test_multi_field_not_single_field") is not True:
        raise RuntimeError("140Z target plan did not require multi-field testing")
    return {
        "root": rel(root),
        "decision": decision.get("decision"),
        "next": decision.get("next"),
        "target_milestone": target.get("milestone"),
        "selected_candidate": target.get("selected_candidate"),
        "positive_gates": reqs.get("positive_gates"),
        "families": reqs.get("families"),
    }
...
def require_140y(root: Path) -> dict[str, Any]:
    ...
    if comparison.get("main_final_answer_accuracy") != 1.0 or comparison.get("pocket_ablation_delta_final_answer_accuracy") != 1.0:
        raise RuntimeError("140Y scale evidence no longer matches required profile")
    return {
        "root": rel(root),
        "decision": decision.get("decision"),
        "next": decision.get("next"),
...
def scan_ast() -> dict[str, Any]:
    ...
    return {"schema_version": "phase_141a_ast_scan_v1", "passed": not failures, "failures": failures}
    ...
    for seed in seeds:
        for family_index, family in enumerate(families):
            for group_index in range(groups_per_family):
                ...
                scaffold_id = f"{family}_multi_field_style_{group_index % 8:02d}"
                for slot in range(group_size):
                    ...
                    row = {
                        "row_id": f"141a_eval_{row_index:07d}",
                        "seed": seed,
                        "family": family,
...
def row_score(row: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_141a_scoring_result_v1",
        "arm": result["arm"],
        "row_id": row["row_id"],
        "seed": row["seed"],
        "family": row["family"],
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    ...
    for group_id, items in sorted(by_group.items()):
        ...
        group_rows.append(
            {
                "schema_version": "phase_141a_contrast_group_result_v1",
                "arm": arm,
                "contrast_group_id": group_id,
...
def marker_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_141a_explicit_marker_audit_v1",
        "row_count": len(rows),
...
def multi_field_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_141a_multi_field_binding_manifest_v1",
        "row_count": len(rows),
...
def forbidden_canary(helper: Any, checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"schema_version": "phase_141a_expected_output_canary_v1", "passed": True, "verdict": "forbidden_input_rejected", "exception_type": type(exc).__name__}
    return {"schema_version": "phase_141a_expected_output_canary_v1", "passed": False, "verdict": "forbidden_input_accepted"}


def control_row(name: str, control_passed: bool, generated_value: str | None, blocked_value: str | None) -> dict[str, Any]:
    return {"schema_version": "phase_141a_control_result_v1", "control": name, "control_passed": control_passed, "control_failed": not control_passed, "generated_value": generated_value, "blocked_value": blocked_value}
...
def run_controls(helper: Any, checkpoints: dict[str, tuple[Path, str]], max_new_tokens: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    return rows, {
        "schema_version": "phase_141a_control_arm_report_v1",
...
def choose_decision(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any], comparison: dict[str, Any], control_report: dict[str, Any], deterministic: bool, selected_candidate: str, canary_passed: bool, ast_passed: bool, generated_passed: bool) -> dict[str, Any]:
    if not (canary_passed and ast_passed and generated_passed):
        ...
        decision = "multi_field_binding_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_FAILS"; next_step = "141C_MULTI_FIELD_BINDING_FAILURE_ANALYSIS"
    elif ablation_metrics["final_answer_accuracy"] > 0.20 or comparison["pocket_ablation_delta"] < 0.30:
    ...
    elif main_metrics["final_answer_accuracy"] < 0.55 or main_metrics["multi_field_binding_accuracy"] < 0.55 or main_metrics["pocket_writeback_rate"] < 0.70 or main_metrics["contrast_group_accuracy"] < 0.55:
        decision = "multi_field_binding_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_FAILS"; next_step = "141C_MULTI_FIELD_BINDING_FAILURE_ANALYSIS"
    else:
        decision = "instnct_pocket_gated_multi_field_transfer_probe_positive"; verdict = "INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_POSITIVE"; next_step = POSITIVE_NEXT
    return {
        "schema_version": "phase_141a_decision_v1",
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "clean_negative_valid": True,
        "pocket_mechanism_claimed": decision == "instnct_pocket_gated_multi_field_transfer_probe_positive",
        "multi_field_transfer_positive": decision == "instnct_pocket_gated_multi_field_transfer_probe_positive",
...
def main() -> int:
    ...
    parser.add_argument("--upstream-140z-root", type=Path, default=DEFAULT_140Z_ROOT)
    ...
    write_json(out / "queue.json", {"schema_version": "phase_141a_queue_v1", "milestone": MILESTONE, "status": "running"})

    seeds = [int(item) for item in args.seeds.split(",") if item.strip()]
    upstream_140z = require_140z(resolve_repo_path(args.upstream_140z_root))
    upstream_140y = require_140y(resolve_repo_path(args.upstream_140y_root))
    write_json(out / "upstream_140z_manifest.json", upstream_140z)
    write_json(out / "upstream_140y_manifest.json", upstream_140y)
    append_progress(out, "upstream verification", upstream_140z=upstream_140z["decision"], upstream_140y=upstream_140y["decision"])

    config = {
        "schema_version": "phase_141a_eval_config_v1",
        "milestone": MILESTONE,
        "boundary": BOUNDARY_TEXT,
        "train_allowed": False,
    ...
    helper = load_helper()
    provenance = {
        "schema_version": "phase_141a_helper_provenance_v1",
    ...
    write_json(out / "multi_field_eval_manifest.json", {"schema_version": "phase_141a_multi_field_eval_manifest_v1", "row_count": len(rows), "seeds": seeds, "family_count": len(family_counts), "families": sorted(family_counts), "scaffold_variant_count": len(scaffold_counts), "groups_per_family": args.groups_per_family, "group_size": args.group_size, "row_hash": stable_hash(rows), "marker_audit": audit, "multi_field": manifest})
    ...
    for candidate in candidate_specs():
        ...
        candidate_rows.append(
            {
                "schema_version": "phase_141a_mutation_candidate_result_v1",
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_141a_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_141a_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_json(out / "visible_bypass_control_report.json", {"schema_version": "phase_141a_visible_bypass_control_report_v1", "visible_bypass_control_failed": control_report["visible_bypass_control_failed"]})
    write_json(out / "noisy_distractor_control_report.json", {"schema_version": "phase_141a_noisy_distractor_control_report_v1", "noisy_distractor_control_failed": control_report["noisy_distractor_control_failed"]})
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_141a_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {"schema_version": "phase_141a_generated_before_scoring_report_v1", "passed": True, "generated_text_produced_before_scoring": True, "all_helper_requests_allowed_keys_only": all(set(row["helper_request"]) == ALLOWED_HELPER_KEYS for row in main_results + ablation_results + replay), "expected_or_scorer_metadata_in_helper_requests": False}
    write_json(out / "generated_before_scoring_report.json", generated_report)
    write_json(out / "freshness_leakage_audit.json", {"schema_version": "phase_141a_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False})

    field_shortcut_report = {"schema_version": "phase_141a_field_shortcut_report_v1", "field_a_shortcut_rate": main_metrics["field_a_shortcut_rate"], "field_b_shortcut_rate": main_metrics["field_b_shortcut_rate"], "intermediate_copy_shortcut_rate": main_metrics["intermediate_copy_shortcut_rate"], "single_field_shortcut_rate": main_metrics["single_field_shortcut_rate"]}
    priority_report = {"schema_version": "phase_141a_priority_conflict_report_v1", "priority_conflict_wrong_field_rate": main_metrics["priority_conflict_wrong_field_rate"], "priority_conflict_failure_detected": main_metrics["priority_conflict_failure_detected"], "priority_conflict_wrong_field_control_failed": control_report["priority_conflict_wrong_field_control_failed"]}
    single_field_report = {"schema_version": "phase_141a_single_field_shortcut_report_v1", "single_field_shortcut_detected": main_metrics["single_field_shortcut_detected"], "single_field_shortcut_rate": main_metrics["single_field_shortcut_rate"], "single_field_shortcut_control_failed": control_report["single_field_shortcut_control_failed"]}
    ...
    comparison = {
        "schema_version": "phase_141a_arm_comparison_v1",
    ...
    write_json(out / "multi_field_transfer_metrics.json", {"schema_version": "phase_141a_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_141a_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_141a_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    ...
    summary = {"schema_version": "phase_141a_summary_v1", "milestone": MILESTONE, "status": "complete", "boundary": BOUNDARY_TEXT, "upstream_140z": upstream_140z, "upstream_140y": upstream_140y, "metrics": comparison, "selection": selection, "helper_provenance": provenance, "canary_passed": canary["passed"], "ast_shortcut_scan_passed": ast_report["passed"], "generated_before_scoring_passed": generated_report["passed"], "controls_failed": control_report["controls_failed"], **decision}
    ...
    write_json(out / "queue.json", {"schema_version": "phase_141a_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
