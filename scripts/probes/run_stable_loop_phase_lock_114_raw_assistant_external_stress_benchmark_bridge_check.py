#!/usr/bin/env python3
"""Static/artifact checker for STABLE_LOOP_PHASE_LOCK_114."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_114_raw_assistant_external_stress_benchmark_bridge/smoke"
REQUIRED_DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_114_RAW_ASSISTANT_EXTERNAL_STRESS_BENCHMARK_BRIDGE_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_114_RAW_ASSISTANT_EXTERNAL_STRESS_BENCHMARK_BRIDGE_RESULT.md",
]
REQUIRED_SOURCE = [
    "scripts/probes/run_stable_loop_phase_lock_114_raw_assistant_external_stress_benchmark_bridge.py",
    "scripts/probes/run_stable_loop_phase_lock_114_raw_assistant_external_stress_benchmark_bridge_check.py",
]
...
REQUIRED_ARTIFACTS = [
    ...
    "fresh_external_style_benchmark_dataset.jsonl",
    "eval_row_hashes.json",
    "freshness_leakage_audit.json",
    "arm_generation_results.jsonl",
    ...
    "helper_path_separation_report.json",
...
REQUIRED_TERMS = [
    "STABLE_LOOP_PHASE_LOCK_114_RAW_ASSISTANT_EXTERNAL_STRESS_BENCHMARK_BRIDGE",
    "eval-only external-style stress bridge",
    ...
    "CONTROLS_REJECTED",
    "115_EXTERNAL_STYLE_RAW_ASSISTANT_STRESS_CONFIRM",
...
def check_artifacts() -> list[str]:
    ...
    if "RAW_ASSISTANT_EXTERNAL_STRESS_BENCHMARK_BRIDGE_POSITIVE" not in verdicts:
        failures.append("RAW_ASSISTANT_EXTERNAL_STRESS_BENCHMARK_BRIDGE_RESULT_MISSING")
    for key, expected in [
    ...
    config = load_json(SMOKE_ROOT / "benchmark_config.json")
    ...
    arm_hashes = row_hashes.get("arms", {})
    if set(arm_hashes) != ARMS or len({payload.get("eval_row_hash") for payload in arm_hashes.values()}) != 1:
    ...
    if set(arms) != ARMS:
        failures.append("BASELINE_EVAL_MISMATCH")
    main_metrics = arms.get(MAIN_ARM, {}).get("metrics", {})
    baseline_metrics = arms.get("CURRENT_RAW_BASELINE", {}).get("metrics", {})
    if main_metrics.get("external_style_raw_accuracy", 0.0) < 0.85:
        failures.append("RAW_EXTERNAL_STYLE_GATE_FAILED")
    if main_metrics.get("external_style_raw_accuracy", 0.0) < baseline_metrics.get("external_style_raw_accuracy", 1.0) + 0.20:
        failures.append("RAW_EXTERNAL_STYLE_GATE_FAILED")
    if main_metrics.get("per_family_min_accuracy", 0.0) < 0.70:
        failures.append("FAMILY_SPECIFIC_EXTERNAL_STYLE_REGRESSION_DETECTED")
    for key, limit in [("namespace_leak_rate", 0.03), ("teacher_namespace_copy_rate", 0.03), ("case_id_drift_rate", 0.05)]:
        if main_metrics.get(key, 1.0) > limit:
    ...
    helper = load_json(SMOKE_ROOT / "helper_path_separation_report.json")
    if helper.get("helper_metrics_used_for_positive_score") is not False or helper.get("raw_helper_metrics_merged") is not False:
        failures.append("RAW_HELPER_METRICS_MERGED")

    retention = load_json(SMOKE_ROOT / "retention_report.json")
    if retention.get("retention_passed") is not True:
    ...
    if decision.get("next") != "115_EXTERNAL_STYLE_RAW_ASSISTANT_STRESS_CONFIRM":
        failures.append("DECISION_NEXT_MISMATCH")
    if decision.get("helper_metrics_used_for_positive_score") is not False:
        failures.append("HELPER_PATH_USED_FOR_POSITIVE_SCORE")

    rows = read_jsonl(SMOKE_ROOT / "arm_generation_results.jsonl")
    raw_rows = [row for row in rows if row.get("arm") == MAIN_ARM]
    if not raw_rows:
        failures.append("RAW_RESULTS_MISSING")
    for row in raw_rows[:100]:
        for key in [
            ...
            "policy_stage_repair_used_during_eval",
        ]:
...
def main() -> int:
    ...
    if failures:
        print("114 checker failed:")
        for failure in sorted(set(failures)):
            print(f" - {failure}")
        return 1
    print("114 checker passed")
    return 0
