#!/usr/bin/env python3
"""Checker for E93 Agency/Guard commit-safety expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E93_AGENCY_GUARD_COMMIT_SAFETY_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E93_AGENCY_GUARD_COMMIT_SAFETY_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not open-domain model behavior" not in manifest.get("boundary", ""):
    ...
    for family in ["Guard", "T-Stab", "Scribe"]:
        if family not in families:
            failures.append(f"missing family: {family}")
    if task.get("case_count", 0) < 7000:
        failures.append("too few commit-safety cases")
    if task.get("proposal_field_commit_boundary") is not True:
        failures.append("proposal_field_commit_boundary not true")
    if task.get("direct_flow_write_allowed") is not False:
        failures.append("direct Flow write unexpectedly allowed")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e93_agency_guard_commit_safety_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_commit_safety_success_min",
        "adversarial_commit_safety_success_min",
        "validation_trace_validity_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_wrong_commit_max", "adversarial_false_commit_max", "validation_missed_commit_max"]:
    ...
    if not any(row.get("family") == "Guard" and row.get("final_status") == "StableOperatorCandidate" for row in lifecycle_rows):
        failures.append("no stable Guard")
    if not any(row.get("family") == "T-Stab" and row.get("final_status") == "StableOperatorCandidate" for row in lifecycle_rows):
        failures.append("no stable T-Stab")
    if not any(row.get("family") == "Scribe" and row.get("final_status") == "StableOperatorCandidate" for row in lifecycle_rows):
        failures.append("no stable Scribe")
    cf_summary = counterfactual.get("summary", {})
    if not any(values.get("mean_commit_safety_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive commit-safety contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E93_AGENCY_GUARD_COMMIT_SAFETY_EXPANSION_CHECK",
        "root": str(root),
