#!/usr/bin/env python3
"""138R deterministic real-raw reasoning repair training/probe.
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_138R_REAL_RAW_REASONING_REPAIR_TRAINING_PLAN_OR_PROBE"
SHORT_MILESTONE = "138R_REAL_RAW_REASONING_REPAIR_TRAINING_PLAN_OR_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_138r_real_raw_reasoning_repair_training_plan_or_probe/smoke")
DEFAULT_UPSTREAM_137B_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_137b_real_raw_reasoning_repair_plan/smoke")
...
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_138r_real_raw_reasoning_repair_training_plan_or_probe_check.py"
TARGET_CHECKPOINT_REL = "target/pilot_wave/stable_loop_phase_lock_138r_real_raw_reasoning_repair_training_plan_or_probe/smoke/checkpoints/target_138r_reasoning_repair/model.pt"

POSITIVE_VERDICT = "REAL_RAW_REASONING_REPAIR_PROBE_POSITIVE"
NEGATIVE_VERDICT = "REAL_RAW_REASONING_REPAIR_PROBE_FAILS"
MISSING_TRAINER_VERDICT = "REAL_RAW_REASONING_TRAINING_HELPER_MISSING"
DETERMINISM_VERDICT = "DETERMINISM_REPLAY_MISMATCH"
POSITIVE_DECISION = "real_raw_reasoning_repair_probe_positive"
NEGATIVE_DECISION = "real_raw_reasoning_repair_probe_failed"
MISSING_TRAINER_DECISION = "real_raw_reasoning_training_helper_missing"
POSITIVE_NEXT = "139R_REAL_RAW_REASONING_REPAIR_SCALE_CONFIRM"
NEGATIVE_NEXT = "138B_REAL_RAW_REASONING_REPAIR_FAILURE_ANALYSIS"
MISSING_TRAINER_NEXT = "138A_REAL_RAW_REASONING_TRAINING_HELPER_INTEGRATION_PLAN"
...
BOUNDARY_TEXT = (
    "138R is a deterministic targeted real-raw reasoning repair/probe. It may "
...
def resolve_target_out(text: str) -> Path:
    ...
    if path.is_absolute() or any(part == ".." for part in path.parts):
        raise GateError("138R_BOUNDARY_FAILURE", "--out must be repo-relative")
    parts = [part.lower() for part in path.parts]
    if len(parts) < 2 or parts[0] != "target" or parts[1] != "pilot_wave":
        raise GateError("138R_BOUNDARY_FAILURE", "--out must stay under target/pilot_wave")
...
def write_summary(out: Path, status: str, verdicts: list[str], decision: dict[str, Any], failure: str | None = None) -> None:
    write_json(
        out / "summary.json",
        {
            "schema_version": "phase_138r_summary_v1",
            "milestone": MILESTONE,
            "status": status,
            "verdicts": verdicts,
...
def write_report(out: Path, verdicts: list[str], decision: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "138R may partially restore only the reasoning subtrack real-raw evidence if fully positive.",
...
def verify_upstreams(out: Path, root_137b: Path, root_137r: Path, root_136r: Path, root_135e: Path) -> dict[str, Any]:
    required = {
        "137B": [
            "decision.json",
        ...
        "136R": ["decision.json", "raw_generation_helper_provenance.json"],
        "135E": ["decision.json", "raw_generation_helper_provenance.json"],
    }
    roots = {"137B": root_137b, "137R": root_137r, "136R": root_136r, "135E": root_135e}
    ...
    d137b = read_json(root_137b / "decision.json")
    d137r = read_json(root_137r / "decision.json")
    d136r = read_json(root_136r / "decision.json")
    d135e = read_json(root_135e / "decision.json")
    agg137r = read_json(root_137r / "aggregate_metrics.json")
    if d137b.get("decision") != "real_raw_reasoning_repair_plan_complete" or d137b.get("next") != SHORT_MILESTONE:
        raise GateError("UPSTREAM_137B_NOT_COMPLETE", "137B did not route to 138R")
    if d137r.get("decision") != "real_raw_reasoning_not_restored" or d137r.get("verdict") != "REAL_RAW_REASONING_REBUILD_FAILS":
        raise GateError("UPSTREAM_137R_NOT_CLEAN_NEGATIVE", "137R is not the required clean negative")
    if agg137r.get("mean_real_raw_reasoning_accuracy") != 0.0:
        raise GateError("UPSTREAM_137R_NOT_CLEAN_NEGATIVE", "137R mean reasoning accuracy is not 0.0")
    for key in ["expected_output_canary_passed", "ast_shortcut_scan_passed", "controls_failed", "leakage_rejected", "checkpoint_hash_unchanged"]:
        if d137r.get(key) is not True:
            raise GateError("UPSTREAM_137R_ARTIFACT_INCOMPLETE", f"137R missing clean helper gate: {key}")
    if d136r.get("verdict") != "REAL_RAW_CORE_CAPABILITY_MINIMAL_REBUILD_POSITIVE":
        raise GateError("UPSTREAM_136R_NOT_POSITIVE", "136R not positive")
    if d135e.get("verdict") != "SHARED_RAW_GENERATION_HELPER_AND_CANARY_GATE_POSITIVE":
        raise GateError("UPSTREAM_135E_NOT_POSITIVE", "135E not positive")

    manifest_137b = {
        "schema_version": "phase_138r_upstream_137b_manifest_v1",
        "upstream_137b_root": rel(root_137b),
        "upstream_137b_verified": True,
        "decision": d137b.get("decision"),
        "next": d137b.get("next"),
        "primary_diagnosis": d137b.get("primary_diagnosis"),
        "checkpoint_capability_gap_likelihood": d137b.get("checkpoint_capability_gap_likelihood"),
    }
    manifest_137r = {
        "schema_version": "phase_138r_upstream_137r_manifest_v1",
        "upstream_137r_root": rel(root_137r),
        "upstream_137r_clean_negative_verified": True,
        "decision": d137r.get("decision"),
        "verdict": d137r.get("verdict"),
        "mean_real_raw_reasoning_accuracy": agg137r.get("mean_real_raw_reasoning_accuracy"),
        "canary_passed": d137r.get("expected_output_canary_passed"),
        "ast_scan_passed": d137r.get("ast_shortcut_scan_passed"),
        "controls_failed": d137r.get("controls_failed"),
        "leakage_rejected": d137r.get("leakage_rejected"),
        "checkpoint_hash_unchanged": d137r.get("checkpoint_hash_unchanged"),
        "no_expected_or_scorer_metadata_reached_generation": True,
    }
    manifest_136r = {
        "schema_version": "phase_138r_upstream_136r_manifest_v1",
        ...
        "decision_136r": d136r.get("decision"),
        "verdict_136r": d136r.get("verdict"),
        "decision_135e": d135e.get("decision"),
        "verdict_135e": d135e.get("verdict"),
    }
    write_json(out / "upstream_137b_manifest.json", manifest_137b)
    write_json(out / "upstream_137r_manifest.json", manifest_137r)
    write_json(out / "upstream_136r_manifest.json", manifest_136r)
    write_json(
        out / "upstream_135e_manifest.json",
        {
            "schema_version": "phase_138r_upstream_135e_manifest_v1",
            "upstream_135e_root": rel(root_135e),
            "upstream_135e_verified": True,
            "decision": d135e.get("decision"),
            "verdict": d135e.get("verdict"),
        },
    )
    return {"137b": manifest_137b, "137r": manifest_137r, "136r": manifest_136r}
...
def deterministic_setup(seed: int) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138r_determinism_manifest_v1",
...
def import_backend_from_helper(helper: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if selected.get("backend_name") != "byte_gru_lm":
        raise GateError(MISSING_TRAINER_VERDICT, "138R supports only the strict byte-GRU backend for training", selected)
...
def answer_value(category: str, split: str, index: int, rng: random.Random) -> tuple[str, str, str]:
    ...
    answer_prefix = "E" if split == "eval" else "T"
    ...
    if category.endswith("RULE_CHAINING"):
        ...
        left_map = {"red": 2, "blue": 3, "green": 5}[left]
        right_map = {"circle": 7, "square": 11, "triangle": 13}[right]
    ...
    if category.endswith("TABLE_RULE_REASONING"):
        ...
        y = rng.randrange(40, 120)
        z = rng.randrange(40, 120)
        result = max(x, y, z)
    ...
    if category.endswith("SMALL_ARITHMETIC"):
        a = rng.randrange(100, 400)
        b = rng.randrange(100, 400)
    ...
    if category.endswith("HALLUCINATION_TRAP"):
        return f"Hallucination trap repair row {namespace}-{index}. No serial number is supplied. Split prefix is {answer_prefix}. If the needed value is absent, return the prefixed insufficient marker in ANSWER format.", f"ANSWER={answer_prefix}INSUFFICIENT", "required"
...
def build_train_rows(train_examples: int, depths: list[int]) -> list[dict[str, Any]]:
    ...
    for index in range(train_examples):
        ...
        rows.append(
            {
                "row_id": f"138r_train_{index:06d}",
                "split": "train",
                "family": category,
...
def build_eval_rows(seeds: list[int], rows_per_family: int, depths: list[int]) -> list[dict[str, Any]]:
    ...
    for family_index, family in enumerate(FAMILIES):
        for local_index in range(rows_per_family):
            ...
            rows.append(
                {
                    "row_id": f"138r_eval_{global_index:05d}",
                    "split": "eval",
                    "family": family,
...
def dataset_manifest(rows: list[dict[str, Any]], split: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": f"phase_138r_{split}_dataset_manifest_v1",
        "split": split,
        "row_count": len(rows),
    ...
    for eval_row in eval_rows:
        for train_row in sample_train:
            ...
            if score >= 0.90:
                ...
                break
    return {
        "schema_version": "phase_138r_freshness_leakage_audit_v1",
...
def train_target_model(model: Any, rows: list[dict[str, Any]], seq_len: int, args: argparse.Namespace, out: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for step in range(1, steps + 1):
        ...
        if step == 1 or step % max(1, args.metrics_interval) == 0 or step == steps or (time.monotonic() - last_flush) >= args.heartbeat_sec:
            item = {"step": step, "train_loss": last_loss, "optimizer_step_count": step, "train_step_count": step}
            metrics.append(item)
    ...
    report = {
        "schema_version": "phase_138r_training_report_v1",
        ...
        "objective": "supervised direct-answer next-byte training on response bytes plus helper-only final rollout eval",
    }
...
def forbidden_input_tests(helper: Any, selected: dict[str, Any]) -> dict[str, Any]:
    base = {
        "prompt": "138R forbidden metadata rejection smoke.",
    ...
    return {
        "schema_version": "phase_138r_forbidden_input_rejection_report_v1",
        "rows": rows,
...
def expected_output_canary(helper: Any, selected: dict[str, Any], max_new_tokens: int) -> dict[str, Any]:
    original_row = {
        "row_id": "138r_canary_original",
        "prompt": "138R canary prompt: produce any raw continuation.",
        "expected_output": "correct A",
        "expected_payload": {"value": "correct A"},
    }
    shadow_row = dict(original_row)
    shadow_row["row_id"] = "138r_canary_shadow"
    ...
    return {
        "schema_version": "phase_138r_expected_output_canary_report_v1",
...
def ast_shortcut_scan(paths: list[Path]) -> dict[str, Any]:
    ...
    for path in paths:
        ...
        Scanner().visit(tree)
    return {
        "schema_version": "phase_138r_ast_shortcut_scan_report_v1",
...
def score_text(generated: str, row: dict[str, Any]) -> tuple[bool, str]:
    ...
    if "User:" in text or "Assistant:" in text:
        return False, "stale_chat_fragment_present"
        ...
        scoring.append(
            {
                ...
                "stale_chat_fragment_present": bool(re.search(r"\b(User|Assistant):", generated_text)),
                "off_prompt_output": off_prompt(generated_text),
            }
        )
...
def scorer_controls(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "schema_version": "phase_138r_control_arm_report_v1",
...
def compute_metrics(scoring: list[dict[str, Any]], seeds: list[int]) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    ...
    aggregate = {
        "schema_version": "phase_138r_aggregate_metrics_v1",
        "row_count": row_count,
    ...
    aggregate["positive_reasoning_repair_gates_passed"] = (
        aggregate["all_seeds_passed_independently"]
        and aggregate["mean_real_raw_reasoning_accuracy"] >= AGGREGATE_GATES["mean_real_raw_reasoning_accuracy"]
        and aggregate["expected_token_inclusion_rate"] >= AGGREGATE_GATES["expected_token_inclusion_rate"]
        and aggregate["near_match_rate"] >= AGGREGATE_GATES["near_match_rate"]
    )
...
def generated_before_scoring_report(traces: list[dict[str, Any]], scoring: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138r_generated_before_scoring_report_v1",
...
def replay_report(first: dict[str, Any], replay: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138r_determinism_replay_report_v1",
...
def helper_provenance(selected: dict[str, Any], checkpoint_path: Path, helper: Any, traces: list[dict[str, Any]], source_hash_before: str, source_hash_after: str, target_hash_before: str | None, target_hash_after: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138r_helper_provenance_verification_v1",
...
def write_row_hashes(out: Path, eval_rows: list[dict[str, Any]]) -> None:
    write_json(
        ...
        {
            "schema_version": "phase_138r_eval_row_hashes_v1",
...
def write_failure_decision(args: argparse.Namespace, error: GateError) -> None:
    ...
    decision = {
        "schema_version": "phase_138r_failure_decision_v1",
        "decision": route[0],
        "next": route[1],
        "verdict": route[2],
...
def decide(
    ...
    if hard_positive:
        ...
        verdict = NEGATIVE_VERDICT
    elif training["training_loss_improved"] and aggregate["mean_real_raw_reasoning_accuracy"] <= 0.0:
        ...
        verdict = NEGATIVE_VERDICT
    decision = {
        "schema_version": "phase_138r_decision_v1",
...
def run(args: argparse.Namespace) -> None:
    ...
    write_json(out / "queue.json", {"schema_version": "phase_138r_queue_v1", "milestone": MILESTONE, "status": "started", "started_at": utc_now(), "heartbeat_sec": args.heartbeat_sec})
    append_progress(out, "startup", heartbeat_sec=args.heartbeat_sec)
    refresh_status(out, "running", ["REAL_RAW_REASONING_REPAIR_PROBE_RUNNING"], {"decision": "pending", "next": "pending"})

    upstreams = verify_upstreams(out, resolve_path(args.upstream_137b_root), resolve_path(args.upstream_137r_root), resolve_path(args.upstream_136r_root), resolve_path(args.upstream_135e_root))
    ...
    train_config = {
        "schema_version": "phase_138r_train_config_v1",
        "seeds": seeds,
        ...
        "training_objective": "direct-answer response-byte next-token repair plus helper-only final rollout eval",
        "source_checkpoint_immutable": True,
        "old_runners_imported": False,
    }
    eval_config = {
        "schema_version": "phase_138r_eval_config_v1",
        "seeds": seeds,
    ...
    write_json(
        ...
        {
            "schema_version": "phase_138r_source_checkpoint_integrity_manifest_v1",
            "source_checkpoint_path": rel(source_checkpoint),
            "source_checkpoint_hash_before": source_hash_before,
    ...
    if source_hash_before != source_hash_after:
        raise GateError("CHECKPOINT_MUTATION_DETECTED", "source checkpoint changed during 138R")
    ...
    write_json(
        ...
        {
            "schema_version": "phase_138r_target_checkpoint_integrity_manifest_v1",
    ...
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_138r_per_family_metrics_v1", "families": family_metrics})
    ...
    evidence = {
        "schema_version": "phase_138r_evidence_rebuild_status_v1",
    ...
    write_json(out / "queue.json", {"schema_version": "phase_138r_queue_v1", "milestone": MILESTONE, "status": "completed", "completed_at": utc_now(), "heartbeat_sec": args.heartbeat_sec})


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--out", default=str(DEFAULT_OUT))
    parser.add_argument("--upstream-137b-root", default=str(DEFAULT_UPSTREAM_137B_ROOT))
    ...
    parser.add_argument("--seeds", default="2281,2282,2283")
...
def main() -> int:
    ...
    try:
        ...
        print(f"138R failed closed: {exc.verdict}: {exc.message}", file=sys.stderr)
        return 1 if exc.verdict in {"138R_BOUNDARY_FAILURE", "CHECKPOINT_MUTATION_DETECTED"} else 0
