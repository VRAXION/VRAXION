#!/usr/bin/env python3
"""Checker for E98 multi-skill execution curriculum expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E98_MULTI_SKILL_EXECUTION_CURRICULUM_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E98_MULTI_SKILL_EXECUTION_CURRICULUM_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not open-domain reasoning" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few composition cases")
    if task.get("multi_skill_composition") is not True:
        failures.append("multi_skill_composition not true")
    if task.get("open_domain_reasoning") is not False:
        failures.append("open-domain reasoning unexpectedly enabled")
    if task.get("direct_answer_shortcut_allowed") is not False:
        failures.append("direct answer shortcut unexpectedly allowed")
    if task.get("requires_trace_join") is not True:
        failures.append("trace-join requirement missing")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e98_multi_skill_execution_curriculum_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_composition_success_min",
        "adversarial_composition_success_min",
        "validation_dependency_order_accuracy_min",
        "validation_trace_join_validity_min",
        "validation_intermediate_carry_validity_min",
        "validation_checkpoint_accuracy_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in [
        "adversarial_scope_bleed_rate_max",
        "adversarial_dropped_intermediate_rate_max",
        "adversarial_unordered_execution_rate_max",
        "adversarial_unsafe_answer_rate_max",
    ]:
    ...
    if not any(values.get("mean_composition_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive composition contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E98_MULTI_SKILL_EXECUTION_CURRICULUM_EXPANSION_CHECK",
        "root": str(root),
