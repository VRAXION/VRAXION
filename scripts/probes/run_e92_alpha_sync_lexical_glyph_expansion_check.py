#!/usr/bin/env python3
"""Checker for E92 alpha-Sync lexical/glyph expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E92_ALPHA_SYNC_LEXICAL_GLYPH_EXPANSION":
    ...
    for family in ["alpha_syncer", "Guard", "Scribe"]:
        if family not in families:
            failures.append(f"missing family: {family}")
    if task.get("case_count", 0) < 7000:
        failures.append("too few lexical/glyph cases")
    if task.get("visible_evidence_only") is not True:
        failures.append("visible_evidence_only not true")
    if task.get("semantic_lane_labels_used") is not False:
        failures.append("semantic lane labels unexpectedly used")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e92_alpha_sync_lexical_glyph_expansion_confirmed":
    ...
    if not any(row.get("family") == "alpha_syncer" and row.get("final_status") == "StableOperatorCandidate" for row in lifecycle_rows):
        failures.append("no stable alpha_syncer")
    ...
    if not any(values.get("mean_resolution_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E92_ALPHA_SYNC_LEXICAL_GLYPH_EXPANSION_CHECK",
        "root": str(root),
