...
REQ_SAMPLE = [
    ...
    "adapter_growth_report_sample.json",
    "row_level_sample.jsonl",
    "adapter_curve_sample.jsonl",
...
def recompute(rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    ...
    for system in sorted({row["system"] for row in rows}):
        ...
        out[system] = {
            ...
            "adapter_success": mean([1.0 if row["adapter_success"] else 0.0 for row in chunk]),
            "heldout_success": split_metric(chunk, "heldout", "adapter_success"),
            "ood_success": split_metric(chunk, "ood", "adapter_success"),
            "counterfactual_success": split_metric(chunk, "counterfactual", "adapter_success"),
            "adversarial_success": split_metric(chunk, "adversarial", "adapter_success"),
            "old_intent_success": mean([1.0 if row["adapter_success"] else 0.0 for row in old_rows]),
            "bit_accuracy": mean([float(row["bit_accuracy"]) for row in chunk]),
            ...
            "source_to_target_transform_accuracy": mean([float(row["bit_accuracy"]) for row in chunk]),
        }
    return out
...
def validate_sample(sample_dir: Path) -> dict[str, Any]:
    ...
    curve = read_jsonl(sample_dir / "adapter_curve_sample.jsonl")
    if schema.get("milestone") != MILESTONE or schema.get("edge_adapter_pocket") is not True:
        failures.append("sample schema missing E47 marker")
...
def validate_target(out: Path, sample_dir: Path, write_summary: bool) -> dict[str, Any]:
    ...
    failures.extend(static_policy_check(Path("scripts/probes/run_e47_edge_adapter_pocket_growth_probe.py")))
    ...
    reports = read_json(out / "adapter_growth_report.json")
    rows = read_jsonl(out / "row_level_results.jsonl")
    curve = read_jsonl(out / "adapter_curve.jsonl")
    ...
    raw_direct = system_results["raw_wire_direct_fixed16"]["overall"]
    raw_prog = system_results["raw_wire_progressive_plus1"]["overall"]
    plus1 = system_results["edge_adapter_progressive_plus1_no_freeze"]["overall"]
    plus1_report = reports["edge_adapter_progressive_plus1_no_freeze"]
    block = system_results["edge_adapter_block_growth_plus4"]["overall"]
    block_report = reports["edge_adapter_block_growth_plus4"]
    if aggregate.get("decision") in {"e47_edge_adapter_pocket_positive", "e47_block_adapter_growth_preferred"}:
        if pass_gate(raw_direct, reports["raw_wire_direct_fixed16"]) or pass_gate(raw_prog, reports["raw_wire_progressive_plus1"]):
            failures.append("adapter-positive decision but raw wire also passes")
        if not pass_gate(plus1, plus1_report):
            failures.append("adapter-positive decision without progressive adapter pass")
        if plus1_report.get("growth_events", 0) < 3:
            failures.append("progressive adapter did not grow by +1 across three events")
    if aggregate.get("decision") == "e47_block_adapter_growth_preferred" and not pass_gate(block, block_report):
        failures.append("block-preferred decision without block adapter pass")
