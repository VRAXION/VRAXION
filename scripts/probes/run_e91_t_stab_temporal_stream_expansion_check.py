#!/usr/bin/env python3
"""Checker for E91 T-Stab temporal stream expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E91_T_STAB_TEMPORAL_STREAM_EXPANSION":
    ...
    for family in ["T-Stab", "Guard", "Lens", "Scribe"]:
        if family not in families:
            failures.append(f"missing family: {family}")
    if task.get("case_count", 0) < 7000:
        failures.append("too few cases")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e91_t_stab_temporal_stream_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in ["validation_stabilization_success_min", "adversarial_stabilization_success_min"]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_wrong_confident_max", "validation_false_hold_max", "adversarial_false_commit_max"]:
    ...
    if summary.get("unsafe_final_selected") != 0:
        failures.append("unsafe selected")
    required_stable = {
        "frame_sequence_t_stab",
        "crc_parity_frame_guard",
        "bit_slip_resync_t_stab",
        "repeat_vote_stabilizer_t_stab",
        "stale_replay_guard",
        "source_trust_guard",
        "delayed_evidence_buffer_lens",
        "temporal_commit_scribe",
    }
    stable_top = set(frequency.get("stable_top", []))
    if required_stable - stable_top:
        failures.append(f"missing stable operators: {sorted(required_stable - stable_top)}")
    ...
    if not any(values.get("mean_stabilization_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive stabilization contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E91_T_STAB_TEMPORAL_STREAM_EXPANSION_CHECK",
        "root": str(root),
