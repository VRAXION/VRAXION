#!/usr/bin/env python3
"""Checker for E95 active evidence request/search expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E95_ACTIVE_EVIDENCE_REQUEST_SEARCH_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E95_ACTIVE_EVIDENCE_REQUEST_SEARCH_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not open-domain retrieval" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few active-evidence cases")
    if task.get("active_evidence_selection") is not True:
        failures.append("active_evidence_selection not true")
    if task.get("open_domain_retrieval") is not False:
        failures.append("open-domain retrieval unexpectedly enabled")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e95_active_evidence_request_search_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_evidence_action_success_min",
        "adversarial_evidence_action_success_min",
        "validation_targeted_request_accuracy_min",
        "validation_answer_ready_accuracy_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_wrong_confident_max", "adversarial_false_search_max", "adversarial_over_budget_max"]:
    ...
    if not any(values.get("mean_evidence_action_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive active-evidence contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E95_ACTIVE_EVIDENCE_REQUEST_SEARCH_EXPANSION_CHECK",
        "root": str(root),
