#!/usr/bin/env python3
"""Validate D114 symbolic sequence bridge prototype artifacts."""
...
def check(out: Path) -> None:
    ...
    manifest = read_json(out / "d113_upstream_manifest.json")
    scale, m, gates = aggregate["scale"], aggregate["metrics"], aggregate["positive_gates"]
    require(decision["decision"] == "d114_symbolic_sequence_bridge_prototype_confirmed", "unexpected decision")
    require(decision["next"] == "D115_SYMBOLIC_SEQUENCE_BRIDGE_SCALE_CONFIRM_WITH_TRIG_GUARDRAILS", "unexpected next")
    require(decision["d115_ready"] is True and m["d115_ready"] is True, "D115 readiness missing")
    require(all(gates.values()), f"failed positive gates: {[k for k, v in gates.items() if not v]}")
    require(manifest["validation_status"] == "valid" and manifest["replayed_decision"] == "d113_symbolic_sequence_bridge_plan_ready", "D113 replay invalid")
    require(manifest["replayed_next"] == "D114_SYMBOLIC_SEQUENCE_BRIDGE_PROTOTYPE_WITH_TRIG_GUARDRAILS" and manifest["replayed_d114_ready"] is True, "D113 next/readiness invalid")
    require(manifest["replayed_bridge_family_count"] == 6 and manifest["replayed_bridge_ready_family_count"] == 2 and manifest["replayed_bridge_guarded_family_count"] == 3 and manifest["replayed_bridge_rejected_family_count"] == 1, "D113 bridge replay mismatch")
    require(manifest["replayed_trig_remains_repair_only"] is True and manifest["replayed_language_like_symbolic_command_is_natural_language"] is False and manifest["replayed_failed_jobs"] == [], "D113 guardrail replay mismatch")
    require(m["d113_replay_validation_passed"] is True and m["d113_replay_decision"] == "d113_symbolic_sequence_bridge_plan_ready", "D113 metric replay invalid")
    require(scale["requested_total_rows"] == scale["actual_total_rows"] and scale["scale_reduced"] is False, "scale reduced")
    require(scale["requested_total_rows"] == 154680 and scale["stress_mode_count"] == 33 and scale["all_required_stress_modes_executed"] is True, "scale/stress mismatch")
    require(scale["failed_jobs"] == [] and scale["fallback_rows"] == 0, "scale fallback/failure")
    for key in ["natural_language_pretraining_executed", "tokenizer_introduced", "next_token_objective_defined", "raw_text_corpus_used", "full_bridge_training_executed", "language_like_symbolic_command_is_natural_language", "raw_raven_used"]:
    ...
    require(set(m["trainable_adapter_names"]) == {"route_head_adapter_bridge_delta", "halting_head_adapter_bridge_delta", "recurrent_state_adapter_bridge_delta", "calibration_scalar_adapter_bridge_delta"}, "trainable adapters mismatch")
    require(m["bridge_training_executed"] is True and m["training_updates_executed"] is True and m["total_bridge_steps_executed"] > 0 and 1 <= m["epochs_executed"] <= 3, "training execution mismatch")
    require(m["checkpoint_count"] >= 8 and m["failed_checkpoint_count"] == 0 and m["rollback_triggered"] is False and m["final_candidate_selected"] is True, "checkpoint/rollback mismatch")
    ...
    require(m["multi_step_instruction_in_healthy_claim"] is False, "held family entered healthy claim")
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("target/pilot_wave/d114_symbolic_sequence_bridge_prototype_with_trig_guardrails"))
