#!/usr/bin/env python3
"""Checker for E125 broad text-understanding candidate expansion wave."""
...
CONTRACT = "E125_BROAD_TEXT_UNDERSTANDING_CANDIDATE_EXPANSION_WAVE"
ALLOWED_DECISIONS = {
...
def check(root: Path) -> list[str]:
    ...
    if aggregate.get("target_gold_count") != 15:
        failures.append("target Gold count mismatch")
    if aggregate.get("supply_gap_to_15") != max(0, 15 - aggregate.get("promoted_to_gold_count", 0)):
        failures.append("supply gap mismatch")
    if aggregate.get("promoted_to_gold_count", 0) < 1:
        failures.append("no Gold promotions for broad candidate wave")
    ...
    if aggregate.get("promoted_to_gold_count", 0) >= 15 and decision.get("decision") != "e125_broad_text_understanding_15plus_gold_positive":
        failures.append("15+ Gold result did not get positive decision")
    if 0 < aggregate.get("promoted_to_gold_count", 0) < 15 and decision.get("decision") != "e125_broad_text_candidate_supply_limited":
        failures.append("sub-15 clean result did not get supply-limited decision")
