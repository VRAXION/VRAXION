#!/usr/bin/env python3
"""Checker for E7L spawn/repair cost and noisy-health falsification."""
...
RUNNER = "scripts/probes/run_e7l_spawn_repair_cost_and_noisy_health_falsification.py"
CHECKER = "scripts/probes/run_e7l_spawn_repair_cost_and_noisy_health_falsification_check.py"
SYSTEMS = (
    "no_adaptation",
    "route_around_only",
    "repair_only",
...
REQUIRED_MEAN_METRICS = (
    ...
    "heldout_repair_precision",
    "heldout_repair_recall",
    "heldout_repair_gain_per_cost",
    "heldout_route_around_success",
    "heldout_health_false_positive_rate",
    "heldout_health_false_negative_rate",
...
def static_scan() -> list[str]:
    ...
    for token in (
        "E7L_SPAWN_REPAIR_COST_AND_NOISY_HEALTH_FALSIFICATION",
        "net_utility_formula",
        "route_around_only",
        "cost_aware_spawn_plus_repair",
        "noisy_health_spawn_plus_repair",
        ...
        "route-around, repair, spawn, and no-op",
    ):
        if token not in text:
            failures.append(f"TOKEN_MISSING_IN_RUNNER:{token}")
    tree = ast.parse(text)
    mutation_functions = {"mutation_worker", "mutate_candidate", "candidate_learning_score", "bootstrap_candidates", "quick_eval"}
...
def check_artifacts(out: Path) -> list[str]:
    ...
    stress = load_json(out / "cost_noise_stress_report.json")
    action = load_json(out / "action_policy_report.json")
    policy = load_json(out / "spawn_repair_policy_report.json")
    health = load_json(out / "health_signal_report.json")
    delayed = load_json(out / "delayed_feedback_report.json")
    ...
    if manifest.get("milestone") != "E7L_SPAWN_REPAIR_COST_AND_NOISY_HEALTH_FALSIFICATION":
    ...
    if manifest.get("source_milestone") != "E7K_DYNAMIC_POCKET_SPAWN_AND_PROMOTION_PROBE":
    ...
    for condition in (
        ...
        if condition not in task.get("stress_conditions", []) or condition not in stress.get("stress_conditions", []):
            failures.append(f"STRESS_CONDITION_MISSING:{condition}")
    ...
    if "raw_usefulness - spawn_cost" not in stress.get("net_utility_formula", ""):
        failures.append("NET_UTILITY_FORMULA_MISSING")
    if action.get("action_policy_winner") != aggregate.get("best_non_oracle_system"):
        failures.append("ACTION_POLICY_WINNER_MISMATCH")
    if policy.get("schema_version") != "e7l_spawn_repair_policy_report_v1":
        failures.append("SPAWN_REPAIR_POLICY_REPORT_BAD")
    if health.get("schema_version") != "e7l_health_signal_report_v1":
        failures.append("HEALTH_SIGNAL_REPORT_BAD")
    if delayed.get("schema_version") != "e7l_delayed_feedback_report_v1":
        failures.append("DELAYED_FEEDBACK_REPORT_BAD")
    ...
    if aggregate.get("best_non_oracle_system") not in SYSTEMS or aggregate.get("best_non_oracle_system") == "oracle_health_spawn_repair_reference":
        ...
        for phase in PHASES:
            ...
            for split in SPLITS:
                ...
                for key in (
                    ...
                    "repair_precision",
                    "repair_recall",
                    "repair_gain_per_cost",
                    "route_around_success",
                    "health_false_positive_rate",
                    "health_false_negative_rate",
    ...
    for row in mutation_rows:
        ...
        if "promoted_pocket_count" not in row.get("final_candidate_summary", {}):
            failures.append(f"CANDIDATE_SUMMARY_MISSING:{label}")
        if row.get("failed_action_rollback_count", 0) < 1:
            failures.append(f"FAILED_ACTION_ROLLBACK_MISSING:{label}")
    ...
    if manifest.get("hardware_identity", {}).get("gpu", {}).get("cuda_available") is True:
        if not any(row.get("device") == "cuda" for row in training_rows):
            failures.append("CUDA_AVAILABLE_BUT_NOT_USED_BY_GRADIENT_HISTORY")
    ...
    if leakage.get("random_spawn_repair_control_passed") is not True and decision.get("decision") != "e7l_leak_or_artifact_detected":
        failures.append("RANDOM_CONTROL_FAILED_WITHOUT_LEAK_DECISION")
...
def main() -> int:
    ...
    parser.add_argument("--out", default="target/pilot_wave/e7l_spawn_repair_cost_and_noisy_health_falsification")
    args = parser.parse_args()
    out = resolve_out(args.out)
    failures = static_scan()
    failures.extend(check_artifacts(out))
    report = {"schema_version": "e7l_checker_report_v1", "failure_count": len(failures), "failures": failures, "checked_root": out.relative_to(REPO_ROOT).as_posix()}
