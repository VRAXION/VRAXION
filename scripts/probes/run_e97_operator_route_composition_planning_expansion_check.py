#!/usr/bin/env python3
"""Checker for E97 Operator route/composition planning expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E97_OPERATOR_ROUTE_COMPOSITION_PLANNING_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E97_OPERATOR_ROUTE_COMPOSITION_PLANNING_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not open-domain planning" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few route planning cases")
    if task.get("operator_route_planning") is not True:
        failures.append("operator_route_planning not true")
    if task.get("open_domain_planning") is not False:
        failures.append("open-domain planning unexpectedly enabled")
    if task.get("full_library_scan_allowed") is not False:
        failures.append("full-library scan unexpectedly allowed")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e97_operator_route_composition_planning_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_route_success_min",
        "adversarial_route_success_min",
        "validation_active_set_precision_min",
        "validation_sequence_accuracy_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_loop_rate_max", "adversarial_overcall_rate_max", "adversarial_over_budget_max", "adversarial_adapter_miss_max"]:
    ...
    if not any(values.get("mean_route_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive route contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E97_OPERATOR_ROUTE_COMPOSITION_PLANNING_EXPANSION_CHECK",
        "root": str(root),
