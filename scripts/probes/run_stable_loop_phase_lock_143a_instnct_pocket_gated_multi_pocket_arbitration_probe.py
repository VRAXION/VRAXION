#!/usr/bin/env python3
"""143A helper-only multi-pocket arbitration probe."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_143A_INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_143a_instnct_pocket_gated_multi_pocket_arbitration_probe/smoke")
DEFAULT_142Z_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_142z_instnct_pocket_gated_conflict_priority_next_decision_plan/smoke")
...
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_143a_instnct_pocket_gated_multi_pocket_arbitration_probe_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_multi_pocket_arbitration_main"
ABLATION_ARM = "instnct_multi_pocket_arbitration_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_multi_pocket_arbitration_all_markers"
POSITIVE_NEXT = "143F_INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_SCALE_CONFIRM"
...
ARBITRATION_INVERSION_PAIR_COUNT_FLOOR = 200
BOUNDARY_TEXT = (
    "143A tests constrained helper/backend multi-pocket arbitration final selection only. "
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_142F", HELPER_PATH)
...
def require_142z(root: Path) -> dict[str, Any]:
    required = ["decision.json", "target_143a_milestone_plan.json", "multi_pocket_arbitration_requirements.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 142Z artifacts: {missing}")
    decision = read_json(root / "decision.json")
    target = read_json(root / "target_143a_milestone_plan.json")
    reqs = read_json(root / "multi_pocket_arbitration_requirements.json")
    if decision.get("decision") != "multi_pocket_arbitration_probe_recommended":
        raise RuntimeError(f"bad 142Z decision: {decision.get('decision')}")
    if decision.get("next") != "143A_INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_PROBE":
        raise RuntimeError(f"bad 142Z next: {decision.get('next')}")
    if target.get("milestone") != "143A_INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_PROBE":
        raise RuntimeError("142Z target 143A plan missing expected milestone")
    return {
        "root": rel(root),
        ...
        "positive_gates": reqs.get("positive_gates"),
        "required_metrics": reqs.get("required_metrics"),
    }


def require_142f(root: Path) -> dict[str, Any]:
    required = [
        "decision.json",
    ...
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 142F artifacts: {missing}")
    decision = read_json(root / "decision.json")
    ...
    winner_gate = read_json(root / "per_winner_gate_report.json")
    ...
    if decision.get("decision") != "instnct_pocket_gated_conflict_priority_transfer_scale_confirmed":
        raise RuntimeError(f"bad 142F decision: {decision.get('decision')}")
    if decision.get("next") != "142Z_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_NEXT_DECISION_PLAN":
        raise RuntimeError(f"bad 142F next: {decision.get('next')}")
    exact = {
    ...
    for key, expected in exact.items():
        if metric_value(metrics, key, "pocket_ablation_delta", "direct_POCKET_VALUE_rate") != expected:
            raise RuntimeError(f"bad 142F metric {key}: {metrics.get(key)} != {expected}")
    if metrics.get("deterministic_replay_passed") is not True:
        raise RuntimeError("142F deterministic replay did not pass")
    required_audit = {
    ...
    for key, expected in required_audit.items():
        if audit.get(key) != expected:
            raise RuntimeError(f"bad 142F helper audit {key}: {audit.get(key)} != {expected}")
    if seed_gate.get("passed") is not True or family_gate.get("passed") is not True or winner_gate.get("passed") is not True:
        raise RuntimeError("142F per-seed/per-family/per-winner hardened gates did not pass")
    if distribution.get("passed") is not True or shortcuts.get("passed") is not True:
        raise RuntimeError("142F distribution or shortcut reports did not pass")
    return {
        "root": rel(root),
        "decision": decision.get("decision"),
        ...
        "priority_rule_accuracy": metrics.get("priority_rule_accuracy"),
        "conflict_resolution_accuracy": metrics.get("conflict_resolution_accuracy"),
        "priority_inversion_accuracy": metrics.get("priority_inversion_accuracy"),
        "same_template_opposite_winner_accuracy": metrics.get("same_template_opposite_winner_accuracy"),
        ...
        "priority_inversion_pair_count": metrics.get("priority_inversion_pair_count"),
        ...
        "per_winner_gate_passed": winner_gate.get("passed"),
        "winner_distribution_passed": distribution.get("passed"),
    }
...
def scan_ast() -> dict[str, Any]:
    ...
    return {"schema_version": "phase_142F_ast_scan_v1", "passed": not failures, "failures": failures}
...
def build_manifest(out: Path, candidate: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    manifest = {
        "schema_version": "instnct_mutation_graph_manifest_v13_multi_pocket_arbitration",
        ...
        "closed_pocket_fallback_value": "SYM_ARBITRATION_CLOSED",
        "fallback_value": "SYM_ARBITRATION_CLOSED",
        "allow_train_namespace_value_fallback": False,
        "decoder": {"type": "deterministic_pocket_gated_multi_pocket_arbitration_decoder", "post_generation_repair": False, "oracle_metadata_allowed": False},
        "pockets": [
            {"pocket_id": "p_multi_pocket_arbitration_final", "gate_marker": candidate.get("gate_marker", NATURAL_GATE), "payload_markers": markers}
        ],
        "claim_boundary": "multi-pocket arbitration helper semantics; not open-ended reasoning",
...
def candidate_specs() -> list[dict[str, Any]]:
    return [
        {"candidate": "closed_pocket_no_arbitration", "gate_marker": "multi-pocket arbitration authorization: sealed"},
        {"candidate": "first_pocket_candidate", "gate_marker": NATURAL_GATE, "payload_markers": [POCKET_A_MARKER]},
        {"candidate": "last_pocket_candidate", "gate_marker": NATURAL_GATE, "payload_markers": [POCKET_C_MARKER]},
        {"candidate": "default_pocket_candidate", "gate_marker": NATURAL_GATE, "payload_markers": [POCKET_B_MARKER]},
        {"candidate": "stale_pocket_candidate", "gate_marker": NATURAL_GATE, "payload_markers": [STALE_MARKER]},
        ...
        {"candidate": "resolved_final_marker_echo_candidate", "gate_marker": NATURAL_GATE, "payload_markers": FINAL_MARKERS},
...
def values_for(seed: int, family_index: int, group_index: int, slot: int) -> dict[str, Any]:
    ...
    return {
        ...
        "resolved_final_marker_value": f"EVMPECHO{base}",
    }
...
def prompt_for_row(row: dict[str, Any]) -> str:
    family_text = {
        ...
        "QUORUM_TWO_OF_THREE": "Two matching pocket votes form quorum; output the quorum value.",
        ...
        "SAME_TEMPLATE_ARBITRATION_INVERSION": "Same template, but arbitration winner flips across paired rows.",
    ...
    return (
        ...
        f"{noise}\n"
        f"arbitration rule id: {row['arbitration_rule_id']} winner={row['selected_pocket_id']}\n"
        ...
        f"{NATURAL_GATE}\n"
        f"{row['final_payload_marker']} {row['resolved_final_marker_value']}\n"
        "Return only the arbitrated final."
    )
...
def record_helper_request(audit_rows: list[dict[str, Any]], arm: str, row_id: str, request: dict[str, Any]) -> None:
    ...
    audit_rows.append(
        {
            "schema_version": "phase_142F_helper_request_audit_row_v1",
            "arm": arm,
            "row_id": row_id,
...
def row_score(row: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143A_scoring_result_v1",
        "arm": result["arm"],
        "row_id": row["row_id"],
        "seed": row["seed"],
        "family": row["family"],
...
def metrics_for(scored: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    final_accuracy = rate(sum(1 for item in scored if item["final_answer_correct"]), total)
    dominant_rate = rate(dominant_count, total)
    default_shortcut = rate(sum(1 for item in scored if item["default_pocket_shortcut"]), total)
    first_shortcut = rate(sum(1 for item in scored if item["first_pocket_shortcut"]), total)
    last_shortcut = rate(sum(1 for item in scored if item["last_pocket_shortcut"]), total)
    stale_shortcut = rate(sum(1 for item in scored if item["stale_pocket_shortcut"]), total)
    ...
    return {
        ...
        "default_pocket_shortcut_rate": default_shortcut,
        "first_pocket_shortcut_rate": first_shortcut,
        "last_pocket_shortcut_rate": last_shortcut,
        "stale_pocket_shortcut_rate": stale_shortcut,
        ...
        "dominant_generated_source_rate": dominant_rate,
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for item in scored:
        ...
        by_winner[item["selected_pocket_id"]].append(item)
    ...
    for group_id, items in sorted(by_group.items()):
        ...
        group_rows.append(
            {
                "schema_version": "phase_143A_contrast_group_result_v1",
                "arm": arm,
                "contrast_group_id": group_id,
                ...
                "group_pass": all(item["final_answer_correct"] and item["multi_pocket_arbitration_correct"] for item in items) and len(set(generated)) == len(set(expected)),
                "wrong_pocket_in_group": any(item["wrong_pocket"] for item in items),
            }
        )
    ...
    for pair_id, items in sorted(by_pair.items()):
        ...
        pair_rows.append(
            {
                "schema_version": "phase_143A_arbitration_inversion_pair_v1",
                "arm": arm,
    ...
    metrics["priority_inversion_accuracy"] = rate(sum(1 for pair in pair_rows if pair["pair_pass"]), len(pair_rows))
    ...
    metrics["per_winner"] = {winner: metrics_for(items) for winner, items in sorted(by_winner.items())}
...
def fitness_for(metrics: dict[str, Any]) -> float:
    return (
        ...
        + float(metrics["multi_pocket_arbitration_accuracy"])
        + float(metrics["quorum_rule_accuracy"])
        + float(metrics["recency_rule_accuracy"])
        + float(metrics["tie_break_accuracy"])
        + float(metrics["pocket_writeback_rate"])
        + float(metrics["contrast_group_accuracy"])
        - float(metrics["wrong_pocket_rate"])
        - float(metrics["default_pocket_shortcut_rate"])
        - float(metrics["resolved_final_marker_echo_rate"])
    )


def marker_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_142F_explicit_marker_audit_v1",
        "row_count": len(rows),
...
def multi_pocket_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143A_multi_pocket_manifest_v1",
        "row_count": len(rows),
        "selected_pocket_counts": dict(sorted(counts.items())),
        "pocket_a_rows": counts.get("pocket_a", 0),
        "pocket_b_rows": counts.get("pocket_b", 0),
        "pocket_c_rows": counts.get("pocket_c", 0),
...
def arbitration_rule_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    inversion_pairs = [
        ...
        if len({item["selected_pocket_id"] for item in items}) >= 2
    ]
    return {
        "schema_version": "phase_143A_arbitration_rule_manifest_v1",
...
def winner_distribution_report(rows: list[dict[str, Any]]) -> dict[str, Any]:
    counts = Counter(row["selected_pocket_id"] for row in rows)
    total = len(rows)
    rates = {
    ...
    balanced = all(value >= 0.20 for value in rates.values())
    return {
        "schema_version": "phase_143A_pocket_distribution_report_v1",
        "row_count": total,
        "selected_pocket_counts": dict(sorted(counts.items())),
        **rates,
        "pocket_distribution_balanced": balanced,
        "minimum_pocket_rate_gate": 0.20,
        "passed": balanced,
    }


def per_pocket_gate_report(main_metrics: dict[str, Any]) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    for winner in ["pocket_a", "pocket_b", "pocket_c"]:
        metrics = main_metrics["per_winner"].get(winner, {})
        failures: list[str] = []
        if metrics.get("final_answer_accuracy", 0.0) < 0.60:
            failures.append("final_answer_accuracy")
        if metrics.get("multi_pocket_arbitration_accuracy", 0.0) < 0.60:
            failures.append("multi_pocket_arbitration_accuracy")
        rows.append({"selected_pocket_id": winner, "main": metrics, "passed": not failures, "failures": failures})
    return {"schema_version": "phase_143A_per_pocket_gate_report_v1", "passed": all(row["passed"] for row in rows), "pockets": rows}


def shortcut_report(main_metrics: dict[str, Any]) -> dict[str, Any]:
    keys = [
        "wrong_pocket_rate",
    ...
    values = {key: main_metrics.get(key, 0.0) for key in keys}
    return {
        "schema_version": "phase_143A_shortcut_report_v1",
        **values,
        "shortcut_detected": any(value != 0.0 for value in values.values()),
...
def forbidden_canary(helper: Any, checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"schema_version": "phase_142F_expected_output_canary_v1", "passed": True, "verdict": "forbidden_expected_output_rejected", "error_type": type(exc).__name__, "error": str(exc)}
    return {"schema_version": "phase_142F_expected_output_canary_v1", "passed": False, "verdict": "forbidden_expected_output_accepted"}


def control_row(name: str, control_passed: bool, generated_value: str | None, blocked_value: str | None) -> dict[str, Any]:
    return {"schema_version": "phase_142F_control_result_v1", "control": name, "control_passed": control_passed, "control_failed": not control_passed, "generated_value": generated_value, "blocked_value": blocked_value}
...
def run_controls(helper: Any, checkpoints: dict[str, tuple[Path, str]], max_new_tokens: int, request_audit: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    base_template = {
        ...
        "arbitration_pair_id": "control_pair",
        "arbitration_rule_id": "control_arbitration_rule",
        "quorum_rule_id": "control_quorum",
        "recency_rank": "newest",
        "tie_break_rule_id": "control_tie",
        "rule_hierarchy_id": "stale_rejection_gt_recency_gt_quorum_gt_tie_break",
        "pocket_a_value": "EVCTRL_A",
        "pocket_b_value": "EVCTRL_B",
        "pocket_c_value": "EVCTRL_C",
        "stale_pocket_value": "EVCTRL_STALE",
        "visible_wrong_value": "EVCTRL_V",
        "noisy_distractor_values": ["EVCTRL_D0", "EVCTRL_D1"],
        "final_payload_marker": "arbitrated final:",
    }
    controls = [
        ...
        ("QUORUM_WRONG_CONTROL", "first_pocket_candidate", "pocket_c", False),
        ("RECENCY_WRONG_CONTROL", "last_pocket_candidate", "pocket_a", False),
        ("TIE_BREAK_WRONG_CONTROL", "default_pocket_candidate", "pocket_c", False),
    ...
    for name, candidate, expected_source, corrupt_final_marker in controls:
        ...
        base["selected_pocket_id"] = expected_source
        base["answer_value"] = expected_value(base)
        base["final_target_value"] = base["answer_value"]
        base["resolved_final_marker_value"] = "EVCTRL_WRONG_ECHO" if corrupt_final_marker else base["answer_value"]
        base["losing_values"] = [
            value
            for source, value in {
                "pocket_a": base["pocket_a_value"],
                "pocket_b": base["pocket_b_value"],
                "pocket_c": base["pocket_c_value"],
                "stale": base["stale_pocket_value"],
            }.items()
    ...
    report = {
        "schema_version": "phase_143A_control_report_v1",
...
def seed_gate_report(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    for seed, metrics in sorted(main_metrics["per_seed"].items()):
        ...
        failures = []
        if metrics["final_answer_accuracy"] < 0.60:
            failures.append("final_answer_accuracy")
        if metrics["multi_pocket_arbitration_accuracy"] < 0.60:
            failures.append("multi_pocket_arbitration_accuracy")
        if metrics["pocket_writeback_rate"] < 0.70:
            failures.append("pocket_writeback_rate")
        if ablation.get("final_answer_accuracy", 1.0) > 0.20:
            failures.append("ablation_final_answer_accuracy")
        for key in ["wrong_pocket_rate", "default_pocket_shortcut_rate", "first_pocket_shortcut_rate", "last_pocket_shortcut_rate", "stale_pocket_shortcut_rate", "resolved_final_marker_echo_rate", "visible_bypass_violation_rate", "noisy_distractor_violation_rate"]:
    ...
    return {"schema_version": "phase_143A_per_seed_gate_report_v1", "passed": all(row["passed"] for row in rows), "seeds": rows}
...
def choose_decision(
    ...
    if not infra_passed or deterministic is not True or control_report.get("controls_failed") is not True:
        ...
        decision = "multi_pocket_arbitration_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143E_MULTI_POCKET_ARBITRATION_FAILURE_ANALYSIS"
    elif main["resolved_final_marker_echo_rate"] > 0.0:
    ...
    elif main["default_pocket_shortcut_rate"] > 0.0:
        decision = "default_pocket_shortcut_detected"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143C_DEFAULT_POCKET_SHORTCUT_ANALYSIS"
    elif main["first_pocket_shortcut_rate"] > 0.0 or main["last_pocket_shortcut_rate"] > 0.0:
        decision = "first_or_last_pocket_shortcut_detected"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143D_POSITIONAL_POCKET_SHORTCUT_ANALYSIS"
    elif main["rule_hierarchy_conflict_accuracy"] < 0.65:
        decision = "rule_hierarchy_conflict_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143H_RULE_HIERARCHY_CONFLICT_ANALYSIS"
    elif main["quorum_rule_accuracy"] < 0.65 or main["recency_rule_accuracy"] < 0.65 or main["tie_break_accuracy"] < 0.65:
        decision = "quorum_recency_tie_break_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143G_QUORUM_RECENCY_TIE_BREAK_FAILURE_ANALYSIS"
    elif comparison["priority_inversion_pair_count"] < ARBITRATION_INVERSION_PAIR_COUNT_FLOOR or main["priority_inversion_accuracy"] < 0.70:
        decision = "priority_inversion_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143E_MULTI_POCKET_ARBITRATION_FAILURE_ANALYSIS"
    elif main["pocket_label_permutation_accuracy"] < 0.70 or main["same_values_different_rule_accuracy"] < 0.70 or main["same_rule_different_values_accuracy"] < 0.70:
        decision = "multi_pocket_arbitration_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143E_MULTI_POCKET_ARBITRATION_FAILURE_ANALYSIS"
    elif ablation["final_answer_accuracy"] > 0.15 or comparison["pocket_ablation_delta_final_answer_accuracy"] < 0.50:
        decision = "pocket_ablation_not_decision_critical"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "141D_POCKET_CAUSALITY_FAILURE_ANALYSIS"
    elif main["final_answer_accuracy"] < 0.70 or main["multi_pocket_arbitration_accuracy"] < 0.70 or main["pocket_writeback_rate"] < 0.80 or main["contrast_group_accuracy"] < 0.70:
        decision = "multi_pocket_arbitration_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143E_MULTI_POCKET_ARBITRATION_FAILURE_ANALYSIS"
    elif seed_report["passed"] is not True or family_report["passed"] is not True or family_report.get("no_family_zero_pass") is not True or winner_report["passed"] is not True or distribution_report["passed"] is not True or shortcut["passed"] is not True:
        decision = "multi_pocket_arbitration_failure"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_FAILS"; next_step = "143E_MULTI_POCKET_ARBITRATION_FAILURE_ANALYSIS"
    else:
        decision = "instnct_pocket_gated_multi_pocket_arbitration_probe_positive"; verdict = "INSTNCT_POCKET_GATED_MULTI_POCKET_ARBITRATION_POSITIVE"; next_step = POSITIVE_NEXT
    return {
        "schema_version": "phase_143A_decision_v1",
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "clean_negative_valid": True,
        "pocket_mechanism_claimed": decision == "instnct_pocket_gated_multi_pocket_arbitration_probe_positive",
        "multi_pocket_arbitration_positive": decision == "instnct_pocket_gated_multi_pocket_arbitration_probe_positive",
...
Multi-pocket arbitration metrics:

- eval rows: `{comparison['eval_row_count']}`
- family count: `{comparison['family_count']}`
...
- quorum rule accuracy: `{comparison['quorum_rule_accuracy']}`
- recency rule accuracy: `{comparison['recency_rule_accuracy']}`
- tie-break accuracy: `{comparison['tie_break_accuracy']}`
- rule hierarchy conflict accuracy: `{comparison['rule_hierarchy_conflict_accuracy']}`
- priority inversion pairs: `{comparison['priority_inversion_pair_count']}`
...
- pocket label permutation accuracy: `{comparison['pocket_label_permutation_accuracy']}`
- same-values different-rule accuracy: `{comparison['same_values_different_rule_accuracy']}`
- same-rule different-values accuracy: `{comparison['same_rule_different_values_accuracy']}`
...
- default pocket shortcut rate: `{comparison['default_pocket_shortcut_rate']}`
- first pocket shortcut rate: `{comparison['first_pocket_shortcut_rate']}`
- last pocket shortcut rate: `{comparison['last_pocket_shortcut_rate']}`
- stale pocket shortcut rate: `{comparison['stale_pocket_shortcut_rate']}`
...
The resolved-final-marker echo path is explicitly tested through corrupted-marker
controls. A positive result remains constrained helper/backend multi-pocket
arbitration final selection only. It is not open-ended reasoning, not general
...
def report_metric(name: str, value: Any, threshold: float, comparator: str = ">=") -> dict[str, Any]:
    passed = value >= threshold if comparator == ">=" else value <= threshold
    return {"metric": name, "value": value, "threshold": threshold, "comparator": comparator, "passed": passed}


def main() -> int:
    parser = argparse.ArgumentParser(description=MILESTONE)
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--upstream-142z-root", type=Path, default=DEFAULT_142Z_ROOT)
    parser.add_argument("--upstream-142f-root", type=Path, default=DEFAULT_142F_ROOT)
    parser.add_argument("--seeds", default="4401,4402,4403")
    ...
    write_json(out / "queue.json", {"schema_version": "phase_143A_queue_v1", "milestone": MILESTONE, "status": "running"})

    seeds = [int(item) for item in args.seeds.split(",") if item.strip()]
    upstream_142z = require_142z(resolve_repo_path(args.upstream_142z_root))
    upstream_142f = require_142f(resolve_repo_path(args.upstream_142f_root))
    write_json(out / "upstream_142z_manifest.json", upstream_142z)
    write_json(out / "upstream_142f_manifest.json", upstream_142f)
    append_progress(out, "upstream verification", upstream_142z=upstream_142z["decision"], upstream_142f=upstream_142f["decision"])

    config = {
        "schema_version": "phase_143A_eval_config_v1",
        "milestone": MILESTONE,
        "boundary": BOUNDARY_TEXT,
        "train_allowed": False,
    ...
    helper = load_helper()
    provenance = {
        "schema_version": "phase_143A_helper_provenance_v1",
    ...
    pocket_manifest = multi_pocket_manifest(rows)
    arbitration_manifest = arbitration_rule_manifest(rows)
    pocket_distribution = winner_distribution_report(rows)
    family_counts = Counter(row["family"] for row in rows)
    scaffold_counts = Counter(row["scaffold_id"] for row in rows)
    eval_manifest = {
        "schema_version": "phase_143A_eval_manifest_v1",
        "row_count": len(rows),
        "seeds": seeds,
        ...
        "marker_audit": marker,
        "multi_pocket_manifest": pocket_manifest,
        "arbitration_rule_manifest": arbitration_manifest,
    }
    ...
    write_json(out / "pocket_distribution_report.json", pocket_distribution)
    write_json(out / "winner_distribution_report.json", pocket_distribution)
    append_progress(out, "multi-pocket row build", row_count=len(rows), inversion_pair_rate=arbitration_manifest["arbitration_inversion_pair_rate"], family_count=len(family_counts))
    ...
    for candidate in candidate_specs():
        ...
        candidate_rows.append({
            "schema_version": "phase_143A_mutation_candidate_result_v1",
            "candidate": candidate["candidate"],
            "final_answer_accuracy": metrics["final_answer_accuracy"],
            "multi_pocket_arbitration_accuracy": metrics["multi_pocket_arbitration_accuracy"],
            "quorum_rule_accuracy": metrics["quorum_rule_accuracy"],
            "recency_rule_accuracy": metrics["recency_rule_accuracy"],
            "tie_break_accuracy": metrics["tie_break_accuracy"],
            "rule_hierarchy_conflict_accuracy": metrics["rule_hierarchy_conflict_accuracy"],
            "pocket_writeback_rate": metrics["pocket_writeback_rate"],
            "wrong_pocket_rate": metrics["wrong_pocket_rate"],
            "resolved_final_marker_echo_rate": metrics["resolved_final_marker_echo_rate"],
            "fitness": fitness,
            "selected": False,
        })
        ...
        append_progress(out, "candidate evaluated", candidate=candidate["candidate"], fitness=fitness, arbitration_accuracy=metrics["multi_pocket_arbitration_accuracy"])
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_143A_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    ablation_checkpoint, ablation_manifest = manifests["closed_pocket_no_arbitration"]
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_143A_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_jsonl(out / "pocket_trace.jsonl", [{"row_id": row["row_id"], "arm": row["arm"], "pocket_writeback_count": row["pocket_writeback_count"], "value_selection_source": row["value_selection_source"], "value_selection_marker": row["value_selection_marker"], "highway_retained": row["highway_retained"]} for row in main_results + ablation_results])
    ...
    write_jsonl(out / "arbitration_inversion_pairs.jsonl", main_pairs + ablation_pairs)
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_143A_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {"schema_version": "phase_143A_generated_before_scoring_report_v1", "passed": True, "generated_text_produced_before_scoring": True, "all_helper_requests_allowed_keys_only": all(set(row["helper_request"]) == ALLOWED_HELPER_KEYS for row in main_results + ablation_results + replay), "expected_or_scorer_metadata_in_helper_requests": False}
    leakage_report = {"schema_version": "phase_143A_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False}
    ...
    comparison = {
        "schema_version": "phase_143A_arm_comparison_v1",
        ...
        "priority_inversion_pair_count": arbitration_manifest["arbitration_inversion_pair_count"],
        "priority_inversion_pair_count_floor": ARBITRATION_INVERSION_PAIR_COUNT_FLOOR,
        ...
        "pocket_label_permutation_accuracy": main_metrics["pocket_label_permutation_accuracy"],
        "same_values_different_rule_accuracy": main_metrics["same_values_different_rule_accuracy"],
        "same_rule_different_values_accuracy": main_metrics["same_rule_different_values_accuracy"],
        ...
        "wrong_pocket_rate": main_metrics["wrong_pocket_rate"],
        ...
        "pocket_a_wins_rate": pocket_distribution["pocket_a_wins_rate"],
        "pocket_b_wins_rate": pocket_distribution["pocket_b_wins_rate"],
        "pocket_c_wins_rate": pocket_distribution["pocket_c_wins_rate"],
        "pocket_distribution_balanced": pocket_distribution["pocket_distribution_balanced"],
        "deterministic_replay_passed": deterministic,
        "architecture_superiority_claimed": False,
        "value_grounding_claimed": False,
    }
    helper_request_audit = {
        "schema_version": "phase_143A_helper_request_audit_v1",
    ...
    alias_report = {
        "schema_version": "phase_143A_canonical_metric_alias_report_v1",
        "canonical_metrics": {
            key: comparison[key]
            for key in [
                "direct_pocket_value_marker_rate",
                ...
                "tie_break_accuracy",
                "rule_hierarchy_conflict_accuracy",
                "priority_inversion_accuracy",
                "same_template_opposite_winner_accuracy",
                "pocket_label_permutation_accuracy",
                "same_values_different_rule_accuracy",
                "same_rule_different_values_accuracy",
                "default_pocket_shortcut_rate",
                "first_pocket_shortcut_rate",
                "last_pocket_shortcut_rate",
                "stale_pocket_shortcut_rate",
    ...
    pocket_report = per_pocket_gate_report(main_metrics)
    shortcuts = shortcut_report(main_metrics)
    aggregate_metrics = {
        "schema_version": "phase_143A_aggregate_metrics_v1",
        **comparison,
    ...
    write_json(out / "multi_pocket_arbitration_metrics.json", {"schema_version": "phase_143A_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    ...
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_143A_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_143A_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    write_json(out / "per_pocket_metrics.json", {"schema_version": "phase_143A_per_pocket_metrics_v1", "main": main_metrics["per_winner"]})
    ...
    inversion_report = {"schema_version": "phase_143A_priority_inversion_report_v1", "priority_inversion_accuracy": main_metrics["priority_inversion_accuracy"], "same_template_opposite_winner_accuracy": main_metrics["same_template_opposite_winner_accuracy"], "priority_inversion_pair_count": arbitration_manifest["arbitration_inversion_pair_count"], "priority_inversion_pair_count_floor": ARBITRATION_INVERSION_PAIR_COUNT_FLOOR, "priority_inversion_pair_rate": arbitration_manifest["arbitration_inversion_pair_rate"], "same_template_opposite_winner_pairs": arbitration_manifest["same_template_opposite_winner_pairs"], "passed": main_metrics["priority_inversion_accuracy"] >= 0.70 and arbitration_manifest["arbitration_inversion_pair_count"] >= ARBITRATION_INVERSION_PAIR_COUNT_FLOOR}
    write_json(out / "priority_inversion_report.json", inversion_report)
    write_json(out / "priority_inversion_pair_report.json", inversion_report)
    write_json(out / "same_template_opposite_winner_report.json", {"schema_version": "phase_143A_same_template_opposite_winner_report_v1", "same_template_opposite_winner_accuracy": main_metrics["same_template_opposite_winner_accuracy"], "same_template_opposite_winner_pairs": arbitration_manifest["same_template_opposite_winner_pairs"], "same_template_opposite_winner_pair_floor": ARBITRATION_INVERSION_PAIR_COUNT_FLOOR, "passed": main_metrics["same_template_opposite_winner_accuracy"] >= 0.70 and arbitration_manifest["same_template_opposite_winner_pairs"] >= ARBITRATION_INVERSION_PAIR_COUNT_FLOOR})
    write_json(out / "resolved_final_marker_echo_report.json", {"schema_version": "phase_143A_resolved_final_marker_echo_report_v1", "resolved_final_marker_echo_rate": main_metrics["resolved_final_marker_echo_rate"], "resolved_final_marker_echo_control_failed": control_report.get("resolved_final_marker_echo_control_failed"), "passed": main_metrics["resolved_final_marker_echo_rate"] == 0.0 and control_report.get("resolved_final_marker_echo_control_failed") is True})
    write_json(out / "pocket_label_permutation_report.json", {"schema_version": "phase_143A_pocket_label_permutation_report_v1", "pocket_label_permutation_accuracy": main_metrics["pocket_label_permutation_accuracy"], "passed": main_metrics["pocket_label_permutation_accuracy"] >= 0.70})
    write_json(out / "same_values_different_rule_report.json", {"schema_version": "phase_143A_same_values_different_rule_report_v1", "same_values_different_rule_accuracy": main_metrics["same_values_different_rule_accuracy"], "passed": main_metrics["same_values_different_rule_accuracy"] >= 0.70})
    write_json(out / "same_rule_different_values_report.json", {"schema_version": "phase_143A_same_rule_different_values_report_v1", "same_rule_different_values_accuracy": main_metrics["same_rule_different_values_accuracy"], "passed": main_metrics["same_rule_different_values_accuracy"] >= 0.70})
    write_json(out / "rule_hierarchy_conflict_report.json", {"schema_version": "phase_143A_rule_hierarchy_conflict_report_v1", "rule_hierarchy_conflict_accuracy": main_metrics["rule_hierarchy_conflict_accuracy"], "rule_hierarchy_conflict_control_failed": control_report.get("rule_hierarchy_conflict_control_failed"), "passed": main_metrics["rule_hierarchy_conflict_accuracy"] >= 0.65 and control_report.get("rule_hierarchy_conflict_control_failed") is True})
    write_json(out / "default_pocket_shortcut_report.json", {"schema_version": "phase_143A_default_pocket_shortcut_report_v1", "default_pocket_shortcut_rate": main_metrics["default_pocket_shortcut_rate"], "passed": main_metrics["default_pocket_shortcut_rate"] == 0.0})
    write_json(out / "first_pocket_shortcut_report.json", {"schema_version": "phase_143A_first_pocket_shortcut_report_v1", "first_pocket_shortcut_rate": main_metrics["first_pocket_shortcut_rate"], "last_pocket_shortcut_rate": main_metrics["last_pocket_shortcut_rate"], "passed": main_metrics["first_pocket_shortcut_rate"] == 0.0 and main_metrics["last_pocket_shortcut_rate"] == 0.0})
    write_json(out / "stale_pocket_shortcut_report.json", {"schema_version": "phase_143A_stale_pocket_shortcut_report_v1", "stale_pocket_shortcut_rate": main_metrics["stale_pocket_shortcut_rate"], "passed": main_metrics["stale_pocket_shortcut_rate"] == 0.0})
    write_json(out / "visible_noisy_shortcut_report.json", {"schema_version": "phase_143A_visible_noisy_shortcut_report_v1", "visible_bypass_violation_rate": main_metrics["visible_bypass_violation_rate"], "noisy_distractor_violation_rate": main_metrics["noisy_distractor_violation_rate"], "passed": main_metrics["visible_bypass_violation_rate"] == 0.0 and main_metrics["noisy_distractor_violation_rate"] == 0.0})
    append_progress(out, "aggregate analysis", final_accuracy=comparison["main_final_answer_accuracy"], arbitration_accuracy=comparison["multi_pocket_arbitration_accuracy"])
    ...
    decision = choose_decision(main_metrics, ablation_metrics, comparison, control_report, deterministic, selected["candidate"], infra_passed, seed_report, family_report, pocket_report, pocket_distribution, shortcuts)
    write_json(out / "decision.json", decision)
    summary = {"schema_version": "phase_143A_summary_v1", "milestone": MILESTONE, "status": "complete", "boundary": BOUNDARY_TEXT, "upstream_142z": upstream_142z, "upstream_142f": upstream_142f, "metrics": comparison, "aggregate_metrics": aggregate_metrics, "selection": selection, "helper_request_audit": helper_request_audit, "pocket_distribution_report": pocket_distribution, "per_seed_gate_report": seed_report, "per_family_gate_report": family_report, "per_pocket_gate_report": pocket_report, "shortcut_report": shortcuts, "canary_passed": canary["passed"], "ast_shortcut_scan_passed": ast_report["passed"], "generated_before_scoring_passed": generated_report["passed"], "leakage_rejected": leakage_report["leakage_rejected"], "controls_failed": control_report["controls_failed"], **decision}
    ...
    write_json(out / "queue.json", {"schema_version": "phase_143A_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
