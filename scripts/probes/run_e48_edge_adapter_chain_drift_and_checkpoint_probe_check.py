...
REQ_SAMPLE = [
    ...
    "chain_drift_report_sample.json",
    "row_level_sample.jsonl",
    "adapter_chain_curve_sample.jsonl",
...
def recompute(rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    ...
    for system in sorted({row["system"] for row in rows}):
        ...
        clean_rows = [row for row in chunk if row["scenario"] == "clean"]
        out[system] = {
            "row_count": float(len(chunk)),
            "chain_success": mean([1.0 if row["chain_success"] else 0.0 for row in chunk]),
            "heldout_success": split_metric(chunk, "heldout", "chain_success"),
            "ood_success": split_metric(chunk, "ood", "chain_success"),
            "counterfactual_success": split_metric(chunk, "counterfactual", "chain_success"),
            "adversarial_success": split_metric(chunk, "adversarial", "chain_success"),
            "clean_success": mean([1.0 if row["chain_success"] else 0.0 for row in clean_rows]),
            "depth_1_success": depth_metric(chunk, 1, "chain_success"),
            "depth_2_success": depth_metric(chunk, 2, "chain_success"),
            "depth_3_success": depth_metric(chunk, 3, "chain_success"),
            "depth_5_success": depth_metric(chunk, 5, "chain_success"),
            ...
            "false_defer_rate": mean([1.0 if row["false_defer"] else 0.0 for row in chunk]),
            "drift_rate": mean([1.0 if row["drift_detected"] else 0.0 for row in chunk]),
            "trace_mismatch_commit_rate": mean([1.0 if row["stale_commit"] else 0.0 for row in chunk]),
            "mean_drift_events": mean([float(row["drift_event_count"]) for row in chunk]),
        }
    return out
...
def pass_gate(metrics: dict[str, Any], report: dict[str, Any]) -> bool:
    return (
        metrics["heldout_success"] >= 0.95
        and metrics["ood_success"] >= 0.95
        and metrics["counterfactual_success"] >= 0.95
        and metrics["adversarial_success"] >= 0.95
...
def validate_sample(sample_dir: Path) -> dict[str, Any]:
    ...
    curve = read_jsonl(sample_dir / "adapter_chain_curve_sample.jsonl")
    if schema.get("milestone") != MILESTONE or schema.get("edge_adapter_chain") is not True:
        failures.append("sample schema missing E48 marker")
...
def validate_target(out: Path, sample_dir: Path, write_summary: bool) -> dict[str, Any]:
    ...
    failures.extend(static_policy_check(Path("scripts/probes/run_e48_edge_adapter_chain_drift_and_checkpoint_probe.py")))
    ...
    reports = read_json(out / "chain_drift_report.json")
    rows = read_jsonl(out / "row_level_results.jsonl")
    curve = read_jsonl(out / "adapter_chain_curve.jsonl")
    ...
    if set(system_results) != SYSTEMS or set(reports) != SYSTEMS:
    ...
    for system in MUTATED_SYSTEMS:
        ...
        if report.get("accepted", 0) <= 0 or report.get("rejected", 0) <= 0:
            failures.append(f"{system} missing accepted/rejected mutation evidence")
    ...
    no_checkpoint = system_results["adapter_chain_no_checkpoint"]["overall"]
    checkpoint = system_results["adapter_chain_with_agency_checkpoint"]["overall"]
    trace = system_results["adapter_chain_plus_trace_validation"]["overall"]
    if aggregate.get("decision") == "e48_adapter_chain_stable":
        if not pass_gate(no_checkpoint, reports["adapter_chain_no_checkpoint"]):
            failures.append("stable decision without no-checkpoint pass")
    if aggregate.get("decision") == "e48_adapter_chain_requires_checkpoint_and_trace_validation":
        if not pass_gate(trace, reports["adapter_chain_plus_trace_validation"]):
            failures.append("checkpoint+trace decision without trace system pass")
        if checkpoint["clean_success"] < 0.95:
            failures.append("checkpoint+trace decision but checkpoint clean path fails")
        if checkpoint["adversarial_success"] >= 0.95:
            failures.append("checkpoint+trace decision but checkpoint also passes adversarial")
    if aggregate.get("decision") == "e48_adapter_chain_requires_agency_checkpoint":
        if not pass_gate(checkpoint, reports["adapter_chain_with_agency_checkpoint"]):
            failures.append("checkpoint-required decision without checkpoint pass")
