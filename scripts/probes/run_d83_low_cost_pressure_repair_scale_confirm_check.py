...
def main():
    ...
    d82=man.get("d82_artifacts",{})
    if d82.get("decision")!="low_cost_pressure_repair_confirmed": fail(f"D82 decision mismatch {d82.get('decision')}")
    if d82.get("next")!="D83_LOW_COST_PRESSURE_REPAIR_SCALE_CONFIRM": fail(f"D82 next mismatch {d82.get('next')}")
    if d82.get("best_arm")!="LOW_COST_PRESSURE_REPAIR_COST_AWARE": fail(f"D82 best mismatch {d82.get('best_arm')}")
    if d82.get("top1_guard_weakened") is True: fail("D82 top1 guard weakened")
    if not man.get("d82_commit_present",{}).get("present") and not man.get("rerun",{}).get("rerun_attempted"): fail("D82 commit missing without rerun")
    if man.get("rerun",{}).get("rerun_attempted") and not man.get("rerun",{}).get("rerun_succeeded"): fail("D82 rerun failed")
    best=agg.get("best_fair_arm",{})
    req=["low_cost_pressure_breakpoint","exact_joint_accuracy","correlated_echo_accuracy","adversarial_distractor_accuracy","external_test_required_accuracy","false_confidence_rate","indistinguishable_abstain_rate","average_total_support_used","counter_support_used","distance_to_concrete_oracle_support","gap_reduction_vs_D73_bound","joint_counter_recall_on_joint_required_rows","external_recall_on_external_required_rows","wrong_concrete_counter_rate","weak_top1_top2_path_failure_rate","top1_top2_sufficient_false_joint_rate","D68_loss_repair_preservation_rate","routing_failure_rows","top1_guard_ablation_routing_failure_rows","top1_guard_ablation_D68_preservation","min_seed_exact","min_seed_correlated","min_seed_adversarial","min_seed_external","fallback_rows","failed_jobs"]
    absent=[k for k in req if k not in best]
    if absent: fail(f"best missing {absent}")
    if best.get("arm")!="D82_LOW_COST_REPAIR_COST_AWARE_REPLAY": fail(f"unexpected best {best.get('arm')}")
    abm=ab.get("ablation_metrics",{})
    gates={"low_cost_pressure_breakpoint":best["low_cost_pressure_breakpoint"]>=0.74,"exact_joint_accuracy":best["exact_joint_accuracy"]>=0.9990,"correlated_echo_accuracy":best["correlated_echo_accuracy"]>=0.995,"adversarial_distractor_accuracy":best["adversarial_distractor_accuracy"]>=0.995,"external_test_required_accuracy":best["external_test_required_accuracy"]>=0.995,"false_confidence_rate":best["false_confidence_rate"]<=0.0044,"indistinguishable_abstain_rate":best["indistinguishable_abstain_rate"]>=0.9948,"average_total_support_used":best["average_total_support_used"]<=6.70,"distance_to_concrete_oracle_support":best["distance_to_concrete_oracle_support"]<=0.38,"gap_reduction_vs_D73_bound":best["gap_reduction_vs_D73_bound"]>=0.1500,"joint_counter_recall_on_joint_required_rows":best["joint_counter_recall_on_joint_required_rows"]>=0.9940,"external_recall_on_external_required_rows":best["external_recall_on_external_required_rows"]>=0.9957,"wrong_concrete_counter_rate":best["wrong_concrete_counter_rate"]<=0.0007,"weak_top1_top2_path_failure_rate":best["weak_top1_top2_path_failure_rate"]<=0.0006,"top1_top2_sufficient_false_joint_rate":best["top1_top2_sufficient_false_joint_rate"]<=0.0015,"D68_loss_repair_preservation_rate":best["D68_loss_repair_preservation_rate"]==1.0,"routing_failure_rows":best["routing_failure_rows"]==0,"top1_guard_preserved":preserve.get("top1_guard_preserved") is True and not preserve.get("top1_guard_weakened"),"top1_guard_ablation_remains_worse":ab.get("guard_ablation_worse") is True and abm.get("routing_failure_rows",0)>best["routing_failure_rows"] and abm.get("D68_loss_repair_preservation_rate",1)<best["D68_loss_repair_preservation_rate"],"rust_path_invoked":best["rust_path_invoked"] is True,"fallback_rows":best["fallback_rows"]==0,"failed_jobs":not best["failed_jobs"]}
    failed=[k for k,v in gates.items() if not v]
    if dec.get("decision")=="low_cost_pressure_repair_scale_confirmed" and failed: fail(f"positive gates failed {failed}")
    for name,rep in [("scale",scale),("sweep",sweep),("preserve",preserve),("ablation",ab),("D68",d68),("loss",loss),("ambiguity",amb),("joint",joint),("ood",ood),("external",ext),("abstain",abst),("safety",safety),("support",supp),("oracle",oracle),("truth",truth),("rust",rust)]:
