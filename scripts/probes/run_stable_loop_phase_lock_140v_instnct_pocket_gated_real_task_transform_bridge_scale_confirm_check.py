#!/usr/bin/env python3
"""Checker for 140V real-task transform bridge scale confirm."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_140v_instnct_pocket_gated_real_task_transform_bridge_scale_confirm/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_140v_instnct_pocket_gated_real_task_transform_bridge_scale_confirm.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_140v_instnct_pocket_gated_real_task_transform_bridge_scale_confirm_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_140V_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_SCALE_CONFIRM_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_140V_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_SCALE_CONFIRM_RESULT.md",
]
ALLOWED_MUTATIONS = {RUNNER, CHECKER, *DOCS}
REQUIRED_ARTIFACTS = [
    "queue.json",
    "progress.jsonl",
    "upstream_140u_manifest.json",
    "scale_config.json",
    ...
    "transform_scale_eval_manifest.json",
    "real_task_transform_scale_prompt_manifest.json",
    ...
    "transform_bridge_scale_metrics.json",
...
def check_static_files() -> list[str]:
    ...
    for rel in [RUNNER, CHECKER, *DOCS]:
        ...
        for term in ["140V", "scale", "transform", "copy", "pocket"]:
            if term not in text:
...
def check_artifacts() -> list[str]:
    ...
    if failures:
        return failures
    upstream = load_json(SMOKE_ROOT / "upstream_140u_manifest.json")
    ...
    eval_manifest = load_json(SMOKE_ROOT / "transform_scale_eval_manifest.json")
    prompt_manifest = load_json(SMOKE_ROOT / "real_task_transform_scale_prompt_manifest.json")
    ...
    metrics = load_json(SMOKE_ROOT / "transform_bridge_scale_metrics.json")
    ...
    if upstream.get("decision") != "instnct_pocket_gated_real_task_transform_bridge_probe_positive":
        failures.append("BAD_140U_DECISION")
    if upstream.get("next") != "140V_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_SCALE_CONFIRM":
        failures.append("BAD_140U_NEXT")
    if upstream.get("main_transform_accuracy", 0.0) < 0.60 or upstream.get("copy_only_shortcut_detected") is not False:
        failures.append("BAD_140U_PROFILE")
    ...
    if len(binding_manifest.get("rule_ids", [])) < 6:
    ...
    if len(selected_rows) != 1 or selected_rows[0].get("candidate") != "open_transform_target_all_markers_scale":
        failures.append("BAD_SELECTED_CANDIDATE")
    if selection.get("gradient_used") is not False or selection.get("selected_candidate") != "open_transform_target_all_markers_scale":
    ...
    if main.get("answer_value_accuracy", 0.0) < 0.85:
        failures.append("MAIN_ACCURACY_TOO_LOW")
    if main.get("transform_accuracy", 0.0) < 0.85:
    ...
    if comparison.get("pocket_ablation_delta_answer_value_accuracy", 0.0) <= 0.60:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 140V real-task transform bridge scale confirm")
    ...
    payload = {"schema_version": "phase_140v_checker_result_v1", "status": "pass" if not failures else "fail", "failures": failures}
