...
VALID_NON_ORACLE_SYSTEMS = SYSTEMS - {"oracle_alu_rule_reference"}
MUTATION_SYSTEMS = {
    "flat_marker_table_router",
    "location_only_fixed_call_router",
    "boolean_alu_without_op_router",
...
def validate_footprint_row(row: dict[str, Any], failures: list[str], prefix: str) -> None:
    ...
    for key in [
        ...
        "scan_cell_count",
        "illegal_write_count",
        "missed_target_write_count",
    ]:
        if key not in footprint:
            failures.append(f"{prefix}: footprint missing {key}")
    for key in ["target_op", "alu_bits", "call", "router_trace"]:
        if key not in row:
            failures.append(f"{prefix}: missing {key}")
    call = row.get("call", {})
    if "location" not in call or "scale" not in call or "op" not in call:
        failures.append(f"{prefix}: missing call location/scale/op")
    ...
    if row.get("system") in VALID_NON_ORACLE_SYSTEMS and trace.get("mode") == "hidden_oracle_reference":
        failures.append(f"{prefix}: non-oracle system used hidden oracle mode")
    if row.get("system") == "mutable_alu_rule_cell_router":
        if "rules_text" not in trace or "scale_rule" not in trace or "op_rule" not in trace:
            failures.append(f"{prefix}: mutable ALU row missing rule trace")
...
def validate_sample(sample_dir: Path) -> dict[str, Any]:
    ...
    if schema.get("milestone") != MILESTONE or schema.get("alu_rule_cell_router") is not True:
        failures.append("sample schema missing E40 ALU marker")
...
def validate_target(out: Path, sample_dir: Path, write_summary: bool) -> dict[str, Any]:
    ...
    runner = Path(__file__).resolve().with_name("run_e40_mutable_alu_rule_cell_router_probe.py")
    ...
    program = read_json(out / "alu_rule_program_report.json")
    ...
    if manifest.get("footprint_logging_v1") is not True or manifest.get("alu_rule_cell_router") is not True:
        failures.append("E40 footprint/ALU marker missing")
    ...
    if program.get("oracle_reference_ineligible") is not True:
        failures.append("ALU report does not mark oracle as ineligible")
    ...
    if aggregate.get("decision") == "e40_mutable_alu_rule_cell_router_positive":
        if alu["exact_rate"] < 0.95:
            failures.append("positive decision but mutable ALU exact below threshold")
        if alu["write_spread_ratio"] > 0.12:
            failures.append("positive decision but mutable ALU write spread too large")
        if flat["exact_rate"] >= 0.75:
            failures.append("positive decision but flat marker table too strong")
        if scale_only["exact_rate"] >= 0.90:
            failures.append("positive decision but scale-only ALU too strong")
        if random_control["exact_rate"] >= 0.40:
            failures.append("positive decision but random control too strong")
        if scan["exact_rate"] < 0.95 or scan["scan_cell_count_mean"] < 200:
            failures.append("positive decision but scan-all control did not solve with exhaustive header scan")
