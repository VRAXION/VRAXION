...
FORBIDDEN_CLAIMS = ["agi", "consciousness", "sentient", "production ready", "raw language reasoning proof"]
...
def validate_sample(sample_dir: Path, expected_run_id: str | None = None) -> dict[str, Any]:
    ...
    if len(rows) < 120:
        failures.append("sample row count below 120")
    if not curves:
        failures.append("sample training curve empty")
    if set(metrics) != SYSTEMS:
        failures.append("sample system set mismatch")
    if schema.get("milestone") != MILESTONE or not schema.get("weak_supervision"):
    ...
    if boundary.get("forbidden_claims_present"):
        failures.append("sample boundary claims failed")
...
def validate_target(out: Path, sample_dir: Path | None, runner_path: Path | None) -> dict[str, Any]:
    ...
    mining = read_json(out / "dataset_mining_report.json")
    ...
    if aggregate.get("decision") != decision.get("decision") or summary.get("decision") != decision.get("decision"):
    ...
    if mining.get("examples_selected", 0) <= 0 or not mining.get("counts_by_action"):
        failures.append("mining report empty")
    if not progress:
    ...
    boundary_text = str(backend.get("boundary", "")).lower()
    for forbidden in FORBIDDEN_CLAIMS:
        if forbidden in report_text and forbidden not in boundary_text:
            failures.append("forbidden claim in report: " + forbidden)
    sample_result = None
    if sample_dir:
...
def main() -> int:
    ...
    if args.sample_only:
        ...
        if args.write_summary:
            write_path = Path(args.sample_only) / "sample_only_checker_result.json"
            write_path.write_text(json.dumps({"sample_only_checker_passed": result["passed"], "checker_failure_count": len(result.get("failures", [])), **result}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    runner = Path(__file__).with_name("run_e28_real_text_unresolved_information_seeking_training_audit.py")
