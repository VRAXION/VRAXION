...
CONTRACT = ROOT / "docs" / "research" / "STABLE_LOOP_WAVEFRONT_001_CONTRACT.md"
DOC_REPORT = ROOT / "docs" / "research" / "STABLE_LOOP_WAVEFRONT_001_RESULT.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="STABLE_LOOP_WAVEFRONT_001 deterministic wavefront probe.")
...
class LocalLoop(nn.Module):
    def __init__(self, arm: str, width: int):
        ...
        self.arm = arm
        self.width = width
        self.init = nn.Conv2d(INPUT_CHANNELS, width, 1)
        self.local = nn.Conv2d(width + INPUT_CHANNELS, width, 3, padding=1)
        self.gate = nn.Conv2d(width + INPUT_CHANNELS, width, 1)
        self.side = nn.Conv2d(width, width, 3, padding=1)
        self.head = nn.Linear(width, 1)
        self.reach_head = nn.Conv2d(width, 1, 1)
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor, steps: int = 1, noise_std: float = 0.0) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        traces = [h]
        for _ in range(steps):
            source = torch.cat([h, x], dim=1)
            proposal = torch.tanh(self.local(source))
            gate = torch.sigmoid(self.gate(source))
            if self.arm == "HIGHWAY_SIDEPOCKET_WAVE_LOOP":
                proposal = proposal + 0.5 * torch.tanh(self.side(h))
            elif self.arm == "PRISMION_PHASE_WAVE_LOOP":
                phase = torch.cos(self.side(h))
                proposal = proposal * phase
            elif self.arm == "ABC_TIED_LOOP_HARD_WALL":
                gate = gate * (1.0 - wall)
            elif self.arm == "ABC_TIED_LOOP_LEARNED_WALL":
                gate = gate * torch.sigmoid(-3.0 * wall + self.gate(source))
            decay = torch.sigmoid(self.decay)
            h = torch.tanh(decay * h + gate * proposal)
            traces.append(h)
        logits = self.head(target_cell(h, target)).squeeze(-1)
        reached = torch.sigmoid(self.reach_head(h))
        stats = trace_stats(traces, x, reached=reached)
        stats["target_reach_channel"] = target_cell(reached, target)[:, 0]
        return logits, stats
...
def train_model(config: Config, rows: list[WaveExample], args: argparse.Namespace, device: torch.device, progress_path: Path, seed: int) -> nn.Module | None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for indices in iter_batches(len(rows), args.batch_size, rng):
            ...
            steps = rng.choice(TRAIN_STEPS) if config.train_mode == "VARIABLE_S" else config.settling_steps
    ...
    with torch.no_grad():
        for start in range(0, len(rows), args.batch_size):
            ...
            for key, value in stats.items():
                if value.ndim == 0:
                    stat_rows[key].append(float(value.detach().cpu()))
...
def evaluate(config: Config, model: nn.Module | None, train_rows: list[WaveExample], eval_rows: list[WaveExample], args: argparse.Namespace, device: torch.device) -> dict[str, float]:
    ...
    metrics: dict[str, float] = {
        ...
        "variable_s_accuracy": accuracy(pred, gold) if config.train_mode == "VARIABLE_S" else math.nan,
...
def stage_defaults(stage: str) -> tuple[list[str], list[int], list[int], list[str]]:
    if stage == "smoke":
        return (
            ["ORACLE_FULL_BFS", "ORACLE_TRUNCATED_BFS_S", "SUMMARY_DIRECT_HEAD", "TARGET_MARKER_ONLY", "UNTIED_CNN_MATCHED_COMPUTE", "LOCAL_MESSAGE_PASSING_GNN", "TIED_LOCAL_CA_LOOP", "ABC_TIED_LOOP_HARD_WALL", "PRISMION_PHASE_WAVE_LOOP"],
            [16],
            [1, 4, 8],
            ["FIXED_S"],
        )
...
def verdict(agg: dict[str, dict[str, Any]]) -> list[str]:
    ...
    prism = best(agg, {"PRISMION_PHASE_WAVE_LOOP"})
    ...
    if learned_loop and (metric_mean(learned_loop, "wall_leak_rate", 0.0) > 0.05 or metric_mean(learned_loop, "boundary_leak_rate", 0.0) > 0.05):
    ...
    if learned_loop and oracle and trunc and summary and untied:
        if (
            ...
            and score(learned_loop) >= score(untied) + 0.03
            and metric_mean(learned_loop, "settling_gain", 0.0) > 0.02
            and metric_mean(learned_loop, "wall_leak_rate", 1.0) <= 0.05
            and metric_mean(learned_loop, "locality_far_reach_rate", 1.0) <= 0.35
        ):
            labels.append("STABLE_LOOP_WAVEFRONT_POSITIVE")
    if prism and learned_loop and metric_mean(prism, "long_path_accuracy", 0.0) >= metric_mean(learned_loop, "long_path_accuracy", 0.0) + 0.05:
        labels.append("PRISMION_WAVEFRONT_POSITIVE")
...
def write_outputs(out_dir: Path, results: list[JobResult], args: argparse.Namespace, status: str, jobs: int, sample_rows: list[WaveExample]) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    lines = [
        "# STABLE_LOOP_WAVEFRONT_001 Report",
        "",
        f"- Status: `{status}`",
        ...
        "",
        "| Arm | W | S | Mode | Reach | Larger | Long | SettleGain | S<=d | FarReach | WallLeak | Overrun | Params |",
        "|---|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in sorted(agg.values(), key=lambda r: (str(r["arm"]), int(r["width"]), int(r["settling_steps"]), str(r["train_mode"]))):
        lines.append("| `{}` | `{}` | `{}` | `{}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.0f}` |".format(row["arm"], row["width"], row["settling_steps"], row["train_mode"], metric_mean(row, "reachable_accuracy"), metric_mean(row, "heldout_larger_grid_accuracy"), metric_mean(row, "long_path_accuracy"), metric_mean(row, "settling_gain"), metric_mean(row, "s_matches_distance_score"), metric_mean(row, "locality_far_reach_rate"), metric_mean(row, "wall_leak_rate"), metric_mean(row, "overrun_stability"), metric_mean(row, "parameter_count")))
...
def write_doc_result(agg: dict[str, dict[str, Any]], labels: list[str], args: argparse.Namespace, jobs: int, completed: int) -> None:
    ...
    for row in sorted(agg.values(), key=lambda r: (str(r["arm"]), int(r["width"]), int(r["settling_steps"]), str(r["train_mode"]))):
        lines.append("| `{}` | `{}` | `{}` | `{}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` |".format(row["arm"], row["width"], row["settling_steps"], row["train_mode"], metric_mean(row, "reachable_accuracy"), metric_mean(row, "heldout_larger_grid_accuracy"), metric_mean(row, "long_path_accuracy"), metric_mean(row, "settling_gain"), metric_mean(row, "wall_leak_rate"), metric_mean(row, "overrun_stability")))
    lines.extend(["", "## Claim Boundary", "", "This is a deterministic 2D wavefront/reachability probe. It is not a parser, factuality system, language benchmark, consciousness claim, or full VRAXION architecture test."])
