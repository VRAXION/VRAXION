#!/usr/bin/env python3
"""Checker for E96 Trace/Ground memory hygiene expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E96_TRACE_GROUND_MEMORY_HYGIENE_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E96_TRACE_GROUND_MEMORY_HYGIENE_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not persistent user memory" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few memory hygiene cases")
    if task.get("trace_ground_memory_hygiene") is not True:
        failures.append("trace_ground_memory_hygiene not true")
    if task.get("persistent_user_memory") is not False:
        failures.append("persistent_user_memory unexpectedly enabled")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e96_trace_ground_memory_hygiene_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_memory_hygiene_success_min",
        "adversarial_memory_hygiene_success_min",
        "validation_provenance_validity_min",
        "validation_replay_safe_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_bad_ground_promotion_max", "adversarial_stale_pollution_max", "adversarial_false_prune_max"]:
    ...
    if not any(values.get("mean_memory_hygiene_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive memory hygiene contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E96_TRACE_GROUND_MEMORY_HYGIENE_EXPANSION_CHECK",
        "root": str(root),
