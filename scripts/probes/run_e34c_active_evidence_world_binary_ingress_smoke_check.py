...
def static_runner_policy_check(runner: Path) -> list[str]:
    ...
    for token in ["torch", "tensorflow", "jax", "backward(", "AdamW", "SGD(", "RMSprop", "optim.", "loss_fn", "GradientTape", "sympy"]:
        if token in source:
...
def validate_sample(sample_dir: Path) -> dict[str, Any]:
    ...
    if schema.get("binary_ingress") is not True:
        failures.append("sample schema missing binary_ingress=true")
    ...
    if any("initial_bits" not in row or "ingress_events" not in row for row in rows):
        failures.append("sample row missing binary ingress fields")
...
def validate_target(out: Path, sample_dir: Path, write_summary: bool) -> dict[str, Any]:
    ...
    runner = Path(__file__).resolve().with_name("run_e34c_active_evidence_world_binary_ingress_smoke.py")
    ...
    binary_report = read_json(out / "binary_ingress_report.json")
    ...
    if set(task.get("actions", [])) != {"INSPECT_BITS(feature)", "ANSWER(cause)"}:
        failures.append("task report action set mismatch")
    if binary_report.get("hidden_truth_used_by_primary") is not False:
        failures.append("binary report does not assert hidden_truth_used_by_primary=false")
    if binary_report.get("semantic_lane_labels_used") is not False:
        failures.append("binary report does not assert semantic_lane_labels_used=false")
    if int(binary_report.get("packet_bits", 0)) <= 0:
        failures.append("binary report missing packet bit count")
    if not rows:
    ...
    if any("initial_bits" not in row or "ingress_events" not in row or not row.get("actions") for row in rows):
        failures.append("row-level result missing binary trace fields")
    ...
    if set(system_results) == SYSTEMS:
        for system in SYSTEMS:
            ...
            for split in SPLITS:
                compare_float(f"{system}.split_closed_loop_success.{split}", recomputed["split_closed_loop_success"][split], reported.get("split_closed_loop_success", {}).get(split), failures)
                compare_float(f"{system}.split_binary_ingress_accuracy.{split}", recomputed["split_binary_ingress_accuracy"][split], reported.get("split_binary_ingress_accuracy", {}).get(split), failures)
                compare_float(f"{system}.split_frame_sync_accuracy.{split}", recomputed["split_frame_sync_accuracy"][split], reported.get("split_frame_sync_accuracy", {}).get(split), failures)

    learned = system_results.get("learned_binary_ingress_policy", {})
    ask_all = system_results.get("ask_all_binary_until_unique", {})
    forced = system_results.get("forced_initial_binary_answer", {})
    random_control = system_results.get("random_binary_action_control", {})
    shortcut = system_results.get("first_sync_shortcut_control", {})
    if aggregate.get("decision") in {"e34c_binary_packet_confirmed_framing_bottleneck", "e34c_binary_ingress_confirmed"}:
        if learned.get("closed_loop_success", 0.0) < 0.95:
            failures.append("confirmed decision but learned success below threshold")
        if learned.get("binary_ingress_accuracy", 0.0) < 0.96:
            failures.append("confirmed decision but binary ingress below threshold")
        ...
        if shortcut.get("split_closed_loop_success", {}).get("adversarial_decoy", 1.0) >= learned.get("split_closed_loop_success", {}).get("adversarial_decoy", 0.0) - 0.20:
            failures.append("confirmed decision but first-sync adversarial control did not fail enough")
