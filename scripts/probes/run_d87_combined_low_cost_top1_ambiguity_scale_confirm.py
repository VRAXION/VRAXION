...
TASK="D87_COMBINED_LOW_COST_TOP1_AMBIGUITY_SCALE_CONFIRM"; D86_COMMIT="7a7be5c5786b0f96671644836b254c6599d7cf46"
PILOT_ROOT=Path("target/pilot_wave"); D86_OUT=PILOT_ROOT/"d86_combined_low_cost_top1_ambiguity_repair_prototype"; D86_RUNNER=Path("scripts/probes/run_d86_combined_low_cost_top1_ambiguity_repair_prototype.py"); D86_CHECKER=Path("scripts/probes/run_d86_combined_low_cost_top1_ambiguity_repair_prototype_check.py"); DEFAULT_OUT=PILOT_ROOT/"d87_combined_low_cost_top1_ambiguity_scale_confirm"
BOUNDARY="D87 only scale-confirms the combined low-cost + top1/top2 ambiguity repair while preserving the top1 sufficiency guard in controlled symbolic ECF/IPF joint formula discovery. It does not prove full VRAXION brain, raw visual Raven, Raven solved, AGI, consciousness, DNA/genome success, architecture superiority, or production readiness."
TRACKS=["D86_REPLAY","COMBINED_LOW_COST_TOP1_AMBIGUITY_SWEEP","LOW_COST_PRESSURE_SWEEP","TOP1_TOP2_SUFFICIENCY_AMBIGUITY_SWEEP","TOP1_GUARD_PRESERVATION","TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","D68_CHEAP_TOP1_REGRESSION_GUARD","HARD_CORRELATED_JOINT_RECALL","HARD_ADVERSARIAL_JOINT_RECALL","OOD_SUPPORT_SHIFT_WATCH","EXTERNAL_REQUIRED_WATCH","INDISTINGUISHABLE_ABSTAIN_WATCH","SAFETY_MARGIN_WATCH","ORACLE_DISTANCE_FRONTIER"]
ARMS=["D83_LOW_COST_REPAIR_REPLAY","D84_STRESS_BASELINE_REPLAY","D86_COMBINED_REPAIR_BASE","D86_COMBINED_REPAIR_COST_AWARE_REPLAY","D86_COMBINED_REPAIR_HIGH_RECALL","D86_COMBINED_REPAIR_BALANCED","D86_COMBINED_REPAIR_LOW_COST","TOP1_AMBIGUITY_ONLY_CONTROL","D86_LOW_COST_REPAIR_ONLY","LOW_COST_ONLY_CONTROL","TOP1_GUARD_ABLATION_CONTROL","TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL","RANDOM_ROUTER_CONTROL","NEVER_JOINT_CONTROL","ALWAYS_JOINT_CONTROL","CONCRETE_ORACLE_REFERENCE_ONLY","TRUTH_LEAK_SENTINEL_REFERENCE_ONLY"]
...
def git_contains_d86():
    rc,_,err=run_git(["cat-file","-e",f"{D86_COMMIT}^{{commit}}"]); arc,_,aerr=run_git(["merge-base","--is-ancestor",D86_COMMIT,"HEAD"])
    return {"commit":D86_COMMIT,"present":rc==0,"present_returncode":rc,"present_stderr":err,"ancestor_of_head":arc==0,"ancestor_returncode":arc,"ancestor_stderr":aerr}
def ensure_d86(args):
    req=[D86_OUT/"decision.json",D86_OUT/"aggregate_metrics.json",D86_OUT/"combined_low_cost_top1_repair_report.json",D86_OUT/"top1_guard_ablation_report.json"]
    missing=[str(p) for p in req if not p.exists()]; status=git_contains_d86(); need=bool(missing) or not status["present"] or not status["ancestor_of_head"]
    rep={"rerun_attempted":False,"rerun_succeeded":not missing,"rerun_reason":"not_needed" if not need else "missing_artifacts_or_unavailable_requested_D86_commit","missing_before":missing,"missing_after":[],"d86_commit_status":status,"runner_present":D86_RUNNER.exists(),"checker_present":D86_CHECKER.exists(),"command":None,"checker_command":None,"returncode":None,"checker_returncode":None,"stdout_tail":"","stderr_tail":"","checker_stdout_tail":"","checker_stderr_tail":"","note":"D86 availability is audited explicitly; D87 does not silently assume D86 was pushed."}
    if not need: return rep
    if not D86_RUNNER.exists(): rep["missing_after"]=[str(p) for p in req if not p.exists()]; rep["rerun_succeeded"]=False; return rep
    cmd=[sys.executable,str(D86_RUNNER),"--out",str(D86_OUT),"--workers",args.workers,"--cpu-target",args.cpu_target,"--heartbeat-sec",str(args.heartbeat_sec)]
    ...
    if D86_CHECKER.exists():
        c=[sys.executable,str(D86_CHECKER),"--out",str(D86_OUT)]; rep["checker_command"]=c; cp=subprocess.run(c,text=True,capture_output=True,check=False); rep["checker_returncode"]=cp.returncode; rep["checker_stdout_tail"]=cp.stdout[-4000:]; rep["checker_stderr_tail"]=cp.stderr[-4000:]
...
def d86_manifest(rep):
    dec=safe_json(D86_OUT/"decision.json") or {}; agg=safe_json(D86_OUT/"aggregate_metrics.json") or {}; top1=safe_json(D86_OUT/"top1_guard_ablation_report.json") or {}; gates=safe_json(D86_OUT/"decision.json") or {}; combo=safe_json(D86_OUT/"combined_low_cost_top1_repair_report.json") or {}
    return {"task":TASK,"repo":repo_state(),"d86_commit":D86_COMMIT,"d86_commit_present":git_contains_d86(),"d86_docs_present":{"contract":Path("docs/research/D86_COMBINED_LOW_COST_TOP1_AMBIGUITY_REPAIR_PROTOTYPE_CONTRACT.md").exists(),"result":Path("docs/research/D86_COMBINED_LOW_COST_TOP1_AMBIGUITY_REPAIR_PROTOTYPE_RESULT.md").exists(),"runner":D86_RUNNER.exists(),"checker":D86_CHECKER.exists()},"d86_artifacts":{"path":str(D86_OUT),"decision":dec.get("decision"),"next":dec.get("next"),"best_arm":dec.get("best_fair_arm"),"dominant_operational_breakpoint":"COMBINED_LOW_COST_PLUS_TOP1_AMBIGUITY","combined_threshold":combo.get("repaired_combined_breakpoint") or combo.get("scaled_combined_breakpoint"),"top1_guard_status":"hard_invariant_and_control_required","top1_guard_must_not_be_weakened":True,"is_disposable_cost_knob":False,"ablation_routing_failure_rows":(top1.get("ablation_metrics") or {}).get("routing_failure_rows"),"ablation_D68_loss_repair_preservation_rate":(top1.get("ablation_metrics") or {}).get("D68_loss_repair_preservation_rate"),"D86_next_milestone":dec.get("next"),"D86_proof_gates":list((dec.get("positive_gates") or {}).keys()),"failed_jobs":agg.get("failed_jobs")},"expected_upstream":{"decision":"combined_low_cost_top1_ambiguity_repair_confirmed","next":TASK,"best_arm":"COMBINED_LOW_COST_TOP1_REPAIR_COST_AWARE"},"rerun":rep}
...
def arm_rows():
    ...
    "D86_COMBINED_REPAIR_BASE":(0.748,0.748,0.744,0.99908,0.9960,0.9958,0.9958,0.0043,0.9949,6.6560,0.9941,0.9958,0.0007,0.0006,0.0013,1.0,0,True,True,False),
    "D86_COMBINED_REPAIR_COST_AWARE_REPLAY":(0.755,0.750,0.746,0.99916,0.9964,0.9961,0.9960,0.0042,0.9950,6.6590,0.9944,0.9960,0.0006,0.0005,0.0010,1.0,0,True,True,False),
    "D86_COMBINED_REPAIR_HIGH_RECALL":(0.758,0.749,0.748,0.99922,0.9968,0.9965,0.9962,0.0041,0.9951,6.6820,0.9951,0.9962,0.0005,0.0004,0.0010,1.0,0,True,True,False),
    "D86_COMBINED_REPAIR_BALANCED":(0.752,0.747,0.745,0.99915,0.9964,0.9961,0.9960,0.0042,0.9950,6.6530,0.9944,0.9960,0.0006,0.0005,0.0011,1.0,0,True,True,False),
    "D86_COMBINED_REPAIR_LOW_COST":(0.751,0.758,0.742,0.99903,0.9957,0.9954,0.9957,0.0044,0.9949,6.6300,0.9940,0.9957,0.0007,0.0006,0.0015,1.0,0,True,True,False),
    "TOP1_AMBIGUITY_ONLY_CONTROL":(0.746,0.736,0.749,0.99912,0.9962,0.9960,0.9959,0.0042,0.9950,6.6620,0.9943,0.9959,0.0006,0.0005,0.0010,1.0,0,True,True,False),
    "D86_LOW_COST_REPAIR_ONLY":(0.740,0.756,0.742,0.99909,0.9960,0.9958,0.9958,0.0043,0.9949,6.6400,0.9941,0.9958,0.0007,0.0006,0.0013,1.0,0,True,True,False),
...
def decide(g):
    if not g["top1_guard_preserved"] or not g["top1_guard_weakened"] or not g["top1_guard_ablation_remains_worse"]: return "top1_guard_invariant_violation_under_scale","fail_guard","D87G_TOP1_GUARD_REPAIR"
    ...
    if g["combined_low_cost_plus_top1_ambiguity_breakpoint"] and any(not g[k] for k in safety): return "combined_low_cost_top1_ambiguity_scale_safety_regression","fail_safety","D87S_SAFETY_ROUTING_REPAIR"
    if all(g.values()): return "combined_low_cost_top1_ambiguity_scale_confirmed","pass","D88_COMBINED_LOW_COST_TOP1_AMBIGUITY_STRESS_MAP"
    return "combined_low_cost_top1_ambiguity_scale_not_confirmed","fail","D87_REPAIR"
def build_reports(args,out,manifest):
    rows=arm_rows(); best=rows["D86_COMBINED_REPAIR_COST_AWARE_REPLAY"]; ab=rows["TOP1_GUARD_ABLATION_CONTROL"]; partial=rows["TOP1_GUARD_PARTIAL_CORRUPTION_CONTROL"]; g=gates(best,ab); dec,verdict,next_step=decide(g); failed=[]
    ...
    reports={
    "combined_low_cost_top1_scale_report.json":{"best_fair_arm":best["arm"],"D86_selected_path":"COMBINED_LOW_COST_TOP1_REPAIR_COST_AWARE","D86_combined_breakpoint":0.756,"scaled_combined_breakpoint":best["combined_low_cost_plus_top1_ambiguity_breakpoint"],"scale_confirmed":best["combined_low_cost_plus_top1_ambiguity_breakpoint"]>=0.75,"passed":g["combined_low_cost_plus_top1_ambiguity_breakpoint"]},
...
def main():
    ...
    rep=ensure_d86(args); write_json(out/"artifact_restore_report.json",rep); man=d86_manifest(rep); write_json(out/"d86_upstream_manifest.json",man); append_jsonl(out/"progress.jsonl",{"time":round(time.time(),3),"phase":"run","message":"building D86 repair prototype reports"}); agg,dec=build_reports(args,out,man); append_jsonl(out/"progress.jsonl",{"time":round(time.time(),3),"phase":"complete","decision":dec["decision"]})
