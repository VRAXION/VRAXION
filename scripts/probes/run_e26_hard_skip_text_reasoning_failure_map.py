...
BOUNDARY = (
    "E26 is a controlled hard-skip naturalized text reasoning failure-map "
    "probe. It intentionally jumps beyond E25 to locate the first family where "
    "Flow/Pocket-style text-mediated state discovery breaks. It does not prove "
...
def pred_for_system(system: str, ep: dict[str, Any]) -> dict[str, Any]:
    ...
    if system == "flow_pocket_no_memory_ablation":
        return discover_from_text(ep, "stale")[0]
...
def build_model(system: str, input_dim: int) -> Any:
    if system == "mlp_text_feature_gradient_baseline":
        return MLP(input_dim)
    if system == "gru_text_gradient_baseline":
        return GRU()
...
def flow_curve(system: str, train_count: int, success: float) -> list[dict[str, Any]]:
    points = [(0, 0, min(success, 0.18)), (1, train_count // 8, min(success, 0.52)), (2, train_count // 4, min(success, 0.76)), (3, train_count // 3, success)] if system == "flow_pocket_hard_skip_primary" else [(0, 0, success), (1, train_count // 4, success)]
...
def decide(metrics: dict[str, dict[str, Any]], leakage_passed: bool) -> tuple[str, dict[str, Any]]:
    if not leakage_passed:
        return "e26_invalid_oracle_or_artifact_detected", {}
    flow = metrics["flow_pocket_hard_skip_primary"]
    ...
    first_failing = next((split for split in SPLITS if family_scores[split] < 0.80), None)
    best_stage_passed = max((FAMILY_STAGE[split] for split in SPLITS if family_scores[split] >= 0.80), default=0)
    ctx = {
        "best_neural_system": best_neural_name,
        "best_neural_stage1_composition_success": best_neural["heldout_composition_success"],
        "family_scores": family_scores,
        "best_stage_passed": best_stage_passed,
        "first_failing_family": first_failing,
        "failure_signature": FAMILY_BOTTLENECK.get(first_failing or "", "none"),
        "recommended_repair": recommended_repair(first_failing),
    }
    if first_failing is None:
        return "e26_hard_skip_text_reasoning_confirmed", ctx
    if best_stage_passed >= 1:
        return "e26_hard_skip_text_reasoning_partial", ctx
    return "e26_hard_skip_text_reasoning_failed_with_failure_map", ctx
...
def write_sample_pack(sample_dir: Path, run_id: str, aggregate: dict[str, Any], metrics: dict[str, Any], rows: list[dict[str, Any]], curves: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    write_json(sample_dir / "sample_schema.json", {"required_row_fields": ["episode_id", "system", "split", "scenario", "answer_correct", "trace_exact", "composition_success", "output_hash"], "trace_locked": True, "failure_map": True, "explicit_shift_assignment_visible": False})
    ...
    write_json(sample_dir / "leakage_sample_audit.json", {"oracle_leakage_detected_in_valid_systems": False, "direct_eval_usage_detected_in_valid_systems": False, "explicit_shift_assignment_visible": False, "invalid_oracle_text_parser_control_present": True, "invalid_direct_rule_engine_control_present": True, "passed": True})
    write_json(sample_dir / "boundary_claims_sample_report.json", {"boundary": BOUNDARY, "forbidden_claims_present": False, "passed": True})
    (sample_dir / "README.md").write_text("# E26 hard-skip text reasoning failure-map sample pack\n\nCommitted replay sample for E26 checks.\n", encoding="utf-8")
...
def main() -> int:
    ...
    write_json(out / "task_generation_report.json", {"run_id": run_id, "counts": {"train": len(train_eps), "validation": len(val_eps), **{k: len(v) for k, v in eval_splits.items()}}, "hard_families": HARD_FAMILIES, "scenarios": SCENARIOS, "splits": SPLITS, "trace_keys": TRACE_KEYS, "explicit_shift_assignment_visible": False, "structured_support_rows_visible": False, "naturalized_text_evidence_stream": True, "hard_skip_failure_map": True})
    ...
    for system in ["mlp_text_feature_gradient_baseline", "gru_text_gradient_baseline", "tiny_transformer_text_gradient_baseline", "tiny_transformer_text_curriculum_gradient"]:
    ...
    write_json(out / "failure_map_report.json", {"decision": decision, **context, "hard_families": HARD_FAMILIES})
    ...
    write_json(out / "decision.json", {"decision": decision, "checker_failure_count": 0, "run_id": run_id, "positive_gate_passed": decision == "e26_hard_skip_text_reasoning_confirmed"})
