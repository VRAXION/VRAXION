#!/usr/bin/env python3
"""Checker for E7A3 neural matrix substrate harness artifacts."""
...
RUNNER = "scripts/probes/run_e7a3_neural_matrix_substrate_harness.py"
CHECKER = "scripts/probes/run_e7a3_neural_matrix_substrate_harness_check.py"
SYSTEMS = (
    "float_mlp_backprop",
...
BASE_REQUIRED = (
    ...
    "report.md",
    "progress.jsonl",
    "e7a3_row_level_eval_sample_heldout.json",
    "e7a3_row_level_eval_sample_ood.json",
    "e7a3_row_level_eval_sample_counterfactual.json",
    "e7a3_row_level_eval_sample_adversarial.json",
)
HASH_ARTIFACTS = (
    "e7a3_task_generation_report.json",
    "e7a3_size_sweep_report.json",
...
def required_artifacts(out: Path) -> list[str]:
    manifest = load_json(out / "e7a3_backend_manifest.json")
    widths = [int(width) for width in manifest.get("widths", [])]
    names = list(BASE_REQUIRED)
    for system in SYSTEMS:
        for width in widths:
            names.append(f"e7a3_candidate_{system}_width{width}_summary.json")
            if system in MUTATION_SYSTEMS:
                names.append(f"e7a3_candidate_{system}_width{width}_initial.json")
                names.append(f"e7a3_candidate_{system}_width{width}_final.json")
                names.append(f"e7a3_parameter_diff_{system}_width{width}.json")
                names.append(f"e7a3_mutation_history_{system}_width{width}.json")
            if system == "float_mlp_backprop":
                names.append(f"e7a3_training_history_float_mlp_backprop_width{width}.json")
    return names
...
def ast_scan() -> list[str]:
    ...
    for token in SYSTEMS + ("FloatMLP", "ProcessPoolExecutor", "integer_matrix_hidden_replacement_mutation"):
    ...
    if not adamw_seen:
        failures.append("ADAMW_MISSING_FOR_FLOAT_BACKPROP_REFERENCE")
    mutation_functions = {"run_integer_mutation_system", "mutate_integer_candidate", "integer_forward", "score_integer"}
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Call):
                    ...
                    if name in forbidden_calls:
                        failures.append(f"MUTATION_ONLY_BACKPROP_CALL:{node.name}:{name}")
...
def check_artifacts(out: Path) -> list[str]:
    failures: list[str] = []
    if not (out / "e7a3_backend_manifest.json").exists():
        return ["MISSING_ARTIFACT:e7a3_backend_manifest.json"]
    ...
    manifest = load_json(out / "e7a3_backend_manifest.json")
    task = load_json(out / "e7a3_task_generation_report.json")
    aggregate = load_json(out / "aggregate_metrics.json")
    substrate = load_json(out / "e7a3_substrate_comparison_report.json")
    matrix_size = load_json(out / "e7a3_matrix_size_report.json")
    mutation = load_json(out / "e7a3_mutation_history.json")
    training = load_json(out / "e7a3_training_history.json")
    audit = load_json(out / "e7a3_no_synthetic_metric_audit.json")
    replay = load_json(out / "e7a3_deterministic_replay_report.json")
    ...
    if manifest.get("cpu_mutation_and_gpu_gradient_overlap_supported") is not True:
    ...
    if task.get("input_dim") != 10 or task.get("class_count") != 4:
        failures.append("TASK_DIMENSIONS_MISMATCH")
    ...
    if set(aggregate.get("smallest_passing_width", {})) != set(SYSTEMS):
        failures.append("SMALLEST_PASSING_WIDTH_SYSTEMS_MISMATCH")
    if set(matrix_size.get("rows", {})) != set(SYSTEMS):
        failures.append("MATRIX_SIZE_SYSTEMS_MISMATCH")
    if set(substrate.get("systems", [])) != set(SYSTEMS):
        failures.append("SUBSTRATE_SYSTEMS_MISMATCH")

    if mutation.get("all_mutation_runs_have_accept_reject_rollback") is not True:
        failures.append("MUTATION_ACCEPT_REJECT_ROLLBACK_AGGREGATE_BAD")
    for system in MUTATION_SYSTEMS:
        for width in widths:
            hist = load_json(out / f"e7a3_mutation_history_{system}_width{width}.json")
            if hist.get("mutation_attempt_count", 0) < 1:
                failures.append(f"NO_MUTATION_ATTEMPTS:{system}:{width}")
            if hist.get("accepted_mutation_count", 0) < 1:
                failures.append(f"NO_ACCEPTED_MUTATIONS:{system}:{width}")
            if hist.get("rejected_mutation_count", 0) < 1:
                failures.append(f"NO_REJECTED_MUTATIONS:{system}:{width}")
            if hist.get("rejected_mutation_count") != hist.get("rollback_count"):
                failures.append(f"ROLLBACK_MISMATCH:{system}:{width}")
            if len(hist.get("history", [])) != manifest.get("settings", {}).get("generations"):
                failures.append(f"GENERATION_COUNT_MISMATCH:{system}:{width}")
            diff = load_json(out / f"e7a3_parameter_diff_{system}_width{width}.json")
            if diff.get("actual_parameter_diff_found") is not True:
                failures.append(f"NO_PARAMETER_DIFF:{system}:{width}")

    for width in widths:
        hist = load_json(out / f"e7a3_training_history_float_mlp_backprop_width{width}.json")
        if len(hist.get("history", [])) != manifest.get("settings", {}).get("gradient_epochs"):
            failures.append(f"GRADIENT_EPOCH_COUNT_MISMATCH:{width}")
    if set(training.get("systems", {}).get("float_mlp_backprop", {})) != {str(width) for width in widths}:
        failures.append("TRAINING_HISTORY_WIDTHS_MISMATCH")
    ...
    if audit.get("gradient_backprop_used_only_for_float_reference") is not True:
        failures.append("GRADIENT_SCOPE_AUDIT_BAD")
    if audit.get("integer_mutation_systems_used_optimizer_or_backprop") is not False:
        failures.append("MUTATION_BACKPROP_AUDIT_BAD")
    ...
    for event in ("startup", "task_generated", "gradient_epoch", "mutation_generation", "final_artifacts_written"):
        if event not in events:
            failures.append(f"PROGRESS_EVENT_MISSING:{event}")
    if manifest.get("settings", {}).get("execution_mode") == "parallel" and "cpu_mutation_lane_started" not in events:
        failures.append("PARALLEL_CPU_MUTATION_PROGRESS_MISSING")

    for split in ("heldout", "ood", "counterfactual", "adversarial"):
        sample = load_json(out / f"e7a3_row_level_eval_sample_{split}.json")
