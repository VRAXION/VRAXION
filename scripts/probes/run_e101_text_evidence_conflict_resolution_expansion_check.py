#!/usr/bin/env python3
"""Checker for E101 text evidence conflict resolution artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E101_TEXT_EVIDENCE_CONFLICT_RESOLUTION_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E101_TEXT_EVIDENCE_CONFLICT_RESOLUTION_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not natural-language reasoning" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few conflict cases")
    if task.get("text_evidence_conflict_resolution") is not True:
        failures.append("text_evidence_conflict_resolution not true")
    if task.get("natural_language_reasoning") is not False:
        failures.append("natural language reasoning unexpectedly enabled")
    if task.get("open_domain_fact_checking") is not False:
        failures.append("open-domain fact checking unexpectedly enabled")
    if task.get("direct_span_commit_allowed") is not False:
        failures.append("direct span commit unexpectedly allowed")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e101_text_evidence_conflict_resolution_expansion_confirmed":
    ...
    if not any(values.get("mean_resolution_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive resolution contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E101_TEXT_EVIDENCE_CONFLICT_RESOLUTION_EXPANSION_CHECK",
        "root": str(root),
