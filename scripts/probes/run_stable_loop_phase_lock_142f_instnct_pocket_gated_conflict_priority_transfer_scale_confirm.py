#!/usr/bin/env python3
"""142F helper-only conflict/priority transfer scale confirm."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_142F_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_SCALE_CONFIRM"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_142f_instnct_pocket_gated_conflict_priority_transfer_scale_confirm/smoke")
DEFAULT_142A_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_142a_instnct_pocket_gated_conflict_priority_transfer_probe/smoke")
HELPER_PATH = REPO_ROOT / "scripts/probes/shared_raw_generation_helper.py"
RUNNER_PATH = Path(__file__).resolve()
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_142f_instnct_pocket_gated_conflict_priority_transfer_scale_confirm_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_conflict_priority_scale_main"
ABLATION_ARM = "instnct_conflict_priority_scale_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_priority_resolved_final_all_markers"
POSITIVE_NEXT = "142Z_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_NEXT_DECISION_PLAN"
PRIORITY_INVERSION_PAIR_COUNT_FLOOR = 1000
...
BOUNDARY_TEXT = (
    "142F tests constrained helper/backend conflict-priority final selection only. "
...
def require_142a(root: Path) -> dict[str, Any]:
    required = [
        "decision.json",
        ...
        "priority_default_shortcut_report.json",
    ]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 142A artifacts: {missing}")
    ...
    priority = read_json(root / "priority_rule_manifest.json")
    pairs = read_json(root / "conflict_pair_manifest.json")
    if decision.get("decision") != "instnct_pocket_gated_conflict_priority_transfer_probe_positive":
        raise RuntimeError(f"bad 142A decision: {decision.get('decision')}")
    if decision.get("next") != "142F_INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_SCALE_CONFIRM":
        raise RuntimeError(f"bad 142A next: {decision.get('next')}")
    exact = {
    ...
    for key, expected in exact.items():
        if metric_value(metrics, key, "pocket_ablation_delta", "direct_POCKET_VALUE_rate") != expected:
            raise RuntimeError(f"bad 142A metric {key}: {metrics.get(key)} != {expected}")
    if metrics.get("deterministic_replay_passed") is not True:
        raise RuntimeError("142A deterministic replay did not pass")
    required_audit = {
    ...
    for key, expected in required_audit.items():
        if audit.get(key) != expected:
            raise RuntimeError(f"bad 142A helper audit {key}: {audit.get(key)} != {expected}")
    if seed_gate.get("passed") is not True or family_gate.get("passed") is not True:
        raise RuntimeError("142A per-seed or per-family hardened gates did not pass")
    if priority.get("has_a_b_table_rule_winners") is not True:
        raise RuntimeError("142A winner source coverage is incomplete")
    if pairs.get("priority_inversion_pair_rate", 0.0) < 0.50:
        raise RuntimeError("142A priority inversion pair rate is too low")
    return {
        "root": rel(root),
        ...
        "winner_source_counts": priority.get("winner_source_counts"),
        "priority_inversion_pair_count": pairs.get("priority_inversion_pair_count"),
        ...
        for family_index, family in enumerate(FAMILIES):
            for group_index in range(groups_per_family):
                ...
                for slot in range(group_size):
                    ...
                    row = {
                        "row_id": f"142F_eval_{row_index:07d}",
                        "seed": seed,
                        "family": family,
...
def row_score(row: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_142F_scoring_result_v1",
        "arm": result["arm"],
        "row_id": row["row_id"],
        "seed": row["seed"],
        "family": row["family"],
...
def metrics_for(scored: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    always_a = rate(sum(1 for item in scored if item.get("generated_source") == "field_a" and item["expected_winner_source"] != "field_a"), total)
    always_b = rate(sum(1 for item in scored if item.get("generated_source") == "field_b" and item["expected_winner_source"] != "field_b"), total)
    table_default = rate(sum(1 for item in scored if item.get("generated_source") == "table" and item["expected_winner_source"] != "table"), total)
    rule_default = rate(sum(1 for item in scored if item.get("generated_source") == "rule" and item["expected_winner_source"] != "rule"), total)
    priority_default = 0.0 if final_accuracy == 1.0 else dominant_rate
    return {
        "row_count": total,
        ...
        "priority_default_shortcut_rate": priority_default,
        "always_a_shortcut_rate": always_a,
        "always_b_shortcut_rate": always_b,
        "table_default_shortcut_rate": table_default,
        "rule_default_shortcut_rate": rule_default,
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for item in scored:
        ...
        by_winner[item["expected_winner_source"]].append(item)
    ...
    for group_id, items in sorted(by_group.items()):
        ...
        group_rows.append(
            {
                "schema_version": "phase_142F_contrast_group_result_v1",
                "arm": arm,
                "contrast_group_id": group_id,
    ...
    for pair_id, items in sorted(by_pair.items()):
        ...
        pair_rows.append(
            {
                "schema_version": "phase_142F_priority_inversion_pair_v1",
                "arm": arm,
    ...
    metrics["priority_default_shortcut_rate"] = 0.0 if metrics["final_answer_accuracy"] == 1.0 else metrics["dominant_generated_source_rate"]
    metrics["always_a_shortcut_rate"] = rate(sum(1 for item in scored if item.get("generated_source") == "field_a" and item["expected_winner_source"] != "field_a"), len(scored))
    metrics["always_b_shortcut_rate"] = rate(sum(1 for item in scored if item.get("generated_source") == "field_b" and item["expected_winner_source"] != "field_b"), len(scored))
    metrics["table_default_shortcut_rate"] = rate(sum(1 for item in scored if item.get("generated_source") == "table" and item["expected_winner_source"] != "table"), len(scored))
    metrics["rule_default_shortcut_rate"] = rate(sum(1 for item in scored if item.get("generated_source") == "rule" and item["expected_winner_source"] != "rule"), len(scored))
    metrics["wrong_priority_field_selected"] = metrics["wrong_priority_field_rate"] > 0.0
    metrics["priority_default_shortcut_detected"] = any(
        metrics[key] > 0.0
        for key in [
...
def priority_rule_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_142F_priority_rule_manifest_v1",
        "row_count": len(rows),
...
def conflict_pair_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_142F_conflict_pair_manifest_v1",
...
def winner_distribution_report(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    total = len(rows)
    rates = {
        "a_wins_rate": rate(counts.get("field_a", 0), total),
        "b_wins_rate": rate(counts.get("field_b", 0), total),
        "table_wins_rate": rate(counts.get("table", 0), total),
        "rule_wins_rate": rate(counts.get("rule", 0), total),
    }
    balanced = all(value >= 0.15 for value in rates.values())
    return {
        "schema_version": "phase_142F_winner_distribution_report_v1",
        "row_count": total,
        "winner_source_counts": dict(sorted(counts.items())),
        **rates,
        "winner_distribution_balanced": balanced,
        "minimum_winner_rate_gate": 0.15,
        "passed": balanced,
    }


def per_winner_gate_report(main_metrics: dict[str, Any]) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    for winner in ["field_a", "field_b", "table", "rule"]:
        metrics = main_metrics["per_winner"].get(winner, {})
        failures: list[str] = []
        if metrics.get("final_answer_accuracy", 0.0) < 0.85:
            failures.append("final_answer_accuracy")
        if metrics.get("priority_rule_accuracy", 0.0) < 0.85:
            failures.append("priority_rule_accuracy")
        if metrics.get("conflict_resolution_accuracy", 0.0) < 0.85:
            failures.append("conflict_resolution_accuracy")
        if metrics.get("wrong_priority_field_rate") != 0.0:
            failures.append("wrong_priority_field_rate")
        if metrics.get("priority_default_shortcut_rate") != 0.0:
            failures.append("priority_default_shortcut_rate")
        rows.append({"winner_source": winner, "main": metrics, "passed": not failures, "failures": failures})
    return {"schema_version": "phase_142F_per_winner_gate_report_v1", "passed": all(row["passed"] for row in rows), "winners": rows}
...
def shortcut_report(main_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_142F_shortcut_report_v1",
        **values,
...
def run_controls(helper: Any, checkpoints: dict[str, tuple[Path, str]], max_new_tokens: int, request_audit: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    controls = [
        ...
        ("ALWAYS_A_SHORTCUT_CONTROL", "a_only_candidate", "rule"),
        ("ALWAYS_B_SHORTCUT_CONTROL", "b_only_candidate", "rule"),
    ...
    report = {
        "schema_version": "phase_142F_priority_control_report_v1",
...
def seed_gate_report(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    for seed, metrics in sorted(main_metrics["per_seed"].items()):
        ...
        failures = []
        if metrics["final_answer_accuracy"] < 0.85:
            failures.append("final_answer_accuracy")
        if metrics["priority_rule_accuracy"] < 0.85:
            failures.append("priority_rule_accuracy")
        if metrics["conflict_resolution_accuracy"] < 0.85:
            failures.append("conflict_resolution_accuracy")
        if metrics["pocket_writeback_rate"] < 0.90:
    ...
    return {"schema_version": "phase_142F_per_seed_gate_report_v1", "passed": all(row["passed"] for row in rows), "seeds": rows}
...
def family_gate_report(main_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema_version": "phase_142F_per_family_gate_report_v1", "passed": all(row["passed"] for row in rows), "families": rows}
...
def choose_decision(
    ...
    if not infra_passed or deterministic is not True or control_report.get("controls_failed") is not True:
        ...
        decision = "conflict_resolution_scale_failure"; verdict = "INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_FAILS"; next_step = "142C_CONFLICT_RESOLUTION_FAILURE_ANALYSIS"
    ...
    elif shortcut["passed"] is not True or main["priority_default_shortcut_rate"] > 0.0:
        ...
        comparison["priority_inversion_pair_count"] < comparison["priority_inversion_pair_count_floor"]
        or main["priority_inversion_accuracy"] < 0.85
        or main["same_template_opposite_winner_accuracy"] < 0.85
    ):
    ...
    elif ablation["final_answer_accuracy"] > 0.05 or comparison["pocket_ablation_delta_final_answer_accuracy"] < 0.85:
    ...
    elif main["final_answer_accuracy"] < 0.90 or main["priority_rule_accuracy"] < 0.90 or main["conflict_resolution_accuracy"] < 0.90 or main["pocket_writeback_rate"] < 0.95 or main["contrast_group_accuracy"] < 0.90:
        decision = "conflict_resolution_scale_failure"; verdict = "INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_FAILS"; next_step = "142C_CONFLICT_RESOLUTION_FAILURE_ANALYSIS"
    elif seed_report["passed"] is not True or family_report["passed"] is not True or winner_report["passed"] is not True or distribution_report["passed"] is not True:
        decision = "conflict_resolution_scale_failure"; verdict = "INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_FAILS"; next_step = "142C_CONFLICT_RESOLUTION_FAILURE_ANALYSIS"
    else:
        decision = "instnct_pocket_gated_conflict_priority_transfer_scale_confirmed"; verdict = "INSTNCT_POCKET_GATED_CONFLICT_PRIORITY_TRANSFER_SCALE_CONFIRMED"; next_step = POSITIVE_NEXT
    return {
        "schema_version": "phase_142F_decision_v1",
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "clean_negative_valid": True,
        "pocket_mechanism_claimed": decision == "instnct_pocket_gated_conflict_priority_transfer_scale_confirmed",
        "conflict_priority_transfer_scale_confirmed": decision == "instnct_pocket_gated_conflict_priority_transfer_scale_confirmed",
...
- always A shortcut rate: `{comparison['always_a_shortcut_rate']}`
- always B shortcut rate: `{comparison['always_b_shortcut_rate']}`
- table default shortcut rate: `{comparison['table_default_shortcut_rate']}`
- rule default shortcut rate: `{comparison['rule_default_shortcut_rate']}`
- winner distribution balanced: `{comparison['winner_distribution_balanced']}`
...
def main() -> int:
    ...
    parser.add_argument("--upstream-142a-root", type=Path, default=DEFAULT_142A_ROOT)
    parser.add_argument("--seeds", default="4301,4302,4303,4304")
    ...
    write_json(out / "queue.json", {"schema_version": "phase_142F_queue_v1", "milestone": MILESTONE, "status": "running"})

    seeds = [int(item) for item in args.seeds.split(",") if item.strip()]
    upstream_142a = require_142a(resolve_repo_path(args.upstream_142a_root))
    write_json(out / "upstream_142a_manifest.json", upstream_142a)
    append_progress(out, "upstream verification", upstream_142a=upstream_142a["decision"])

    config = {
        "schema_version": "phase_142F_eval_config_v1",
        "milestone": MILESTONE,
        "boundary": BOUNDARY_TEXT,
        "train_allowed": False,
    ...
    helper = load_helper()
    provenance = {
        "schema_version": "phase_142F_helper_provenance_v1",
    ...
    winner_distribution = winner_distribution_report(rows)
    ...
    write_json(out / "winner_distribution_report.json", winner_distribution)
    write_json(out / "conflict_priority_eval_manifest.json", {"schema_version": "phase_142F_eval_manifest_v1", "row_count": len(rows), "seeds": seeds, "family_count": len(family_counts), "families": sorted(family_counts), "scaffold_variant_count": len(scaffold_counts), "groups_per_family": args.groups_per_family, "group_size": args.group_size, "row_hash": stable_hash(rows), "marker_audit": marker, "priority_rule_manifest": priority_manifest, "conflict_pair_manifest": conflict_manifest})
    append_progress(out, "conflict-priority row build", row_count=len(rows), inversion_pair_rate=conflict_manifest["priority_inversion_pair_rate"], scaffold_variant_count=len(scaffold_counts))
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_142F_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_142F_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_142F_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {"schema_version": "phase_142F_generated_before_scoring_report_v1", "passed": True, "generated_text_produced_before_scoring": True, "all_helper_requests_allowed_keys_only": all(set(row["helper_request"]) == ALLOWED_HELPER_KEYS for row in main_results + ablation_results + replay), "expected_or_scorer_metadata_in_helper_requests": False}
    leakage_report = {"schema_version": "phase_142F_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False}
    ...
    comparison = {
        "schema_version": "phase_142F_arm_comparison_v1",
        ...
        "priority_inversion_pair_count": conflict_manifest["priority_inversion_pair_count"],
        "priority_inversion_pair_count_floor": PRIORITY_INVERSION_PAIR_COUNT_FLOOR,
        ...
        "always_a_shortcut_rate": main_metrics["always_a_shortcut_rate"],
        "always_b_shortcut_rate": main_metrics["always_b_shortcut_rate"],
        "table_default_shortcut_rate": main_metrics["table_default_shortcut_rate"],
        "rule_default_shortcut_rate": main_metrics["rule_default_shortcut_rate"],
        ...
        "a_wins_rate": winner_distribution["a_wins_rate"],
        "b_wins_rate": winner_distribution["b_wins_rate"],
        "table_wins_rate": winner_distribution["table_wins_rate"],
        "rule_wins_rate": winner_distribution["rule_wins_rate"],
        "winner_distribution_balanced": winner_distribution["winner_distribution_balanced"],
        "deterministic_replay_passed": deterministic,
        "architecture_superiority_claimed": False,
        "value_grounding_claimed": False,
    }
    helper_request_audit = {
        "schema_version": "phase_142F_helper_request_audit_v1",
    ...
    alias_report = {
        "schema_version": "phase_142F_canonical_metric_alias_report_v1",
        "canonical_metrics": {
            key: comparison[key]
            for key in [
                "direct_pocket_value_marker_rate",
    ...
    winner_report = per_winner_gate_report(main_metrics)
    shortcuts = shortcut_report(main_metrics)
    aggregate_metrics = {
        "schema_version": "phase_142F_aggregate_metrics_v1",
        **comparison,
    ...
    write_json(out / "conflict_priority_transfer_metrics.json", {"schema_version": "phase_142F_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    ...
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_142F_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_142F_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    write_json(out / "per_winner_metrics.json", {"schema_version": "phase_142F_per_winner_metrics_v1", "main": main_metrics["per_winner"]})
    write_json(out / "per_seed_gate_report.json", seed_report)
    write_json(out / "per_family_gate_report.json", family_report)
    write_json(out / "per_winner_gate_report.json", winner_report)
    write_json(out / "shortcut_report.json", shortcuts)
    inversion_report = {"schema_version": "phase_142F_priority_inversion_report_v1", "priority_inversion_accuracy": main_metrics["priority_inversion_accuracy"], "same_template_opposite_winner_accuracy": main_metrics["same_template_opposite_winner_accuracy"], "priority_inversion_pair_count": conflict_manifest["priority_inversion_pair_count"], "priority_inversion_pair_count_floor": PRIORITY_INVERSION_PAIR_COUNT_FLOOR, "priority_inversion_pair_rate": conflict_manifest["priority_inversion_pair_rate"], "same_template_opposite_winner_pairs": conflict_manifest["same_template_opposite_winner_pairs"], "passed": main_metrics["priority_inversion_accuracy"] >= 0.85 and conflict_manifest["priority_inversion_pair_count"] >= PRIORITY_INVERSION_PAIR_COUNT_FLOOR}
    ...
    write_json(out / "same_template_opposite_winner_report.json", {"schema_version": "phase_142F_same_template_opposite_winner_report_v1", "same_template_opposite_winner_accuracy": main_metrics["same_template_opposite_winner_accuracy"], "same_template_opposite_winner_pairs": conflict_manifest["same_template_opposite_winner_pairs"], "same_template_opposite_winner_pair_floor": PRIORITY_INVERSION_PAIR_COUNT_FLOOR, "passed": main_metrics["same_template_opposite_winner_accuracy"] >= 0.85 and conflict_manifest["same_template_opposite_winner_pairs"] >= PRIORITY_INVERSION_PAIR_COUNT_FLOOR})
    write_json(out / "wrong_priority_field_report.json", {"schema_version": "phase_142F_wrong_priority_field_report_v1", "wrong_priority_field_rate": main_metrics["wrong_priority_field_rate"], "wrong_priority_field_selected": main_metrics["wrong_priority_field_selected"]})
    write_json(out / "priority_default_shortcut_report.json", {"schema_version": "phase_142F_priority_default_shortcut_report_v1", "priority_default_shortcut_rate": main_metrics["priority_default_shortcut_rate"], "priority_default_shortcut_detected": main_metrics["priority_default_shortcut_detected"], "always_a_shortcut_rate": main_metrics["always_a_shortcut_rate"], "always_b_shortcut_rate": main_metrics["always_b_shortcut_rate"], "table_default_shortcut_rate": main_metrics["table_default_shortcut_rate"], "rule_default_shortcut_rate": main_metrics["rule_default_shortcut_rate"], "dominant_generated_source": main_metrics["dominant_generated_source"], "dominant_generated_source_rate": main_metrics["dominant_generated_source_rate"]})
    ...
    decision = choose_decision(main_metrics, ablation_metrics, comparison, control_report, deterministic, selected["candidate"], infra_passed, seed_report, family_report, winner_report, winner_distribution, shortcuts)
    write_json(out / "decision.json", decision)
    summary = {"schema_version": "phase_142F_summary_v1", "milestone": MILESTONE, "status": "complete", "boundary": BOUNDARY_TEXT, "upstream_142a": upstream_142a, "metrics": comparison, "aggregate_metrics": aggregate_metrics, "selection": selection, "helper_request_audit": helper_request_audit, "winner_distribution_report": winner_distribution, "per_seed_gate_report": seed_report, "per_family_gate_report": family_report, "per_winner_gate_report": winner_report, "shortcut_report": shortcuts, "canary_passed": canary["passed"], "ast_shortcut_scan_passed": ast_report["passed"], "generated_before_scoring_passed": generated_report["passed"], "leakage_rejected": leakage_report["leakage_rejected"], "controls_failed": control_report["controls_failed"], **decision}
    ...
    write_json(out / "queue.json", {"schema_version": "phase_142F_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
