...
def validate_sample(sample_dir: Path, expected_run_id: str | None = None) -> dict[str, Any]:
    ...
    loc = read_json(sample_dir / "bottleneck_localization_sample.json")
    ...
    if set(schema.get("rungs", [])) != set(RUNGS):
        failures.append("sample rung schema mismatch")
    if "first_breakpoint" not in ladder:
        failures.append("sample ladder missing first breakpoint")
    if "baseline_heldout_resolution" not in loc:
        failures.append("sample localization missing baseline metric")
...
def validate_target(out: Path, sample_dir: Path | None, runner_path: Path | None) -> dict[str, Any]:
    ...
    if set(backend.get("primary_systems", [])) != PRIMARY_SYSTEMS:
        failures.append("primary system set mismatch")
    if set(backend.get("diagnostic_oracle_systems", [])) != DIAGNOSTIC_SYSTEMS:
        failures.append("diagnostic oracle set mismatch")
    ...
    if task.get("rungs") != RUNGS:
        failures.append("rung order mismatch")
    ...
    for system in SYSTEMS:
        ...
        for rung in RUNGS:
            ...
            for key in ["heldout_resolution_success", "heldout_action_accuracy", "heldout_trace_exact", "heldout_trace_bit_accuracy"]:
                assert_close(f"{system}.{rung}.{key}", rec.get("rung_metrics", {}).get(rung, {}).get(key, 0.0), stored.get("rung_metrics", {}).get(rung, {}).get(key), failures)
    oracle_reports = [read_json(out / "oracle_ingress_report.json"), read_json(out / "oracle_evidence_span_report.json")]
    if not all(report.get("diagnostic_only") for report in oracle_reports):
        failures.append("oracle reports must be marked diagnostic_only")
    sample_result = None
    if sample_dir:
...
def main() -> int:
    ...
    runner_path = Path(__file__).with_name("run_e31_breakpoint_ladder_and_bottleneck_localization.py")
