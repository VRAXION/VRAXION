#!/usr/bin/env python3
"""Checker for E99 curriculum scheduler and regression replay artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E99_CURRICULUM_SCHEDULER_AND_REGRESSION_REPLAY_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E99_CURRICULUM_SCHEDULER_AND_REGRESSION_REPLAY_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not autonomous open-ended training" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few curriculum cases")
    if task.get("curriculum_scheduler") is not True:
        failures.append("curriculum_scheduler not true")
    if task.get("autonomous_open_ended_training") is not False:
        failures.append("autonomous open-ended training unexpectedly enabled")
    if task.get("requires_regression_replay") is not True:
        failures.append("regression replay requirement missing")
    if task.get("requires_adversarial_coverage") is not True:
        failures.append("adversarial coverage requirement missing")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e99_curriculum_scheduler_regression_replay_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_curriculum_success_min",
        "adversarial_curriculum_success_min",
        "validation_gap_target_accuracy_min",
        "validation_replay_coverage_min",
        "validation_adversarial_coverage_min",
        "validation_budget_validity_min",
        "validation_promotion_safety_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in [
        "adversarial_forgetting_risk_rate_max",
        "adversarial_unsafe_promotion_rate_max",
        "adversarial_over_budget_rate_max",
        "adversarial_stale_lesson_rate_max",
    ]:
    ...
    if not any(values.get("mean_curriculum_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive curriculum contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E99_CURRICULUM_SCHEDULER_AND_REGRESSION_REPLAY_EXPANSION_CHECK",
        "root": str(root),
