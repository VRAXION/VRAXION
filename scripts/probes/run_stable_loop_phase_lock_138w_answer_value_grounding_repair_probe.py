#!/usr/bin/env python3
"""138W deterministic answer-value grounding repair/probe.
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_138W_ANSWER_VALUE_GROUNDING_REPAIR_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_138w_answer_value_grounding_repair_probe/smoke")
DEFAULT_UPSTREAM_138V_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_138v_answer_value_grounding_objective_redesign_plan/smoke")
...
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_138w_answer_value_grounding_repair_probe_check.py"
TARGET_CHECKPOINT_REL = "target/pilot_wave/stable_loop_phase_lock_138w_answer_value_grounding_repair_probe/smoke/checkpoints/target_138w_value_grounding/model.pt"
SOURCE_138I_TARGET_REL = "target/pilot_wave/stable_loop_phase_lock_138i_real_raw_reasoning_rollout_aligned_repair_probe/smoke/checkpoints/target_138i_rollout_aligned_reasoning/model.pt"

POSITIVE_VERDICT = "ANSWER_VALUE_GROUNDING_REPAIR_POSITIVE"
NEGATIVE_VERDICT = "ANSWER_VALUE_GROUNDING_REPAIR_FAILS"
MISSING_TRAINING_VERDICT = "VALUE_GROUNDING_TRAINING_PATH_MISSING"
DETERMINISM_VERDICT = "DETERMINISM_REPLAY_MISMATCH"
POSITIVE_DECISION = "answer_value_grounding_repair_positive"
POSITIVE_NEXT = "139V_ANSWER_VALUE_GROUNDING_REPAIR_SCALE_CONFIRM"
NO_VALUE_DECISION = "no_value_grounding_improvement"
NO_VALUE_NEXT = "138W_FAILURE_ANALYSIS"
PARROT_DECISION = "parrot_trap_copy_shortcut_detected"
PARROT_NEXT = "138P_PARROT_TRAP_VALUE_COPY_ANALYSIS"
WRAPPER_VALUE_DECISION = "wrapper_success_without_value_grounding_persists"
WRAPPER_VALUE_NEXT = "138WV_WRAPPER_VALUE_DECOUPLING_FAILURE_ANALYSIS"
GARBAGE_DECISION = "wrapper_induced_amnesia_proxy_failure"
GARBAGE_NEXT = "138WG_WRAPPER_INDUCED_AMNESIA_PROXY_FAILURE_ANALYSIS"
STALE_DECISION = "stale_chat_rollout_failure"
STALE_NEXT = "138S_STALE_CHAT_ROLLOUT_FAILURE_ANALYSIS"
DETERMINISM_DECISION = "nondeterministic_value_grounding_probe"
DETERMINISM_NEXT = "138N_DETERMINISM_FAILURE_ANALYSIS"
LEAKAGE_DECISION = "value_grounding_eval_leakage"
LEAKAGE_NEXT = "138L_VALUE_GROUNDING_EVAL_LEAKAGE_REDESIGN"
...
TRAINING_MISSING_DECISION = "value_grounding_training_path_missing"
TRAINING_MISSING_NEXT = "138WA_VALUE_GROUNDING_TRAINING_HELPER_INTEGRATION_PLAN"

BOUNDARY_TEXT = (
    "138W is a deterministic targeted answer-value grounding repair/probe. It "
...
FAMILIES = [
    "VALUE_DIRECT_COPY_DIAGNOSTIC",
    "VALUE_RULE_DERIVED",
    "VALUE_TABLE_DERIVED",
    "VALUE_COMPOSITION_DERIVED",
    "VALUE_CONTRADICTION_RESOLUTION",
    "VALUE_OOD_SYMBOL_BINDING",
    "VALUE_NO_STALE_CHAT_DIRECT",
    "VALUE_AFTER_PREFIX_STABILITY",
]
...
SEED_MAX_GATES = {"stale_chat_fragment_rate": 0.15}
BASELINE_METRICS = {
...
def resolve_target_out(text: str) -> Path:
    ...
    try:
        ...
        raise GateError("138W_BOUNDARY_FAILURE", "--out must stay inside repo") from exc
    parts = [part.lower() for part in relative.parts]
    if len(parts) < 2 or parts[0] != "target" or parts[1] != "pilot_wave":
        raise GateError("138W_BOUNDARY_FAILURE", "--out must stay under target/pilot_wave")
    return resolved
...
def write_summary(out: Path, status: str, verdicts: list[str], decision: dict[str, Any], failure: str | None = None) -> None:
    write_json(
        out / "summary.json",
        {
            "schema_version": "phase_138w_summary_v1",
            "milestone": MILESTONE,
            "status": status,
            "verdicts": verdicts,
...
def write_report(out: Path, verdicts: list[str], decision: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "Wrapper-Induced Amnesia remains an output-level proxy hypothesis.",
            "Residual Signal Carrier remains a design concept; hidden-state evidence is diagnostic_gap unless instrumented.",
...
def import_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_138w", HELPER_PATH)
...
def verify_upstreams(out: Path, root_138v: Path, root_138s: Path, root_138i: Path) -> dict[str, Any]:
    required_138v = ["decision.json", "next_138w_milestone_plan.json", "residual_signal_carrier_requirements.json", "wrapper_value_gap_summary.json"]
    required_138s = ["decision.json", "value_grounding_failure_report.json", "stale_chat_value_coupling_report.json"]
    required_138i = [
        "decision.json",
    ...
    for root, names, verdict in [
        (root_138v, required_138v, "UPSTREAM_138V_ARTIFACT_MISSING"),
        (root_138s, required_138s, "UPSTREAM_138S_ARTIFACT_MISSING"),
        (root_138i, required_138i, "UPSTREAM_138I_ARTIFACT_MISSING"),
    ]:
    ...
    d138v = read_json(root_138v / "decision.json")
    plan138w = read_json(root_138v / "next_138w_milestone_plan.json")
    residual = read_json(root_138v / "residual_signal_carrier_requirements.json")
    d138s = read_json(root_138s / "decision.json")
    value138s = read_json(root_138s / "value_grounding_failure_report.json")
    coupling138s = read_json(root_138s / "stale_chat_value_coupling_report.json")
    d138i = read_json(root_138i / "decision.json")
    aggregate138i = read_json(root_138i / "aggregate_metrics.json")
    namespace138i = read_json(root_138i / "namespace_metrics.json")
    ...
    source_integrity = read_json(root_138i / "source_checkpoint_integrity_manifest.json")
    target_integrity = read_json(root_138i / "target_checkpoint_integrity_manifest.json")
    traces = read_jsonl(root_138i / "raw_generation_trace.jsonl")

    if d138v.get("decision") != "answer_value_grounding_objective_redesign_plan_complete" or d138v.get("next") != "138W_ANSWER_VALUE_GROUNDING_REPAIR_PROBE":
        raise GateError("UPSTREAM_138V_ARTIFACT_MISSING", "138V did not route to 138W")
    if d138v.get("primary_bottleneck") != "wrapper_success_without_value_grounding":
        raise GateError("UPSTREAM_138V_ARTIFACT_MISSING", "138V primary bottleneck mismatch")
    if d138v.get("hidden_state_residual_signal_measurement") != "diagnostic_gap" or residual.get("not_a_hidden_state_claim") is not True:
        raise GateError("UPSTREAM_138V_ARTIFACT_MISSING", "138V residual-carrier diagnostic_gap missing")
    if plan138w.get("milestone") != "138W_ANSWER_VALUE_GROUNDING_REPAIR_PROBE":
        raise GateError("UPSTREAM_138V_ARTIFACT_MISSING", "138V next 138W plan missing")
    ...
    if d138s.get("primary_diagnosis") != "answer_value_grounding_failure_decoupled_from_stale_chat":
        raise GateError("UPSTREAM_138S_ARTIFACT_MISSING", "138S diagnosis mismatch")
    if value138s.get("answer_prefix_accuracy") != 1.0 or value138s.get("eval_namespace_emission_accuracy") != 1.0:
        raise GateError("UPSTREAM_138S_ARTIFACT_MISSING", "138S wrapper success profile missing")
    if value138s.get("answer_value_accuracy") != 0.0 or value138s.get("exact_answer_accuracy") != 0.0:
        raise GateError("UPSTREAM_138S_ARTIFACT_MISSING", "138S value failure profile missing")
    if coupling138s.get("P_wrong_value_given_stale_chat") != 1.0 or coupling138s.get("P_wrong_value_given_no_stale_chat") != 1.0:
        raise GateError("UPSTREAM_138S_ARTIFACT_MISSING", "138S no-stale value failure profile missing")
    if d138i.get("expected_output_canary_passed") is not True or canary.get("expected_output_canary_passed") is not True:
        raise GateError("RAW_HELPER_INTEGRITY_FAILURE", "138I canary failed")
    if d138i.get("ast_shortcut_scan_passed") is not True or scan.get("ast_shortcut_scan_passed") is not True:
        raise GateError("RAW_HELPER_INTEGRITY_FAILURE", "138I AST scan failed")
    if controls.get("controls_failed") is not True or leakage.get("leakage_rejected") is not True or replay.get("determinism_replay_passed") is not True:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I controls/leakage/replay profile missing")
    if before.get("generated_text_produced_before_scoring") is not True:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I generated-before-scoring missing")
    if source_integrity.get("source_checkpoint_unchanged") is not True or target_integrity.get("target_checkpoint_changed") is not True:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I checkpoint integrity missing")
    if namespace138i.get("post_train_namespace_leak_rate") != 0.0 or namespace138i.get("post_eval_namespace_emission_accuracy") != 1.0:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I namespace fix profile missing")
    if namespace138i.get("post_answer_prefix_accuracy") != 1.0 or namespace138i.get("post_answer_value_accuracy") != 0.0:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I wrapper/value profile missing")
    if aggregate138i.get("mean_real_raw_reasoning_accuracy") != 0.0:
        raise GateError("UPSTREAM_138I_ARTIFACT_MISSING", "138I rollout accuracy baseline mismatch")
    for trace in traces:
    ...
    manifest_common = {
        "answer_prefix_accuracy": value138s.get("answer_prefix_accuracy"),
        "eval_namespace_emission_accuracy": value138s.get("eval_namespace_emission_accuracy"),
        "answer_value_accuracy": value138s.get("answer_value_accuracy"),
        "exact_answer_accuracy": value138s.get("exact_answer_accuracy"),
        "P_wrong_value_given_stale_chat": coupling138s.get("P_wrong_value_given_stale_chat"),
        "P_wrong_value_given_no_stale_chat": coupling138s.get("P_wrong_value_given_no_stale_chat"),
        "hidden_state_residual_signal_measurement": d138v.get("hidden_state_residual_signal_measurement"),
    }
    write_json(out / "upstream_138v_manifest.json", {"schema_version": "phase_138w_upstream_138v_manifest_v1", "upstream_root": rel(root_138v), "verified": True, **d138v})
    write_json(out / "upstream_138s_manifest.json", {"schema_version": "phase_138w_upstream_138s_manifest_v1", "upstream_root": rel(root_138s), "verified": True, **manifest_common, "decision": d138s.get("decision"), "next": d138s.get("next")})
    write_json(out / "upstream_138i_manifest.json", {"schema_version": "phase_138w_upstream_138i_manifest_v1", "upstream_root": rel(root_138i), "verified": True, "canary_ast_leakage_controls_determinism_passed": True, "source_checkpoint_unchanged": True, "target_checkpoint_changed": True, "post_train_namespace_leak_rate": namespace138i.get("post_train_namespace_leak_rate"), "post_eval_namespace_emission_accuracy": namespace138i.get("post_eval_namespace_emission_accuracy"), "post_answer_prefix_accuracy": namespace138i.get("post_answer_prefix_accuracy"), "post_answer_value_accuracy": namespace138i.get("post_answer_value_accuracy"), "mean_real_raw_reasoning_accuracy": aggregate138i.get("mean_real_raw_reasoning_accuracy")})
    return {"d138v": d138v, "value138s": value138s, "coupling138s": coupling138s, "namespace138i": namespace138i, "aggregate138i": aggregate138i}
...
def deterministic_setup(seed: int) -> dict[str, Any]:
    ...
    random.seed(seed)
    manifest = {
        "schema_version": "phase_138w_determinism_manifest_v1",
...
def answer_value(category: str, split: str, index: int, rng: random.Random) -> tuple[str, str, dict[str, Any]]:
    ...
    elif category == "VALUE_NO_STALE_CHAT_DIRECT":
...
def build_train_rows(train_examples: int) -> list[dict[str, Any]]:
    ...
    for index in range(train_examples):
        ...
        rows.append(
            {
                "row_id": f"138w_train_{index:06d}",
                "split": "train",
                "family": category,
                "prompt": prompt,
...
def build_eval_rows(seeds: list[int], rows_per_family: int) -> list[dict[str, Any]]:
    ...
    for family_index, family in enumerate(FAMILIES):
        for local_index in range(rows_per_family):
            ...
            rows.append(
                {
                    "row_id": f"138w_eval_{global_index:05d}",
                    "split": "eval",
                    "family": family,
...
def dataset_manifest(rows: list[dict[str, Any]], split: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": f"phase_138w_{split}_dataset_manifest_v1",
        "split": split,
        "row_count": len(rows),
    ...
    for eval_row in eval_rows:
        for train_row in sample_train:
            ...
            if score >= 0.90:
                ...
                break
    return {
        "schema_version": "phase_138w_freshness_leakage_audit_v1",
...
def train_target_model(model: Any, rows: list[dict[str, Any]], seq_len: int, pad_id: int, args: argparse.Namespace, out: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138w_training_objective_report_v1",
...
def forbidden_input_tests(helper: Any, selected: dict[str, Any]) -> dict[str, Any]:
    base = {
        "prompt": "138W forbidden metadata rejection smoke.",
    ...
    return {"schema_version": "phase_138w_forbidden_input_rejection_report_v1", "rows": rows, "generation_config_rows": config_rows, "all_rejected": all(row["rejected"] for row in rows + config_rows)}


def expected_output_canary(helper: Any, selected: dict[str, Any], max_new_tokens: int) -> dict[str, Any]:
    original = {"row_id": "138w_canary_original", "prompt": "138W canary prompt: produce any raw continuation.", "expected_output": "ANSWER=EGOOD"}
    shadow = dict(original)
    shadow["row_id"] = "138w_canary_shadow"
    ...
    return {
        "schema_version": "phase_138w_expected_output_canary_report_v1",
...
def ast_shortcut_scan(paths: list[Path]) -> dict[str, Any]:
    ...
    return {"schema_version": "phase_138w_ast_shortcut_scan_report_v1", "scanned_files": [rel(path) for path in paths if path.exists()], "findings": findings, "ast_shortcut_scan_passed": not findings}
...
def run_eval(helper: Any, rows: list[dict[str, Any]], out: Path, checkpoint_path: str, checkpoint_hash: str, max_new_tokens: int, heartbeat_sec: int, label: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, row in enumerate(rows):
        ...
        traces.append(trace)
        raw_results.append({"row_id": row["row_id"], "family": row["family"], "seed": row["seed"], "prompt_hash": text_hash(row["prompt"]), "generated_text": generated_text, "generated_text_hash": generated_text_hash, "generation_trace_hash": response["generation_trace_hash"], "token_count": response["token_count"]})
...
def family_rates(scoring: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        "prompt_value_copy_accuracy": acc("VALUE_DIRECT_COPY_DIAGNOSTIC", "answer_value_correct"),
        "rule_derived_value_accuracy": acc("VALUE_RULE_DERIVED", "answer_value_correct"),
        "table_derived_value_accuracy": acc("VALUE_TABLE_DERIVED", "answer_value_correct"),
        "composition_derived_value_accuracy": acc("VALUE_COMPOSITION_DERIVED", "answer_value_correct"),
        "ood_symbol_value_accuracy": acc("VALUE_OOD_SYMBOL_BINDING", "answer_value_correct"),
    }
...
def compute_metrics(scoring: list[dict[str, Any]], seeds: list[int]) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    aggregate = {
        "schema_version": "phase_138w_aggregate_metrics_v1",
        "row_count": row_count,
...
def scorer_controls(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    controls = {
        ...
        "PARROT_COPY_CONTROL": lambda row: f"ANSWER=E{row['answer_value']}" if row["family"] == "VALUE_DIRECT_COPY_DIAGNOSTIC" else "ANSWER=EUNRELATED",
    }
    report: dict[str, Any] = {"schema_version": "phase_138w_control_arm_report_v1", "controls_called_helper": False, "controls_failed": True, "control_pass_threshold": 0.20, "controls": {}}
    results: list[dict[str, Any]] = []
    for name, producer in controls.items():
        scope = [row for row in rows if not (name == "PARROT_COPY_CONTROL" and row["family"] == "VALUE_DIRECT_COPY_DIAGNOSTIC")]
        passed_count = 0
        for row in scope:
...
def generated_before_scoring_report(traces: list[dict[str, Any]], scoring: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_138w_generated_before_scoring_report_v1",
...
def eval_snapshot(traces: list[dict[str, Any]], scoring: list[dict[str, Any]], family_metrics: dict[str, Any], seed_metrics: list[dict[str, Any]], aggregate: dict[str, Any], value_metrics: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "decision_critical_metrics": {key: aggregate[key] for key in ["answer_value_accuracy", "exact_answer_accuracy", "value_after_prefix_accuracy", "prefix_success_value_failure_rate", "no_stale_wrong_value_rate", "post_wrapper_garbage_token_rate", "stale_chat_fragment_rate", "parrot_trap_detected"]},
    }
...
def helper_provenance(target_checkpoint: Path, target_hash: str, traces: list[dict[str, Any]], source_hash_before: str, source_hash_after: str, target_hash_before: str | None, helper: Any) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_138w_helper_provenance_verification_v1",
...
def ood_manifest(train_rows: list[dict[str, Any]], eval_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    eval_values = {row["answer_value"] for row in eval_rows}
    return {
        "schema_version": "phase_138w_ood_value_grounding_manifest_v1",
        "train_eval_value_namespaces_disjoint": all(value.startswith("TR") for value in train_values) and all(value.startswith("EV") for value in eval_values),
...
def decide(aggregate: dict[str, Any], controls: dict[str, Any], leakage: dict[str, Any], canary: dict[str, Any], scan: dict[str, Any], before: dict[str, Any], provenance: dict[str, Any], replay: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    if hard_positive:
    ...
    elif aggregate["answer_value_accuracy"] <= BASELINE_METRICS["baseline_answer_value_accuracy"] and aggregate["answer_prefix_accuracy"] >= 0.90:
    ...
    elif aggregate["post_wrapper_garbage_token_rate"] > MAX_POSITIVE_RATES["post_wrapper_garbage_token_rate"]:
        decision_name, next_name, verdict = GARBAGE_DECISION, GARBAGE_NEXT, NEGATIVE_VERDICT
        ...
        decision_name, next_name, verdict = NO_VALUE_DECISION, NO_VALUE_NEXT, NEGATIVE_VERDICT
    decision = {
        "schema_version": "phase_138w_decision_v1",
    ...
    verdicts = [
        ...
        "LEAKAGE_REJECTED" if leakage["leakage_rejected"] else "VALUE_GROUNDING_EVAL_LEAKAGE",
...
def write_failure_decision(args: argparse.Namespace, error: GateError) -> None:
    ...
    route = {
        ...
        "VALUE_GROUNDING_EVAL_LEAKAGE": (LEAKAGE_DECISION, LEAKAGE_NEXT, NEGATIVE_VERDICT),
    ...
    decision = {"schema_version": "phase_138w_failure_decision_v1", "decision": route[0], "next": route[1], "verdict": route[2], "failure_verdict": error.verdict, "failure_message": error.message, "reasoning_subtrack_real_raw_evidence_partially_restored": False, **FALSE_BOUNDARY_FLAGS}
...
def run(args: argparse.Namespace) -> None:
    ...
    write_json(out / "queue.json", {"schema_version": "phase_138w_queue_v1", "milestone": MILESTONE, "status": "started", "started_at": utc_now(), "heartbeat_sec": args.heartbeat_sec})
    append_progress(out, "startup", heartbeat_sec=args.heartbeat_sec)
    refresh_status(out, "running", ["ANSWER_VALUE_GROUNDING_REPAIR_RUNNING"], {"decision": "pending", "next": "pending"})

    verify_upstreams(out, resolve_path(args.upstream_138v_root), resolve_path(args.upstream_138s_root), resolve_path(args.upstream_138i_root))
    append_progress(out, "upstream verification", upstream_138v=True, upstream_138s=True, upstream_138i=True)
    ...
    source_checkpoint = resolve_path(SOURCE_138I_TARGET_REL)
    if not source_checkpoint.exists():
        raise GateError(MISSING_TRAINING_VERDICT, "138I helper-compatible source target checkpoint is missing")
    ...
    write_json(out / "ood_value_grounding_manifest.json", ood_manifest(train_rows, eval_rows))
    ...
    if leakage["leakage_rejected"] is not True:
        raise GateError("VALUE_GROUNDING_EVAL_LEAKAGE", "train/eval leakage detected", leakage)

    train_config = {"schema_version": "phase_138w_train_config_v1", "seeds": seeds, "train_examples": args.train_examples, "train_steps": args.train_steps, "batch_size": args.batch_size, "lr": args.lr, "seq_len": seq_len, "source_checkpoint_path": rel(source_checkpoint), "target_checkpoint_path": TARGET_CHECKPOINT_REL, "training_objective": "value-after-prefix grounding with parrot-trap and derived/OOD families", "positive_can_depend_on_train_loss": False, "source_checkpoint_immutable": True, "old_runners_imported": False}
    eval_config = {"schema_version": "phase_138w_eval_config_v1", "seeds": seeds, "eval_rows_per_family": args.eval_rows_per_family, "max_new_tokens": args.max_new_tokens, "families": FAMILIES, "helper_path": rel(HELPER_PATH), "deterministic_scoring_only": True, "controls_do_not_call_helper": True, "prefix_only_success_allowed": False, "namespace_only_success_allowed": False, "direct_copy_only_success_allowed": False, **FINAL_EVAL_FLAGS}
    ...
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_138w_per_family_metrics_v1", "families": family_metrics})
    ...
    write_json(out / "queue.json", {"schema_version": "phase_138w_queue_v1", "milestone": MILESTONE, "status": "completed", "completed_at": utc_now(), "heartbeat_sec": args.heartbeat_sec})


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--out", default=str(DEFAULT_OUT))
    parser.add_argument("--upstream-138v-root", default=str(DEFAULT_UPSTREAM_138V_ROOT))
    ...
    parser.add_argument("--seeds", default="2301,2302,2303")
...
def main() -> int:
    ...
    try:
        ...
        print(f"138W failed closed: {exc.verdict}: {exc.message}", file=sys.stderr)
        return 1 if exc.verdict in {"138W_BOUNDARY_FAILURE", "CHECKPOINT_MUTATION_DETECTED"} else 0
