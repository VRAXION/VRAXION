#!/usr/bin/env python3
"""Checker for E94 Scribe output hygiene expansion artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E94_SCRIBE_OUTPUT_HYGIENE_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E94_SCRIBE_OUTPUT_HYGIENE_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not chatbot behavior" not in manifest.get("boundary", ""):
    ...
    for family in ["Scribe", "Guard"]:
        if family not in families:
            failures.append(f"missing family: {family}")
    if task.get("case_count", 0) < 7000:
        failures.append("too few output hygiene cases")
    if task.get("external_output_only_after_agency_action") is not True:
        failures.append("external output action boundary missing")
    if task.get("hidden_answer_solving") is not False:
        failures.append("hidden answer solving unexpectedly enabled")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e94_scribe_output_hygiene_expansion_confirmed":
        failures.append("decision not confirmed")
    for key in [
        "validation_render_success_min",
        "adversarial_render_success_min",
        "validation_action_accuracy_min",
        "validation_citation_validity_min",
    ]:
        if aggregate.get(key) != 1.0:
            failures.append(f"{key} != 1.0")
    for key in ["adversarial_false_answer_max", "adversarial_wrong_output_max", "validation_missed_answer_max"]:
    ...
    if not any(values.get("mean_render_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive render contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E94_SCRIBE_OUTPUT_HYGIENE_EXPANSION_CHECK",
        "root": str(root),
