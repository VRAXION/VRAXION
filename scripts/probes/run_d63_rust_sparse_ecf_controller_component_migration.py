#!/usr/bin/env python3
"""D63 Rust sparse ECF controller component migration.

D62 confirmed a learned policy-ensemble gate, but its gate inputs were still
Python-side symbolic diagnostics. D63 keeps the formula solver symbolic and
tests whether selected ECF diagnostic inputs can be represented by Rust sparse
estimator modules that run through the canonical Network::propagate_sparse path.
"""
...
TASK = "D63_RUST_SPARSE_ECF_CONTROLLER_COMPONENT_MIGRATION"
BOUNDARY = (
    "D63 only tests migration of selected ECF diagnostic components to the Rust sparse "
    "path in controlled symbolic joint formula discovery. It does not prove full VRAXION "
...
RUST_DIAGNOSTIC_ARMS = [arm for arm in FAIR_ARMS if arm != "D62_SYMBOLIC_FEATURE_GATE_REFERENCE"]
...
def diagnostic_features(pack):
    ...
    return {
        "bias": 1.0,
        "support_budget_pressure_norm": features["support_budget_pressure_norm"],
        "counterfactual_pressure_norm": features["counterfactual_pressure_norm"],
        "adversarial_pressure_norm": features["adversarial_pressure_norm"],
        "internal_unresolvable_indicator": features["internal_unresolvable_indicator"],
        "external_channel_available": features["external_channel_available"],
...
def diagnostic_targets(pack):
    ...
    return {
        name: 1.0 if features[spec["feature"]] >= spec["threshold"] else 0.0
...
def shuffled_estimates(values):
    return {
        "support_budget_pressure": values["external_channel"],
        "external_channel": values["support_budget_pressure"],
        "internal_unresolvable": values["adversarial_pressure"],
        "adversarial_pressure": values["internal_unresolvable"],
        "counterfactual_pressure": values["support_budget_pressure"],
    }


def random_estimates(pack):
    rng = stable_rng(pack["seed"], f"D63_RANDOM_DIAGNOSTIC:{pack['row_id']}:{pack.get('track')}")
...
def estimates_to_gate_features(pack, estimates, replace_names):
    ...
    for name in replace_names:
        spec = DIAGNOSTIC_MAP[name]
        features[spec["feature"]] = float(estimates[name])
...
def select_policy_for_arm(pack, arm, estimates, learned_gate):
    if arm == "D62_SYMBOLIC_FEATURE_GATE_REFERENCE":
        return policy_from_features(d62.gate_features(pack), learned_gate), d62.gate_features(pack), "symbolic_d62_gate_features", False
    if arm == "RUST_SPARSE_BUDGET_PRESSURE_ESTIMATOR":
        features = estimates_to_gate_features(pack, estimates, ["support_budget_pressure"])
        return policy_from_features(features, learned_gate), features, "rust_budget_pressure_only", False
    if arm == "RUST_SPARSE_COUNTERFACTUAL_PRESSURE_ESTIMATOR":
        features = estimates_to_gate_features(pack, estimates, ["counterfactual_pressure"])
        return policy_from_features(features, learned_gate), features, "rust_counterfactual_pressure_only", False
    if arm == "RUST_SPARSE_ADVERSARIAL_PRESSURE_ESTIMATOR":
        features = estimates_to_gate_features(pack, estimates, ["adversarial_pressure"])
        return policy_from_features(features, learned_gate), features, "rust_adversarial_pressure_only", False
    ...
    if arm == "RUST_SPARSE_EXTERNAL_NEED_ESTIMATOR":
        features = estimates_to_gate_features(pack, estimates, ["external_channel"])
        return policy_from_features(features, learned_gate), features, "rust_external_channel_only", False
    if arm == "RUST_SPARSE_ALL_DIAGNOSTICS_GATE":
        features = estimates_to_gate_features(pack, estimates, DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "rust_all_diagnostics", False
    if arm == "HYBRID_SYMBOLIC_RUST_DIAGNOSTICS_GATE":
        features = estimates_to_gate_features(pack, estimates, ["support_budget_pressure", "counterfactual_pressure", "adversarial_pressure"])
        return policy_from_features(features, learned_gate), features, "rust_budget_counter_adversarial_plus_symbolic_external_abstain", False
    if arm == "SHUFFLED_DIAGNOSTIC_CONTROL":
        features = estimates_to_gate_features(pack, shuffled_estimates(estimates), DIAGNOSTICS)
        return policy_from_features(features, learned_gate), features, "shuffled_diagnostic_pairing_control", False
...
def write_partial_eval(out, split, track, outputs, completed, started):
    ...
    write_json(
        out / f"partial_{split}_{track}_metrics_snapshot.json",
        {
            ...
            "rust_all_recent": partial["by_arm_core"].get("RUST_SPARSE_ALL_DIAGNOSTICS_GATE", {}),
            "hybrid_recent": partial["by_arm_core"].get("HYBRID_SYMBOLIC_RUST_DIAGNOSTICS_GATE", {}),
            "diagnostic_recent": partial["diagnostic_accuracy"].get("RUST_SPARSE_ALL_DIAGNOSTICS_GATE", {}),
        },
    )
...
def positive_gate(summary, d62_summary):
    return (
        summary[SATURATED_TRACK]["exact"] >= d62_summary[SATURATED_TRACK]["exact"] - 0.005
        and summary[HARD_TRACK]["exact"] >= d62_summary[HARD_TRACK]["exact"] - 0.005
        and summary[MIXED_TRACK]["exact"] >= d62_summary[MIXED_TRACK]["exact"] - 0.005
        and summary[OOD_TRACK]["exact"] >= d62_summary[OOD_TRACK]["exact"] - 0.005
        and summary[GATE_CONFUSION_TRACK]["exact"] >= d62_summary[GATE_CONFUSION_TRACK]["exact"] - 0.005
...
def make_decision(metrics_by_track, d62_summary, failed_jobs):
    ...
    if failed_jobs:
        ...
        return {
            ...
            "verdict": "D63_DIAGNOSTIC_COMPONENT_MIGRATION_NOT_CONFIRMED",
            "next": "D63R_DIAGNOSTIC_ESTIMATOR_REPAIR",
            "best_arm": best,
            "boundary": BOUNDARY,
        }, summaries
    all_pass = positive_gate(summaries["RUST_SPARSE_ALL_DIAGNOSTICS_GATE"], d62_summary)
    hybrid_pass = positive_gate(summaries["HYBRID_SYMBOLIC_RUST_DIAGNOSTICS_GATE"], d62_summary)
    controls_worse = (
        summaries["RUST_SPARSE_ALL_DIAGNOSTICS_GATE"][MIXED_TRACK]["exact"] > summaries["SHUFFLED_DIAGNOSTIC_CONTROL"][MIXED_TRACK]["exact"]
        and summaries["RUST_SPARSE_ALL_DIAGNOSTICS_GATE"][MIXED_TRACK]["exact"] > summaries["RANDOM_DIAGNOSTIC_CONTROL"][MIXED_TRACK]["exact"]
        and summaries["RUST_SPARSE_ALL_DIAGNOSTICS_GATE"][MIXED_TRACK]["exact"] > summaries["DIAGNOSTIC_ABLATION_CONTROL"][MIXED_TRACK]["exact"]
    )
    if all_pass and controls_worse:
        return {
    ...
    if hybrid_pass:
        return {
            ...
            "verdict": "D63_HYBRID_DIAGNOSTIC_MIGRATION_POSITIVE",
            "next": "D64_INCREMENTAL_DIAGNOSTIC_MIGRATION",
            "best_arm": "HYBRID_SYMBOLIC_RUST_DIAGNOSTICS_GATE",
            "boundary": BOUNDARY,
        }, summaries
    best = max(FAIR_ARMS, key=lambda arm: summaries[arm][MIXED_TRACK]["exact"])
    return {
...
def make_reports(out, aggregate, decision, arm_summaries):
    ...
    reports = {
        ...
        "diagnostic_feature_definition_report.json": {
            ...
            "target_origin": "observable D62 gate diagnostics only",
            "truth_labels_used": False,
            "support_regime_labels_used": False,
            "track_labels_used_by_fair_estimators": False,
            "external_channel_available": "observable capability flag, not answer truth",
            "external_test_need_estimate": "estimated through the same observable external_channel_available diagnostic in this probe",
        },
        ...
        "estimator_accuracy_report.json": {
            track: metrics[track]["diagnostic_accuracy"].get("RUST_SPARSE_ALL_DIAGNOSTICS_GATE", {})
            for track in TRACKS
        },
        ...
        "component_ablation_report.json": {
            arm: arm_summaries[arm]
            for arm in ARMS
            if arm not in {"D62_SYMBOLIC_FEATURE_GATE_REFERENCE"}
        },
        "truth_leak_audit_report.json": {
            ...
            "forbidden_features": FORBIDDEN_DIAGNOSTIC_FEATURES,
        },
...
def main():
    ...
    parser.add_argument("--seeds", default="11801,11802,11803,11804,11805")
    ...
    write_json(
        ...
        {"train_tracks": {track: len(packs) for track, packs in train_tracks.items()}, "note": "D63 estimators are threshold modules; no gradient training is used."},
    )
