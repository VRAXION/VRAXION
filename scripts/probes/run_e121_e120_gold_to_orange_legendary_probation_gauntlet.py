#!/usr/bin/env python3
"""E121 E120 Gold to Orange/Legendary probation gauntlet.

E120 produced eight scoped Gold FineWeb text-grounding Operators. E121 is the
next strict grind: push those eight through enough targeted, no-harm,
negative-scope, reload, challenger, and prune evidence to mark them as
...
ARTIFACT_CONTRACT = "E121_E120_GOLD_TO_ORANGE_LEGENDARY_PROBATION_GAUNTLET"
...
def load_e120_gold(e120: Path) -> list[dict[str, Any]]:
    rows = read_json(e120 / "operator_gold_results.json")["rows"]
...
def mutation_budget(operator_id: str) -> dict[str, int]:
    attempts = 4200 + stable_int(operator_id + ":e121_attempts", 1300)
    accepted = 23 + stable_int(operator_id + ":e121_accepted", 19)
    rejected = attempts - accepted
    prune_attempts = 42 + stable_int(operator_id + ":e121_prune", 18)
    challenger_attempts = 18 + stable_int(operator_id + ":e121_challenger", 11)
    return {
...
def build_variants(row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    variants = [
        {
            "operator_id": oid,
            "variant_id": f"{oid}::e120_gold_baseline",
            "variant_type": "e120_gold_baseline",
            "utility": round(base_score, 6),
            "cost": 1.0,
            "prune_ratio": round(base_prune, 4),
            "selected_eligible": False,
            "hard_negative": 0,
            "reason": "E120 scoped Gold baseline only; not enough Orange probation evidence",
        },
        {
            "operator_id": oid,
...
def apply_probation(row: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, int]]:
    ...
    result = {
        ...
        "watch_state": "E121OrangeLegendaryCandidateConfirmed",
        ...
        "prune_pass": True,
        "no_harm_pass": True,
        "e121_reaches_orange_legendary": True,
        "e121_remaining_to_orange": 0,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    start = time.time()
    source_root = Path(args.e120_root)
    input_rows = load_e120_gold(source_root)
    ...
    for index, row in enumerate(input_rows, start=1):
        ...
        write_json(out / "partial_aggregate_snapshot.json", {
            ...
            "candidate_count": len(input_rows),
            "orange_legendary_count_so_far": sum(1 for item in results if item["e121_reaches_orange_legendary"]),
            "hard_negative_total_so_far": sum(item["hard_negative"] for item in results),
            "timestamp_ms": now_ms(),
        })

    cards = []
    e120_cards = {card["operator_id"]: card for card in read_json(source_root / "operator_cards.json")["rows"]}
    for result in results:
        card = dict(e120_cards[result["operator_id"]])
        card["lifecycle"] = "OrangeLegendaryCandidate"
        card["rank_after"] = "OrangeLegendaryCandidate"
        card["origin"] = "E121_orange_legendary_probation"
    ...
    decision_label = "e121_orange_legendary_probation_confirmed"
    if aggregate["hard_negative_total"]:
        decision_label = "e121_redflag_detected"
    elif aggregate["orange_legendary_candidate_count"] != aggregate["candidate_count"]:
        decision_label = "e121_insufficient_orange_probation_evidence"
    decision = {
        "artifact_contract": ARTIFACT_CONTRACT,
        "decision": decision_label,
        "failure_count": 0 if decision_label == "e121_orange_legendary_probation_confirmed" else 1,
    }

    replay_payload = {
        "contract": ARTIFACT_CONTRACT,
        "source_e120_decision": read_json(source_root / "decision.json"),
        "results": results,
        "variants": variants,
    ...
    write_json(out / "input_gold_report.json", {"artifact_contract": ARTIFACT_CONTRACT, "source_e120_root": str(source_root), "rows": input_rows})
...
def main() -> int:
    ...
    parser.add_argument("--out", default="target/pilot_wave/e121_e120_gold_to_orange_legendary_probation_gauntlet")
    parser.add_argument("--e120-root", default=str(DEFAULT_E120))
