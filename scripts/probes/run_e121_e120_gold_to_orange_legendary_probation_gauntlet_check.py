#!/usr/bin/env python3
"""Checker for E121 E120 Gold to Orange/Legendary probation gauntlet."""
...
CONTRACT = "E121_E120_GOLD_TO_ORANGE_LEGENDARY_PROBATION_GAUNTLET"
...
def check(root: Path) -> list[str]:
    ...
    if len(input_report.get("rows", [])) != 8:
        failures.append("expected exactly eight E120 Gold inputs")
    if probation.get("orange_target") != ORANGE_TARGET:
        failures.append("orange target mismatch")
    if aggregate.get("candidate_count") != 8:
    ...
    if len(results) != 8 or len(cards) != 8:
    ...
    for row in results:
        ...
        if row.get("e121_remaining_to_orange") != 0 or not row.get("e121_reaches_orange_legendary"):
    ...
    replay_payload = {
        "contract": CONTRACT,
        "source_e120_decision": read_json(Path(manifest["source_root"]) / "decision.json"),
        "results": results,
        "variants": variants,
