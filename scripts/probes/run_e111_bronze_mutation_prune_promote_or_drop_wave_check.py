#!/usr/bin/env python3
"""Checker for E111 Bronze mutation/prune promote-or-drop artifacts."""
...
CONTRACT = "E111_BRONZE_MUTATION_PRUNE_PROMOTE_OR_DROP_WAVE"
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if wave_manifest.get("candidate_source_rank") != "Bronze" or wave_manifest.get("target_rank") != "Gold_or_drop":
    ...
    if "mutation" not in wave_manifest.get("mutation_mode", "") or "prune" not in wave_manifest.get("mutation_mode", ""):
        failures.append("mutation/prune mode missing")
    ...
    if input_report.get("wave2_bronze_candidates") != aggregate.get("candidate_count"):
        failures.append("Bronze candidate count mismatch")
    ...
    if len(variant_rows) < len(rows) * 4:
    ...
    if aggregate.get("mutation_attempts_total", 0) <= 0 or aggregate.get("accepted_mutations_total", 0) <= 0:
        failures.append("mutation attempts/acceptance missing")
    if aggregate.get("prune_attempts_total", 0) <= 0:
        failures.append("prune attempts missing")
    if aggregate.get("rollback_count_total") != aggregate.get("rejected_mutations_total"):
        failures.append("rollback/rejected mismatch")
    if aggregate.get("mutated_candidate_count") != aggregate.get("promoted_to_gold_count"):
        failures.append("not all promoted candidates used mutated/pruned variant")
    if aggregate.get("promoted_to_gold_count", 0) + aggregate.get("dropped_deprecated_count", 0) + aggregate.get("red_flag_count", 0) != aggregate.get("candidate_count", -1):
        failures.append("not all candidates resolved")
    if duration.get("measured_wall_seconds", 0) < 0:
        failures.append("invalid duration")
    if duration.get("estimated_seconds_per_1000_candidates", 0) < 0:
        failures.append("invalid extrapolation")
    if mutation_summary.get("mutation_mode") != "active_variant_search_mutation_prune_challenger_accept_reject_rollback":
        failures.append("mutation summary mode mismatch")
    ...
    if len(promotion.get("promoted_to_gold", [])) != aggregate.get("promoted_to_gold_count"):
        failures.append("promotion count mismatch")

    variant_by_id = {row.get("variant_id"): row for row in variant_rows}
    for row in rows:
        operator_id = row.get("operator_id")
        if row.get("rank_before") != "Bronze":
            failures.append(f"non-Bronze candidate in wave: {operator_id}")
        if row.get("hard_negative_add") != 0 or row.get("hard_negative") != 0:
            failures.append(f"hard negative row: {operator_id}")
        if row.get("selected_variant_type") == "base_unmodified":
            failures.append(f"base variant selected for promoted candidate: {operator_id}")
        ...
        if row.get("rank_after") == "Gold":
            if row.get("qualified_activation", 0) < 3000:
                failures.append(f"Gold below activation threshold: {operator_id}")
            if row.get("combined_family_coverage", 0) < 5:
                failures.append(f"Gold below coverage threshold: {operator_id}")
            if row.get("campaign_count", 0) < 3:
                failures.append(f"Gold below campaign threshold: {operator_id}")
            if not row.get("reload_shadow_pass") or not row.get("challenger_pass") or not row.get("prune_pass"):
                failures.append(f"Gold missing reload/challenger/prune pass: {operator_id}")
        if row.get("rank_after") in {"Bronze", "Silver"}:
            failures.append(f"unresolved lower rank after wave: {operator_id}")
        if row.get("rank_after") in {"DiamondCandidate", "CoreMemoryCandidate", "PermaCore"}:
            failures.append(f"forbidden higher rank in E111: {operator_id}")
        if row.get("mutation_attempts", 0) <= 0 or row.get("accepted_mutations", 0) <= 0:
            failures.append(f"missing mutation budget: {operator_id}")
    ...
    if decision.get("failure_count") != 0 or decision.get("decision") != "e111_bronze_mutation_prune_wave_gold_conversion_confirmed":
...
def main() -> int:
    ...
    summary = {
        "checker": "E111_BRONZE_MUTATION_PRUNE_PROMOTE_OR_DROP_WAVE_CHECK",
        "root": str(root),
