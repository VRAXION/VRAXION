#!/usr/bin/env python3
"""Checker for 141A multi-field transfer probe."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_141a_instnct_pocket_gated_multi_field_transfer_probe/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_141a_instnct_pocket_gated_multi_field_transfer_probe.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_141a_instnct_pocket_gated_multi_field_transfer_probe_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE_RESULT.md",
]
...
REQUIRED_ARTIFACTS = [
    "queue.json", "progress.jsonl", "upstream_140z_manifest.json", "upstream_140y_manifest.json",
    ...
    "multi_field_transfer_metrics.json", "field_shortcut_report.json", "priority_conflict_report.json",
...
def require_upstreams(root: Path, failures: list[str]) -> None:
    z = load_json(root / "upstream_140z_manifest.json")
    y = load_json(root / "upstream_140y_manifest.json")
    if z.get("decision") != "multi_field_transfer_probe_recommended":
        failures.append(f"BAD_140Z_DECISION:{z.get('decision')}")
    if z.get("next") != "141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE":
        failures.append(f"BAD_140Z_NEXT:{z.get('next')}")
    if z.get("selected_candidate") != "open_multi_field_final_all_markers":
        failures.append(f"BAD_140Z_SELECTED_CANDIDATE:{z.get('selected_candidate')}")
    if y.get("decision") != "instnct_pocket_gated_multi_step_transfer_scale_confirmed":
        failures.append(f"BAD_140Y_DECISION:{y.get('decision')}")
    if y.get("next") != "140Z_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_DECISION_PLAN":
        failures.append(f"BAD_140Y_NEXT:{y.get('next')}")
...
def require_manifests(root: Path, failures: list[str]) -> None:
    ...
    if eval_manifest.get("row_count", 0) < 700:
    ...
    if eval_manifest.get("scaffold_variant_count", 0) < 48:
...
def require_metrics(root: Path, failures: list[str]) -> None:
    ...
    lower_bounds = {
        "main_final_answer_accuracy": 0.55,
        "main_multi_field_binding_accuracy": 0.55,
        "main_pocket_writeback_rate": 0.70,
        "main_contrast_group_accuracy": 0.55,
        "pocket_ablation_delta": 0.30,
    }
    ...
    if metrics.get("ablation_final_answer_accuracy", 1.0) > 0.20:
...
def require_decision(root: Path, failures: list[str]) -> None:
    ...
    if decision.get("verdict") != "INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_POSITIVE":
...
def require_report(root: Path, failures: list[str]) -> None:
    ...
    required_phrases = [
        ...
        "constrained pocket-gated helper evidence",
    ]
...
def require_docs(failures: list[str]) -> None:
    for doc in DOCS:
        ...
        for phrase in [
            "141A_INSTNCT_POCKET_GATED_MULTI_FIELD_TRANSFER_PROBE",
            "multi-field",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 141A multi-field transfer probe artifacts.")
    ...
    if failures:
        print("141A CHECK FAIL")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("141A CHECK PASS")
    return 0
