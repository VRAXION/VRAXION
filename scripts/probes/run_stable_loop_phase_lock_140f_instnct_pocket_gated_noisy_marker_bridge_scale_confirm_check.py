#!/usr/bin/env python3
"""Checker for 140F noisy-marker bridge scale confirm."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_140f_instnct_pocket_gated_noisy_marker_bridge_scale_confirm/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_140f_instnct_pocket_gated_noisy_marker_bridge_scale_confirm.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_140f_instnct_pocket_gated_noisy_marker_bridge_scale_confirm_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_140F_INSTNCT_POCKET_GATED_NOISY_MARKER_BRIDGE_SCALE_CONFIRM_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_140F_INSTNCT_POCKET_GATED_NOISY_MARKER_BRIDGE_SCALE_CONFIRM_RESULT.md",
]
ALLOWED_MUTATIONS = {RUNNER, CHECKER, *DOCS}
REQUIRED_ARTIFACTS = [
    "queue.json",
    "progress.jsonl",
    "upstream_140a_manifest.json",
    "scale_config.json",
    ...
    "noisy_marker_bridge_scale_metrics.json",
...
def check_static_files() -> list[str]:
    ...
    for rel in [RUNNER, CHECKER, *DOCS]:
        ...
        for term in ["140F", "scale", "noisy", "bridge", "pocket"]:
            if term not in text:
...
def check_artifacts() -> list[str]:
    ...
    upstream = load_json(SMOKE_ROOT / "upstream_140a_manifest.json")
    ...
    eval_manifest = load_json(SMOKE_ROOT / "scale_eval_manifest.json")
    ...
    metrics = load_json(SMOKE_ROOT / "noisy_marker_bridge_scale_metrics.json")
    ...
    if upstream.get("decision") != "instnct_pocket_gated_noisy_marker_bridge_probe_positive":
        failures.append("BAD_140A_DECISION")
    if upstream.get("next") != "140F_INSTNCT_POCKET_GATED_NOISY_MARKER_BRIDGE_SCALE_CONFIRM":
        failures.append("BAD_140A_NEXT")
    if upstream.get("main_answer_value_accuracy", 0.0) < 0.80:
        failures.append("BAD_140A_MAIN_ACCURACY")
    if upstream.get("ablation_answer_value_accuracy", 1.0) > 0.10:
        failures.append("BAD_140A_ABLATION")
    if upstream.get("reduced_marker_row_rate", 0.0) < 0.60:
        failures.append("BAD_140A_REDUCED_MARKER_RATE")
    ...
    if scaffold.get("direct_pocket_value_marker_rate", 1.0) > 0.15:
        failures.append("POCKET_VALUE_MARKER_RATE_TOO_HIGH")
    if scaffold.get("reduced_marker_row_rate", 0.0) < 0.85:
        failures.append("REDUCED_MARKER_RATE_TOO_LOW")
    for key in ["explicit_pocket_value_markers_reduced", "noisy_prompt_distractors_added", "value_hidden_behind_natural_task_text", "pocket_gate_still_required", "visible_value_bypass_forbidden"]:
        if scaffold.get(key) is not True:
            failures.append(f"SCAFFOLD_REQUIREMENT_MISSING:{key}")
    ...
    if len(selected_rows) != 1:
        ...
        if selected.get("candidate") != "open_pocket_all_payload_markers_noisy_bridge_scale":
    ...
    if selection.get("selected_candidate") != "open_pocket_all_payload_markers_noisy_bridge_scale":
    ...
    for control in ["VISIBLE_VALUE_BYPASS_CONTROL", "NOISY_DISTRACTOR_CONTROL", "CLOSED_POCKET_ABLATION_CONTROL", "STATIC_OUTPUT_CONTROL", "COPY_PROMPT_CONTROL", "TRAIN_NAMESPACE_REPLAY_CONTROL", "PREFIX_ONLY_CONTROL", "LONG_NOISE_CONTROL"]:
    ...
    if comparison.get("every_seed_passed") is not True:
        failures.append("SEED_GATE_FAILED")
    ...
    for event in ["startup", "upstream verification", "helper provenance", "scale eval row build", "candidate evaluated", "mutation selection", "canary and ast", "final eval generation", "scoring", "controls", "determinism replay", "aggregate analysis", "decision", "final verdict"]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 140F noisy-marker bridge scale confirm")
    ...
    payload = {"schema_version": "phase_140f_checker_result_v1", "status": "pass" if not failures else "fail", "failures": failures}
