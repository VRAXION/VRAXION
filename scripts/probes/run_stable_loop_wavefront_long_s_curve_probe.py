...
CONTRACT = ROOT / "docs" / "research" / "STABLE_LOOP_WAVEFRONT_003_LONG_S_CURVE_CONTRACT.md"
DOC_REPORT = ROOT / "docs" / "research" / "STABLE_LOOP_WAVEFRONT_003_LONG_S_CURVE_RESULT.md"

MAX_GRID = 16
INPUT_CHANNELS = 3
WALL, START, TARGET = 0, 1, 2
MAX_S = 32
DISTANCE_BUCKETS = ((1, 4), (5, 8), (9, 16), (17, 24))
DISTANCE_BUCKET_NAMES = ("1-4", "5-8", "9-16", "17-24")
...
FAIR_LOCAL_ARMS = LOCAL_ARMS | {"UNTIED_LOCAL_CNN_TARGET_READOUT"}
TRUNCATED_TRAIN_ARMS = FAIR_LOCAL_ARMS - {"LOCAL_MESSAGE_PASSING_GNN"}
GLOBAL_CONTROL_ARMS = {
    "SUMMARY_DIRECT_HEAD",
    "TARGET_MARKER_ONLY",
    "UNTIED_GLOBAL_CNN",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="STABLE_LOOP_WAVEFRONT_003_LONG_S_CURVE deterministic truncated-S wavefront probe.")
...
class UntiedGlobalCNN(nn.Module):
    def __init__(self, width: int, max_steps: int):
        super().__init__()
        self.inp = nn.Conv2d(INPUT_CHANNELS, width, 3, padding=1)
        self.layers = nn.ModuleList([nn.Conv2d(width + INPUT_CHANNELS, width, 3, padding=1) for _ in range(max_steps)])
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor, steps: int = 1) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        for layer in self.layers[:steps]:
            ...
            traces.append(h)
        pooled = torch.cat([h.mean(dim=(2, 3)), h.amax(dim=(2, 3))], dim=1)
        return self.head(pooled).squeeze(-1), trace_stats(traces, x)
...
def make_model(config: Config) -> nn.Module | None:
    ...
    if config.arm == "UNTIED_LOCAL_CNN_TARGET_READOUT":
        return UntiedLocalCNN(config.width, MAX_S)
    if config.arm == "UNTIED_GLOBAL_CNN":
        return UntiedGlobalCNN(config.width, MAX_S)
...
def forward_model(model: nn.Module, config: Config, batch: dict[str, Any], indices: np.ndarray, steps: int, noise_std: float = 0.0) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    if isinstance(model, (UntiedLocalCNN, UntiedGlobalCNN, DeterministicMessagePassing, LocalLoop)):
...
def training_targets(config: Config, batch: dict[str, Any], indices: np.ndarray, steps: int) -> torch.Tensor:
    if config.arm in TRUNCATED_TRAIN_ARMS:
        distance = batch["distance"][indices]
        return ((distance >= 0) & (distance <= steps)).float()
    return batch["y"][indices]
...
def train_model(config: Config, rows: list[WaveExample], args: argparse.Namespace, device: torch.device, progress_path: Path, seed: int) -> nn.Module | None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for indices in iter_batches(len(rows), args.batch_size, rng):
            ...
            steps = rng.choice(TRAIN_STEPS) if config.train_mode == "SAME_WEIGHTS_S_CURVE" else config.settling_steps
            logits, _ = forward_model(model, config, batch, indices, steps)
            loss = F.binary_cross_entropy_with_logits(logits, training_targets(config, batch, indices, steps))
            loss.backward()
...
def evaluate(config: Config, model: nn.Module | None, train_rows: list[WaveExample], eval_rows: list[WaveExample], args: argparse.Namespace, device: torch.device) -> dict[str, float]:
    ...
    full_gold = np.array([row.label for row in eval_rows], dtype=np.int64)
    truncated_gold = np.array([truncated_label(row, config.settling_steps) for row in eval_rows], dtype=np.int64)
    correct_full = pred == full_gold
    correct_truncated = pred == truncated_gold
    covered_full_idx = np.array(
        [row.label == 0 or (row.distance >= 0 and row.distance <= config.settling_steps) for row in eval_rows],
        dtype=bool,
    )
    metrics: dict[str, float] = {
        "reachable_accuracy": accuracy(pred, truncated_gold),
        "truncated_accuracy_by_S": accuracy(pred, truncated_gold),
        "full_reachability_accuracy": accuracy(pred, full_gold),
        "full_reachability_accuracy_at_large_S": accuracy(pred[covered_full_idx], full_gold[covered_full_idx]) if covered_full_idx.any() else math.nan,
        "oracle_full_accuracy": accuracy(pred, full_gold) if config.arm == "ORACLE_FULL_BFS" else math.nan,
        "oracle_truncated_S_accuracy": accuracy(pred, truncated_gold) if config.arm == "ORACLE_TRUNCATED_BFS_S" else math.nan,
        "heldout_larger_grid_accuracy": metric_by_filter(eval_rows, correct_truncated, lambda row: row.grid_size > 8),
        "long_path_accuracy": metric_by_filter(eval_rows, correct_truncated, lambda row: row.nominal_distance >= 9),
        "unreachable_near_miss_accuracy": metric_by_filter(eval_rows, correct_truncated, lambda row: "near_miss" in row.tags),
        "same_target_neighborhood_pair_accuracy": pair_accuracy(eval_rows, correct_truncated),
        "false_reach_rate": float(np.mean((pred == 1) & (truncated_gold == 0))),
        "false_block_rate": float(np.mean((pred == 0) & (truncated_gold == 1))),
        "locality_audit": 1.0 if config.arm in FAIR_LOCAL_ARMS else math.nan,
        "fixed_s_accuracy": accuracy(pred, truncated_gold) if config.train_mode == "FIXED_S" else math.nan,
        "same_weights_s_curve_accuracy": accuracy(pred, truncated_gold) if config.train_mode == "SAME_WEIGHTS_S_CURVE" else math.nan,
        "variable_s_accuracy": accuracy(pred, truncated_gold) if config.train_mode == "SAME_WEIGHTS_S_CURVE" else math.nan,
        "target_marker_only_baseline": max(float(truncated_gold.mean()), 1.0 - float(truncated_gold.mean())),
        "full_label_used_too_early": 0.0 if config.arm in TRUNCATED_TRAIN_ARMS else math.nan,
    }
    metrics.update(bucket_metrics(eval_rows, correct_truncated))
    metrics.update(stats)
    reachable_near_idx = np.array([row.label == 1 and 0 <= row.distance <= config.settling_steps for row in eval_rows], dtype=bool)
    reachable_far_idx = np.array([row.label == 1 and row.distance > config.settling_steps for row in eval_rows], dtype=bool)
    unreachable_idx = full_gold == 0
    metrics["acc_d_le_S"] = float(np.mean(pred[reachable_near_idx] == 1)) if reachable_near_idx.any() else math.nan
    metrics["missed_reach_d_le_S"] = float(np.mean(pred[reachable_near_idx] == 0)) if reachable_near_idx.any() else math.nan
    metrics["acc_d_gt_S"] = float(np.mean(pred[reachable_far_idx] == 0)) if reachable_far_idx.any() else math.nan
    metrics["false_reach_d_gt_S"] = float(np.mean(pred[reachable_far_idx] == 1)) if reachable_far_idx.any() else math.nan
    metrics["unreachable_false_reach_all_S"] = float(np.mean(pred[unreachable_idx] == 1)) if unreachable_idx.any() else math.nan
    metrics["s_matches_distance_score"] = metrics["acc_d_le_S"]
    metrics["locality_far_reach_rate"] = metrics["false_reach_d_gt_S"]
    metrics["propagation_curve_score"] = float(np.nanmean([metrics["acc_d_le_S"], metrics["acc_d_gt_S"], 1.0 - metrics["unreachable_false_reach_all_S"]]))
    metrics["s_matches_distance_score"] = metrics["acc_d_le_S"]
    if model is not None and config.arm in FAIR_LOCAL_ARMS:
        ...
        over_truncated = np.array([truncated_label(row, over_steps) for row in eval_rows], dtype=np.int64)
        stable_idx = np.array([row.label == 0 or (row.label == 1 and 0 <= row.distance <= config.settling_steps) for row in eval_rows], dtype=bool)
        metrics["overrun_stability"] = accuracy(over_pred[stable_idx], full_gold[stable_idx]) if stable_idx.any() else math.nan
        metrics["overrun_stability_truncated_oracle"] = accuracy(over_pred, over_truncated)
        metrics["s_over_path_overrun_accuracy"] = metrics["overrun_stability"]
        ...
        _, base_pred, _ = predictions_for(model, config, eval_rows, args, device, steps=EVAL_STEPS[0])
        base_truncated = np.array([truncated_label(row, EVAL_STEPS[0]) for row in eval_rows], dtype=np.int64)
        metrics["settling_gain"] = accuracy(pred, truncated_gold) - accuracy(base_pred, base_truncated)
        metrics["target_cell_reach_channel_accuracy"] = 1.0 - abs(float(np.nan_to_num(metrics.get("target_reach_channel", 0.5))) - float(truncated_gold.mean()))
        if config.train_mode == "SAME_WEIGHTS_S_CURVE":
            curve_acc = []
            for step in EVAL_STEPS:
                _, step_pred, _ = predictions_for(model, config, eval_rows, args, device, steps=step)
                step_gold = np.array([truncated_label(row, step) for row in eval_rows], dtype=np.int64)
                step_acc = accuracy(step_pred, step_gold)
                metrics[f"same_weights_truncated_accuracy_s{step}"] = step_acc
                curve_acc.append(step_acc)
            metrics["same_weights_s_curve_accuracy"] = float(np.mean(curve_acc)) if curve_acc else math.nan
            if len(curve_acc) > 1:
                drops = [max(0.0, curve_acc[i - 1] - curve_acc[i]) for i in range(1, len(curve_acc))]
                metrics["monotonic_s_curve_score"] = max(0.0, 1.0 - float(np.mean(drops)))
    else:
        metrics.setdefault("overrun_stability", math.nan)
        metrics.setdefault("overrun_stability_truncated_oracle", math.nan)
        metrics.setdefault("s_over_path_overrun_accuracy", math.nan)
    ...
    metrics.setdefault("monotonic_s_curve_score", metrics["same_weights_s_curve_accuracy"] if config.train_mode == "SAME_WEIGHTS_S_CURVE" else math.nan)
    metrics["maze_family_accuracy"] = metrics["truncated_accuracy_by_S"]
    metrics["s_curve_accuracy"] = metrics["truncated_accuracy_by_S"]
    return metrics
...
def stage_defaults(stage: str) -> tuple[list[str], list[int], list[int], list[str]]:
    if stage == "smoke":
        return (
            ["ORACLE_FULL_BFS", "ORACLE_TRUNCATED_BFS_S", "SUMMARY_DIRECT_HEAD", "TARGET_MARKER_ONLY", "LOCAL_MESSAGE_PASSING_GNN", "HARD_WALL_ABC_LOOP", "HARD_WALL_PRISMION_PHASE_LOOP", "UNTIED_LOCAL_CNN_TARGET_READOUT", "UNTIED_GLOBAL_CNN"],
            [16],
            list(EVAL_STEPS),
            ["FIXED_S", "SAME_WEIGHTS_S_CURVE"],
        )
...
def build_queue(args: argparse.Namespace) -> list[Config]:
    ...
    configs: list[Config] = []
    for arm in arms:
        arm_steps = steps if arm in FAIR_LOCAL_ARMS or arm == "ORACLE_TRUNCATED_BFS_S" else [1]
        arm_modes = modes if arm in TRUNCATED_TRAIN_ARMS else ["FIXED_S"]
        ...
        for width in arm_widths:
            for mode in arm_modes:
                mode_steps = [max(TRAIN_STEPS)] if mode == "SAME_WEIGHTS_S_CURVE" else arm_steps
...
def score(row: dict[str, Any]) -> float:
    return float(np.nanmean([
        metric_mean(row, "truncated_accuracy_by_S"),
        metric_mean(row, "acc_d_le_S"),
        metric_mean(row, "acc_d_gt_S"),
        1.0 - metric_mean(row, "unreachable_false_reach_all_S"),
        metric_mean(row, "same_target_neighborhood_pair_accuracy"),
    ]))
...
def best_by_metric(agg: dict[str, dict[str, Any]], arms: set[str], metric: str) -> dict[str, Any] | None:
    rows = [row for row in agg.values() if row["arm"] in arms and not math.isnan(metric_mean(row, metric))]
    return max(rows, key=lambda row: metric_mean(row, metric), default=None)
...
def control_gaps(agg: dict[str, dict[str, Any]]) -> dict[str, float]:
    ...
    untied_local = best(agg, {"UNTIED_LOCAL_CNN_TARGET_READOUT"})
    untied_global = best(agg, {"UNTIED_GLOBAL_CNN"})
    return {
        "summary_direct_gap": round(score(loop) - score(summary), 6) if loop and summary else math.nan,
        "untied_local_compute_gap": round(score(loop) - score(untied_local), 6) if loop and untied_local else math.nan,
        "global_compute_gap": round(score(loop) - score(untied_global), 6) if loop and untied_global else math.nan,
    }
...
def verdict(agg: dict[str, dict[str, Any]]) -> list[str]:
    ...
    propagation_rows = {key: row for key, row in agg.items() if not math.isnan(metric_mean(row, "false_reach_d_gt_S"))}
    prop_loop = best_by_metric(propagation_rows, LOCAL_ARMS - {"LOCAL_MESSAGE_PASSING_GNN"}, "propagation_curve_score")
    full_loop = best_by_metric(agg, LOCAL_ARMS - {"LOCAL_MESSAGE_PASSING_GNN"}, "full_reachability_accuracy_at_large_S")
    same_weight_rows = {key: row for key, row in agg.items() if row["train_mode"] == "SAME_WEIGHTS_S_CURVE"}
    same_weight_loop = best_by_metric(same_weight_rows, LOCAL_ARMS - {"LOCAL_MESSAGE_PASSING_GNN"}, "same_weights_s_curve_accuracy")
    message = best(agg, {"LOCAL_MESSAGE_PASSING_GNN"})
    prism = best(agg, {"HARD_WALL_PRISMION_PHASE_LOOP"})
    non_prism = best(agg, {"HARD_WALL_ABC_LOOP"})
    if oracle and metric_mean(oracle, "full_reachability_accuracy", 0.0) < 0.99:
        labels.append("TASK_TOO_HARD")
    if trunc and metric_mean(trunc, "oracle_truncated_S_accuracy", 0.0) < 0.99:
        labels.append("TASK_TOO_HARD")
    if marker and score(marker) > 0.85:
        labels.append("TARGET_MARKER_SHORTCUT")
    if summary and score(summary) > 0.85:
    ...
    if learned_loop and untied_local and score(untied_local) >= score(learned_loop) - 0.03:
        labels.append("UNTIED_COMPUTE_SOLVES_FAIR_TASK")
    if learned_loop and untied_global and score(untied_global) >= score(learned_loop) - 0.03:
        labels.append("GLOBAL_COMPUTE_SOLVES_TASK")
    ...
    if learned_loop and metric_mean(learned_loop, "false_reach_d_gt_S", 0.0) > 0.35:
    ...
    if learned_loop and any(metric_mean(row, "full_label_used_too_early", 0.0) > 0.0 for row in agg.values() if row["arm"] in TRUNCATED_TRAIN_ARMS):
        labels.append("FULL_LABEL_USED_TOO_EARLY")
    if prop_loop and trunc:
        if (
            metric_mean(trunc, "oracle_truncated_S_accuracy", 0.0) >= 0.99
            and metric_mean(prop_loop, "acc_d_le_S", 0.0) >= 0.90
            and metric_mean(prop_loop, "missed_reach_d_le_S", 1.0) <= 0.10
            and metric_mean(prop_loop, "false_reach_d_gt_S", 1.0) <= 0.10
            and metric_mean(prop_loop, "unreachable_false_reach_all_S", 1.0) <= 0.10
            and metric_mean(prop_loop, "post_mask_reached_wall_leak_rate", 0.0) <= 0.02
        ):
            labels.append("PROPAGATION_CURVE_POSITIVE")
    if full_loop and metric_mean(full_loop, "full_reachability_accuracy_at_large_S", 0.0) >= 0.85:
        labels.append("FULL_REACHABILITY_POSITIVE")
    if same_weight_loop and metric_mean(same_weight_loop, "same_weights_s_curve_accuracy", 0.0) >= 0.75 and metric_mean(same_weight_loop, "settling_gain", 0.0) >= 0.0:
        labels.append("SAME_WEIGHTS_S_CURVE_POSITIVE")
    if learned_loop and oracle and trunc and summary and untied_local:
        if (
            metric_mean(oracle, "full_reachability_accuracy", 0.0) >= 0.99
            and metric_mean(trunc, "oracle_truncated_S_accuracy", 0.0) >= 0.99
            and score(learned_loop) >= score(summary) + 0.15
            and score(learned_loop) >= score(message or learned_loop) - 0.03
            ...
            and metric_mean(learned_loop, "false_reach_d_gt_S", 1.0) <= 0.25
        ):
            labels.append("PROPAGATION_CURVE_POSITIVE")
    prism_prop = best_by_metric(propagation_rows, {"HARD_WALL_PRISMION_PHASE_LOOP"}, "propagation_curve_score")
    abc_prop = best_by_metric(propagation_rows, {"HARD_WALL_ABC_LOOP"}, "propagation_curve_score")
    message_prop = best_by_metric(propagation_rows, {"LOCAL_MESSAGE_PASSING_GNN"}, "propagation_curve_score")
    if prism_prop and abc_prop and message_prop and metric_mean(prism_prop, "propagation_curve_score", 0.0) >= metric_mean(abc_prop, "propagation_curve_score", 0.0) + 0.02 and metric_mean(prism_prop, "propagation_curve_score", 0.0) >= metric_mean(message_prop, "propagation_curve_score", 0.0) + 0.02 and metric_mean(prism_prop, "post_mask_reached_wall_leak_rate", 1.0) <= metric_mean(abc_prop, "post_mask_reached_wall_leak_rate", 1.0):
        labels.append("PRISMION_LONG_S_POSITIVE")
    return sorted(set(labels or ["LONG_S_CURVE_PARTIAL"]))
...
def write_side_outputs(out_dir: Path, agg: dict[str, dict[str, Any]], rows: list[WaveExample]) -> None:
    ...
    for key, row in sorted(agg.items()):
        ...
        curve_row = {**base, "key": key, "truncated_accuracy_by_S": metric_mean(row, "truncated_accuracy_by_S"), "full_reachability_accuracy_at_large_S": metric_mean(row, "full_reachability_accuracy_at_large_S"), "acc_d_le_S": metric_mean(row, "acc_d_le_S"), "acc_d_gt_S": metric_mean(row, "acc_d_gt_S"), "false_reach_d_gt_S": metric_mean(row, "false_reach_d_gt_S"), "missed_reach_d_le_S": metric_mean(row, "missed_reach_d_le_S"), "unreachable_false_reach_all_S": metric_mean(row, "unreachable_false_reach_all_S"), "same_weights_s_curve_accuracy": metric_mean(row, "same_weights_s_curve_accuracy"), "monotonic_s_curve_score": metric_mean(row, "monotonic_s_curve_score"), "propagation_curve_score": metric_mean(row, "propagation_curve_score"), "s_matches_distance_score": metric_mean(row, "s_matches_distance_score"), "s_over_path_overrun_accuracy": metric_mean(row, "s_over_path_overrun_accuracy"), "overrun_stability_truncated_oracle": metric_mean(row, "overrun_stability_truncated_oracle")}
        for step in EVAL_STEPS:
            curve_row[f"same_weights_truncated_accuracy_s{step}"] = metric_mean(row, f"same_weights_truncated_accuracy_s{step}")
        s_curve.append(curve_row)
        for name in DISTANCE_BUCKET_NAMES:
...
def write_outputs(out_dir: Path, results: list[JobResult], args: argparse.Namespace, status: str, jobs: int, sample_rows: list[WaveExample]) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    lines = [
        "# STABLE_LOOP_WAVEFRONT_003_LONG_S_CURVE Report",
        "",
        f"- Status: `{status}`",
        ...
        "",
        "| Arm | W | S | Mode | TruncAcc | FullLarge | d<=S | d>S | FalseReach>d | UnreachFalse | SettleGain | PostWall | PreWall | OverrunTrunc | Params |",
        "|---|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in sorted(agg.values(), key=lambda r: (str(r["arm"]), int(r["width"]), int(r["settling_steps"]), str(r["train_mode"]))):
        ...
        lines.append("| `{}` | `{}` | `{}` | `{}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.0f}` |".format(row["arm"], row["width"], row["settling_steps"], row["train_mode"], metric_mean(row, "truncated_accuracy_by_S"), metric_mean(row, "full_reachability_accuracy_at_large_S"), metric_mean(row, "acc_d_le_S"), metric_mean(row, "acc_d_gt_S"), metric_mean(row, "false_reach_d_gt_S"), metric_mean(row, "unreachable_false_reach_all_S"), metric_mean(row, "settling_gain"), post_wall, pre_wall, metric_mean(row, "overrun_stability_truncated_oracle"), metric_mean(row, "parameter_count")))
    lines.extend(["", "## Control Gaps", "", f"- summary_direct_gap: `{gaps['summary_direct_gap']:.3f}`", f"- untied_local_compute_gap: `{gaps['untied_local_compute_gap']:.3f}`", f"- global_compute_gap: `{gaps['global_compute_gap']:.3f}`"])
...
def write_doc_result(agg: dict[str, dict[str, Any]], labels: list[str], args: argparse.Namespace, jobs: int, completed: int) -> None:
    ...
    for row in sorted(agg.values(), key=lambda r: (str(r["arm"]), int(r["width"]), int(r["settling_steps"]), str(r["train_mode"]))):
        pre_wall = max(metric_mean(row, "pre_mask_frontier_wall_write_norm"), metric_mean(row, "pre_mask_reached_wall_write_norm"))
        post_wall = max(metric_mean(row, "post_mask_frontier_wall_leak_rate"), metric_mean(row, "post_mask_reached_wall_leak_rate"))
        lines.append("| `{}` | `{}` | `{}` | `{}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` | `{:.3f}` |".format(row["arm"], row["width"], row["settling_steps"], row["train_mode"], metric_mean(row, "truncated_accuracy_by_S"), metric_mean(row, "full_reachability_accuracy_at_large_S"), metric_mean(row, "acc_d_le_S"), metric_mean(row, "acc_d_gt_S"), metric_mean(row, "false_reach_d_gt_S"), metric_mean(row, "settling_gain"), post_wall, pre_wall, metric_mean(row, "overrun_stability_truncated_oracle")))
    lines.extend(["", "## Claim Boundary", "", "This is a deterministic 2D long-S hard-wall wavefront probe. S-curve metrics use the truncated reachability oracle; full reachability is reported separately for S values that cover the relevant path bucket. It is not a parser, factuality system, language benchmark, consciousness claim, or full VRAXION architecture test."])
