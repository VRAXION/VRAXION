#!/usr/bin/env python3
"""Checker for E102 grounded answer decision artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E102_GROUNDED_ANSWER_DECISION_AND_TRACE_RENDERING_EXPANSION":
            failures.append("sample artifact contract mismatch")
    else:
        manifest = read_json(root / "run_manifest.json")
        if manifest.get("artifact_contract") != "E102_GROUNDED_ANSWER_DECISION_AND_TRACE_RENDERING_EXPANSION":
            failures.append("artifact contract mismatch")
        if "not open-domain question answering" not in manifest.get("boundary", ""):
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few answer decision cases")
    if task.get("grounded_answer_decision") is not True:
        failures.append("grounded_answer_decision not true")
    if task.get("open_domain_question_answering") is not False:
        failures.append("open-domain QA unexpectedly enabled")
    if task.get("direct_answer_without_evidence_allowed") is not False:
        failures.append("direct answer without evidence unexpectedly allowed")
    if task.get("requires_citation_trace") is not True:
        failures.append("citation/trace requirement missing")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e102_grounded_answer_decision_expansion_confirmed":
    ...
    if not any(values.get("mean_answer_decision_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive answer decision contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E102_GROUNDED_ANSWER_DECISION_AND_TRACE_RENDERING_EXPANSION_CHECK",
        "root": str(root),
