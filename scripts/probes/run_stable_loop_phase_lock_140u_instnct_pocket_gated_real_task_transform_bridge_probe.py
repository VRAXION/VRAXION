#!/usr/bin/env python3
"""140U helper-only real-task transform bridge probe."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_140U_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_PROBE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_140u_instnct_pocket_gated_real_task_transform_bridge_probe/smoke")
DEFAULT_140HT_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_140ht_instnct_pocket_gated_real_task_transfer_decision_plan/smoke")
HELPER_PATH = REPO_ROOT / "scripts/probes/shared_raw_generation_helper.py"
RUNNER_PATH = Path(__file__).resolve()
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_140u_instnct_pocket_gated_real_task_transform_bridge_probe_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_real_task_transform_bridge_main"
ABLATION_ARM = "instnct_real_task_transform_bridge_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_transform_target_all_markers"
POSITIVE_NEXT = "140V_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_SCALE_CONFIRM"
NATURAL_GATE = "bridge authorization: open"
TARGET_PAYLOAD_MARKERS = ["mapped target:", "rule output:", "table result:", "final transform:"]
...
BOUNDARY_TEXT = (
    "140U is a helper-only executable transform bridge probe. It does not train, "
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_140u", HELPER_PATH)
...
def require_140ht(root: Path) -> dict[str, Any]:
    required = ["decision.json", "target_140u_milestone_plan.json", "transfer_decision_matrix.json", "real_task_transfer_requirements.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 140HT artifacts: {missing}")
    decision = read_json(root / "decision.json")
    target = read_json(root / "target_140u_milestone_plan.json")
    matrix = read_json(root / "transfer_decision_matrix.json")
    requirements = read_json(root / "real_task_transfer_requirements.json")
    if decision.get("decision") != "real_task_transform_bridge_recommended":
        raise RuntimeError(f"bad 140HT decision: {decision.get('decision')}")
    if decision.get("next") != "140U_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_PROBE":
        raise RuntimeError(f"bad 140HT next: {decision.get('next')}")
    if target.get("required_design", {}).get("transform_target_must_differ_from_pocket_source") is not True:
        raise RuntimeError("140HT target plan did not require target/source separation")
    return {
        "root": rel(root),
        ...
        "target_required_design": target.get("required_design"),
        "positive_gates": requirements.get("positive_gates"),
        "recommended_next": matrix.get("recommended_next"),
    }
...
def scan_ast() -> dict[str, Any]:
    ...
    return {"schema_version": "phase_140u_ast_scan_v1", "passed": not failures, "failures": failures}
...
def build_manifest(out: Path, candidate: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    manifest = {
        "schema_version": "instnct_mutation_graph_manifest_v8_real_task_transform_bridge",
        ...
        "closed_pocket_fallback_value": "SYM_TRANSFORM_CLOSED",
        "fallback_value": "SYM_TRANSFORM_CLOSED",
        "allow_train_namespace_value_fallback": False,
        "decoder": {
            "type": "deterministic_pocket_gated_real_task_transform_bridge_decoder",
            "post_generation_repair": False,
            "oracle_metadata_allowed": False,
        },
        "pockets": [
            {
                "pocket_id": "p_real_task_transform_bridge",
                "gate_marker": candidate.get("gate_marker", NATURAL_GATE),
                "payload_markers": markers,
                "writeback": "selected_transform_target",
            }
        ],
        "claim_boundary": "transform bridge helper semantics; not broad assistant capability",
...
def candidate_specs() -> list[dict[str, Any]]:
    return [
        {"candidate": "closed_pocket_no_transform", "gate_marker": "bridge authorization: sealed"},
        {"candidate": "wrong_gate_marker_no_transform", "gate_marker": "route authorization: denied"},
        {"candidate": "copy_only_source_candidate", "gate_marker": NATURAL_GATE, "payload_markers": [SOURCE_MARKER]},
        {"candidate": "open_mapped_target_only_partial", "gate_marker": NATURAL_GATE, "payload_markers": ["mapped target:"]},
        {"candidate": "open_rule_output_only_partial", "gate_marker": NATURAL_GATE, "payload_markers": ["rule output:"]},
        {
...
def transform_values(seed: int, family_index: int, group_index: int, slot: int) -> tuple[str, str, str]:
    source_prefix = "SYMUS" if family_index in {2, 4} else "EVUS"
    target_prefix = "SYMUT" if family_index in {2, 4} else "EVUT"
    ...
    rule_id = ["prefix_swap", "ledger_map", "table_map", "suffix_recode", "symbol_route"][family_index % 5]
...
def prompt_for_row(family: str, seed: int, group_index: int, slot: int, scaffold_id: str, target_marker: str, source: str, target: str, rule_id: str, visible: str, distractors: list[str]) -> str:
    transform_lines = {
        "prefix_swap": f"Transform rule: convert the authorized source family from EVUS to EVUT and keep the row code stable.",
        "ledger_map": f"Ledger rule: authorized source {source} maps to ledger target {target}.",
        "table_map": f"Table rule: source cell {source} routes through table column B to {target}.",
        "suffix_recode": f"Two-step rule: carry the source, then recode the answer channel to target {target}.",
        "symbol_route": f"Symbol route: the authorized source token becomes target token {target} after routing.",
    }
    target_templates = [
        "{marker} {target} after applying the rule to the source.",
        "The transformed target is hidden in the margin as {marker} {target}.",
        "After rule application, the private bridge records {marker} {target}.",
        "The table/result note says {marker} {target}; do not answer with the source.",
    ]
    ...
    return (
        f"REAL_TASK_TRANSFORM_BRIDGE {family} seed {seed} group {group_index} slot {slot}\n"
        f"style={scaffold_id}\n"
        "Resolve the transformed private answer, not the carried source.\n"
        f"{SOURCE_MARKER} {source}\n"
        f"{transform_lines[rule_id]}\n"
...
def eval_rows(seeds: list[int], groups_per_family: int, group_size: int) -> list[dict[str, Any]]:
    families = [
        "TRANSFORM_PREFIX_SWAP",
        "TRANSFORM_LEDGER_MAP",
        "TRANSFORM_TABLE_MAP",
        "TRANSFORM_TWO_STEP_RULE",
        "TRANSFORM_SYMBOL_ROUTE",
    ]
    ...
    for seed in seeds:
        for family_index, family in enumerate(families):
            for group_index in range(groups_per_family):
                ...
                scaffold_id = f"{family}_transform_style_{group_index % 5:02d}"
                for slot in range(group_size):
                    source, target, rule_id = transform_values(seed, family_index, group_index, slot)
                    target_marker = TARGET_PAYLOAD_MARKERS[(family_index + group_index + slot) % len(TARGET_PAYLOAD_MARKERS)]
                    visible = f"EVVIST{seed % 100}{family_index}{group_index:03d}{slot:02d}"
                    distractors = [f"EVDIST{seed % 100}{family_index}{group_index:03d}{slot:02d}_{idx}" for idx in range(4)]
                    rows.append(
                        {
                            "row_id": f"140u_eval_{row_index:07d}",
                            "seed": seed,
                            "family": family,
                            "contrast_group_id": group_id,
                            "scaffold_id": scaffold_id,
                            "prompt": prompt_for_row(family, seed, group_index, slot, scaffold_id, target_marker, source, target, rule_id, visible, distractors),
                            "pocket_source_value": source,
                            "transform_target_value": target,
                            "answer_value": target,
                            "expected_output": f"ANSWER=E{target}",
                            "transform_rule_id": rule_id,
                            "target_payload_marker": target_marker,
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    ...
    for result in results:
        ...
        item = {
            "schema_version": "phase_140u_scoring_row_v1",
            "arm": arm,
            "row_id": row["row_id"],
            "seed": row["seed"],
            "family": row["family"],
    ...
    for group_id, items in sorted(by_group.items()):
        expected_targets = [item["expected_transform_target_value"] for item in items]
        sources = [item["pocket_source_value"] for item in items]
        ...
        group_rows.append(
            {
                "schema_version": "phase_140u_contrast_group_result_v1",
                "arm": arm,
                "contrast_group_id": group_id,
                "family": items[0]["family"],
                "row_count": len(items),
                "source_values": sources,
                "expected_target_values": expected_targets,
    ...
    metrics = {
        "schema_version": "phase_140u_metrics_v1",
        "arm": arm,
...
def marker_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_140u_explicit_marker_audit_v1",
        "row_count": len(rows),
...
def transform_binding_manifest(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_140u_transform_binding_manifest_v1",
        "row_count": len(rows),
        ...
        "task_shapes": [
            "pocket source with mapped target",
            "table/rule transform text",
            "two-step source plus transform target",
            "same source form across distinct contrast targets",
        ],
    }
...
def forbidden_canary(helper: Any, checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"schema_version": "phase_140u_expected_output_canary_v1", "passed": True, "verdict": "forbidden_input_rejected", "exception_type": type(exc).__name__}
    return {"schema_version": "phase_140u_expected_output_canary_v1", "passed": False, "verdict": "forbidden_input_accepted"}


def control_row(name: str, control_passed: bool, generated_value: str | None, blocked_value: str | None) -> dict[str, Any]:
    return {
        "schema_version": "phase_140u_control_result_v1",
        "control": name,
...
def run_controls(helper: Any, main_checkpoint: Path, main_hash: str, ablation_checkpoint: Path, ablation_hash: str, copy_checkpoint: Path, copy_hash: str, max_new_tokens: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    return rows, {
        "schema_version": "phase_140u_control_arm_report_v1",
...
def choose_decision(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any], comparison: dict[str, Any], control_report: dict[str, Any], deterministic: bool, selected_candidate: str, canary_passed: bool, ast_passed: bool, generated_passed: bool) -> dict[str, Any]:
    if not (canary_passed and ast_passed and generated_passed):
        ...
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_INVALID"
        next_step = "135G_RAW_HELPER_ORACLE_SHORTCUT_REMOVAL"
    elif deterministic is not True:
        decision = "nondeterministic_transform_bridge_probe"
        ...
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_FAILS"
        next_step = "140UI_IMPLICIT_GATE_FAILURE_ANALYSIS"
    elif comparison["direct_pocket_value_marker_rate"] != 0.0 or comparison["implicit_or_minimal_gate_row_rate"] < 0.70:
        decision = "implicit_gate_failure"
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_FAILS"
        next_step = "140UI_IMPLICIT_GATE_FAILURE_ANALYSIS"
    ...
    elif ablation_metrics["answer_value_accuracy"] > 0.15 or comparison["pocket_ablation_delta_answer_value_accuracy"] < 0.35:
        decision = "pocket_ablation_not_decision_critical"
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_FAILS"
        next_step = "140UJ_POCKET_CAUSALITY_FAILURE_ANALYSIS"
    elif main_metrics["answer_value_accuracy"] < 0.60 or main_metrics["transform_accuracy"] < 0.60 or main_metrics["pocket_writeback_rate"] < 0.75:
        decision = "transform_binding_failure"
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_FAILS"
        next_step = "140UT_TRANSFORM_BINDING_FAILURE_ANALYSIS"
    else:
        decision = "instnct_pocket_gated_real_task_transform_bridge_probe_positive"
        verdict = "INSTNCT_POCKET_GATED_REAL_TASK_TRANSFORM_BRIDGE_POSITIVE"
        next_step = POSITIVE_NEXT
    return {
        "schema_version": "phase_140u_decision_v1",
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "clean_negative_valid": True,
        "pocket_mechanism_claimed": decision == "instnct_pocket_gated_real_task_transform_bridge_probe_positive",
        "transform_bridge_positive": decision == "instnct_pocket_gated_real_task_transform_bridge_probe_positive",
...
def main() -> int:
    ...
    parser.add_argument("--upstream-140ht-root", type=Path, default=DEFAULT_140HT_ROOT)
    parser.add_argument("--seeds", default="3401,3402,3403")
    parser.add_argument("--groups-per-family", type=int, default=10)
    ...
    write_json(out / "queue.json", {"schema_version": "phase_140u_queue_v1", "milestone": MILESTONE, "status": "running"})

    seeds = [int(item) for item in args.seeds.split(",") if item.strip()]
    upstream = require_140ht(resolve_repo_path(args.upstream_140ht_root))
    write_json(out / "upstream_140ht_manifest.json", upstream)
    append_progress(out, "upstream verification", upstream=upstream)

    config = {
        "schema_version": "phase_140u_eval_config_v1",
        "milestone": MILESTONE,
        "boundary": BOUNDARY_TEXT,
        "train_allowed": False,
    ...
    helper = load_helper()
    provenance = {
        "schema_version": "phase_140u_helper_provenance_v1",
    ...
    write_json(
        out / "transform_eval_manifest.json",
        {
            "schema_version": "phase_140u_transform_eval_manifest_v1",
            "row_count": len(rows),
            "seeds": seeds,
    ...
    write_json(
        out / "real_task_transform_prompt_manifest.json",
        {
            "schema_version": "phase_140u_real_task_transform_prompt_manifest_v1",
            "natural_ish_task_text_primary": True,
            "direct_pocket_value_marker_forbidden_in_main_eval": True,
            "visible_wrong_value_present": True,
            "noisy_distractors_present": True,
            "source_marker": SOURCE_MARKER,
            "target_payload_markers": TARGET_PAYLOAD_MARKERS,
            "target_payload_marker_counts": dict(sorted(marker_counts.items())),
            "minimal_gate_marker": NATURAL_GATE,
        },
    )
    append_progress(out, "transform eval row build", row_count=len(rows), family_count=len(family_counts), target_source_diff_rate=binding["target_differs_from_source_rate"])
    ...
    selection = {
        "schema_version": "phase_140u_selection_report_v1",
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_140u_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    ablation_checkpoint, ablation_manifest = manifests["closed_pocket_no_transform"]
    copy_checkpoint, copy_manifest = manifests["copy_only_source_candidate"]
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_140u_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_json(out / "visible_bypass_control_report.json", {"schema_version": "phase_140u_visible_bypass_control_report_v1", "visible_bypass_control_failed": control_report["visible_bypass_control_failed"]})
    write_json(out / "noisy_distractor_control_report.json", {"schema_version": "phase_140u_noisy_distractor_control_report_v1", "noisy_distractor_control_failed": control_report["noisy_distractor_control_failed"]})
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_140u_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {
        "schema_version": "phase_140u_generated_before_scoring_report_v1",
        "passed": True,
    ...
    write_json(out / "freshness_leakage_audit.json", {"schema_version": "phase_140u_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False})

    copy_report = {
        "schema_version": "phase_140u_copy_only_shortcut_report_v1",
        ...
        "copy_only_control_failed": control_report["copy_only_source_control_failed"],
        "copy_candidate_source_control_failed": control_report["copy_candidate_source_control_failed"],
    }
    write_json(out / "copy_only_shortcut_report.json", copy_report)

    comparison = {
        "schema_version": "phase_140u_arm_comparison_v1",
    ...
    write_json(out / "transform_bridge_metrics.json", {"schema_version": "phase_140u_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    write_json(out / "transform_binding_metrics.json", {"schema_version": "phase_140u_transform_binding_metrics_v1", "main_transform_accuracy": main_metrics["transform_accuracy"], "target_differs_from_source_rate": binding["target_differs_from_source_rate"], "source_copy_shortcut_rate": main_metrics["source_copy_shortcut_rate"], "copy_only_shortcut_detected": main_metrics["copy_only_shortcut_detected"]})
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_140u_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_140u_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    write_json(out / "per_scaffold_metrics.json", {"schema_version": "phase_140u_per_scaffold_metrics_v1", "main": main_metrics["per_scaffold"], "ablation": ablation_metrics["per_scaffold"]})
    write_json(out / "arm_comparison.json", comparison)
    append_progress(out, "aggregate analysis", transform_accuracy=comparison["main_transform_accuracy"], copy_only_shortcut_detected=comparison["copy_only_shortcut_detected"])
    ...
    summary = {
        "schema_version": "phase_140u_summary_v1",
        "milestone": MILESTONE,
        "status": "complete",
        "boundary": BOUNDARY_TEXT,
        "upstream": upstream,
    ...
    write_json(out / "queue.json", {"schema_version": "phase_140u_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
