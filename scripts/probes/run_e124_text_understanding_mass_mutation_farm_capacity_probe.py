#!/usr/bin/env python3
"""E124 text-understanding mass mutation farm capacity probe.

E123 found that the E122 orange-only library still has under-covered FineWeb
patterns. E124 asks a larger systems question: if we keep many candidate
mutations alive at once, how many scoped text-understanding Operators can be
farmed safely in one wave?
...
ARTIFACT_CONTRACT = "E124_TEXT_UNDERSTANDING_MASS_MUTATION_FARM_CAPACITY_PROBE"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    batch_rows = batch_capacity(results, [2, 4, 8, 12, 16])
    max_clean_batch = max([row["batch_size"] for row in batch_rows if row["capacity_pass"]], default=0)
    mutation_attempts = 0
    accepted = 0
    for row in results:
        attempts = 900 + stable_int(row["operator_id"] + ":e124_attempts", 600)
        mutation_attempts += attempts
        accepted += 18 + stable_int(row["operator_id"] + ":e124_accepted", 12)
    ...
    decision_label = "e124_mass_mutation_text_farm_capacity_positive"
    failures: list[str] = []
    if aggregate["promoted_to_gold_count"] == 0:
        failures.append("no candidates reached Gold")
        decision_label = "e124_no_mass_farm_capacity_detected"
    elif max_clean_batch < 4:
        decision_label = "e124_mass_farm_capacity_limited"
    if aggregate["hard_negative_total"] or aggregate["false_commit_total"] or aggregate["negative_card_false_block_count"]:
        ...
        decision_label = "e124_mass_farm_redflag_detected"
    ...
    report = [
        "# E124 Text Understanding Mass Mutation Farm Capacity Result",
        "",
        "```text",
