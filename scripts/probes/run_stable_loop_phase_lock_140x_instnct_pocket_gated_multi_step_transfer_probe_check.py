#!/usr/bin/env python3
"""Checker for 140X multi-step transfer probe."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_140x_instnct_pocket_gated_multi_step_transfer_probe/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_140x_instnct_pocket_gated_multi_step_transfer_probe.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_140x_instnct_pocket_gated_multi_step_transfer_probe_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_140X_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_PROBE_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_140X_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_PROBE_RESULT.md",
]
ALLOWED_MUTATIONS = {RUNNER, CHECKER, *DOCS}
REQUIRED_ARTIFACTS = [
    "queue.json", "progress.jsonl", "upstream_140w_manifest.json", "eval_config.json",
...
def check_static_files() -> list[str]:
    ...
    for rel in [RUNNER, CHECKER, *DOCS]:
        ...
        for term in ["140X", "multi-step", "source", "intermediate", "pocket"]:
            if term not in text:
...
def check_artifacts() -> list[str]:
    ...
    if failures:
        return failures
    upstream = load_json(SMOKE_ROOT / "upstream_140w_manifest.json")
    ...
    if upstream.get("decision") != "multi_step_transfer_probe_recommended":
        failures.append("BAD_140W_DECISION")
    if upstream.get("next") != "140X_INSTNCT_POCKET_GATED_MULTI_STEP_TRANSFER_PROBE":
        failures.append("BAD_140W_NEXT")
    ...
    if main.get("final_answer_accuracy", 0.0) < 0.60:
        failures.append("FINAL_ACCURACY_TOO_LOW")
    if main.get("step1_intermediate_accuracy", 0.0) < 0.60:
        failures.append("STEP1_ACCURACY_TOO_LOW")
    if main.get("step2_final_accuracy", 0.0) < 0.60:
        failures.append("STEP2_ACCURACY_TOO_LOW")
    if main.get("pocket_writeback_rate", 0.0) < 0.75:
        failures.append("WRITEBACK_TOO_LOW")
    if main.get("contrast_group_accuracy", 0.0) < 0.60:
        failures.append("CONTRAST_TOO_LOW")
    if ablation.get("final_answer_accuracy", 1.0) > 0.15:
    ...
    if binding.get("main_step2_final_accuracy", 0.0) < 0.60:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 140X multi-step transfer probe")
    ...
    payload = {"schema_version": "phase_140x_checker_result_v1", "status": "pass" if not failures else "fail", "failures": failures}
