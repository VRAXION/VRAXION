#!/usr/bin/env python3
"""Static/artifact checker for STABLE_LOOP_PHASE_LOCK_128."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_128_post_calibration_repair_ceiling_and_gap_remap/smoke"
REQUIRED_DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_128_POST_CALIBRATION_REPAIR_CEILING_AND_GAP_REMAP_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_128_POST_CALIBRATION_REPAIR_CEILING_AND_GAP_REMAP_RESULT.md",
]
REQUIRED_SOURCE = [
    "scripts/probes/run_stable_loop_phase_lock_128_post_calibration_repair_ceiling_and_gap_remap.py",
    "scripts/probes/run_stable_loop_phase_lock_128_post_calibration_repair_ceiling_and_gap_remap_check.py",
]
...
REQUIRED_ARTIFACTS = [
    ...
    "post_calibration_ceiling_dataset.jsonl",
...
REQUIRED_TERMS = [
    "STABLE_LOOP_PHASE_LOCK_128_POST_CALIBRATION_REPAIR_CEILING_AND_GAP_REMAP",
    "eval-only post-calibration ceiling/gap remap",
...
ARMS = {
    MAIN_ARM,
    "PRE_CALIBRATION_REPAIR_BASELINE",
...
def check_artifacts() -> list[str]:
    ...
    if "POST_CALIBRATION_REPAIR_CEILING_AND_GAP_REMAP_POSITIVE" not in verdicts:
        failures.append("POST_CALIBRATION_REPAIR_CEILING_AND_GAP_REMAP_RESULT_MISSING")
    for key, expected in [
    ...
    dataset = read_jsonl(SMOKE_ROOT / "post_calibration_ceiling_dataset.jsonl")
    ...
    if (
        ...
        or checkpoint.get("target_126_checkpoint_read_only") is not True
        or checkpoint.get("provenance_matches_127") is not True
        or not checkpoint.get("upstream_127_repaired_checkpoint_path")
        or not checkpoint.get("upstream_126_target_checkpoint_path")
    ):
    ...
    if not {"112", "118", "119", "120", "121", "122", "123", "124", "125", "126", "127"}.issubset(compared):
    ...
    if len(load_json(SMOKE_ROOT / "family_metrics.json").get("families", {})) != 24:
    ...
    delta = load_json(SMOKE_ROOT / "post_calibration_delta_vs_124.json")
    if delta.get("hallucination_refusal_breakpoint_resolved") is not True or delta.get("calibration_preserved") is not True:
        failures.append("CALIBRATION_REGRESSION_DETECTED")
    preservation = load_json(SMOKE_ROOT / "reasoning_state_calibration_preservation_report.json")
    for key in ["reasoning_preserved", "state_preserved", "calibration_preserved"]:
    ...
    if targets.get("recommended_next") != "129_TARGETED_POST_CALIBRATION_REPAIR_OR_SCALE_PLAN" or not targets.get("primary_next_repair_target"):
    ...
    if decision.get("decision") != "post_calibration_repair_ceiling_gap_map_complete" or decision.get("next") != "129_TARGETED_POST_CALIBRATION_REPAIR_OR_SCALE_PLAN":
        failures.append("DECISION_NEXT_MISMATCH")
    for key in ["ceiling_status", "first_breakpoint_family", "top_failure_families", "primary_next_repair_target", "reasoning_preserved", "state_preserved", "calibration_preserved"]:
...
def main() -> int:
    ...
    if failures:
        print("128 checker failed:")
        for failure in sorted(set(failures)):
            print(f" - {failure}")
        return 1
    print("128 checker passed")
    return 0
