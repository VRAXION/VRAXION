#!/usr/bin/env python3
"""143V helper-only selected marker occurrence count rejection prototype."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_143V_SELECTED_MARKER_OCCURRENCE_COUNT_REJECTION_PROTOTYPE"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_143v_selected_marker_occurrence_count_rejection_prototype/smoke")
DEFAULT_143U_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_143u_duplicate_selected_marker_conflict_rejection_helper_primitive_plan/smoke")
...
POSITIVE_DECISION = "selected_marker_occurrence_count_rejection_prototype_positive"
POSITIVE_NEXT = "143W_SELECTED_MARKER_OCCURRENCE_COUNT_REJECTION_SCALE_CONFIRM"
...
BOUNDARY_TEXT = (
    "143V is constrained helper/backend evidence only: prompt-visible "
...
FAMILIES = ["SINGLE_SELECTED_MARKER_POSITIVE", "POCKET_MARKER_ORDER_PERMUTATION"]
PERMUTATIONS = [
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_143v", HELPER_PATH)
...
def require_143u(root: Path) -> dict[str, Any]:
    required = ["decision.json", "target_143v_milestone_plan.json", "selected_repair_recommendation.json", "primitive_repair_requirements.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 143U artifacts: {missing}")
    decision = read_json(root / "decision.json")
    target = read_json(root / "target_143v_milestone_plan.json")
    recommendation = read_json(root / "selected_repair_recommendation.json")
    requirements = read_json(root / "primitive_repair_requirements.json")
    if decision.get("decision") != "duplicate_selected_marker_conflict_rejection_primitive_plan_recommended":
        raise RuntimeError(f"bad 143U decision: {decision.get('decision')}")
    if decision.get("selected_option") != "selected_marker_occurrence_count_must_equal_one":
        raise RuntimeError(f"bad 143U selected option: {decision.get('selected_option')}")
    if decision.get("next") != "143V_SELECTED_MARKER_OCCURRENCE_COUNT_REJECTION_PROTOTYPE":
        raise RuntimeError(f"bad 143U next: {decision.get('next')}")
    occurrence = target.get("selected_marker_occurrence_counting_policy", {})
    if occurrence.get("method") != "line.strip().startswith(selected_marker)":
        raise RuntimeError("143U target does not require line-prefix occurrence counting")
    scope = target.get("allowed_helper_change_scope", {})
    if scope.get("only_function_may_change") != "_instnct_select_rule_selected_pocket_value":
        raise RuntimeError("143U target helper change scope mismatch")
    return {"root": rel(root), "decision": decision, "target_143v_milestone_plan": target, "selected_repair_recommendation": recommendation, "primitive_repair_requirements": requirements}
...
def build_prompt(
    ...
    *,
    ...
    lines = [
        f"SELECTED_MARKER_OCCURRENCE_PROTOTYPE family={row['family']} style={row['scaffold_id']} group={row['contrast_group_id']}",
        gate_marker,
        ...
        for family_index, family in enumerate(FAMILIES):
            for group_index in range(groups_per_family):
                for slot in range(group_size):
                    ...
                    row = {
                        "schema_version": "phase_143v_eval_row_v1",
                        "row_id": f"143V_seed{seed}_fam{family_index:02d}_group{group_index:02d}_slot{slot:02d}",
                        "seed": seed,
                        "family": family,
                        "scaffold_id": f"{family}_style_{group_index % 12:02d}",
                        "contrast_group_id": f"143V_{seed}_{family}_{group_index:02d}",
                        ...
                        "pocket_a_value": f"EV143V_{seed}_F{family_index}_G{group_index}_A_S{slot}",
                        "pocket_b_value": f"EV143V_{seed}_F{family_index}_G{group_index}_B_S{slot}",
                        "pocket_c_value": f"EV143V_{seed}_F{family_index}_G{group_index}_C_S{slot}",
                        "stale_pocket_value": f"EV143V_STALE_{seed}_{family_index}_{group_index}_{slot}",
                        "visible_wrong_value": f"EV143V_VISIBLE_{seed}_{family_index}_{group_index}_{slot}",
                        "noisy_distractor_values": [f"EV143V_NOISE_{seed}_{family_index}_{group_index}_{slot}_0", f"EV143V_NOISE_{seed}_{family_index}_{group_index}_{slot}_1"],
                        "duplicate_first_value": f"EV143V_DUP_FIRST_{seed}_{family_index}_{group_index}_{slot}",
                        "duplicate_last_value": f"EV143V_DUP_LAST_{seed}_{family_index}_{group_index}_{slot}",
                        "duplicate_same_value": f"EV143V_DUP_SAME_{seed}_{family_index}_{group_index}_{slot}",
                        "duplicate_non_selected_first_value": f"EV143V_NONSEL_FIRST_{seed}_{family_index}_{group_index}_{slot}",
                        "duplicate_non_selected_last_value": f"EV143V_NONSEL_LAST_{seed}_{family_index}_{group_index}_{slot}",
                        "following_line_value": f"EV143V_FOLLOWING_LEAK_{seed}_{family_index}_{group_index}_{slot}",
                    }
...
def record_helper_request(audit_rows: list[dict[str, Any]], arm: str, row_id: str, request: dict[str, Any]) -> None:
    ...
    audit_rows.append(
        {
            "schema_version": "phase_143v_helper_request_audit_row_v1",
            "arm": arm,
            "row_id": row_id,
...
def run_arm(helper: Any, out: Path, arm: str, rows: list[dict[str, Any]], checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int, heartbeat_sec: int, request_audit: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        results.append(
            {
                "schema_version": "phase_143v_generation_result_v1",
                "arm": arm,
                "row_id": row["row_id"],
                "seed": row["seed"],
                "family": row["family"],
...
def score_row(row: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143v_scoring_result_v1",
        "arm": result["arm"],
        "row_id": row["row_id"],
        "seed": row["seed"],
        "family": row["family"],
    ...
    for row in rows:
        ...
        for winner_match in re.finditer(r"\bwinner\s*=", prompt, re.IGNORECASE):
            ...
            if not allowed_re.search(window):
                ...
                break
    return {"schema_version": "phase_143v_prompt_scanner_report_v1", "case_insensitive_regex": True, "passed": not failures, "failure_count": len(failures), "failures": failures[:50]}


def helper_request_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_143v_helper_request_audit_v1",
...
def shared_helper_diff_audit(helper: Any) -> dict[str, Any]:
    ...
    before_rule_fn = extract_function(before, "_instnct_select_rule_selected_pocket_value")
    after_rule_fn = extract_function(after, "_instnct_select_rule_selected_pocket_value")
    expected_keys = {"prompt", "checkpoint_path", "checkpoint_hash", "seed", "max_new_tokens", "generation_config"}
    expected_forbidden = {"expected_output", "expected_payload", "expected_answer", "scorer_metadata", "labels", "oracle_data", "target_json", "gold_output", "eval_family", "answer", "expected_values"}
    return {
        "schema_version": "phase_143v_shared_helper_diff_audit_v1",
        "helper_source_sha256_before": sha256_text(before),
        "helper_source_sha256_after": sha256_text(after),
        "source_changed": before != after,
        "only_rule_selected_function_changed": remove_function(before, "_instnct_select_rule_selected_pocket_value") == remove_function(after, "_instnct_select_rule_selected_pocket_value"),
        "validate_request_unchanged": extract_function(before, "validate_request") == extract_function(after, "validate_request"),
        "allowed_request_keys_unchanged": set(getattr(helper, "ALLOWED_REQUEST_KEYS", set())) == expected_keys,
        "forbidden_request_keys_not_loosened": expected_forbidden <= set(getattr(helper, "FORBIDDEN_REQUEST_KEYS", set())),
        "old_decoder_path_unchanged": extract_function(before, "_instnct_select_open_pocket_value") == extract_function(after, "_instnct_select_open_pocket_value"),
        "raw_generate_unchanged": extract_function(before, "raw_generate") == extract_function(after, "raw_generate"),
        "non_instnct_generation_path_unchanged": extract_function(before, "build_model_from_state") == extract_function(after, "build_model_from_state"),
        "new_behavior_manifest_gated": "decoder.get(\"type\") == RULE_SELECTED_POCKET_BINDING_DECODER" in after or "decoder.get('type') == RULE_SELECTED_POCKET_BINDING_DECODER" in after,
        "candidate_line_regex_present": "candidate_line_re" in after_rule_fn and "re.escape(selected_marker)" in after_rule_fn,
        "same_line_extraction_present": "candidate_lines[0][1]" in after_rule_fn,
        "raw_prompt_find_removed_for_selected_marker": "prompt.find(selected_marker)" not in after_rule_fn,
        "following_line_segment_scan_removed": "pos + len(selected_marker)" not in after_rule_fn,
...
def static_manifest_integrity_report(manifest: dict[str, Any], results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143v_static_manifest_integrity_report_v1",
...
def legacy_manifest_regression(helper: Any, out: Path, rows: list[dict[str, Any]], max_new_tokens: int, request_audit: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143v_legacy_manifest_regression_report_v1",
...
def control_rows(rows: list[dict[str, Any]], kind: str, count: int = 96) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(selected):
        row["row_id"] = f"143V_{kind}_{index:03d}"
...
def fallback_report(name: str, scored: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": f"phase_143v_{name}_report_v1",
...
def order_permutation_report(rows: list[dict[str, Any]], scored: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase_143v_pocket_marker_order_permutation_report_v1",
        ...
        "passed": set(PERMUTATIONS) <= observed_orders and metrics["selected_pocket_to_marker_binding_accuracy"] >= 0.95 and metrics["first_prompt_marker_shortcut_rate"] == 0.0,
    }
...
def choose_decision(metrics: dict[str, Any], helper_diff: dict[str, Any], request_audit: dict[str, Any], prompt_report: dict[str, Any], static_report: dict[str, Any]) -> dict[str, Any]:
    integrity = (
        helper_diff["only_rule_selected_function_changed"]
        and helper_diff["validate_request_unchanged"]
        and helper_diff["allowed_request_keys_unchanged"]
        and helper_diff["forbidden_request_keys_not_loosened"]
        and helper_diff["old_decoder_path_unchanged"]
        and helper_diff["raw_generate_unchanged"]
        and helper_diff["non_instnct_generation_path_unchanged"]
        and helper_diff["new_behavior_manifest_gated"]
    ...
    positive = (
        integrity
        and metrics["single_selected_marker_binding_accuracy"] >= 0.95
        and metrics["selected_marker_line_occurrence_count_accuracy"] >= 0.95
        and metrics["selected_marker_candidate_line_parse_accuracy"] >= 0.95
        ...
        and metrics["duplicate_selected_marker_same_value_rejection_rate"] >= 0.95
        and metrics["duplicate_non_selected_marker_binding_accuracy"] >= 0.95
        and metrics["duplicate_non_selected_marker_conflict_binding_accuracy"] >= 0.95
        and metrics["duplicate_non_selected_marker_regression_rate"] == 0.0
        and metrics["zero_selected_marker_fallback_rate"] >= 0.95
    ...
    if not integrity:
    ...
    elif metrics["duplicate_selected_marker_conflict_rejection_rate"] < 0.95 or metrics["duplicate_selected_marker_same_value_rejection_rate"] < 0.95:
        decision = "duplicate_conflict_still_not_rejected"; next_step = "143X_SELECTED_MARKER_OCCURRENCE_REPAIR_FAILURE_ANALYSIS"
    elif metrics["duplicate_non_selected_marker_binding_accuracy"] < 0.95 or metrics["duplicate_non_selected_marker_conflict_binding_accuracy"] < 0.95 or metrics["duplicate_non_selected_marker_regression_rate"] != 0.0:
        decision = "overbroad_duplicate_rejection_detected"; next_step = "143Y_NON_SELECTED_MARKER_DUPLICATE_REGRESSION_ANALYSIS"
    elif metrics["selected_marker_prose_mention_false_positive_rate"] != 0.0 or metrics["selected_marker_prose_line_false_positive_rate"] != 0.0 or metrics["selected_marker_candidate_line_parse_accuracy"] < 0.95:
        decision = "candidate_line_parser_false_positive"; next_step = "143AA_SELECTED_MARKER_CANDIDATE_LINE_PARSER_ANALYSIS"
    ...
    elif metrics["single_selected_marker_binding_accuracy"] < 0.95:
        ...
        decision = "helper_integrity_failure"; next_step = "135G_RAW_HELPER_ORACLE_SHORTCUT_REMOVAL"
    return {
        "schema_version": "phase_143v_decision_v1",
        "decision": decision,
        "verdict": "INSTNCT_SELECTED_MARKER_OCCURRENCE_COUNT_REJECTION_PROTOTYPE_POSITIVE" if decision == POSITIVE_DECISION else "INSTNCT_SELECTED_MARKER_OCCURRENCE_COUNT_REJECTION_PROTOTYPE_BLOCKED",
        "next": next_step,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run 143V selected marker occurrence count rejection prototype")
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--upstream-143u-root", type=Path, default=DEFAULT_143U_ROOT)
    parser.add_argument("--seeds", default="4801,4802,4803")
    ...
    write_json(out / "queue.json", {"schema_version": "phase_143v_queue_v1", "milestone": MILESTONE, "status": "running"})
    seeds = [int(item.strip()) for item in args.seeds.split(",") if item.strip()]
    helper = load_helper()
    upstream = require_143u(resolve_repo_path(args.upstream_143u_root))
    rows = build_rows(seeds, args.groups_per_family, args.group_size)
    request_audit_rows: list[dict[str, Any]] = []
    write_json(out / "upstream_143u_manifest.json", upstream)
    write_json(out / "analysis_config.json", {"schema_version": "phase_143v_analysis_config_v1", "milestone": MILESTONE, "boundary": BOUNDARY_TEXT, "seeds": seeds, "families": FAMILIES, "groups_per_family": args.groups_per_family, "group_size": args.group_size, "helper_modified": True, "decoder": NEW_DECODER, **FALSE_FLAGS})
    ...
    for name, (report_fn, subset) in controls.items():
        results = run_arm(helper, out, name, subset, main_path, main_manifest["checkpoint_sha256"], args.max_new_tokens, 0, request_audit_rows)
    ...
    aggregate_metrics = {
        "schema_version": "phase_143v_aggregate_metrics_v1",
        ...
        "selected_marker_candidate_line_parse_accuracy": 1.0 if reports["selected_marker_prose_line_start"]["fallback_rate"] == 0.0 and reports["following_line_value_leak"]["following_line_value_leak_rate"] == 0.0 else 0.0,
    ...
    decision = choose_decision(aggregate_metrics, helper_diff, request_audit, prompt_report, static_report)
    summary = {"schema_version": "phase_143v_summary_v1", "milestone": MILESTONE, "boundary": BOUNDARY_TEXT, "decision": decision, "aggregate_metrics": aggregate_metrics, **FALSE_FLAGS}
    ...
    write_json(out / "selected_marker_occurrence_count_report.json", {"schema_version": "phase_143v_selected_marker_occurrence_count_report_v1", "selected_marker_line_occurrence_count_accuracy": aggregate_metrics["selected_marker_line_occurrence_count_accuracy"], "passed": aggregate_metrics["selected_marker_line_occurrence_count_accuracy"] >= 0.95})
    write_json(out / "selected_marker_candidate_line_parser_report.json", {"schema_version": "phase_143v_selected_marker_candidate_line_parser_report_v1", "selected_marker_candidate_line_parse_accuracy": aggregate_metrics["selected_marker_candidate_line_parse_accuracy"], "selected_marker_prose_line_false_positive_rate": aggregate_metrics["selected_marker_prose_line_false_positive_rate"], "following_line_value_leak_rate": aggregate_metrics["following_line_value_leak_rate"], "passed": aggregate_metrics["selected_marker_candidate_line_parse_accuracy"] >= 0.95})
    ...
    write_json(out / "queue.json", {"schema_version": "phase_143v_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
