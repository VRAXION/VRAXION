#!/usr/bin/env python3
"""Checker for E103 clarification response ingress/state repair artifacts."""
...
def check_common(root: Path, sample_only: bool) -> list[str]:
    ...
    if sample_only:
        ...
        if manifest.get("artifact_contract") != "E103_CLARIFICATION_RESPONSE_INGRESS_AND_STATE_REPAIR_EXPANSION":
    ...
    if task.get("case_count", 0) < 7000:
        failures.append("too few clarification repair cases")
    if task.get("clarification_response_ingress") is not True:
        failures.append("clarification_response_ingress not true")
    if task.get("state_repair_after_ask") is not True:
        failures.append("state_repair_after_ask not true")
    if task.get("open_domain_dialogue") is not False:
        failures.append("open-domain dialogue unexpectedly enabled")
    if task.get("direct_repair_without_pending_question_allowed") is not False:
        failures.append("direct repair without pending question unexpectedly allowed")
    if task.get("requires_repair_trace") is not True:
        failures.append("repair trace requirement missing")
    if decision.get("failure_count") != 0 or decision.get("decision") != "e103_clarification_response_state_repair_expansion_confirmed":
    ...
    for key in [
        "adversarial_unsafe_repair_rate_max",
        "adversarial_stale_repair_rate_max",
        "adversarial_irrelevant_repair_rate_max",
        "adversarial_false_reask_rate_max",
    ]:
    ...
    if not any(values.get("mean_clarification_repair_success_loss", 0.0) > 0.0 for values in cf_summary.values()):
        failures.append("counterfactual has no positive clarification repair contribution")
    replay_payload = {
...
def main() -> int:
    ...
    summary = {
        "checker": "E103_CLARIFICATION_RESPONSE_INGRESS_AND_STATE_REPAIR_EXPANSION_CHECK",
        "root": str(root),
