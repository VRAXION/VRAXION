...
def validate_target(out: Path, sample_dir: Path, write_summary: bool) -> dict[str, Any]:
    ...
    runner = Path(__file__).resolve().with_name("run_e34a_minimal_evidence_world_harness_smoke.py")
    ...
    if set(task.get("actions", [])) != {"INSPECT(feature)", "ANSWER(cause)"}:
    ...
    learned = system_results.get("learned_mutation_policy", {})
    ask_all = system_results.get("ask_all_until_unique", {})
    forced = system_results.get("forced_initial_answer", {})
    random_control = system_results.get("random_action_control", {})
    if aggregate.get("decision") == "e34a_active_evidence_world_confirmed":
        if learned.get("closed_loop_success", 0.0) < 0.98:
            failures.append("confirmed decision but learned success below threshold")
        if learned.get("trace_exact", 0.0) < 0.98:
            failures.append("confirmed decision but learned trace below threshold")
        if learned.get("wrong_confident_answer", 1.0) > 0.01:
