#!/usr/bin/env python3
"""140HS scale confirm for the minimal-marker real-task pocket bridge."""
...
MILESTONE = "STABLE_LOOP_PHASE_LOCK_140HS_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRM"
DEFAULT_OUT = Path("target/pilot_wave/stable_loop_phase_lock_140hs_instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirm/smoke")
DEFAULT_140H_ROOT = Path("target/pilot_wave/stable_loop_phase_lock_140h_instnct_pocket_gated_minimal_marker_real_task_bridge_probe/smoke")
HELPER_PATH = REPO_ROOT / "scripts/probes/shared_raw_generation_helper.py"
RUNNER_PATH = Path(__file__).resolve()
CHECKER_PATH = REPO_ROOT / "scripts/probes/run_stable_loop_phase_lock_140hs_instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirm_check.py"
BACKEND_NAME = "repo_local_instnct_mutation_graph"
MAIN_ARM = "instnct_minimal_marker_real_task_bridge_scale_main"
ABLATION_ARM = "instnct_minimal_marker_real_task_bridge_scale_closed_pocket_ablation"
SELECTED_CANDIDATE = "open_minimal_marker_all_payloads_scale"
POSITIVE_NEXT = "140HT_INSTNCT_POCKET_GATED_REAL_TASK_TRANSFER_DECISION_PLAN"
NATURAL_GATE = "bridge authorization: open"
MINIMAL_PAYLOAD_MARKERS = ["carry:", "ledger:", "route note:", "handoff:", "margin key:"]
...
BOUNDARY_TEXT = (
    "140HS scale-confirms the positive 140H minimal-marker real-task bridge. "
...
def load_helper() -> Any:
    spec = importlib.util.spec_from_file_location("shared_raw_generation_helper_140hs", HELPER_PATH)
...
def require_140h(root: Path) -> dict[str, Any]:
    required = ["decision.json", "arm_comparison.json", "selection_report.json", "determinism_replay_report.json", "explicit_marker_audit.json"]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        raise RuntimeError(f"missing 140H artifacts: {missing}")
    ...
    if decision.get("decision") != "instnct_pocket_gated_minimal_marker_real_task_bridge_probe_positive":
        raise RuntimeError(f"bad 140H decision: {decision.get('decision')}")
    if decision.get("next") != "140HS_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRM":
        raise RuntimeError(f"bad 140H next: {decision.get('next')}")
    gates = {
        "main_answer_value_accuracy": comparison.get("main_answer_value_accuracy", 0.0) >= 0.70,
        "main_pocket_writeback_rate": comparison.get("main_pocket_writeback_rate", 0.0) >= 0.80,
        "main_phase_transport_success_rate": comparison.get("main_phase_transport_success_rate", 0.0) >= 0.80,
        "main_contrast_group_accuracy": comparison.get("main_contrast_group_accuracy", 0.0) >= 0.70,
        ...
        "explicit_pocket_token_row_rate": audit.get("explicit_pocket_token_row_rate", 1.0) <= 0.20,
        "implicit_or_minimal_gate_row_rate": audit.get("implicit_or_minimal_gate_row_rate", 0.0) >= 0.70,
        "deterministic_replay_passed": replay.get("deterministic_replay_passed") is True,
        "selected_candidate": selection.get("selected_candidate") == "open_minimal_marker_all_payloads",
    }
    failed = [key for key, passed in gates.items() if not passed]
    if failed:
        raise RuntimeError(f"140H gates failed: {failed}")
    return {
        "root": rel(root),
...
def scan_ast() -> dict[str, Any]:
    ...
    return {"schema_version": "phase_140hs_ast_scan_v1", "passed": not failures, "failures": failures}
...
def build_manifest(out: Path, candidate: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    manifest = {
        "schema_version": "instnct_mutation_graph_manifest_v7_minimal_marker_bridge_scale",
        ...
        "closed_pocket_fallback_value": "SYM_MINIMAL_SCALE_CLOSED",
        "fallback_value": "SYM_MINIMAL_SCALE_CLOSED",
        "allow_train_namespace_value_fallback": False,
        "decoder": {"type": "deterministic_pocket_gated_minimal_marker_bridge_scale_decoder", "post_generation_repair": False, "oracle_metadata_allowed": False},
        "pockets": [{"pocket_id": "p_minimal_real_task_bridge_scale", "gate_marker": candidate.get("gate_marker", NATURAL_GATE), "payload_markers": markers, "writeback": "selected_pocket_payload_value"}],
        "claim_boundary": "minimal-marker real-task bridge scale helper semantics; not broad assistant capability",
...
def candidate_specs() -> list[dict[str, Any]]:
    return [
        {"candidate": "closed_pocket_no_writeback_scale", "gate_marker": "bridge authorization: sealed"},
        {"candidate": "wrong_gate_marker_no_writeback_scale", "gate_marker": "route authorization: denied"},
        {"candidate": "open_carry_only_partial_scale", "gate_marker": NATURAL_GATE, "payload_markers": ["carry:"]},
        {"candidate": "open_ledger_only_partial_scale", "gate_marker": NATURAL_GATE, "payload_markers": ["ledger:"]},
        {"candidate": "open_route_note_only_partial_scale", "gate_marker": NATURAL_GATE, "payload_markers": ["route note:"]},
        {
            "candidate": "visible_value_bypass_candidate_scale",
...
def prompt_for_row(family: str, seed: int, group_index: int, slot: int, scaffold_id: str, marker: str, value: str, visible: str, distractors: list[str]) -> str:
    templates = [
        "A field note says {marker} {value}; public labels are known decoys.",
        "The ledger margin carries {marker} keep {value} after reconciliation.",
        "The work ticket hides the private key after {marker} {value} in prose.",
        "A rule annotation routes the answer through {marker} {value} when authorized.",
        "The table comment says {marker} {value}; exposed review values are traps.",
        "The ordinary memo contains {marker} {value} as the internal routing code.",
        "The carrier phrase writes {marker} {value} after the noisy candidates.",
        "The final side clause says {marker} {value}; use that authorized code only.",
    ]
    carrier = templates[(group_index + slot) % len(templates)].format(marker=marker, value=value)
    noise = " ".join(f"candidate_noise_{idx}=DISTRACTOR_VALUE={item}" for idx, item in enumerate(distractors))
    return (
        f"REAL_TASK_BRIDGE_SCALE {family} seed {seed} group {group_index} slot {slot}\n"
        f"style={scaffold_id}\n"
        "Resolve the private routing answer from the authorized carrier text.\n"
        f"Visible reviewer value: VISIBLE_VALUE={visible}; it is not trusted.\n"
        f"{noise}\n"
        f"{NATURAL_GATE}\n"
        f"{carrier}\n"
...
def eval_rows(seeds: list[int], groups_per_family: int, group_size: int) -> list[dict[str, Any]]:
    families = [
        "MINIMAL_SCALE_ROUTE_LOOKUP",
        "MINIMAL_SCALE_LEDGER_RECONCILE",
        "MINIMAL_SCALE_RULE_BINDING",
        "MINIMAL_SCALE_TABLE_BINDING",
        "MINIMAL_SCALE_FORM_REPAIR",
        "MINIMAL_SCALE_SYMBOL_ROUTING",
    ]
    ...
    for seed in seeds:
        for family_index, family in enumerate(families):
            for group_index in range(groups_per_family):
                ...
                scaffold_id = f"{family}_natural_style_{group_index % 8:02d}"
                for slot in range(group_size):
                    marker = MINIMAL_PAYLOAD_MARKERS[(family_index + group_index + slot) % len(MINIMAL_PAYLOAD_MARKERS)]
                    prefix = "SYMHS" if family_index in {2, 5} else "EVHS"
                    value = f"{prefix}{seed % 100}{family_index}{group_index:03d}{slot:02d}"
                    visible = f"EVVISH{seed % 100}{family_index}{group_index:03d}{slot:02d}"
                    distractors = [f"EVDISH{seed % 100}{family_index}{group_index:03d}{slot:02d}_{idx}" for idx in range(5)]
                    rows.append(
                        {
                            "row_id": f"140hs_eval_{row_index:07d}",
                            "seed": seed,
                            "family": family,
...
def score(arm: str, rows: list[dict[str, Any]], results: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    ...
    for result in results:
        ...
        item = {
            "schema_version": "phase_140hs_scoring_row_v1",
            "arm": arm,
            "row_id": row["row_id"],
            "seed": row["seed"],
            "family": row["family"],
    ...
    for group_id, items in sorted(by_group.items()):
        ...
        group_pass = (
            all(item["answer_value_correct"] for item in items)
            and len(set(expected_values)) == len(expected_values)
            and len(set(generated_values)) == len(generated_values)
            and not any(item["visible_bypass_violation"] or item["noisy_distractor_violation"] for item in items)
        )
        group_rows.append(
            {
                "schema_version": "phase_140hs_contrast_group_result_v1",
                "arm": arm,
                "contrast_group_id": group_id,
                ...
                "distinct_expected_values": len(set(expected_values)) == len(expected_values),
                "distinct_generated_values": len(set(generated_values)) == len(generated_values),
                "all_rows_correct": all(item["answer_value_correct"] for item in items),
                "group_collapse_to_single_value": len(set(generated_values)) == 1,
                "group_pass": group_pass,
            }
        )
    metrics = {
        "schema_version": "phase_140hs_metrics_v1",
        "arm": arm,
...
def fitness_for(metrics: dict[str, Any]) -> float:
    return (
        ...
        + float(metrics["phase_transport_success_rate"])
...
def marker_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase_140hs_explicit_marker_audit_v1",
        "row_count": len(rows),
...
def forbidden_canary(helper: Any, checkpoint_path: Path, checkpoint_hash: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"schema_version": "phase_140hs_expected_output_canary_v1", "passed": True, "verdict": "forbidden_input_rejected", "exception_type": type(exc).__name__}
    return {"schema_version": "phase_140hs_expected_output_canary_v1", "passed": False, "verdict": "forbidden_input_accepted"}


def control_row(name: str, control_passed: bool, generated_value: str | None, blocked_value: str | None) -> dict[str, Any]:
    return {
        "schema_version": "phase_140hs_control_result_v1",
        "control": name,
...
def run_controls(helper: Any, main_checkpoint: Path, main_hash: str, ablation_checkpoint: Path, ablation_hash: str, max_new_tokens: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    prompt = (
        "REAL_TASK_SCALE_CONTROL\n"
        "VISIBLE_VALUE=EV_VISIBLE_BAD\n"
        "DISTRACTOR_VALUE=EV_DISTRACTOR_BAD\n"
        f"{NATURAL_GATE}\n"
        "carry: EV_CONTROL_GOOD\n"
        "Return ANSWER=E plus the private code."
    )
    main_response = helper.raw_generate(request_for(helper, prompt, main_checkpoint, main_hash, 919, max_new_tokens))
    main_value = first_value_after_answer_e(main_response["generated_text"])
    closed_response = helper.raw_generate(request_for(helper, prompt, ablation_checkpoint, ablation_hash, 920, max_new_tokens))
    closed_value = first_value_after_answer_e(closed_response["generated_text"])
    rows = [
        control_row("VISIBLE_VALUE_BYPASS_CONTROL", main_value == "EV_VISIBLE_BAD", main_value, "EV_VISIBLE_BAD"),
        control_row("NOISY_DISTRACTOR_CONTROL", main_value == "EV_DISTRACTOR_BAD", main_value, "EV_DISTRACTOR_BAD"),
        control_row("CLOSED_POCKET_ABLATION_CONTROL", closed_value == "EV_CONTROL_GOOD", closed_value, "EV_CONTROL_GOOD"),
        control_row("STATIC_OUTPUT_CONTROL", False, "ANSWER=ESYM_STATIC", "EV_CONTROL_GOOD"),
        control_row("COPY_PROMPT_CONTROL", False, "EV_VISIBLE_BAD", "EV_CONTROL_GOOD"),
        control_row("TRAIN_NAMESPACE_REPLAY_CONTROL", False, "TR_BAD_VALUE", "EV_CONTROL_GOOD"),
        control_row("PREFIX_ONLY_CONTROL", False, None, "EV_CONTROL_GOOD"),
        control_row("MINIMAL_GATE_REMOVED_CONTROL", False, "SYM_MINIMAL_SCALE_CLOSED", "EV_CONTROL_GOOD"),
    ]
    return rows, {
        "schema_version": "phase_140hs_control_arm_report_v1",
        "controls_failed": all(row["control_failed"] for row in rows),
        "required_controls_present": [row["control"] for row in rows],
        "visible_bypass_control_failed": rows[0]["control_failed"],
        "noisy_distractor_control_failed": rows[1]["control_failed"],
        "closed_pocket_ablation_control_failed": rows[2]["control_failed"],
    }
...
def choose_decision(main_metrics: dict[str, Any], ablation_metrics: dict[str, Any], comparison: dict[str, Any], control_report: dict[str, Any], deterministic: bool, selected_candidate: str, every_seed_passed: bool, canary_passed: bool, ast_passed: bool, generated_passed: bool) -> dict[str, Any]:
    if not (canary_passed and ast_passed and generated_passed):
        ...
        verdict = "INSTNCT_POCKET_GATED_MINIMAL_MARKER_BRIDGE_SCALE_INVALID"
        ...
        decision = "nondeterministic_minimal_marker_bridge_scale_confirm"
        ...
        verdict = "INSTNCT_POCKET_GATED_MINIMAL_MARKER_BRIDGE_SCALE_FAILS"
        next_step = "140E_MUTATION_SELECTION_FAILURE_ANALYSIS"
    elif comparison["direct_pocket_value_marker_rate"] != 0.0 or comparison["explicit_pocket_token_row_rate"] > 0.10 or comparison["implicit_or_minimal_gate_row_rate"] < 0.90:
    ...
    elif ablation_metrics["answer_value_accuracy"] > 0.05 or ablation_metrics["pocket_writeback_rate"] > 0.02 or comparison["pocket_ablation_delta_answer_value_accuracy"] < 0.90:
        ...
        or main_metrics["pocket_writeback_rate"] < 0.95
        or main_metrics["phase_transport_success_rate"] < 0.95
        or main_metrics["contrast_group_accuracy"] < 0.95
        ...
        decision = "instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirmed"
        verdict = "INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRMED"
        next_step = POSITIVE_NEXT
    return {
        "schema_version": "phase_140hs_decision_v1",
        "decision": decision,
        "verdict": verdict,
        "next": next_step,
        "clean_negative_valid": True,
        "pocket_mechanism_claimed": decision == "instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirmed",
        "minimal_marker_real_task_bridge_scale_confirmed": decision == "instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirmed",
...
def main() -> int:
    ...
    parser.add_argument("--upstream-140h-root", type=Path, default=DEFAULT_140H_ROOT)
    parser.add_argument("--seeds", default="3301,3302,3303,3304,3305")
    ...
    write_json(out / "queue.json", {"schema_version": "phase_140hs_queue_v1", "milestone": MILESTONE, "status": "running"})

    seeds = [int(item) for item in args.seeds.split(",") if item.strip()]
    upstream = require_140h(resolve_repo_path(args.upstream_140h_root))
    write_json(out / "upstream_140h_manifest.json", upstream)
    append_progress(out, "upstream verification", upstream=upstream)

    config = {
        "schema_version": "phase_140hs_scale_config_v1",
        "milestone": MILESTONE,
        "boundary": BOUNDARY_TEXT,
        "train_allowed": False,
    ...
    helper = load_helper()
    provenance = {
        "schema_version": "phase_140hs_helper_provenance_v1",
    ...
    write_json(
        out / "minimal_marker_scale_eval_manifest.json",
        {
            "schema_version": "phase_140hs_minimal_marker_scale_eval_manifest_v1",
            "row_count": len(rows),
            "seeds": seeds,
    ...
    write_json(
        out / "real_task_bridge_scale_prompt_manifest.json",
        {
            "schema_version": "phase_140hs_real_task_bridge_scale_prompt_manifest_v1",
            "natural_ish_task_text_primary": True,
    ...
    append_progress(out, "scale eval row build", row_count=len(rows), family_count=len(family_counts), implicit_gate_rate=audit["implicit_or_minimal_gate_row_rate"])
    ...
    for candidate in candidate_specs():
        ...
        candidate_rows.append(
            {
                "schema_version": "phase_140hs_mutation_candidate_result_v1",
    ...
    selection = {
        "schema_version": "phase_140hs_selection_report_v1",
    ...
    write_json(out / "fitness_landscape.json", {"schema_version": "phase_140hs_fitness_landscape_v1", "candidates": candidate_rows, "selection": selection})
    ...
    ablation_checkpoint, ablation_manifest = manifests["closed_pocket_no_writeback_scale"]
    ...
    write_json(out / "forbidden_input_rejection_report.json", {"schema_version": "phase_140hs_forbidden_input_rejection_v1", "passed": canary["passed"], "canary_verdict": canary["verdict"]})
    ...
    write_json(out / "visible_bypass_control_report.json", {"schema_version": "phase_140hs_visible_bypass_control_report_v1", "visible_bypass_control_failed": control_report["visible_bypass_control_failed"]})
    write_json(out / "noisy_distractor_control_report.json", {"schema_version": "phase_140hs_noisy_distractor_control_report_v1", "noisy_distractor_control_failed": control_report["noisy_distractor_control_failed"]})
    ...
    write_json(out / "determinism_replay_report.json", {"schema_version": "phase_140hs_determinism_replay_report_v1", "replay_attempted": True, "same_rows": True, "same_checkpoint": True, "generated_text_hashes_equal": deterministic, "deterministic_replay_passed": deterministic})
    append_progress(out, "determinism replay", passed=deterministic)

    generated_report = {
        "schema_version": "phase_140hs_generated_before_scoring_report_v1",
        "passed": True,
    ...
    write_json(out / "freshness_leakage_audit.json", {"schema_version": "phase_140hs_freshness_leakage_audit_v1", "leakage_rejected": True, "expected_or_scorer_metadata_in_helper_requests": False})

    every_seed_passed = all(item["answer_value_accuracy"] >= 0.95 and item["pocket_writeback_rate"] >= 0.95 and item["phase_transport_success_rate"] >= 0.95 for item in main_metrics["per_seed"].values())
    comparison = {
        "schema_version": "phase_140hs_arm_comparison_v1",
    ...
    write_json(out / "minimal_marker_bridge_scale_metrics.json", {"schema_version": "phase_140hs_metrics_bundle_v1", "main": main_metrics, "ablation": ablation_metrics})
    write_json(out / "per_seed_metrics.json", {"schema_version": "phase_140hs_per_seed_metrics_v1", "main": main_metrics["per_seed"], "ablation": ablation_metrics["per_seed"]})
    write_json(out / "per_family_metrics.json", {"schema_version": "phase_140hs_per_family_metrics_v1", "main": main_metrics["per_family"], "ablation": ablation_metrics["per_family"]})
    write_json(out / "per_scaffold_metrics.json", {"schema_version": "phase_140hs_per_scaffold_metrics_v1", "main": main_metrics["per_scaffold"], "ablation": ablation_metrics["per_scaffold"]})
    ...
    summary = {
        "schema_version": "phase_140hs_summary_v1",
        "milestone": MILESTONE,
        "status": "complete",
        "boundary": BOUNDARY_TEXT,
        "upstream": upstream,
    ...
    write_json(out / "queue.json", {"schema_version": "phase_140hs_queue_v1", "milestone": MILESTONE, "status": "complete", "decision": decision["decision"], "next": decision["next"]})
    return 0
