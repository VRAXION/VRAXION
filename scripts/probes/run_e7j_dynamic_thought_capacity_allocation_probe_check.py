#!/usr/bin/env python3
"""Checker for E7J dynamic thought capacity allocation artifacts."""
...
REQUIRED_MEAN_METRICS = (
    ...
    "heldout_average_K",
    "heldout_capacity_fit_rate",
    "heldout_compute_cost",
    "heldout_parameter_cost",
...
def static_scan() -> list[str]:
    ...
    for token in (
        "E7J_DYNAMIC_THOUGHT_CAPACITY_ALLOCATION_PROBE",
    ...
    tree = ast.parse(text)
    mutation_functions = {"mutation_worker", "mutate_candidate", "score_candidate", "candidate_policy", "candidate_pockets"}
...
def check_artifacts(out: Path) -> list[str]:
    ...
    allocation = load_json(out / "capacity_allocation_report.json")
    value = load_json(out / "capacity_value_report.json")
    family_winner = load_json(out / "family_capacity_winner_report.json")
    ...
    if manifest.get("milestone") != "E7J_DYNAMIC_THOUGHT_CAPACITY_ALLOCATION_PROBE":
    ...
    if task.get("public_inputs") != "microsegment_path_plus_family_token_no_public_capacity_labels":
        failures.append("TASK_PUBLIC_INPUTS_BAD")
    if task.get("hidden_capacity_targets_used_for_eval_only") is not True:
        failures.append("HIDDEN_CAPACITY_FLAG_BAD")
    ...
    if allocation.get("external_interface") != "CALL(pocket_id, Flow[D]) -> Flow[D]":
        failures.append("FLOW_INTERFACE_BAD")
    if set(allocation.get("families", {})) != set(FAMILIES):
        failures.append("CAPACITY_FAMILY_SET_BAD")
    if value.get("schema_version") != "e7j_capacity_value_report_v1":
        failures.append("CAPACITY_VALUE_REPORT_BAD")
    ...
    for row in result_rows:
        for family in FAMILIES:
            ...
            for split in ("train", "validation", "heldout", "ood", "counterfactual", "adversarial"):
                ...
                for key in ("answer_accuracy", "route_accuracy", "mean_route_steps", "average_K", "K_distribution", "capacity_fit_rate", "compute_cost", "parameter_cost", "local_repair_gain", "capacity_value_score", "row_level_samples"):
    ...
    for row in mutation_rows:
        ...
        if "K_distribution" not in row.get("final_candidate_summary", {}):
            failures.append(f"K_SUMMARY_MISSING:{label}")
    ...
    if leakage.get("hidden_capacity_target_used_as_model_input") is not False:
        failures.append("LEAK_HIDDEN_CAPACITY_INPUT_BAD")
    ...
    if leakage.get("random_control_passed") is not True and decision.get("decision") != "e7j_leak_or_artifact_detected":
...
def main() -> int:
    ...
    parser.add_argument("--out", default="target/pilot_wave/e7j_dynamic_thought_capacity_allocation_probe")
    args = parser.parse_args()
    out = resolve_out(args.out)
    failures = static_scan()
    failures.extend(check_artifacts(out))
    report = {"schema_version": "e7j_checker_report_v1", "failure_count": len(failures), "failures": failures, "checked_root": out.relative_to(REPO_ROOT).as_posix()}
