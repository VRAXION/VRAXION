#!/usr/bin/env python3
"""Checker for 140HS minimal-marker real-task bridge scale confirm."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_140hs_instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirm/smoke"
RUNNER = "scripts/probes/run_stable_loop_phase_lock_140hs_instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirm.py"
CHECKER = "scripts/probes/run_stable_loop_phase_lock_140hs_instnct_pocket_gated_minimal_marker_real_task_bridge_scale_confirm_check.py"
DOCS = [
    "docs/research/STABLE_LOOP_PHASE_LOCK_140HS_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRM_CONTRACT.md",
    "docs/research/STABLE_LOOP_PHASE_LOCK_140HS_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRM_RESULT.md",
]
ALLOWED_MUTATIONS = {RUNNER, CHECKER, *DOCS}
REQUIRED_ARTIFACTS = [
    "queue.json",
    "progress.jsonl",
    "upstream_140h_manifest.json",
    "scale_config.json",
    ...
    "minimal_marker_scale_eval_manifest.json",
    "real_task_bridge_scale_prompt_manifest.json",
    ...
    "minimal_marker_bridge_scale_metrics.json",
...
def check_static_files() -> list[str]:
    ...
    for rel in [RUNNER, CHECKER, *DOCS]:
        ...
        for term in ["140HS", "scale", "minimal-marker", "real-task", "pocket"]:
            if term not in text:
...
def check_artifacts() -> list[str]:
    ...
    if failures:
        return failures
    upstream = load_json(SMOKE_ROOT / "upstream_140h_manifest.json")
    ...
    eval_manifest = load_json(SMOKE_ROOT / "minimal_marker_scale_eval_manifest.json")
    prompt_manifest = load_json(SMOKE_ROOT / "real_task_bridge_scale_prompt_manifest.json")
    ...
    metrics = load_json(SMOKE_ROOT / "minimal_marker_bridge_scale_metrics.json")
    ...
    if upstream.get("decision") != "instnct_pocket_gated_minimal_marker_real_task_bridge_probe_positive":
        failures.append("BAD_140H_DECISION")
    if upstream.get("next") != "140HS_INSTNCT_POCKET_GATED_MINIMAL_MARKER_REAL_TASK_BRIDGE_SCALE_CONFIRM":
        failures.append("BAD_140H_NEXT")
    if upstream.get("main_answer_value_accuracy", 0.0) < 0.70 or upstream.get("direct_pocket_value_marker_rate", 1.0) != 0.0:
        failures.append("BAD_140H_PROFILE")
    ...
    if eval_manifest.get("row_count", 0) < 2000:
        failures.append("SCALE_EVAL_TOO_SMALL")
    ...
    if audit.get("explicit_gate_pocket_open_row_rate", 1.0) > 0.20:
        failures.append("EXPLICIT_GATE_RATE_TOO_HIGH")
    if audit.get("implicit_or_minimal_gate_row_rate", 0.0) < 0.90:
    ...
    if len(selected_rows) != 1 or selected_rows[0].get("candidate") != "open_minimal_marker_all_payloads_scale":
        failures.append("BAD_SELECTED_CANDIDATE")
    if selection.get("gradient_used") is not False or selection.get("selected_candidate") != "open_minimal_marker_all_payloads_scale":
    ...
    if main.get("exact_answer_accuracy", 0.0) < 0.95:
    ...
    if main.get("phase_transport_success_rate", 0.0) < 0.95:
        failures.append("MAIN_PHASE_TOO_LOW")
    if main.get("contrast_group_accuracy", 0.0) < 0.95:
    ...
    if ablation.get("answer_value_accuracy", 1.0) > 0.05 or ablation.get("pocket_writeback_rate", 1.0) > 0.02:
        failures.append("ABLATION_TOO_STRONG")
    if comparison.get("pocket_ablation_delta_answer_value_accuracy", 0.0) < 0.90:
        failures.append("ABLATION_DELTA_TOO_LOW")
    if comparison.get("every_seed_passed") is not True:
        failures.append("EVERY_SEED_GATE_FAILED")
    ...
    for event in ["startup", "upstream verification", "helper and ast verification", "scale eval row build", "candidate evaluated", "mutation selection", "canary", "final eval generation", "scoring", "controls", "determinism replay", "aggregate analysis", "decision", "final verdict"]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 140HS minimal-marker real-task bridge scale confirm")
    ...
    payload = {"schema_version": "phase_140hs_checker_result_v1", "status": "pass" if not failures else "fail", "failures": failures}
