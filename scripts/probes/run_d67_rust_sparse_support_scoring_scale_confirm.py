#!/usr/bin/env python3
"""D67 Rust sparse support scoring scale confirm.

D66 confirmed Rust sparse aggregation-backed support scoring as a support-cost
control layer. D67 scale-confirms that narrower claim and keeps the
unnecessary/missed counter-support frontier visible.
"""
...
TASK = "D67_RUST_SPARSE_SUPPORT_SCORING_SCALE_CONFIRM"
BOUNDARY = (
    "D67 only scale-confirms Rust sparse aggregation-backed support scoring "
    "and counter-support cost control in controlled symbolic joint formula "
...
D65_ARM_FOR = {
    "D66_BEST_REPLAY": "RUST_SPARSE_SET_INVARIANT_IPF_AGGREGATION",
    "RUST_SPARSE_SUPPORT_SCORING": "RUST_SPARSE_SET_INVARIANT_IPF_AGGREGATION",
    "RUST_SPARSE_COUNTER_SUPPORT_TRIAGE": "RUST_SPARSE_COUNTERFACTUAL_DELTA_SET_AGGREGATION",
    "RUST_SPARSE_SUPPORT_SCORING_CAP_7": "RUST_SPARSE_SET_INVARIANT_IPF_AGGREGATION",
    "RUST_SPARSE_SUPPORT_SCORING_CAP_9": "RUST_SPARSE_SET_INVARIANT_IPF_AGGREGATION",
...
def make_d66_upstream_manifest(repo_root):
    root = repo_root / "target/pilot_wave/d66_rust_sparse_support_scoring_with_aggregation_cost_control/smoke"
    manifest = {
        "upstream": "D66_RUST_SPARSE_SUPPORT_SCORING_WITH_AGGREGATION_COST_CONTROL",
        "expected_decision": "rust_sparse_support_scoring_cost_control_confirmed",
        "expected_next": TASK,
...
def build_source_pack(row, bundle, arm, idx, rust_features):
    ...
    if arm in {"RUST_SPARSE_SUPPORT_SCORING_CAP_7", "COST_MATCHED_RANDOM_SUPPORT_CONTROL"}:
        out = d65r.cap_pack_actions(out, SUPPORT_BUDGET_CAP_7)
        out["support_budget_cap"] = SUPPORT_BUDGET_CAP_7
    if arm == "RUST_SPARSE_SUPPORT_SCORING_CAP_9":
...
def select_policy(arm, pack, learned_gate, policy_actions, idx):
    ...
    if arm == "RUST_SPARSE_COUNTER_SUPPORT_TRIAGE":
...
def make_decision(test_metrics, failed_jobs):
    if failed_jobs:
        return {
            "decision": "support_scoring_scale_not_confirmed",
            "verdict": "D67_FAILED_JOBS",
            "next": "D67_REPAIR",
            "best_arm": None,
    ...
    reference = test_metrics["by_arm_core"].get("D66_BEST_REPLAY", {})
    ...
    random_control = test_metrics["by_arm_core"].get("COST_MATCHED_RANDOM_SUPPORT_CONTROL", {})
    content = test_metrics["by_arm_core"].get("SUPPORT_CONTENT_CORRUPTION_CONTROL", {})
    non_aggregate = test_metrics["by_arm_core"].get("NON_AGGREGATE_DIAGNOSTIC_ONLY", {})
    best = max(
        candidate_arms,
        key=lambda arm: (
    ...
    controls_worse = (
        ...
        and non_aggregate.get("cost_adjusted_accuracy", 0.0) < metric(test_metrics, best, "cost_adjusted_accuracy")
    ...
    pass_gate = (
        ...
        and external >= 0.990
        and support_saved_vs_ablation >= 2.0
        and support_saved_vs_always >= 2.0
        and false_conf <= 0.01
    ...
    if pass_gate:
        high_unnecessary = max(
            unnecessary_core,
            clean_unnecessary,
            mixed_corr_unnecessary,
            mixed_adv_unnecessary,
        ) > HIGH_UNNECESSARY_COUNTER_RATE
        if high_unnecessary:
            return {
        ...
        return {
            "decision": "rust_sparse_support_scoring_scale_confirmed",
            "verdict": "D67_RUST_SPARSE_SUPPORT_SCORING_SCALE_CONFIRMED",
    ...
    return {
        "decision": "support_scoring_scale_not_confirmed",
        "verdict": "D67_SUPPORT_SCORING_SCALE_NOT_CONFIRMED",
        "next": "D67_REPAIR",
        "best_arm": best,
        "reason": reason,
    }
...
def write_reports(out, aggregate, decision):
    ...
    reports = {
        "d66_upstream_manifest.json": aggregate["d66_upstream_manifest"],
        ...
        "fixed_budget_sweep_report.json": {
            "cap_7": metrics["by_arm_core"].get("RUST_SPARSE_SUPPORT_SCORING_CAP_7", {}),
            "cap_9": metrics["by_arm_core"].get("RUST_SPARSE_SUPPORT_SCORING_CAP_9", {}),
            "cost_matched_random_cap_7": metrics["by_arm_core"].get("COST_MATCHED_RANDOM_SUPPORT_CONTROL", {}),
        },
        ...
        "support_budget_report.json": {
            ...
            "cap_7_arm": metrics["by_arm_core"].get("RUST_SPARSE_SUPPORT_SCORING_CAP_7", {}),
            "cap_9_arm": metrics["by_arm_core"].get("RUST_SPARSE_SUPPORT_SCORING_CAP_9", {}),
            "cost_matched_random": metrics["by_arm_core"].get("COST_MATCHED_RANDOM_SUPPORT_CONTROL", {}),
        },
        "ablation_compensation_report.json": {
            "reference": metrics["by_arm_core"].get("D66_BEST_REPLAY", {}),
...
def main():
    ...
    parser.add_argument("--seeds", default="12601,12602,12603,12604,12605")
    ...
    d66_manifest = make_d66_upstream_manifest(repo_root)
    write_json(out / "d66_upstream_manifest.json", d66_manifest)
    ...
    aggregate = {
        ...
        "d66_upstream_manifest": d66_manifest,
