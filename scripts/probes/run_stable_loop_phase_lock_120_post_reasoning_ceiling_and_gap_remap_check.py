#!/usr/bin/env python3
"""Static/artifact checker for STABLE_LOOP_PHASE_LOCK_120."""
...
SMOKE_ROOT = REPO_ROOT / "target/pilot_wave/stable_loop_phase_lock_120_post_reasoning_ceiling_and_gap_remap/smoke"
REQUIRED_DOCS = [
...
REQUIRED_ARTIFACTS = [
    ...
    "post_reasoning_ceiling_dataset.jsonl",
...
REQUIRED_TERMS = [
    "STABLE_LOOP_PHASE_LOCK_120_POST_REASONING_CEILING_AND_GAP_REMAP",
    "eval-only ceiling/gap remap",
...
ARMS = {MAIN_ARM, "PRE_REASONING_REPAIR_BASELINE", "STATIC_OUTPUT_CONTROL", "COPY_PROMPT_CONTROL", "RANDOM_FACT_CONTROL", "RANDOM_SLOT_CONTROL"}
...
def check_artifacts() -> list[str]:
    ...
    if "POST_REASONING_CEILING_AND_GAP_REMAP_POSITIVE" not in verdicts:
        failures.append("POST_REASONING_CEILING_AND_GAP_REMAP_RESULT_MISSING")
    for key, expected in [
    ...
    if config.get("expected_row_count") != 26112 or config.get("full_configured_run_used") is not True:
        failures.append("FULL_CONFIGURED_RUN_NOT_USED")
    if config.get("positive_scored_arm") != MAIN_ARM or set(config.get("arms", [])) != ARMS:
        failures.append("FULL_CONFIGURED_RUN_NOT_USED")
    for key in ["training_performed", "repair_performed", "llm_judge_used", "subjective_scoring_used", "current_world_fact_scoring_used"]:
        if config.get(key) is not False:
            failures.append("ORACLE_SHORTCUT_DETECTED")

    dataset = read_jsonl(SMOKE_ROOT / "post_reasoning_ceiling_dataset.jsonl")
    if len(dataset) != 26112:
        failures.append("FULL_CONFIGURED_RUN_NOT_USED")
    progress_events = {row.get("event") for row in read_jsonl(SMOKE_ROOT / "progress.jsonl")}
    for event in ["start", "upstream_verification", "checkpoint_provenance", "dataset_build", "freshness_leakage_audit", "tier_seed_eval", "aggregate_analysis", "decision_writing", "final_verdict"]:
    ...
    if checkpoint.get("checkpoint_hash_unchanged") is not True or checkpoint.get("checkpoint_mutated") is not False:
    ...
    if len(family_metrics.get("families", {})) != 17:
    ...
    delta = load_json(SMOKE_ROOT / "post_reasoning_delta_vs_116.json")
    if delta.get("reasoning_breakpoint_resolved") is not True:
        failures.append("REASONING_REGRESSION_DETECTED")

    reasoning = load_json(SMOKE_ROOT / "reasoning_regression_report.json")
    if reasoning.get("reasoning_regression_rejected") is not True:
        failures.append("REASONING_REGRESSION_DETECTED")
    if reasoning.get("tier4_reasoning_accuracy", 0.0) < 0.97 or reasoning.get("tier8_reasoning_combo_accuracy", 0.0) < 0.90 or reasoning.get("reasoning_failure_rate", 1.0) > 0.05:
        failures.append("REASONING_REGRESSION_DETECTED")
    ...
    if targets.get("recommended_next") != "121_TARGETED_POST_REASONING_REPAIR_OR_SCALE_PLAN" or not targets.get("primary_next_repair_target"):
    ...
    if decision.get("decision") != "post_reasoning_ceiling_gap_map_complete" or decision.get("next") != "121_TARGETED_POST_REASONING_REPAIR_OR_SCALE_PLAN":
        failures.append("DECISION_NEXT_MISMATCH")
    for key in ["ceiling_status", "top_failure_families", "primary_next_repair_target"]:
...
def main() -> int:
    ...
    if failures:
        print("120 checker failed:")
        for failure in sorted(set(failures)):
            print(f" - {failure}")
        return 1
    print("120 checker passed")
    return 0
