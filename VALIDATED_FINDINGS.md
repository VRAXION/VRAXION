...
| Input Encoding: FREQ vs One-hot vs SDR (with FREQ output) | Validated finding | SDR phi overlap + FREQ out = `22.4%` (best). SDR 64 + FREQ = `14.5%`. FREQ + FREQ (symmetric) = `12.3%`. One-hot 256 + FREQ = `9.5%`. One-hot is too sparse (1/256 active = weak signal), FREQ input too dense (noisy). SDR (20% sparse) is the optimal input: sharp enough to discriminate, strong enough to propagate through 8 ticks. Input and output need **opposite** encoding: input=sparse/discrete, output=smooth/continuous. | Recipes at `instnct/recipes/test_freq_input.py`, `test_onehot_input.py` |
| Output Projection: Language-Aware Sweep | Validated finding | FREQ_ORDER `22.4%` > BIGRAM_SVD `21.8%` > RANDOM `20.8%` > RIVAL_PAIRS `19.0%`. Frequency-ordered output projection (bytes sorted by frequency on a smooth curve) beats random by `+1.6%`. Key insight: **output space topology must match target distribution topology**. Natural language is smooth and frequency-hierarchical, so the output projection should be too. Explicit rivalry axes (discrete +-1) are worse than random — the network needs continuous space, not decision planes. Next frontier: bigram matrix as output projection (contextual output space), pyramid/hierarchical readout. | Recipe at `instnct/recipes/sweep_output_projection.py` |
