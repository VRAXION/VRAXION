...
| Output Dimension Sweep | Validated finding | Sweeping random output projection dimension (H=256, SDR_64 input, learnable theta): 16=`0%`, 32=`0%`, 48=`10.1%`, 64=`14.1%`, 96=`17.2%`, 112=`18.8%`, 128=`12.9%`, 144=`16.6%`, 160=`20.0%`, 176=`18.4%`. **Sweet spot: out_dim=160 (hidden=32) at `20.0%`** — new all-time peak. Larger output dim = richer readout = better byte discrimination. The network needs surprisingly few hidden neurons (32) when the readout space is rich enough (160-dim random projection separates 256 classes well). Non-monotonic: dip at 128, peak at 160. | Not promoted; recipe at `instnct/recipes/sweep_output_dim.py` |
