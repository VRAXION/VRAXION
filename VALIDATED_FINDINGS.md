...
- **Current recipe candidate on `main`:** [`v4.2/english_1024n_18w.py`](v4.2/english_1024n_18w.py) now uses `8` ticks with a triangle-derived `2 add / 1 flip / 5 decay` schedule; it still uses the existing float ternary hidden mask.
...
The current English recipe candidate on `main` is [`v4.2/english_1024n_18w.py`](v4.2/english_1024n_18w.py). It currently uses the triangle-derived `2 add / 1 flip / 5 decay` schedule with the older float ternary edge mask. It is useful evidence, but it is not the canonical architecture default.
