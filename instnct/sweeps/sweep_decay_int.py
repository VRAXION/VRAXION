"""
Int Decay Sweep — pure int retention [0-15]
=============================================
Decay as int: retention_int [0-15], ret = retention_int / 15.0
0 = full forget, 15 = perfect memory.
Sweet spot: int 11-14 (= retention 0.73-0.93).
Random init in sweet spot vs float baseline.
18 workers, 8 ticks, bigram 2seq, charge ReLU, theta=0, dur=2.
"""
...
_use_int_decay = False

def init_w(b, d, sl, nt, wi, wo, bg, uid):
    global _bp, _all_data, _seq_len, _n_train, _input_projection, _output_projection, _bigram, _use_int_decay
...
def _eval_bigram(mask, H, decay_param, seqs):
    rs, cs = np.where(mask != 0)
    sp_vals = mask[rs, cs]
    pat_norm = _bp / (np.linalg.norm(_bp, axis=1, keepdims=True) + 1e-8)
    if _use_int_decay:
        ret = decay_param.astype(np.float32) / 15.0  # int [0-15] -> float [0-1]
    else:
        ret = 1.0 - decay_param  # float decay -> retention
    total = 0.0
...
def worker_eval(args):
    mask_flat, decay_param, H, seed, proposal_type = args
    rng = random.Random(seed)
    np_rng = np.random.RandomState(seed)
    mask = mask_flat.reshape(H, H)
    new_mask = mask.copy()
    new_decay = decay_param.copy()
    ...
    if proposal_type == 'add':
        ...
        if _use_int_decay:
            new_decay[idx] = rng.randint(0, 15)  # resample int [0-15]
        else:
            new_decay[idx] = rng.uniform(0.01, 0.50)  # resample float
    ...
    old_score = _eval_bigram(mask, H, decay_param, seqs)
...
def eval_accuracy_classic(mask, H, input_projection, output_projection, decay_param, use_int, text_bytes, bp):
    pat_norm = bp / (np.linalg.norm(bp, axis=1, keepdims=True) + 1e-8)
    rs, cs = np.where(mask != 0); sp_vals = mask[rs, cs]
    if use_int:
        ret = decay_param.astype(np.float32) / 15.0
    else:
        ret = 1.0 - decay_param
...
def run_config(name, use_int, init_decay, bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection,
               max_steps=1000, n_workers=18, threshold=0.00005):
    mask = np.zeros((H, H), dtype=np.float32)
    decay_param = init_decay.copy()

    schedule = ['add', 'add', 'flip', 'decay', 'decay', 'decay', 'decay', 'decay']

    if use_int:
        desc = f"int [0-15], mean={decay_param.mean():.1f}, range=[{decay_param.min()}-{decay_param.max()}]"
    else:
        desc = f"float, mean={decay_param.mean():.3f}, range=[{decay_param.min():.3f}-{decay_param.max():.3f}]"
    print(f"\n--- {name} ({desc}) ---")
    sys.stdout.flush()
    ...
    try:
        for step in range(1, max_steps+1):
            ...
            args = [(mask_flat, decay_param.copy(), H,
                     33000+step*50+w, ptype) for w in range(n_workers)]
            ...
            if best_r['delta'] > threshold:
                if best_r['type'] in ('add', 'flip') and best_r['new_mask_flat'] is not None:
                    ...
                    decay_param = best_r['new_decay']
                    accepts['decay'] += 1

            if step % 100 == 0:
                elapsed = time.time() - t0
                edges = int(np.count_nonzero(mask))
                ea = np.mean([eval_accuracy_classic(mask, H, input_projection, output_projection, decay_param, use_int, s, bp)
                              for s in eval_seqs])
                acc_history.append((step, ea))
                quality = ea / max(edges, 1) * 100
                dm = decay_param.mean(); ds = decay_param.std()

                print(f"  [{step:4d}] acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
                      f"A={accepts['add']}|F={accepts['flip']}|D={accepts['decay']} "
                      f"decay={dm:.2f}+/-{ds:.2f} [{decay_param.min():.2f}-{decay_param.max():.2f}] "
                      f"{elapsed:.0f}s")
                sys.stdout.flush()
    finally:
    ...
    print(f"  FINAL: acc={ea*100:.2f}% edges={edges} q={quality:.3f}%/e "
          f"D={accepts['decay']} decay={decay_param.mean():.2f}+/-{decay_param.std():.2f} {elapsed:.0f}s")
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    # A: Float baseline (decay [0.08, 0.24])
    drng_a = np.random.RandomState(99)
    results.append(run_config("FLOAT [0.08,0.24]", False,
                              drng_a.uniform(0.08, 0.24, H).astype(np.float32),
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # B: Int [11-14] (= retention 0.73-0.93, matches float sweet spot)
    drng_b = np.random.RandomState(99)
    results.append(run_config("INT [11-14]", True,
                              drng_b.randint(11, 15, size=H).astype(np.int32),
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # C: Int full range [0-15]
    drng_c = np.random.RandomState(99)
    results.append(run_config("INT [0-15] full", True,
                              drng_c.randint(0, 16, size=H).astype(np.int32),
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    # D: Int fix 13 (= retention 0.867 ≈ 0.85)
    results.append(run_config("INT fix=13", True,
                              np.full(H, 13, dtype=np.int32),
                              bp, ALL_DATA, bigram, eval_seqs, H, input_projection, output_projection))

    print(f"\n{'='*70}")
    print(f"  SUMMARY -- INT vs FLOAT DECAY (1000 steps, 2a/1f/5d)")
    print(f"{'='*70}")
    print(f"  {'Name':<22} {'Acc%':>6} {'Edges':>6} {'Q(%/e)':>8} {'D_acc':>6}")
    print(f"  {'-'*22} {'-'*6} {'-'*6} {'-'*8} {'-'*6}")
    for r in results:
        print(f"  {r['name']:<22} {r['acc']*100:6.2f} {r['edges']:6d} {r['quality']:8.3f} {r['decay_accepts']:6d}")
    sys.stdout.flush()
