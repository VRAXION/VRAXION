"""
INSTNCT Injection Scale Sweep + Spike
=======================================
Current injection is too weak (max 0.1) for spiking to matter.
Sweep: scale injection by 1x, 8x, 16x with spike_thr=0.5.
LL eval, w=2 superposition, threshold=0.00005, 2000 steps.
"""
...
_output_projection_f = None; _inj_table = None; _spike_thr = None; _inj_scale = 1.0

def init_w(b, d, sl, nt, wof, it, sthr, iscale):
    global _bp, _all_data, _seq_len, _n_train, _output_projection_f, _inj_table, _spike_thr, _inj_scale
    _bp, _all_data, _seq_len, _n_train = b, d, sl, nt
    _output_projection_f, _inj_table, _spike_thr, _inj_scale = wof, it, sthr, iscale
...
def _eval_loglik(msign, mmag, H, seqs):
    ...
    for text_bytes in seqs:
        ...
        for i in range(len(text_bytes)-1):
            act = state.copy()
            inj = _inj_table[text_bytes[i]].astype(np.float32) / 128.0 * _inj_scale
            if i > 0:
                inj = inj + _inj_table[text_bytes[i-1]].astype(np.float32) / 128.0 * _inj_scale * 0.5
            for t in range(8):
                if t < 2:
                    act = act + inj
...
def eval_facts(msign, mmag, H, wof, bp, it, sthr, iscale):
    ...
    for key, val in FACTS:
        ...
        for i in range(len(tb)-1):
            act = st.copy()
            inj = it[tb[i]].astype(np.float32) / 128.0 * iscale
            if i > 0: inj = inj + it[tb[i-1]].astype(np.float32) / 128.0 * iscale * 0.5
            for t in range(8):
                if t < 2: act = act + inj
                raw = np.zeros(H, dtype=np.float32)
...
def run_config(name, inj_scale, spike_thr, H, bp, inj_table, wof, ALL_DATA, n_steps, n_workers, schedule):
    SEQ_LEN = 120
    eval_rng = np.random.RandomState(9999)
    eval_seqs = [ALL_DATA[off:off+SEQ_LEN] for off in
                 [eval_rng.randint(0, len(ALL_DATA)-SEQ_LEN) for _ in range(20)]]
    sthr_str = f"{spike_thr}" if spike_thr else "None"
    print(f"\n{'='*60}")
    print(f"  {name} (inj_scale={inj_scale}x, spike={sthr_str})")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    pool = Pool(n_workers, initializer=init_w,
                initargs=(bp, ALL_DATA, SEQ_LEN, 2, wof, inj_table, spike_thr, inj_scale))
    try:
        for step in range(1, n_steps+1):
            ptype = schedule[(step-1) % len(schedule)]
            if ptype in ('flip', 'mag_resample') and (mmag > 0).sum() == 0:
                ptype = 'add'
            args = [(msign.flatten(), mmag.flatten(), H, 10000+step*50+w, ptype) for w in range(n_workers)]
            ...
            if step % 500 == 0:
                ...
                overall = eval_facts(msign, mmag, H, wof, bp, inj_table, spike_thr, inj_scale)
...
if __name__ == "__main__":
    ...
    print(f"INJECTION SCALE + SPIKE SWEEP")
    print(f"Prev best (scale=1x, no spike, w=2): 21.8%")

    results = []
    for name, scale, spike in [
        ("1x no spike (baseline)", 1, None),
        ("8x + spike=0.5", 8, 0.5),
        ("16x + spike=1.0", 16, 1.0),
        ("16x + spike=3.0", 16, 3.0),
    ]:
        results.append(run_config(name, scale, spike, H, bp, inj_table, wof,
                                  ALL_DATA, BUDGET, N_WORKERS, SCHEDULE))

    print(f"\n{'='*60}")
    print(f"  INJECTION SCALE SWEEP RESULTS")
    print(f"{'='*60}")
