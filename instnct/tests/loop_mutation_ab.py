"""A/B test: loop macro-mutations vs baseline (drive-only).

Tests whether atomically adding complete loop circuits (random length 2-6)
improves learning compared to single-edge mutations.

The hypothesis: single-edge add can't evolve intentional loops because a
partial loop shows no eval benefit ("half-wheel" problem).  Loop mutations
add the full cycle atomically so the eval sees the reverberating benefit.

Variants:
  - drive:      baseline, single-edge mutations only
  - loop:       50% loop mutations, 50% drive
  - loop_0.3:   30% loop, 70% drive
  - loop_0.7:   70% loop, 30% drive
"""
...
DEFAULT_POLICIES = ["drive", "loop_0.3", "loop", "loop_0.7"]
...
def main():
    ...
    print("LOOP MUTATION A/B TEST", flush=True)
    print(
    ...
    for policy_name in policies:
        ...
        conns_mean = statistics.mean(row["connections"] for row in rows)
        print(
            f"  {policy_name:16s} score={score_mean:5.1f}% +/-{score_std:4.1f}pp "
            f"acc={acc_mean:5.1f}% conns={conns_mean:.0f}",
            flush=True,
        )
