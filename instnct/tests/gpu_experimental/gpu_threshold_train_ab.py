"""Deterministic GPU add-only A/B for English threshold calibration."""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--thresholds", default="0.5,0.1")
...
def summarize_budget(rows: list[dict[str, object]]) -> dict[str, dict[str, float]]:
    ...
    for threshold in thresholds:
        group = [r for r in rows if float(r["threshold"]) == threshold]
    ...
    for step in range(1, attempts + 1):
        ...
        if step % log_every == 0 or step == attempts:
            ...
            if eval_metrics["acc"] > best_eval_acc:
                ...
                best_eval_step = step
            print(
                f"seed={seed} thr={threshold:.2f} attempts={attempts} step={step} "
...
def main() -> None:
    ...
    output = {
        "branch_intent": "threshold_train_ab",
    ...
    output_path = make_output_path(args.output, "gpu_threshold_train_ab.json")
