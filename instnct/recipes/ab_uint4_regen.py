"""
INSTNCT — Uint4 Stamina: regen period sweep
=============================================
With uint4 [0-15] stamina, the old regen=2 ticks might be wrong.
Max stamina=15, drain≈4 → ~4 tokens to deplete.
Test regen periods: 1 (every tick), 2, 3, 4.
"""
...
def run_arm(label, regen_p, seed, steps, eval_seqs):
    ...
    def avg_acc():
        resize()
        return np.mean([eval_crosstoken(net, s, drain, regen_p) for s in eval_seqs])
...
def main():
    ...
    print("=" * 70)
    print("  Uint4 Stamina: regen period sweep")
    print(f"  H={H} | Stamina=[0-15] | Drain=[1-6] per-edge")
    print("=" * 70)

    results = []
    for regen_p in [1, 2, 3, 4]:
        label = f"regen={regen_p}"
        print(f"\n  {label}...", end="", flush=True)
        t0 = time.time()
        acc, accepts = run_arm(label, regen_p, SEED, STEPS, eval_seqs)
        print(f" acc={acc:.3f} | acc#={accepts} | {time.time()-t0:.0f}s")
        results.append((regen_p, acc, accepts))

    print(f"\n{'='*70}")
    results.sort(key=lambda x: -x[1])
    for rp, acc, accepts in results:
        marker = " <<<" if acc == results[0][1] else ""
        print(f"  regen={rp}: acc={acc:.3f} | acc#={accepts}{marker}")

    print(f"\n  Winner: regen={results[0][0]} ({results[0][1]:.3f})")
