"""
Loop evolution v2: adaptive schedule + smart remove
====================================================
- BUILD phase: full ops including loops
- When edges > EDGE_CAP: switch to CRYSTAL (heavy remove, smart prune)
- Smart remove: prefers dead-end (non-loop) edges
- Tracks per-op acceptance rates
"""
...
# Adaptive: switch schedule when edges cross threshold
EDGE_TARGET = 4000  # sweet spot from v1 (peak was ~3700)
...
SCHEDULE_CRYSTAL = [
    'smart_remove', 'reverse', 'theta', 'channel',
    'smart_remove', 'flip', 'smart_remove', 'smart_remove',
]

BP_IN = None
_bp_out = None; _all_data = None; _bigram = None; _pol = None
_tri_neurons = None  # shared loop state for smart remove
...
def worker_eval(args):
    qdata_bytes, theta, channel, pol_f, seed, pt, tri_bytes = args
    ...
    tri_neurons = np.frombuffer(tri_bytes, dtype=bool).copy() if tri_bytes else None
    ...
    if pt == 'add':
        ...
        if not undo: return {'delta': -1e9, 'type': pt}
    elif pt == 'smart_remove':
        undo = []; nq.mutate_remove_smart(rng, undo, tri_neurons=tri_neurons)
        if not undo: return {'delta': -1e9, 'type': pt}
    elif pt == 'flip':
...
if __name__ == "__main__":
    ...
    print(f"H={H}, ticks={TICKS}, edge_target={EDGE_TARGET}")
    ...
    try:
        for step in range(1, MAX_STEPS + 1):
            ...
            if edges > EDGE_TARGET and phase == 'BUILD':
                phase = 'CRYSTAL'
                print(f"  >>> SWITCH TO CRYSTAL at step {step} (edges={edges})")
            elif edges < EDGE_TARGET * 0.8 and phase == 'CRYSTAL':
                phase = 'BUILD'
                print(f"  >>> SWITCH TO BUILD at step {step} (edges={edges})")

            schedule = SCHEDULE_BUILD if phase == 'BUILD' else SCHEDULE_CRYSTAL
            pt = schedule[(step - 1) % len(schedule)]

            if pt in ('remove', 'smart_remove', 'reverse') and edges < 50:
                pt = 'add'
            if edges == 0:
                pt = 'add'

            # Update loop cache every 50 steps for smart_remove
            if 'smart_remove' in pt and (step - tri_cache_step > 50 or tri_neurons_cache is None):
                _, tri_neurons_cache = qm_tmp.loop_levels()
                tri_cache_step = step

            tri_bytes = tri_neurons_cache.tobytes() if tri_neurons_cache is not None else None

            proposed_by_type[pt] += 1
            args = [(qdata.tobytes(), theta.copy(), channel.copy(), pol_f.copy(),
                     1000 + step * 50 + w, pt, tri_bytes) for w in range(N_WORKERS)]
            ...
            if step % EVAL_EVERY == 0:
                ...
                rates = {}
                for op in sorted(set(list(SCHEDULE_BUILD) + list(SCHEDULE_CRYSTAL))):
                    p = proposed_by_type[op]; a = acc_by_type[op]
                    if p > 0: rates[op] = f"{a}/{p}({a/p*100:.0f}%)"
                rate_str = ' '.join(f"{k}={v}" for k, v in rates.items())

                eval_history.append({
                    'step': step, 'eval': round(ea * 100, 2),
                    'best': round(best_eval * 100, 2), 'phase': phase,
                    'edges': cur_edges, 'bidir': qm_now.count_bidir(),
                    'tri_n': int(tri_s.sum()),
                })

                sps = step / elapsed
                print(f"[{step:5d}] {phase:7s} eval={ea*100:.1f}% best={best_eval*100:.1f}% "
                      f"edges={cur_edges} bidir={qm_now.count_bidir()} tri={tri_s.sum()} "
                      f"| {rate_str} | {elapsed:.0f}s ({sps:.1f}sps)")
                sys.stdout.flush()
    ...
    for op in sorted(set(list(SCHEDULE_BUILD) + list(SCHEDULE_CRYSTAL))):
        p = proposed_by_type[op]; a = acc_by_type[op]
        if p > 0: print(f"    {op:15s}: {a:4d}/{p:4d} = {a/p*100:5.1f}%")
    print(f"{'=' * 70}")

    out = {
        'config': {'H': H, 'ticks': TICKS, 'edge_target': EDGE_TARGET,
                   'schedule_build': SCHEDULE_BUILD, 'schedule_crystal': SCHEDULE_CRYSTAL},
        ...
        'acceptance': {op: {'accepted': acc_by_type[op], 'proposed': proposed_by_type[op]}
                       for op in sorted(set(list(SCHEDULE_BUILD) + list(SCHEDULE_CRYSTAL)))
                       if proposed_by_type[op] > 0},
        'history': eval_history,
    }
    out_path = os.path.join(BASE_DIR, "data", "loop_evolution_v2_result.json")
