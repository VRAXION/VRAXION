"""
Quad trit: 4 types x 3 levels (E=0.7, ~=1.0, H=1.3)
=====================================================
Best of both: 4 channels (like sin binary) + trit values (like C winner).
No sin(), just LUT.

A: trit 2 types (C winner baseline = 16.6% at H=64)
B: quad trit 4 types (balanced 4-4 with pi/2 shifts)
C: quad trit 4 types (hand-tuned max separation)
D: quad trit 4 types (interleaved pattern)
"""
...
E = 0.7; M = 1.0; H_ = 1.3  # easy / neutral / hard
...
# A: trit 2 types (C winner from previous run)
TRIT_2 = np.array([
    [H_, H_, M,  E,  E,  E,  M,  H_],
    [H_, E,  E,  M,  H_, H_, E,  M ],
], dtype=np.float32)

# B: quad trit — shift by 2 ticks (pi/2 in 8-tick cycle)
QUAD_SHIFT = np.array([
    [H_, H_, M,  E,  E,  E,  M,  H_],   # type0: base
    [M,  H_, H_, H_, M,  E,  E,  E ],   # type1: shift +2
    [E,  E,  M,  H_, H_, H_, M,  E ],   # type2: shift +4
    [M,  E,  E,  E,  M,  H_, H_, H_],   # type3: shift +6
], dtype=np.float32)

# C: quad trit — hand-tuned max separation (each type "owns" 2 ticks)
QUAD_OWN = np.array([
    [E,  E,  H_, M,  H_, M,  M,  H_],   # type0: easy t0,t1
    [H_, M,  E,  E,  M,  H_, H_, M ],   # type1: easy t2,t3
    [M,  H_, M,  H_, E,  E,  H_, M ],   # type2: easy t4,t5
    [H_, M,  H_, M,  M,  H_, E,  E ],   # type3: easy t6,t7
], dtype=np.float32)

# D: quad trit — interleaved (each type easy every 4th tick)
QUAD_INTER = np.array([
    [E,  M,  H_, M,  E,  M,  H_, M ],   # type0: easy t0,t4
    [M,  E,  M,  H_, M,  E,  M,  H_],   # type1: easy t1,t5
    [H_, M,  E,  M,  H_, M,  E,  M ],   # type2: easy t2,t6
    [M,  H_, M,  E,  M,  H_, M,  E ],   # type3: easy t3,t7
], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    MODES = [
        ('A', 'trit 2 types (baseline)', TRIT_2),
        ('B', 'quad trit shift (4 types, rotate +2)', QUAD_SHIFT),
        ('C', 'quad trit own (4 types, each owns 2 ticks)', QUAD_OWN),
        ('D', 'quad trit interleaved (4 types, every 4th)', QUAD_INTER),
    ]

    print(f"\n{'='*60}")
    print(f"  QUAD TRIT: 4 types x 3 levels (E/~/H)")
    print(f"  H={H}, 1000 steps")
    print(f"{'='*60}")

    for mk, label, table in MODES:
        print(f"\n  {mk}: {label}  ({table.shape[0]} types)")
        for i in range(table.shape[0]):
            row = ' '.join(f"{'E' if table[i,t]<0.85 else ('H' if table[i,t]>1.15 else '~')}" for t in range(TICKS))
            print(f"    type{i}: {row}")
    sys.stdout.flush()
    ...
    for mk, label, table in MODES:
        ...
        results.append({'mode':mk,'label':label,'best':float(best),'time':elapsed,'n_types':n_types})
        print(f"  >> DONE {mk}: best={best*100:.1f}% {elapsed:.0f}s")
        sys.stdout.flush()

    print(f"\n{'='*60}")
    print(f"  QUAD TRIT RESULTS")
    print(f"{'='*60}")
    for r in sorted(results, key=lambda x: -x['best']):
        print(f"  {r['mode']}: {r['label']:45s} types={r['n_types']}  best={r['best']*100:5.1f}%")
    print(f"{'='*60}")
