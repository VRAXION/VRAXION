"""
Phi overlap with EMPTY start — 0% init density
================================================
Same as phi overlap test but the network starts with ZERO edges.
Everything is built by evolution. Saves full checkpoint at the end.
"""
...
SDR_K = int(round(IN_DIM * 0.20))  # 32
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"  PHI OVERLAP EMPTY START")
    print(f"  H={H}, in={IN_DIM}, out={OUT_DIM}, K={SDR_K}")
    print(f"  Init density = 0% (EMPTY)")
    print(f"{'='*60}")
    ...
    print(f"  edges={int(mask.sum())} (EMPTY)")

    pool = Pool(N_WORKERS, initializer=init_w,
        initargs=(ALL_DATA, bigram, pol, ref.freq, ref.phase, ref.rho))

    best_eval=0;stall=0;accepts=0;t_acc=0;add_acc=0;rem_acc=0;flip_acc=0;dec_acc=0
    t0=time.time()
    try:
        for step in range(1, 3001):
            pt = SCHEDULE[(step-1) % len(SCHEDULE)]
            edges = int(mask.sum())
            # Force add if no edges for flip/remove/decay
            ...
            if br['delta'] > THRESHOLD:
                if br['type'] in ('add','remove','flip') and br['new_mask_flat'] is not None:
                    ...
                    if br['type']=='add': add_acc+=1
                    elif br['type']=='remove': rem_acc+=1
                    else: flip_acc+=1
                    accepts+=1
                    ...
                    decay[:]=br['new_decay']; dec_acc+=1; accepts+=1
            if step % EVAL_EVERY == 0:
                ...
                if step % 100 == 0:
                    ...
                    print(f"  [{step:4d}] eval={ea*100:.1f}% best={best_eval*100:.1f}% "
                          ...
                          f"[A={add_acc} R={rem_acc} F={flip_acc} T={t_acc} D={dec_acc}] "
    ...
    ckpt_path = os.path.join(ckpt_dir, "phi_overlap_trained.npz")

    # Build a SelfWiringGraph and inject our trained state
    g = SelfWiringGraph(256, hidden=H, phi_overlap=True, seed=42)
    g.mask = mask
    g.theta = theta
    g.decay = decay
    g.polarity = pol.astype(np.int8)
    g._polarity_f32 = pol
    g.resync_alive()
    g.save(ckpt_path)
    print(f"\n  SAVED: {ckpt_path}")
    ...
    print(f"  Accepts: A={add_acc} R={rem_acc} F={flip_acc} T={t_acc} D={dec_acc} total={accepts}")
    print(f"  Degree: mean={total_deg.mean():.1f} std={total_deg.std():.1f} [{total_deg.min()},{total_deg.max()}]")
    print(f"  Time: {elapsed:.0f}s")
    print(f"  Compare: 5% prefill = 20.8%")
    print(f"{'='*60}")
