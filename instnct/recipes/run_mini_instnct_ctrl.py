"""
Mini INSTNCT as mutation controller — evolves itself
=====================================================
A tiny self-wiring graph (H=16) that learns to pick tools.
Same architecture as main network. No backprop — mutation+selection.
Fitness = accept rate of its picks over rolling 50 steps.
Uses SPIKE COUNT readout (not charge, which resets to 0).
"""
import sys, os, time, random
import numpy as np
from multiprocessing import Pool
from pathlib import Path

sys.stdout.reconfigure(line_buffering=True)

ROOT_DIR = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT_DIR)); sys.path.insert(0, str(ROOT_DIR / "model"))
from graph import SelfWiringGraph

H = 256; N_WORKERS = 18; TICKS = 8; INPUT_DURATION = 2
THRESHOLD = 0.00005; EVAL_EVERY = 20
INIT_DENSITY = 0.05
PHI = (1 + 5**0.5) / 2
IN_DIM = int(round(H / PHI))
OUT_DIM = int(round(H / PHI))
SDR_K = int(round(IN_DIM * 0.20))
WAVE_LUT = SelfWiringGraph.WAVE_LUT

ALL_OPS = ['add','flip','theta','channel','reverse','remove']
N_OPS = len(ALL_OPS)

# Mini INSTNCT controller
CH = 16  # tiny
C_IN = 7   # 6 tool rates + accuracy
C_OUT = 6  # one per op

class MiniINSTNCT:
    """Self-wiring graph controller. Evolves by mutation+selection."""
    def __init__(self, seed=42):
        rng = np.random.RandomState(seed)
        self.H = CH
        self.mask = (rng.rand(CH,CH) < 0.25).astype(bool)  # 25% dense (small net needs more)
        np.fill_diagonal(self.mask, False)
        self.theta = rng.randint(1, 8, size=CH).astype(np.uint8)  # low range for responsiveness
        self.channel = rng.randint(1, 9, size=CH).astype(np.uint8)
        self.polarity = rng.rand(CH) > 0.2  # 80% excitatory
        self.pol_f32 = np.where(self.polarity, 1.0, -1.0).astype(np.float32)
        # Fitness tracking
        self.recent_picks = []  # (op_idx, accepted) tuples
        self.fitness = 0.5  # start neutral

    def vote(self, tool_rates, accuracy):
        """Forward pass: inject features, count spikes per output neuron."""
        inj = np.zeros(CH, np.float32)
        # Input zone [0:C_IN]
        for i, op in enumerate(ALL_OPS):
            inj[i] = tool_rates.get(op, 0.5) * 10.0  # scale to useful range
        inj[6] = accuracy * 10.0

        # Run forward pass — count spikes per neuron across all ticks
        sc = SelfWiringGraph.build_sparse_cache(self.mask)
        state = np.zeros(CH, np.float32)
        charge = np.zeros(CH, np.float32)
        spike_counts = np.zeros(CH, np.float32)

        theta_f = self.theta.astype(np.float32)

        for tick in range(TICKS):
            if tick % 6 == 0:
                charge = np.maximum(charge - 1.0, 0.0)
            if tick < 2:
                state = state + inj
            # Propagate
            if len(sc) >= 2 and len(sc[0]) > 0:
                raw = np.zeros(CH, np.float32)
                rows, cols = sc[0], sc[1]
                np.add.at(raw, cols, state[rows])
                charge += raw
            np.clip(charge, 0.0, 15.0, out=charge)
            # Spike with channel LUT
            theta_mult = WAVE_LUT[self.channel, tick % 8]
            eff_theta = np.clip(theta_f * theta_mult, 1.0, 15.0)
            fired = charge >= eff_theta
            spike_counts += fired.astype(np.float32)
            state = fired.astype(np.float32) * self.pol_f32
            charge[fired] = 0.0

        # Output: last C_OUT neurons' spike counts = votes
        votes = spike_counts[CH-C_OUT:]
        return votes

    def save_state(self):
        return {
            'mask': self.mask.copy(),
            'theta': self.theta.copy(),
            'channel': self.channel.copy(),
            'polarity': self.polarity.copy(),
            'pol_f32': self.pol_f32.copy(),
        }

    def restore_state(self, state):
        self.mask[:] = state['mask']
        self.theta[:] = state['theta']
        self.channel[:] = state['channel']
        self.polarity[:] = state['polarity']
        self.pol_f32[:] = state['pol_f32']

    def mutate(self):
        """One random mutation on the controller."""
        rng = random.Random()
        op = rng.choice(['add', 'remove', 'theta', 'channel', 'flip_pol'])
        if op == 'add':
            r = rng.randint(0, CH-1); c = rng.randint(0, CH-1)
            if r != c and not self.mask[r,c]:
                self.mask[r,c] = True
        elif op == 'remove':
            alive = list(zip(*np.where(self.mask)))
            if alive:
                r,c = alive[rng.randint(0, len(alive)-1)]
                self.mask[r,c] = False
        elif op == 'theta':
            idx = rng.randint(0, CH-1)
            self.theta[idx] = np.uint8(rng.randint(1, 8))
        elif op == 'channel':
            idx = rng.randint(0, CH-1)
            self.channel[idx] = np.uint8(rng.randint(1, 8))
        elif op == 'flip_pol':
            idx = rng.randint(0, CH-1)
            self.polarity[idx] = not self.polarity[idx]
            self.pol_f32[idx] = 1.0 if self.polarity[idx] else -1.0

    def update_fitness(self, accepted):
        """Track rolling accept rate as fitness."""
        self.recent_picks.append(1.0 if accepted else 0.0)
        if len(self.recent_picks) > 50:
            self.recent_picks = self.recent_picks[-50:]
        self.fitness = np.mean(self.recent_picks) if self.recent_picks else 0.5

def build_sdr(n, dim, k, seed):
    rng = np.random.RandomState(seed)
    t = np.zeros((n, dim), np.float32)
    for v in range(n): t[v, rng.choice(dim, size=k, replace=False)] = 1.0
    return t

def build_freq_order(dim, bigram, seed=12345):
    freq = bigram.sum(axis=0) + bigram.sum(axis=1)
    rank = np.argsort(freq)[::-1]
    rng = np.random.RandomState(seed)
    p = np.zeros((256, dim), np.float32)
    for i, byte_idx in enumerate(rank):
        t = i / 255.0
        for d in range(dim):
            p[byte_idx, d] = np.sin(2 * np.pi * t * (d+1) / dim * 3) + rng.randn() * 0.3
    p /= np.linalg.norm(p, axis=1, keepdims=True) + 1e-8
    return p.astype(np.float32)

BP_IN = build_sdr(256, IN_DIM, SDR_K, 42)
_bp_out=None;_all_data=None;_bigram=None;_pol=None

def init_w(bpo,data,bg,pol):
    global _bp_out,_all_data,_bigram,_pol
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol

def _eval_bigram(mask, theta, channel, polarity_f, seqs):
    sc = SelfWiringGraph.build_sparse_cache(mask); total = 0.0
    for tb in seqs:
        state=np.zeros(H,np.float32);charge=np.zeros(H,np.float32);s=0.0;n=0
        for i in range(len(tb)-1):
            inj=np.zeros(H,np.float32);inj[0:IN_DIM]=BP_IN[tb[i]]
            state,charge=SelfWiringGraph.rollout_token(inj,mask=mask,theta=theta,
                decay=np.float32(0.16),ticks=TICKS,input_duration=INPUT_DURATION,
                state=state,charge=charge,sparse_cache=sc,polarity=polarity_f,
                channel=channel)
            logits=np.dot(_bp_out,charge[H-OUT_DIM:])
            e=np.exp(logits-logits.max());pred=e/e.sum()
            tgt=_bigram[tb[i]]
            cos=np.dot(pred,tgt)/(np.linalg.norm(pred)*np.linalg.norm(tgt)+1e-8)
            s+=cos;n+=1
        total+=s/n if n else 0
    return total/len(seqs)

def worker_eval(args):
    mf,theta,channel,pol_f,seed,pt=args
    rng=random.Random(seed);nrng=np.random.RandomState(seed)
    mask=mf.reshape(H,H);nm=mask;nt=theta;nc=channel;npf=pol_f
    if pt=='add':
        r=rng.randint(0,H-1);c=rng.randint(0,H-1)
        if r==c or mask[r,c]:return{'delta':-1e9,'type':'add'}
        nm=mask.copy();nm[r,c]=True
    elif pt=='flip':
        idx=rng.randint(0,H-1)
        pb=(pol_f>0);pb[idx]=not pb[idx]
        npf=np.where(pb,1.0,-1.0).astype(np.float32)
    elif pt=='theta':
        idx=rng.randint(0,H-1);nt=theta.copy();nt[idx]=float(rng.randint(1,15))
    elif pt=='channel':
        idx=rng.randint(0,H-1);nc=channel.copy();nc[idx]=np.uint8(rng.randint(1,8))
    elif pt=='reverse':
        alive=list(zip(*np.where(mask)))
        if len(alive)<1:return{'delta':-1e9,'type':'reverse'}
        r,c=alive[rng.randint(0,len(alive)-1)]
        if r==c or mask[c,r]:return{'delta':-1e9,'type':'reverse'}
        nm=mask.copy();nm[r,c]=False;nm[c,r]=True
    elif pt=='remove':
        alive=list(zip(*np.where(mask)))
        if len(alive)<1:return{'delta':-1e9,'type':'remove'}
        r,c=alive[rng.randint(0,len(alive)-1)];nm=mask.copy();nm[r,c]=False
    seqs=[]
    for _ in range(2):
        off=nrng.randint(0,len(_all_data)-100);seqs.append(_all_data[off:off+100])
    old=_eval_bigram(mask,theta,channel,pol_f,seqs)
    new=_eval_bigram(nm,nt,nc,npf,seqs)
    return{'delta':float(new-old),'type':pt,
           'new_mask_flat':nm.flatten() if new>old else None,
           'new_theta':nt if pt=='theta' else None,
           'new_channel':nc if pt=='channel' else None,
           'new_pol_f':npf if pt=='flip' else None}

if __name__ == "__main__":
    sys.path.insert(0, str(ROOT_DIR))
    from lib.data import load_fineweb_bytes, resolve_fineweb_path
    resolve_fineweb_path(); ALL_DATA = load_fineweb_bytes()
    print(f"Loaded {len(ALL_DATA)/1e6:.1f} MB")

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    bigram = np.load(os.path.join(BASE_DIR, "data", "bigram_table.npy"))
    bp_out = build_freq_order(OUT_DIM, bigram)
    eval_rng = np.random.RandomState(9999)
    eval_seqs = [ALL_DATA[o:o+100] for o in [eval_rng.randint(0,len(ALL_DATA)-100) for _ in range(5)]]

    print(f"\n{'='*60}")
    print(f"  MINI INSTNCT CONTROLLER (H={CH}, self-evolving)")
    print(f"  Input: 6 tool rates + accuracy")
    print(f"  Output: spike count per op neuron")
    print(f"  Fitness: rolling 50-step accept rate")
    print(f"  Mutates every 20 steps, keeps if fitness improves")
    print(f"  baselines: fixed=23.8%, Swish=18.8%, trio=18.2%")
    print(f"{'='*60}")
    sys.stdout.flush()

    random.seed(42); np.random.seed(42)
    ref = SelfWiringGraph(max(IN_DIM,16), hidden=H, projection_scale=1.0)
    pol_f32 = np.where(ref.polarity, 1.0, -1.0).astype(np.float32)
    irng = np.random.RandomState(42)
    mask = (irng.rand(H,H) < INIT_DENSITY).astype(bool); np.fill_diagonal(mask, False)
    theta = np.full(H, 1.0, np.float32)
    nrng = np.random.RandomState(42)
    channel = nrng.randint(1, 9, size=H).astype(np.uint8)

    ctrl = MiniINSTNCT(seed=77)
    current_accuracy = 0.0
    tool_rates = {op: 0.5 for op in ALL_OPS}
    op_history = {op: [0.5]*5 for op in ALL_OPS}

    init_w(bp_out, ALL_DATA, bigram, pol_f32)
    pool = Pool(N_WORKERS, initializer=init_w,
        initargs=(bp_out, ALL_DATA, bigram, pol_f32))

    best=0;acc=0;t0=time.time()
    op_counts = {op: 0 for op in ALL_OPS}
    op_accepts = {op: 0 for op in ALL_OPS}
    ctrl_mutations = 0; ctrl_improvements = 0

    for step in range(1, 2001):
        # Controller votes via spike count
        votes = ctrl.vote(tool_rates, current_accuracy)

        # Softmax over spike counts
        if votes.sum() > 0:
            temp = 1.5
            logits = votes / temp
            logits -= logits.max()
            exp_l = np.exp(logits)
            probs = exp_l / (exp_l.sum() + 1e-8)
        else:
            probs = np.ones(N_OPS) / N_OPS  # uniform if no spikes

        # 10% exploration
        if random.random() < 0.10:
            pt = random.choice(ALL_OPS)
        else:
            pt = ALL_OPS[np.random.choice(N_OPS, p=probs)]

        if mask.sum()==0: pt='add'
        op_counts[pt] += 1

        args=[(mask.flatten(),theta.copy(),channel.copy(),pol_f32.copy(),
               1000+step*50+w,pt) for w in range(N_WORKERS)]
        res=pool.map(worker_eval,args);br=max(res,key=lambda x:x['delta'])

        accepted = br['delta'] > THRESHOLD
        if accepted:
            if br['new_mask_flat'] is not None: mask[:]=br['new_mask_flat'].reshape(H,H)
            if br['new_theta'] is not None: theta[:]=br['new_theta']
            if br['new_channel'] is not None: channel[:]=br['new_channel']
            if br['new_pol_f'] is not None: pol_f32[:]=br['new_pol_f']
            acc+=1; op_accepts[br['type']]+=1

        # Update controller fitness
        ctrl.update_fitness(accepted)

        # Update rolling rates
        op_history[pt].append(1.0 if accepted else 0.0)
        if len(op_history[pt]) > 100: op_history[pt] = op_history[pt][-100:]
        tool_rates[pt] = np.mean(op_history[pt])

        # Evolve controller every 20 steps
        if step % 20 == 0:
            old_fitness = ctrl.fitness
            saved = ctrl.save_state()
            ctrl.mutate()
            ctrl_mutations += 1
            # Run 5 votes to check new fitness direction
            # Simple: if current fitness > 0.6, keep (good enough)
            # Otherwise revert
            if ctrl.fitness < old_fitness - 0.05:
                ctrl.restore_state(saved)
            else:
                ctrl_improvements += 1

        if step%EVAL_EVERY==0:
            ea_list=[]
            for s in eval_seqs:
                sc=SelfWiringGraph.build_sparse_cache(mask)
                st=np.zeros(H,np.float32);ch=np.zeros(H,np.float32);cor=0;tot=0
                for i in range(len(s)-1):
                    inj=np.zeros(H,np.float32);inj[0:IN_DIM]=BP_IN[s[i]]
                    st,ch=SelfWiringGraph.rollout_token(inj,mask=mask,theta=theta,
                        decay=np.float32(0.16),ticks=TICKS,input_duration=INPUT_DURATION,
                        state=st,charge=ch,sparse_cache=sc,polarity=pol_f32,
                        channel=channel)
                    logits_e=np.dot(bp_out,ch[H-OUT_DIM:])
                    if np.argmax(logits_e)==s[i+1]:cor+=1
                    tot+=1
                ea_list.append(cor/tot if tot else 0)
            ea=np.mean(ea_list)
            current_accuracy = ea
            if ea>best:best=ea
            if step%200==0:
                v_str = ' '.join(f"{ALL_OPS[i]}={votes[i]:.0f}" for i in range(N_OPS))
                p_str = ' '.join(f"{ALL_OPS[i]}={probs[i]:.0%}" for i in range(N_OPS))
                print(f"  [{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% th={theta.mean():.1f}")
                print(f"    Spikes: {v_str}")
                print(f"    Probs: {p_str}")
                print(f"    Ctrl fitness={ctrl.fitness:.0%} edges={ctrl.mask.sum()} mut={ctrl_mutations} imp={ctrl_improvements}")
                print(f"    Ops: {dict(op_counts)}")
                print(f"    {time.time()-t0:.0f}s")
                sys.stdout.flush()

    pool.terminate();pool.join()
    elapsed=time.time()-t0

    print(f"\n{'='*60}")
    print(f"  MINI INSTNCT CONTROLLER RESULTS")
    print(f"{'='*60}")
    print(f"  best={best*100:.1f}% theta={theta.mean():.1f} {elapsed:.0f}s")
    print(f"  Ops: {dict(op_counts)}")
    print(f"  Accepts: {dict(op_accepts)}")
    rates = ' '.join(f"{op}={op_accepts[op]/max(op_counts[op],1)*100:.0f}%" for op in ALL_OPS)
    print(f"  Accept rates: {rates}")
    print(f"  Controller: fitness={ctrl.fitness:.0%} edges={ctrl.mask.sum()} mutations={ctrl_mutations} improvements={ctrl_improvements}")
    print(f"  baselines: fixed=23.8%, Swish=18.8%, trio=18.2%")

    # Save both main network + controller checkpoints
    ckpt_dir = os.path.join(BASE_DIR, "data")
    np.savez_compressed(os.path.join(ckpt_dir, "mini_ctrl_main_network.npz"),
        mask=mask, theta=theta, channel=channel, pol_f32=pol_f32)
    np.savez_compressed(os.path.join(ckpt_dir, "mini_ctrl_controller.npz"),
        mask=ctrl.mask, theta=ctrl.theta, channel=ctrl.channel,
        polarity=ctrl.polarity, pol_f32=ctrl.pol_f32)
    print(f"\n  Saved: mini_ctrl_main_network.npz + mini_ctrl_controller.npz")
