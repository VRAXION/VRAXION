"""
INSTNCT Permutation Sorting
============================
Can the SWG learn: read shuffled 0-9, output "0123456789" after "="?

Input:  "7350218964=0123456789\n" (21 bytes)
Output is ALWAYS the same -> bigram signal is very strong.

2000 steps baseline, measure sort accuracy on post-= positions.
Random baseline: 10% per digit position.
"""
...
_bp = None; _all_data = None; _seq_len = 63; _n_train = 2
...
# --- Sorting-specific eval ---
EQ_BYTE = ord('=')
NL_BYTE = ord('\n')

def eval_perm_sort(msign, mmag, H, output_projection_f, text_bytes, bp, inj_table):
    """Eval: accuracy per sorted position (0-9 after '=')"""
    ...
    # Track per-position accuracy (10 sorted positions)
    pos_correct = np.zeros(10, dtype=np.int32)
    pos_total = np.zeros(10, dtype=np.int32)
    sort_correct = 0; sort_total = 0
    all_correct = 0; all_total = 0
    output_pos = -1  # -1 = not in output mode
    ...
    for i in range(len(text_bytes)-1):
        ...
        if text_bytes[i] == EQ_BYTE:
            ...
            if actual_next == NL_BYTE or output_pos >= 10:
                output_pos = -1
            else:
                if pred_byte == actual_next:
                    sort_correct += 1
...
if __name__ == "__main__":
    ...
    BUDGET = 2000
    THRESHOLD = 0.00005
    SEQ_LEN = 63  # 3 examples per window
    ...
    DATA = os.path.join(BASE_DIR, "data", "perm_sorting.traindat")
    with open(DATA, 'rb') as f:
        ALL_DATA = np.frombuffer(f.read(), dtype=np.uint8)
    bigram = np.load(os.path.join(BASE_DIR, "data", "perm_sorting_bigram.npy"))
    ...
    print(f"Loaded {len(ALL_DATA)//21} perm sorting examples")
    print(f"H={H}, {N_WORKERS}w, seq_len={SEQ_LEN}")
    print(f"Bigram: '='->0 = {bigram[ord('='), ord('0')]:.3f}")
    print(f"Random baseline: 10% per position")
    ...
    try:
        for step in range(1, BUDGET+1):
            ...
            if step % 200 == 0:
                ...
                # Detailed eval
                all_sa, all_aa, all_pc, all_pt = [], [], np.zeros(10), np.zeros(10)
                for s in eval_seqs:
                    sa, aa, pc, pt = eval_perm_sort(msign, mmag, H, output_projection_f, s, bp, inj_table)
    ...
    print(f"\n{'='*60}")
    print(f"  PERMUTATION SORTING RESULTS ({BUDGET} steps)")
    print(f"{'='*60}")
    print(f"  Sort accuracy: {sort_acc*100:.2f}% (random=10%)")
    print(f"  All accuracy:  {all_acc*100:.2f}%")
    print(f"  Edges: {edges}")
    print(f"  Time: {elapsed:.0f}s")
    print(f"\n  Per-position accuracy (0=first sorted, 9=last):")
    for i in range(10):
        bar = '#' * int(pos_acc[i] * 50)
        print(f"    [{i}] {pos_acc[i]*100:5.1f}% {bar}")
    print(f"{'='*60}")

    np.savez_compressed(os.path.join(CKPT_DIR, "perm_sort_final.npz"),
