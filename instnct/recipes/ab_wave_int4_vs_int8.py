"""
Wave gating: int4 (16 levels) vs uint8 (256 levels) frozen random
==================================================================
Does 16-level quantization lose the wave gating advantage?
freq int4: [0-15] -> [0.5, 2.0] step=0.1
phase int4: [0-15] -> [0, 2pi] step=pi/8
"""
...
if __name__ == "__main__":
    ...
    MODES = [
        ('A', 'int4 frozen (16 levels)', 'int4'),
        ('B', 'uint8 frozen (256 levels)', 'uint8'),
        ('C', 'int2 frozen (4 levels)', 'int2'),
        ('D', 'binary frozen (2 levels)', 'binary'),
    ]

    print(f"\n{'='*60}")
    print(f"  WAVE QUANTIZATION: int4 vs uint8 vs int2 vs binary")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    for mk, label, mode in MODES:
        ...
        if mode == 'uint8':
            # 256 levels: freq [0.5, 2.0], phase [0, 2pi]
            freq_arr = (frng.rand(H).astype(np.float32) * 1.5 + 0.5)
            phase_arr = (frng.rand(H).astype(np.float32) * 2.0 * np.pi)
        elif mode == 'int4':
            # 16 levels each
            freq_int = frng.randint(0, 16, size=H)
            phase_int = frng.randint(0, 16, size=H)
            freq_arr = (0.5 + freq_int * (1.5 / 15.0)).astype(np.float32)
            phase_arr = (phase_int * (2.0 * np.pi / 15.0)).astype(np.float32)
        elif mode == 'int2':
            # 4 levels each
            freq_int = frng.randint(0, 4, size=H)
            phase_int = frng.randint(0, 4, size=H)
            freq_arr = (0.5 + freq_int * (1.5 / 3.0)).astype(np.float32)
            phase_arr = (phase_int * (2.0 * np.pi / 3.0)).astype(np.float32)
        elif mode == 'binary':
            # 2 levels each
            freq_int = frng.randint(0, 2, size=H)
            phase_int = frng.randint(0, 2, size=H)
            freq_arr = (0.5 + freq_int * 1.5).astype(np.float32)  # 0.5 or 2.0
            phase_arr = (phase_int * np.pi).astype(np.float32)     # 0 or pi

        n_unique_freq = len(np.unique(np.round(freq_arr, 4)))
        n_unique_phase = len(np.unique(np.round(phase_arr, 4)))
        print(f"   freq: {n_unique_freq} unique values [{freq_arr.min():.3f}, {freq_arr.max():.3f}]")
        print(f"   phase: {n_unique_phase} unique values [{phase_arr.min():.3f}, {phase_arr.max():.3f}]")
        sys.stdout.flush()
        ...
        results.append({'mode':mk,'label':label,'best':float(best),'time':elapsed,
                        'n_freq':n_unique_freq,'n_phase':n_unique_phase})
        print(f"  >> DONE {mk}: {label} best={best*100:.1f}% {elapsed:.0f}s")
        sys.stdout.flush()

    print(f"\n{'='*60}")
    print(f"  WAVE QUANTIZATION RESULTS")
    print(f"  (reference: no wave = 6.7%)")
    print(f"{'='*60}")
    for r in results:
        bits = {'int4':'4bit','uint8':'8bit','int2':'2bit','binary':'1bit'}[r['mode'].split()[0] if ' ' in r['mode'] else r['mode']]
        print(f"  {r['mode']}: {r['label']:35s} best={r['best']*100:5.1f}% freq={r['n_freq']:3d}lv phase={r['n_phase']:3d}lv")
    print(f"{'='*60}")
    sys.stdout.flush()
