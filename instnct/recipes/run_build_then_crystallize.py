"""
BUILD 1800 steps → full crystallize → report how much was trimmed.
Quick experiment to see if crystallize replaces the CRYSTAL schedule.
"""
...
if __name__ == "__main__":
    ...
    print(f"=== PHASE 1: BUILD ({BUILD_STEPS} steps) ===")
    print(f"Init: {qm.count_edges()} edges")
    ...
    # === PHASE 2: FULL CRYSTALLIZE ===
    print(f"\n=== PHASE 2: CRYSTALLIZE ===")
    print(f"  Starting with {edges_pre} edges...")

    # Build a SelfWiringGraph from current state for crystallize
    ...
    edges_post = qm_post.count_edges()
    eval_post = np.mean([eval_accuracy(qm_post.data, theta, channel, pol_f, s, bp_in, bp_out)
                         for s in eval_seqs])

    print(f"\n=== CRYSTALLIZE DONE ===")
    print(f"  Removed: {removed} edges ({removed/edges_pre*100:.1f}%)")
    print(f"  Edges:  {edges_pre} -> {edges_post}")
    ...
    print(f"  Time:   {crystal_time:.0f}s")

    print(f"\n{'='*50}")
    print(f"  TOTAL: build={build_time:.0f}s + crystal={crystal_time:.0f}s")
    print(f"  {edges_pre} edges -> crystallize -> {edges_post} edges")
    print(f"  {removed} removed ({removed/edges_pre*100:.0f}%)")
    print(f"  Loops survived: bidir {qm_post.count_bidir()}, tri {tri_post.sum()}")
    print(f"{'='*50}")
