"""
A/B test: Standard BUILD schedule vs BUILD + cluster_connect mutation.

cluster_connect: pick 2-3 neurons, add edges between all of them in one step.
A macro-mutation that creates small connected subgraphs.

Run: python instnct/recipes/ab_cluster_connect.py
"""
...
# B) Standard + cluster_connect (replaces one 'add' with 'cluster2', adds 'cluster3')
SCHEDULE_B = [
    'cluster2', 'enhance', 'reverse', 'mirror',
    'loop3', 'loop5', 'loop8',
    'flip', 'theta', 'channel', 'cluster3', 'remove',
]

# C) Heavier cluster (2x cluster, no plain add)
SCHEDULE_C = [
    'cluster2', 'cluster3', 'reverse', 'mirror',
    'loop3', 'loop5', 'loop8',
    'flip', 'theta', 'channel', 'enhance', 'remove',
]

CONFIGS = {
    'A_baseline': {'label': 'Standard schedule (no cluster)', 'schedule': SCHEDULE_A},
    'B_mild_cluster': {'label': 'Standard + cluster2/3 (mild)', 'schedule': SCHEDULE_B},
    'C_heavy_cluster': {'label': 'Heavy cluster (2x cluster)', 'schedule': SCHEDULE_C},
}

BP_IN = None
_bp_out = None; _all_data = None; _bigram = None; _pol = None
_schedule = SCHEDULE_A
...
def init_w(bpi, bpo, data, bg, pol, sched):
    global BP_IN, _bp_out, _all_data, _bigram, _pol, _schedule
...
def worker_eval(args):
    ...
    elif pt in ('cluster2', 'cluster3'):
        # Pick N random distinct neurons, add edges between all pairs
        cluster_size = int(pt[-1])  # 2 or 3
        nodes = []
        for _ in range(cluster_size):
            n = rng.randint(0, H - 1)
            if n in nodes: return {'delta': -1e9, 'type': pt}
            nodes.append(n)
        # Add directed edges between all pairs (both directions)
        added = 0
        for i in range(len(nodes)):
            for j in range(len(nodes)):
                if i == j: continue
                r, c = nodes[i], nodes[j]
                if nq.get_pair(r, c) == 0:
                    nq.set_pair(r, c, 1)
                    added += 1
        if added == 0: return {'delta': -1e9, 'type': pt}
...
def run_config(name, cfg, init_state, bp_in, bp_out, all_data, bigram, eval_seqs):
    ...
    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp_in, bp_out, all_data, bigram, pol_f, schedule))
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(BASE_DIR, "data", "cluster_connect_ab_result.json")
