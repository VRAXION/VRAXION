"""
INSTNCT Adaptive v5 — prune pressure
======================================
Same as v4 but prune_rate drifts UP (+1 every 250 steps).
Network must actively mutate it DOWN to keep edges.
Natural equilibrium between growth and pruning.
"""
...
if __name__ == "__main__":
    ...
    print(f"ADAPTIVE v5 — prune pressure (rate drifts UP, network fights DOWN)")
    print(f"Types: {MUT_TYPES}")
    print(f"Prune: every {PRUNE_EVERY} steps, remove bottom N (N=learnable int4)")
    print(f"Prune PRESSURE: rate +1 every 250 steps, network must fight it down")
    ...
    try:
        for step in range(1, BUDGET+1):
            ...
            # CHECKPOINT
            if step % 1000 == 0:
                ckpt_path = os.path.join(CKPT_DIR, f"adaptive_v5_step{step}.npz")
    ...
    print(f"\n{'='*70}")
    print(f"  ADAPTIVE v5 FINAL ({BUDGET} steps)")
    print(f"{'='*70}")
    ...
    # Final checkpoint
    np.savez_compressed(os.path.join(CKPT_DIR, "adaptive_v5_final.npz"),
        msign=msign, mmag=mmag, ret_int4=ret_int4,
        inj_table=inj_table, output_projection_int8=woi,
        sched_weights=sched_weights, prune_rate=prune_rate)
    sys.stdout.flush()
