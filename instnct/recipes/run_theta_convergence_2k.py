"""
Long convergence run: 2000 steps, single seed, detailed theta distribution tracking.
Shows full relay/compute/gate zone emergence.

Run: python instnct/recipes/run_theta_convergence_2k.py
"""
...
if __name__ == "__main__":
    ...
    try:
        for step in range(1, BUILD_STEPS + 1):
            ...
            if step % EVAL_EVERY == 0:
                ...
                n_relay = int(np.sum((t_int >= 1) & (t_int <= 4)))
                n_compute = int(np.sum((t_int >= 6) & (t_int <= 10)))
                n_gate = int(np.sum((t_int >= 12) & (t_int <= 15)))
                ...
                snap = {
                    ...
                    'relay': n_relay, 'compute': n_compute, 'gate': n_gate,
                    'theta_mean': round(t_mean, 2),
                }
                snapshots.append(snap)

                # Print theta distribution as mini-bar
                bar = ""
                for v in range(1, 16):
                    bar += f"{hist[v]:>3d}"
                print(f"  [{step:5d}] eval={ea*100:.1f}% best={best_eval*100:.1f}% "
                      f"R={n_relay} C={n_compute} G={n_gate} "
    ...
    print(f"\n{'='*60}")
    print(f"  CONVERGENCE RESULT (2000 steps)")
    print(f"{'='*60}")
    ...
    for v in range(1, 16):
        bar = '#' * hist[v]
        pct = hist[v] / H * 100
        zone = "RELAY" if v <= 4 else ("COMPUTE" if v <= 10 else "GATE")
        print(f"    theta={v:>2d}: {hist[v]:>4d} ({pct:4.1f}%) {bar}  [{zone}]")

    n_relay = int(np.sum((t_int >= 1) & (t_int <= 4)))
    n_compute = int(np.sum((t_int >= 6) & (t_int <= 10)))
    n_gate = int(np.sum((t_int >= 12) & (t_int <= 15)))
    print(f"\n  Zones: RELAY(1-4)={n_relay} ({n_relay/H*100:.0f}%)  "
          f"COMPUTE(6-10)={n_compute} ({n_compute/H*100:.0f}%)  "
          f"GATE(12-15)={n_gate} ({n_gate/H*100:.0f}%)")

    out_path = os.path.join(BASE_DIR, "data", "theta_convergence_2k.json")
    with open(out_path, 'w') as f:
        json.dump({'snapshots': snapshots, 'final_hist': hist.tolist(),
                   'final_eval': round(final_eval * 100, 2),
                   'best_eval': round(best_eval * 100, 2)}, f, indent=2)
    print(f"\nSaved to {out_path}")
