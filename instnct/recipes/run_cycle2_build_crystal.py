"""
Cycle 2: BUILD on crystallized network -> CRYSTALLIZE again
===========================================================
Load the checkpoint from cycle 1 crystallize (should have ~1650 edges, 16.2% eval).
Run another BUILD phase to add structure back, then crystallize again.
The hypothesis: repeated BUILD+CRYSTAL cycles push accuracy higher each time.
Uses 10-token tiered eval for speed (validated: r=0.82 correlation).
"""
...
# Tiered eval: 10-token screen (r=0.82), 99-token confirm for eval checkpoints
SCREEN_TOKENS = 10
FULL_TOKENS = 99
...
def eval_bigram_ntok(qdata, theta, channel, pol_f, text_bytes, n_tok):
    ...
    s = 0.0; n = 0
    for i in range(min(n_tok, len(text_bytes) - 1)):
...
def worker_eval(args):
    ...
    # 10-token screen eval (validated r=0.82)
    seq_off = nrng.randint(0, len(_all_data) - 100)
    seq = _all_data[seq_off:seq_off + 100]
    old = eval_bigram_ntok(qm.data, theta, channel, pol_f, seq, SCREEN_TOKENS)
    new = eval_bigram_ntok(nq.data, nt, nc, npf, seq, SCREEN_TOKENS)
    return {
...
def eval_accuracy_full(qdata, theta, channel, pol_f, text_bytes, bp_in, bp_out):
    qm = QuaternaryMask(H, qdata)
    sc = qm.to_directed_edges()
    _dummy = np.zeros((H, H), dtype=bool)
    state = np.zeros(H, np.float32); charge = np.zeros(H, np.float32)
...
if __name__ == "__main__":
    ...
    eval_seqs = [ALL_DATA[off:off + 100]
                 for off in [eval_rng.randint(0, len(ALL_DATA) - 100)
                             for _ in range(N_EVAL_SEQS)]]

    # Load cycle 1 checkpoint
    ckpt_path = os.path.join(BASE_DIR, "data", "build_checkpoint.npz")
    ckpt = np.load(ckpt_path)
    qdata = ckpt['qdata'].copy()
    theta = ckpt['theta'].copy()
    channel = ckpt['channel'].copy()
    pol_f = ckpt['pol_f'].copy()

    qm0 = QuaternaryMask(H, qdata)
    bidir0, tri0 = qm0.loop_levels()
    init_eval = np.mean([eval_accuracy_full(qdata, theta, channel, pol_f, s, bp_in, bp_out)
                         for s in eval_seqs])
    print(f"=== CYCLE 2: BUILD on crystallized network ===")
    print(f"  Loaded: {qm0.count_edges()} edges, {qm0.count_bidir()} bidir, {tri0.sum()} tri")
    print(f"  Starting eval: {init_eval*100:.1f}%")
    print(f"  Workers: {N_WORKERS}, Screen: {SCREEN_TOKENS} tokens, Steps: {BUILD_STEPS}")
    print()

    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp_in, bp_out, ALL_DATA, bigram, pol_f))
    acc_by_type = defaultdict(int)
    t0 = time.time(); best_eval = init_eval
    ...
    try:
        for step in range(1, BUILD_STEPS + 1):
            ...
            args = [(qdata.tobytes(), theta.copy(), channel.copy(), pol_f.copy(),
                     2000 + step * 50 + w, pt) for w in range(N_WORKERS)]
            ...
            if step % EVAL_EVERY == 0:
                # Full 99-token eval for checkpoints
                ea = np.mean([eval_accuracy_full(qdata, theta, channel, pol_f, s, bp_in, bp_out)
                              for s in eval_seqs])
    ...
    bidir_pre, tri_pre = qm_pre.loop_levels()
    eval_pre = np.mean([eval_accuracy_full(qdata, theta, channel, pol_f, s, bp_in, bp_out)
                        for s in eval_seqs])
    print(f"\n=== BUILD 2 DONE ===")
    print(f"  edges={qm_pre.count_edges()}, bidir={qm_pre.count_bidir()}, tri={tri_pre.sum()}")
    print(f"  eval={eval_pre*100:.1f}%, best={best_eval*100:.1f}%")
    print(f"  accepts: {dict(acc_by_type)}")

    # Save cycle 2 build checkpoint
    ckpt2_path = os.path.join(BASE_DIR, "data", "cycle2_build_checkpoint.npz")
    np.savez_compressed(ckpt2_path, qdata=qdata, theta=theta, channel=channel, pol_f=pol_f)
    ...
    net.theta = theta.copy()
    net._theta_f32 = theta.astype(np.float32)
    net.channel = channel.copy()
    net.polarity = (pol_f > 0)
    net._polarity_f32 = pol_f.copy()
    net.resync_alive()
    ...
    eval_post = np.mean([eval_accuracy_full(qm_post.data, theta, channel, pol_f, s, bp_in, bp_out)
                         for s in eval_seqs])

    print(f"\n=== CYCLE 2 CRYSTALLIZE DONE ===")
    print(f"  Removed: {removed} edges ({removed/qm_pre.count_edges()*100:.1f}%)")
    print(f"  Edges:  {qm_pre.count_edges()} -> {qm_post.count_edges()}")
    print(f"  Bidir:  {qm_pre.count_bidir()} -> {qm_post.count_bidir()}")
    print(f"  Tri:    {tri_pre.sum()} -> {tri_post.sum()}")
    print(f"  Eval:   {eval_pre*100:.1f}% -> {eval_post*100:.1f}%")
    print(f"  Time:   build={build_time:.0f}s crystal={crystal_time:.0f}s")

    # Save final checkpoint
    final_path = os.path.join(BASE_DIR, "data", "cycle2_crystal_checkpoint.npz")
    np.savez_compressed(final_path, qdata=qm_post.data, theta=theta, channel=channel, pol_f=pol_f)
    print(f"  Final checkpoint: {final_path}")

    print(f"\n{'='*50}")
    print(f"  CYCLE 1: 10.3% -> 16.2% (crystal)")
    print(f"  CYCLE 2: {eval_pre*100:.1f}% -> {eval_post*100:.1f}% (crystal)")
    print(f"  Total improvement: {init_eval*100:.1f}% -> {eval_post*100:.1f}%")
    print(f"{'='*50}")
