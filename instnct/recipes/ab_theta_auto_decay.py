"""
Auto theta decay: theta naturally decays in the decay tick
==========================================================
Every decay tick (every 6th), pick a random neuron and set its theta
to random(1, current_mean). Self-regulating: range shrinks as mean drops.

A: baseline (13.5% from prior run, reproduced here)
B: auto theta decay in eval loop (every token, decay pass after ticks)
"""
...
_bp_out=None;_all_data=None;_bigram=None;_pol=None;_channel=None;_use_decay=None

def init_w(bpo,data,bg,pol,ch,ud):
    global _bp_out,_all_data,_bigram,_pol,_channel,_use_decay
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_channel=ch;_use_decay=ud
...
def _eval_bigram(mask, theta, seqs):
    ...
    for tb in seqs:
        ...
        for i in range(len(tb)-1):
            ...
            state,charge=SelfWiringGraph.rollout_token(inj,mask=mask,theta=th_local,
            ...
            # Auto theta decay: after each token, one neuron decays
            if _use_decay:
                mean_th = max(2, int(th_local.mean()))
                victim = random.randint(0, H-1)
                th_local[victim] = float(max(1, random.randint(1, mean_th)))
...
if __name__ == "__main__":
    ...
    MODES = [
        ('B', 'auto theta decay (per token)', True),
    ]

    print(f"\n{'='*60}")
    print(f"  AUTO THETA DECAY at H={H}")
    print(f"  ref: baseline=13.5%")
    print(f"{'='*60}")
    sys.stdout.flush()

    results = []
    for mk, label, use_decay in MODES:
        print(f"\n>> {mk}: {label}")
        sys.stdout.flush()
        ...
        init_w(bp_out, ALL_DATA, bigram, pol_f32, channel, use_decay)
        pool = Pool(N_WORKERS, initializer=init_w,
            initargs=(bp_out, ALL_DATA, bigram, pol_f32, channel, use_decay))
            ...
            if step%EVAL_EVERY==0:
                ...
                if step%400==0:
                    print(f"  [{mk}:{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% "
                          f"th={theta.mean():.1f} {time.time()-t0:.0f}s")
                    sys.stdout.flush()
        pool.terminate();pool.join()
        elapsed=time.time()-t0
        results.append({'mode':mk,'label':label,'best':float(best),
                        'theta_mean':float(theta.mean()),'time':elapsed})
        print(f"  >> DONE {mk}: best={best*100:.1f}% theta={theta.mean():.1f} {elapsed:.0f}s")
        sys.stdout.flush()

    print(f"\n{'='*60}")
    print(f"  AUTO THETA DECAY RESULTS (H={H})")
    print(f"{'='*60}")
    for r in results:
        print(f"  {r['mode']}: {r['label']:40s} best={r['best']*100:5.1f}% theta={r['theta_mean']:.1f}")
    print(f"{'='*60}")
