"""
INSTNCT Adaptive v6 — sprouting
=================================
v5 + sprout mutation: pick important neuron, try 20 edges at once,
keep only positives. 7x faster edge search.
...
MUT_TYPES = ['chaotic_add', 'certain_add', 'sprout', 'flip', 'mag', 'ret']
...
if __name__ == "__main__":
    ...
    print(f"ADAPTIVE v6 — sprouting (try 20 edges at once from important neurons)")
        ...
        for step in range(1, BUDGET+1):
            ...
            if ptype == 'sprout' and (importance > 0).sum() > 0:
                ...
                for _ in range(20):
                    ...
                    if tgt != neuron and mmag[neuron, tgt] == 0:
            ...
            # CHECKPOINT
            if step % 1000 == 0:
                ckpt_path = os.path.join(CKPT_DIR, f"adaptive_v6_step{step}.npz")
    ...
    print(f"\n{'='*70}")
    print(f"  ADAPTIVE v6 FINAL ({BUDGET} steps)")
    print(f"{'='*70}")
    ...
    # Final checkpoint
    np.savez_compressed(os.path.join(CKPT_DIR, "adaptive_v6_final.npz"),
        msign=msign, mmag=mmag, ret_int4=ret_int4,
        inj_table=inj_table, output_projection_int8=woi,
        sched_weights=sched_weights, prune_rate=prune_rate)
    sys.stdout.flush()
