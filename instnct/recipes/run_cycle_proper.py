"""
Proper BUILD+CRYSTAL cycle with tiered eval
=============================================
Fix from cycle 2 failure:
1. Workers use 2-seq x 99-token eval (reliable, not 10-token screen)
2. Checkpoint saved AFTER crystallize
3. N_WORKERS=12 for thermal safety
4. Can load either pre or post-crystal checkpoint

Usage: python run_cycle_proper.py [checkpoint.npz]
       If no checkpoint, starts fresh.
"""
...
def _eval_bigram(qdata, theta, channel, pol_f, seqs):
    """Full reliable eval: 2 seqs x 99 tokens."""
...
def worker_eval(args):
    ...
    # Reliable 2-seq x 99-token eval
    seqs = []
...
if __name__ == "__main__":
    ...
    if ckpt_arg and os.path.exists(ckpt_arg):
        ...
        init_mask = (init_rng.rand(H, H) < 0.05).astype(bool)
    ...
    init_eval = np.mean([eval_accuracy(qdata, theta, channel, pol_f, s, bp_in, bp_out)
                         for s in eval_seqs])
    print(f"Init: {qm0.count_edges()} edges, {qm0.count_bidir()} bidir, "
          f"{tri0.sum()} tri, eval={init_eval*100:.1f}%")
    print(f"Workers={N_WORKERS}, Steps={BUILD_STEPS}, Ticks={TICKS}")
    print()
    ...
    try:
        for step in range(1, BUILD_STEPS + 1):
            ...
            args = [(qdata.tobytes(), theta.copy(), channel.copy(), pol_f.copy(),
                     3000 + step * 50 + w, pt) for w in range(N_WORKERS)]
    ...
    print(f"    Tri:    {qm_pre.count_edges()} -> {tri_post.sum()}")
    print(f"    Eval:   {eval_pre*100:.1f}% -> {eval_post*100:.1f}%")

    # Save POST-crystal checkpoint (the important one!)
    out_path = os.path.join(BASE_DIR, "data", "cycle_crystal_checkpoint.npz")
    np.savez_compressed(out_path, qdata=qm_post.data, theta=theta, channel=channel, pol_f=pol_f)
    print(f"    Saved: {out_path}")

    print(f"\n{'='*50}")
    print(f"  START:   {init_eval*100:.1f}%  ({qm0.count_edges()} edges)")
