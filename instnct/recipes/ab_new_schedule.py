"""
New mutation schedule with enhance, reverse, mirror
=====================================================
1. add       — random connection
2. enhance   — connection to top hub neurons
3. reverse   — flip direction A->B to B->A
4. mirror    — A->B(+) creates B->A(-) (negative feedback loop)
5. flip      — excitatory <-> inhibitory
6. theta     — random [1-15]
7. channel   — random [1-8]
8. remove    — remove connection

vs baseline: add, flip, theta, channel, theta, channel, flip, remove
"""
...
SCHEDULE_OLD = ['add','flip','theta','channel','theta','channel','flip','remove']
SCHEDULE_NEW = ['add','enhance','reverse','mirror','flip','theta','channel','remove']
...
_bp_out=None;_all_data=None;_bigram=None;_pol=None;_pol_bool=None;_channel=None

def init_w(bpo,data,bg,pol,pb,ch):
    global _bp_out,_all_data,_bigram,_pol,_pol_bool,_channel
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_pol_bool=pb;_channel=ch

def _eval_bigram(mask, theta, polarity_f, seqs):
...
def worker_eval(args):
    ...
    if pt=='add':
        ...
        # Add connection targeting top-degree neurons
        in_deg = mask.sum(axis=0)
        top_neurons = np.argsort(in_deg)[::-1][:H//4]  # top 25%
        c = int(nrng.choice(top_neurons))
        r = rng.randint(0,H-1)
        if r==c or mask[r,c]:return{'delta':-1e9,'type':'enhance'}
        nm=mask.copy();nm[r,c]=True

    elif pt=='reverse':
        # Pick random edge A->B, reverse to B->A
        alive=list(zip(*np.where(mask)))
        if len(alive)<1:return{'delta':-1e9,'type':'reverse'}
        r,c=alive[rng.randint(0,len(alive)-1)]
        if r==c or mask[c,r]:return{'delta':-1e9,'type':'reverse'}  # already exists
        nm=mask.copy();nm[r,c]=False;nm[c,r]=True

    elif pt=='mirror':
        # Pick random edge A->B, add B->A with opposite polarity effect
        alive=list(zip(*np.where(mask)))
        if len(alive)<1:return{'delta':-1e9,'type':'mirror'}
        r,c=alive[rng.randint(0,len(alive)-1)]
        if mask[c,r]:return{'delta':-1e9,'type':'mirror'}  # mirror already exists
        nm=mask.copy();nm[c,r]=True
        # Flip polarity of the mirror source (c) to create negative feedback
        # Only if it creates exc->inh pair: if A is exc, make mirror source inh
        if pol_bool[r] == pol_bool[c]:
            # Same polarity: flip mirror source to create opposition
            npb=pol_bool.copy();npb[c]=not npb[c]
            npf=np.where(npb, 1.0, -1.0).astype(np.float32)

    elif pt=='flip':
        # Flip polarity of random neuron
        ...
        return{'delta':-1e9,'type':pt}

    seqs=[]
    for _ in range(2):
        off=nrng.randint(0,len(_all_data)-100);seqs.append(_all_data[off:off+100])
    old=_eval_bigram(mask,theta,pol_f,seqs)
    new=_eval_bigram(nm,nt,npf,seqs)
...
if __name__ == "__main__":
    ...
    # Only new schedule since baseline = 13.5%
    print(f"\n{'='*60}")
    print(f"  NEW SCHEDULE at H={H}")
    print(f"  baseline (old schedule) = 13.5%")
    print(f"  old: {SCHEDULE_OLD}")
    print(f"  new: {SCHEDULE_NEW}")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    init_w(bp_out, ALL_DATA, bigram, pol_f32, pol_bool, channel)
    pool = Pool(N_WORKERS, initializer=init_w,
        initargs=(bp_out, ALL_DATA, bigram, pol_f32, pol_bool, channel))

    best=0;acc=0;t0=time.time()
    op_counts = {op: 0 for op in SCHEDULE_NEW}
    for step in range(1, 2001):
        pt=SCHEDULE_NEW[(step-1)%len(SCHEDULE_NEW)]
        if pt in('enhance','reverse','mirror','flip','remove','theta','channel') and mask.sum()==0:pt='add'
        ...
        if br['delta']>THRESHOLD:
            if br['type'] in('add','enhance','reverse','mirror','remove') and br['new_mask_flat'] is not None:
                mask[:]=br['new_mask_flat'].reshape(H,H);acc+=1;op_counts[br['type']]+=1
            if br['type'] in('flip','mirror') and br['new_pol_bool'] is not None:
                pol_bool[:]=br['new_pol_bool'];pol_f32[:]=br['new_pol_f']
                if br['type']=='flip':op_counts['flip']+=1
            if br['type']=='theta' and br['new_theta'] is not None:
                theta[:]=br['new_theta'];acc+=1;op_counts['theta']+=1
            if br['type']=='channel' and br['new_channel'] is not None:
        ...
        if step%EVAL_EVERY==0:
            ...
            if step%200==0:
                ...
                print(f"  [{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% th={theta.mean():.1f} "
                      f"e={mask.sum()} recip={recip} inh={inh_pct:.0f}% ops={dict(op_counts)} {time.time()-t0:.0f}s")
                sys.stdout.flush()
    ...
    print(f"  >> edges={mask.sum()} recip={recip} ({recip*2/max(mask.sum(),1)*100:.1f}%)")
    print(f"  >> degree: mean={in_deg.mean():.1f} std={in_deg.std():.1f} [{in_deg.min()}-{in_deg.max()}]")
    print(f"  >> inh: {(~pol_bool).sum()}/{H} ({(~pol_bool).sum()/H*100:.1f}%)")
    print(f"  >> ops: {dict(op_counts)}")
    print(f"  >> baseline: 13.5%")
