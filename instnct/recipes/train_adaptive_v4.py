"""
INSTNCT Adaptive v4 — explore vs exploit
==========================================
5 mutation types, each with int4 learnable weight:
  chaotic_add:  random edge placement (exploration)
  certain_add:  importance-guided edge placement (exploitation)
  flip:         sign change
  mag:          weight tuning
  ret:          retention tuning (only active neurons)

Auto soft prune: every 250 steps, remove bottom N (N=learnable int4)
No hard prune.
"""
...
_output_projection_f = None; _inj_table = None; _importance = None

def init_w(b, d, sl, nt, wof, it, imp):
    global _bp, _all_data, _seq_len, _n_train, _output_projection_f, _inj_table, _importance
    _bp, _all_data, _seq_len, _n_train = b, d, sl, nt
    _output_projection_f, _inj_table, _importance = wof, it, imp
...
def worker_eval(args):
    ...
    if ptype == 'chaotic_add':
        # EXPLORATION: random or targeted add (reuse src/tgt from existing)
    ...
    elif ptype == 'certain_add':
        # EXPLOITATION: connect near important neurons
        important_neurons = np.where(_importance > 0)[0] if _importance is not None else np.array([])
        if len(important_neurons) > 0:
            imp_n = important_neurons[rng.randint(0, len(important_neurons)-1)]
            if rng.random() < 0.5:
                r = imp_n; c = rng.randint(0, H-1)
            else:
                r = rng.randint(0, H-1); c = imp_n
        else:
            r = rng.randint(0, H-1); c = rng.randint(0, H-1)
        if r == c or mmag[r, c] > 0:
            return {'delta': -1e9, 'type': ptype}
        ...
        # SMART RET: only mutate neurons with importance > 0
        active = np.where(_importance > 0)[0] if _importance is not None else np.array([])
        if len(active) > 0:
            idx = active[rng.randint(0, len(active)-1)]
        else:
            idx = rng.randint(0, H-1)  # fallback if no important neurons yet
...
def compute_importance(msign, mmag, ret_int4, H, inj_table, eval_seqs):
    """Compute per-neuron importance: in_deg * out_deg * fire_ratio."""
    rs, cs = np.where(mmag > 0)
    in_deg = np.bincount(cs, minlength=H).astype(np.float32)
    out_deg = np.bincount(rs, minlength=H).astype(np.float32)

    s = msign[rs, cs].astype(np.float32) * 2 - 1
    sp_vals = s * mmag[rs, cs].astype(np.float32) / 128.0
    ret_vec = 1.0 - ret_int4.astype(np.float32) * 0.01

    fire_count = np.zeros(H, dtype=np.float32)
    max_ticks = 0
    for text_bytes in eval_seqs:
        state = np.zeros(H, dtype=np.float32)
        charge = np.zeros(H, dtype=np.float32)
        for i in range(len(text_bytes)-1):
            ...
            for t in range(8):
                ...
                charge += raw; charge *= ret_vec
                act = np.maximum(charge, 0.0); charge = np.maximum(charge, 0.0)
                fire_count += (charge > 0.01).astype(np.float32)
                max_ticks += 1
            state = act.copy()

    fire_ratio = fire_count / max(max_ticks, 1)
    raw_imp = in_deg * out_deg * fire_ratio
    if raw_imp.max() > 0:
        importance = (raw_imp / raw_imp.max() * 255).astype(np.uint8)
    else:
        importance = np.zeros(H, dtype=np.uint8)

    # SINK FIX: neurons with in>0, out=0, fire>0 → need outgoing edge
    sinks = (in_deg > 0) & (out_deg == 0) & (fire_count > 0)
    n_sinks = int(sinks.sum())

    return importance, n_sinks, int((importance > 0).sum())


MUT_TYPES = ['chaotic_add', 'certain_add', 'flip', 'mag', 'ret']
...
if __name__ == "__main__":
    ...
    print(f"ADAPTIVE v4 — explore vs exploit (chaotic + certain add)")
    print(f"Types: {MUT_TYPES}")
    print(f"Prune: every {PRUNE_EVERY} steps, remove bottom N (N=learnable int4)")
    print(f"No hard prune — soft prune only")
    ...
    # Learnable params
    sched_weights = np.array([3, 2, 1, 1, 1], dtype=np.uint8)  # 3chaotic/2certain/1f/1m/1ret
    ...
    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp, ALL_DATA, 120, 2, wof, inj_table, importance))
    try:
        for step in range(1, BUDGET+1):
            # UPDATE IMPORTANCE every 250 steps + refresh pool
            if step % 250 == 1 and step > 1:
                importance, n_sinks, n_important = compute_importance(
                    msign, mmag, ret_int4, H, inj_table, eval_seqs[:3])
            ...
            if ptype in ('flip', 'mag') and (mmag > 0).sum() == 0:
                ptype = 'chaotic_add'
            elif ptype == 'certain_add' and (mmag > 0).sum() == 0:
                ptype = 'chaotic_add'
            ...
            # CHECKPOINT
            if step % 1000 == 0:
                ckpt_path = os.path.join(CKPT_DIR, f"adaptive_v4_step{step}.npz")
    ...
    print(f"\n{'='*70}")
    print(f"  ADAPTIVE v4 FINAL ({BUDGET} steps)")
    print(f"{'='*70}")
    ...
    # Final checkpoint
    np.savez_compressed(os.path.join(CKPT_DIR, "adaptive_v4_final.npz"),
        msign=msign, mmag=mmag, ret_int4=ret_int4,
        inj_table=inj_table, output_projection_int8=woi,
        sched_weights=sched_weights, prune_rate=prune_rate)
    sys.stdout.flush()
