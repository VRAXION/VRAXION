"""
Phase binary: {0, pi} vs {0.80pi, 1.27pi} (calculated optimal)
===============================================================
freq=1.0 fixed, only phase differs. H mode showed 16.6% with {0, pi}.
Does the orthogonality-optimal phase pair beat it?
"""
...
if __name__ == "__main__":
    ...
    MODES = [
        ('A', 'phase {0, pi} (current)',
         [0.0, np.pi]),
        ('B', 'phase {0.80pi, 1.27pi} (orth optimal)',
         [0.80 * np.pi, 1.27 * np.pi]),
        ('C', 'phase {0.32pi, 0.83pi} (orth #2)',
         [0.32 * np.pi, 0.83 * np.pi]),
        ('D', 'phase {pi/4, 3pi/4} (quarter symmetric)',
         [np.pi/4, 3*np.pi/4]),
        ('E', 'phase {pi/2, 3pi/2} (quadrature)',
         [np.pi/2, 3*np.pi/2]),
    ]

    print(f"\n{'='*60}")
    print(f"  PHASE BINARY OPTIMIZATION (freq=1.0 fixed)")
    print(f"  ref: H mode {{0,pi}} = 16.6%")
    print(f"{'='*60}")
    sys.stdout.flush()

    results = []
    for mk, label, phase_vals in MODES:
        print(f"\n>> {mk}: {label}")
        print(f"   phase values: {[f'{p:.4f} ({p/np.pi:.2f}pi)' for p in phase_vals]}")
        sys.stdout.flush()
        ...
        freq_arr = np.ones(H, dtype=np.float32)  # freq=1.0 fixed
        frng = np.random.RandomState(42)
        phase_binary = (frng.rand(H) > 0.5).astype(int)
        phase_arr = np.array([phase_vals[b] for b in phase_binary], dtype=np.float32)
    ...
    print(f"\n{'='*60}")
    print(f"  PHASE OPTIMIZATION RESULTS (freq=1.0 fixed)")
    print(f"{'='*60}")
    for r in results:
