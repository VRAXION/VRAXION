"""
INSTNCT — Drain Resolution: how many bits does per-edge drain need?
====================================================================
The learnable sweep found drain in [1,6] range. Test quantized versions:
A) Ternary: {1, 3, 5} — 3 options (low/med/high)
B) Int4: {1,2,3,4,5,6} — 6 options (exact range found)
C) Float: continuous 0.1-10.0 — full resolution (baseline)
"""
...
def run_arm(label, mode, seed, steps, eval_seqs):
    ...
    # Init drain based on mode
    if mode == 'ternary':
        options = [1, 3, 5]
        drain = np.array([random.choice(options) for _ in range(n_edges)], dtype=np.int16)
    elif mode == 'int4':
        drain = np.random.randint(1, 7, size=n_edges).astype(np.int16)
    else:  # float → rounded to int
        drain = np.clip(np.round(np.random.uniform(0.5, 6.5, size=n_edges)), 1, 10).astype(np.int16)
    ...
    def resize_drain():
        ...
        if n > len(drain):
            if mode == 'ternary':
                extra = np.array([random.choice([1,3,5]) for _ in range(n - len(drain))], dtype=np.int16)
            elif mode == 'int4':
                extra = np.random.randint(1, 7, size=n - len(drain)).astype(np.int16)
            else:
                extra = np.clip(np.round(np.random.uniform(0.5, 6.5, size=n - len(drain))), 1, 10).astype(np.int16)
            drain = np.concatenate([drain, extra])
        elif n < len(drain):
            drain = drain[:n]
    ...
    for step in range(1, steps + 1):
        ...
        # Mutate drain (1-5 edges)
        for _ in range(random.randint(1, 5)):
            if len(drain) == 0: break
            idx = random.randint(0, len(drain) - 1)
            if mode == 'ternary':
                drain[idx] = random.choice([1, 3, 5])
            elif mode == 'int4':
                drain[idx] = np.clip(drain[idx] + random.choice([-1, 0, 1]), 1, 6)
            else:
                drain[idx] = np.clip(drain[idx] + random.choice([-2, -1, 0, 1, 2]), 1, 10)

        new_acc = avg_acc()
        if new_acc > best_acc:
            best_acc = new_acc; accepts += 1; stale = 0
        else:
            net.restore_state(snap); drain = old_drain
            resize_drain(); stale += 1

        if step % log_every == 0:
            print(f"    {label:>10} step {step:5d} | acc={best_acc:.3f} "
                  f"| drain={np.mean(drain):.1f}±{np.std(drain):.1f} | acc#={accepts}")

    unique = len(np.unique(drain))
    return best_acc, accepts, drain.copy(), unique
...
def main():
    ...
    print("=" * 80)
    print("  Drain Resolution: ternary vs int4 vs float")
    ...
    for label, mode in arms:
        print(f"\n>>> {label}")
        t0 = time.time()
        acc, accepts, drain, unique = run_arm(label, mode, SEED, STEPS, eval_seqs)
        print(f"    Done in {time.time()-t0:.0f}s: acc={acc:.3f}, unique_vals={unique}")
        results.append((label, acc, accepts, drain, unique))

    print(f"\n{'='*80}")
    print(f"  RESULTS")
    print(f"{'='*80}")
    for label, acc, accepts, drain, unique in results:
        print(f"  {label:>12}: acc={acc:.3f} | drain={np.mean(drain):.1f}±{np.std(drain):.1f} "
              f"| unique={unique} | acc#={accepts}")

    best = max(results, key=lambda x: x[1])
    print(f"\n  Winner: {best[0]} ({best[1]:.3f})")

    # Value distribution for each
    for label, _, _, drain, _ in results:
        vals, counts = np.unique(drain, return_counts=True)
        top = sorted(zip(counts, vals), reverse=True)[:6]
        dist = " ".join(f"{v}:{c}" for c, v in top)
        print(f"    {label}: {dist}")
