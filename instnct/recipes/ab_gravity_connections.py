"""
Gravitational connection mutation: F=m*a for neurons
=====================================================
Heavier neurons (more connections) attract more connections.
Lighter neurons lose connections more easily.

A: baseline (flat random target) = 13.5%
B: gravity add (target proportional to in_degree)
   + gravity remove (victim inversely proportional to in_degree)
"""
...
_bp_out=None;_all_data=None;_bigram=None;_pol=None;_channel=None;_use_gravity=None

def init_w(bpo,data,bg,pol,ch,ug):
    global _bp_out,_all_data,_bigram,_pol,_channel,_use_gravity
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_channel=ch;_use_gravity=ug
...
def worker_eval(args):
    ...
    if pt=='add':
        if _use_gravity:
            # Gravity: target proportional to in_degree
            in_deg = mask.sum(axis=0).astype(np.float64) + 1.0  # +1 to avoid zero
            probs = in_deg / in_deg.sum()
            c = nrng.choice(H, p=probs)
            r = rng.randint(0, H-1)
        else:
        ...
        if _use_gravity:
            # Inverse gravity: remove from low-degree neurons more likely
            in_deg = mask.sum(axis=0).astype(np.float64) + 1.0
            inv_weights = 1.0 / in_deg
            # Weight each edge by inverse degree of its TARGET
            edge_weights = np.array([inv_weights[c] for r,c in alive])
            edge_weights /= edge_weights.sum()
            idx = nrng.choice(len(alive), p=edge_weights)
            r,c = alive[idx]
        else:
...
if __name__ == "__main__":
    ...
    # Only B (gravity) since A baseline = 13.5% known
    print(f"\n{'='*60}")
    print(f"  GRAVITY CONNECTIONS at H={H}")
    print(f"  baseline (flat random) = 13.5%")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    print(f"  Init: in_deg mean={in_deg.mean():.1f} std={in_deg.std():.1f} [{in_deg.min()}-{in_deg.max()}]")

    init_w(bp_out, ALL_DATA, bigram, pol_f32, channel, True)
    pool = Pool(N_WORKERS, initializer=init_w,
        initargs=(bp_out, ALL_DATA, bigram, pol_f32, channel, True))
        ...
        if step%EVAL_EVERY==0:
            ...
            if step%400==0:
                ...
                print(f"  [B:{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% "
                      f"th={theta.mean():.1f} deg={in_deg.mean():.1f}[{in_deg.min()}-{in_deg.max()}] "
                      f"std={in_deg.std():.1f} {time.time()-t0:.0f}s")
                sys.stdout.flush()
    pool.terminate();pool.join()
    elapsed=time.time()-t0

    in_deg = mask.sum(axis=0)
    print(f"\n  >> DONE B: best={best*100:.1f}% theta={theta.mean():.1f} {elapsed:.0f}s")
    print(f"  >> degree: mean={in_deg.mean():.1f} std={in_deg.std():.1f} [{in_deg.min()}-{in_deg.max()}]")
    print(f"  >> baseline: 13.5% degree std=3.3")
    print(f"  >> diff: {(best-0.135)*100:+.1f}%")

    # Hub analysis
    top10 = np.argsort(in_deg)[::-1][:10]
    bot10 = np.argsort(in_deg)[:10]
    print(f"\n  Top 10 hubs: {[(int(i), int(in_deg[i])) for i in top10]}")
    print(f"  Bottom 10:   {[(int(i), int(in_deg[i])) for i in bot10]}")
