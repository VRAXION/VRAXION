"""
INSTNCT Adaptive v3 — importance-guided mutations
===================================================
v2 + importance score per neuron:
  importance(n) = in_deg * out_deg * fire_ratio
  = 0 if sink/source/dead → skip ret mutation, don't target for add

Changes from v2:
  - ret mutation: ONLY targets neurons with importance > 0
  - add mutation: 50% chance to connect near important neurons
  - Sink fix: auto add outgoing edge if in>0, out=0, fire>0
  - Importance tracked and reported every 250 steps
"""
...
def worker_eval(args):
    ...
    if ptype == 'add':
        # SMART ADD: 50% chance to connect near important neurons
        important_neurons = np.where(_importance > 0)[0] if _importance is not None else np.array([])
        if len(important_neurons) > 0 and rng.random() < 0.5:
            # Pick an important neuron, add edge FROM or TO it
            ...
            # Targeted add (reuse src/tgt from existing edge)
...
if __name__ == "__main__":
    ...
    print(f"ADAPTIVE v3 — importance-guided mutations")
    ...
    try:
        for step in range(1, BUDGET+1):
            ...
            # CHECKPOINT
            if step % 1000 == 0:
                ckpt_path = os.path.join(CKPT_DIR, f"adaptive_v3_step{step}.npz")
    ...
    print(f"\n{'='*70}")
    print(f"  ADAPTIVE v3 FINAL ({BUDGET} steps)")
    print(f"{'='*70}")
    ...
    # Final checkpoint
    np.savez_compressed(os.path.join(CKPT_DIR, "adaptive_v3_final.npz"),
        msign=msign, mmag=mmag, ret_int4=ret_int4,
        inj_table=inj_table, output_projection_int8=woi,
        sched_weights=sched_weights, prune_rate=prune_rate)
    sys.stdout.flush()
