"""
INSTNCT Periodic Prune — remove worst edge every 100 steps
=============================================================
A/B test:
  A: baseline (no pruning, w=2 superposition, LL eval)
  B: same + remove worst edge every 100 steps via alignment scoring

Hypothesis: periodic removal of junk edges → cleaner network → better accuracy.
"""
...
def compute_edge_alignment(msign, mmag, H, output_projection_f, bp, bigram, inj_table, eval_seqs):
    """Score each edge: does it push output toward target? (1 forward pass)"""
    rs, cs = np.where(mmag > 0)
    n_edges = len(rs)
    if n_edges == 0:
    ...
    for text_bytes in eval_seqs:
        ...
        for i in range(len(text_bytes)-1):
            ...
            if i > 0:
                inj = inj + inj_table[text_bytes[i-1]].astype(np.float32) / 128.0 * 0.5
            ...
            for t in range(8):
                ...
                raw = np.zeros(H, dtype=np.float32)
                if n_edges: np.add.at(raw, cs, edge_act)
                charge += raw; charge *= ret
...
def run_config(name, do_prune, H, bp, inj_table, wof, ALL_DATA, bigram,
    ...
    print(f"\n{'='*60}")
    print(f"  {name} (prune={'YES every 100 steps' if do_prune else 'NO'})")
    print(f"{'='*60}")
    sys.stdout.flush()
    ...
    try:
        for step in range(1, n_steps+1):
            ...
            # PERIODIC PRUNE: every 100 steps, remove worst edge
            if do_prune and step % 100 == 0 and (mmag > 0).sum() > 10:
                scores, e_rs, e_cs = compute_edge_alignment(
                    msign, mmag, H, wof, bp, bigram, inj_table, eval_seqs[:3])
                if len(scores) > 0:
                    worst_idx = int(np.argmin(scores))
                    mmag[e_rs[worst_idx], e_cs[worst_idx]] = 0
                    msign[e_rs[worst_idx], e_cs[worst_idx]] = False
                    pruned_total += 1
...
if __name__ == "__main__":
    ...
    print(f"PERIODIC PRUNE A/B TEST")
    print(f"w=2 superposition, LL eval, threshold=0.00005")
    print(f"B: remove worst edge every 100 steps via alignment scoring")

    results = []
    results.append(run_config("A: no pruning", False, H, bp, inj_table, wof,
                              ALL_DATA, bigram, BUDGET, N_WORKERS, SCHEDULE))
    results.append(run_config("B: prune worst/100", True, H, bp, inj_table, wof,
                              ALL_DATA, bigram, BUDGET, N_WORKERS, SCHEDULE))

    print(f"\n{'='*60}")
    print(f"  PERIODIC PRUNE RESULTS")
    print(f"{'='*60}")
    print(f"  {'Config':<25} {'Acc%':>6} {'Edges':>6} {'Pruned':>6} {'Time':>6}")
    print(f"  {'-'*25} {'-'*6} {'-'*6} {'-'*6} {'-'*6}")
    for r in results:
        print(f"  {r['name']:<25} {r['acc']*100:6.2f} {r['edges']:6d} {r['pruned']:6d} {r['time']:5.0f}s")
