"""
Direct 256 output + Potential-Aware fitness smoke test
======================================================
H=384: 64 SDR in + 256 direct out + 64 hidden
Output neuron[i].charge = logit for byte i. No projection.

A: Direct 256, standard fitness
B: Direct 256, potential fitness (w=0.05)

Learnable theta, short run to see if it works at all.
"""
...
def init_w(data,bg,pol,fr,ph,rh,pw):
    global _all_data,_bigram,_pol,_freq,_phase,_rho,_pw
    _all_data=data;_bigram=bg;_pol=pol;_freq=fr;_phase=ph;_rho=rh;_pw=pw
...
def _eval_bigram(mask, theta, decay, seqs):
    ...
    for tb in seqs:
        ...
        for i in range(len(tb)-1):
            ...
            # DIRECT readout: last 256 neurons = logits
            logits = charge[H-256:]
            # Bigram cosine
            e=np.exp(logits-logits.max());pred=e/e.sum()
            tgt=_bigram[tb[i]]
            cos=np.dot(pred,tgt)/(np.linalg.norm(pred)*np.linalg.norm(tgt)+1e-8)
            # Potential bonus: target byte's logit directly
            score = cos
            if _pw > 0:
                score += _pw * logits[tb[i+1]]
            s+=score;n+=1
...
if __name__ == "__main__":
    ...
    MODES = [
        ('A', 'DIRECT256 standard',    0.00),
        ('B', 'DIRECT256 potential',    0.05),
    ]

    print(f"\n{'='*70}")
    print(f"  DIRECT 256 OUTPUT + POTENTIAL FITNESS")
    print(f"  H={H}, in=64 SDR, out=256 direct, hidden=64")
    print(f"{'='*70}")
    ...
    for mk, label, pw in MODES:
        ...
        try:
            for step in range(1, 2001):
                ...
                if step % EVAL_EVERY == 0:
                    el=time.time()-t0
                    # PURE accuracy eval (no potential bonus)
                    ea_list=[]
                    for s in eval_seqs:
                        sc=SelfWiringGraph.build_sparse_cache(mask)
        ...
        all_results.append({'mode':mk,'label':label,'best':best_eval,'pw':pw,
            'theta':float(theta.mean()),'acc':accepts,'time':elapsed})
        print(f"\n  [{mk}] DONE: best={best_eval*100:.1f}% theta={theta.mean():.2f} {elapsed:.0f}s")

    print(f"\n{'='*70}")
    print(f"  DIRECT 256 OUTPUT RESULTS")
    print(f"{'='*70}")
