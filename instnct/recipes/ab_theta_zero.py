"""
A/B: Does theta=0 (always-fire relay neuron) help?

3 seeds each, parallel. Configs:
  G1: theta range 1-15 (current)
  G2: theta range 0-15 (0 = always fires, no clip)

Run: python instnct/recipes/ab_theta_zero.py
"""
...
INIT_DENSITY = 0.05; BUILD_STEPS = 1200  # shorter for speed, 3 seeds compensate
PHI = (1 + 5**0.5) / 2
...
def init_w(bpi, bpo, data, bg, pol, theta_min):
    global BP_IN, _bp_out, _all_data, _bigram, _pol, _theta_min
    BP_IN = bpi; _bp_out = bpo; _all_data = data; _bigram = bg; _pol = pol
    _theta_min = theta_min

def _eval_bigram(qdata, theta, channel, pol_f, seqs, theta_min):
    qm = QuaternaryMask(H, qdata)
    sc = qm.to_directed_edges()
    _dummy = np.zeros((H, H), dtype=bool)

    # Monkey-patch clip min for theta=0 support
    if theta_min == 0:
        # Custom forward: don't clip to 1.0, allow 0.0
        ...
        for tb in seqs:
            ...
            for i in range(len(tb) - 1):
                ...
                for tick in range(TICKS):
                    ...
                    # KEY CHANGE: clip min=0.0 instead of 1.0
                    effective_theta = np.clip(theta * theta_mult, 0.0, MAX_CHARGE)
...
def worker_eval(args):
    ...
    if pt == 'add':
        ...
        nt[idx] = float(rng.randint(_theta_min, 15))
    elif pt == 'channel':
    ...
    old = _eval_bigram(qm.data, theta, channel, pol_f, seqs, _theta_min)
    new = _eval_bigram(nq.data, nt, nc, npf, seqs, _theta_min)
    return {
...
def eval_accuracy(qdata, theta, channel, pol_f, text_bytes, bp_in, bp_out, theta_min):
    qm = QuaternaryMask(H, qdata)
    sc = qm.to_directed_edges()
    _dummy = np.zeros((H, H), dtype=bool)
    MAX_CHARGE = 15.0
...
def run_one(seed, theta_min, bp_in, bp_out, all_data, bigram, eval_seqs):
    ...
    label = f"theta_min={theta_min}, seed={seed}"
    print(f"\n  Starting: {label}")

    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp_in, bp_out, all_data, bigram, pol_f, theta_min))
    ...
    try:
        for step in range(1, BUILD_STEPS + 1):
            ...
            if step % EVAL_EVERY == 0:
                ea = np.mean([eval_accuracy(qdata, theta, channel, pol_f, s, bp_in, bp_out, theta_min)
                              for s in eval_seqs])
                if ea > best_eval: best_eval = ea
                elapsed = time.time() - t0
                # Count theta=0 neurons
                n_zero = int(np.sum(theta < 0.5))
                print(f"  [{label}][{step:5d}] eval={ea*100:.1f}% best={best_eval*100:.1f}% "
                      f"edges={QuaternaryMask(H, qdata).count_edges()} theta0={n_zero} {elapsed:.0f}s")
                sys.stdout.flush()
    finally:
        pool.terminate(); pool.join()

    elapsed = time.time() - t0
    final_eval = np.mean([eval_accuracy(qdata, theta, channel, pol_f, s, bp_in, bp_out, theta_min)
                          for s in eval_seqs])
    n_zero = int(np.sum(theta < 0.5))
    return {
        'theta_min': theta_min, 'seed': seed,
...
if __name__ == "__main__":
    ...
    # Run all configs sequentially (each uses full N_WORKERS)
    for theta_min in [1, 0]:
        for seed in SEEDS:
            r = run_one(seed, theta_min, bp_in, bp_out, ALL_DATA, bigram, eval_seqs)
            all_results.append(r)
            print(f"  DONE: theta_min={theta_min} seed={seed} -> best={r['best']}% theta0={r['theta0_count']}")

    # Summary
    print(f"\n{'='*60}")
    print(f"  SUMMARY (3 seeds each)")
    print(f"{'='*60}")
    for tmin in [1, 0]:
        runs = [r for r in all_results if r['theta_min'] == tmin]
        bests = [r['best'] for r in runs]
        finals = [r['final'] for r in runs]
        t0s = [r['theta0_count'] for r in runs]
        print(f"  theta_min={tmin}: best={np.mean(bests):.1f}% +/- {np.std(bests):.1f}%  "
              f"final={np.mean(finals):.1f}% +/- {np.std(finals):.1f}%  "
              f"theta0_neurons={np.mean(t0s):.0f}")

    out_path = os.path.join(BASE_DIR, "data", "theta_zero_ab_result.json")
