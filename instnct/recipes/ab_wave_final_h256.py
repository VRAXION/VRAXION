"""
Final wave test at H=256: trit LUT vs sin() binary
====================================================
Same setup as all prior H=256 tests for direct comparison.
Reference points: no wave=6.7%, sin binary=21.4%, uint8=16.2%

A: sin() binary {0.5,2.0}x{0,pi} (4 types) — reproduce 21.4%
B: trit 2 types (H=64 winner, 16.6%) — does it scale?
"""
...
def _rollout_trit(injected, mask, theta, ticks, input_duration, state, charge, sparse_cache, polarity):
    HS = mask.shape[0]
    ...
    for tb in seqs:
        ...
        for i in range(len(tb)-1):
            ...
            if _mode == 'sin':
                ...
                state,charge=_rollout_trit(inj,mask=mask,theta=theta,
...
if __name__ == "__main__":
    ...
    neuron_types = nrng.randint(0, 2, size=H).astype(np.int32)

    MODES = [
        ('A', 'sin() binary {0.5,2.0}x{0,pi}', 'sin', freq_sin, phase_sin, rho, TRIT_2, neuron_types),
        ('B', 'trit LUT 2 types (E/~/H)', 'trit', None, None, None, TRIT_2, neuron_types),
    ]

    print(f"\n{'='*60}")
    print(f"  FINAL H=256: sin() binary vs trit LUT")
    print(f"  ref: no wave=6.7%, uint8=16.2%")
    print(f"{'='*60}")
    print(f"  Trit LUT:")
    print(f"    type0: H H ~ E E E ~ H")
    print(f"    type1: H E E ~ H H E ~")
    sys.stdout.flush()
    ...
    for mk, label, mode, fr, ph, rh, wt, nt in MODES:
        ...
        for step in range(1, 2001):
            ...
            if step%EVAL_EVERY==0:
                ...
                for s in eval_seqs:
                    ...
                    for i in range(len(s)-1):
                        ...
                        if mode == 'sin':
                            ...
                            st,ch=_rollout_trit(inj,mask=mask,theta=theta,
                ...
                if ea>best:best=ea
                if step%200==0:
                    print(f"  [{mk}:{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% th={theta.mean():.1f} acc={acc} {time.time()-t0:.0f}s")
                    sys.stdout.flush()
    ...
    print(f"\n{'='*60}")
    print(f"  FINAL H=256 RESULTS")
    print(f"  ref: no wave=6.7%, uint8 frozen=16.2%")
    print(f"{'='*60}")
    for r in results:
