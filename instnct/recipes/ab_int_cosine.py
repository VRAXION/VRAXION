"""
A/B: Float cosine LUT vs Integer x10 cosine LUT.
Verifies that integer approximation (7,8,10,12,13) matches float (0.70..1.30).

Run: python instnct/recipes/ab_int_cosine.py
"""
...
# Integer x10 LUT: 7,8,10,12,13 (hardcoded, no cos())
INT_WAVE_LUT = np.array([
    [10, 10, 10, 10, 10, 10, 10, 10],  # ch0 neutral
    [ 7,  8, 10, 12, 13, 12, 10,  8],  # ch1
    [ 8,  7,  8, 10, 12, 13, 12, 10],  # ch2
    [10,  8,  7,  8, 10, 12, 13, 12],  # ch3
    [12, 10,  8,  7,  8, 10, 12, 13],  # ch4
    [13, 12, 10,  8,  7,  8, 10, 12],  # ch5
    [12, 13, 12, 10,  8,  7,  8, 10],  # ch6
    [10, 12, 13, 12, 10,  8,  7,  8],  # ch7
    [ 8, 10, 12, 13, 12, 10,  8,  7],  # ch8
], dtype=np.int32)

BP_IN = None
_bp_out = None; _all_data = None; _bigram = None; _pol = None
_mode = 'float'
_int_lut = INT_WAVE_LUT
        ...
        for i in range(len(tb) - 1):
            ...
            if _mode == 'float':
                ...
                for tick in range(TICKS):
                    ...
                    # Integer comparison: charge*10 >= theta * int_lut
                    lut_vals = _int_lut[channel, tick % 8].astype(np.float32)
                    fired = (charge * 10.0) >= (theta * lut_vals)
                    state[:] = 0.0
...
CONFIGS = {
    'F1_float': {'label': 'Float cosine (current)', 'mode': 'float'},
    'F2_int10': {'label': 'Integer x10 (7,8,10,12,13)', 'mode': 'int'},
}
...
def run_config(name, cfg, init_state, bp_in, bp_out, all_data, bigram, eval_seqs):
    ...
    print(f"\n  DONE: {name} -> final={final_eval*100:.1f}% best={best_eval*100:.1f}% time={elapsed:.0f}s")
    return result
...
if __name__ == "__main__":
    ...
    for name, r in all_results.items():
        print(f"  {r['label']:40s}  final={r['final_eval']:5.1f}%  best={r['best_eval']:5.1f}%")

    out_path = os.path.join(BASE_DIR, "data", "int_cosine_ab_result.json")
