"""
Designed LUT vs sin() binary at H=256
======================================
Hand-designed fair schedule: 4 types, each gets exactly 2 easy ticks.
No sin(), just a clean TDMA-style round-robin.

A: sin() binary (current best, reproduce ~21.4%)
B: designed LUT (fair 4-4, round-robin)
C: designed LUT v2 (paired: 0+1 vs 2+3 staggered)
"""
...
# A: sin() binary — use actual rollout_token with sin
RHO = 0.3

# B: designed fair round-robin
#    tick:  0  1  2  3  4  5  6  7
# type0:    E  ~  H  H  E  ~  H  H   easy: 0,4
# type1:    H  E  ~  H  H  E  ~  H   easy: 1,5
# type2:    H  H  E  ~  H  H  E  ~   easy: 2,6
# type3:    ~  H  H  E  ~  H  H  E   easy: 3,7
DESIGNED_A = np.array([
    [E,  M,  H_, H_, E,  M,  H_, H_],
    [H_, E,  M,  H_, H_, E,  M,  H_],
    [H_, H_, E,  M,  H_, H_, E,  M ],
...
# C: paired stagger — type 0+2 and 1+3 share easy windows but offset
#    tick:  0  1  2  3  4  5  6  7
# type0:    E  E  ~  H  H  H  ~  M   easy: 0,1 (early burst)
# type1:    H  H  ~  M  E  E  ~  H   easy: 4,5 (late burst)
# type2:    ~  H  E  E  ~  M  H  H   easy: 2,3 (mid burst)
# type3:    ~  M  H  H  ~  H  E  E   easy: 6,7 (end burst)
DESIGNED_B = np.array([
    [E,  E,  M,  H_, H_, H_, M,  M ],
    [H_, H_, M,  M,  E,  E,  M,  H_],
    [M,  H_, E,  E,  M,  M,  H_, H_],
...
_mode=None;_freq=None;_phase=None;_rho=None;_wave_table=None;_neuron_types=None

def init_w(bpo,data,bg,pol,mode,fr,ph,rh,wt,nt):
    global _bp_out,_all_data,_bigram,_pol,_mode,_freq,_phase,_rho,_wave_table,_neuron_types
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_mode=mode
    _freq=fr;_phase=ph;_rho=rh;_wave_table=wt;_neuron_types=nt
...
if __name__ == "__main__":
    ...
    sin_binary_f = frng.randint(0, 2, size=H)
    sin_binary_p = frng.randint(0, 2, size=H)
    freq_sin = (0.5 + sin_binary_f * 1.5).astype(np.float32)
    phase_sin = (sin_binary_p * np.pi).astype(np.float32)
    rho = np.full(H, 0.3, np.float32)

    # Neuron types for LUT modes
    nrng = np.random.RandomState(42)
    neuron_types = nrng.randint(0, 4, size=H).astype(np.int32)

    MODES = [
        ('A', 'sin() binary (baseline)', 'sin', freq_sin, phase_sin, rho, DESIGNED_A, neuron_types),
        ('B', 'designed round-robin', 'lut', None, None, None, DESIGNED_A, neuron_types),
        ('C', 'designed paired burst', 'lut', None, None, None, DESIGNED_B, neuron_types),
    ]

    print(f"\n{'='*60}")
    print(f"  DESIGNED LUT vs SIN() at H={H}")
    print(f"  ref: sin binary=21.4%, no wave=6.7%")
    print(f"{'='*60}")

    for mk, label, mode, fr, ph, rh, wt, nt in MODES:
        if mode == 'lut':
            print(f"\n  {mk}: {label}")
            for i in range(wt.shape[0]):
                row = ' '.join(f"{'E' if wt[i,t]<0.85 else ('H' if wt[i,t]>1.15 else '~')}" for t in range(TICKS))
                easy = sum(1 for t in range(TICKS) if wt[i,t] < 0.85)
                print(f"    type{i}: {row}  ({easy} easy)")
    sys.stdout.flush()
    ...
    for mk, label, mode, fr, ph, rh, wt, nt in MODES:
        ...
        init_w(bp_out, ALL_DATA, bigram, pol_f32, mode, fr, ph, rh, wt, nt)
        pool = Pool(N_WORKERS, initializer=init_w,
            initargs=(bp_out, ALL_DATA, bigram, pol_f32, mode, fr, ph, rh, wt, nt))
    ...
    print(f"\n{'='*60}")
    print(f"  DESIGNED vs SIN RESULTS (H={H})")
    print(f"{'='*60}")
    for r in results:
        print(f"  {r['mode']}: {r['label']:35s} best={r['best']*100:5.1f}%")
    print(f"{'='*60}")
