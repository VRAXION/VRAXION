"""
Quick test: Freq-ordered INPUT vs SDR input (both with freq-ordered output)
==========================================================================
A: SDR input + FREQ output (current best, 22.4%)
B: FREQ input + FREQ output (symmetric!)
"""
...
def build_freq_order(dim, bigram, seed=42):
...
_pol=None;_freq=None;_phase=None;_rho=None;_in_dim=None

def init_w(bpi,bpo,data,bg,pol,fr,ph,rh,ind):
    global _bp_in,_bp_out,_all_data,_bigram,_pol,_freq,_phase,_rho,_in_dim
    _bp_in=bpi;_bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_freq=fr;_phase=ph;_rho=rh;_in_dim=ind
...
if __name__ == "__main__":
    ...
    bp_out = build_freq_order(OUT_DIM, bigram, seed=12345)

    MODES = [
        ('A', 'SDR_in + FREQ_out',  build_sdr(256, IN_DIM, SDR_K, 42), IN_DIM),
        ('B', 'FREQ_in + FREQ_out', build_freq_order(IN_DIM, bigram, seed=42), IN_DIM),
    ]

    results = []
    for mk, label, bp_in, in_dim in MODES:
        print(f"\n{'='*60}")
        print(f"  {mk}: {label}")
        print(f"{'='*60}")
        ...
        pool = Pool(N_WORKERS, initializer=init_w,
            initargs=(bp_in, bp_out, ALL_DATA, bigram, pol, ref.freq, ref.phase, ref.rho, in_dim))
        ...
        try:
            for step in range(1, 2001):
                ...
                if step%EVAL_EVERY==0:
                    ...
                    if step%200==0:
                        print(f"  [{mk}][{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% theta={theta.mean():.2f} acc={acc} {el:.0f}s")
                    sys.stdout.flush()
