"""Freq+Phase mode B: wave FROZEN at init (diversity but no learning)"""
...
if __name__ == "__main__":
    ...
    print(f"[B] Loaded {len(ALL_DATA)/1e6:.1f} MB")
    ...
    for step in range(1, 2001):
        ...
        if step%EVAL_EVERY==0:
            ...
            if ea>best:best=ea
            if step%200==0:
                print(f"[B] [{step:4d}] eval={ea*100:.1f}% best={best*100:.1f}% theta={theta.mean():.1f} edges={mask.sum()} acc={acc} {time.time()-t0:.0f}s")
            sys.stdout.flush()
    pool.terminate();pool.join()
    elapsed=time.time()-t0
    print(f"[B] DONE: wave FROZEN best={best*100:.1f}% {elapsed:.0f}s")
    try:
        save_experiment("freq_phase_B_frozen",mask=mask,theta=theta,
            metadata={'mode':'B','label':'wave FROZEN at init','best':float(best),'acc':acc},
            extra_arrays={'freq':freq_arr,'phase':phase_arr})
    except Exception as e: print(f"[B] Archive: {e}")
    with open(os.path.join(BASE_DIR,"data/fp_B_result.json"),"w") as f:
        json.dump({'mode':'B','best':float(best),'acc':acc,'time':elapsed},f)
