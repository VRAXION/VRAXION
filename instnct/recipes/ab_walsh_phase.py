"""
A/B/C test: Cosine LUT vs Walsh 3-bool vs Walsh 2-bool phase gating.

Identical network init, mutations, eval — only the phase LUT differs.
Monkey-patches SelfWiringGraph.WAVE_LUT per config, no graph.py changes.

Run: python instnct/recipes/ab_walsh_phase.py
"""
...
# Walsh LUT builder
# -------------------------------------------------------------------------
WALSH_MASKS = [0xF0, 0xCC, 0xAA]  # period 8, 4, 2

def build_walsh_lut(num_layers):
    """Build Walsh-basis phase gating LUT.
    num_layers=2 -> 4 combos, num_layers=3 -> 8 combos.
    Returns numpy array shape (9, 8) float32 — padded to match WAVE_LUT shape.
    Row 0 = neutral (all 1.0). Rows beyond combos = neutral."""
    combos = 1 << num_layers
    lut = np.ones((9, 8), dtype=np.float32)  # same shape as original WAVE_LUT
    for flags in range(1, combos):
        for tick in range(8):
            active = total = 0
            for layer in range(num_layers):
                if flags & (1 << layer):
                    total += 1
                    if WALSH_MASKS[layer] & (1 << (7 - tick)):
                        active += 1
            lut[flags, tick] = 0.7 if active * 2 > total else 1.3
    return lut

# The three configs
CONFIGS = {
    'A_cosine': {
        'label': 'Cosine LUT (current)',
        'lut': None,  # use default WAVE_LUT
        'channel_range': (1, 9),  # [1, 8]
    },
    'B_walsh3': {
        'label': 'Walsh 3-bool (8 combos)',
        'lut': build_walsh_lut(3),  # shape (9, 8), rows 1-7 used
        'channel_range': (1, 8),  # [1, 7] — 7 Walsh combos (flags 1-7)
    },
    'C_walsh2': {
        'label': 'Walsh 2-bool (4 combos)',
        'lut': build_walsh_lut(2),  # shape (9, 8), rows 1-3 used
        'channel_range': (1, 4),  # [1, 3] — 3 Walsh combos (flags 1-3)
    },
}

# Save original WAVE_LUT
_ORIGINAL_WAVE_LUT = SelfWiringGraph.WAVE_LUT.copy()
...
def init_w(bpi, bpo, data, bg, pol, ch_lo, ch_hi, wave_lut):
    global BP_IN, _bp_out, _all_data, _bigram, _pol, _channel_lo, _channel_hi
    BP_IN = bpi; _bp_out = bpo; _all_data = data; _bigram = bg; _pol = pol
    _channel_lo = ch_lo; _channel_hi = ch_hi
    # Monkey-patch inside worker process too
...
def worker_eval(args):
    ...
    if pt == 'add':
        ...
        nc[idx] = np.uint8(rng.randint(_channel_lo, _channel_hi - 1))
...
def run_config(name, cfg, init_state, bp_in, bp_out, all_data, bigram, eval_seqs):
    """Run one BUILD config. Returns dict of results."""
    qdata, theta, channel, pol_f = [x.copy() for x in init_state]

    # Monkey-patch the WAVE_LUT
    if cfg['lut'] is not None:
        SelfWiringGraph.WAVE_LUT = cfg['lut']
    else:
        SelfWiringGraph.WAVE_LUT = _ORIGINAL_WAVE_LUT

    ch_lo, ch_hi = cfg['channel_range']

    # Re-init channel to match this config's range
    rng_ch = np.random.RandomState(42)
    channel = rng_ch.randint(ch_lo, ch_hi, size=H).astype(np.uint8)

    print(f"\n{'='*60}")
    print(f"  Config: {name} -- {cfg['label']}")
    print(f"  Channel range: [{ch_lo}, {ch_hi})")
    print(f"  LUT shape: {SelfWiringGraph.WAVE_LUT.shape}")
    print(f"{'='*60}")

    active_lut = SelfWiringGraph.WAVE_LUT
    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp_in, bp_out, all_data, bigram, pol_f, ch_lo, ch_hi, active_lut))
...
if __name__ == "__main__":
    ...
    # Shared init state (same for all configs)
    ...
    channel_init = init_rng.randint(1, 9, size=H).astype(np.uint8)  # will be overridden
    ...
    for name, r in all_results.items():
        print(f"  {r['label']:30s}  final={r['final_eval']:5.1f}%  "
              f"best={r['best_eval']:5.1f}%  edges={r['edges']}  time={r['elapsed_sec']:.0f}s")

    # Save results
    out_path = os.path.join(BASE_DIR, "data", "walsh_phase_ab_result.json")
