"""
Triangle wave vs ramp vs sin() at H=256
=========================================
Triangle goes up-down (like sin) but with simple arithmetic.
No sin(), no LUT. Just: position → triangle value.

A: triangle 4 offsets (0,2,4,6)
B: triangle 8 offsets (0-7)
ref: sin binary = 21.4%, ramp 4 = 14.7%
"""
...
# Triangle wave: up from 0.7 to 1.3, then back down
# position: 0    1    2    3    4    5    6    7
# value:   0.70 0.87 1.04 1.21 1.30 1.13 0.96 0.79
#          E    ~    ~    H    H    ~    ~    E
TRIANGLE = np.array([
    0.70, 0.87, 1.04, 1.21,  # up: 0→3
    1.30, 1.13, 0.96, 0.79,  # down: 4→7
], dtype=np.float32)
...
_bp_out=None;_all_data=None;_bigram=None;_pol=None;_offsets=None;_wave=None

def init_w(bpo,data,bg,pol,offs,wave):
    global _bp_out,_all_data,_bigram,_pol,_offsets,_wave
    _bp_out=bpo;_all_data=data;_bigram=bg;_pol=pol;_offsets=offs;_wave=wave

def _rollout_wave(injected, mask, theta, ticks, input_duration, state, charge, sparse_cache, polarity):
    HS = mask.shape[0]
...
def _eval_bigram(mask, theta, seqs):
    ...
    for tb in seqs:
        ...
        for i in range(len(tb)-1):
            ...
            state,charge=_rollout_wave(inj,mask=mask,theta=theta,
...
if __name__ == "__main__":
    ...
    print(f"\n  Triangle wave: {[f'{v:.3f}' for v in TRIANGLE]}")

    MODES = [
        ('A', 'triangle 4 offsets (0,2,4,6)', np.array([0,2,4,6])),
        ('B', 'triangle 8 offsets (0-7)', np.arange(8)),
    ]

    print(f"\n{'='*60}")
    print(f"  TRIANGLE WAVE at H={H}")
    print(f"  ref: sin=21.4%, ramp4=14.7%")
    print(f"{'='*60}")

    for mk, label, offset_vals in MODES:
        print(f"\n  {mk}: {label}")
        for o in offset_vals:
            positions = [(t + o) % 8 for t in range(8)]
            mults = [TRIANGLE[p] for p in positions]
            tags = ['E' if m<0.85 else ('H' if m>1.15 else '~') for m in mults]
            print(f"    offset={o}: [{' '.join(f'{m:.2f}{t}' for m,t in zip(mults,tags))}]")
    sys.stdout.flush()
    ...
    for mk, label, offset_vals in MODES:
        ...
        init_w(bp_out, ALL_DATA, bigram, pol_f32, offsets, TRIANGLE)
        pool = Pool(N_WORKERS, initializer=init_w,
            initargs=(bp_out, ALL_DATA, bigram, pol_f32, offsets, TRIANGLE))
            ...
            if step%EVAL_EVERY==0:
                ...
                for s in eval_seqs:
                    ...
                    for i in range(len(s)-1):
                        ...
                        st,ch=_rollout_wave(inj,mask=mask,theta=theta,
    ...
    print(f"\n{'='*60}")
    print(f"  TRIANGLE WAVE RESULTS (H={H})")
    print(f"  ref: sin=21.4%, ramp4=14.7%")
    print(f"{'='*60}")
    for r in results:
