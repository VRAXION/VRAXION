"""
A/B: theta=0 as "never fire" (neuron-level pruning).

3 seeds each. Configs:
  H1: theta 1-15 (current)
  H2: theta 0-15 where 0 = never fires (permanent off)

Run: python instnct/recipes/ab_theta_neverfire.py
"""
...
def init_w(bpi, bpo, data, bg, pol, theta_min, neverfire):
    global BP_IN, _bp_out, _all_data, _bigram, _pol, _theta_min, _neverfire_mode
    BP_IN = bpi; _bp_out = bpo; _all_data = data; _bigram = bg; _pol = pol
    _theta_min = theta_min; _neverfire_mode = neverfire
...
def _eval_bigram(qdata, theta, channel, pol_f, seqs):
    ...
    for tb in seqs:
        ...
        for i in range(len(tb) - 1):
            ...
            # Apply never-fire mask: theta=0 neurons forced to activation=0
            if _neverfire_mode:
                neverfire_mask = theta < 0.5
                state[neverfire_mask] = 0.0
...
def run_one(seed, theta_min, neverfire, bp_in, bp_out, all_data, bigram, eval_seqs):
    ...
    label = f"neverfire={neverfire}, seed={seed}"
    print(f"\n  Starting: {label}")

    pool = Pool(N_WORKERS, initializer=init_w,
                initargs=(bp_in, bp_out, all_data, bigram, pol_f, theta_min, neverfire))
    ...
    try:
        for step in range(1, BUILD_STEPS + 1):
            ...
            if step % EVAL_EVERY == 0:
                ...
                n_dead = int(np.sum(theta < 0.5))
                elapsed = time.time() - t0
                print(f"  [{label}][{step:5d}] eval={ea*100:.1f}% best={best_eval*100:.1f}% "
                      f"edges={QuaternaryMask(H, qdata).count_edges()} dead={n_dead} {elapsed:.0f}s")
                sys.stdout.flush()
    finally:
...
if __name__ == "__main__":
    ...
    # H1: baseline theta 1-15
    for seed in SEEDS:
        r = run_one(seed, 1, False, bp_in, bp_out, ALL_DATA, bigram, eval_seqs)
        all_results.append(r)
        print(f"  DONE: baseline seed={seed} -> best={r['best']}%")

    # H2: neverfire theta 0-15
    for seed in SEEDS:
        r = run_one(seed, 0, True, bp_in, bp_out, ALL_DATA, bigram, eval_seqs)
        all_results.append(r)
        print(f"  DONE: neverfire seed={seed} -> best={r['best']}% dead={r['dead_neurons']}")

    print(f"\n{'='*60}")
    print(f"  SUMMARY (3 seeds each)")
    print(f"{'='*60}")
    for nf in [False, True]:
        runs = [r for r in all_results if r['neverfire'] == nf]
        bests = [r['best'] for r in runs]
        finals = [r['final'] for r in runs]
        deads = [r['dead_neurons'] for r in runs]
        label = "neverfire=0" if not nf else "neverfire=1"
        print(f"  {label}: best={np.mean(bests):.1f}% +/- {np.std(bests):.1f}%  "
              f"final={np.mean(finals):.1f}% +/- {np.std(finals):.1f}%  "
              f"dead_neurons={np.mean(deads):.0f}")

    out_path = os.path.join(BASE_DIR, "data", "theta_neverfire_ab_result.json")
