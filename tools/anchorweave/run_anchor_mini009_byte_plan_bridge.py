#!/usr/bin/env python3
"""Parallel ANCHOR-MINI-009 byte-serialized PLAN bridge sweep."""
...
CONTRACT_FILE = Path("docs/research/ANCHOR_MINI_009_BYTE_PLAN_BRIDGE_CONTRACT.md")
CARRIERS = [
    "BYTE_DIRECT_ANSWER",
...
JOB_COMPLETE = "ANCHOR_MINI_009_JOB_COMPLETE"
JOB_INVALID = "ANCHOR_MINI_009_JOB_INVALID_STRESS"

STATUS_STRONG = "ANCHOR_MINI_009_BYTE_PLAN_STRONG_POSITIVE"
STATUS_WEAK = "ANCHOR_MINI_009_BYTE_PLAN_WEAK_POSITIVE"
STATUS_ORACLE_ONLY = "ANCHOR_MINI_009_ORACLE_ONLY"
STATUS_NEGATIVE = "ANCHOR_MINI_009_NEGATIVE"
STATUS_INVALID = "ANCHOR_MINI_009_INVALID_STRESS"
STATUS_BLOCKED = "ANCHOR_MINI_009_RESOURCE_BLOCKED"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ANCHOR-MINI-009 byte PLAN bridge sweep.")
...
def ensure_binary(root: Path, skip_build: bool) -> Path:
    exe = root / "target" / "release" / "examples" / (
        "evolve_anchor_mini009.exe" if os.name == "nt" else "evolve_anchor_mini009"
    )
    if skip_build:
        if not exe.exists():
        ...
        return exe
    subprocess.run(
        ["cargo", "build", "--release", "-p", "instnct-core", "--example", "evolve_anchor_mini009"],
        cwd=root,
        check=True,
    )
    if not exe.exists():
...
def serialize_task(goal_category: int, effect_categories: list[int], buckets: list[int]) -> str:
        ...
        rows.append(
            {
                ...
                "split": split,
                "task_bytes": serialize_task(goal_category, effect_categories, buckets),
                "goal_id": goal_id,
...
def make_dataset(
    *,
    ...
    write_json(
        ...
        {
            "metadata": {
                "source": "run_anchor_mini009_byte_plan_bridge.py",
                "mini": "ANCHOR-MINI-009",
                "seed": seed,
                "train_examples": train_examples,
                "eval_examples": eval_examples,
                "train_surface_gold_prob": train_surface_gold_prob,
                "eval_surface_wrong_prob": eval_surface_wrong_prob,
                "serialization": "fixed_ascii_byte_record",
                "task_bytes_shape": "G=<cat>;A=E<cat>:S<bucket>;B=E<cat>:S<bucket>;C=E<cat>:S<bucket>;D=E<cat>:S<bucket>",
            },
...
def aggregate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    hybrid = by_carrier.get("BYTE_PLAN_FIRST_HYBRID")
    shuffled = by_carrier.get("BYTE_SHUFFLED_TEACHER")
    shortcut = by_carrier.get("BYTE_SHORTCUT_TEACHER")
    oracle = by_carrier.get("BYTE_ORACLE_PLAN_VISIBLE")
    byte_integrity_all = bool(valid) and all(
        row.get("stress", {}).get("byte_input_integrity") is True for row in valid
    )
    conditions = {
        "valid_seeds_at_least_80": valid_seed_count >= 80,
        "byte_input_integrity_all": byte_integrity_all,
...
def write_report(path: Path, summary: dict[str, Any], *, elapsed: float, queue_len: int) -> None:
    lines = [
        "# ANCHOR-MINI-009 Byte-Serialized PLAN Bridge Sweep",
        "",
        "## Verdict",
        "",
        "```text",
        ...
        "## Metrics",
        "",
        "| carrier | jobs | answer_eval | trap_rate | shortcut | validity | invalid_reject | policy_bit | plan_exact | consistency | byte_ok | edges |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for carrier in CARRIERS:
    ...
    lines.extend(
        [
            ...
            "## Claim Boundary",
            "",
            "This is a byte-serialized sparse audit-carrier result. It does not prove",
            "natural-language AnchorCells, Qwen behavior, full INSTNCT recurrent",
            "behavior, or symbol grounding at scale.",
            "",
        ]
    )
...
def write_result_doc(root: Path, summary: dict[str, Any], command: str, elapsed: float) -> None:
    path = root / "docs" / "research" / "ANCHOR_MINI_009_RESULT.md"
    lines = [
