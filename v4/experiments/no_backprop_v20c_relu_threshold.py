"""
No-Backprop v20c — ReLU + Learnable Thresholds + Arousal
=========================================================
v18 self-wiring graph + per-neuron threshold + global arousal.
ReLU shifted by threshold: relu(raw - threshold * arousal)
Neurons need stronger signal to fire, but output is continuous.
...
class SelfWiringThresholdNet:
    """v18 self-wiring + per-neuron threshold + global arousal."""
    ...
    def __init__(self, n_neurons, n_in, n_out, addr_dim=4, density=0.06):
        ...
        # Per-neuron threshold (init low — "baby brain", learn sparse)
        self.thresholds = torch.full((n_neurons,), 0.01)
        # Global arousal multiplier (noradrenalin)
        self.arousal = 1.0
    ...
    def forward(self, world, diff, ticks=8):
        ...
        for t in range(ticks):
            ...
            act = torch.relu(raw - self.thresholds * self.arousal)  # shifted ReLU
            act[:self.n_in] = inp
    ...
    def mutate_structure(self, rate=0.05):
        # Thresholds + arousal evolve in structure phase too
...
if __name__ == "__main__":
    print(f"v20c ReLU + Learnable Thresholds + Arousal")
    print(f"=" * 60)
    ...
    net = SelfWiringThresholdNet(n_neurons, n_in, n_out, addr_dim=4, density=0.06)

    print(f"  Neurons: {n_neurons} | Ticks: {ticks}")
    print(f"  Activation: relu(raw - threshold * arousal)")
    print(f"  Per-neuron threshold (init=0.01) + global arousal (init=1.0)")
    print(f"  Everything evolves in all phases")
    ...
    print(f"  Arousal: {net.arousal:.3f}")

    print(f"\n  Compare 32-class:")
    print(f"  v18  (ReLU, no threshold):       100% at ~10k (~386s)")
    print(f"  v20c (ReLU + threshold + arousal): {final_acc*100:.0f}%")
