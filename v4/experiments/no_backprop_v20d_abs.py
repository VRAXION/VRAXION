"""
No-Backprop v20d — abs() Activation in Self-Wiring Graph
=========================================================
v18 self-wiring but relu → abs(). Online Claude found abs() is 3x faster
...
app = modal.App("vraxion-v20d-abs")
...
def run_experiment():
    ...
    class SelfWiringAbsNet:
        """v18 self-wiring graph with abs() activation."""
        ...
        def forward(self, world, diff, ticks=8):
            ...
            for t in range(ticks):
                act = act * self.decay
                act[:self.n_in] = inp
                act = torch.abs(act @ Weff + act * 0.1)  # abs() instead of relu
                act[:self.n_in] = inp
    ...
    print(f"v20d abs() Activation — Self-Wiring Graph")
    print(f"=" * 60)
    ...
    net = SelfWiringAbsNet(n_neurons, n_in, n_out, addr_dim=4, density=0.06)

    print(f"  Neurons: {n_neurons} | Ticks: {ticks}")
    print(f"  Activation: abs() — preserves negative signal magnitude")
    ...
    print(f"  v20d (abs):   {final_acc*100:.0f}%")
    print(f"  Online Claude claims: abs 100% at ~1000, 1770 conns")
...
def main():
    print("Running v20d abs() activation...")
