"""
No-Backprop v18 ABLATION — Self-Wiring DISABLED
==================================================
Exact same as v18 but _self_wire is a no-op.
If this also hits 100% at similar speed, self-wiring doesn't matter.
32-class only.
"""
...
app = modal.App("vraxion-v18-ablation")
...
def run_experiment():
    ...
    class SelfWiringNet_NoWire:
        """Same as v18 but _self_wire disabled."""
    ...
    print(f"v18 ABLATION — Self-Wiring DISABLED")
    print(f"=" * 60)
    ...
    net = SelfWiringNet_NoWire(n_neurons, n_in, n_out, addr_dim=4, density=0.06)

    print(f"  Neurons: {n_neurons} | Ticks: {ticks}")
    print(f"  SELF-WIRING: **DISABLED**")
    print(f"  Everything else identical to v18")
    ...
    print(f"\n  v18 WITH self-wiring:    100% at ~10k attempts (~386s)")
    print(f"  v18 WITHOUT self-wiring: {final_acc*100:.0f}% | {elapsed:.1f}s")
    if final_acc >= 1.0:
        print(f"  -> Self-wiring DOESN'T MATTER")
    else:
        print(f"  -> Self-wiring DOES MATTER")
...
def main():
    print("Running v18 ABLATION (no self-wiring)...")
