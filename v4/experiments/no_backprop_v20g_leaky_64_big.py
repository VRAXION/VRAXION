"""
No-Backprop v20g — 64-CLASS with MORE neurons (480)
=====================================================
v20e/f plateaued at 55% with 320 neurons. Theory: not enough
internal neurons (320 - 128 - 64 = 128 internal).
Try 480 neurons (480 - 128 - 64 = 288 internal, 2.25x more).
Also: more aggressive self-wiring (top_k=5, max_new=3).
leaky_relu, density=0.06.
"""

import modal
import time

app = modal.App("vraxion-v20g-64class-big")
...
@app.function(cpu=2, memory=2048, timeout=3600, image=image)
def run_experiment():
    import torch
    ...
    class SelfWiringLeakyNet:
        ...
        def _self_wire(self, activations, top_k=5, max_new=3):
            """More aggressive self-wiring for 64-class."""
    ...
    print(f"v20g — 64-CLASS, 480 neurons, aggressive self-wiring")
    print(f"=" * 60)
    ...
    print(f"  Ticks: {ticks} | Self-wire: top_k=5, max_new=3")
    ...
    for attempt in range(max_attempts):
        ...
        if (attempt + 1) % 2000 == 0:
            ...
            if phase == "STRUCTURE" and not phase_switched and acc >= 1.0:
                phase = "BOTH"
                phase_switched = True
                print(f"  -- ACC 100%, switching to BOTH --")
                stale = 0
...
def main():
    print("Running v20g 64-class with 480 neurons...")
