"""
No-Backprop v20b — Binary Activation + Learnable Thresholds
=============================================================
Same as v18 but activation is binary: neuron fires (1) or not (0).
Per-neuron learnable threshold — the network decides its own sparsity.

Compare: ReLU (sparse-ish) vs C19 (dense) vs Binary (learnable sparse)
...
app = modal.App("vraxion-v20b-binary")
...
def run_experiment():
    ...
    class SelfWiringBinaryNet:
        """v18 self-wiring graph with binary activation + per-neuron threshold."""
        ...
        def __init__(self, n_neurons, n_in, n_out, addr_dim=4, density=0.06):
            ...
            self.thresholds = torch.full((n_neurons,), 0.01)
            # Global arousal — multiplier on all thresholds (noradrenalin)
            self.arousal = 1.0
        ...
        def forward(self, world, diff, ticks=8):
            ...
            for t in range(ticks):
                ...
                act = (raw > self.thresholds * self.arousal).float()  # threshold * arousal
                act[:self.n_in] = inp
        ...
        def _self_wire(self, activations, top_k=3, max_new=2):
            ...
            active = (act > 0).nonzero(as_tuple=False).squeeze(-1)
            if len(active) == 0:
                return
            n_candidates = min(top_k, len(active))
            perm = torch.randperm(len(active))[:n_candidates]
            top_idx = active[perm] + internal_start

            new_connections = 0
            for neuron_idx in top_idx:
                ni = neuron_idx.item()

                target = self.addresses[ni] + self.target_W[ni]
        ...
        def save_state(self):
            return (
                ...
                self.thresholds.clone(), self.arousal
            )
        ...
        def restore_state(self, saved):
            ...
            self.thresholds = saved[5].clone()
            self.arousal = saved[6]

        def mutate_structure(self, rate=0.05):
            # Thresholds mutate in structure phase too
            self.thresholds += torch.randn_like(self.thresholds) * 0.05
            self.thresholds.clamp_(0.01, 5.0)
            self.arousal += random.gauss(0, 0.03)
            self.arousal = max(0.1, min(5.0, self.arousal))
        ...
        def mutate_weights(self, scale=0.05):
            ...
            # Evolve thresholds + arousal
            self.thresholds += torch.randn_like(self.thresholds) * scale * 0.3
            self.thresholds.clamp_(0.01, 5.0)
            self.arousal += random.gauss(0, scale * 0.2)
            self.arousal = max(0.1, min(5.0, self.arousal))
    ...
    print(f"v20b Binary + Learnable Thresholds — Self-Wiring Graph")
    print(f"=" * 60)
    ...
    net = SelfWiringBinaryNet(n_neurons, n_in, n_out, addr_dim=4, density=0.06)

    print(f"  Neurons: {n_neurons} | Ticks: {ticks}")
    print(f"  Activation: BINARY (raw > threshold → 1, else 0)")
    print(f"  Per-neuron threshold (init=0.01, range [0.01, 5.0]) — start dense, learn sparse")
    print(f"  Thresholds + arousal evolve in ALL phases")
    ...
    for attempt in range(max_attempts):
        ...
        if (attempt + 1) % 500 == 0:
            _, acc = evaluate(net, inputs, perm, v, ticks)
            elapsed = time.time() - t0
            # Count active neurons on a sample input
            net.reset_state()
            world = torch.zeros(v); world[0] = 1.0
            net.forward(world, torch.zeros(v), ticks=ticks)
            n_active = (net.state > 0).sum().item()
            thr_mean = net.thresholds.mean().item()
            thr_std = net.thresholds.std().item()
            print(f"  [{phase:9s}] {attempt+1:6d} | "
                  f"Acc: {acc*100:.0f}% | Score: {current_score:.3f} | "
                  f"Kept: {kept} | Conns: {net.count_connections()} | "
                  f"Active: {n_active}/{n_neurons} | "
                  f"Thr: {thr_mean:.2f}±{thr_std:.2f} | "
                  f"Arousal: {net.arousal:.2f} | "
    ...
    thr_final = net.thresholds.mean().item()
    thr_std = net.thresholds.std().item()
    thr_min = net.thresholds.min().item()
    thr_max = net.thresholds.max().item()

    print(f"\n  Final: {final_acc*100:.0f}% | Score: {best_score:.3f} | "
          f"Conns: {net.count_connections()} | {elapsed:.1f}s")
    print(f"  Thresholds: {thr_final:.3f} ± {thr_std:.3f} "
          f"(min={thr_min:.3f}, max={thr_max:.3f})")

    print(f"\n  Compare 32-class:")
    print(f"  v18  (ReLU + self-wire):    100% at ~10k (~386s)")
    print(f"  v20  (C19 + self-wire):      59% at 4.5k (timeout)")
    print(f"  v20b (Binary + thresholds): {final_acc*100:.0f}%")

    return {"acc": final_acc, "score": best_score, "time": elapsed,
            "conns": net.count_connections(),
            "thr_mean": thr_final, "thr_std": thr_std}


@app.local_entrypoint()
def main():
    print("Running v20b binary + learnable thresholds...")
    results = run_experiment.remote()
    print(f"\nDone! {results['acc']*100:.0f}% | "
          f"threshold={results['thr_mean']:.2f}±{results['thr_std']:.2f}")
