"""
No-Backprop v20f — Dense Start + Prune — 64-CLASS
====================================================
Baby brain approach: start with HIGH density (0.30) and let
mutation prune it down. Self-wiring still active to add
targeted connections where needed.

v20e (density=0.06) plateaued at 55% — not enough material.
Baby brains have 2x adult synapses at birth, then prune.
...
app = modal.App("vraxion-v20f-64class-dense")
...
def run_experiment():
    ...
    class SelfWiringLeakyNet:
        def __init__(self, n_neurons, n_in, n_out, addr_dim=4, density=0.30):
            self.N = n_neurons
            self.n_in = n_in
            self.n_out = n_out
    ...
    print(f"v20f Dense Start — 64-CLASS SCALE TEST")
    print(f"=" * 60)
    ...
    net = SelfWiringLeakyNet(n_neurons, n_in, n_out, addr_dim=4, density=0.30)

    print(f"  Neurons: {n_neurons} | Ticks: {ticks}")
    print(f"  Classes: {v} | n_in: {n_in} | n_out: {n_out}")
    print(f"  Activation: leaky_relu")
    print(f"  Starting density: 0.30 (baby brain — dense start, prune down)")
    print(f"  Self-wiring: active")
    ...
    for attempt in range(max_attempts):
        ...
        if (attempt + 1) % 2000 == 0:
            ...
            # BOTH trigger: acc high but score low
    ...
    print(f"\n  Final: {final_acc*100:.0f}% | Best acc: {best_acc*100:.0f}% | "
          f"Score: {best_score:.3f} | Conns: {net.count_connections()} | {elapsed:.1f}s")

    print(f"\n  Compare 64-class:")
    print(f"  v20e (density=0.06): 55% at 10k (plateau)")
    print(f"  v20f (density=0.30): {final_acc*100:.0f}%")
...
def main():
    print("Running v20f dense-start 64-class...")
