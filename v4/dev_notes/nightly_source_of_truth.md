...
1. **`ring_signal_norm` — ~~DELETE~~ DONE** (12,288 params, LayerNorm)
   - Removed. Was never called in forward(). Dead since dotprod gate.

2. **`S_raw` + `s_constraint` — ~~DELETE~~ DONE** (1 scalar + attribute)
   - Removed from model, factory, and all test specs. Probe scripts guarded with hasattr.

3. **`c19_rho_input` + `c19_C_input` — ~~MAKE CONDITIONAL~~ DONE** (12,288 params)
   - Now only created when `self._bitlift` is True. Hidden c19 params unchanged.

4. **`write_gate` init — ~~A/B TEST~~ APPLIED** (6,145 params, not dead but collapsed)
   - Changed from `zeros_` to `xavier_uniform_(gain=0.01)` + same bias=-0.847.
   - Needs fresh run to validate (old checkpoints will overwrite this init on resume).
