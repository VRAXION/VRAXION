"""
Precise Profiling — What takes how long?
...
for cfg in configs:
    ...
    # ===== 4. forward() — single input, 6 ticks =====
    ...
    print(f"\n  KOMPONENS IDŐK (microsec):")
    print(f"    save_state:     {t_save*1e6:>10.1f} µs")
    print(f"    mutate:         {t_mutate*1e6:>10.1f} µs")
    print(f"    reset:          {t_reset*1e6:>10.1f} µs")
    print(f"    forward (1x):   {t_forward_single*1e6:>10.1f} µs")
    print(f"    softmax:        {t_softmax*1e6:>10.1f} µs")
    print(f"    matmul (NxN):   {t_matmul*1e6:>10.1f} µs")
    print(f"    full_eval:      {t_full_eval*1e6:>10.1f} µs")
    print(f"    restore_state:  {t_restore*1e6:>10.1f} µs")

    print(f"\n  FULL EVAL BREAKDOWN:")
    fwd_in_eval = t_forward_single * V * 2
    softmax_in_eval = t_softmax * V
    other_in_eval = t_full_eval - fwd_in_eval - softmax_in_eval
    print(f"    forward calls:  {V*2:>5} × {t_forward_single*1e6:.1f}µs = {fwd_in_eval*1e6:>10.1f} µs ({fwd_in_eval/t_full_eval*100:.0f}%)")
    print(f"    softmax calls:  {V:>5} × {t_softmax*1e6:.1f}µs = {softmax_in_eval*1e6:>10.1f} µs ({softmax_in_eval/t_full_eval*100:.0f}%)")
    print(f"    overhead:       {other_in_eval*1e6:>10.1f} µs ({other_in_eval/t_full_eval*100:.0f}%)")

    print(f"\n  TELJES ATTEMPT:")
    print(f"    1 attempt:      {t_one_attempt*1e6:>10.1f} µs ({t_one_attempt*1000:.2f} ms)")
    ...
    print(f"    full_eval:      {t_full_eval/total*100:>5.1f}%  ← EZ A KÉRDÉS")
    print(f"    restore_state:  {t_restore/total*100:>5.1f}%")

    print(f"\n  MATMUL vs FORWARD:")
    print(f"    1 matmul (v@A): {t_matmul*1e6:.1f} µs")
    print(f"    1 forward:      {t_forward_single*1e6:.1f} µs")
    print(f"    forward/matmul: {t_forward_single/t_matmul:.1f}× (TICKS={6} + overhead)")

    print(f"\n  'MI LENNE HA' SCENARIOS:")
    # Half eval
    t_half_eval = t_forward_single * V + softmax_in_eval/2 + other_in_eval/2
    t_half_att = t_save + t_mutate + t_half_eval + t_restore
    print(f"    Half eval ({V//2} samples):  {1/t_half_att:.0f} att/s ({1/t_half_att/(1/t_one_attempt):.1f}× faster)")
    ...
    print(f"    GPU matmul:     {1/t_gpu_att:.0f} att/s ({1/t_gpu_att/(1/t_one_attempt):.1f}× faster)")

    # Online (1 sample)
    t_online_att = t_save + t_mutate + t_forward_single*2 + t_softmax*2 + t_restore
    print(f"    Online (1 sample): {1/t_online_att:.0f} att/s ({1/t_online_att/(1/t_one_attempt):.1f}× faster)")
