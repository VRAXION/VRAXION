"""Ablation bench: phase ON vs phase OFF, tanh only, all 3 tasks.
Toggles phase via a flag in the model forward pass.

Usage: python tests/bench_ablation.py
"""

import sys, time, math, numpy as np, torch, torch.nn.functional as F
...
# ── Data generators (same as bench_activations_v2) ──
...
def run_one(task_info, phase_enabled, device='cpu'):
    torch.manual_seed(SEED)
    np.random.seed(SEED)

    model = INSTNCT(**MODEL_CFG).to(device)

    # If phase disabled, zero out and freeze phase params
    if not phase_enabled:
        with torch.no_grad():
            model.phase_cos.zero_()
            model.phase_sin.zero_()
        model.phase_cos.requires_grad_(False)
        model.phase_sin.requires_grad_(False)
...
def main():
    print("=" * 70)
    print("  ABLATION: Phase ON vs Phase OFF (tanh, depth removed)")
    print("=" * 70)
    ...
    for task_name, task_info in TASKS.items():
        ...
        for phase_on in [False, True]:
            label = "phase ON " if phase_on else "phase OFF"
            print(f"    {label}...", end='', flush=True)
            acc, wall, n_params = run_one(task_info, phase_on)
