...
MTAP_DIAG_LIST_PREFIXES = (
    ...
    "head_unique_frac_",
    "fixed_head_offset_mean_abs_",
    "fixed_head_near_local_frac_",
    "fixed_head_center_dist_mean_",
    "fixed_head_unique_frac_",
    "fixed_head_gate_mean_",
)
...
VARIANTS: dict[str, dict] = {
    ...
    "LLT3F216SG": {
        ...
        "mtaps_enabled": True,
        "mtaps_lags": [1, 2, 4],
        "mtaps_mixer_mode": "hybrid_heads_fixed_scalar_gate",
        "mtaps_aux_fixed_offsets": [2, 16],
    },
    "LLT3F4SG": {
        "read_kernel_mode": "vshape",
        "write_address_mode": "pointer",
...
def _build_meta(surface: str, variant: str, cfg: dict, overrides: dict | None = None) -> dict:
    ...
    meta = {
        ...
        "mtaps_aux_fixed_offsets": list(variant_cfg.get("mtaps_aux_fixed_offsets", [])),
        "seq": cfg["seq"],
        "steps": cfg["steps"],
        "ring_slots": cfg["M"],
...
def _run_small_wikitext_fresh(surface: str, variant: str, cfg: dict, heartbeat_path: Path | None = None) -> dict:
    ...
    result = run_wikitext_fresh(
        ...
        mtaps_mixer_mode=variant_cfg.get("mtaps_mixer_mode", "current"),
        mtaps_aux_fixed_offsets=tuple(variant_cfg.get("mtaps_aux_fixed_offsets", [])),
        context_mode=cfg.get("context_mode", "dotprod"),
        ring_trace=True,
        device=cfg["device"],
