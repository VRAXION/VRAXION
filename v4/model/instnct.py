...
class INSTNCT(nn.Module):
    ...
    def _process_chunk_batched(self, x_chunk, ring_tns, ptr_tns, hidden_tns,
        ...
        # ── Build compile-safe diag tensors (match sequential _process_chunk signature) ──
        ...
        for i in range(N):
            core_diag_expert_tns[i, _COMPILE_DIAG_HIDDEN_FINAL_NORM] = hidden_tns_out[i].detach().norm(dim=-1).mean()
            core_diag_expert_tns[i, _COMPILE_DIAG_PTR_POS] = ptr_tns[i].detach().float().mean()

        return ring_tns, ptr_tns, hidden_tns_out, bb_buf, bb_keys, bb_write_ptr, bb_steps, outs_tns, core_diag_expert_tns, core_diag_global_tns
